...
def prepare_delivery_envelope(package_id: str, *, target_ref: str = "current_task", channel: str = "codex_final_reply", db_path: Path = QUEUE_PATH) -> dict[str, Any]:
    from workflow_review_delivery import prepare_delivery_envelope as owner

    return owner(package_id, target_ref=target_ref, channel=channel, db_path=db_path)
...
def decision_plan(package_id: str, action: str, *, include: list[str] | None = None, exclude: list[str] | None = None, envelope_id: str = "", db_path: Path = QUEUE_PATH, thread_id: str = "", authorization_state_root: Path | str | None = None) -> dict[str, Any]:
    from workflow_review_delivery import decision_plan as owner

    return owner(package_id, action, include=include, exclude=exclude, envelope_id=envelope_id, db_path=db_path, thread_id=thread_id, authorization_state_root=authorization_state_root)


def decision_apply(confirm_token: str, *, authorization_grant_ref: str = "", user_evidence_ref: str = "", note: str = "", db_path: Path = QUEUE_PATH, authorization_state_root: Path | str | None = None) -> dict[str, Any]:
    from workflow_review_delivery import decision_apply as owner

    return owner(confirm_token, authorization_grant_ref=authorization_grant_ref, user_evidence_ref=user_evidence_ref, note=note, db_path=db_path, authorization_state_root=authorization_state_root)
...
def main() -> int:
    ...
    if args.command == "snapshot":
        ...
        payload = prepare_delivery_envelope(args.package_id, target_ref=args.target_ref, channel=args.channel)
