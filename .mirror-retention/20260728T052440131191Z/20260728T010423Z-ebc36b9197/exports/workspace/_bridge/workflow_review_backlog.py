...
def _deterministic_noise_reason(row: dict[str, Any]) -> str:
    """Return a terminal reason only for explicit machine/test noise."""

    source_id = _clean(row["item"].get("source_item_id")).lower()
    if source_id in {"tool-unverified:unverified-1", "tool-negative:negative-1:1"}:
        return "auto_discarded: explicit workflow validation placeholder is not a user decision"
    attributes = row["item"].get("attributes") if isinstance(row["item"].get("attributes"), dict) else {}
    if bool(attributes.get("test_placeholder")):
        return "auto_discarded: producing owner marked the row as a test placeholder"
    return ""
...
def classify(rows: list[dict[str, Any]], *, limit: int = 20) -> dict[str, Any]:
    ...
    noise_dispositions = {
        row["review_id"]: reason
        for row in rows
        if (reason := _deterministic_noise_reason(row))
    }
    machine_dispositions = {**duplicate_dispositions, **evidence_dispositions, **noise_dispositions}
    ...
    return {
        ...
        "deterministic_noise_count": len(noise_dispositions),
        ...
        "contracts": {
            ...
            "test_placeholders_never_require_user_approval": True,
        },
    }
...
def reconcile(
    *,
    ...
    packages = prepare_delivery_packages(db_path=db_path) if operation.get("ok") else {}
    accounted = int(operation.get("discarded_count") or 0) + int(after.get("approval_required_count") or 0)
    return {
        "schema": "workflow_review_backlog.reconcile.v1",
        ...
        "accounting": {
            ...
            "machine_closed": int(operation.get("discarded_count") or 0),
            "approval_required": int(after.get("approval_required_count") or 0),
            "accounted": accounted,
            "complete": accounted == int(before.get("pending_count") or 0),
        },
    }
