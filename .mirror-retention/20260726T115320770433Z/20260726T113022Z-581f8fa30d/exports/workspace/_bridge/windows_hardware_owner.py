...
def doctor(snapshot: dict[str, Any] | None = None) -> dict[str, Any]:
    if os.name != "nt" and snapshot is None:
        return platform_deferred("doctor")
...
def validate(snapshot: dict[str, Any] | None = None) -> dict[str, Any]:
    if os.name != "nt" and snapshot is None:
        return platform_deferred("validate")
    supplied_snapshot = snapshot is not None
    current = snapshot or collect_snapshot()
    text = (SNAPSHOT_QUERY_SCRIPT + DETAIL_QUERY_SCRIPT + _events_script(1, 1)).casefold()
    checks = [
        {"name": "fixed_powershell_or_supplied_snapshot", "ok": supplied_snapshot or POWERSHELL.is_file(), "path": str(POWERSHELL)},
...
def platform_deferred(command: str) -> dict[str, Any]:
    return {
        ...
        "current_platform": "non_windows",
        "target_platform": PLATFORM_SCOPE,
        "owner": "windows_hardware_owner",
        "command": f"python _bridge\\windows_hardware_owner.py {command}",
    }
