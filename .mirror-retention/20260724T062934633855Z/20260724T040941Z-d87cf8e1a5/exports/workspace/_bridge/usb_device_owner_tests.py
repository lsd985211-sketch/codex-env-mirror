...
class UsbDeviceOwnerTests(unittest.TestCase):
    def test_storage_binding_exposes_stable_identity_and_safe_handoff(self) -> None:
        raw = {
            "captured_at": "now",
            "disk": {
                "friendly_name": "USB Disk",
                "model": "USB Disk",
                "serial_number": "SERIAL",
                "pnp_device_id": "USBSTOR\\DISK&VEN_TEST",
                "bus_type": "USB",
            ...
            "partition": {"partition_number": 1, "drive_letter": "D", "size_bytes": 900},
            "volume": {"health_status": "Healthy", "operational_status": ["OK"]},
        }
        with patch.object(owner, "_run_powershell", return_value=raw) as run:
            result = owner.storage_binding("d:")

        self.assertTrue(result["ok"])
        self.assertTrue(result["safe_for_content_mutation"])
        self.assertEqual(result["drive_letter"], "D")
        self.assertEqual(len(result["stable_fingerprint"]), 64)
        self.assertNotIn("D", result["stable_fingerprint"])
        self.assertIn("$drive='D'", run.call_args.args[0])

    def test_storage_binding_blocks_read_only_or_unhealthy_media(self) -> None:
        raw = {
            "disk": {
                "bus_type": "USB",
                "operational_status": ["Online"],
            ...
            "partition": {"partition_number": 1, "drive_letter": "E", "size_bytes": 900},
            "volume": {"health_status": "Healthy", "operational_status": ["OK"]},
        }
        with patch.object(owner, "_run_powershell", return_value=raw):
            result = owner.storage_binding("E")

        self.assertFalse(result["safe_for_content_mutation"])
        self.assertEqual({item["code"] for item in result["issues"]}, {"storage_read_only", "storage_unhealthy"})

    def test_storage_fingerprint_does_not_depend_on_drive_letter(self) -> None:
        disk = {"friendly_name": "USB", "serial_number": "S", "size_bytes": 10}
        first = owner.storage_identity_fingerprint(disk, {"partition_number": 1, "size_bytes": 9, "drive_letter": "D"})
        second = owner.storage_identity_fingerprint(disk, {"partition_number": 1, "size_bytes": 9, "drive_letter": "Z"})

        self.assertEqual(first, second)

    def test_storage_binding_rejects_non_letter_input_before_powershell(self) -> None:
        with patch.object(owner, "_run_powershell") as run:
            with self.assertRaisesRegex(ValueError, "drive_letter"):
                owner.storage_binding("D; Remove-Item")
    ...
    def test_transport_normalizes_windows_shared_and_wsl_attached_states(self) -> None:
        listing = """Connected:
BUSID  VID:PID    DEVICE                              STATE
2-1    22d9:2764  OPPO Reno4 SE 5G                    Not shared
2-2    12d1:107e  HONOR 20S, USB Mass Storage Device  Shared
2-3    12d1:107f  HONOR 20S, USB Mass Storage Device  Attached
"""
        with (
            patch.object(owner.os, "name", "nt"),
            patch.object(owner, "usbipd_path", return_value=Path("C:/usbipd.exe")),
            patch.object(Path, "is_file", return_value=True),
            patch.object(owner, "_run_fixed", return_value=listing) as run,
        ):
            result = owner.usbip_transport()

        self.assertTrue(result["ok"])
        self.assertEqual([row["transport_state"] for row in result["devices"]], ["windows_available", "shared_waiting_for_wsl", "wsl_attached"])
        self.assertEqual(result["devices"][2]["windows_mtp_access"], "unavailable_while_attached")
        self.assertEqual(run.call_args.args[0], ["C:\\usbipd.exe", "list"])

    def test_transport_rejects_unbounded_busid_without_running_usbipd(self) -> None:
        with patch.object(owner, "_run_fixed") as run:
            with self.assertRaisesRegex(ValueError, "busid"):
                owner.usbip_transport(busid="2-2; attach")
        run.assert_not_called()

    def test_transport_filters_exact_busid_and_reports_missing_usbipd(self) -> None:
        listing = "2-2    12d1:107e  HONOR 20S  Attached\n"
        with (
            patch.object(owner, "usbipd_path", return_value=Path("C:/usbipd.exe")),
            patch.object(Path, "is_file", return_value=True),
            patch.object(owner, "_run_fixed", return_value=listing),
        ):
            result = owner.usbip_transport(busid="2-2")
        self.assertEqual(result["device_count"], 1)
        self.assertEqual(result["devices"][0]["busid"], "2-2")

        with patch.object(owner, "usbipd_path", return_value=Path("C:/usbipd.exe")), patch.object(Path, "is_file", return_value=False):
            missing = owner.usbip_transport()
        self.assertFalse(missing["ok"])
        self.assertEqual(missing["reason"], "usbipd_win_not_installed")
    ...
    def test_validate_defers_when_called_from_wsl(self) -> None:
        with patch.object(owner.os, "name", "posix"):
            result = owner.validate()

        self.assertTrue(result["ok"])
        self.assertTrue(result["deferred"])
        self.assertEqual(result["platform_scope"], "windows_host")
        self.assertEqual(result["owner_status"], "deferred_to_platform_owner")
        self.assertEqual(result["diagnostics"]["reason"], "owner_deferred_platform_scope")
