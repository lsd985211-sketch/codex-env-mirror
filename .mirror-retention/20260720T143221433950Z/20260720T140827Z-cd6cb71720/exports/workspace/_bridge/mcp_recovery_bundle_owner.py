...
def import_release(archive_root: Path, *, repo: str, tag: str, gh_command: str = "gh") -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="mcp-release-") as temporary:
        ...
        for asset in release_index.get("assets", []):
            ...
            name = str(asset["name"])
            command = [gh_command, "release", "download", tag, "--repo", repo, "--pattern", name, "--dir", str(staging), "--clobber"]
            try:
            ...
            entry = {key: asset.get(key) for key in ("id", "name", "sha256", "size_bytes", "platform", "entrypoints")}
            entry["archive"] = name
            entry["source_fingerprint"] = "remote_release"
