...
- Confirm the article needs illustration and that the source structure is available.
- Decide whether the output is per-article or per-section before prompting.
- Load preferences before asking style questions.

## Output Contract

- Return the outline state and the generated image set.
- State whether the article text was preserved verbatim in the prompt workflow.
- Mention if any illustration position was skipped or merged.
