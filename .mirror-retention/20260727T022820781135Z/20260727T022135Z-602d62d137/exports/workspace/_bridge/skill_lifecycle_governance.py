...
try:
    ...
    from _bridge.platform_paths import host_accessible_path, resource_library_root, windows_user_root
    ...
    from platform_paths import host_accessible_path, resource_library_root, windows_user_root  # type: ignore[no-redef]

ROOT = Path(__file__).resolve().parents[1]
BRIDGE = ROOT / "_bridge"
GLOBAL_SKILLS = Path.home() / ".codex" / "skills"
SYSTEM_SKILLS = GLOBAL_SKILLS / ".system"
CODEX_RUNTIME_HOME = next(
    (candidate for candidate in (Path.home() / ".codex-app", Path.home() / ".codex") if (candidate / "config.toml").is_file()),
    Path.home() / ".codex",
)
CODEX_CONFIG_PATH = CODEX_RUNTIME_HOME / "config.toml"
PLUGIN_CACHE = CODEX_RUNTIME_HOME / "plugins" / "cache"
WORK_GIT_SKILLS = ROOT.parent / "codex-home" / "skills"
RUNTIME_SKILLS = CODEX_RUNTIME_HOME / "skills"
WORKSPACE_SKILLS = ROOT / ".codex" / "skills"
CANONICAL_SKILLS_ROOT = windows_user_root() / ".codex" / "skills"
RESOURCE_BACKUP_ROOT = resource_library_root() / "_backup" / "skills"
MYSKILLS_DB = windows_user_root() / "AppData" / "Roaming" / "com.kanbenzhi.myskills" / "myskills.db"
...
def work_git_projection_coverage() -> dict[str, Any]:
    """Verify every declarative skill is exposed by both governed runtime roots."""
    targets = {"canonical": GLOBAL_SKILLS, "runtime": RUNTIME_SKILLS}
    rows: list[dict[str, Any]] = []
    if not WORK_GIT_SKILLS.is_dir():
        return {
            "schema": "skill_lifecycle.work_git_projection_coverage.v1",
            "ok": False,
            "reason": "work_git_skills_missing",
            "source": str(WORK_GIT_SKILLS),
            "rows": [],
        }
    for source in sorted(WORK_GIT_SKILLS.iterdir(), key=lambda item: item.name):
        if source.name.startswith(".") or not source.is_dir():
            continue
        for target_name, target_root in targets.items():
            projected = target_root / source.name
            resolved = projected.resolve() if projected.is_symlink() else projected
            managed_target = bool(
                projected.is_symlink()
                and resolved.name == source.name
                and resolved.parent.name == "skills"
                and resolved.parent.parent.name == "codex-home"
            )
            source_contract = source / "SKILL.md"
            projected_contract = projected / "SKILL.md"
            contract_current = bool(
                source_contract.is_file()
                and projected_contract.is_file()
                and file_hash(source_contract) == file_hash(projected_contract)
            )
            linked = managed_target and contract_current
            rows.append({
                "skill": source.name,
                "target": target_name,
                "path": str(projected),
                ...
                "resolved_target": str(resolved) if projected.is_symlink() else "",
                "status": "linked" if linked else ("conflict" if projected.exists() or projected.is_symlink() else "missing"),
            })
    issues = [row for row in rows if not row["linked"]]
    return {
        "schema": "skill_lifecycle.work_git_projection_coverage.v1",
        "ok": not issues,
        "source": str(WORK_GIT_SKILLS),
        "target_roots": {key: str(value) for key, value in targets.items()},
        "skill_count": len({row["skill"] for row in rows}),
        "row_count": len(rows),
        "issue_count": len(issues),
        "issues": issues[:20],
        "repair_owner": "wsl_codex_runtime.py skills-apply",
    }
...
def skill_record(path: Path, source: str, text: str = "") -> dict[str, Any]:
    ...
    platform_scope = str(((metadata.get("codex") or {}).get("platform_scope")) or "").strip().casefold()
    ...
    if re.search(r"(?m)^\s*(python3|pip3)\s+", body) and platform_scope not in {"wsl", "wsl_work_git"}:
...
def canonical_platform_snapshot() -> dict[str, Any]:
    ...
    expected_path = str(host_accessible_path(CANONICAL_SKILLS_ROOT, platform_name="nt"))
    ...
    try:
        ...
        global_platform = connection.execute(
            "SELECT id FROM platforms WHERE lower(skills_dir)=lower(?)", (expected_path,)
        ).fetchone()
        orphaned_skills = [
...
def audit() -> dict[str, Any]:
    ...
    plugin_catalog = skill_active_catalog.catalog_snapshot(plugin_cache=PLUGIN_CACHE, config_path=CODEX_CONFIG_PATH)
...
def validate() -> dict[str, Any]:
    report = audit()
    projection_coverage = work_git_projection_coverage()
    checks = [
        ...
        {"name": "configured_plugin_skill_catalog_is_readable", "ok": report.get("plugin_catalog", {}).get("ok") is True, "detail": report.get("plugin_catalog", {})},
        ...
        {"name": "work_git_skills_projected_to_runtime_and_canonical_roots", "ok": projection_coverage.get("ok") is True, "detail": projection_coverage},
    ]
    return {
