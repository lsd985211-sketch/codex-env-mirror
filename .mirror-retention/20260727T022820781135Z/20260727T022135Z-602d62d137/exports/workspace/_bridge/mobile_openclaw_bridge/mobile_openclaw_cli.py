...
from simple_check_handlers import NO_ARG_CHECK_FUNCTIONS, build_simple_check_command_handlers  # noqa: E402
...
def load_config(path: Path) -> dict[str, Any]:
    ...
    if db_path:
        runtime_db_path = host_accessible_path(db_path)
        if not runtime_db_path.is_absolute():
            runtime_db_path = ROOT / runtime_db_path
        config.setdefault("queue", {})["db_path"] = str(runtime_db_path)
...
def active_system_maintenance_lock() -> dict[str, Any] | None:
    lock_path = resource_library_root() / "文档" / "系统维护" / "运行态" / "performance-maintenance.lock"
...
def gui_ocr_gpu_probe(config: dict[str, Any]) -> dict[str, Any]:
    ...
    gui = gui_automation_health(config, mcp_config)
    ocr = gui.get("ocr") if isinstance(gui.get("ocr"), dict) else {}
    gpu_python, default_cpu_python = gui_ocr_python_paths()
    cpu_python = Path(str(ocr.get("fallback_python") or ocr.get("python") or default_cpu_python))
...
def reply_to_weixin(
    ...
    node = str(openclaw.get("node_path") or openclaw_node_path())
    state_dir = Path(
...
def run_queue_independent_command(args: argparse.Namespace) -> int | None:
    """Dispatch owner commands that must not initialize the mobile queue."""
    handlers: dict[str, Callable[[argparse.Namespace], dict[str, Any]]] = {
        "backup-hygiene": run_backup_hygiene_command,
        "backup-router": run_backup_router_command,
        "resource-process": run_resource_process_command,
        "resource-layer-smoke-check": lambda _args: resource_layer_smoke_check(),
        "mcp-session": run_mcp_session_command,
        "performance": run_performance_command,
    }
    handler = handlers.get(str(args.cmd or ""))
    if handler is None:
        return None
...
def main(argv: list[str] | None = None) -> int:
    ...
    no_arg_check = dict(NO_ARG_CHECK_FUNCTIONS).get(str(args.cmd or ""))
    if no_arg_check:
        payload = globals()[no_arg_check]()
        print_json(payload)
        return 0 if payload.get("ok") else 1
    queue_independent_result = run_queue_independent_command(args)
    if queue_independent_result is not None:
        return queue_independent_result
    queue = queue_from_config(config)
