...
def _atomic_install_target(target_dir: Path, package_spec: str, package: dict[str, Any], timeout: int) -> tuple[dict[str, Any], Path]:
    ...
    return {"ok": True, "returncode": proc.returncode, "stdout": proc.stdout or "", "stderr": proc.stderr or ""}, staging_dir
...
def execute_python_package_install(
    ...
    install, _staging_dir = _atomic_install_target(target_dir, package_spec, package, timeout)
    if not install.get("ok"):
        return json_result(
            ...
            writes_files=True,
            error_class="python_package_install_timeout" if install.get("returncode") is None else "python_package_install_failed",
            reason=str(install.get("stderr") or install.get("stdout") or "python package install failed")[-1000:],
            metadata={"package": package_name, "package_spec": package_spec, "target_dir": str(target_dir), "will_install": True, "runtime": runtime_identity(), "atomic_replace": True},
            next_action="retry_with_larger_timeout_or_refine_network_route" if install.get("returncode") is None else "inspect_pip_output_and_retry_with_correct_package_spec",
        )
