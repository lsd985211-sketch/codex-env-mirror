...
function Ensure-DesktopAdminShortcut {
    ...
    $runHidden = Join-Path $env:USERPROFILE ".codex\scripts\run-hidden.vbs"
    ...
    $manualLaunchCommand = "`$env:CODEX_ALLOW_STALE_CODEX_CLEANUP = '1'; & '$escapedStartScript'; exit `$LASTEXITCODE"
    $arguments = "//B //Nologo `"$runHidden`" `"$powershell`" -NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -Command `"$manualLaunchCommand`""
...
function Ensure-CodexDesktopScheduledTaskHiddenWrapper {
    ...
    $runHidden = Join-Path $env:USERPROFILE ".codex\scripts\run-hidden-wait.vbs"
    if (-not (Test-Path -LiteralPath $runHidden)) {
