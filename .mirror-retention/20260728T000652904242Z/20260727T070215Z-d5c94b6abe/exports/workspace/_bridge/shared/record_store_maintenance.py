...
def validation_snapshot() -> dict[str, Any]:
    """Return root availability metadata without traversing record contents."""

    generated = now_utc()
    roots = [empty_root_summary(root) | {"scan_mode": "metadata_only"} for root in RECORD_ROOTS]
    return {
        "schema": "record-store.validation_snapshot.v1",
        "ok": True,
        "generated_at": generated.isoformat(),
        "resource_root": str(RESOURCE_ROOT),
        "policy": {
            "default_scan": "metadata_only",
            "deep_scan_entrypoints": ["snapshot", "doctor", "repair-plan", "archive"],
            "mutation": "none",
        },
        "summary": {
            "root_count": len(roots),
            ...
            "cold_candidate_bytes": 0,
            "cold_candidate_mb": 0.0,
        },
        "roots": roots,
    }
...
def archive_plan(
    *,
    ...
    del snap
    candidates = cold_archive_candidates(now_utc()) if scan_candidates else []
    ...
    return {
        ...
        "generated_at": now_iso(),
        "apply": apply,
        "scan_mode": "full" if scan_candidates else "contract_only",
...
def inspect_index() -> dict[str, Any]:
    info: dict[str, Any] = {
        ...
        "record_count": None,
        "fts_exists": False,
        "records_query_ok": False,
        "resource_query_ok": False,
        "ok": False,
        "error": "",
    }
    ...
    try:
        ...
        try:
            ...
            conn.execute("SELECT 1 FROM records ORDER BY created_at DESC LIMIT 1").fetchone()
            conn.execute(
                "SELECT 1 FROM records WHERE area = 'resource_layer' AND kind = 'resource_request_manifest' LIMIT 1"
            ).fetchone()
        finally:
            conn.close()
        info["record_count"] = int(count)
        info["fts_exists"] = bool(fts_exists)
        info["records_query_ok"] = True
        info["resource_query_ok"] = True
        info["ok"] = True
    except Exception as exc:  # pragma: no cover - defensive diagnostic path
...
def metrics(
    ...
    *,
    ...
    archive: dict[str, Any] | None = None,
    scheduler_tasks: dict[str, Any] | None = None,
) -> dict[str, Any]:
    snap = snap or snapshot()
    ...
    if scheduler_tasks is None:
        scheduler_tasks = ensure_scheduler_tasks(apply=False)
    ...
    return {
        ...
        "archive_scan_mode": str(archive.get("scan_mode") or "full"),
        "apply_supported": {
            "index": True,
            "ensure_tasks": True,
            "archive": True,
        ...
        "scheduler_tasks_installed": all(
            ...
            for action in scheduler_tasks.get("actions", [])
        ),
    }


def validate() -> dict[str, Any]:
    snap = validation_snapshot()
    index_info = inspect_index()
    scheduler_tasks = ensure_scheduler_tasks(apply=False)
    archive = archive_plan(snap=snap, apply=False, scan_candidates=False)
    plan = repair_plan(snap, archive=archive, index_info=index_info)
    issue_data = doctor(snap, index_info=index_info)
    ...
    checks = [
        ...
        },
        {
            ...
            "ok": not index_info.get("exists") or bool(index_info.get("records_query_ok")),
            "detail": str(INDEX_PATH),
        },
        {
            "name": "resource_query_uses_existing_index",
            "ok": not index_info.get("exists") or bool(index_info.get("resource_query_ok")),
        ...
        },
        {
            ...
            "ok": bool(scheduler_tasks.get("actions")),
        ...
        },
        {
            ...
            "ok": metric_payload.get("record_write_contract", {}).get("default") == "summary_metadata_raw_ref",
