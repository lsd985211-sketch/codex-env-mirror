...
# The Windows Desktop host owns this profile even when its agent runtime is WSL.
...
if ([bool]$finalConfigPreflight.WslEnabled -and -not [bool]$finalConfigPreflight.WslRuntimeReady) {
    Write-StartLog "WSL runtime projection is not ready after the clean restart boundary. Status=$($finalConfigPreflight.WslProjectionStatus). Continuing with the native compatibility launch to preserve Codex availability; WSL mode can be retried on the next governed restart."
}
