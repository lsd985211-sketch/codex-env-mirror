...
ROUTE_DEFINITION_REVISION = "2026-07-24.headroom-context-compression.v1"
...
MANUAL_ROUTES: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "terms": ["gitnexus", "semantic code", "hybrid code search", "execution flow", "symbol context", "cross repository code", "cypher code graph"],
        "native_mcp": "gitnexus",
        "hub_route": "gitnexus.list_tools|gitnexus.call through the Local MCP Hub; use hub.call with the Hub acknowledgement when the active turn has not refreshed the newly exposed aliases",
        "local_fallback": "validate the Local MCP Hub and the pinned GitNexus package first; use CodeGraph for ordinary source structure, then targeted rg/read only after the same-boundary graph route is unavailable or insufficient",
        "permission_boundary": "read_only_work_git_code_graph_query",
        "validation_command": "python _bridge\\local_mcp_hub.py validate; python _bridge\\mcp_capability_routes.py lookup --terms gitnexus semantic code; python _bridge\\mcp_session_doctor.py validate",
        "current_turn_evidence_rule": "Hub GitNexus list_tools or call evidence must identify a read-only upstream tool and Work Git scope; use hub.call only as the same-Hub dynamic wrapper, never as a permission expansion",
        "usage_policy": "use_for_semantic_or_hybrid_code_flow_search_symbol_context_trace_impact_diff_analysis_and_cross_repository_groups; use CodeGraph instead for ordinary narrow symbol callers callees and source blocks",
        "fallback_order": "hub_gitnexus_list_tools_then_hub_gitnexus_call_then_local_hub_validation_then_codegraph_then_targeted_rg",
    },
    {
        "capability": "graphify_knowledge_graph",
        "terms": ["graphify", "mixed artifact graph", "cross artifact graph", "graph guided review", "review delta", "review analysis", "managed graph json"],
        "native_mcp": "graphify",
        "hub_route": "graphify.list_tools|graphify.call through the Local MCP Hub; use hub.call with the Hub acknowledgement when the active turn has not refreshed the newly exposed aliases",
        "local_fallback": "validate the Local MCP Hub and the managed graph path first; use CodeGraph or GitNexus for source-only questions, then targeted rg/read only after the same-boundary graph route is unavailable or insufficient",
        "permission_boundary": "read_only_managed_graphify_state_query",
        "validation_command": "python _bridge\\local_mcp_hub.py validate; python _bridge\\mcp_capability_routes.py lookup --terms graphify review delta; python _bridge\\mcp_session_doctor.py validate",
        "current_turn_evidence_rule": "Hub Graphify list_tools or call evidence must identify a managed graph.json and a fixed allowlisted query tool; it never authorizes extraction, graph creation, hooks, watches, or external models",
        "usage_policy": "use_for_managed_mixed_artifact_graphs_graph_guided_review_review_delta_review_analysis_and_persistent_cross_artifact_exploration; use CodeGraph or GitNexus for source-only questions",
        "fallback_order": "hub_graphify_list_tools_then_hub_graphify_call_then_local_hub_validation_then_source_graph_then_targeted_rg",
    },
    {
        "capability": "context_compression",
        "terms": ["headroom", "context compression", "reversible compression", "large JSON", "large logs", "large tool output", "compress retrieve", "上下文压缩", "大型日志", "大型 JSON"],
        "native_mcp": "headroom",
        "hub_route": "headroom.compress|headroom.retrieve|headroom.stats through the Local MCP Hub",
        "local_fallback": "python _bridge\\headroom_runtime.py validate; keep the original owner artifact or bounded output when Headroom is unavailable",
        "permission_boundary": "ttl_bound_reversible_context_cache_no_provider_or_long_term_memory_changes",
        "validation_command": "python _bridge\\headroom_runtime.py validate; python _bridge\\local_mcp_hub.py validate; real compress then retrieve exact-match smoke",
        "current_turn_evidence_rule": "use Headroom only when a large payload would consume material context; preserve the original owner receipt or stable artifact reference as authority and retrieve omitted detail only when needed",
        "usage_policy": "use_for_large_json_logs_code_search_results_and_tool_outputs; do_not_compress_decisions_gates_permissions_failure_causes_or_only_copies_of_owner_evidence",
        "fallback_order": "hub_headroom_compress_then_local_headroom_owner_validation_then_original_bounded_artifact",
    },
    {
...
def validate() -> dict[str, Any]:
    ...
    for capability, expected_tool, usage_marker in (
        ("gitnexus_semantic_graph", "gitnexus.call", "semantic_or_hybrid_code_flow_search"),
        ("graphify_knowledge_graph", "graphify.call", "managed_mixed_artifact_graphs"),
    ):
        graph_route = next((route for route in payload["routes"] if route.get("capability") == capability), {})
        graph_chain_ids = [str(step.get("id") or "") for step in graph_route.get("fallback_chain", []) if isinstance(step, dict)]
        if not graph_route:
            issues.append({"severity": "risk", "code": "graph_capability_route_missing", "capability": capability})
            continue
        if graph_route.get("execution_affinity") != "hub_first" or graph_route.get("required_first_step") != "hub_mcp_direct":
            issues.append({"severity": "risk", "code": "graph_capability_not_hub_first", "capability": capability})
        if expected_tool not in (graph_route.get("direct_hub_tools") or []):
            issues.append({"severity": "risk", "code": "graph_capability_direct_hub_tool_missing", "capability": capability, "tool": expected_tool})
        if usage_marker not in str(graph_route.get("usage_policy") or ""):
            issues.append({"severity": "risk", "code": "graph_capability_usage_policy_missing", "capability": capability})
        if "terminal_local_read" not in graph_chain_ids or graph_chain_ids.index("terminal_local_read") <= graph_chain_ids.index("hub_mcp_direct"):
            issues.append({"severity": "risk", "code": "graph_capability_terminal_read_before_hub", "capability": capability})
