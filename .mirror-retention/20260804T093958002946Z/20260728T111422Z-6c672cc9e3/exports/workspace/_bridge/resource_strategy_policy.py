...
def _result_source_identities(result: dict[str, Any]) -> tuple[str, ...]:
    ...
    for candidate in candidates:
        if isinstance(candidate, dict):
            for key in ("source_id", "url", "landing_url"):
...
def owner_result_sufficiency(
    *,
    ...
    if tool in {"context7", "microsoftdocs", "openai-docs", "github", "playwright", "chrome-devtools", "generic_search"}:
...
def _generic_search_content_text(result: dict[str, Any]) -> str:
    """Extract evidence fields without counting the echoed search request."""
    candidates: list[Any] = []
    ...
    for candidate in candidates:
        if not isinstance(candidate, dict):
            continue
        for key in ("title", "summary", "description", "snippet", "body", "url", "landing_url", "source_id", "source"):
            value = str(candidate.get(key) or "").strip()
            if value:
                values.append(value)
    if not values:
        direct_content = str(result.get("content") or "").strip()
        if direct_content and not direct_content.lstrip().startswith(("{", "[")):
            values.append(direct_content)
    return "\n".join(values).lower()
...
def _structured_delivery_acceptance(
    request: dict[str, Any],
    result: dict[str, Any],
    ...
    acceptance = execution.get("acceptance", {}) if isinstance(execution, dict) else {}
    requested = execution.get("deliverables") if isinstance(execution.get("deliverables"), list) else []
    required = acceptance.get("required_deliverables") if isinstance(acceptance.get("required_deliverables"), list) else []
    required = list(dict.fromkeys(str(item).strip().lower() for item in (required or requested) if str(item).strip()))
    result_metadata = result.get("metadata") if isinstance(result.get("metadata"), dict) else {}
    completed = list(
        dict.fromkeys(
            str(item).strip().lower()
            for item in result_metadata.get("completed_deliverables") or []
            if str(item).strip()
        )
    )
    ...
    required_phases = acceptance.get("required_phases") if isinstance(acceptance.get("required_phases"), list) else []
    ...
    phase_results = result_metadata.get("phase_results") if isinstance(result_metadata.get("phase_results"), list) else []
    completed_phases = list(
        dict.fromkeys(
            str(value).strip()
            for row in phase_results
            if isinstance(row, dict) and row.get("ok")
            for value in (row.get("phase_id"), row.get("operation"))
            if str(value or "").strip()
        )
    )
    missing_phases = [str(item) for item in required_phases if str(item) not in completed_phases]
    minimum_completed = int(acceptance.get("minimum_completed_phases") or 0)
    if minimum_completed and len(completed_phases) < minimum_completed:
        missing_phases.append(f"minimum_completed_phases:{minimum_completed}")
    allow_partial = bool(acceptance.get("allow_partial"))
    complete = not missing and not missing_phases
    partial = allow_partial and bool(completed or completed_phases)
    return complete or partial, completed, missing, completed_phases, missing_phases
...
def _owner_no_result_reason(*, tool: str, content: str) -> str:
    ...
    if tool == "microsoftdocs" and any(marker in content for marker in ("no results found", "0 results")):
        return "owner_no_results"
    if tool == "openai-docs" and any(marker in content for marker in ("no results found", "0 results", "no matching openai docs")):
        return "owner_no_results"
...
def owner_result_relevance(
    *,
    ...
    if tool not in {"context7", "microsoftdocs", "openai-docs", "github", "package_manager", "markitdown", "playwright", "chrome-devtools", "generic_search"}:
        return RelevanceDecision(True, 1.0, threshold or 0.0, (), (), "relevance_not_required_for_tool")
    metadata = request.get("metadata") if isinstance(request.get("metadata"), dict) else {}
    custom = metadata.get("custom_delegation") if isinstance(metadata.get("custom_delegation"), dict) else {}
    if threshold is None:
        raw_threshold = (
            custom["relevance_threshold"]
            if "relevance_threshold" in custom
            else metadata.get("relevance_threshold")
        )
        threshold = _float_value(raw_threshold, 0.25) if raw_threshold not in (None, "") else None
    terms = request_key_terms(request)
    if not terms:
        return RelevanceDecision(True, 1.0, threshold or 0.0, (), (), "no_specific_terms_required")
    content = _generic_search_content_text(result) if tool == "generic_search" else _content_text(result)
    no_result_reason = _owner_no_result_reason(tool=tool, content=content)
    if no_result_reason:
        return RelevanceDecision(False, 0.0, threshold if threshold is not None else 0.25, (), terms[:8], no_result_reason)
    ...
    required_threshold = 0.25 if threshold is None else max(0.0, min(1.0, threshold))
    if tool == "generic_search" and request_requires_multi_source_research(request):
        required_threshold = max(required_threshold, 0.5)
    if tool in {"context7", "microsoftdocs", "openai-docs"}:
...
def resource_result_satisfaction(*, request: dict[str, Any], tool: str, result: dict[str, Any]) -> ResourceSatisfactionDecision:
    ...
    acceptance_contract = build_acceptance_contract(request)
    contract_decision = evaluate_acceptance(acceptance_contract, result)
    if acceptance_contract.get("explicit") and not contract_decision.get("accepted"):
        return ResourceSatisfactionDecision(
            False,
            "metadata",
            str(contract_decision.get("reason") or "acceptance_contract_not_satisfied"),
            str(contract_decision.get("next_action") or "try_next_route"),
            {"acceptance_contract": acceptance_contract, "acceptance_decision": contract_decision},
            {},
        )
    ...
    delivery_ok, completed_deliverables, missing_deliverables, completed_phases, missing_phases = _structured_delivery_acceptance(request, result)
    if not delivery_ok:
        return ResourceSatisfactionDecision(
            False,
            "metadata",
            "required_deliverables_not_met",
            "refine_resource_delegation_and_retry",
            {
    ...
    relevance_payload = relevance.to_dict()
    relevance_payload["acceptance_contract"] = acceptance_contract
    relevance_payload["acceptance_decision"] = contract_decision
    return ResourceSatisfactionDecision(True, result_kind, "completion_predicate_satisfied", "consume_resource", relevance_payload, sufficiency.to_dict())
