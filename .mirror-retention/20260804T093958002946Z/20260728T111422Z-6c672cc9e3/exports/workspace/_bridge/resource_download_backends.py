    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            **asdict(self),
            ...
            "yt_dlp_available": bool(self.yt_dlp_path),
...
def _tool_candidates(name: str) -> list[tuple[str, str]]:
    current = shutil.which(name) or ""
    user = windows_user_root()
    known: dict[str, list[Path]] = {
        "curl": [Path("/mnt/c/Windows/System32/curl.exe")],
        "aria2c": [Path("/mnt/c/ProgramData/chocolatey/bin/aria2c.exe")],
        "yt-dlp": [user / "AppData/Roaming/Python/Python314/Scripts/yt-dlp.exe"],
        "ffmpeg": [user / "AppData/Local/Microsoft/WinGet/Links/ffmpeg.exe"],
    }
    result = [(current, "current_runtime")] if current else []
    result.extend((str(path), "windows_host") for path in known.get(name, []) if path.exists() and str(path) != current)
    return result
...
def availability() -> BackendAvailability:
    ...
    for name in ("curl", "aria2c", "yt-dlp", "ffmpeg"):
        candidates = _tool_candidates(name)
        current = next((path for path, platform in candidates if platform == "current_runtime"), "")
        host = next((path for path, platform in candidates if platform == "windows_host"), "")
        selected[name] = current or host
        evidence[name] = {
            "installed": bool(candidates),
            "callable": bool(selected[name]),
            "platform": "current_runtime" if current else ("windows_host" if host else "unavailable"),
            "path": selected[name],
            "candidates": [{"path": path, "platform": platform} for path, platform in candidates],
            "validation": "path_exists_or_runtime_which",
        }
    return BackendAvailability(
        curl_path=selected["curl"],
        aria2c_path=selected["aria2c"],
        yt_dlp_path=selected["yt-dlp"],
        ffmpeg_path=selected["ffmpeg"],
        tools=evidence,
    )
