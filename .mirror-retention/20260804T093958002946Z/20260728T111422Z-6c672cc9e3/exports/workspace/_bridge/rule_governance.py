...
RUNTIME_ENFORCEMENT_PROBES = {
    ...
    "workflow_route": {
        "script": "workflow_rule_routing_tests.py",
        "args": (),
        "rules": (
            "workflow.route_plan",
    ...
    "workflow_execution_economy": {
        "script": "workflow_automation_delegation_tests.py",
        "args": (),
        "rules": ("workflow.execution_economy",),
    },
    "workflow_scoped_authorization": {
        "script": "scoped_authorization_tests.py",
        "args": (),
        "rules": ("workflow.scoped_authorization",),
    },
    "workflow_terminal_convergence": {
        "script": "workflow_terminal_convergence_tests.py",
        "args": (),
        "rules": ("workflow.terminal_convergence",),
    },
...
def pattern_matches(path: str, pattern: str) -> bool:
    ...
    legacy = LEGACY_CODEX_HOME.casefold()
    if normalized_pattern.casefold() == legacy or normalized_pattern.casefold().startswith(legacy + "/"):
        expected_suffix = normalized_pattern[len(LEGACY_CODEX_HOME):].lstrip("/")
        for declarative_prefix in ("worktree:codex-home/", "codex-home/"):
            if candidate.casefold().startswith(declarative_prefix):
                candidate_suffix = candidate[len(declarative_prefix):]
                return fnmatch.fnmatchcase(candidate_suffix.casefold(), expected_suffix.casefold())
...
def discover_rule_surfaces() -> list[dict[str, Any]]:
    ...
    candidates.update(
        ...
        for path in (
            ...
            ROOT / "_bridge" / "workflow_terminal_convergence.py",
    ...
    candidates.update(
        path
        for path in (ROOT / "_bridge" / "docs" / "maintenance_surfaces").glob("*.md")
        if path.is_file()
    )
    return [
        {
