...
def annotate_candidate(candidate: dict[str, Any], *, resource_kind: str, constraints: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    vector = {
        "entity": max(0.0, min(1.0, raw_score)),
        "authority": source_trust,
        "freshness": max(0.0, min(1.0, float(item.get("freshness_score") or 0.5))),
        "completeness": max(0.0, min(1.0, float(item.get("completeness_score") or raw_score))),
        "format": 1.0 if format_status == "matched" else (0.5 if format_status == "unknown" else 0.0),
        "license": 1.0 if license_status == "open" else (0.5 if license_status in {"declared", "unknown"} else 0.0),
        "reachability": max(0.0, min(1.0, float(item.get("reachability_score") or 0.5))),
        "cost": max(0.0, min(1.0, float(item.get("cost_score") or 0.5))),
        "materializability": 1.0 if item.get("direct_url") or estimated_size > 0 else 0.5,
        "novelty": max(0.0, min(1.0, float(item.get("novelty_score") or 0.5))),
        "coverage_contribution": max(0.0, min(1.0, float(item.get("coverage_contribution") or 0.5))),
    }
    weights = {
        "entity": 0.32, "authority": 0.18, "freshness": 0.06,
        "completeness": 0.10, "format": 0.08, "license": 0.05,
        "reachability": 0.05, "cost": 0.03, "materializability": 0.05,
        "novelty": 0.03, "coverage_contribution": 0.05,
    }
    quality_score = sum(vector[key] * weights[key] for key in weights)
    ...
    item.update(
        {
            ...
            "quality_vector": {key: round(value, 3) for key, value in vector.items()},
