    ...
    *,
    ...
    try:
        ...
        scope = _authorization().build_scope(
            ...
            phase="decision_apply",
            source_signature=hashlib.sha256(
                json.dumps({"package_id": package_id, "action": action, "item_refs": selected}, sort_keys=True, separators=(",", ":")).encode("utf-8")
            ).hexdigest(),
            requested_by_owner="workflow_review_delivery",
        )
        ...
        conn.execute(
            "INSERT INTO review_decisions(decision_id,confirm_token,package_id,envelope_id,action,item_refs_json,status,created_at,authorization_thread_id,authorization_challenge_ref) VALUES(?,?,?,?,?,?, 'planned', ?,?,?)",
            (decision_id, token, package_id, envelope_id, action, json.dumps(selected, sort_keys=True), queue.now_iso(), authorization_thread_id, challenge["challenge_ref"]),
        )
        conn.commit()
