...
from platform_paths import windows_user_root
...
def resolve_verge_home() -> Path:
    """Resolve the active Clash Verge Rev data root on Windows or WSL."""

    explicit = str(os.environ.get("CLASH_VERGE_HOME") or "").strip()
    if explicit:
        return Path(explicit).expanduser()
    appdata = str(os.environ.get("APPDATA") or "").strip()
    candidates: list[Path] = []
    if appdata:
        candidates.append(Path(appdata) / "io.github.clash-verge-rev.clash-verge-rev")
    candidates.append(windows_user_root() / "AppData" / "Roaming" / "io.github.clash-verge-rev.clash-verge-rev")
...
DEFAULT_VERGE_HOME = resolve_verge_home()
...
def verge_profile_snapshot(home: Path | None = None) -> dict[str, Any]:
    """Return a secret-free view of current and alternate Verge profiles."""

    root = home or resolve_verge_home()
    profiles_path = root / "profiles.yaml"
    data: dict[str, Any] = {}
    if profiles_path.exists():
        try:
            loaded = yaml.safe_load(profiles_path.read_text(encoding="utf-8", errors="replace")) or {}
    ...
    for raw in data.get("items") or []:
        if not isinstance(raw, dict) or str(raw.get("type") or "") != "remote":
            continue
        uid = str(raw.get("uid") or "")
        file_name = str(raw.get("file") or "")
        profile_path = root / "profiles" / file_name if file_name else Path("")
        items.append({
            "uid": uid,
            ...
            "current": uid == current,
            "profile_file_exists": bool(file_name and profile_path.exists()),
            "expiry": "unknown",
        })
    ...
    if generated.exists():
        try:
            loaded = yaml.safe_load(generated.read_text(encoding="utf-8", errors="replace")) or {}
            config = loaded if isinstance(loaded, dict) else {}
        except Exception:
            config = {}
    groups = [
        {"name": str(item.get("name") or ""), "type": str(item.get("type") or ""), "node_count": len(item.get("proxies") or [])}
        for item in config.get("proxy-groups") or []
        if isinstance(item, dict) and item.get("name")
    ]
    recommended = choose_gateway_group(groups).get("name", "") if groups else ""
    return {
        "schema": f"{SCHEMA_PREFIX}.verge_profiles.v1",
        "ok": profiles_path.exists(),
        "home": str(root),
        "current_uid": current,
        "remote_profile_count": len(items),
        "profiles": items,
        "recommended_group": recommended,
        "config_digest": hashlib.sha256(generated.read_bytes()).hexdigest() if generated.exists() else "",
...
def controller_diagnosis(base_url: str) -> dict[str, Any]:
    ...
    proxy_port = next((int(item.get("mixed_port")) for item in configs if isinstance(item.get("mixed_port"), int)), 0)
    proxy_listener_reachable = bool(proxy_port and tcp_reachable("127.0.0.1", proxy_port))
    ...
    return {
        ...
        "proxy_listener_reachable": proxy_listener_reachable,
        "proxy_status": "current_proxy_listener_reachable" if proxy_listener_reachable else "current_proxy_listener_unreachable",
        "node_control_status": "ready" if requested_reachable or reachable_config_controllers else "degraded",
