...
@contextmanager
def _index_transaction(path: Path):
...
def apply_iteration_candidate(
    ...
    *,
    ...
    with _index_transaction(index_path) as lock_path:
        ...
        if existing is not None:
            return {
                **plan,
                ...
                "transaction_lock": str(lock_path),
            }
...
def recall_iteration_candidate(
    ...
    *,
    ...
    active = bool(item) and item.get("status") == "approved_applied"
    return {
        ...
        "path": str(index_path),
        "item": item or {},
        "reason": "found" if active else ("candidate_authorization_invalidated" if item else "candidate_not_found"),
    }


def invalidate_iteration_candidates(
    ...
    *,
    ...
    """Invalidate exact absorbed candidates while preserving audit history."""

    ids = list(dict.fromkeys(str(value).strip().lower() for value in candidate_ids if str(value).strip()))
    plan = {
        "schema": "memory_iteration_owner.invalidation_plan.v1",
        "ok": bool(ids and reason and evidence_ref),
        "dry_run": True,
        "candidate_ids": ids,
        "reason": str(reason),
    ...
    if not confirm:
        return plan
    if not plan["ok"]:
        return {**plan, "dry_run": False, "ok": False, "reason_code": "invalidation_scope_incomplete"}
    if not backup and index_path.resolve() == DEFAULT_MEMORY_INDEX.resolve():
        return {**plan, "dry_run": False, "ok": False, "reason_code": "production_memory_backup_cannot_be_disabled"}
    with _index_transaction(index_path) as lock_path:
        payload, error = _read_index(index_path)
        if error:
            return {**plan, "dry_run": False, "ok": False, "reason_code": "memory_index_unreadable", "error": error}
        entries = payload.get("iteration_candidates")
        if not isinstance(entries, list):
            return {**plan, "dry_run": False, "ok": False, "reason_code": "iteration_candidates_not_list"}
        matches = [item for item in entries if isinstance(item, dict) and str(item.get("candidate_id") or "").lower() in ids]
        missing = sorted(set(ids) - {str(item.get("candidate_id") or "").lower() for item in matches})
        if missing:
            return {**plan, "dry_run": False, "ok": False, "reason_code": "candidate_not_found", "missing": missing}
        backup_result: dict[str, Any] = {"ok": True, "skipped": "test_override"}
        if backup and index_path.exists():
            backup_result = create_backup([str(index_path)], remark=f"iteration-invalidate-{len(ids)}", purpose="invalidate incorrectly authorized memory absorption", category="memory", trigger="memory_iteration_owner")
            if not backup_result.get("ok"):
                return {**plan, "dry_run": False, "ok": False, "reason_code": "backup_failed", "backup": backup_result}
        invalidated_at = now_iso()
        for item in matches:
            item.update({
                "status": "authorization_invalidated",
                "invalidated_at": invalidated_at,
                "invalidation_reason": str(reason)[:1000],
                "invalidation_evidence_ref": str(evidence_ref)[:1000],
                "previous_status": str(item.get("status") or ""),
            })
        payload["generated_at"] = invalidated_at
        _write_json_atomic(index_path, payload)
    return {**plan, "schema": "memory_iteration_owner.invalidation_apply.v1", "dry_run": False, "ok": True, "invalidated_count": len(matches), "backup": backup_result, "transaction_lock": str(lock_path)}
...
def validate_iteration_candidate(
    ...
    *,
    ...
    identity_ok = verify_candidate_identity(candidate)
    content_ok = bool(item) and item.get("status") == "approved_applied" and item.get("text") == str(candidate.get("summary") or "")
