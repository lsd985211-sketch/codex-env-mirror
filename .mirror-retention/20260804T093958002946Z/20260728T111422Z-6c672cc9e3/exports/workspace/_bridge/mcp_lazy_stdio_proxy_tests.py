...
from mcp_session_doctor import SMOKE_SPECS
...
class McpLazyStdioProxyTests(unittest.TestCase):
    ...
    def test_remote_doc_profiles_use_current_platform_executables(self) -> None:
        for profile, launcher_profile in (("context7", "ctx7"), ("openai-docs", "oadocs"), ("microsoftdocs", "msdocs")):
            with self.subTest(profile=profile):
                smoke_command = list(SMOKE_SPECS[profile].command)
                _, guarded_command = profile_command(launcher_profile, lazy=False)
                self.assertEqual(smoke_command[0], sys.executable)
                self.assertEqual(guarded_command[0], sys.executable)
                if sys.platform != "win32":
                    self.assertFalse(any(item.casefold().endswith(".exe") for item in guarded_command[:2]))
