---
name: context7-cli
description: "ctx7 CLI：获取库文档、管理技能、配置 MCP。触发：ctx7, context7, 安装技能"
---
...
- **[Documentation](references/docs.md)** 鈥?Fetch current docs for any library. Use when writing code, verifying API signatures, or when training data may be outdated.
- **[Skills management](references/skills.md)** 鈥?Install, search, suggest, list, remove, and generate AI coding skills.
- **[Setup](references/setup.md)** 鈥?Configure Context7 MCP for Claude Code / Cursor / OpenCode.
...
- Library IDs require a `/` prefix 鈥?`/facebook/react` not `facebook/react`
- Always run `ctx7 library` first 鈥?`ctx7 docs react "hooks"` will fail without a valid ID
- Repository format for skills is `/owner/repo` 鈥?e.g., `ctx7 skills install /anthropics/skills`
- `skills generate` requires login 鈥?run `ctx7 login` first
