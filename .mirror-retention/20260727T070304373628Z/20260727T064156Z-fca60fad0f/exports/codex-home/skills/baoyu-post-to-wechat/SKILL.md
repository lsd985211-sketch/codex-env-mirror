...
- Confirm the target is a WeChat Official Account draft/publish flow.
- Decide whether API or browser publishing is the right path before converting.
- Validate metadata and cover image requirements before publishing.

## Output Contract

- Return the publish method, title, and draft result.
- Mention comment settings and whether media or HTML conversion happened.
- If publishing failed, say which prerequisite was missing.
