...
from shared.windows_runtime_assets import gui_ocr_python_paths  # noqa: E402
...
def expected_codex_mcp_specs(parsed_config: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    gui_ocr_python_path, gui_ocr_fallback_path = gui_ocr_python_paths()
    gui_ocr_python = str(gui_ocr_python_path)
    gui_ocr_fallback_python = str(gui_ocr_fallback_path)
    servers = parsed_config.get("servers") if isinstance(parsed_config.get("servers"), dict) else {}
...
def gui_automation_health(config: dict[str, Any], codex_mcp_config: dict[str, Any]) -> dict[str, Any]:
    ...
    default_ocr_python, default_ocr_fallback = gui_ocr_python_paths()
    ocr_python = Path(str(env.get("GUI_OCR_PYTHON") or default_ocr_python))
    ocr_fallback_python = Path(str(env.get("GUI_OCR_FALLBACK_PYTHON") or default_ocr_fallback))
