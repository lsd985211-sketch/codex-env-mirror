...
def curl_probe_command(platform_name: str | None = None) -> tuple[str, str]:
    """Return the native curl command and null sink for the execution host."""

    target_platform = platform_name or os.name
    executable = "curl.exe" if target_platform == "nt" else "curl"
    resolved = shutil.which(executable)
    return (resolved or executable, "NUL" if target_platform == "nt" else "/dev/null")
...
def curl_timing(
    ...
    *,
    ...
    curl_executable, null_sink = curl_probe_command(platform_name)
    command = [
        curl_executable,
        "-L",
        "-o",
    ...
    try:
        ...
        return {
            ...
            "proxy": proxy,
            "probe_executable": curl_executable,
            "execution_platform": platform_name or os.name,
        }
