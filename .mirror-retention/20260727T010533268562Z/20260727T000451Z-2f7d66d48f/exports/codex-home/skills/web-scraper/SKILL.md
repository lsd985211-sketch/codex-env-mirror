...
```bash
python scripts/fetch_url.py "https://example.com/article" --timeout 60
```
...
- Confirm the task is URL content capture or article reading.
- Prefer scraping only when the content is reachable without browser interaction.
- If the page needs JS or blocks fetch, switch to browser automation instead of pretending success.

## Output Contract

- Return the fetched URL and the extraction result.
- Mention truncation, blocking, or JS-rendering risk explicitly.
- Do not claim full fidelity when the fetch was partial.
