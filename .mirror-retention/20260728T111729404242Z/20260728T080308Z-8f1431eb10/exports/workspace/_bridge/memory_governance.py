...
from _bridge.memory_iteration_owner import (
    ...
    invalidate_iteration_candidates as _invalidate_iteration_candidates,
...
def iteration_candidates_invalidate(candidate_ids: list[str], *, reason: str, evidence_ref: str, confirm: bool = False, index_path: Path = ITERATION_MEMORY_INDEX, backup: bool = True) -> dict[str, Any]:
    return _invalidate_iteration_candidates(candidate_ids, reason=reason, evidence_ref=evidence_ref, confirm=confirm, index_path=index_path, backup=backup)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        choices=[
            ...
            "iteration-candidates-invalidate",
            "recall-checks",
            "recall-verify",
            "work-note-add",
            "work-note-read",
    ...
    parser.add_argument("--evidence-ref", default="", help="Stable evidence reference for exact invalidation")
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    elif args.command == "iteration-candidates-invalidate":
        payload = iteration_candidates_invalidate([value.strip() for value in args.ids.split(",")], reason=args.reason, evidence_ref=args.evidence_ref, confirm=bool(args.confirm_apply))
