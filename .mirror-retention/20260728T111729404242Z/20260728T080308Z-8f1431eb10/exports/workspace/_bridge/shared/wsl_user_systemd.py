...
def unit_path_value(value: Path | str) -> str:
    """Serialize one absolute path for systemd path-valued directives."""
    text = str(value)
    if not Path(text).is_absolute():
        raise ValueError(f"systemd path must be absolute: {text}")
    if any(character in text for character in ("\0", "\n", "\r")):
        raise ValueError("systemd path contains a forbidden control character")
    return text.replace("\\", "\\x5c").replace("%", "%%").replace(" ", "\\x20").replace("\t", "\\x09")
