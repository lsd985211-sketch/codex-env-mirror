...
def bounded_query_result(
    ...
    *,
    ...
    for inline_count in dict.fromkeys((len(all_items), 12, 8, 5, 3, 1)):
        ...
        if not candidate.get("compression_blocked") and all(key in candidate for key in required_keys):
            if inline_count < len(all_items):
    ...
    *,
    ...
    for selected_system in selected_systems:
        ...
        for row in source_rows:
            if term and row_query_score(row, term) <= 0:
                continue
            if action and action not in row.get("actions", []):
                continue
            rows.append(row)
    rows.sort(key=lambda row: (-row_query_score(row, term), str(row.get("module_path") or "")))
    ...
    return bounded_query_result(
        {
            ...
            "derived_refresh_recommended": index_reason in {"index_missing", "index_stale"},
...
def query_registry(*, system: str = "", term: str = "", action: str = "", limit: int = 20) -> dict[str, Any]:
    ...
    try:
        raw_rows = connection.execute(
            f"SELECT * FROM capabilities {where} ORDER BY system, module_path", params
        ).fetchall()
    finally:
        connection.close()
    decoded_rows = [_row_to_dict(row) for row in raw_rows]
    matching_rows = [row for row in decoded_rows if not term or row_query_score(row, term) > 0]
    matching_rows.sort(key=lambda row: (-row_query_score(row, term), str(row.get("system") or ""), str(row.get("module_path") or "")))
    selected_rows = matching_rows[:effective_limit]
    ...
    return bounded_query_result(
        {
            ...
            "has_more": total > len(selected_rows),
            "limit": effective_limit,
            "items": selected_rows,
            "index_status": "fresh",
