...
- Authorization is non-transitive and non-inheritable. For approval, external write, installation, deletion, permission, configuration, business-state, release, or other high-impact actions, a static confirmation word or earlier broad instruction is only an anti-mistake signal. The owning system must consume a current-task, post-challenge, short-lived, single-use authorization reference bound to the exact action, target fingerprint, risk, phase, and source/version signature before its first side effect. Scope changes require a new challenge; callers and durable executors may pass the stable reference but must not reinterpret, copy, widen, or consume it twice.
...
- Before repository-scoped file, state, or tool work, query Codex App thread state for other active tasks in the same canonical Git repository. Use this snapshot only to identify candidates; exclude the current task and tasks that are idle, sleeping, not loaded, completed, archived, or outside that repository; identify linked worktrees by Git common-dir/repository identity rather than cwd spelling alone.
- When another task is active in that repository, immediately before every `send_message_to_thread` call re-query the specific target's current Codex App state. Read `CODEX_THREAD_ID` as the authoritative caller identity before this check: when it is available and the target ID equals it, do not call the send tool. `currentTask=false` supplied in a candidate record is not an authority that can override this comparison. A Codex App `threadId` is not a collaboration-agent ID: deliver to a listed App task only through `send_message_to_thread`, with its current `threadId` and `hostId`; use collaboration-agent messaging only for an agent registered in the current collaboration tree, never as a fallback for an App thread. By default, dispatch only when the target is still `active`, in the same repository, non-current, and non-archived; a prior discovery snapshot, a message receipt, or natural-language intent never proves present activity. Do not send to or wake idle, sleeping, not-loaded, completed, or archived tasks. The sole exception is an explicit user request to contact that specific non-active task; a coordination need, stale observation, or inferred intent is not an exception. A bounded work-start message names the goal, declared paths or surfaces, expected state writes, branch/worktree, and requested ownership or overlap coordination. A send receipt proves notice, not agreement: resolve reported or plausible overlap before editing, while clearly disjoint work may proceed without waiting indefinitely. If thread discovery is unavailable, shared-worktree mutation is blocked; an isolated declared-path worktree may proceed only when available Git evidence shows no known overlap.
- Before reporting that a requested tool has no entry point, classify its surface first. Missing MCP configuration, CLI visibility, a persisted dynamic-tool row, or a collaboration-agent registration is not evidence that a native Codex App tool is unavailable; inspect the current exposed App tool schema and use its operation-specific discovery entrypoint. Only a current-turn absence or call failure under that authoritative schema may support an unavailable claim, with the exact observation recorded for the owner.
...
- For repeatable machine flows and closeout, consume `execution_route_pack.terminal_convergence` as the workflow-owned construction decision. Complete source-affecting finalization before at most one terminal mutation, then use read-only acceptance; reuse matching receipts and resume from the first invalid action. Conflicts, dependency cycles, undeclared mutations, or a source change after terminal mutation block completion instead of triggering an automatic loop.
...
- For long commands, consume the native process or session handle until exit. If direct handle consumption is unavailable or unreliable, derive one stable intent ID and use the durable long-command owner's single-call `converge` entry, which submits or reuses once and remains responsible until a terminal receipt exists. Reuse matching terminal failures and reject changed command/cwd/timeout signatures. Running observations are human diagnostics only: they must not instruct Codex to issue another `status` or `follow`, and Codex must not build a polling loop. Compact output retains terminal state, exit code, and a stable full-result reference.
- Approval-facing surfaces default to Chinese as their primary language, including approval cards, design specifications, implementation plans, repair or migration plans, risk reports, change proposals, and decision summaries. Use Chinese for user-facing titles, labels, explanations, conclusions, risks, options, recommendations, review items, acceptance criteria, and next actions. Preserve exact code, commands, paths, configuration keys, environment variables, protocol fields, schema names, API identifiers, product names, hashes, stable IDs, and external quotations. Machine-readable files and source code do not require Chinese, but their user-facing approval projections do. Workflow approval owners must consume the shared presentation policy instead of relying on instruction compliance alone. An explicit user language request or an external format contract overrides this default.
