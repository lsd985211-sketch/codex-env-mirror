...
description: Operate the governed Codex environment recovery mirror and GitHub recovery seed. Use when inspecting mirror readiness, refreshing or publishing the active-only snapshot from validated WSL Work Git state, validating control-plane freshness, planning or publishing semantic recovery milestones, diagnosing drift, preparing an isolated restore plan, or staging a recovery without activation. Trigger for Codex environment backup/mirror/recovery requests, not ordinary project backups.
---
...
Normal publication is one-way:

```text
WSL Work Git -> Windows bare Git -> validated mirror publish -> GitHub recovery repository
```

The WSL worktree is the daily source. The Windows bare Git repository is the
local durable handoff. The mirror repository is a downstream recovery product,
not a place to accumulate working edits.

Mirror source reads are read-only. Snapshot, validation, and publish may run
generated export commands, but they must not trigger WSL runtime materialization
or reverse-project host state back into the Work Git tree.

MCP recovery has a separate materialization boundary. Work Git stores source,
routing, manifests, and the single MCP bundle contract; the bundle owner builds
or verifies content-addressed offline archives for platform-specific executable
implementations and dependencies. A fresh-agent restore is not executable
until every required MCP has a verified archive or an explicit owner
reacquisition/reconnect receipt. `capability_restore_ready` therefore means
source plus required implementation readiness, not merely that files can be
staged. Secrets, sessions, runtime databases, private Windows binaries, plugin
caches, active model/provider selection, and mutable CC Switch settings remain
outside Git and public Release assets. Recovery preserves the owners,
capability inventory, logical secret requirements, and reacquisition path; it
does not freeze machine-local model preferences into a public snapshot.
...
Before MCP-sensitive restore or publication, consume the bundle owner:

```powershell
python _bridge\mcp_recovery_bundle_owner.py validate
python _bridge\mcp_recovery_bundle_owner.py readiness
python _bridge\mcp_recovery_bundle_owner.py build --bundle <id>
```

On a fresh device, acquire the milestone's public MCP implementations before
materialization. The owner downloads the release index and archives through
`gh`, verifies every SHA-256 value, and only then writes the local bundle index:

```powershell
python _bridge\mcp_recovery_bundle_owner.py --archive-root <isolated-bundle-root> import-release --repo lsd985211-sketch/codex-env-mirror --tag <seed-vX.Y.Z>
python _bridge\mcp_recovery_bundle_owner.py --archive-root <isolated-bundle-root> verify
python _bridge\mcp_recovery_bundle_owner.py --archive-root <isolated-bundle-root> materialize --target-root <isolated-mcp-root>
```

The Release must contain `mcp-bundle-index.json` and every archive named by
that index. Missing or hash-mismatched assets block release and restoration.
Encrypted Windows archives, secrets, plugins, and remote MCP credentials stay
on their existing owner reacquisition/import paths.

Start with `status` for an existing mirror. For WSL-originated work, first
ensure the WSL worktree is committed and pushed to the Windows bare Git store;
then use `publish` when the user asks to publish the recovery seed. `publish`
must refresh or reuse the verified snapshot, update the local mirror
repository, push the GitHub recovery remote, and verify the remote head. When
no explicit `--changed` paths are supplied, `publish` derives the Work Git diff
since the last mirrored Work Git head and runs `affected-source-plan`; it uses
directed capture only when that plan is safe, otherwise it reports the fallback
reason and performs a full rebuild. Use `plan` before a refresh when source
scope or readiness is uncertain. Read
[references/operations.md](references/operations.md) only when exact commands,
confirmations, or result fields are needed.

When a terminal transport may lose long-command output, use the unified durable
entry once instead of launching a second publish: `python
_bridge/codex_workflow_entry.py mirror submit --operation publish --intent-id
<stable-intent> --confirm PUBLISH-CODEX-MIRROR`. The facade submits or reuses
the intent and stays alive until one terminal receipt exists. Do not issue
`status` or `follow` in a loop; those commands are observation-only. The single
call preserves confirmation, source gates, and the one-at-a-time mirror lock.
Every mirror write (`refresh`, `publish`, `finalize-and-publish`, and `release`)
is automatically routed through a source- and argument-bound durable intent;
release additionally binds the immutable tag. Compatibility commands are safe
to call once. If the outer transport is lost after submission, call the same
`mirror submit` command once with
`--recover-only`; it verifies the original intent and execution signature and
consumes the stored terminal receipt without launching another business command.
...
| User asks to publish the recovery seed | `mirror publish --confirm PUBLISH-CODEX-MIRROR` | Consume validated Work Git, derive changed paths from Work Git head diff, refresh/reuse, validate, commit mirror repo, push GitHub remote, verify remote |
...
5. Run one approved publish only after finalization when the user wants the validated recovery seed released to GitHub. The source must already be committed in WSL Work Git and synchronized to the Windows bare Git store. `publish` wraps the approved refresh/reuse path, then commits any mirror repository metadata and pushes the selected GitHub recovery branch after validation. Use `refresh` only when the requested outcome is local capture without remote publication. If no explicit `--changed` paths are supplied, `publish` derives them from the Work Git diff between the latest mirrored head and current head, then consumes `affected-source-plan`. Use directed capture only when the plan is safe; otherwise use full refresh. A valid committed snapshot with unchanged live sources is reused only when no explicit changed paths were supplied.
...
The mirror repository is an output, not a mirror source or daily work area. Its
own commits, tags, releases, and files must not trigger another refresh.
Post-closeout refresh is idempotent: reuse an existing successful receipt
within one closeout and reuse the committed snapshot when live-source
validation proves that source content is unchanged.
...
- Preserve CC Switch skills only when they are stable user-owned assets.
  Reacquire model/provider selection and mutable CC Switch settings through
  their owners; do not publish the active config, model catalog, settings, or a
  semantic database export. Preserve plugins through enabled
  identity/version/hash inventory and reacquire them through the plugin owner.
