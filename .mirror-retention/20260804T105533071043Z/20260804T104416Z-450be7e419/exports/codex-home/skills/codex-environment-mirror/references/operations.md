...
| Refresh verified snapshot | `python _bridge\codex_workflow_entry.py mirror refresh --confirm REFRESH-CODEX-MIRROR` | Automatically enters a Work Git HEAD- and argument-bound durable intent; creates, validates, prunes superseded snapshots, commits |
...
| Publish recovery seed | `python _bridge\codex_workflow_entry.py mirror publish --confirm PUBLISH-CODEX-MIRROR` | Automatically enters a Work Git HEAD- and argument-bound durable intent; consumes validated Work Git state, derives changed paths, refreshes or reuses, validates, commits mirror repo, pushes and verifies the recovery remote |
...
| Publish milestone | `python _bridge\codex_workflow_entry.py mirror release --tag seed-vX.Y.Z --confirm RELEASE-CODEX-MIRROR` | Automatically enters the tag-bound durable intent, updates generated current state, verifies, tags, publishes Release and manifest attachment |
...
If the terminal transport disappears during any mirror write, do not run the
business operation through a new task id and do not poll the business owner.
Reuse the exact prior submit command with
`--recover-only`; intent, command, cwd, timeout, terminal state, and exit code
must all match before the durable result is returned.
