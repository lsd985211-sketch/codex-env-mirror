...
DECLARATIVE_CODEX_HOME = WORKTREE_ROOT / "codex-home"
CODEX_HOME = Path(os.environ.get("CODEX_HOME") or DECLARATIVE_CODEX_HOME).expanduser()
RESOURCE_DOC_ROOT = Path(
    os.environ.get("CODEX_RULE_MIRROR_ROOT")
    or (
        r"C:\Users\45543\Desktop\Codex资源库\文档\系统维护\Codex准则"
        if os.name == "nt"
        else "/mnt/c/Users/45543/Desktop/Codex资源库/文档/系统维护/Codex准则"
    )
)
...
MIRRORS: tuple[dict[str, Any], ...] = (
    ...
    {
        "id": "workspace_wsl_work_git",
        "title": "WSL Work Git 工作区准则镜像",
        "source": WORKTREE_ROOT / "AGENTS.md",
        "target": RESOURCE_DOC_ROOT / "工作区准则-mcsmanager-镜像.txt",
        "scope": "当前 WSL Work Git 规则。实际生效源文件为 /home/codexlab/work/codex-workspace/AGENTS.md。",
    },
)
...
def sync(_args: argparse.Namespace | None = None) -> dict[str, Any]:
    ...
    index.write_text(
        ...
        "文件:\n"
        "- 全局准则镜像.txt\n"
        "- 工作区准则-mcsmanager-镜像.txt（内容为 WSL Work Git 工作区准则，保留文件名仅为兼容现有引用）\n\n"
        "维护命令:\n"
        "- WSL 刷新: python3 workspace/_bridge/agents_rule_mirror.py sync\n"
        "- WSL 检查: python3 workspace/_bridge/agents_rule_mirror.py doctor\n"
        "- WSL 验证: python3 workspace/_bridge/agents_rule_mirror.py validate\n",
        encoding="utf-8",
    )
    return {
