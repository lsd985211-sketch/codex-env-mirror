...
import local_mcp_hub_authorization as hub_authorization
from platform_paths import wsl_worktree_linux_root
from shared.windows_powershell import powershell_encoded_command
...
def hub_source_signature() -> str:
    """Return the authoritative Work Git commit loaded by the Hub owner."""
    ...
    *,
    ...
    return "\n".join(
        [
            ...
            f"WorkingDirectory={unit_path_value(script.parent)}",
...
def windows_powershell_command(script: str) -> list[str]:
    ...
    return windows_interop_command(command[0], *command[1:])
...
def reload_authorization_plan(
    *, thread_id: str, assessment: dict[str, Any], rollout_path: Path | str,
    ...
    port: int = DEFAULT_PORT, state_root: Path | str | None = None,
) -> dict[str, Any]:
    service = hub_service_status()
    authority = "wsl_user_systemd" if service.get("unit_exists") else "windows_scheduled_task_fallback"
    processes = hub_runtime_state(port, process_retry_seconds=0).get("processes", [])
    source_signature = hub_source_signature()
    if not source_signature:
        return {"ok": False, "reason": "hub_source_signature_unavailable"}
    return hub_authorization.prepare_reload(
        thread_id=thread_id, source_signature=source_signature, authority=authority,
        port=port, matched_processes=processes, assessment=assessment,
...
def reload_local_hub(
    *, confirm_reload: bool, port: int = DEFAULT_PORT, wait_seconds: float = 5.0,
    authorization_permit_ref: str = "", authorization_operation_id: str = "",
    authorization_thread_id: str = "", workflow_semantic_hash: str = "",
    ...
    bytecode_cache = clear_hub_bytecode_cache(dry_run=True)
    plan = {
    ...
    if not confirm_reload:
        plan["next_step"] = "prepare a current-task reload permit, then rerun with --confirm-reload and its stable authorization references"
        return plan
    source_signature = hub_source_signature()
    authorization = hub_authorization.authorize_reload(
        authorization_permit_ref, operation_id=authorization_operation_id,
        workflow_semantic_hash=workflow_semantic_hash,
        thread_id=authorization_thread_id, source_signature=source_signature,
        authority=authority, port=port, matched_processes=before,
    ...
    if not authorization.get("ok"):
        plan.update({
            ...
            "reason": authorization.get("reason", "hub_reload_authorization_blocked"),
            "next_step": "prepare and consume an exact current-task reload permit; no cache or process side effect occurred",
        })
        return plan
    bytecode_cache = clear_hub_bytecode_cache(dry_run=False)
    plan["bytecode_cache"] = bytecode_cache
    if not bytecode_cache.get("ok"):
        ...
        plan["authorization_effect"] = hub_authorization.finish_reload(
            authorization_operation_id, plan, state_root=authorization_state_root,
        )
        return plan
    if systemd_primary:
    ...
    effect = hub_authorization.finish_reload(
        authorization_operation_id, plan, state_root=authorization_state_root,
    )
    plan["authorization_effect"] = effect
    if not effect.get("ok"):
        plan.update({
            "ok": False,
            "reason": effect.get("reason", "hub_reload_effect_journal_failed"),
        })
    return plan
...
def main(argv: list[str] | None = None) -> int:
    ...
    auth_parser = sub.add_parser("reload-authorization-plan")
    auth_parser.add_argument("--thread-id", required=True)
    auth_parser.add_argument("--assessment-json", required=True)
    auth_parser.add_argument("--rollout-path", required=True)
    auth_parser.add_argument("--user-message-ref", required=True)
    auth_parser.add_argument("--operation-id", required=True)
    auth_parser.add_argument("--workflow-semantic-hash", required=True)
    auth_parser.add_argument("--authorization-state-root", default="")
    ...
    reload_parser.add_argument("--authorization-permit-ref", default="")
    reload_parser.add_argument("--authorization-operation-id", default="")
    reload_parser.add_argument("--authorization-thread-id", default="")
    reload_parser.add_argument("--workflow-semantic-hash", default="")
    reload_parser.add_argument("--authorization-state-root", default="")
    ...
    elif args.command == "reload-authorization-plan":
        try:
            assessment = json.loads(Path(args.assessment_json).read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            payload = {"ok": False, "reason": "hub_reload_assessment_unreadable"}
        else:
            payload = reload_authorization_plan(
                thread_id=args.thread_id, assessment=assessment,
                rollout_path=args.rollout_path, user_message_ref=args.user_message_ref,
                operation_id=args.operation_id,
                workflow_semantic_hash=args.workflow_semantic_hash, port=args.port,
