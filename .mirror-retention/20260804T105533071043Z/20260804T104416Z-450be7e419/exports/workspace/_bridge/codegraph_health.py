...
PROJECT_CODEGRAPH_BIN = ROOT / "_bridge" / "tools" / "codegraph" / "node_modules" / ".bin" / "codegraph"
...
def detect_installed_version() -> dict[str, Any]:
    ...
    local_exe = PROJECT_CODEGRAPH_BIN if os.name != "nt" and PROJECT_CODEGRAPH_BIN.is_file() else None
    if local_exe is None and PROJECT_CODEGRAPH_CMD.is_file():
        local_exe = PROJECT_CODEGRAPH_CMD
...
def snapshot(indexes: list[Path]) -> dict[str, Any]:
    return {
        ...
        "environment": {
            ...
            "project_local_codegraph": bool(PROJECT_CODEGRAPH_BIN.is_file() or PROJECT_CODEGRAPH_CMD.is_file()),
        },
