...
from intent_routing import IntentRule, has_install_action, matched_terms, rank_intents, term_matches
...
INTENT_RULES: dict[str, tuple[str, ...]] = {
    ...
    "package_or_library_docs": ("库文档", "package docs", "library docs", "sdk docs", "api docs"),
    "package_install": (
        "安装", "install", "package install", "dependency install", "升级", "upgrade", "upgrade package", "卸载", "uninstall",
        "choco", "chocolatey", "winget", "aria2", "aria2c",
    ),
...
RESOURCE_RULES: tuple[ResourceRule, ...] = (
    ResourceRule(
        ...
        labels=("外部依赖", "第三方依赖", "external dependency", "external dependencies", "dependency confirmation", "依赖确认"),
        owner_mcp="resource_layer_owner_selector",
        read_tools=("_bridge/codex_workflow_entry.py resource delegate --json", "_bridge/codex_workflow_entry.py resource route --json"),
        blocked_write_tools=("download_or_materialize_without_request_policy", "install_or_clone_without_scoped_authorization"),
        fallback_reasons=("resource_layer_no_owner_mcp_for_source", "owner_mcp_unavailable", "owner_mcp_insufficient"),
        route_terms=("resource_acquisition", "external_dependency"),
    ),
    ResourceRule(
        key="github",
    ...
    ResourceRule(
        key="package_manager",
        labels=(
            "安装", "install", "package install", "dependency", "升级", "upgrade", "upgrade package", "卸载", "uninstall",
            "pip", "npm", "pnpm", "uv", "uvx", "choco", "chocolatey", "winget", "aria2", "aria2c",
        ),
...
def _has_explicit_library_lookup(text: str) -> bool:
    return any(_contains(text, term) for term in EXPLICIT_LIBRARY_LOOKUP_TERMS)


def detect_intents(message: str) -> list[dict[str, Any]]:
    text = str(message or "").lower()
    ranked = rank_intents(message, tuple(IntentRule(key, terms) for key, terms in INTENT_RULES.items()))
    existing_search_artifact = matched_terms(
        text,
        ("搜索结果", "检索结果", "代码搜索结果", "search result", "search output"),
    )
    explicit_external_search = matched_terms(
        text,
        ("联网", "查资料", "相关知识", "官方文档", "research", "web search", "look up"),
    )
    if existing_search_artifact and not explicit_external_search:
        ranked = [
            item
            for item in ranked
            if item.get("key") != "external_research"
            or any(hit != "搜索" for hit in item.get("hits", []))
        ]
    if not has_install_action(text):
        ranked = [item for item in ranked if item.get("key") != "package_install"]
    return ranked
...
def detect_resources(message: str) -> list[dict[str, Any]]:
    ...
    for rule in RESOURCE_RULES:
        ...
        if not evidence:
            continue
        if rule.key == "library_docs" and not _has_explicit_library_lookup(text):
            continue
        if rule.key == "package_manager" and not has_install_action(text):
            continue
        resources.append(
            {
                "key": rule.key,
...
def build_route(message: str, *, source_domains: list[str] | None = None) -> dict[str, Any]:
    text = str(message or "").lower()
    structured_domains = list(dict.fromkeys(str(item or "").strip().lower() for item in (source_domains or []) if str(item or "").strip()))
    intents = detect_intents(message)
    resources = detect_resources(message)
    explicit_external = bool(structured_domains) or any(
        item["key"] in {"external_research", "external_dependency", "official_docs", "github_remote", "package_or_library_docs"}
        for item in intents
    )
    ...
    if structured_domains:
        owner_routes = []
    needs_resource_layer_owner_selection = bool(explicit_external and (structured_domains or not owner_routes))
        ...
        owner_routes.append(
            {
                ...
                "structured_source_domains": structured_domains,
            }
        )
    generic_web_gate = {
    ...
    return {
        ...
        "resource_layer_contract": {
            ...
            "owner_selection_source": "structured_source_domains" if structured_domains else "natural_language_route",
            "structured_source_domains": structured_domains,
            "task_class": (
                "package_install"
                if package_install
            ...
            "confirmation_policy": {
                "schema": "resource_confirmation_policy.v1",
                "read_only_stages": ["discover", "probe", "preview", "audit"],
                "confirmation_required_stages": ["materialize"],
                "side_effect_operations": ["materialize", "install", "clone", "remote_write"],
                "authorization": "scoped_authorization_for_side_effects",
                "rule": "Read-only dependency evidence may proceed; confirmation is required only for side-effecting operations.",
            },
...
def validate() -> dict[str, Any]:
    samples = [
        ...
        ("检查外部依赖确认策略", {"external_dependency"}, True),
        ...
        ("修复 Python 模块中的函数调用问题并验证影响范围", set(), False),
        ("查询 Python 官方文档", {"library_docs"}, True),
        ("联网研究 Python 3.14", {"library_docs"}, True),
        ("识别本机已经安装和可用的 OCR 能力，选择正确 owner 与工具处理扫描 PDF，并说明降级路径", set(), False),
        ("检查安装状态并列出已安装 OCR", set(), False),
        ("未安装 PaddleOCR，需要安装依赖", {"package_manager"}, True),
        ("Upgrade package pytest", {"package_manager"}, True),
    ]
    ...
    for message, expected_resources, should_block_generic_first in samples:
        ...
        if should_block_generic_first:
            ...
            if resource_contract.get("task_class") != "package_install":
                item_ok = (
                    item_ok
        ...
        if "检查外部依赖确认策略" in message:
            item_ok = item_ok and resource_contract.get("confirmation_policy", {}).get("confirmation_required_stages") == ["materialize"]
        if message == "修复 Python 模块中的函数调用问题并验证影响范围":
            item_ok = (
                item_ok
                and resource_contract.get("required") is False
                and resource_contract.get("task_class") != "research_only"
                and not any(item.get("key") == "library_docs" for item in route.get("resources", []))
            )
        ok = ok and item_ok
        cases.append(
            {
