...
import local_pmb_memory_service
from local_pmb_memory_process import hidden_creation_kwargs, run_pmb_command
from mcp_execution_priority import HUB_MANAGED_MCP_NAMES
from platform_paths import codex_home, memory_root, worktree_root
...
PMB_VENV = local_pmb_memory_platform.pmb_venv_root(BRIDGE_ROOT)
PMB_RUNTIME_BRIDGE_ROOT = PMB_VENV.parent
PMB_EXECUTABLES = local_pmb_memory_platform.pmb_executables(PMB_VENV)
PMB_EXE = PMB_EXECUTABLES["pmb"]
PMB_PYTHON = PMB_EXECUTABLES["python"]
PMB_PYTHONW = PMB_EXECUTABLES["pythonw"]
NO_WINDOW_KW = hidden_creation_kwargs()
MEMORY_ROOT = memory_root()
PMB_HOME = MEMORY_ROOT / "pmb" / "data"
PMB_PROCESS_HOME = local_pmb_memory_platform.process_argument_path(PMB_HOME, PMB_EXE)
...
PMB_USAGE_FEEDBACK_TYPES = {"success", "failure", "used"}
PMB_USAGE_FEEDBACK_LOG_NAME = "usage_feedback.jsonl"
PMB_RECALL_EVIDENCE_SCHEMA = "pmb-recall-evidence.v1"
PMB_RECALL_EVIDENCE_MAX_BYTES = 1400
PMB_RECALL_EVIDENCE_MAX_CANDIDATES = 0


def usage_feedback_log_path(default_root: Path) -> Path:
    configured = os.environ.get("CODEX_PMB_USAGE_FEEDBACK_LOG", "").strip()
    return Path(configured).expanduser() if configured else default_root / PMB_USAGE_FEEDBACK_LOG_NAME


def append_usage_feedback(path: Path, payload: dict[str, Any]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("a", encoding="utf-8") as handle:
        handle.write(json.dumps(payload, ensure_ascii=False, separators=(",", ":")) + "\n")


def usage_feedback_excerpt(value: str) -> str:
...
def record_recall_usage(
    *,
    ...
    """Append bounded recall metadata without changing PMB retrieval behavior."""

    identifier = query_id or uuid.uuid4().hex
    normalized_preview = str(preview or "")
    payload = {
        "schema": "pmb-usage-feedback.recall.v1",
        "event": "recall",
        "query_id": identifier,
        "query_excerpt": usage_feedback_excerpt(query),
        "query_sha256": sha256_text(str(query)),
        "top_k": int(top_k),
        "ok": bool(ok),
        "has_matches": bool(has_matches),
        "elapsed_ms": max(0, int(elapsed_ms)),
        "preview_sha256": sha256_text(normalized_preview),
        "created_at": now_iso(),
    }
    append_usage_feedback(usage_feedback_log_path(default_root), payload)
    return identifier
...
def build_recall_evidence(
    *,
    ...
    """Build a bounded, content-free shadow projection of one PMB recall.

    PMB's CLI output is not a stable candidate protocol. Until a source-provided
    identifier contract exists, successful matches are deliberately ``opaque``:
    the projection preserves diagnostic truth without copying memory content or
    inventing a path, rank, or candidate identifier.
    """

    trace_state = "failed" if not ok else ("empty" if not has_matches else "opaque")
    degradation_reason = "pmb_recall_failed" if not ok else (
        "no_matches" if not has_matches else "source_candidates_not_structurally_parseable"
    )
    payload = {
        "schema": PMB_RECALL_EVIDENCE_SCHEMA,
        "event": "recall_evidence",
        "query_id": str(query_id),
        "l0": {
            "schema": "pmb-recall-evidence.l0.v1",
            "recall_ok": bool(ok),
            "has_matches": bool(has_matches),
            "elapsed_ms": max(0, int(elapsed_ms)),
            "top_k": int(top_k),
            "candidate_trace_state": trace_state,
            "candidate_count": 0,
            "l2_preview_sha256": sha256_text(str(preview or "")),
            "degradation_reason": degradation_reason,
        },
        "l1": {
            "schema": "pmb-recall-evidence.l1.v1",
            "trace_state": trace_state,
            "candidates": [],
            "candidate_limit": PMB_RECALL_EVIDENCE_MAX_CANDIDATES,
            "next_action": "consume_l2" if trace_state == "opaque" else "none",
        },
        "parser_signature": sha256_text("pmb-recall-evidence:opaque-v1"),
        "created_at": now_iso(),
    }
    encoded = json.dumps(payload, ensure_ascii=False, separators=(",", ":")).encode("utf-8")
    if len(encoded) > PMB_RECALL_EVIDENCE_MAX_BYTES:
        raise ValueError("recall_evidence_exceeds_bounded_size")
    return payload


def record_recall_evidence(
    *,
    default_root: Path,
    query_id: str,
    top_k: int,
    ...
    """Append shadow-only recall evidence without changing PMB retrieval."""

    payload = build_recall_evidence(
        query_id=query_id,
        top_k=top_k,
        ok=ok,
...
def record_usage_feedback(*, default_root: Path, query_id: str, feedback_type: str, note: str = "") -> dict[str, Any]:
    ...
    if normalized_type not in PMB_USAGE_FEEDBACK_TYPES:
        raise ValueError(f"unsupported feedback_type: {feedback_type}")
    payload = {
        "schema": "pmb-usage-feedback.feedback.v1",
        "event": "feedback",
        "query_id": normalized_query_id,
        "feedback_type": normalized_type,
        "note": usage_feedback_excerpt(note),
        "created_at": now_iso(),
    }
    append_usage_feedback(usage_feedback_log_path(default_root), payload)
    return payload
...
def read_usage_feedback_events(*, default_root: Path) -> list[dict[str, Any]]:
    path = usage_feedback_log_path(default_root)
    if not path.exists():
        return []
...
def usage_feedback_summary(*, default_root: Path) -> dict[str, Any]:
    events = read_usage_feedback_events(default_root=default_root)
    recalls = [item for item in events if item.get("event") == "recall"]
    feedback = [item for item in events if item.get("event") == "feedback"]
    evidence = [item for item in events if item.get("event") == "recall_evidence"]
    by_type = Counter(str(item.get("feedback_type") or "unknown") for item in feedback)
    query_ids = {str(item.get("query_id")) for item in recalls if item.get("query_id")}
    trace_states = Counter(
        str((item.get("l0") or {}).get("candidate_trace_state") or "unknown")
        for item in evidence
        if isinstance(item.get("l0"), dict)
    )
    evidence_ids = {str(item.get("query_id")) for item in evidence if item.get("query_id")}
    return {
        "schema": "pmb-usage-feedback.summary.v1",
        "ok": True,
        "log_path": str(usage_feedback_log_path(default_root)),
        "recall_count": len(recalls),
        "feedback_count": len(feedback),
        "feedback_by_type": dict(sorted(by_type.items())),
        "distinct_query_count": len(query_ids),
        "unmatched_feedback_count": sum(1 for item in feedback if item.get("query_id") not in query_ids),
        "recall_evidence_count": len(evidence),
        "evidence_coverage_count": len(query_ids & evidence_ids),
        "evidence_missing_count": len(query_ids - evidence_ids),
        "evidence_trace_state_counts": dict(sorted(trace_states.items())),
    }
...
def pmb_env() -> dict[str, str]:
    return local_pmb_memory_platform.process_environment(
        os.environ,
        pmb_home=PMB_HOME,
...
def pmb_recall(query: str, *, top_k: int = 5) -> dict[str, Any]:
    ...
    payload = {
        ...
        "contract": {
            ...
            "writes_append_only_usage_feedback": True,
    ...
    try:
        record_recall_usage(
            default_root=REPORT_DIR,
            query=clean_query,
            top_k=top,
            ok=payload["ok"],
            has_matches=payload["has_matches"],
        ...
        payload["usage_feedback"] = {"ok": False, "reason": f"{type(exc).__name__}: {exc}"}
        return payload
    try:
        evidence = record_recall_evidence(
            default_root=REPORT_DIR,
            query_id=query_id,
            top_k=top,
            ok=payload["ok"],
        ...
        payload["recall_evidence"] = {
            "ok": True,
            "trace_state": evidence["l0"]["candidate_trace_state"],
            "schema": evidence["schema"],
        }
    except Exception as exc:
        payload["recall_evidence"] = {"ok": False, "reason": f"{type(exc).__name__}: {exc}"}
    return payload
...
def daemon_status() -> dict[str, Any]:
    ...
    return {
        ...
        "policy": {
            ...
            "persistent_residency_required": True,
            "bind_host": "127.0.0.1",
            "mcp_access": "Hub PMB tools connect to the single systemd-owned daemon and never start another process.",
            "idle_exit": "disabled by codex-pmb-memory.service with --idle-exit-min 0",
            "fallback": "disabled in wrapper to avoid hidden heavy in-process PMB servers",
        },
    }


def effective_daemon_state(daemon: dict[str, Any], processes: dict[str, Any]) -> dict[str, bool]:
    return {
        "running": bool(daemon.get("running")) or int(processes.get("root_count") or 0) == 1,
        "warm": bool(daemon.get("warm")),
    }


def daemon_start() -> dict[str, Any]:
    result = local_pmb_memory_service.ensure_service()
    return {
        "schema": "local-pmb-memory.daemon_start.v2",
        "ok": bool(result.get("ok")),
        "applied": bool(result.get("start", {}).get("ok")),
        "authority": local_pmb_memory_service.SERVICE_NAME,
...
def daemon_restart() -> dict[str, Any]:
    result = local_pmb_memory_service.restart_service()
    return {
        "schema": "local-pmb-memory.daemon_restart.v2",
        "ok": bool(result.get("ok")),
        "applied": bool(result.get("restart", {}).get("ok")),
        "authority": local_pmb_memory_service.SERVICE_NAME,
        "service_result": result,
        "after": daemon_status(),
    }
...
def snapshot() -> dict[str, Any]:
    ...
    effective_daemon = effective_daemon_state(pmb_daemon, pmb_processes)
    ...
    return {
        ...
        "pmb": {
            ...
            "effective_daemon_running": effective_daemon["running"],
            "effective_daemon_warm": effective_daemon["warm"],
        },
...
def validate() -> dict[str, Any]:
    ...
    surface_checks = [
        {
            "name": "pmb_daemon_warm",
            "ok": bool(snap["pmb"].get("effective_daemon_warm")),
            "detail": (snap["pmb"].get("daemon") or {}).get("status_preview", ""),
        },
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--feedback-type", default="")
    ...
    elif args.command == "usage-feedback":
        try:
            payload = record_usage_feedback(
                default_root=REPORT_DIR,
            ...
            payload = {"schema": "pmb-usage-feedback.feedback.v1", "ok": False, "reason": str(exc)}
    elif args.command == "usage-summary":
        payload = usage_feedback_summary(default_root=REPORT_DIR)
    else:
