...
from _bridge import system_membership  # noqa: E402
...
DOMAINS: tuple[SkillDomain, ...] = (
    SkillDomain(
        ...
        ("global-framework", "diagnose", "memory-systems"),
    ...
    SkillDomain(
        "hardware",
        "Cross-platform hardware routing, Windows host truth, WSL projections, USB, storage, and GPU diagnostics",
        ("hardware", "usb", "usbip", "usbipd", "mtp", "gpu", "nvidia", "pci", "block device", "storage health", "设备描述符", "code 43", "设备管理器", "硬件诊断", "显卡", "硬盘健康", "device busy", "hardware diagnostic"),
        ("hardware-ops", "windows-usb-ops", "diagnose"),
        ("hardware_system_owner.py", "wsl_hardware_owner.py", "windows_hardware_owner.py", "usb_device_owner.py", "usb_device_control.py"),
        ("Windows host truth and WSL-visible projection ownership",),
        ("hardware-ops", "windows-usb-ops"),
    ),
    SkillDomain(
        "audio",
        "Windows audio and governed media-library operations",
        ("audio playback", "music library", "音乐库", "音频处理", "歌词", "album artwork"),
        ("windows-audio-ops",),
        ("music_library_owner.py", "audio_toolkit.py"),
        ("Windows audio and media-library ownership",),
        ("windows-audio-ops",),
    ),
    SkillDomain(
        "network_routing",
        "network gateway and connectivity operations",
        ("network gateway", "proxy lease", "network routing", "网络网关", "代理租约", "网络连通"),
        ("windows-codex-ops", "global-framework"),
        ("codex_network_gateway.py",),
        ("network route policy",),
    ),
    SkillDomain(
        "resource_acquisition",
        "governed package and external resource acquisition",
        ("resource acquisition", "package dependency", "资源获取", "依赖安装", "资源层"),
        ("global-framework", "find-docs"),
        ("resource_cli.py", "resource_broker.py"),
        ("structured resource acquisition",),
    ),
    SkillDomain(
        "backup",
        "backup hygiene and recovery mirror operations",
        ("recovery mirror", "backup hygiene", "恢复镜像", "备份治理"),
        ("codex-environment-mirror", "global-framework"),
        ("backup_router.py", "codex_environment_mirror.py"),
        ("recovery mirror and backup hygiene",),
        ("codex-environment-mirror",),
    ),
    SkillDomain(
        "records_resources",
        "record indexes, retention, and bounded query surfaces",
        ("record retention", "record store", "记录索引", "记录保留", "归档记录"),
        ("global-framework",),
        ("record_store_maintenance.py",),
        ("record retention and indexed state",),
    ),
    SkillDomain(
        "wsl_workspace",
        "WSL workspace lifecycle and platform projection",
        ("wsl workspace", "work git", "bare git", "wsl 工作区", "工作git", "工作区投影"),
        ("codex-cli", "global-framework"),
        ("wsl_workspace_owner.py", "work_git_change_owner.py"),
        ("WSL workspace lifecycle",),
    ),
    SkillDomain(
        "drafts",
        "draft lifecycle and review references",
        ("draft lifecycle", "review reference", "草案生命周期", "草案审阅"),
        ("global-framework",),
        ("draft_governance.py",),
        ("draft lifecycle",),
    ),
    ...
    SkillDomain(
        "gitnexus_semantic_graph",
        "GitNexus semantic code graph analysis",
        (
            "gitnexus",
            "semantic code",
            "hybrid code",
        ...
        ("gitnexus-ops", "codegraph-ops", "graphify-ops"),
        ("gitnexus", "codegraph"),
        ("GitNexus semantic execution flow", "cross-repository code graph impact"),
        ("gitnexus-ops",),
    ),
    SkillDomain(
        "graphify_knowledge_graph",
        "Graphify managed mixed-artifact graph analysis",
        (
            "graphify",
            "mixed artifact graph",
        ...
        ("graphify-ops", "gitnexus-ops", "codegraph-ops"),
        ("graphify", "gitnexus", "codegraph"),
        ("Graphify mixed-artifact graph review", "managed graph review delta"),
        ("graphify-ops",),
    ),
    SkillDomain(
        "codex_runtime",
    ...
    SkillDomain(
        ...
        ("open-code-review-delegate", "global-framework", "diagnose", "codegraph-ops", "bug-hunt-swarm"),
        ("ocr", "codegraph", "filesystem", "github", "code_maintainability.py"),
    ...
    SkillDomain(
        ...
        ("演示文稿", "幻灯片", "课件", "slide deck", "slides", "presentation", "powerpoint", "ppt", "pptx"),
    ...
    return [
        {
            ...
            "systems": system_membership.systems_for_domain(str(item["key"]), consumer="skill"),
        }
...
def capability_matches(
    ...
    *,
    ...
    """Expose bounded description matches without changing execution selection.

    This is intentionally a discovery side-channel: a description match makes
    a Skill visible to the environment capability view, but does not grant
    admission, permission, or execution priority over the selected owner.
    """
    text = str(message or "").casefold()
    terms = list(dict.fromkeys(re.findall(r"[a-z][a-z0-9_+.-]{1,}|[\u4e00-\u9fff]{2,}", text)))
    ignored = {"the", "and", "with", "from", "this", "task", "能力", "本机", "已经", "选择"}
    terms = [term for term in terms if term not in ignored and term not in {"pdf", "owner", "tool"}][:24]
    source = inventory if inventory is not None else merged_inventory()
    matches: list[dict[str, Any]] = []
    for skill in source.values():
        description = f"{skill.get('description') or ''} {' '.join(str(item) for item in skill.get('scenarios') or [])}".casefold()
        normalized_name = normalize_name(str(skill.get("name") or ""))
        hits = [
            term for term in terms
            if term in normalized_name or (term in {"ocr", "paddle", "paddleocr"} and term_matches(description, term))
        ]
        if not hits:
            continue
        name = str(skill.get("name") or "")
        name_hits = [term for term in hits if term in normalized_name]
        matches.append(
            {
                "name": name,
                "score": min(40, len(hits) * 8 + (12 if name_hits else 0)),
                "reasons": [*(f"capability_name_match:{term}" for term in name_hits[:2]), *(f"capability_description_match:{term}" for term in hits[:4])],
                "path": str(skill.get("path") or ""),
                "source": str(skill.get("source") or ""),
                "admission_state": "unregistered",
    ...
    if matches:
        best_score = int(matches[0].get("score") or 0)
        matches = [item for item in matches if int(item.get("score") or 0) >= max(8, best_score // 2)]
    return matches[: max(1, min(int(limit or 8), 16))]
...
def prepare_readonly_routing_context(*, myskills_inventory: str = "") -> dict[str, Any]:
    """Build routing context from current owner state without refreshing it."""

    inventory = merged_inventory(myskills_inventory)
    records = []
    for row in skill_lifecycle_state.active_rows().values():
        record = dict(row.get("record") or {})
        record.update(
            {key: row.get(key) for key in ("path", "source", "name", "skill_id", "admission_state", "trust_state")}
        )
        record.setdefault("routing_eligible", True)
    ...
    return {
        "lifecycle_refresh": {
            "records": records,
            "summary": {"discovered_count": len(records), "change_count": 0, "read_only": True},
            "state_db": str(skill_lifecycle_state.STATE_DB),
        },
        "inventory": inventory,
        "lifecycle_records_by_path": {
            normalize_path(str(row.get("path") or "")): row for row in records
        },
        "quality_by_key": quality_by_key,
        "discovered_skill_keys": {normalize_name(str(row.get("name") or "")) for row in records},
        "myskills_inventory_path": myskills_inventory_path,
    }
...
def build_plan(
    ...
    *,
    ...
    return {
        ...
        "capability_candidates": capability_matches(message, inventory=inventory),
        "selection_policy": {
        ...
        "tools": {
            ...
            "matrix": str(MATRIX),
            "matrix_lookup": "python _bridge\\mcp_capability_routes.py lookup --terms <terms>",
            "rule": "Use the route lookup first and expand only the matching matrix section; then use the owning MCP/tool and record current-turn failures separately.",
        },
...
def record_usage(
    ...
    normalized_event_key = normalize_name(event_key)[:120]
    event_prefix = "usage:" + (normalized_event_key or uuid.uuid4().hex)
...
def validate(myskills_inventory: str = "") -> dict[str, Any]:
    samples = [
        ...
        "Huawei 平板 MTP 被占用，WSL USB/IP 返回 Device busy",
        "刚刚主账号收到了三条桥接信息",
        "邮箱收件箱待处理回信任务",
        "记忆 note 吸收和 PMB 整理",
        ...
        "把文章生成十页演示文稿",
        "创建一套 Windows PowerPoint 课件并检查中文字体",
        "帮我做一套二年级语文课件",
    ...
    specialized_expectations = {
        "Huawei 平板 MTP 被占用，WSL USB/IP 返回 Device busy": "windows-usb-ops",
        ...
        "创建一套 Windows PowerPoint 课件并检查中文字体": "baoyu-slide-deck",
        "帮我做一套二年级语文课件": "baoyu-slide-deck",
        ...
        "审查当前工作树中的代码变更并找出回归风险": "open-code-review-delegate",
    }
    ...
    membership_domains = system_membership.domain_binding_report(
        skill_domains={domain.key for domain in DOMAINS},
    )
    checks = [
        {
        ...
        {
            "name": "all_system_members_have_discoverable_skill_domains",
            "ok": membership_domains.get("ok") is True,
            "detail": membership_domains,
        },
    ]
    return {
...
def main(argv: list[str] | None = None) -> int:
    ...
    record.add_argument("--event-key", default="", help="Stable idempotency key for one already-identified task outcome.")
