...
def _route_documentation_url() -> RouteDecision:
    return RouteDecision(
        ...
        reasons=["a known documentation URL should return readable page content instead of restarting source discovery"],
        risks=[],
        notes=[],
    )
...
def _route_unknown_source(*, task_text: str, intent: str) -> RouteDecision:
    ...
    entity_discovery_terms = (
        "电影", "影片", "影视", "视频", "片名", "观看", "在线观看", "正版", "租赁", "购买", "流媒体", "平台",
        "豆瓣", "腾讯视频", "爱奇艺", "优酷", "芒果tv", "哔哩哔哩",
        "movie", "film", "watch", "streaming", "rent", "rental", "buy", "purchase", "platform",
    )
    ...
    if intent == ResourceIntent.PACKAGE_DEPENDENCY:
        return RouteDecision(
            primary="package_manager",
            intent=ResourceIntent.PACKAGE_DEPENDENCY,
            stage=ResourceStage.AUDIT,
            secondary=[],
            reasons=["explicit package/dependency intent takes precedence over version or documentation discovery"],
    ...
    if matched_terms(task_text, entity_discovery_terms):
        return RouteDecision(
            primary="resource_router",
            intent=ResourceIntent.EXTERNAL_DEPENDENCY,
            stage=ResourceStage.DISCOVER,
            secondary=[],
            reasons=["entity or platform discovery is incompatible with library documentation resolution"],
            risks=["external_source_selection"],
            notes=["Treat declared documentation intent as a hint when entity-discovery semantics contradict it."],
        )
    ...
    if matched_terms(task_text, dependency_terms):
        return RouteDecision(
            primary="package_manager",
            intent=ResourceIntent.PACKAGE_DEPENDENCY,
            stage=ResourceStage.AUDIT,
