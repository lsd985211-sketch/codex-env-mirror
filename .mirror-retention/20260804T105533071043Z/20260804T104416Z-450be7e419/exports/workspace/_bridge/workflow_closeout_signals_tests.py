...
class WorkflowCloseoutSignalsTests(unittest.TestCase):
    def test_identical_closeout_rerun_reuses_publish_receipt_and_performs_zero_mutations(self) -> None:
        source_state = {"changed_files": ["AGENTS.md"], "work_git_head": "abc", "bare_head": "abc"}
        receipt = signals.terminal_publish_receipt(
            convergence_id="c1",
            intent_id="i1",
        ...
        with (
            ...
            payload = signals.converge_closeout_actions(
                {"ok": True, "pre_owner_reconciliation": {"ok": True, "work_git_finalization": {"ok": True, "paths": []}}},
        ...
        self.assertEqual(payload["post_closeout_mirror"]["decision"], "reuse")
    def test_matching_terminal_publish_receipt_is_consumed_without_publish(self) -> None:
        source_state = {"changed_files": ["AGENTS.md"], "work_git_head": "abc", "bare_head": "abc"}
        receipt = signals.terminal_publish_receipt(
            convergence_id="c1",
            intent_id="i1",
        ...
        publish.assert_not_called()
        self.assertEqual(payload["post_closeout_mirror"]["decision"], "reuse")
        self.assertEqual(payload["terminal_convergence"]["next_action"]["decision"], "reuse")

    def test_read_only_acceptance_uses_status_and_never_publish(self) -> None:
        with (
            patch.object(codex_environment_mirror, "publish") as publish,
            patch.object(
        ...
        publish.assert_not_called()
        status.assert_called_once_with(force_fresh=True)
        self.assertEqual(payload["post_closeout_mirror"]["decision"], "verify")

    def test_source_change_after_publish_blocks_without_loop(self) -> None:
        payload = signals.converge_closeout_actions(
            {"ok": True},
            changed_files=["AGENTS.md"],
            apply=True,
            ...
            convergence_id="c1",
            intent_id="i1",
            source_state={"head": "new"},
            published_signature="old",
        )
        self.assertFalse(payload["ok"])
        self.assertEqual(payload["blocked_reason"], "terminal_signature_changed_after_mutation")
        self.assertEqual(payload["terminal_convergence"]["first_invalid_action"], "mirror.publish")
        self.assertFalse(payload["terminal_convergence"]["automatic_loop_allowed"])
    ...
    def test_post_closeout_mirror_publishes_after_successful_finalization(self) -> None:
        ...
        terminal = {"ok": True, "result": owner_result}
        with patch.object(codex_environment_mirror, "finalize_and_publish", return_value=terminal) as publish:
    ...
    def test_post_closeout_commits_generated_finalization_before_publish(self) -> None:
        ...
        with (
            ...
            patch.object(codex_environment_mirror, "finalize_and_publish", side_effect=lambda *_args, **_kwargs: events.append("publish") or {"ok": True, "result": {"ok": True}}),
        ):
            payload = signals.apply_post_closeout_mirror(
                {"ok": True},
    ...
    def test_post_closeout_reuses_pre_owner_git_handoff(self) -> None:
        ...
        with (
            patch.object(signals, "commit_generated_finalization") as commit,
            patch.object(codex_environment_mirror, "finalize_and_publish", return_value={"ok": True, "result": {"ok": True}}) as publish,
        ):
            payload = signals.apply_post_closeout_mirror(
                finalization,
                changed_files=["workspace/_bridge/example.py"],
    ...
    def test_single_active_workspace_thread_auto_selects_with_message_receipt(self) -> None:
        handoff = signals.concurrent_closeout_handoff(
            ...
            active_workspace_threads=[
                '{"threadId":"thread-active","status":"active","sameRepository":true,"currentTask":false}'
            ],
            revalidated_workspace_threads=[
                '{"threadId":"thread-active","status":"active","sameRepository":true,"currentTask":false}'
            ],
    ...
    def test_multiple_active_workspace_threads_require_explicit_target(self) -> None:
        handoff = signals.concurrent_closeout_handoff(
            current_task_complete=True,
            active_workspace_threads=[
                '{"threadId":"thread-a","status":"active","sameRepository":true,"currentTask":false}',
                '{"threadId":"thread-b","status":"active","sameRepository":true,"currentTask":false}',
            ],
    ...
    def test_current_thread_is_never_a_handoff_target(self) -> None:
        handoff = signals.concurrent_closeout_handoff(
            current_task_complete=True,
            current_thread_id="thread-self",
            handoff_target_thread="thread-self",
            active_workspace_threads=[
                '{"threadId":"thread-self","status":"active","sameRepository":true,"currentTask":false}',
                '{"threadId":"thread-other","status":"active","sameRepository":true,"currentTask":false}',
            ],
            revalidated_workspace_threads=[
                '{"threadId":"thread-other","status":"active","sameRepository":true,"currentTask":false}'
            ],
        ...
        self.assertFalse(handoff["ok"])
        self.assertEqual(handoff["active_workspace_threads"], ["thread-other"])
        self.assertEqual(handoff["blockers"][0]["code"], "handoff_target_is_current_thread")

    def test_handoff_without_current_thread_identity_preserves_other_target_dispatch(self) -> None:
        with patch.dict(os.environ, {"CODEX_THREAD_ID": ""}, clear=False):
            handoff = signals.concurrent_closeout_handoff(
                current_task_complete=True,
                active_workspace_threads=[
                    '{"threadId":"thread-other","status":"active","sameRepository":true,"currentTask":false}'
                ...
                handoff_receipts=["thread_id=thread-other"],
            )
        self.assertTrue(handoff["ok"])
        self.assertTrue(handoff["required"])
        self.assertEqual(handoff["target_thread"], "thread-other")

    def test_environment_current_thread_id_excludes_untrusted_self_candidate(self) -> None:
        with patch.dict(os.environ, {"CODEX_THREAD_ID": "thread-self"}, clear=False):
            handoff = signals.concurrent_closeout_handoff(
                current_task_complete=True,
                active_workspace_threads=[
                    '{"threadId":"thread-self","status":"active","sameRepository":true,"currentTask":false}'
        ...
        self.assertEqual(handoff["active_workspace_threads"], [])
        self.assertEqual(handoff["target_thread"], "")

    def test_handoff_requires_a_second_send_time_thread_snapshot(self) -> None:
        handoff = signals.concurrent_closeout_handoff(
            current_task_complete=True,
            active_workspace_threads=[
                '{"threadId":"thread-active","status":"active","sameRepository":true,"currentTask":false}'
        ...
        self.assertFalse(handoff["ok"])
        self.assertTrue(handoff["send_time_revalidation_required"])
        self.assertFalse(handoff["send_time_revalidation_supplied"])
        self.assertEqual(handoff["blockers"][0]["code"], "handoff_live_revalidation_required")

    def test_closed_target_between_discovery_and_dispatch_is_not_woken(self) -> None:
        handoff = signals.concurrent_closeout_handoff(
            current_task_complete=True,
            active_workspace_threads=[
                '{"threadId":"thread-ended","status":"active","sameRepository":true,"currentTask":false}'
            ],
            revalidated_workspace_threads=[
                '{"threadId":"thread-ended","status":"completed","sameRepository":true,"currentTask":false}'
            ],
        )
        self.assertFalse(handoff["ok"])
        self.assertEqual(handoff["send_time_active_workspace_threads"], [])
        self.assertEqual(handoff["blockers"][0]["code"], "handoff_target_not_active_at_send")
    ...
    def test_closeout_uses_explicit_delivery_thread_when_environment_is_unavailable(self) -> None:
        def optional(**_kwargs):
            return {
                "profile_candidates": {},
                "external_candidates": {},
        ...
        prepared = {
            "ok": True,
            "packages": [{"package_id": "review-package:test", "package_signature": "signature", "topic": "proposals", "count": 1, "item_refs": []}],
        }
        envelope = {
            "ok": True,
        ...
        with patch.object(codex_workflow_entry, "optional_closeout_sections", side_effect=optional), \
                ...
                patch.object(codex_workflow_entry, "sync_review_groups", return_value=[]), \
                patch.object(codex_workflow_entry, "build_review_summary", return_value={}), \
                patch.object(codex_workflow_entry, "resolve_rollout_path", return_value={"ok": False, "reason": "not_found"}) as resolve, \
                patch.object(codex_workflow_entry, "prepare_delivery_packages", return_value=prepared), \
                patch.object(codex_workflow_entry, "prepare_delivery_envelope", return_value=envelope) as prepare:
            codex_workflow_entry.closeout(
                task_kind="workflow_governance",
                outcome="ok",
                delivery_thread_id="thread-explicit",
            )

        resolve.assert_called_once_with("thread-explicit")
        self.assertEqual(prepare.call_args.kwargs["target_ref"], "thread:thread-explicit")

    def test_idle_or_not_loaded_threads_do_not_trigger_handoff(self) -> None:
        handoff = signals.concurrent_closeout_handoff(
            current_task_complete=True,
            active_workspace_threads=[
                '{"threadId":"thread-idle","status":"idle","sameRepository":true,"currentTask":false}',
                '{"threadId":"thread-cold","status":"notLoaded","sameRepository":true,"currentTask":false}',
            ],
            handoff_receipts=['{"threadId":"thread-idle"}'],
        )
        self.assertFalse(handoff["required"])
        self.assertEqual(handoff["active_workspace_threads"], [])
        self.assertEqual(
            {item["thread_id"]: item["reason"] for item in handoff["ineligible_thread_facts"]},
            {"thread-idle": "status_not_active", "thread-cold": "status_not_active"},
        )

    def test_bare_thread_id_is_not_accepted_as_live_state_evidence(self) -> None:
        handoff = signals.concurrent_closeout_handoff(
            current_task_complete=True,
            active_workspace_threads=["thread-claimed-active"],
            handoff_receipts=['{"threadId":"thread-claimed-active"}'],
        )
        self.assertFalse(handoff["required"])
        self.assertEqual(handoff["active_workspace_threads"], [])
        self.assertEqual(handoff["ineligible_thread_facts"][0]["reason"], "live_state_evidence_required")

    def test_message_receipt_does_not_prove_thread_activity_or_handoff_agreement(self) -> None:
        handoff = signals.concurrent_closeout_handoff(
            current_task_complete=True,
            active_workspace_threads=[
                '{"threadId":"thread-idle","status":"idle","sameRepository":true,"currentTask":false}'
        ...
        self.assertFalse(handoff["required"])
        self.assertFalse(handoff["receipt_proves_activity"])
        self.assertFalse(handoff["receipt_proves_agreement"])
    ...
    def test_closeout_runs_self_update_before_post_mirror_publish(self) -> None:
        ...
        with patch.object(codex_workflow_entry, "optional_closeout_sections", side_effect=optional), \
                ...
                patch.object(codex_workflow_entry, "converge_closeout_actions", side_effect=mirror), \
                patch.object(codex_workflow_entry, "capture_iteration_candidates", return_value={}), \
                patch.object(codex_workflow_entry, "build_closeout_package", side_effect=package), \
                patch.object(codex_workflow_entry, "sync_review_groups", return_value=[]), \
                patch.object(codex_workflow_entry, "build_review_summary", return_value={}):
    ...
    def test_closeout_projection_preserves_concurrent_handoff(self) -> None:
        payload = codex_workflow_entry.closeout_cli_projection({
            ...
            "finalization": {
                ...
                "concurrent_handoff": {
                    ...
                    "required": True,
                    "complete": True,
                    "ineligible_thread_facts": [
                        {"thread_id": "thread-idle", "reason": "status_not_active"}
                    ],
                    "target_thread": "thread-active",
                    "receipt_proves_activity": False,
                    "receipt_proves_agreement": False,
        ...
        self.assertEqual(handoff["ineligible_thread_facts"][0]["thread_id"], "thread-idle")
        self.assertFalse(handoff["receipt_proves_activity"])
        self.assertFalse(handoff["receipt_proves_agreement"])
        self.assertTrue(handoff["complete"])
        self.assertEqual(handoff["target_thread"], "thread-active")
