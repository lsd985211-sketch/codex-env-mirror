...
from shared import scheduler_authorization  # noqa: E402
...
RETRY_EXHAUSTED_COOLDOWN_SECONDS = 6 * 60 * 60
DEFAULT_TASKS: list[dict[str, Any]] = [
    {
        "id": "resource_transfer_convergence",
        "name": "耐久资源传输收敛协调",
        "enabled": True,
        "trigger": {"type": "interval", "every_seconds": 300},
        "action": {
            "type": "command",
            "command": ["python", "_bridge/resource_transfer_owner.py", "reconcile", "--limit", "20"],
        },
        "policy": {
            "mode": "resource-transfer-convergence",
            "risk": "low",
            "timeout_seconds": 90,
            ...
            "allowed_effect": "wake the single transfer owner to requeue only expired unstarted leases or consume an existing durable terminal receipt; never start a new transfer, download bytes, infer authorization, or create another timer or retry loop",
        },
    },
    {
    ...
    {
        "id": "codex_update_intelligence_periodic_scan",
        "name": "Codex 更新情报每日只读扫描",
        "enabled": True,
        "trigger": {"type": "daily", "at": "03:40"},
        "action": {
            "type": "command",
            "command": [
                "python",
                "_bridge/dependency_change_intelligence.py",
                "scan",
                "--profile",
                "codex",
                "--trigger",
        ...
        "policy": {
            "mode": "read-only-intelligence-and-approval-proposal",
            "risk": "low",
            "timeout_seconds": 120,
            "retry_interval_seconds": 3600,
            "max_retry_count": 2,
            "latest_lag_seconds": 79200,
            "retry_exhausted_action": "record_and_continue",
            "allowed_effect": "fetch official read-only evidence, refresh derived indexes, run bounded read-only probes, and generate Chinese-first approval proposals; never apply repairs or advance last_validated",
        },
    },
    {
        "id": "workflow_observation_iteration_daily",
        "name": "Codex观测模式每日审查候选",
        "enabled": True,
        "trigger": {"type": "daily", "at": "04:10"},
        "action": {
            "type": "command",
            "command": [
                "python",
                "_bridge/workflow_observation_iteration.py",
                "apply",
                "--confirm",
                "APPLY-OBSERVATION-PROPOSALS",
            ],
        },
        "policy": {
            "mode": "derived-observation-proposal-only",
            "risk": "low",
            "timeout_seconds": 90,
            "retry_interval_seconds": 3600,
            "max_retry_count": 2,
            "latest_lag_seconds": 79200,
            "retry_exhausted_action": "record_and_continue",
            "allowed_effect": "read bounded observer events and upsert pending review candidates; never write work notes, PMB, rules, skills, config, or apply remediation",
        },
    },
    {
        "id": "workflow_review_approved_consumer",
        "name": "Codex已批准审阅项生命周期消费",
        "enabled": True,
        "trigger": {"type": "interval", "every_seconds": 900},
        "action": {
            "type": "command",
        ...
        "policy": {
            "mode": "approved-memory-owner-lifecycle-only",
            "risk": "low",
            "timeout_seconds": 90,
            "retry_interval_seconds": 900,
            "max_retry_count": 2,
            "latest_lag_seconds": 1800,
            "retry_exhausted_action": "record_and_continue",
            "allowed_effect": "consume only review items already approved through revision-bound decision evidence; apply through memory_governance with backup, validate readback, then resolve; never infer approval or write rules, skills, config, or business state",
        },
    },
    {
        "id": "maintenance_convergence_shadow_scan",
        "name": "全局维护按需收敛影子计划",
        "enabled": True,
        "trigger": {"type": "interval", "every_seconds": 3600},
        "action": {
            ...
            "command": [
                ...
                "shadow-scan",
                "--message",
                "scheduled global maintenance coverage",
                "--signal",
                "coverage.freshness",
            ],
        },
        "policy": {
            "mode": "maintenance-convergence-shadow",
            "risk": "low",
            "timeout_seconds": 90,
            "retry_interval_seconds": 1800,
            "max_retry_count": 2,
            "latest_lag_seconds": 10800,
            "retry_exhausted_action": "record_and_continue",
            "allowed_effect": "derive and persist one bounded maintenance plan artifact; never execute owner repair commands, infer approval, or create a second scheduler loop",
        },
    },
    {
...
def get_due_reason(task: dict[str, Any], task_state: dict[str, Any], now: datetime) -> str | None:
    ...
    max_retry_count = int(policy.get("max_retry_count") or 0)
    retry_exhausted = task_state.get("last_status") == "retry_exhausted" or (
        task_state.get("last_status") == "failed" and retry_count > max_retry_count
    )
    ...
    if trigger_type == "interval":
        ...
        if every <= 0:
            return None
        if retry_exhausted:
            exhausted_at = parse_iso(task_state.get("retry_exhausted_at")) or last_attempt
            if exhausted_at is None:
                return None
            if (now - exhausted_at).total_seconds() < RETRY_EXHAUSTED_COOLDOWN_SECONDS:
                return None
            return "retry_exhausted_recovery"
        if task_state.get("last_status") == "failed" and last_attempt:
...
def run_task(task: dict[str, Any], due_reason: str, dry_run: bool) -> TaskRun:
    ...
    started = now_bj()
    if dry_run:
        authorization = scheduler_authorization.authorization_plan(task, due_reason=due_reason)
        run = TaskRun(
            task_id=task_id,
            ok=True,
            mode=mode,
            ...
            skipped=True,
            stdout_preview=json.dumps({"would_run": command_for_task(task), "authorization": authorization}, ensure_ascii=False),
            authorization={"required": True, "consumed": False, "workflow_semantic_hash": authorization["workflow_semantic_hash"]},
        )
        run.record_path = str(write_run_record(task, run))
        return run

    authorization = scheduler_authorization.authorize(task, due_reason=due_reason)
    if not authorization.get("ok"):
        run = TaskRun(
            task_id=task_id, ok=False, mode=mode, due_reason=due_reason,
            started_at=iso(started), finished_at=iso(now_bj()), skipped=True,
            error=f"authorization blocked: {authorization.get('reason', 'unknown')}",
    ...
    try:
        ...
        run = TaskRun(
            ...
            authorization={"permit_ref": authorization.get("permit_ref"), "operation_id": authorization.get("operation_id"), "consumed": True},
        )
    ...
    run.record_path = str(write_run_record(task, run))
    effect = scheduler_authorization.finish(authorization, ok=run.ok, receipt_ref=run.record_path)
    if run.authorization is not None:
        run.authorization["effect_status"] = effect.get("status", effect.get("reason", ""))
    return run
...
def validate() -> dict[str, Any]:
    ...
    convergence_tasks = [
        task
        for task in tasks
        if "maintenance_convergence_runtime.py" in " ".join(
            str(item) for item in task.get("action", {}).get("command", [])
        )
    ]
    if len(convergence_tasks) != 1:
        issues.append(f"maintenance convergence dispatcher count must be 1, got {len(convergence_tasks)}")
    elif convergence_tasks[0].get("id") != "maintenance_convergence_shadow_scan":
        issues.append("maintenance convergence dispatcher has unexpected task id")
    elif convergence_tasks[0].get("policy", {}).get("mode") != "maintenance-convergence-shadow":
        issues.append("maintenance convergence dispatcher must remain shadow-only")
    for task in tasks:
        command_text = " ".join(str(item) for item in task.get("action", {}).get("command", []))
        if "maintenance_upgrade_governance.py" in command_text and "convergence-plan" in command_text:
            issues.append(f"{task.get('id')}: static convergence plan bypasses runtime dispatcher")
    drift = task_drift_snapshot()
    if not drift.get("ok"):
...
def metrics() -> dict[str, Any]:
    ...
    for key, value in state.get("tasks", {}).items():
        ...
        policy = (task or {}).get("policy") if isinstance(task, dict) else {}
        max_retry_count = int((policy or {}).get("max_retry_count") or 0)
        exhausted = str(value.get("last_status") or "") == "retry_exhausted" or (
            str(value.get("last_status") or "") == "failed"
            and int(value.get("retry_count") or 0) > max_retry_count
        )
        if not exhausted or not isinstance(task, dict):
            continue
        if str((task.get("trigger") or {}).get("type") or "") != "interval":
            continue
        if get_due_reason(task, value, now) != "retry_exhausted_recovery":
            continue
        retry_storm_candidates.append({
            "id": key,
    ...
    return {
        ...
        "retry_exhausted_cooldown_seconds": RETRY_EXHAUSTED_COOLDOWN_SECONDS,
...
def retry_storm_check() -> dict[str, Any]:
    ...
    stale_attempt = iso(now - timedelta(seconds=RETRY_EXHAUSTED_COOLDOWN_SECONDS + 1))
    task = {
    ...
    cases = {
        ...
        "exhausted_elapsed_due_recovery": get_due_reason(task, exhausted_elapsed, now),
    ...
    ok = (
        ...
        and cases["exhausted_elapsed_due_recovery"] == "retry_exhausted_recovery"
    ...
    return {
        ...
        "temp_only": True,
        "cases": cases,
        "assertion": "retry-exhausted interval tasks wait for the bounded cooldown; manual retry remains available",
    }
