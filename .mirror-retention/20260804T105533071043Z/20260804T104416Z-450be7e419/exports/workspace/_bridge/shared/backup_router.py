...
if __package__:
    ...
    from json_cli import now_iso
from platform_paths import host_accessible_path
...
def find_git_root(path: Path) -> Path | None:
    """Return the enclosing, valid Git worktree without trusting marker presence."""
...
def validate_manifest(path: Path) -> dict[str, Any]:
    ...
    for item in items:
        ...
        if not backup_value:
            ...
            continue
        backup_path = host_accessible_path(backup_value, platform_name=os.name)
...
def validate(root: str = "", manifest: str = "") -> dict[str, Any]:
    if manifest:
        ...
        if root:
            ...
            if not is_relative_to(manifest_path, base.resolve()):
                return {
        ...
        item = validate_manifest(manifest_path)
        return {
            "schema": "backup_router.validate.v2",
            "ok": bool(item.get("ok")),
            "generated_at": now_iso(),
            "root": str(Path(root).expanduser().resolve()) if root else str(manifest_path.parent),
            "manifest_path": str(manifest_path),
            "manifest_count": 1,
            "skipped_manifest_count": int(bool(item.get("skipped"))),
            "failure_count": int(not item.get("ok")),
...
def main() -> int:
    ...
    p_validate.add_argument("--manifest", default="")
    ...
    if args.command == "plan":
        ...
        payload = validate(args.root, args.manifest)
