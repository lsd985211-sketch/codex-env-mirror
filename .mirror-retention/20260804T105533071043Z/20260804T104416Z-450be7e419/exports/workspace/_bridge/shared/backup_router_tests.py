...
if __package__:
    from . import backup_router
else:
    import backup_router
...
class WorkGitBackupRoutingTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        subprocess.run(["git", "init", "-q", str(self.git_root)], check=True)
    ...
    def test_direct_script_entrypoint_resolves_bridge_dependencies(self) -> None:
        result = subprocess.run(
            [sys.executable, str(Path(backup_router.__file__).resolve()), "--help"],
            cwd=Path(backup_router.__file__).resolve().parents[3],
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
        ...
        self.assertIn("Plan and create routed backups with manifests", result.stdout)

    def test_empty_git_marker_does_not_define_repository(self) -> None:
        false_root = Path(self.temp.name) / "false-root"
        (false_root / ".git").mkdir(parents=True)
        source = false_root / "runtime" / "config.toml"
        source.parent.mkdir()
        source.write_text("model = \"test\"\n", encoding="utf-8")

        self.assertIsNone(backup_router.find_git_root(source))

    def test_top_level_shared_import_resolves_bridge_dependencies(self) -> None:
        bridge_root = Path(backup_router.__file__).resolve().parents[1]
        result = subprocess.run(
            [sys.executable, "-c", "from shared import backup_router; print(backup_router.BRIDGE_ROOT)"],
            cwd=bridge_root.parent,
            env={"PYTHONPATH": str(bridge_root)},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
        ...
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertEqual(result.stdout.strip(), str(bridge_root))
    ...
    def test_validate_accepts_create_manifest_path_and_rejects_disagreeing_root(self) -> None:
        source = self.bridge_root / "owner.py"
        source.write_text("owner = True\n", encoding="utf-8")
        created = backup_router.create_backup([str(source)], category="owner", remark="manifest-cli")
        manifest = created["manifest_paths"][0]

        direct = backup_router.validate(manifest=manifest)
        disagreeing = backup_router.validate(str(self.git_root), manifest=manifest)

        self.assertTrue(direct["ok"], direct)
        self.assertEqual(manifest, direct["manifest_path"])
        self.assertFalse(disagreeing["ok"])
        self.assertEqual("manifest_outside_root", disagreeing["reason"])
