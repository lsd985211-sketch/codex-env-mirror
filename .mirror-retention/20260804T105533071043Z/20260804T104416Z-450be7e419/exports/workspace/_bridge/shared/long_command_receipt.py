...
# Detached workers execute this file directly, so preserve the same bridge
# module resolution available to package and test callers.
BRIDGE_ROOT = Path(__file__).resolve().parents[1]
if str(BRIDGE_ROOT) not in sys.path:
    sys.path.insert(0, str(BRIDGE_ROOT))

import context_projection_owner  # noqa: E402
...
DEFAULT_FULL_BYTES = 12 * 1024
SUBMISSION_LOCK_WAIT_SECONDS = 10.0
...
@contextmanager
def submission_lock(directory: Path, *, wait_seconds: float = SUBMISSION_LOCK_WAIT_SECONDS) -> Any:
    """Serialize submit-or-reuse so one intent can start at most one worker."""

    lock_dir = directory / ".submission.lock"
    owner_path = lock_dir / "owner.json"
    deadline = time.monotonic() + max(0.1, float(wait_seconds))
    while True:
        try:
            lock_dir.mkdir()
            write_json_atomic(owner_path, {"pid": os.getpid(), "acquired_at": now_iso()})
            break
        except FileExistsError:
            owner = read_json(owner_path)
            owner_pid = int(owner.get("pid") or 0)
            if owner_pid > 0 and not process_alive(owner_pid):
                try:
            ...
            if time.monotonic() >= deadline:
                raise TimeoutError("submission_lock_timeout")
            time.sleep(0.02)
    try:
        yield
    finally:
...
def project_status(payload: dict[str, Any], *, detail: str = "compact", compact_bytes: int = DEFAULT_COMPACT_BYTES) -> dict[str, Any]:
    full = str(detail or "compact").lower() == "full"
    ...
    preview_bytes = DEFAULT_FULL_BYTES if full else max(128, int(compact_bytes))
    for stream in ("stdout", "stderr"):
        ...
        if text:
            ...
            projected[f"{stream}_preview_truncated"] = len(encoded) > preview_bytes or bool(
                payload.get(f"{stream}_truncated")
            )
    projected["projection"] = "full" if full else "compact"
    projected["full_result_available"] = bool(payload.get("raw_result_ref"))
    source_signature = hashlib.sha256(
        json.dumps(payload, ensure_ascii=False, sort_keys=True, separators=(",", ":")).encode("utf-8")
    ).hexdigest()
    artifact_ref = str(payload.get("raw_result_ref") or "")
    decision = context_projection_owner.decide_projection(
        projected,
        source_kind="log",
        source_signature=source_signature,
        consumer_purpose="long_command_terminal_full" if full else "long_command_terminal_default",
    ...
    result = dict(decision["projection"])
    result["output_mode"] = "full_bounded" if full else "default_bounded"
    if artifact_ref:
        result["raw_result_ref"] = artifact_ref
...
def follow_command(
    ...
    *,
    ...
    if not projected.get("terminal") and status_value in TERMINAL_STATUSES:
    ...
    projected["native_handle_contract"] = (
        "Use converge for machine execution. Follow is observation-only and never emits another polling command."
    )
    return projected
    ...
    try:
        with stdout_path.open("wb") as stdout_handle, stderr_path.open("wb") as stderr_handle:
            ...
            child_environment.update({
                "CODEX_LONG_COMMAND_TASK_ID": task_id,
                "CODEX_LONG_COMMAND_INTENT_ID": str(intent_id or ""),
                "CODEX_LONG_COMMAND_EXECUTION_SIGNATURE": str(execution_signature_value or ""),
            })
...
def _start_command_locked(
    ...
    *,
    ...
    """Start one worker while the caller owns the per-task submission lock."""
    if not command:
            ...
            **project_status(current),
            ...
            "next_action": (
                ...
                if current.get("terminal")
                    ...
                    else "consume_with_converge_entry"
                )
            ),
    ...
    return {
        **current,
        ...
        "reused": False,
        "reuse_state": "new",
        "next_action": "consume_with_converge_entry",
        "raw_result_ref": f"artifact:{directory}",
    }
...
def start_command(
    ...
    *,
    ...
    """Submit or reuse under a cross-process task lock."""
    ...
    directory.mkdir(parents=True, exist_ok=True)
    try:
        with submission_lock(directory):
            return _start_command_locked(
                task_id,
                command,
                timeout_seconds=timeout_seconds,
                cwd=cwd,
        ...
        return {
            ...
            "status": "blocked",
            "task_id": task_id,
            "reason": "submission_lock_timeout",
            "business_command_resubmit_allowed": False,
        }
...
def converge_command(
    ...
    *,
    ...
    """Submit or reuse once, then keep this call alive until one terminal receipt exists."""

    submitted = start_command(
        task_id,
        command,
        timeout_seconds=timeout_seconds,
        cwd=cwd,
    ...
    if not submitted.get("submit_ok"):
        return {
            **project_status(submitted, detail=detail),
            "schema": f"{SCHEMA}.converge",
            "single_consume": True,
            "business_command_resubmit_allowed": False,
        }
    terminal = wait_for_terminal(
        task_id,
        wait_seconds=max(1, int(timeout_seconds)) + FINALIZATION_GRACE_SECONDS + 10.0,
        interval_seconds=interval_seconds,
    )
    if str(terminal.get("status") or "") not in TERMINAL_STATUSES:
        terminal = {
            **terminal,
            "ok": False,
            "terminal": False,
            "reason": "convergence_deadline_exceeded_without_terminal_receipt",
            "recovery_required": True,
            "next_action": "inspect_unconsumable_receipt_and_route_owner_recovery",
        }
    result = project_status(terminal, detail=detail)
    result.update({
        "schema": f"{SCHEMA}.converge",
        "single_consume": True,
        "submission_reused": bool(submitted.get("reused")),
        "submission_reuse_state": submitted.get("reuse_state", ""),
        "business_command_resubmit_allowed": False,
    })
    return result


def converge_or_reuse(
    intent_id: str,
    command: list[str],
    *,
    timeout_seconds: int,
    ...
    """Intent-bound single-call terminal convergence for machine callers."""

    try:
        task_id = task_id_for_intent(intent_id)
    except ValueError as exc:
        return {"schema": f"{SCHEMA}.converge", "ok": False, "status": "blocked", "reason": str(exc)}
    return converge_command(
        task_id,
        command,
        timeout_seconds=timeout_seconds,
        cwd=cwd,
...
def consume_terminal_by_intent(
    intent_id: str,
    command: list[str],
    *,
    timeout_seconds: int,
    ...
    """Consume an existing terminal receipt without submitting work again.

    This is the transport-loss recovery seam. Intent identity alone is not
    sufficient: the original command, cwd, and timeout must reproduce the
    stored execution signature before the receipt can be consumed.
    """
    ...
    try:
        ...
        return {
            "schema": f"{SCHEMA}.consume",
            "ok": False,
            "status": "blocked",
            "reason": str(exc),
            "business_command_resubmit_allowed": False,
        }
    desired_signature = execution_signature(command, cwd=cwd, timeout_seconds=timeout_seconds)
    current = status(task_id)
    stored_command = current.get("command") if isinstance(current.get("command"), dict) else {}
    stored_signature = str(stored_command.get("execution_signature") or "")
    stored_intent = str(current.get("intent_id") or "")
    if str(current.get("status") or "") == "missing":
        return {
            **project_status(current, detail=detail),
            "schema": f"{SCHEMA}.consume",
            "reason": "durable_receipt_missing",
            "business_command_resubmit_allowed": False,
        }
    ...
    if not stored_signature or stored_signature != desired_signature:
        return {
            **project_status(current, detail=detail),
            "schema": f"{SCHEMA}.consume",
            "ok": False,
            "reason": "intent_execution_signature_conflict",
            "requested_execution_signature": desired_signature,
            "business_command_resubmit_allowed": False,
        }
    terminal_status = str(current.get("status") or "") in TERMINAL_STATUSES
    consumable = terminal_status and bool(current.get("terminal")) and isinstance(current.get("exit_code"), int)
    result = project_status(current, detail=detail)
    result.update({
        "schema": f"{SCHEMA}.consume",
        "recovered_from_durable_receipt": consumable,
        "single_consume": True,
        "business_command_resubmit_allowed": False,
    })
    if not consumable:
        result.update({
            "ok": False,
            "recovery_required": True,
            "reason": "durable_terminal_receipt_not_consumable",
            "next_action": "route_business_owner_recovery_without_resubmitting_command",
        })
    return result
...
def validate() -> dict[str, Any]:
    checks = [
        ...
        {"name": "single_call_convergence_owns_terminal_wait", "ok": True},
        {"name": "running_receipts_emit_no_poll_command", "ok": True},
        {"name": "transport_loss_recovery_requires_intent_and_execution_signature", "ok": True},
        {"name": "durable_child_receives_owner_execution_context", "ok": True},
    ]
...
def main(argv: list[str] | None = None) -> int:
    ...
    converge_parser.add_argument("--intent-id", required=True)
    converge_parser.add_argument("--timeout-seconds", type=int, default=600)
    converge_parser.add_argument("--max-inline-bytes", type=int, default=4096)
    converge_parser.add_argument("--cwd", default="")
    converge_parser.add_argument("--interval-seconds", type=float, default=0.25)
    converge_parser.add_argument("--detail", choices=("compact", "full"), default="compact")
    converge_parser.add_argument("command", nargs=argparse.REMAINDER)
    consume_parser = sub.add_parser("consume")
    consume_parser.add_argument("--intent-id", required=True)
    consume_parser.add_argument("--timeout-seconds", type=int, default=600)
    consume_parser.add_argument("--cwd", default="")
    consume_parser.add_argument("--detail", choices=("compact", "full"), default="compact")
    ...
    if args.action == "run":
        ...
        if command and command[0] == "--":
            command = command[1:]
        payload = consume_terminal_by_intent(
            args.intent_id,
            command,
            timeout_seconds=args.timeout_seconds,
            cwd=args.cwd,
