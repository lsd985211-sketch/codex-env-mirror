...
import resource_process_authorization as cleanup_authorization
import resource_process_doctor as doctor
...
class ProcessHostChainTests(unittest.TestCase):
    def test_wsl_resolves_windows_powershell_when_alias_is_missing(self) -> None:
        with patch("shared.windows_powershell.shutil.which", return_value=None):
    ...
    def test_wsl_node_repl_chain_owned_by_codex_desktop_is_not_orphaned(self) -> None:
        processes = {
            10: {"pid": 10, "parent_pid": 20, "command_line": "node_repl.exe"},
            20: {"pid": 20, "parent_pid": 30, "command_line": "wsl.exe -d Codex-Wsl-Lab -- /usr/bin/bash"},
            30: {"pid": 30, "parent_pid": 40, "command_line": "wsl.exe -d Codex-Wsl-Lab"},
            40: {
                "pid": 40,
                "parent_pid": 50,
                "command_line": (
                    r'"C:\Program Files\WindowsApps\OpenAI.Codex_26.715.9079.0_x64__2p2nqsd0c76g0'
                    r'\app\ChatGPT.exe" --remote-debugging-port=9231'
                ),
            },
        }
...
class CleanupAuthorizationTests(unittest.TestCase):
    @staticmethod
    def payload(pid: int = 11) -> dict:
        return {
            "safe_apply": True,
            "include_protected": False,
            "min_age_minutes": 30.0,
            "selected_groups": ["node_repl"],
            "selected": [{"group": "node_repl", "pid": pid, "start_time": "2026-07-30T00:00:00Z", "selection_mode": "old_batch"}],
        }

    def test_scope_is_bound_to_revalidated_candidate_identity(self) -> None:
        first = cleanup_authorization.cleanup_scope(self.payload(11), thread_id="thread-a")
        changed = cleanup_authorization.cleanup_scope(self.payload(12), thread_id="thread-a")

        self.assertNotEqual(first["target_fingerprint"], changed["target_fingerprint"])
        self.assertEqual(first["action"], cleanup_authorization.ACTION)

    def test_apply_requires_grant_before_process_stop(self) -> None:
        context = {
            "plan": {}, "process_by_pid": {}, "now_for_age": None,
            "positive_by_group": {}, "negative_by_group": {},
            "current_turn_observations": {}, "anchor": {},
        }
        with patch.object(doctor, "build_cleanup_selection_context", return_value=context), \
                patch.object(doctor, "select_cleanup_candidates", return_value=(self.payload()["selected"], [])), \
                patch.object(doctor, "cleanup_execution_result") as execute:
            result = doctor.cleanup_orphan_candidates(apply=True, safe_apply=True)

        self.assertFalse(result["ok"])
        self.assertEqual(result["reason"], "scoped_authorization_grant_required")
