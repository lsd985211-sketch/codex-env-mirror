...
def _attempt_result_source_ids(attempts: list[dict[str, Any]]) -> list[str]:
    identities: set[str] = set()
    for attempt in attempts:
        tool = str(attempt.get("tool") or "").strip()
        result = attempt.get("result") if isinstance(attempt.get("result"), dict) else {}
        metadata = result.get("metadata") if isinstance(result.get("metadata"), dict) else {}
        for value in (tool, result.get("source"), metadata.get("source"), metadata.get("owner_tool")):
            text = str(value or "").strip()
            if text:
                identities.add(text)
...
def _attempt_completed_at(attempts: list[dict[str, Any]]) -> dt.datetime | None:
    timestamps: list[dt.datetime] = []
    for attempt in attempts:
        value = str(attempt.get("finished_at") or "").strip()
        if not value:
            continue
        try:
...
def _attempt_completed_deliverables(attempts: list[dict[str, Any]]) -> list[str]:
    values: set[str] = set()
    for attempt in attempts:
        result = attempt.get("result") if isinstance(attempt.get("result"), dict) else {}
        metadata = result.get("metadata") if isinstance(result.get("metadata"), dict) else {}
        for value in metadata.get("completed_deliverables") if isinstance(metadata.get("completed_deliverables"), list) else []:
            text = str(value or "").strip()
            if text:
                values.add(text)
...
def evaluate_item_acceptance(request: ResourceBrokerRequest, receipt: Any, index: int) -> dict[str, Any]:
    ...
    attempts = list(receipt.attempts or [])
    candidate_count = _attempt_result_candidate_count(attempts)
    provenance_count = _attempt_result_provenance_count(attempts)
    source_ids = _attempt_result_source_ids(attempts)
    completed_at = _attempt_completed_at(attempts)
    completed_deliverables = _attempt_completed_deliverables(attempts)
    ...
    source_contract = contract.get("source") if isinstance(contract.get("source"), dict) else {}
    freshness_contract = contract.get("freshness") if isinstance(contract.get("freshness"), dict) else {}
    allowed_sources = [
        str(value)
        for value in (
            acceptance.get("allowed_sources")
            if isinstance(acceptance.get("allowed_sources"), list)
            else source_contract.get("allowed")
            if isinstance(source_contract.get("allowed"), list)
            else []
        )
        if str(value or "").strip()
    ]
    maximum_age_hours = float(
        acceptance.get("maximum_age_hours")
        or freshness_contract.get("maximum_age_hours")
        or freshness_contract.get("max_age_hours")
        or 0
    )
    required_deliverables = [
        str(value)
        for value in acceptance.get("required_deliverables", [])
        if str(value or "").strip()
    ] if isinstance(acceptance.get("required_deliverables"), list) else []
    ...
    if allowed_sources and not set(allowed_sources).intersection(source_ids):
        reasons.append("allowed_source_not_met")
    freshness_age_hours: float | None = None
    if maximum_age_hours > 0:
        if completed_at is None:
            reasons.append("freshness_evidence_missing")
        else:
            freshness_age_hours = max(0.0, (dt.datetime.now(dt.UTC) - completed_at).total_seconds() / 3600)
            if freshness_age_hours > maximum_age_hours:
                reasons.append("freshness_target_not_met")
    missing_deliverables = [value for value in required_deliverables if value not in completed_deliverables]
    if missing_deliverables:
        reasons.append("required_deliverables_not_met")
    ...
    return {
        ...
        "source_id": next((value for value in source_ids if not allowed_sources or value in allowed_sources), ""),
        "freshness_age_hours": round(freshness_age_hours, 3) if freshness_age_hours is not None else None,
...
def semantic_host_key_for_request(request: ResourceBrokerRequest, route: dict[str, Any]) -> str:
    ...
    if host_key != "local":
        return host_key
    source_plan = candidate_source_plan(asdict(request), route, limit=1)
    if source_plan.get("resource_kind") == "academic_paper":
...
def execute_one(
    ...
    *,
    ...
    with class_sem:
        with host_sem:
            with circuit_lock:
                circuit_reason = failure_circuits.get(item.host_key, "")
            if circuit_reason:
                return cancelled_item_result(item, reason=circuit_reason)
            ...
            if item.queue_class == "academic_metadata" and str(receipt.error_class or "") in {
                "academic_backends_unavailable",
                "academic_backends_partially_unavailable",
                "route_misclassification",
            }:
                with circuit_lock:
                    failure_circuits.setdefault(item.host_key, "academic_owner_circuit_open")
...
def batch_status_from_manifest(path: Path) -> dict[str, Any]:
    ...
    for item in results:
        current = dict(item)
        request_manifest = Path(str(item.get("manifest_path") or ""))
        if request_manifest.exists():
            manifest = read_manifest(request_manifest)
            receipt = manifest.get("receipt") if isinstance(manifest.get("receipt"), dict) else {}
            if receipt:
                current.update(
                    status=str(receipt.get("status") or current.get("status") or "unknown"),
                    ok=bool(receipt.get("ok")),
                    result_kind=str(receipt.get("result_kind") or current.get("result_kind") or "none"),
                    next_action=str(receipt.get("next_action") or current.get("next_action") or ""),
                    error_class=str(receipt.get("error_class") or ""),
                    satisfaction=receipt.get("satisfaction") if isinstance(receipt.get("satisfaction"), dict) else current.get("satisfaction", {}),
                )
                consumable = bool(receipt.get("content_ref") or receipt.get("artifact_path") or receipt.get("owner_result"))
                if receipt.get("status") == "completed" and receipt.get("ok") and consumable:
                    current["acceptance"] = {
                        **(current.get("acceptance") or {}),
                        "accepted": True,
                        "reason": "attached_result_consumable",
                        "reasons": [],
                    }
        refreshed.append(current)
    results = refreshed
    accepted_items = [item for item in results if bool((item.get("acceptance") or {}).get("accepted"))]
    deferred_items = [item for item in results if item.get("status") in {"deferred", "handoff_required"}]
    failed_items = [
        item for item in results
        if not bool((item.get("acceptance") or {}).get("accepted"))
        and item.get("status") not in {"deferred", "handoff_required"}
    ]
    required_items = [item for item in results if bool(item.get("required", True))]
    unmet_required = [item for item in required_items if not bool((item.get("acceptance") or {}).get("accepted"))]
    aggregate_status = (
        "completed" if results and not unmet_required
        else "partial" if accepted_items
        else "deferred" if results and len(deferred_items) == len(results)
        else "blocked" if results and all(item.get("status") == "blocked" for item in results)
        else str(payload.get("status") or "failed")
    )
    ...
    return {
        ...
        "ok": True,
        "read_ok": True,
        "batch_ok": aggregate_status == "completed",
        "batch_id": payload.get("batch_id", ""),
        "status": aggregate_status,
        "manifest_path": str(path),
        "request_count": payload.get("request_count", len(planned)),
        "completed_count": len(accepted_items),
        "accepted_count": len(accepted_items),
        "required_count": len(required_items) if results else payload.get("required_count", 0),
        "unmet_required_count": len(unmet_required) if results else payload.get("unmet_required_count", 0),
        "deferred_count": len(deferred_items),
        "failed_count": len(failed_items),
        "accepted_item_ids": [str(item.get("item_id") or "") for item in accepted_items],
        "failed_item_ids": [str(item.get("item_id") or "") for item in failed_items],
        "deferred_item_ids": [str(item.get("item_id") or "") for item in deferred_items],
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max(1, int(config.max_active))) as pool:
            def submit_item(item: ScheduledResource) -> concurrent.futures.Future[dict[str, Any]]:
                return pool.submit(
                    ...
                    failure_circuits=failure_circuits,
