...
class ResourceSearchHealthTests(unittest.TestCase):
    ...
    def test_auto_backends_share_the_total_budget(self) -> None:
        ...
        self.assertEqual(payload["error_class"], "search_backends_returned_no_results")
        self.assertEqual(
            payload["attempted_backends"],
            [
                {"backend": "duckduckgo", "status": "no_results", "error_class": "no_results", "reason": "no results"},
                {"backend": "brave", "status": "no_results", "error_class": "no_results", "reason": "no results"},
            ],
        )

    def test_all_backend_exceptions_are_not_reported_as_no_results(self) -> None:
        class FakeClient:
        ...
        with mock.patch.object(
            ...
            return_value={
                "ok": True,
                "factory": lambda **_kwargs: FakeClient(),
                "dependency_root": "",
                "runtime_available_backends": {"text": ["duckduckgo", "brave"]},
            },
        ):
            payload = search._resource_search_call_in_process("resource_search.text", {"query": "adaptive learning"})
        self.assertFalse(payload["ok"])
        self.assertEqual(payload["status"], "backend_unavailable")
        self.assertEqual(payload["error_class"], "search_backends_unavailable")
        self.assertTrue(all(item["status"] == "backend_unavailable" for item in payload["attempted_backends"]))
