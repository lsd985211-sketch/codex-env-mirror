...
import codex_state_repair_mount_compatibility
...
def expected_marketplace_fields(name: str, spec: dict, *, target: str) -> dict:
    """Keep runtime-owned marketplace fields out of baseline repair writes."""

    external = set(state_write_authority.external_runtime_field_paths(target))
    return {
        key: value
        for key, value in spec.items()
        if ("marketplaces", name, str(key)) not in external
    }
...
def write_repaired_state(
    *,
    ...
    project_text: str,
    global_state_path: Path,
    global_state_text: str | None,
    write_decisions: dict[str, bool],
    written: list[str],
    changed: list[str],
    project_config_required: bool,
) -> Path | None:
    """Persist repair output only under the shared config generation."""
    ...
    if write_decisions["global_config"]:
        state_lease = state_write_authority.try_acquire_state_write_lease(
            "codex_config",
            "codex_state_repair",
            state_root=global_config.parent,
            timeout_seconds=10.0,
        )
        if state_lease is None:
            raise RuntimeError("state_write_authority_busy")
    try:
        ...
        backup_dir = (
            backup_files(backup_paths, labels=backup_labels, changed=changed)
            if written
            else None
        )
        if write_decisions["global_config"]:
            assert state_lease is not None
        ...
        if write_decisions["global_state"] and global_state_text is not None:
            global_state_path.write_text(global_state_text, encoding="utf-8", newline="\n")
...
def repair(
    ...
    *,
    ...
    explicit_registration_only = bool(
    ...
    environment_selection = (
        ...
        if reconcile_desktop_environment and not explicit_registration_only
        else {
            "ok": True,
    ...
    if not isinstance(decommissioned_mcp, dict):
        decommissioned_mcp = {}
    reconcile_mcp_tables = reconcile_mcp_registration or (
        not explicit_registration_only and not wsl_runtime_owned
    )
    ...
    reconcile_plugin_tables = reconcile_plugin_registration or (
        not explicit_registration_only and not wsl_runtime_owned
    )
    if reconcile_plugin_tables:
        decommissioned_plugins = baseline.get("decommissioned_plugins") or []
        if not isinstance(decommissioned_plugins, list):
            decommissioned_plugins = []
        for plugin in sorted(str(item) for item in decommissioned_plugins):
            table_names = (f'plugins."{plugin}"',)
            global_text, did = remove_table_tree(global_text, table_names)
            if did:
                changed.append(f"global_plugin_remove_decommissioned_{plugin}")

        decommissioned_marketplaces = baseline.get("decommissioned_marketplaces") or []
        if not isinstance(decommissioned_marketplaces, list):
            decommissioned_marketplaces = []
        for marketplace in sorted(str(item) for item in decommissioned_marketplaces):
            table_names = (f"marketplaces.{marketplace}", f'marketplaces."{marketplace}"')
            global_text, did = remove_table_tree(global_text, table_names)
            if did:
                changed.append(f"global_marketplace_remove_decommissioned_{marketplace}")
        ...
        if not explicit_registration_only:
            config_target = "wsl_codex_config" if wsl_runtime_owned else "windows_codex_config"
            for name, declared_spec in (baseline.get("expected_marketplaces") or {}).items():
                spec = expected_marketplace_fields(name, declared_spec, target=config_target)
    ...
    if not dry_run:
        backup_dir = write_repaired_state(
            global_config=global_config,
            global_text=global_text,
            project_config=project_config,
            project_text=project_text,
            global_state_path=global_state_path,
            global_state_text=global_state_text,
            write_decisions=write_decisions,
            written=written,
            changed=changed,
            project_config_required=project_config_required,
        )

    wsl_runtime_projection = (
        {
            "ok": True,
            "status": "skipped_explicit_registration",
            "enabled": desktop_wsl_enabled,
            "changed": False,
            "ready": True,
        }
        if explicit_registration_only
        else ensure_wsl_runtime_projection(
            enabled=desktop_wsl_enabled,
            dry_run=dry_run,
        )
    )
    windows_mount_compatibility = (
        codex_state_repair_mount_compatibility.reconcile(
            config_path=desktop_host_config,
            apply=not dry_run,
        )
        if desktop_wsl_enabled and not explicit_registration_only
        else {
            "ok": True,
            "status": "not_required",
            "changed": False,
    ...
    if not dry_run and runtime_validation and reconcile_mcp_registration:
    ...
    return {
        "ok": bool(wsl_runtime_projection.get("ok")) and bool(windows_mount_compatibility.get("ok")),
        "dry_run": dry_run,
        ...
        "global_config_owner": (
            ...
            if reconcile_mcp_registration and reconcile_plugin_registration
            else (
                "explicit_mcp_registration"
                if reconcile_mcp_registration
                else (
                    "explicit_plugin_registration"
                    if reconcile_plugin_registration
        ...
        "windows_mount_compatibility": windows_mount_compatibility,
