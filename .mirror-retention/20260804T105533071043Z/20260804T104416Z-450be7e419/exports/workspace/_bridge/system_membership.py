...
MEMBER_INTEGRATION_SURFACES: list[dict[str, Any]] = [
    ...
    surface(
        ...
        "member owner plus workflow, skill, and utilization-audit owners",
        "each active capability declares representative tasks, expected route and skill or owner selection, runtime evidence, result consumption, and minimum real usage evidence; file presence or registration alone cannot satisfy activation",
        "python _bridge\\tool_utilization_audit.py validate plus the declared runtime and usage readback",
        update_when="adding a capability or changing its intended task placement, trigger, route, owner, or acceptance evidence",
    ),
    surface(
        "dependency_contract",
...
INTEGRATION_POLICY = {
    ...
    "activation_rule": "registration, generated config, route catalogs, runtime launch guards, maintenance baselines, user-visible discovery, representative-task selection, and consumed usage evidence must converge before activation is complete",
    "utilization_rule": "new capabilities declare representative tasks and expected workflow/skill/owner selection in their authoritative profile; activation requires route readback, callable runtime evidence, at least one consumed use, and an underuse signal when expected usage remains absent",
...
WORKFLOW_MEMBER_SURFACES: list[dict[str, Any]] = [
    surface(
        "terminal_convergence_contract",
        "_bridge/workflow_terminal_convergence.py",
        "pure action contracts, intent-bound signatures, DAG validation, minimal invalidation, mutation barriers, and one next action remain workflow-owned without executing domain owners",
        "python -m unittest _bridge.workflow_terminal_convergence_tests",
        update_when="changing terminal action phases, receipt identity, dependency invalidation, or mutation/read-only barrier semantics",
    ),
    surface(
...
CONTRACTS: dict[str, dict[str, Any]] = {
    ...
    "workflow": {
        "system": "workflow",
        "member_kinds": ["facade", "owner_adapter", "route_projection", "lifecycle_contract", "rule_governance_owner", "permission_evidence_owner", "external_dependency_intelligence"],
        ...
        "health_commands": [
            ...
            health_command(
                ...
                ["_bridge/dependency_change_intelligence.py", "validate"],
                timeout=30,
            ),
            health_command(
                "rule_governance",
        ...
        "activation_acceptance": {
            "capability_id": "workflow.machine_first_execution_economy",
            "representative_tasks": [
                {
                    "id": "receipt-reuse-and-batching",
                    "message": "将可重复的低风险步骤交给机器执行并复用已有收据，避免重复命令",
                    "expected_workflow_domain": "workflow_governance",
                    "expected_plan_path": "automation_delegation.schema",
                    "expected_plan_prefix": "workflow_automation_delegation.",
                }
            ],
            "runtime_evidence": {
                "owner": "workflow_automation_delegation.py",
                "validation": "python _bridge\\workflow_automation_delegation_tests.py",
            },
            "result_consumption": "the route plan exposes owner, input signature, reuse or invalidation, batching, and acceptance evidence before execution",
        },
        "non_goals": [
    ...
    "scheduler": generic_contract(
        "scheduler",
        ["maintenance_scheduler", "persistent_task_kernel", "controlled_dispatcher"],
        [
            health_command("maintenance_scheduler_service", ["_bridge/maintenance_scheduler_service.py", "validate"], timeout=60),
            health_command("scheduler_runner", ["_bridge/shared/codex_scheduler_runner.py", "validate"], timeout=90),
            health_command("persistent_task_kernel", ["_bridge/persistent_task_kernel.py", "validate"], timeout=45),
        ],
        [
            "do not create a second task table, retry engine, scheduler loop, or receipt authority",
            "do not execute arbitrary commands or script text through a dispatcher",
            "do not clear retry-exhausted state without a fresh accepted owner result",
            "do not keep the periodic scheduler resident in Windows after WSL service acceptance",
        ],
    ),
    "records": generic_contract(
        ...
        [
            health_command(
                "record_store",
                ["_bridge/shared/record_store_maintenance.py", "doctor"],
                timeout=150,
                compatibility_args=["_bridge/shared/record_store_maintenance.py", "validate"],
                compatibility_timeout=30,
            )
        ],
    ...
    "startup": generic_contract(
        ...
        [
            ...
            health_command("state_write_authority", ["_bridge/state_write_authority.py", "validate"], timeout=45),
            health_command(
                "session_store",
    ...
    "wsl_workspace": {
        ...
        "required_surfaces": [
            ...
            surface("lifecycle", "wsl_workspace owner", "status, plan, bootstrap, validate, handoff, fixed-allowlist Work Git generated-artifact cleanup, projection-aware host cleanup, hash-verified host-audio migration, and work-Git release-readiness are reusable lifecycle operations", "python _bridge\\wsl_workspace_owner.py validate"),
            ...
            surface("desktop_project_registration", "wsl_workspace owner", "Desktop persists the Windows-visible WSL UNC Git root through Electron IPC; the same owner admits only that exact UNC root to Windows Git safe.directory, projects the installed Git directory once into the user PATH without rewriting malformed machine entries, and requires the live Desktop Git worker stable-metadata result before readiness", "python _bridge\\wsl_workspace_owner.py desktop-project-status"),
            surface("desktop_restart", "wsl_workspace owner", "a signed short-lived request is consumed by the existing governed Windows launcher through the fixed Task Scheduler lane; completion requires a launcher receipt plus live Desktop Git-worker acceptance and never enables force termination", "python _bridge\\wsl_workspace_owner.py desktop-restart-status"),
            ...
            surface("rollback", "wsl_workspace owner", "cleanup is explicit; Work Git cleanup removes only ignored root-relative editor/test caches, host cleanup removes only fixed-classification regenerated or redundant artifacts after projection validation, and host-audio migration hashes preserved results before source deletion; none removes the default WSL distribution, mirror, host runtime, bridge, or opaque user data", "python _bridge\\wsl_workspace_owner.py cleanup-plan"),
...
SYSTEM_DOMAIN_BINDINGS: dict[str, dict[str, str]] = {
    ...
    "scheduler": {"workflow_domain": "workflow_governance", "skill_domain": "workflow_governance"},
...
MIRROR_MEMBER_REGISTRY: list[dict[str, Any]] = [
    {
        "member_id": "workflow.task-execution-budget",
        "system": "workflow",
        "kind": "read_only_projection_owner",
        "lifecycle": "active",
        "owner": "workflow_task_execution_budget",
        "source_ids": ["workspace-bridge-source"],
        "generated_source_ids": [],
        "change_roots": [
            "workspace:_bridge/workflow_task_execution_budget.py",
            "workspace:_bridge/workflow_task_execution_budget_tests.py",
            "workspace:_bridge/workflow_orchestrator.py",
            "workspace:_bridge/workflow_rule_routing_tests.py",
        ],
    },
    {
        "member_id": "workflow.context-projection-shadow",
        "system": "workflow",
        "kind": "read_only_projection_owner",
        "lifecycle": "active",
        ...
        "generated_source_ids": [],
        "change_roots": [
            "workspace:_bridge/context_projection_owner.py",
            "workspace:_bridge/context_artifact_selector.py",
            "workspace:_bridge/context_projection_shadow.py",
            "workspace:_bridge/policies/context_projection_contracts.json",
            "workspace:_bridge/bounded_output.py",
        ],
    },
    {
        "member_id": "workflow.scoped-authorization",
        "system": "workflow",
        "kind": "permission_evidence_owner",
        "lifecycle": "active",
        "owner": "scoped_authorization",
        "source_ids": ["workspace-bridge-source"],
        "generated_source_ids": [],
        "change_roots": ["workspace:_bridge/scoped_authorization.py", "workspace:_bridge/scoped_authorization_policy.py", "workspace:_bridge/policies/scoped_authorization_risk_cost_policy.json", "workspace:_bridge/scoped_authorization_environment.py", "workspace:_bridge/authorization_environment_provider.py", "workspace:_bridge/execution_route_pack.py", "workspace:_bridge/workflow_plan_detail.py", "workspace:_bridge/shared/scheduler_authorization.py", "workspace:_bridge/shared/codex_scheduler_runner.py", "workspace:_bridge/workflow_review_delivery.py", "workspace:_bridge/codex_environment_mirror.py", "workspace:_bridge/codex_workflow_entry.py"],
    },
    {
        ...
        "generated_source_ids": [],
        "change_roots": [
            "workspace:_bridge/business_environment_control_plane.py",
            "workspace:_bridge/business_environment_control_plane_tests.py",
            "workspace:_bridge/business_environment_capability_resolver.py",
            "workspace:_bridge/business_environment_capability_resolver_tests.py",
            "workspace:_bridge/workflow_asset_domain_projection.py",
            "workspace:_bridge/workflow_asset_domain_projection_tests.py",
            "workspace:_bridge/workflow_asset_guidance.py",
            "workspace:_bridge/workflow_asset_guidance_tests.py",
            "workspace:_bridge/workflow_plan_detail.py",
            "workspace:_bridge/workflow_plan_detail_tests.py",
            "workspace:_bridge/business_environment_durable_executor_process.py",
            "workspace:_bridge/business_environment_durable_executor_tests.py",
            "workspace:_bridge/codex_workflow_entry.py",
        ],
    },
    {
        "member_id": "workflow.semantic-capability-owner",
        "system": "workflow",
        "kind": "owner_adapter",
        "lifecycle": "active",
        ...
        "generated_source_ids": [],
        "change_roots": [
            "workspace:_bridge/semantic_capability_owner.py",
            "workspace:_bridge/semantic_capability_owner_tests.py",
            "workspace:_bridge/environment_capability_view.py",
            "workspace:_bridge/workflow_environment_context.py",
            "workspace:_bridge/workflow_asset_guidance.py",
        ],
        "activation_acceptance": {
            ...
            "representative_tasks": [
                {
                    "id": "bge-m3-hybrid-rerank",
                    "message": "用 BGE-M3 做语义检索与概念归一",
                    "expected_workflow_domain": "semantic_retrieval",
                    "expected_owner": "semantic_capability_owner",
                }
            ],
            "runtime_evidence": {
                "owner": "semantic_capability_owner",
                "validation": "semantic_capability_owner.status(probe=True)",
                "probe": {
                    "module": "semantic_capability_owner",
                    "function": "status",
                    "kwargs": {"probe": True},
                    "expect": {"callable": True, "healthy": True},
                },
            },
            "usage_evidence": {
                "owner": "semantic_capability_owner",
                "validation": "semantic_capability_owner.hybrid_rerank",
                "probe": {
                    "module": "semantic_capability_owner",
                    "function": "hybrid_rerank",
                    "kwargs": {
                        "query": "概念归一",
                        "candidates": [
                            {
                                "id": "bge-m3-acceptance-candidate",
                                "text": "概念归一与语义检索候选",
                                "source_ref": "tool_utilization_audit",
                                "lexical_rank": 1,
                            },
                            {
                                "id": "bge-m3-fallback-candidate",
                                "text": "结构化全文和图谱回退候选",
                                "source_ref": "tool_utilization_audit",
                                "lexical_rank": 2,
                            },
                    ...
                    "expect": {"vector_used": True, "results_min": 1, "retrievers_contains": ["dense"]},
                },
            },
            "result_consumption": "the acceptance caller consumes vector_used and ranked results while retaining source anchors and the structured FTS/graph fallback",
        },
    },
    {
        "member_id": "workflow.capability-decision-session",
        "system": "workflow",
        "kind": "read_only_projection_owner",
        "lifecycle": "active",
        "owner": "capability_decision_session",
        "source_ids": ["workspace-bridge-source"],
        "generated_source_ids": [],
        "change_roots": [
            "workspace:_bridge/capability_decision_session.py",
            "workspace:_bridge/capability_decision_session_tests.py",
        ],
    },
    {
    ...
    {
        "member_id": "workflow.observation-iteration",
        "system": "workflow",
        "kind": "owner_adapter",
        "lifecycle": "active",
        "owner": "workflow_observation_iteration",
        "source_ids": ["workspace-bridge-source"],
        "generated_source_ids": [],
        "change_roots": [
            "workspace:_bridge/workflow_observation_iteration.py",
            "workspace:_bridge/workflow_observation_iteration_tests.py",
            "workspace:_bridge/workflow_efficiency_cycle_projection.py",
            "workspace:_bridge/workflow_efficiency_cycle_projection_tests.py",
            "workspace:_bridge/business_environment_control_plane.py",
            "workspace:_bridge/business_environment_control_plane_tests.py",
            "workspace:_bridge/codex_rule_observer.py",
    ...
    {
        "member_id": "mcp.headroom-context-compression",
        "system": "mcp",
        "kind": "hub_adapter",
        "lifecycle": "active",
        "owner": "headroom_runtime",
        "source_ids": ["workspace-bridge-source"],
        "generated_source_ids": ["mcp-bundle-readiness"],
        "change_roots": [
            "workspace:_bridge/headroom_runtime.py",
            "workspace:_bridge/local_mcp_hub_headroom.py",
            "workspace:_bridge/local_mcp_stdio_client.py",
            "workspace:_bridge/mcp_recovery_bundle_manifest.json",
        ],
    },
    {
        "member_id": "memory.pmb-runtime",
        "system": "memory",
        "kind": "pmb",
        "lifecycle": "active",
        "owner": "local_pmb_memory_service",
        "source_ids": ["workspace-bridge-source"],
        "generated_source_ids": ["pmb-memory-user-unit", "pmb-memory-service-status"],
        "change_roots": [
            "workspace:_bridge/local_pmb_memory.py",
            "workspace:_bridge/local_pmb_memory_service.py",
            "workspace:_bridge/local_pmb_memory_service_tests.py",
            "workspace:_bridge/local_mcp_hub.py",
            "workspace:_bridge/local_mcp_hub_pmb_tests.py",
        ],
    },
    {
        ...
        "source_ids": ["workspace-bridge-source"],
        "generated_source_ids": ["maintenance-scheduler-user-unit", "maintenance-scheduler-service-status", "windows-control-plane-wake-task"],
        "change_roots": [
            "workspace:_bridge/maintenance_scheduler_service.py",
            "workspace:_bridge/maintenance_scheduler_service_tests.py",
            "workspace:_bridge/shared/codex_scheduler_runner.py",
            "workspace:_bridge/shared/install-codex-scheduler-task.ps1",
            "workspace:_bridge/windows_execution_agent.py",
        ],
    },
    {
        "member_id": "scheduler.persistent-task-kernel",
        "system": "scheduler",
        "kind": "persistent_task_kernel",
        "lifecycle": "active",
        "owner": "persistent_task_kernel",
        "source_ids": ["workspace-bridge-source"],
        "generated_source_ids": ["persistent-task-kernel-status"],
        "change_roots": [
            "workspace:_bridge/persistent_task_kernel.py",
            "workspace:_bridge/persistent_task_kernel_tests.py",
        ],
    },
    {
    ...
    },
    {
        ...
        "change_roots": ["worktree:AGENTS.md", "workspace:AGENTS.md", "workspace:_bridge/wsl_workspace_owner.py", "workspace:_bridge/wsl_workspace_owner_authorization.py", "workspace:_bridge/wsl_workspace_generated_artifacts.py", "workspace:_bridge/wsl_interop_guard.py", "workspace:_bridge/platform_paths.py", "workspace:_bridge/bootstrap_wsl_workspace.py", "worktree:WSL_WORKSPACE.md"],
    },
    {
    ...
    },
    {
        ...
        "change_roots": [
            ...
            "workspace:_bridge/work_git_change_owner_process.py",
            "workspace:_bridge/work_git_change_owner_replay.py",
            "workspace:_bridge/work_git_change_owner_tests.py",
        ],
    },
    {
        ...
        "change_roots": [
            ...
            "workspace:_bridge/codex_node_repl_wsl.sh",
            "workspace:_bridge/codex_node_repl_wsl_proxy.py",
...
IMPACT_RULES: list[dict[str, Any]] = [
    {
        "prefix": "_bridge/capability_decision_session",
        "systems": ["workflow"],
        "surfaces": ["workflow_route", "result_contract", "maintenance_surface", "maintenance_regression", "closeout_reconciliation"],
        "reason": "Capability Decision Session emits a bounded, read-only route decision receipt from caller-owned facts.",
    },
    {"prefix": "_bridge/resource_acceptance", "systems": ["resource", "workflow"], "surfaces": ["execution_contract", "result_contract", "maintenance_regression", "closeout_reconciliation"], "reason": "Canonical resource business acceptance contract or regression coverage changed."},
    {"prefix": "_bridge/resource_strategy_shadow", "systems": ["resource", "records"], "surfaces": ["resource_owner_route", "result_contract", "maintenance_regression", "closeout_reconciliation"], "reason": "Read-only adaptive resource strategy projection or regression coverage changed."},
    {"prefix": "_bridge/resource_transfer_", "systems": ["resource", "records", "scheduler", "workflow"], "surfaces": ["member_identity", "state_store", "permission_boundary", "execution_contract", "result_contract", "scheduler_integration", "maintenance_regression", "closeout_reconciliation"], "reason": "Durable direct-file transfer identity, lifecycle, authorization fencing, worker, or convergence behavior changed."},
    {"prefix": "_bridge/resource_candidate_quality.py", "systems": ["resource"], "surfaces": ["resource_owner_route", "result_contract", "maintenance_regression"], "reason": "Resource candidate quality vector or ranking compatibility changed."},
    {"prefix": "_bridge/resource_strategy_policy.py", "systems": ["resource", "workflow"], "surfaces": ["resource_owner_route", "execution_contract", "result_contract", "maintenance_regression"], "reason": "Canonical resource satisfaction, recovery, or fallback policy changed."},
    {"prefix": "_bridge/resource_broker.py", "systems": ["resource", "workflow", "records"], "surfaces": ["resource_owner_route", "execution_contract", "result_contract", "closeout_reconciliation"], "reason": "Canonical resource request broker orchestration or receipt projection changed."},
    {"prefix": "_bridge/resource_event_store_tests.py", "systems": ["resource", "records"], "surfaces": ["state_store", "maintenance_regression"], "reason": "Resource event projection regression coverage changed."},
    {"prefix": "_bridge/clash_mihomo_control.py", "systems": ["network", "resource"], "surfaces": ["execution_contract", "result_contract", "platform_projection", "maintenance_regression"], "reason": "Clash Verge profile, controller, or isolated network evidence ownership changed."},
    {"prefix": "_bridge/clash_node_metrics.py", "systems": ["network", "resource", "records"], "surfaces": ["state_store", "resource_owner_route", "platform_projection", "maintenance_regression"], "reason": "Clash node metric identity, cache signature, or recommendation behavior changed."},
    {"prefix": "_bridge/clash_resource_adaptive_tests.py", "systems": ["network", "resource", "wsl_workspace"], "surfaces": ["platform_projection", "maintenance_regression"], "reason": "Multi-profile Clash and cross-platform resource tool regression coverage changed."},
    {"prefix": "_bridge/resource_download_backends.py", "systems": ["resource", "wsl_workspace"], "surfaces": ["dependency_contract", "platform_projection", "execution_contract", "maintenance_regression"], "reason": "Cross-platform resource transfer tool discovery or backend execution changed."},
    {
        "prefix": "_bridge/scoped_authorization",
        "systems": ["workflow", "scheduler", "drafts", "backup", "memory"],
        "surfaces": ["member_identity", "execution_contract", "result_contract", "state_store", "maintenance_surface", "maintenance_regression", "closeout_reconciliation"],
        "reason": "Scoped authorization intent, turn/run permit, dynamic environment fencing, effect journal, replay, or revocation semantics changed.",
    },
    {
        "prefix": "_bridge/policies/scoped_authorization_risk_cost_policy.json",
        "systems": ["workflow", "scheduler", "drafts", "backup", "memory"],
        "surfaces": ["member_identity", "execution_contract", "result_contract", "maintenance_surface", "maintenance_regression", "closeout_reconciliation"],
        "reason": "Central risk-or-cost gate thresholds, hard forbids, bounded preflight, or shadow policy semantics changed.",
    },
    {
        "prefix": "_bridge/authorization_environment_provider.py",
        "systems": ["workflow", "scheduler", "wsl_workspace"],
        "surfaces": ["execution_contract", "result_contract", "state_authority", "maintenance_surface", "maintenance_regression", "closeout_reconciliation"],
        "reason": "The shared read-only authorization environment projection or its owner signatures changed.",
    },
    {
        "prefix": "_bridge/shared/scheduler_authorization.py",
        "systems": ["scheduler", "workflow", "wsl_workspace"],
        "surfaces": ["scheduler_integration", "execution_contract", "result_contract", "state_authority", "maintenance_regression", "closeout_reconciliation"],
        "reason": "The unified scheduler's per-run authorization PEP, registry-backed system intent, environment signature, or effect-journal handoff changed.",
    },
    {
        "prefix": "_bridge/policies/scoped_authorization_consumers.json",
        "systems": ["workflow", "drafts", "backup", "memory"],
        "surfaces": ["execution_contract", "maintenance_surface", "maintenance_regression", "closeout_reconciliation"],
        "reason": "The machine-readable high-impact authorization-consumer migration boundary changed.",
    },
    {
        "prefix": "_bridge/business_environment_",
        "systems": ["workflow", "skills"],
        "surfaces": [
            "member_identity",
            "workflow_route",
            "facade_lifecycle",
        ...
        "reason": "Read-only business environment control, capability composition, authority references, or focused regressions changed.",
    },
    {
        "prefix": "_bridge/dependency_change_intelligence",
        "systems": ["workflow", "resource", "records"],
        "surfaces": ["member_identity", "dependency_contract", "execution_contract", "result_contract", "maintenance_surface", "maintenance_regression", "closeout_reconciliation"],
        "reason": "External dependency event, probe, classification, proposal, or incident-handoff behavior changed.",
    },
    {
        "prefix": "_bridge/contracts/external_dependencies/",
        "systems": ["workflow", "resource"],
        "surfaces": ["dependency_contract", "execution_contract", "result_contract", "maintenance_regression", "closeout_reconciliation"],
        "reason": "A declarative external dependency profile, source task, capability contract, or validation condition changed.",
    },
    {
        "prefix": "codex-home/scripts/codex-version-fingerprint.ps1",
        "systems": ["startup", "wsl_workspace", "workflow"],
        "surfaces": ["activation_contract", "host_compatibility_projection", "execution_contract", "result_contract", "maintenance_regression"],
        "reason": "The hidden launcher-owned local version fingerprint collector or trigger envelope changed.",
    },
    {
        "prefix": "_bridge/network_doctor.py",
        "systems": ["network", "resource"],
        "surfaces": ["execution_contract", "platform_projection", "maintenance_regression", "maintenance_surface"],
        "reason": "Cross-platform network probe selection, reachability evidence, or route diagnostics changed.",
    },
    {
    ...
    {
        "prefix": "_bridge/workflow_observation_iteration",
        "systems": ["workflow", "memory", "scheduler"],
        "surfaces": ["member_identity", "workflow_route", "execution_contract", "result_contract", "maintenance_surface", "maintenance_regression", "closeout_reconciliation"],
        "reason": "Bounded observer-pattern derivation, stable iteration identity, or pending review-queue handoff behavior changed.",
    },
    {
        "prefix": "codex-home/hooks.wsl.template.json",
        "systems": ["workflow", "startup", "wsl_workspace"],
        "surfaces": ["registration", "activation_contract", "execution_contract", "maintenance_regression", "closeout_reconciliation"],
        "reason": "The governed WSL Codex observer hook template or its reload and trust boundary changed.",
    },
    {
    ...
    },
    {
        ...
        "systems": ["workflow", "scheduler"],
        "surfaces": ["execution_contract", "result_contract", "maintenance_surface", "maintenance_regression", "closeout_reconciliation", "utilization_acceptance"],
        "reason": "Maintenance capability classification, global member coverage, bounded discovery, or owner resolution changed.",
    },
    {
        "prefix": "_bridge/maintenance_convergence",
        "systems": ["workflow", "scheduler"],
        "surfaces": ["execution_contract", "result_contract", "state_store", "maintenance_surface", "maintenance_regression", "closeout_reconciliation", "utilization_acceptance", "prevention_guard"],
        "reason": "Maintenance convergence runtime planning, result routing, approval handoff, or anti-loop behavior changed.",
    },
    {
    ...
    {
        "prefix": "_bridge/context_projection",
        "systems": ["workflow", "mcp", "resource", "bridge", "memory"],
        "surfaces": ["member_identity", "result_contract", "diagnostics", "maintenance_surface", "maintenance_regression", "closeout_reconciliation"],
        "reason": "Canonical context-projection policy, read-only shadow evaluation, or projection contract changed.",
    },
    {
        "prefix": "_bridge/context_artifact_selector",
        "systems": ["workflow", "mcp", "resource", "bridge", "memory"],
        "surfaces": ["source_authority", "result_contract", "diagnostics", "maintenance_surface", "maintenance_regression", "closeout_reconciliation"],
        "reason": "Bounded local artifact selection, hash validation, or expansion contract changed.",
    },
    {
        "prefix": "_bridge/policies/context_projection_contracts.json",
        "systems": ["workflow", "mcp", "resource", "bridge", "memory"],
        "surfaces": ["contract_template", "result_contract", "diagnostics", "maintenance_surface", "maintenance_regression", "closeout_reconciliation"],
        "reason": "Declarative context-projection functional fields, selectors, or compression thresholds changed.",
    },
    {
    ...
    {
        "prefix": "_bridge/codex_state_audit.py",
        "systems": ["startup", "mcp", "wsl_workspace"],
        "surfaces": ["platform_projection", "registration", "diagnostics", "maintenance_regression", "maintenance_surface"],
        "reason": "Startup configuration audit or its WSL/Windows runtime-entry interpretation changed.",
    },
    {
    ...
    {
        "prefix": "_bridge/codex_node_repl_wsl",
        "systems": ["startup", "wsl_workspace", "mcp", "backup"],
        "surfaces": ["platform_projection", "activation_contract", "execution_contract", "result_contract", "maintenance_regression", "maintenance_surface"],
        "reason": "WSL node_repl bridge or Windows sandbox metadata projection changed.",
    },
    {
    ...
    },
    {
        ...
        "systems": ["memory", "mcp", "startup", "wsl_workspace"],
        "surfaces": ["hub_adapter", "activation_contract", "execution_contract", "diagnostics", "maintenance_regression", "maintenance_surface", "runtime_exit", "closeout_receipt"],
        "reason": "PMB memory behavior or its single WSL user-systemd daemon lifecycle changed.",
    },
    {
    ...
    {
        "prefix": "_bridge/workflow_task_execution_budget",
        "systems": ["workflow"],
        "surfaces": ["workflow_route", "contract_template", "maintenance_regression", "closeout_reconciliation"],
        "reason": "Task-level Codex work budget or its side-effect-driven validation tier changed.",
    },
    {
    ...
    {
        "prefix": "_bridge/environment_capability_view",
        "systems": ["workflow", "mcp", "skills"],
        "surfaces": ["capability_matrix", "derived_route_index", "workflow_route", "maintenance_regression", "closeout_reconciliation"],
        "reason": "The bounded environment capability projection exposes owner-backed tools and semantic models without becoming an execution owner.",
    },
    {
        "prefix": "_bridge/semantic_capability_owner",
        "systems": ["workflow", "mcp", "skills"],
        "surfaces": ["capability_matrix", "execution_contract", "result_contract", "diagnostics", "maintenance_regression", "closeout_reconciliation"],
        "reason": "The shared BGE-M3 adapter owns bounded loopback transport and vector evidence while domain consumers retain indexes and acceptance.",
    },
    {
    ...
    {
        "prefix": "_bridge/tool_utilization_audit",
        "systems": ["workflow", "skills"],
        "surfaces": ["utilization_acceptance", "domain_discovery", "maintenance_regression", "closeout_reconciliation"],
        "reason": "Capability activation cases and consumed-use acceptance determine whether registered members are actually usable.",
    },
    {
    ...
    {
        "prefix": "codex-home/skills/",
        "systems": ["skills", "workflow"],
        "surfaces": ["active_catalog", "lifecycle", "domain_discovery", "maintenance_regression", "closeout_reconciliation"],
        "reason": "A Work Git skill authority changed; skill lifecycle and workflow discoverability require reconciliation without transferring business-owner authority.",
    },
    {
    ...
    {
        "prefix": "_bridge/codex_workflow_entry",
        "systems": ["workflow", "mcp", "office"],
    ...
    {
        "prefix": "_bridge/iteration_layer_review",
        "systems": ["workflow", "memory"],
        "surfaces": ["domain_discovery", "route_projection", "validation", "maintenance_regression", "closeout_reconciliation", "impact_mapping"],
        "reason": "Controlled iteration observations, actionable proposal admission, owner validation routes, or approval requirements changed.",
    },
    {
    ...
    {
        "prefix": "_bridge/docs/maintenance_surfaces/",
        "systems": ["workflow"],
        "surfaces": ["maintenance_surface", "contract_template", "impact_mapping"],
        "reason": "An on-demand maintenance authority shard or its navigation metadata changed.",
    },
    {
    ...
    },
    {
        ...
        "systems": ["workflow", "scheduler", "mcp", "resource"],
        "surfaces": ["contract_template", "impact_mapping", "maintenance_surface", "workflow_route", "execution_contract", "result_contract", "prevention_guard"],
        "reason": "Maintenance convergence planning can alter how signals select minimal owner dependency and reverse-validation closures.",
    },
    ...
    {"prefix": "_bridge/wsl_workspace_owner_authorization.py", "systems": ["wsl_workspace", "workflow", "backup", "startup"], "surfaces": ["host_compatibility_projection", "execution_contract", "validation", "maintenance_surface", "closeout_reconciliation"], "reason": "Host compatibility projection scoped-authorization assessment, permit consumption, effect journal, or fail-closed boundary changed."},
    {"prefix": "_bridge/desktop_restart_request", "systems": ["wsl_workspace", "startup", "workflow", "backup"], "surfaces": ["desktop_restart", "activation_contract", "execution_contract", "result_contract", "validation", "maintenance_regression", "closeout_reconciliation"], "reason": "Typed Desktop full-restart request, fixed scheduled-launcher handoff, receipt acceptance, or regression behavior changed."},
    {"prefix": "_bridge/wsl_workspace_generated_artifacts", "systems": ["wsl_workspace", "workflow", "backup", "startup"], "surfaces": ["lifecycle", "source_authority", "worktree_hygiene", "validation", "rollback", "maintenance_regression", "maintenance_surface", "closeout_reconciliation"], "reason": "Fixed-allowlist Work Git generated-artifact classification, ignore contract, cleanup safety, or regression behavior changed."},
    ...
    {"prefix": "_bridge/managed_python_dependency_runtime", "systems": ["resource", "mcp", "wsl_workspace"], "surfaces": ["dependency_contract", "platform_projection", "execution_contract", "result_contract", "maintenance_regression", "maintenance_surface", "closeout_reconciliation"], "reason": "Managed Python dependency ABI/platform identity, scoped target selection, or fresh-process import acceptance changed."},
    {"prefix": "_bridge/policies/managed_python_dependencies.lock.json", "systems": ["resource", "mcp", "wsl_workspace", "workflow"], "surfaces": ["dependency_contract", "source_authority", "permission_boundary", "recovery_contract", "maintenance_regression", "maintenance_surface", "closeout_reconciliation"], "reason": "Managed Python dependency versions, artifact URLs, hashes, imports, or recovery inputs changed."},
    {"prefix": "_bridge/resource_content_store", "systems": ["resource", "mcp"], "surfaces": ["content_authority", "result_contract", "dependency_contract", "maintenance_regression", "maintenance_surface", "closeout_reconciliation"], "reason": "Reversible full-content storage, bounded preview, offline expansion, or managed HTML parsing changed."},
    {"prefix": "_bridge/__init__.py", "systems": ["workflow", "mcp", "wsl_workspace"], "surfaces": ["module_import_contract", "test_execution_contract", "maintenance_regression", "closeout_reconciliation"], "reason": "Repository-root package imports or direct-script compatibility for bridge modules changed."},
    {"prefix": "_bridge/bridge_import_compatibility_tests.py", "systems": ["workflow", "mcp", "wsl_workspace"], "surfaces": ["module_import_contract", "test_execution_contract", "maintenance_regression", "closeout_reconciliation"], "reason": "Repository-root bridge import regression coverage changed."},
    {"prefix": "_bridge/headroom_runtime", "systems": ["mcp", "resource", "wsl_workspace"], "surfaces": ["member_identity", "domain_discovery", "dependency_contract", "activation_contract", "execution_contract", "result_contract", "maintenance_regression", "maintenance_surface", "closeout_reconciliation"], "reason": "Managed Headroom runtime, TTL context-cache boundary, or standalone MCP launch contract changed."},
    {"prefix": "_bridge/resource_python_package_installer", "systems": ["resource", "mcp", "wsl_workspace"], "surfaces": ["dependency_contract", "permission_boundary", "platform_projection", "execution_contract", "result_contract", "maintenance_regression", "maintenance_surface", "closeout_reconciliation"], "reason": "Approved isolated package installation, atomic replacement, or runtime import acceptance changed."},
    {"prefix": "_bridge/local_mcp_hub_resource_search", "systems": ["resource", "mcp", "wsl_workspace"], "surfaces": ["dependency_contract", "platform_projection", "execution_contract", "result_contract", "maintenance_regression", "maintenance_surface", "closeout_reconciliation"], "reason": "Hub resource-search dependency selection, ABI health, or bounded search execution changed."},
    {"prefix": "_bridge/local_mcp_stdio_client", "systems": ["mcp", "workflow", "wsl_workspace"], "surfaces": ["stdio_transport", "permission_boundary", "execution_contract", "result_contract", "process_cleanup", "maintenance_regression", "closeout_reconciliation"], "reason": "Shared one-shot MCP stdio initialization, allowlist enforcement, timeout, or child-process cleanup changed."},
    ...
    {"prefix": "_bridge/maintenance_scheduler_service", "systems": ["scheduler", "wsl_workspace", "workflow", "startup", "backup"], "surfaces": ["member_identity", "activation_contract", "execution_contract", "result_contract", "windows_execution_plane", "maintenance_regression", "maintenance_surface", "closeout_reconciliation"], "reason": "Unified scheduler user-systemd lifecycle, heartbeat acceptance, or Windows login-wake conversion changed."},
    {"prefix": "_bridge/shared/install-codex-scheduler-task.ps1", "systems": ["scheduler", "wsl_workspace", "startup"], "surfaces": ["activation_contract", "windows_execution_plane", "permission_boundary", "maintenance_regression", "maintenance_surface", "closeout_reconciliation"], "reason": "The fixed Windows login wake action for the WSL control plane changed."},
    {"prefix": "_bridge/shared/restart-bridge-appserver-if-idle.ps1", "systems": ["scheduler", "bridge", "wsl_workspace"], "surfaces": ["scheduler_integration", "windows_execution_plane", "platform_projection", "execution_contract", "result_contract", "maintenance_regression", "closeout_reconciliation"], "reason": "The WSL scheduler's typed Windows bridge idle-restart action, host-runtime database boundary, or durable result capture changed."},
    {"prefix": "_bridge/shared/run-codex-scheduler.ps1", "systems": ["scheduler", "wsl_workspace", "startup"], "surfaces": ["implementation_exit", "generation_exit", "runtime_exit", "prevention_guard", "maintenance_regression", "closeout_reconciliation"], "reason": "The retired Windows resident scheduler launcher changed or was reintroduced."},
    {"prefix": "_bridge/shared/codex_scheduler_runner.py", "systems": ["scheduler", "wsl_workspace", "workflow", "startup", "backup"], "surfaces": ["scheduler_integration", "state_authority", "host_compatibility_projection", "windows_execution_plane", "execution_contract", "result_contract", "maintenance_regression", "maintenance_surface", "closeout"], "reason": "Unified scheduler task declarations, WSL-local durable state, confirmed legacy-state import, retries, heartbeat identity, runtime reconciliation, or Windows execution-plane health scheduling changed."},
    {"prefix": "_bridge/shared/scheduler_task_recovery", "systems": ["scheduler", "wsl_workspace", "workflow", "startup"], "surfaces": ["scheduler_integration", "execution_contract", "result_contract", "maintenance_regression", "maintenance_surface", "closeout"], "reason": "Guarded single-task scheduler recovery, confirmation, state handoff, or its regression contract changed."},
    ...
    {"prefix": "_bridge/persistent_task_kernel", "systems": ["scheduler", "bridge"], "surfaces": ["member_identity", "contract_template", "execution_contract", "result_contract", "impact_mapping", "maintenance_regression", "maintenance_surface", "closeout_reconciliation"], "reason": "Persistent task ownership, leases, retries, dead-letter, recovery, or scheduler integration changed."},
    ...
    {"prefix": "_bridge/shared/email_scheduler_contract_tests.py", "systems": ["mail"], "surfaces": ["maintenance_regression", "impact_mapping"], "reason": "Mail owner contract regression coverage changed."},
    ...
    {"prefix": "_bridge/windows_kernel_pool_", "systems": ["memory", "scheduler"], "surfaces": ["windows_execution_plane", "execution_contract", "result_contract", "maintenance_regression", "maintenance_surface"], "reason": "Windows kernel-pool diagnostics, governance, remediation, scheduling, or regression coverage changed."},
    {"prefix": "_bridge/performance_doctor", "systems": ["hardware", "scheduler"], "surfaces": ["windows_execution_plane", "execution_contract", "result_contract", "maintenance_regression", "maintenance_surface"], "reason": "Cross-platform performance collection, Windows host probes, or scheduled metric evidence changed."},
    ...
    {"prefix": "_bridge/state_write_authority", "systems": ["startup", "workflow", "backup"], "surfaces": ["contract_template", "impact_mapping", "maintenance_surface", "publication_gate", "maintenance_regression"], "reason": "State-domain writer authority, fencing, information entry, or publish stability changed."},
    {"prefix": "_bridge/contracts/state_write_authorities.json", "systems": ["startup", "workflow", "backup"], "surfaces": ["contract_template", "authority_registry", "publication_gate"], "reason": "Machine-readable state-domain authority membership changed."},
    {"prefix": "_bridge/system_state_index", "systems": ["workflow", "startup", "memory", "scheduler"], "surfaces": ["information_entry", "state_signal", "maintenance_regression"], "reason": "The machine-readable cross-system state information entry, workflow-evolution projection, or one of its owner signals changed."},
    ...
    {"prefix": "_bridge/workflow_review_delivery.py", "systems": ["drafts", "workflow"], "surfaces": ["state_store", "execution_contract", "result_contract", "maintenance_surface", "closeout_receipt"], "reason": "Review package identity, delivery receipt, or atomic user decision semantics changed."},
    {"prefix": "_bridge/workflow_review_backlog", "systems": ["drafts", "workflow"], "surfaces": ["state_store", "execution_contract", "result_contract", "maintenance_surface", "maintenance_regression", "closeout_reconciliation"], "reason": "Review backlog semantic identity, evidence-only filtering, deterministic consolidation, or bounded cohort reporting changed."},
]
...
SURFACE_NEXT_COMMANDS: dict[str, list[str]] = {
    ...
    "capability_matrix": [
        "python _bridge\\mcp_capability_routes.py lookup --terms <terms>",
        "targeted readback of the returned matrix section only when lookup detail is insufficient",
    ],
    ...
    "maintenance_surface": [
        "python _bridge\\codex_workflow_entry.py maintenance catalog --system <system> --term <term> --limit 20",
        "read the returned source shard when complete contract detail is needed",
        "python _bridge\\global_coherence_doctor.py validate",
    ],
...
def plan(
    ...
    return {
        ...
        "completion_checks": [
            ...
            "representative_tasks_select_the_member_and_consumed_usage_meets_activation_acceptance",
...
def impact(changed: list[str]) -> dict[str, Any]:
    ...
    maintenance_shard_prefixes = (
        "_bridge/docs/maintenance_surfaces/",
        "workspace/_bridge/docs/maintenance_surfaces/",
    )
    for path in normalized:
        ...
        for prefix in maintenance_shard_prefixes:
            if not path.startswith(prefix):
                continue
            shard_system = Path(path[len(prefix):]).stem
            if shard_system in CONTRACTS and shard_system not in affected_systems:
                affected_systems.append(shard_system)
            break
...
def upgrade_plan(system: str) -> dict[str, Any]:
    ...
    return {
        ...
        "migration_steps": [
            ...
            "update the owning maintenance surface shard and compact navigation index so the contract is discoverable",
...
def validate() -> dict[str, Any]:
    ...
    try:
        ...
        utilization_acceptance = tool_utilization_audit.activation_acceptance_with_build_plan(
            workflow_orchestrator.build_plan
        )
    except Exception as exc:  # noqa: BLE001 - activation failure must remain consumable.
        utilization_acceptance = {"ok": False, "error": f"{type(exc).__name__}: {exc}"}
    if not utilization_acceptance.get("ok"):
        issues.append({
            "severity": "risk",
            "code": "member_utilization_acceptance_incomplete",
            "detail": utilization_acceptance.get("underused", utilization_acceptance),
        })
    ...
    for key in ("admission_rule", "domain_discovery_rule", "utilization_rule", "change_propagation_rule", "reconciliation_rule", "closeout_enforcement_rule"):
    ...
    scheduler_member_plans = [
        plan("scheduler", "scheduler.maintenance", "maintenance_scheduler"),
        plan("scheduler", "scheduler.persistent-task-kernel", "persistent_task_kernel"),
        plan("scheduler", "scheduler.dispatcher", "controlled_dispatcher"),
    ]
    if not all(item.get("ok") for item in scheduler_member_plans):
        issues.append({"severity": "risk", "code": "scheduler_member_kinds_missing"})
    ...
    return {
        ...
        "probes": {
            ...
            "utilization_acceptance_ok": utilization_acceptance.get("ok"),
            "declared_activation_case_count": utilization_acceptance.get("case_count", 0),
            "resource_member_kinds_ok": all(
                item.get("ok") for item in (resource_package_plan, resource_batch_plan, resource_browser_plan)
            ),
            "scheduler_member_kinds_ok": all(item.get("ok") for item in scheduler_member_plans),
