...
from platform_paths import host_accessible_path, resource_library_root  # noqa: E402
...
WINDOWS_BRIDGE_PROBE = host_accessible_path(
    PROJECT_ROOT / "_bridge" / "shared" / "restart-bridge-appserver-if-idle.ps1",
    platform_name="nt",
)
...
def run_windows_bridge_probe() -> dict[str, Any]:
    """Read the Windows-owned mobile queue once for all bridge decisions."""
    return run_json(
        [
            "powershell.exe",
            "-NoProfile",
...
def run_maintenance(apply_safe: bool, trigger: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    pmb_service = run_json([sys.executable, "_bridge/local_pmb_memory_service.py", "validate"], timeout=120)
    steps.append(Step("local_pmb_service_validate", ok=bool(pmb_service.get("ok")), payload=pmb_service))
    if apply_safe and not pmb_service.get("ok"):
        pmb_ensure = run_json([sys.executable, "_bridge/local_pmb_memory_service.py", "ensure"], timeout=180)
        steps.append(Step("local_pmb_service_ensure", ok=bool(pmb_ensure.get("ok")), applied=True, payload=pmb_ensure))
    ...
    bridge_metrics = run_windows_bridge_probe()
    queue = bridge_metrics.get("queue") if isinstance(bridge_metrics.get("queue"), dict) else {}
    recent_bridge_activity = (
        bridge_metrics.get("recent_bridge_activity")
        if isinstance(bridge_metrics.get("recent_bridge_activity"), dict)
        else {}
    )
    bridge_issue = (
        bridge_metrics.get("ok") is False
        or recent_bridge_activity.get("ok") is False
    ...
    bridge_probe_ok = bool(bridge_metrics.get("ok"))
    steps.append(Step("bridge_queue_pre_appserver_restart_guard", ok=bridge_probe_ok, payload=bridge_metrics))
    ...
    appserver_dry = bridge_metrics
    steps.append(Step("bridge_appserver_idle_restart_dry_run", ok=bridge_probe_ok, payload=appserver_dry))
    owner_pids = {
        int(item.get("pid"))
    ...
    steps.append(Step("bridge_queue_report_only_check", ok=bridge_probe_ok, payload=bridge_metrics))
    if bridge_issue:
