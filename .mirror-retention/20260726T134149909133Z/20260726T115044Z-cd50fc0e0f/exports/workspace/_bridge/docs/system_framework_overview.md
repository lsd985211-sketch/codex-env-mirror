...
规则层采用渐进披露，而不是把所有细节静态合并：

| 层级 | 常驻内容 | 执行责任 |
| --- | --- | --- |
| 全局 AGENTS | 跨工作区安全边界、工作流触发、通用完成标准 | 决定哪些事不可跳过 |
| 工作区 AGENTS | 本项目入口、权威面、局部不变量 | 把全局要求连接到本地 owner |
| Route pack / task contract | 本任务的 `required_gates`、`stop_if`、owner、验证和收口 | 把规则变成本轮可执行义务 |
| Closeout reconciliation | 实际 changed files、membership impact、owner receipts | 在任务文本漏判时阻止虚假完成 |
| Owner contract / skill / reference | 领域细节、schema、命令、边界和回归方法 | 按需加载并证明具体结果 |

这套分层吸收了成熟系统中的共同做法：policy-as-code 把关键规则变成可失败的检查；软件目录为成员声明 owner、类型、生命周期和所属系统；admission 在激活前校验；reconciliation 在变更后持续比较期望状态与实际状态；渐进披露只让必要规则常驻上下文。

因此，“读过规则”不等于“遵守规则”。工作流返回的 gate 必须完成并留下回执，否则任务仍处于未收口状态。

成员纳入采用前后两道门槛：任务契约在激活前生成 admission plan；收口阶段不再相信任务文本，而是根据实际 changed files 自动运行 `system_membership.py impact`。只要命中架构面，就必须通过当前 membership validator 并提供 `system_membership=ok` 回执。成员权威状态仍留在各 owner，不建立第二套中央成员数据库。
...
```mermaid
flowchart TB
    User["用户 / 手机 / 邮件 / GitHub / 本机操作"] --> Rules["规则层<br/>Global AGENTS + Workspace AGENTS"]
    Rules --> Entry["工作入口层<br/>codex_workflow_entry + workflow_orchestrator"]
    Entry --> MemoryRouter["记忆路由<br/>memory_router"]
    MemoryRouter --> Memory["记忆层<br/>PMB / 用户画像 / notes / external knowledge / record-store"]
    Entry --> Skills["技能层<br/>Skill inventory / MySkills / SKILL.md"]
    Entry --> Templates["模板层<br/>custom slash commands"]
    Entry --> Tools["工具层<br/>Native MCP / Hub / CLI / SQLite / GUI"]
    Tools --> Resources["资源层<br/>resource_cli / broker / cache / receipts"]
    Tools --> Business["业务模块<br/>微信桥接 / 邮箱 / GitHub / GUI / 记录库"]
    Business --> Maint["维护面<br/>snapshot / doctor / repair-plan / validate / metrics"]
    Maint --> Closeout["收口层<br/>work notes / proposals / memory absorb / baseline / tool matrix"]
    Closeout --> Memory
    Closeout --> Rules
    Closeout --> Skills
```
...
成员变化采用准入与对账两阶段：变更前通过 `system_membership.py plan` 明确身份、owner、依赖、激活和退役义务；变更后通过 `impact --changed` 按实际文件对账注册、生成、路由、运行时、维护、文档和重载边界。只有 owner 验证和实际生效边界均成立，工作流才可声明集成完成。
...
flowchart LR
    ...
    R --> N["network"]
    W --> K["memory + skills"]
    W --> B["bridge + mail + office"]
    B --> S["startup/runtime"]
    M --> O["records + observability"]
    R --> O
    B --> O
    O --> G["maintenance + closeout"]
    G --> W
```
...
```mermaid
flowchart LR
    A["收到任务"] --> B["preflight<br/>分类/风险/边界"]
    B --> C["memory route<br/>按任务选择记忆层"]
    C --> C2["recall<br/>只取必要记忆"]
    C2 --> D["skill selection<br/>最小技能集"]
    D --> E["template<br/>固定流程 checklist"]
    E --> F["tool route<br/>MCP/Hub/CLI/SQLite"]
    F --> G["execution<br/>owning tool"]
    G --> H["validation<br/>doctor/validate/readback"]
    H --> I["closeout<br/>提案/记忆/规则/技能/工具表"]
```
...
| 维护归属 | `_bridge/codex_workflow_entry.py maintenance catalog --system <system> --term <term> --limit 20` | 先查派生索引，再按返回引用读取单个权威分片 |
| 模块复用 | `_bridge/code_maintainability.py lookup-module ...` | 先找可复用模块 |
| 收口 | `_bridge/codex_workflow_entry.py closeout ...` | 统一收口包 |

大型治理资产保留完整功能和显式深度展开入口，但默认流程只消费派生索引、匹配条目和稳定引用。索引缺失或过期时，回退也必须保持作用域；只有需要跨域审计、重建索引或验证全部合同完整性时才读取全部权威分片。
...
```mermaid
flowchart TB
    Need["需要工具能力"] --> Matrix["查 capability matrix / routes"]
    Matrix --> Start["从 profile 的 affinity 阶段开始"]
    Start --> Call{"调用成功?"}
    Call -->|是| Verify["验证 call_completed"]
    Call -->|否| Evidence["记录失败类型和当前阶段"]
    Evidence --> Next["只移动到下一阶段"]
    Next --> Hub{"已知 Hub direct?"}
    Hub -->|是| HubCall["Hub direct"]
    Hub -->|否| Gateway{"支持 fresh stdio gateway?"}
    Gateway -->|是| GatewayCall["同权限 gateway call"]
    Gateway -->|否| Specific{"有 profile-specific fallback?"}
    Specific -->|是| Local["专用 CLI / adapter"]
    Specific -->|否| Diagnose["complete-route 诊断或明确不可用"]
    HubCall --> Verify
    GatewayCall --> Verify
    Local --> Verify
    Verify --> Record["记录正/负证据，不混淆调用层级"]
```
...
```mermaid
flowchart TB
    Req["resource request"] --> Classify["分类<br/>URL/本地文件/包/外部工具/未知"]
    Classify --> Risk["风险判断<br/>safe / review / blocked"]
    Risk --> Route["路线选择<br/>local fetch / owner MCP / package manager / handoff"]
    Route --> Attempt["执行尝试或 handoff"]
    Attempt --> Store["manifest / preview / artifact / receipt"]
    Store --> Record["events / record-store / strategy review"]
    Record --> Iterate["迭代层整理策略改进"]
```
...
stateDiagram-v2
    ...
    classified --> network_gateway
    network_gateway --> planned
    planned --> attempting
    attempting --> completed
    attempting --> degraded
    attempting --> deferred
    attempting --> handoff_required
    attempting --> failed
    handoff_required --> completed: owner result attached
    degraded --> attempting: refine request
    deferred --> attempting: conditions satisfied
    completed --> consumed
    consumed --> [*]
```
...
```mermaid
flowchart TB
    Work["当前工作"] --> Temp["one-shot work notes<br/>只服务当前任务收口"]
    Work --> Candidate["ad hoc notes<br/>长期候选"]
    Work --> EK["external knowledge<br/>联网证据层"]
    Candidate --> Plan["absorb-plan / review-summary"]
    EK --> Distill["distill candidate"]
    Distill --> Plan
    Plan --> Approval{"用户批准?"}
    Approval -->|是| PMB["PMB / user profile / baseline / skill proposal"]
    Approval -->|否| Hold["保留候选或丢弃"]
    PMB --> Recall["recall-checks / recall-verify"]
```
...
```mermaid
flowchart LR
    Task["任务"] --> SkillPlan["skill_orchestrator"]
    Registry["admission registry\napproval + trust + stable skill_id"] --> Refresh
    SkillPlan --> Refresh["incremental lifecycle refresh"]
    Refresh --> State["SQLite derived index\nstate + change + quality + lineage"]
    State --> SkillPlan
    SkillPlan --> Skill["读取最小 SKILL.md"]
    Task --> Slash["custom slash template"]
    Slash --> Checklist["流程 checklist"]
    Task --> Module["module capability index"]
    Module --> Reuse["复用/改造已有模块"]
    Skill --> Action["执行"]
    Checklist --> Action
    Reuse --> Action
```
...
- 权威边界保持单一：`registry.json` 负责审批、信任和稳定身份；SQLite 只是可重建的查询与证据索引；orchestrator 负责任务时路由，不复制审批状态机。
- `audit-pending` 不等于不可用。只有明确 `rejected` 才阻止自动路由；运行时依赖缺失、契约错误或被 supersede 仍按原有技术门控处理。
- 真实任务质量事件写入 SQLite，按 skill 汇总成功、失败、fallback 和 validation。质量信号限制在 `-2..2`，只处理业务分数相同的候选，不能让低相关技能越过领域和技术资格。
- 技能移动时，只有来源、名称和完整指纹恰好匹配一个旧记录才沿用 `skill_id`；存在多个候选时不猜测，生成新身份并交由治理面审查。
- 技能演化只记录 `FIX`、`DERIVED`、`CAPTURED` 三类血缘及验证证据。索引不自动改写技能，也不以短期未使用为理由退役技能。
- 历史 JSONL 使用记录只作为幂等迁移输入保留；新增事件和使用窗口统一查询 SQLite，避免重复扫描和双写漂移。
- 模板不执行，只给 checklist。
...
```mermaid
sequenceDiagram
    participant WX as 微信/OpenClaw
    participant DB as bridge DB
    participant Perm as 权限/令牌
    participant Worker as worker
    participant Codex as Codex app-server/CDP
    participant Attach as 附件区
    participant Send as 回发通道

    WX->>DB: 入站消息/附件/命令
    DB->>Perm: 风险/权限/令牌检查
    Perm-->>DB: allow / wait_passphrase / reject
    Worker->>Codex: 投递 task prompt
    Codex-->>DB: ack / owned-result markers
    Worker->>Attach: 生成物按账号隔离
    Worker->>Send: 文本/附件回发
    Send-->>DB: receipt / failure classification
```
...
```mermaid
flowchart TB
    IMAP["IMAP 轮询"] --> Inbox["收件箱入库<br/>按入箱顺序"]
    Inbox --> Detect{"主题含 待处理?"}
    Detect -->|否| Archive["normal_inbox<br/>只入库"]
    Detect -->|是| Class["分类<br/>reply_required / review_required"]
    Class --> Task["生成一次性回信任务"]
    Task --> Codex["Codex 生成回信"]
    Codex --> Outbox["正常邮件 outbox"]
    Outbox --> SMTP["SMTP 发送"]
    SMTP --> Done["一次性任务完成归档"]
```
...
```mermaid
flowchart LR
    Edit["准备修改文件"] --> Plan["查规划备份路径"]
    Plan -->|有| Project["项目/模块备份根"]
    Plan -->|无| Central["统一备份库"]
    Project --> Backup["backup_router create"]
    Central --> Backup
    Backup --> Patch["apply_patch / owner CLI"]
    Patch --> Validate["readback / hash / validate"]
```
...
```mermaid
sequenceDiagram
    participant P as Provider/CC Switch
    participant C as config.toml/catalog
    participant G as Config Guard/Watcher
    participant D as Codex Desktop
    participant R as Reasoning Catalog
    P->>C: 切换 provider 与模型目录
    G->>C: 校验缺失项和一致性
    G->>D: 触发目录刷新/页面重载
    D->>C: 重新读取模型 catalog
    D->>R: 重新读取每模型 reasoning capabilities
    D-->>G: 模型数、reasoning 选项、provider 指纹
```
...
```mermaid
flowchart TB
    Change["代码任务"] --> Context["module-context / lookup-module"]
    Context --> Existing{"已有模块可复用?"}
    Existing -->|是| Reuse["复用或扩展现有模块"]
    Existing -->|否| New["新建用途明确模块"]
    Reuse --> Test["目标 validator / regression"]
    New --> Test
    Test --> Index["更新派生 module capability index"]
```
...
| OpenAI Codex AGENTS.md | 全局、仓库根和子目录按作用域分层，越具体的规则覆盖越宽泛的规则 | 不把所有 owner 细节塞进常驻 AGENTS |
| Open Policy Agent | 把策略决策与执行机制分离，并让策略可测试、可分发 | 不引入独立策略语言或常驻策略服务 |
| Kubernetes Admission Control | 在激活前执行 admission，并用独立验证阶段阻止不满足策略的变更 | 不复制集群级 webhook、控制器或高可用机制 |

参考入口：OpenAI Codex AGENTS.md `https://developers.openai.com/codex/guides/agents-md`；OPA `https://www.openpolicyagent.org/docs`；Kubernetes Admission Control `https://kubernetes.io/docs/reference/access-authn-authz/admission-controllers/`；C4 `https://c4model.com/diagrams`；arc42 `https://arc42.org/overview`；Google SRE `https://sre.google/workbook/error-budget-policy/`；Backstage Catalog/TechDocs `https://backstage.io/docs/features/software-catalog/`、`https://backstage.io/docs/features/techdocs/`；OpenTelemetry `https://opentelemetry.io/docs/concepts/semantic-conventions/`。
