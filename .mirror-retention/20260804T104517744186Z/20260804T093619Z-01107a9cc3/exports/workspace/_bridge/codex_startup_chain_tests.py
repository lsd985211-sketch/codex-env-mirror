...
class CodexStartupChainTests(unittest.TestCase):
    ...
    def test_launcher_records_pid_bound_cdp_acceptance_without_new_termination(self) -> None:
        managed_source = ROOT.parent / "codex-home" / "scripts" / "start-codex-desktop-elevated.ps1"
        launcher = managed_source.read_text(encoding="utf-8-sig")
        start = launcher.index("function Get-CodexCdpLaunchAcceptanceEvidence")
        end = launcher.index("function Get-CodexDesktopUserDataDir", start)
        acceptance = launcher[start:end]
        self.assertIn("Get-CimInstance Win32_Process", acceptance)
        self.assertIn("Get-NetTCPConnection -LocalPort $Port -State Listen", acceptance)
        self.assertIn("expected_launch_argument_observed", acceptance)
        self.assertIn("listener_owner_matches_started_pid", acceptance)
        self.assertIn("cdp_ready_owned_by_started_elevated_process", acceptance)
        self.assertNotIn("Stop-Process", acceptance)
        self.assertNotIn("taskkill", acceptance.lower())
        self.assertNotIn("TerminateProcess", acceptance)
        self.assertIn('"--remote-debugging-port=$remoteDebuggingPort"', launcher)
        self.assertIn('"--user-data-dir=`"$userDataDir`""', launcher)
        self.assertIn("cdp_acceptance", launcher)
        self.assertIn("force_kill_used = $false", launcher)
