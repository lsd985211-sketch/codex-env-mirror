...
def request_text(request: dict[str, Any]) -> str:
    parts = [str(request.get(key) or "").strip() for key in ("task", "target", "name")]
...
def execute_source_selection(request: dict[str, Any], route: dict[str, Any], *, timeout: int = 20) -> dict[str, Any]:
    ...
    concrete_source = str(request.get("url") or request.get("path") or "").strip()
    if not concrete_source and registered_owner and kind not in {"documentation", "generic_web"}:
        return _json_result(
            ok=False,
            status="degraded",
            result_kind="source_selection",
            error_class="discovery_owner_execution_required",
            reason="source kind is classified; continue with the registered discovery owner adapter",
            query=query_terms(request),
            resource_kind=kind,
            source_strategy=plan,
            execution_capability=capability,
            available_owner_adapter=registered_owner,
            next_action="continue_resource_layer_with_registered_discovery_owner",
        )
    if kind == "image":
