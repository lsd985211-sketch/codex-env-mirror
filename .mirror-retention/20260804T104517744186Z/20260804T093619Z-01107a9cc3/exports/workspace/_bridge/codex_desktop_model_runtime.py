...
APP_INITIAL_ASSET_PATTERN = re.compile(r"^webview/assets/app-initial-[^/]+\.js$")
NATIVE_DESKTOP_RUNTIME_SIGNATURES = (
    b"model_picker_persists_ultra_effort",
    b"account_user_setting",
    b"show-ultra-in-model-picker-slider",
    b"list-models-for-host",
    b"enabled-reasoning-efforts",
)
...
def native_desktop_model_runtime_state() -> dict[str, Any]:
    """Detect the Desktop generation that natively owns host models and Ultra."""
    errors: list[str] = []
    for asar_path in codex_appserver_model_bridge.candidate_app_asars():
        try:
            files = codex_appserver_model_bridge.asar_files(asar_path)
            relative = next((name for name in files if APP_INITIAL_ASSET_PATTERN.match(name)), "")
            if not relative:
                errors.append(f"{asar_path}:app_initial_asset_missing")
                continue
            payload = codex_appserver_model_bridge.read_asar_asset(asar_path, relative)
            signatures = {
                signature.decode("ascii"): signature in payload
                for signature in NATIVE_DESKTOP_RUNTIME_SIGNATURES
            }
            native_authoritative = all(signatures.values())
            return {
                "schema": "codex-desktop-model-runtime/native-runtime/v1",
                "ok": True,
                ...
                "reason": (
                    ...
                    else "native_desktop_runtime_signatures_incomplete"
                ),
            }
        except Exception as exc:
            errors.append(f"{asar_path}:{type(exc).__name__}")
    return {
        ...
        "asset_path": "",
        "signatures": {},
        "reason": "desktop_runtime_inspection_unavailable",
        "errors": errors,
    }
...
def _legacy_runtime_shim_probe_expression() -> str:
    return r"""
(() => {
  const storageWrapperActive = typeof window.__codexStatsigAllowlistStorageWrapper === 'function'
    && Storage.prototype.setItem === window.__codexStatsigAllowlistStorageWrapper;
  const markers = {
    modelList: String(window.__codexModelListShimVersion || ''),
    appServer: String(window.__codexAppServerModelShimVersion || ''),
    statsig: String(window.__codexStatsigAllowlistProtectionVersion || ''),
    reasoning: String(window.__codexReasoningCapabilityVersion || ''),
  };
  return {
    schema: 'codex-desktop-model-runtime/legacy-shim-state/v1',
    ok: true,
    markers,
    reasoningEfforts: window.__codexReasoningCapabilityEfforts || [],
    storageWrapperActive,
    originalSendStored: typeof window.__codexModelListShimOriginalSendMessageFromView === 'function',
    legacyMarkersPresent: Boolean(
      markers.modelList || markers.appServer || markers.statsig || markers.reasoning || storageWrapperActive
    ),
  };
})()
"""
...
def legacy_runtime_shim_state(wait_seconds: float = 0.0) -> dict[str, Any]:
    ...
    if not ws_url:
        return {**state, "skipped": True, "reason": reason or "codex_desktop_cdp_unavailable"}
    client: _CdpClient | None = None
    try:
        client = _CdpClient(ws_url)
        result = client.evaluate(_legacy_runtime_shim_probe_expression())
        result = result if isinstance(result, dict) else {"raw": result}
        return {
            **state,
            "ok": bool(result.get("ok", True)),
            "legacy_markers_present": bool(result.get("legacyMarkersPresent")),
            "result": result,
            "reason": "legacy_runtime_shims_present" if result.get("legacyMarkersPresent") else "native_runtime_clean",
        }
    except Exception as exc:
        return {**state, "ok": False, "reason": "legacy_runtime_shim_probe_failed", "error": repr(exc)}
    finally:
        if client is not None:
            client.close()


def _legacy_runtime_shim_retire_expression(module_specifier: str) -> str:
    payload = json.dumps({"moduleSpecifier": module_specifier}, separators=(",", ":"))
    return r"""
(async () => {
  const payload = __PAYLOAD__;
  const result = {
    schema: 'codex-desktop-model-runtime/legacy-shim-retirement-result/v1',
    ok: true,
    retired: false,
    modelListRestored: false,
    appServerRestored: false,
    storageRestored: false,
    statsigClientsRestored: 0,
    reasoningClientsRestored: 0,
    timersCleared: 0,
    globalsCleared: 0,
  ...
  try {
    ...
    if (bridge && typeof window.__codexModelListShimOriginalSendMessageFromView === 'function') {
      bridge.sendMessageFromView = window.__codexModelListShimOriginalSendMessageFromView;
      result.modelListRestored = true;
    }
  ...
  } catch (error) { result.errors.push('model-list:' + String(error)); }
  ...
  try {
    if (payload.moduleSpecifier) {
      ...
      const appServerBridge = bridgeEntry && bridgeEntry[1];
      const original = appServerBridge && (
        appServerBridge.__codexAppServerModelShimOriginalSendRequest
        || window.__codexAppServerModelShimOriginalSendRequest
      );
      if (appServerBridge && typeof original === 'function') {
        appServerBridge.sendRequest = original;
        delete appServerBridge.__codexAppServerModelShimOriginalSendRequest;
        delete appServerBridge.__codexAppServerModelShimWrapperSendRequest;
        delete appServerBridge.__codexAppServerModelShimGeneration;
        delete appServerBridge.__codexAppServerModelShimCatalogSignature;
        result.appServerRestored = true;
      }
    }
  } catch (error) { result.errors.push('app-server:' + String(error)); }

  try {
    const statsig = window.__STATSIG__;
    const clients = statsig && typeof statsig === 'object'
      ? Array.from(new Set([statsig.firstInstance, ...Object.values(statsig.instances || {})]
          .filter((item) => item && typeof item === 'object')))
      : [];
    for (const client of clients) {
      if (typeof client.__codexAllowlistOriginalGetDynamicConfig === 'function') {
        client.getDynamicConfig = client.__codexAllowlistOriginalGetDynamicConfig;
        result.statsigClientsRestored += 1;
      }
      if (typeof client.__codexReasoningOriginalCheckGate === 'function') {
        client.checkGate = client.__codexReasoningOriginalCheckGate;
        result.reasoningClientsRestored += 1;
      }
      for (const key of Object.keys(client)) {
        if (key.startsWith('__codexAllowlist') || key.startsWith('__codexReasoning')) delete client[key];
      }
    }
    if (typeof window.__codexStatsigNativeSetItem === 'function') {
      Storage.prototype.setItem = window.__codexStatsigNativeSetItem;
      result.storageRestored = true;
    }
  } catch (error) { result.errors.push('statsig:' + String(error)); }

  for (const key of ['__codexStatsigAllowlistProtectionTimer', '__codexReasoningCapabilityPoller']) {
    try {
      if (window[key]) {
        clearInterval(window[key]);
        result.timersCleared += 1;
      }
    } catch (error) { result.errors.push('timer:' + key + ':' + String(error)); }
  }

  const prefixes = [
    '__codexModelListShim', '__codexInstallModelListBridgeShim',
    '__codexAppServerModelShim', '__codexInstallAppServerModelShim',
    '__codexStatsig', '__codexReasoning',
  ];
  for (const key of Object.getOwnPropertyNames(window)) {
    if (!prefixes.some((prefix) => key.startsWith(prefix))) continue;
    try { if (delete window[key]) result.globalsCleared += 1; } catch {}
  }
  window.dispatchEvent(new MessageEvent('message', {data: {
    type: 'ipc-broadcast', method: 'query-cache-invalidate',
    params: {queryKey: ['models', 'list']}, sourceClientId: 'codex-native-runtime-retirement',
  }}));
  result.modelQueryInvalidated = true;
  result.retired = Boolean(
    result.modelListRestored || result.appServerRestored || result.storageRestored
    || result.statsigClientsRestored || result.reasoningClientsRestored
    || result.timersCleared || result.globalsCleared
  );
...
def retire_legacy_runtime_shims(wait_seconds: float = 0.0) -> dict[str, Any]:
    native = native_desktop_model_runtime_state()
    state: dict[str, Any] = {
        "schema": "codex-desktop-model-runtime/legacy-shim-retirement/v1",
        "ok": True,
        "native_runtime": native,
        "retired": False,
        "reason": "",
    }
    if not native.get("native_authoritative"):
        return {**state, "skipped": True, "reason": "legacy_compatibility_runtime"}
    port, ws_url, pages, reason = _wait_for_codex_page(wait_seconds)
    state.update({"cdp_port": port, "page_count": len(pages)})
    if not ws_url:
        return {**state, "skipped": True, "reason": reason or "codex_desktop_cdp_unavailable"}
    discovery = codex_appserver_model_bridge.discover_host_module()
    module_specifier = str(discovery.get("module_specifier") or "")
    client: _CdpClient | None = None
    try:
        client = _CdpClient(ws_url)
        result = client.evaluate(_legacy_runtime_shim_retire_expression(module_specifier))
        result = result if isinstance(result, dict) else {"raw": result}
        return {
            **state,
            ...
            "result": result,
            "reason": "legacy_runtime_shims_retired" if result.get("retired") else "native_runtime_already_clean",
        }
    except Exception as exc:
        return {**state, "ok": False, "reason": "legacy_runtime_shim_retirement_failed", "error": repr(exc)}
    finally:
        if client is not None:
            client.close()


def _native_runtime_retirement_skip(
    ...
    *,
    ...
    if not native.get("native_authoritative"):
        return None
    # An app.asar signature only proves that this Desktop version can contain
    # the native handler.  A legacy renderer shim can make the live probe look
    # successful and would then be removed incorrectly.  Retire shims only
    # when the current Electron process answers the real request and no legacy
    # marker is still present.
    if not expected_models:
        return None
    live = desktop_runtime_state(list(expected_models))
    if live.get("list_models_for_host_ok") is not True:
        return None
    legacy = legacy_runtime_shim_state(wait_seconds=wait_seconds)
    if not legacy.get("ok") or legacy.get("skipped") or legacy.get("legacy_markers_present"):
        return None
    retirement = retire_legacy_runtime_shims(wait_seconds=wait_seconds)
    return {
        ...
        "reason": "native_desktop_runtime_authoritative",
    }
...
def apply_model_list_bridge_shim(
    ...
    native_skip = _native_runtime_retirement_skip(wait_seconds, expected_models=fallback_model_ids)
    if native_skip is not None:
        return {
    ...
    models = _catalog_bridge_models(catalog_path, fallback_model_ids)
    state: dict[str, Any] = {
        "schema": "codex-desktop-model-runtime/model-list-bridge-shim/v1",
        "ok": False,
        "catalog_path": str(catalog_path) if catalog_path else "",
...
def apply_appserver_model_shim(
    ...
    native_skip = _native_runtime_retirement_skip(wait_seconds)
    if native_skip is not None:
        return {
...
def model_picker_view_sync_state(
    ...
    *,
    ...
    """
    native = native_desktop_model_runtime_state()
    if native.get("native_authoritative"):
        retirement = retire_legacy_runtime_shims(wait_seconds=wait_seconds) if apply else None
        return {
            "schema": "codex-desktop-model-runtime/model-picker-view-sync/v2",
            "ok": bool(retirement.get("ok")) if retirement is not None else True,
            "signature": signature,
            "apply": apply,
            "skipped": True,
            "result": {},
    ...
    if not catalog_path or not catalog_path.exists():
        return [
            ...
            for model_id in dict.fromkeys(
                str(item).strip() for item in (fallback_model_ids or []) if str(item).strip()
            )
        ]
    try:
        data = json.loads(catalog_path.read_text(encoding="utf-8"))
...
(async () => {{
  ...
  for (const key of keys) {{
    try {{
      ...
      // The custom provider's live /models response is authoritative.  Do
      // not retain stale Desktop or Statsig cache entries after that provider
      // stops advertising them.
      const synced = requiredModels.slice();
      const added = synced.filter((item) => !before.includes(item));
      const removed = before.filter((item) => !synced.includes(item));
...
def statsig_allowlist_sync_state(
    ...
    *,
    ...
    model_ids = list(dict.fromkeys(
        str(item).strip()
        for item in (fallback_model_ids if fallback_model_ids is not None else _catalog_visible_model_ids(catalog_path))
        if str(item).strip()
    ))
    native = native_desktop_model_runtime_state()
    if native.get("native_authoritative"):
        live = desktop_runtime_state(model_ids)
        if live.get("list_models_for_host_ok") is True or not model_ids:
            retirement = retire_legacy_runtime_shims(wait_seconds=wait_seconds) if apply else None
            return {
...
(() => {{
  ...
  const signature = String(payload.signature || '');
  const marker = 'codex-statsig-allowlist-protection/v5';
  ...
  function mergeOuter(raw) {{
    ...
    const before = value.available_models.filter((item) => typeof item === 'string');
    const merged = activeRequiredModels();
    const added = merged.filter((item) => !before.includes(item));
    const removed = before.filter((item) => !merged.includes(item));
  ...
  function patchDynamicConfig(config) {{
    ...
    config.value.available_models = activeRequiredModels();
    return config;
  }}
...
def statsig_allowlist_protection_state(
    ...
    *,
    ...
    native = native_desktop_model_runtime_state()
    # With no provider models there is no compatibility payload to protect.
    # Preserve the native no-op path and, crucially, do not retire any live
    # renderer state merely because the installed package advertises support.
    if not model_ids and native.get("native_authoritative"):
        return {
            ...
            "native_runtime": native,
            "retirement": None,
            "reason": "native_desktop_runtime_no_provider_models",
        }
    native_skip = _native_runtime_retirement_skip(wait_seconds, expected_models=model_ids)
    if native_skip is not None:
        return {
            "schema": "codex-desktop-model-runtime/statsig-allowlist-protection/v1",
            "catalog_path": str(catalog_path) if catalog_path else "",
...
def desktop_runtime_state(expected_models: list[str]) -> dict[str, Any]:
    ...
    state: dict[str, Any] = {
        ...
        "list_models_for_host_models": [],
        "list_models_for_host_missing_expected_models": [],
        "list_models_for_host_unexpected_models": [],
        "list_models_for_host_exact": False,
    ...
    if list_ok:
        try:
            payload = json.loads(str(list_models.get("bodyJsonString") or "{}"))
        ...
        if isinstance(rows, list):
            ...
            for row in rows:
                if not isinstance(row, dict):
                    continue
                model_id = _catalog_model_id(row)
                if model_id:
                    returned.append(model_id)
            state["list_models_for_host_models"] = returned
        expected = list(expected_models)
        returned = list(state["list_models_for_host_models"])
        state["list_models_for_host_missing_expected_models"] = [
            model for model in expected if model not in returned
        ]
        state["list_models_for_host_unexpected_models"] = [
            model for model in returned if model not in expected
        ]
        state["list_models_for_host_exact"] = returned == expected
    else:
    ...
    elif not state["list_models_for_host_exact"]:
        advisory_findings.append("electron_list_models_for_host_result_mismatch")
...
def catalog_reasoning_repair_plan(catalog_path: Path | None) -> dict[str, Any]:
    ...
    if native.get("native_authoritative"):
        return {
            ...
            "catalog_path": str(catalog_path) if catalog_path else "",
            "planned_models": [],
            "target_reasoning_levels": [],
            "native_runtime": native,
            "reason": "native_desktop_runtime_authoritative",
            "policy": "Desktop and its selected host own model and reasoning capability metadata",
        }
...
def main() -> int:
    ...
    if args.action == "native-runtime":
        result = native_desktop_model_runtime_state()
    elif args.action == "legacy-runtime-shims-retire":
        result = retire_legacy_runtime_shims(wait_seconds=args.wait_seconds)
    elif args.action == "page-reload":
