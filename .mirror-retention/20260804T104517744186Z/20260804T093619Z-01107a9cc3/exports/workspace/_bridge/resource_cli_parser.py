...
class ResourceCliParserConfig:
    ...
    command_expand_content: CommandHandler
...
def add_package_manager_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        choices=("", "inspect_then_install", "verified_auto_install"),
        default="",
        help="Python package owner mode: return a verified install plan, or complete an already-approved isolated install.",
    )
    ...
    parser.add_argument("--authorization-grant-ref", default="", help="Scoped-authorization grant reference for this exact install operation.")
    parser.add_argument("--authorization-operation-id", default="", help="Stable idempotent operation id bound to the scoped authorization grant.")
    parser.add_argument("--authorization-thread-id", default="", help="Current user-turn or automation-run identity used by the authorization scope.")
...
def add_routing_and_request_commands(
    ...
    def add_job_execution_args(job_parser: argparse.ArgumentParser) -> None:
        ...
        job_parser.add_argument("--receipt-detail", choices=("compact", "full"), default="compact", help="Codex-facing receipt projection; full is richer but remains bounded.")
    ...
    for name, help_text in (
        ...
        job_cmd.add_argument("--receipt-detail", choices=("compact", "full"), default="compact", help="Codex-facing receipt projection; full is richer but remains bounded.")
        if name == "wait":
    ...
    job_consume.add_argument("--receipt-detail", choices=("compact", "full"), default="compact", help="Codex-facing receipt projection; full is richer but remains bounded.")
...
def add_progress_and_result_commands(
    ...
    expand_content_parser = subparsers.add_parser("expand-content", help="Read a bounded fragment from an existing full-content reference without network access.")
    expand_content_parser.add_argument("--content-ref", required=True, help="Full content path returned by the resource receipt.")
    expand_content_parser.add_argument("--query", default="", help="Case-insensitive literal query to locate in the full content.")
    expand_content_parser.add_argument("--line-start", type=int, default=0)
    expand_content_parser.add_argument("--line-count", type=int, default=0)
    expand_content_parser.add_argument("--byte-start", type=int, default=0)
    expand_content_parser.add_argument("--byte-count", type=int, default=0)
    expand_content_parser.add_argument("--max-chars", type=int, default=12000)
    expand_content_parser.add_argument("--json", action="store_true", help="Emit JSON.")
    expand_content_parser.set_defaults(func=config.command_expand_content)
