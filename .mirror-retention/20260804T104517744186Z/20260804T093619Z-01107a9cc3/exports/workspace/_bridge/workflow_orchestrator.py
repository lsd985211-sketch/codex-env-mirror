...
from intent_routing import IntentRule, has_install_action, matched_terms, rank_intents, term_matches, term_weight
from mcp_route_policy import call_priority_pack
from maintenance_capability_registry import (
    RegistrySnapshot,
    create_registry_snapshot,
    query_registry as query_maintenance_registry,
...
from workflow_validation_shadow import (
    ValidationPlanShadow,
    compact_shadow,
    signature_for_files,
    signature_for_value,
)
from workflow_validation_plan_session import ValidationPlanSession
from workflow_validation_dag_shadow import attach_dag_shadow, build_validation_dag_shadow, compact_dag_shadow
from workflow_validation_receipt_reuse import (
    attach_reuse_projection,
    context_signature as validation_receipt_context_signature,
    load_receipt as load_validation_receipt,
    store_receipt as store_validation_receipt,
)
from workflow_task_execution_budget import build_execution_budget
...
try:
    from system_membership import domain_binding_report, retirement_signal as build_retirement_signal, systems_for_domain
    ...
    domain_binding_report = None
    systems_for_domain = None
...
DOMAINS: tuple[Domain, ...] = (
    ...
    Domain(
        ...
        ("hardware-ops", "windows-usb-ops", "global-framework"),
        (),
        (),
        ("_bridge/hardware_system_owner.py", "_bridge/wsl_hardware_owner.py", "_bridge/windows_hardware_owner.py", "_bridge/usb_device_owner.py", "_bridge/usb_device_control.py", "_bridge/docs/hardware_system_capability_model.md"),
        ("hardware_system_owner.py routes", "hardware_system_owner.py validate", "wsl_hardware_owner.py doctor", "wsl_hardware_owner.py validate", "windows_hardware_owner.py doctor", "windows_hardware_owner.py validate", "usb_device_owner.py doctor", "usb_device_owner.py validate", "usb_device_control.py validate"),
    ),
    Domain(
        "mcp_tools",
        "MCP / tool stability",
    ...
    Domain(
        ...
        "code review / deterministic scope and rule preparation",
        (
            "code review",
            "review changes",
        ...
        ("open-code-review-delegate", "bug-hunt-swarm", "codegraph-ops"),
        ("workflow-router", "post-work-closeout"),
        ("ocr", "codegraph", "git"),
        ("_bridge/skill_orchestrator.py", "_bridge/tool_utilization_audit.py"),
        ("ocr version", "ocr delegate preview", "ocr delegate rule <paths>", "targeted tests"),
    ),
    Domain(
    ...
    Domain(
        "document_processing",
        "local PDF and OCR capability discovery / document processing",
        (
            "扫描pdf",
            "扫描 pdf",
            "扫描件",
        ...
        (),
        ("pdf", "ocr"),
        ("_bridge/environment_capability_view.py", "_bridge/workflow_environment_context.py"),
        ("read-only local capability discovery", "OCR owner evidence", "manual OCR fallback"),
    ),
    Domain(
        "textbook_knowledge",
        "local medical textbook retrieval with source-grounded citations",
        (
            "医学教材",
            "教材概念",
            "教材知识包",
        ...
        ("medical-textbook-study",),
        (),
        ("medical-textbook-study",),
        ("medical_textbook_mcp.py medical_search",),
        ("citation-grounded textbook result", "consumer-owned FTS/graph fallback"),
    ),
    Domain(
        "semantic_retrieval",
        "shared semantic model / vector retrieval / bounded reranking",
        (
            "语义检索",
            "语义搜索",
            "向量检索",
        ...
        ("global-framework", "diagnose"),
        ("workflow-router", "post-work-closeout"),
        ("semantic_capability_owner", "selected_consumer_adapter"),
        ("_bridge/environment_capability_view.py", "_bridge/workflow_environment_context.py"),
        ("semantic owner status", "consumer adapter evidence", "lexical/graph fallback"),
    ),
    Domain(
        "editing_backup",
...
CODE_IMPLEMENTATION_CONTEXT_TERMS = (
    "代码",
    "脚本",
    "模块",
    "适配器",
...
EXISTING_SEARCH_ARTIFACT_TERMS = ("搜索结果", "检索结果", "代码搜索结果", "search result", "search output")
EXPLICIT_EXTERNAL_SEARCH_TERMS = ("联网", "查资料", "相关知识", "官方文档", "research", "web search")
...
def contextual_domain_bonus(text: str, domain: Domain, hits: list[str]) -> int:
    ...
    has_implementation_context = bool(matched_terms(text, CODE_IMPLEMENTATION_CONTEXT_TERMS))
    ...
    has_system_context = bool(matched_terms(text, WORKFLOW_SYSTEM_CONTEXT_TERMS))
    ...
    if has_workflow_context and has_action:
        return 2
    if has_system_context and has_action:
        return 3
...
def domain_match_record(
    ...
    record = {
        ...
        "systems": systems_for_domain(domain.key, consumer="workflow") if systems_for_domain is not None else [],
    }
    if evidence:
...
def classify(message: str, *, max_domains: int = 3) -> list[dict[str, Any]]:
    ...
    install_action_present = has_install_action(text)
    existing_search_artifact = bool(matched_terms(text, EXISTING_SEARCH_ARTIFACT_TERMS))
    explicit_external_search = bool(matched_terms(text, EXPLICIT_EXTERNAL_SEARCH_TERMS))
    domain_keywords = {}
    for domain in DOMAINS:
        keywords = []
        for keyword in domain.keywords:
            if (
                domain.key == "external_docs_research"
                and existing_search_artifact
                and not explicit_external_search
                and keyword == "搜索"
            ):
                continue
            if (
                domain.key == "resource_acquisition"
                and not install_action_present
                and keyword in {"安装", "install", "package install"}
            ):
                continue
            keywords.append(keyword)
        domain_keywords[domain.key] = tuple(keywords)
    hits_by_key = {domain.key: matched_terms(text, domain_keywords[domain.key]) for domain in DOMAINS}
    bonuses = {
    ...
    ranked = rank_intents(
        text,
        tuple(IntentRule(domain.key, domain_keywords[domain.key], tuple(LOW_SIGNAL_KEYWORDS)) for domain in DOMAINS),
        bonuses=bonuses,
    )
...
def phase_tool_route(ctx: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "read_only": False,
        "approval_required": False,
        "approval_reason": "build only writes derived runtime cache; source contracts remain unchanged",
        "fallback": {
            "kind": "targeted_route_lookup",
            "command": f"python _bridge\\mcp_capability_routes.py lookup --terms {ctx['route_terms_arg']}",
            "detail_ref": str(MATRIX),
            "detail_rule": "expand only the matching matrix section after the route index is insufficient",
        },
...
def phase_module_context(ctx: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "commands": [
            ...
            machine_command(context_command, read_only=True, required=True),
...
def apply_execution_budget_to_phases(
    phases: list[dict[str, Any]], validation_tier: str, phase_selection: dict[str, Any] | None = None
) -> list[dict[str, Any]]:
    """Project validation tier and only the budget owner's guarded soft skips."""

    early = {
        "phase_1_preflight", "phase_2_recall", "phase_3_skill_selection",
        "phase_4_template_render", "phase_5_tool_route",
    }
    projected: list[dict[str, Any]] = []
    selected = phase_selection if isinstance(phase_selection, dict) else {}
    skip_ids = set(str(item) for item in selected.get("skip_phase_ids") or [])
    skip_reasons = selected.get("skip_reasons") if isinstance(selected.get("skip_reasons"), dict) else {}
    for phase in phases:
        row = deepcopy(phase)
        phase_id = str(row.get("id") or "")
        enabled = row.get("enabled") is True and phase_id not in skip_ids
        if phase_id in skip_ids:
            row["skip_reason"] = str(skip_reasons.get(phase_id) or "budget_soft_phase_dedup")
            row["budget_skipped"] = True
            row["commands"] = []
        tier = "none" if not enabled else ("quick" if row.get("id") in early else validation_tier)
        row["validation_tier"] = tier
        row["action_contract"] = phase_action_contract(row, enabled, tier)
...
def workflow_tools_contract(
    *,
    ...
    return {
        "matrix": str(MATRIX),
        "matrix_role": "explicit_detail_reference_not_default_full_load",
        "lookup_command": "python _bridge\\mcp_capability_routes.py lookup --terms <terms>",
...
def build_plan(
    ...
    *,
    ...
    task_contract = resolve_task_route_contract(message, selected_domain_keys)
    task_facts = task_contract.task_facts or {}
    from workflow_asset_guidance import SELF_CONTAINED_SIMPLE_TERMS, SUBSTANTIVE_TASK_FACTS

    simple_fast_path = (
        selected_domain_keys == ["general"]
        and not any(bool(task_facts.get(key)) for key in SUBSTANTIVE_TASK_FACTS)
        and bool(matched_terms(message.casefold(), SELF_CONTAINED_SIMPLE_TERMS))
    )
    execution_budget = build_execution_budget(
        profile=str(profile.get("profile") or "general"),
        task_facts=task_facts,
        machine_phases=machine_phases,
        risk=risk,
        selected_asset_count=len(selected_skills) + len(selected_slash),
        route_term_count=len(matrix_terms),
    ...
    profile = {
        **profile,
        "validation_tier": execution_budget["validation_tier"],
        "validation_tier_owner": "codex_task_execution_budget",
        "validation_tier_reason": execution_budget["validation_reason"],
    }
    machine_phases = apply_execution_budget_to_phases(
        machine_phases,
        execution_budget["validation_tier"],
        execution_budget.get("phase_selection"),
    )
    ...
    plan = {
        ...
        "profile": profile,
        "task_execution_budget": execution_budget,
        ...
        "workflow": [
            ...
            "query mcp_capability_routes.py for the owning MCP and fallback; expand only the selected matrix section when needed",
        ...
        "execution_plan": {
            ...
            "validation_tier": profile.get("validation_tier"),
            "state_change_expected": profile.get("state_change_expected"),
            "task_execution_budget_ref": "task_execution_budget",
...
def doctor(*, registry_snapshot: RegistrySnapshot | None = None) -> dict[str, Any]:
    snap = snapshot()
    ...
    if MAINTENANCE_SURFACE_MAP.exists():
        required_terms = {
            "workflow": ("workflow_orchestrator.py", "maintenance_upgrade_governance.py", "tool_coordination.py"),
            "mcp": ("mcp_session_doctor.py", "local_mcp_hub.py"),
            "backup": ("backup_hygiene_doctor.py",),
            "memory": ("memory_governance.py",),
            "skills": ("code_maintainability.py",),
        }
        missing_terms = []
        for system, terms in required_terms.items():
            for term in terms:
                result = query_maintenance_registry(
                    system=system, term=term, limit=1, snapshot=registry_snapshot
                )
                if not result.get("ok") or not result.get("returned"):
            ...
            issues.append(
                {
                    ...
                    "message": f"Maintenance registry missing terms: {', '.join(missing_terms)}",
                }
            )
...
def stable_skill_context_signature(context: Any) -> str:
    """Hash skill evidence, excluding refresh metadata that changes per call."""

    if not isinstance(context, dict):
        return signature_for_value(context)
    stable = deepcopy(context)
    refresh = stable.get("lifecycle_refresh")
    if isinstance(refresh, dict):
        refresh.pop("generated_at", None)
    return signature_for_value(stable)


def validate() -> dict[str, Any]:
    observation = ValidationObservation()
    observation.start_group("preflight")
    registry_candidate = create_registry_snapshot()
    registry_snapshot = registry_candidate if registry_candidate.ok else None
    registry_report = {
        "schema": "maintenance_registry_snapshot_session.v1",
        "ok": registry_snapshot is not None,
        "snapshot": registry_candidate.summary(),
        "fallback_reason": "" if registry_snapshot is not None else registry_candidate.reason,
        "scope": "single_workflow_orchestrator_validate_invocation",
        "persistent": False,
        "authorization_reuse": False,
        "validation_receipt_reuse": False,
        "validation_skipping": False,
    }
    doc = doctor(registry_snapshot=registry_snapshot)
    ...
    if prepare_skill_routing_context is not None:
        try:
            ...
            skill_routing_context = None
    skill_context_signature = stable_skill_context_signature(skill_routing_context)
    workflow_source_signature = signature_for_files([Path(__file__), BRIDGE / "workflow_validation.py"])
    registry_source_signature = (
        str(registry_snapshot.read_view.source_signature or "")
        if registry_snapshot is not None else ""
    )
    receipt_context_signature = validation_receipt_context_signature(
        workflow_source_signature=workflow_source_signature,
        skill_context_signature=skill_context_signature,
        registry_source_signature=registry_source_signature,
    )
    cached_receipt = load_validation_receipt(receipt_context_signature)
    if cached_receipt.get("ok"):
        return attach_reuse_projection(
            cached_receipt["payload"], reused=True,
            receipt_ref=str(cached_receipt.get("receipt_ref") or ""),
        )
    shadow = ValidationPlanShadow(
        skill_context_signature=skill_context_signature,
        workflow_source_signature=workflow_source_signature,
    )
    def validation_build_plan(message: str, **kwargs: Any) -> dict[str, Any]:
        return build_plan(message, registry_snapshot=registry_snapshot, **kwargs)

    plan_session = ValidationPlanSession(
        validation_build_plan,
    ...
    def validation_plan(
        ...
        *,
        ...
        consumer = str(_consumer or (caller.f_code.co_name if caller else "unknown"))
        source_owner = str(
            _source_owner
            or (Path(caller.f_code.co_filename).stem if caller else "workflow_orchestrator")
        )
        scenario_counts[consumer] = scenario_counts.get(consumer, 0) + 1
        scenario_id = str(_scenario_id or f"{consumer}.{scenario_counts[consumer]}")
        plan = plan_session.plan_for(message, detail=detail)
        observation.record_plan(plan)
        return shadow.record_plan(
            plan,
            message=message,
            detail=detail,
    ...
    observation.finish_group("preflight")
    observation.start_group("core_validation_contracts")
    plans = [
        validation_plan(
            sample,
            _scenario_id=f"validation_sample.{index}",
            _consumer="phase_contract",
            _source_owner="workflow_validation",
        )
    ...
    observation.finish_group("core_validation_contracts")
    observation.start_group("authority_and_routing_contracts")
    workflow_domain_coverage = (
        domain_binding_report(workflow_domains={domain.key for domain in DOMAINS})
        if domain_binding_report is not None
        else {"ok": False, "reason": "system_membership_unavailable"}
    )
    checks.append(
        {
            "name": "all_system_members_have_discoverable_workflow_domains",
            "ok": workflow_domain_coverage.get("ok") is True,
            "detail": workflow_domain_coverage,
        }
    )
    ...
    # This check consumes execution_route_pack.route_decision; micro projection
    # intentionally omits that field, so use the full validation projection.
    member_change_plan = validation_plan("新增一个 MCP server 并纳入工作环境", detail="full")
    ...
    observation.finish_group("authority_and_routing_contracts")
    observation.start_group("environment_context_contracts")
    environment_validation = validate_environment_context(registry_snapshot=registry_snapshot)
    checks.append(
        {
    ...
    observation.finish_group("environment_context_contracts")
    authoritative = {
        ...
        "validation_plan_session": plan_session.report(),
        "maintenance_registry_session": registry_report,
    }
    dag_queries = [
        {"system": system, "term": "validate doctor", "limit": 100}
        for system in ("workflow", "mcp", "resource", "windows")
    ]
    dag_batch = query_registry_batch(dag_queries, snapshot=registry_snapshot)
    dag_shadow = build_validation_dag_shadow(
        ["workspace/_bridge/workflow_orchestrator.py"],
        registry_batch=dag_batch,
        registry_rows=registry_snapshot.rows() if registry_snapshot is not None else None,
    )
    result = observation.attach(shadow.attach(attach_dag_shadow(authoritative, dag_shadow)))
    stored = store_validation_receipt(result, receipt_context_signature)
    return attach_reuse_projection(
        result, reused=False, receipt_ref=str(stored.get("receipt_ref") or ""),
    )
...
def cli_projection(payload: dict[str, Any], command: str, *, full: bool = False) -> dict[str, Any]:
    if command != "validate":
        return payload
    projected = aggregate_validator_cli_payload(
        payload,
        full=full,
        full_result_ref="command:python _bridge/workflow_orchestrator.py validate --full",
    )
    if isinstance(payload.get("validation_observation"), dict):
        projected["validation_observation"] = payload["validation_observation"]
    if isinstance(payload.get("validation_plan_session"), dict):
        projected["validation_plan_session"] = payload["validation_plan_session"]
    if isinstance(payload.get("maintenance_registry_session"), dict):
        projected["maintenance_registry_session"] = payload["maintenance_registry_session"]
    if isinstance(payload.get("validation_shadow"), dict):
        projected["validation_shadow"] = (
            payload["validation_shadow"]
            if full
            else compact_shadow(payload["validation_shadow"])
        )
    if isinstance(payload.get("validation_dag_shadow"), dict):
        projected["validation_dag_shadow"] = (
            payload["validation_dag_shadow"]
            if full
            else compact_dag_shadow(payload["validation_dag_shadow"])
        )
    for field in (
        "validation_receipt_reuse",
        "validation_receipt_reuse_status",
...
def main(argv: list[str] | None = None) -> int:
    ...
    plan_parser.add_argument(
        "--feedback-json",
        type=Path,
        help="Optional existing observation recommendation projection; it is consumed only when its budget signature and guardrails match.",
    )
    ...
    if args.command == "snapshot":
        ...
        if args.feedback_json:
            try:
                loaded = json.loads(args.feedback_json.read_text(encoding="utf-8"))
                feedback_projection = loaded if isinstance(loaded, Mapping) else {"invalid": "feedback_projection_not_object"}
            except (OSError, json.JSONDecodeError):
                feedback_projection = {"invalid": "feedback_projection_unreadable"}
        payload = build_plan(
            args.message,
            risk=args.risk,
            detail=args.detail,
