...
class ModelProviderRuntimeTests(unittest.TestCase):
    def test_empty_proxy_catalog_falls_back_to_active_provider_upstream(self) -> None:
        config = {
            "model": "gpt-5.6-terra",
            "model_provider": "custom",
            "model_providers": {"custom": {"base_url": "http://127.0.0.1:15721/v1"}},
        }
        settings = {
        ...
        response.status = 200
        response.read.return_value = json.dumps({"data": [{"id": "gpt-5.6-terra"}, {"id": "gpt-5.6-sol"}]}).encode()
        response.__enter__.return_value = response
        response.__exit__.return_value = False
        with (
            mock.patch.object(config_guard.sqlite3, "connect", return_value=connection),
            mock.patch.object(config_guard.urllib.request, "urlopen", return_value=response),
        ):
            state = config_guard._direct_provider_model_list_state(config, {
                "base_url": "http://127.0.0.1:15721/v1",
                "configured_model": "gpt-5.6-terra",
            })
        self.assertIsNotNone(state)
        self.assertEqual(state["discovery_capability"], "cc_switch_provider_direct")
        self.assertEqual(state["model_ids"], ["gpt-5.6-terra", "gpt-5.6-sol"])

    def test_dynamic_model_ids_can_back_bridge_without_persisted_catalog(self) -> None:
        models = runtime._catalog_bridge_models(None, ["gpt-5.6-terra", "gpt-5.6-sol"])
        self.assertEqual([item["model"] for item in models], ["gpt-5.6-terra", "gpt-5.6-sol"])
        self.assertTrue(all(item["hidden"] is False for item in models))

    def test_native_desktop_runtime_detection_requires_account_ultra_and_host_models(self) -> None:
        asset = Path("webview/assets/app-initial-test.js")
        payload = b"model_picker_persists_ultra_effort account_user_setting show-ultra-in-model-picker-slider list-models-for-host enabled-reasoning-efforts"
        with (
            mock.patch.object(appserver_bridge, "candidate_app_asars", return_value=[Path("app.asar")]),
            mock.patch.object(appserver_bridge, "asar_files", return_value={str(asset): {"size": len(payload)}}),
            mock.patch.object(appserver_bridge, "read_asar_asset", return_value=payload),
        ):
            state = runtime.native_desktop_model_runtime_state()

        self.assertTrue(state["ok"])
        self.assertTrue(state["native_authoritative"])
        self.assertEqual(str(asset), state["asset_path"])

    def test_native_desktop_runtime_skips_legacy_model_list_install_and_retires_shims(self) -> None:
        native = {"ok": True, "native_authoritative": True, "reason": "native_desktop_runtime_available"}
        retired = {"ok": True, "retired": True, "reason": "legacy_runtime_shims_retired"}
        live = {"list_models_for_host_ok": True, "list_models_for_host_exact": True}
        clean = {"ok": True, "skipped": False, "legacy_markers_present": False}
        with (
            mock.patch.object(runtime, "native_desktop_model_runtime_state", return_value=native),
            mock.patch.object(runtime, "desktop_runtime_state", return_value=live),
            mock.patch.object(runtime, "legacy_runtime_shim_state", return_value=clean),
            mock.patch.object(runtime, "retire_legacy_runtime_shims", return_value=retired) as retire,
        ):
            state = runtime.apply_model_list_bridge_shim(Path("catalog.json"), ["provider-a"])

        self.assertTrue(state["ok"])
        self.assertTrue(state["skipped"])
        self.assertEqual("native_desktop_runtime_authoritative", state["reason"])
        self.assertEqual(retired, state["retirement"])
        retire.assert_called_once()

    def test_native_runtime_does_not_retire_legacy_shim_that_answers_the_probe(self) -> None:
        native = {"ok": True, "native_authoritative": True}
        live = {"list_models_for_host_ok": True, "list_models_for_host_exact": True}
        legacy = {"ok": True, "skipped": False, "legacy_markers_present": True}
        with (
            mock.patch.object(runtime, "native_desktop_model_runtime_state", return_value=native),
            mock.patch.object(runtime, "desktop_runtime_state", return_value=live),
            mock.patch.object(runtime, "legacy_runtime_shim_state", return_value=legacy),
            mock.patch.object(runtime, "retire_legacy_runtime_shims") as retire,
        ):
            state = runtime._native_runtime_retirement_skip(expected_models=["provider-a"])
        self.assertIsNone(state)
        retire.assert_not_called()

    def test_native_runtime_binding_is_clean_without_legacy_renderer_markers(self) -> None:
        native = {"ok": True, "native_authoritative": True, "reason": "native_desktop_runtime_available"}
        clean = {"ok": True, "legacy_markers_present": False, "reason": "native_runtime_clean"}
        with (
            mock.patch.object(runtime, "native_desktop_model_runtime_state", return_value=native),
            mock.patch.object(runtime, "desktop_runtime_state", return_value={"list_models_for_host_exact": True}),
            mock.patch.object(runtime, "legacy_runtime_shim_state", return_value=clean),
        ):
            state = watcher.runtime_binding_state(["provider-a"], ["low", "medium", "high", "xhigh", "max", "ultra"])

        self.assertTrue(state["bound"])
        self.assertEqual("native", state["binding_route"])
        self.assertFalse(state["legacy_runtime_shims_present"])

    def test_config_guard_uses_platform_codex_home(self) -> None:
        self.assertEqual(config_guard.CODEX_HOME, config_guard.codex_home())
        self.assertEqual(config_guard.CODEX_HOME / "config.toml", config_guard.CODEX_CONFIG)

    def test_desktop_runtime_config_stays_separate_from_wsl_authority(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            wsl_config = root / "wsl-config.toml"
            host_config = root / "host-config.toml"
            baseline = {
                "configuration_authority": "wsl_active",
                "global_config": str(wsl_config),
                "desktop_host_config": str(host_config),
            }

            resolved = config_guard.desktop_runtime_config_path(baseline, wsl_config)

        self.assertEqual(host_config, resolved)

    def test_windows_appserver_discovery_does_not_probe_wsl_mount(self) -> None:
        original_is_dir = Path.is_dir

        def guarded_is_dir(path: Path) -> bool:
            if str(path) == "/mnt/c":
                raise AssertionError("Windows must not probe the WSL mount path")
            return original_is_dir(path)

        with mock.patch.object(appserver_bridge, "_running_codex_executables", return_value=[]), \
                mock.patch.object(appserver_bridge.os, "name", "nt"), \
                mock.patch.object(Path, "is_dir", guarded_is_dir):
            self.assertEqual([], appserver_bridge.candidate_app_asars())

    def test_wsl_validation_defers_windows_scheduled_task_owner(self) -> None:
        with mock.patch.object(watcher.os, "name", "posix"), \
                mock.patch.object(watcher.subprocess, "run") as run:
            task = watcher._scheduled_task_state()
        self.assertTrue(task["deferred"])
        self.assertEqual(task["reason"], "windows_scheduled_task_owner_deferred")
        run.assert_not_called()

    def test_wsl_validate_does_not_fail_on_windows_only_watcher_state(self) -> None:
        snapshot = {
            "source": {"signature_hash": "signature", "catalog_provenance": {"ok": True}},
            "scheduled_task": {"deferred": True},
            "runtime_binding": {"page_count": 0},
            "config_projection": {"ok": True},
            "signature_current": False,
            "implementation_current": False,
        }
        with mock.patch.object(watcher, "snapshot", return_value=snapshot):
            result = watcher.validate()
        self.assertTrue(result["ok"], result)

    def test_provider_scoped_repair_cannot_reconcile_desktop_environment(self) -> None:
        with mock.patch.object(watcher.codex_state_repair, "repair", return_value={"ok": True}) as repair:
            result = watcher.provider_scoped_startup_baseline_repair(dry_run=False)
        self.assertTrue(result["ok"])
        repair.assert_called_once_with(
            dry_run=False, runtime_validation=False, reconcile_desktop_environment=False,
        )

    def test_reconcile_defers_projection_during_mirror_capture_lease(self) -> None:
        source = {"ok": True, "catalog_path": "", "catalog_models": []}
        lease = {"active": True, "path": "C:/mirror/runtime/capture-lease.json"}
        with mock.patch.object(watcher, "source_state", return_value=source), \
                mock.patch.object(watcher.codex_config_projection, "capture_lease_state", return_value=lease), \
                mock.patch.object(watcher.codex_config_projection, "apply_projection") as apply:
            result = watcher.reconcile(dry_run=False)
        self.assertTrue(result["ok"])
        self.assertTrue(result["deferred"])
        self.assertEqual(result["reason"], "mirror_capture_lease_active")
        apply.assert_not_called()
    def test_page_reload_cli_reuses_runtime_owner(self) -> None:
        result = {"ok": True, "requested": True, "skipped": False, "cdp_port": 9231}
        with (
            mock.patch("sys.argv", ["codex_desktop_model_runtime.py", "page-reload", "--wait-seconds", "3"]),
            mock.patch.object(runtime, "request_desktop_page_reload", return_value=result) as reload_page,
        ):
            exit_code = runtime.main()

        self.assertEqual(exit_code, 0)
        reload_page.assert_called_once_with(wait_seconds=3.0)
    ...
    def test_model_signature_tracks_cc_switch_provider_identity(self) -> None:
        config = {
            "model": "provider-a",
            "model_provider": "custom",
            "model_providers": {
        ...
        first = config_guard.model_signature(
            config,
            None,
            provider_authority={"provider_id": "provider-one", "provider_name": "One"},
        )
        second = config_guard.model_signature(
            config,
            None,
            provider_authority={"provider_id": "provider-two", "provider_name": "Two"},
        )
        self.assertNotEqual(config_guard.model_signature_hash(first), config_guard.model_signature_hash(second))

    def test_provider_without_catalog_rejects_stale_catalog_pointer(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            config_path = Path(temp_dir) / "config.toml"
            config = {"model_catalog_json": "stale-catalog.json"}
            state = config_guard.model_catalog_provenance_state(
                config,
                config_path,
                {
                    "found": True,
                    "provider_id": "provider-without-catalog",
                    "model_catalog_active": False,
                },
            )
        self.assertFalse(state["ok"])
        self.assertEqual(state["reason"], "stale_cross_provider_catalog_pointer")

    def test_unreadable_provider_authority_is_not_treated_as_a_safe_skip(self) -> None:
        state = config_guard.model_catalog_provenance_state(
            {},
            Path("config.toml"),
            {
        ...
        self.assertEqual(state["reason"], "cc_switch_provider_authority_unreadable")

    def test_provider_catalog_requires_projected_catalog_file(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            config_path = Path(temp_dir) / "config.toml"
            config = {"model_catalog_json": "provider-catalog.json"}
            authority = {
                "found": True,
                "provider_id": "provider-with-catalog",
                "model_catalog_active": True,
                "model_catalog_sha256": "provider-hash",
            }
            missing = config_guard.model_catalog_provenance_state(config, config_path, authority)
            (config_path.parent / "provider-catalog.json").write_text('{"models": []}', encoding="utf-8")
            ready = config_guard.model_catalog_provenance_state(config, config_path, authority)
        self.assertFalse(missing["ok"])
        self.assertEqual(missing["reason"], "provider_catalog_projection_missing")
        self.assertTrue(ready["ok"])
        self.assertEqual(ready["reason"], "provider_catalog_projection_ready")
    ...
    def test_statsig_protection_uses_mutable_provider_state(self) -> None:
        ...
        self.assertIn("codex-statsig-allowlist-protection/v5", source)
        self.assertIn("window.__codexStatsigAllowlistProtectionRequiredModels = requiredModels", source)
        self.assertIn("const merged = activeRequiredModels()", source)
        self.assertIn("const removed = before.filter((item) => !merged.includes(item))", source)
    ...
    def test_current_desktop_uses_native_runtime_or_legacy_appserver_module(self) -> None:
        native = runtime.native_desktop_model_runtime_state()
        discovery = appserver_bridge.discover_host_module()
        self.assertTrue(native["native_authoritative"] or discovery["ok"], {"native": native, "legacy": discovery})

    def test_current_desktop_uses_native_runtime_or_legacy_persisted_state_module(self) -> None:
        native = runtime.native_desktop_model_runtime_state()
        discovery = runtime.discover_persisted_state_host_module()
        self.assertTrue(native["native_authoritative"] or discovery["ok"], {"native": native, "legacy": discovery})

    def test_statsig_sync_exactly_matches_current_provider_models(self) -> None:
        source = runtime._statsig_allowlist_sync_expression(["provider-b"], apply=True, reload_if_changed=True)
        self.assertIn("const synced = requiredModels.slice()", source)
        self.assertIn("const removed = before.filter((item) => !synced.includes(item))", source)

    def test_native_allowlist_fallback_runs_when_live_model_handler_is_unavailable(self) -> None:
        native = {"ok": True, "native_authoritative": True}
        sync_ready = {"ok": True, "skipped": False, "result": {"after": ["provider-a"]}}
        with (
            mock.patch.object(runtime, "native_desktop_model_runtime_state", return_value=native),
            mock.patch.object(runtime, "desktop_runtime_state", side_effect=[
                {"list_models_for_host_ok": False, "list_models_for_host_exact": False},
                {"list_models_for_host_ok": True, "list_models_for_host_exact": True},
            ]),
            mock.patch.object(runtime, "statsig_allowlist_sync_state", return_value=sync_ready) as sync,
        ):
            state = runtime.statsig_allowlist_protection_state(
                None,
                fallback_model_ids=["provider-a"],
                apply=False,
            )
    ...
    def test_catalog_reasoning_repair_detects_and_restores_missing_ultra(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with mock.patch.object(runtime, "native_desktop_model_runtime_state", return_value={"native_authoritative": False}):
    ...
    def test_provider_watcher_has_bounded_runtime_binding_reconciliation(self) -> None:
        ...
        self.assertIn("default=DEFAULT_DRIFT_CHECK_SECONDS", source)
        self.assertEqual(watcher.DEFAULT_DRIFT_CHECK_SECONDS, 10.0)
        ...
        self.assertIn("reconcile(reload_if_changed=True, source_changed=True)", source)
        self.assertIn("A renderer-local repair must stay in that renderer", source)
        self.assertIn("repair_startup_baseline=False", source)
        self.assertIn("RUNTIME_RECONCILE_COOLDOWN_SECONDS", source)
        self.assertIn("binding_key != runtime_reconcile_key or now >= next_runtime_reconcile_at", source)
        self.assertIn("REQUIRED_STABLE_SOURCE_POLLS", source)
        self.assertIn("RESTART_FOR_IMPLEMENTATION_CHANGE", source)
        self.assertIn('"event": "implementation_changed"', source)
        self.assertIn('"watcher_restart_required": True', source)
        self.assertIn('"watcher_implementation_current"', source)

    def test_provider_watcher_clears_legacy_implementation_restart_flag_after_reload(self) -> None:
        source = {
            "ok": True,
            "native_desktop_runtime": {"native_authoritative": False},
        ...
        previous_state = {
            ...
            "next_implementation_fingerprint": "implementation-a",
        ...
        with (
            ...
            mock.patch.object(watcher, "_read_state", return_value=previous_state),
            mock.patch.object(watcher, "_write_json") as write_state,
        ...
        startup_record = write_state.call_args_list[0].args[1]
        self.assertFalse(startup_record["restart_required"])
        self.assertNotIn("restart_reason", startup_record)
        self.assertNotIn("next_implementation_fingerprint", startup_record)
    ...
    def test_provider_watcher_cools_down_repeated_unbound_runtime_reconcile(self) -> None:
        source = {
            "ok": True,
            "native_desktop_runtime": {"native_authoritative": False},
            "signature_hash": "signature-a",
        ...
        with (
            ...
            mock.patch.object(watcher, "_source_probe_fingerprint", return_value="source-metadata-a"),
            mock.patch.object(watcher, "source_state", return_value=source),
            mock.patch.object(watcher, "runtime_binding_state", return_value={
                "bound": False,
                "reason": "appserver_bridge_unavailable",
            }),
            mock.patch.object(watcher, "reconcile", return_value={"ok": True}) as reconcile,
            mock.patch.object(watcher.time, "monotonic", side_effect=[0.0, 1.0, 2.0, 3.0, 4.0]),
            mock.patch.object(watcher.time, "sleep"),
        ):
            result = watcher.watch(
                poll_seconds=2.0,
                debounce_seconds=1.5,
                drift_check_seconds=1.0,
                max_iterations=4,
            )
    ...
    def test_provider_watcher_compacts_event_log_without_archive_growth(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            log_path = Path(temp_dir) / "events.jsonl"
            old_rows = [
                json.dumps({"event": "old", "index": index}) + "\n"
                for index in range(40)
            ]
            log_path.write_text("".join(old_rows), encoding="utf-8")
            with (
                mock.patch.object(watcher, "LOG_PATH", log_path),
                mock.patch.object(watcher, "MAX_EVENT_LOG_BYTES", 200),
                mock.patch.object(watcher, "EVENT_LOG_TAIL_BYTES", 120),
            ):
                watcher._append_event({"event": "current", "index": 40})

            rows = [json.loads(line) for line in log_path.read_text(encoding="utf-8").splitlines()]
            self.assertEqual(rows[-1], {"event": "current", "index": 40})
            self.assertTrue(all(row["index"] > 0 for row in rows))
            self.assertLess(log_path.stat().st_size, 200)
            self.assertFalse((log_path.parent / "events.previous.jsonl").exists())

    def test_provider_watcher_reads_existing_state_before_reporting_log_health(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            state_path = Path(temp_dir) / "state.json"
            state_path.write_text('{"last_successful_signature_hash":"signature-a"}', encoding="utf-8")
            with mock.patch.object(watcher, "STATE_PATH", state_path):
                state = watcher._read_state()

        self.assertEqual(state["last_successful_signature_hash"], "signature-a")

    def test_provider_watcher_retries_failed_runtime_reconcile_on_retry_interval(self) -> None:
        source = {
            "ok": True,
            "native_desktop_runtime": {"native_authoritative": False},
            "signature_hash": "signature-a",
        ...
        with (
            ...
            mock.patch.object(watcher, "reconcile", return_value={"ok": False}) as reconcile,
            mock.patch.object(watcher.time, "monotonic", side_effect=[0.0, 1.0, 2.0, 16.0, 17.0]),
            mock.patch.object(watcher.time, "sleep"),
        ):
            result = watcher.watch(
                poll_seconds=2.0,
    ...
    def test_provider_without_catalog_preserves_native_desktop_behavior(self) -> None:
        source = {
            ...
            "catalog_provenance": {
                "ok": True,
                "skipped": False,
                "reason": "native_provider_without_catalog",
            },
        }
        ready = {"ok": True, "skipped": False}
        with (
            mock.patch.object(watcher, "source_state", return_value=source),
            mock.patch.object(watcher.codex_config_projection, "apply_projection", return_value=ready),
            mock.patch.object(watcher.codex_state_repair, "repair", return_value=ready),
    ...
    def test_provider_bridge_continues_when_startup_baseline_is_noncritical_failure(self) -> None:
        source = {
            "ok": True,
            "native_desktop_runtime": {"native_authoritative": False},
            "catalog_path": "",
            "catalog_models": [],
            "provider_model_ids": ["gpt-5.6-terra", "gpt-5.6-sol"],
            "require_advanced_model_picker": False,
            "catalog_provenance": {
                "ok": True,
                "skipped": False,
        ...
        projection_ready = {"ok": True, "skipped": False}
        startup_failure = {
            "ok": False,
            "reason": "session_projection_failed",
            "error": "NOT NULL constraint failed: threads.first_user_message",
        }
        bridge_ready = {"ok": True, "applied": True, "skipped": False, "model_count": 2}
        with (
            mock.patch.object(watcher, "source_state", return_value=source),
            mock.patch.object(watcher.codex_config_projection, "apply_projection", return_value=projection_ready),
            mock.patch.object(watcher.codex_state_repair, "repair", return_value=startup_failure),
            mock.patch.object(runtime, "desktop_runtime_state", side_effect=[
                {"list_models_for_host_ok": False, "list_models_for_host_exact": False},
                {"list_models_for_host_ok": True, "list_models_for_host_exact": True},
            ]),
            mock.patch.object(runtime, "apply_model_list_bridge_shim", return_value=bridge_ready) as apply_bridge,
            mock.patch.object(runtime, "statsig_allowlist_protection_state", return_value={"ok": True, "skipped": False}),
        ):
            result = watcher.reconcile(reload_if_changed=False)

        self.assertTrue(result["ok"], result)
        self.assertTrue(result["bridge_runtime_readback"]["list_models_for_host_exact"])
        self.assertEqual(result["startup_baseline"]["error"], startup_failure["error"])
        apply_bridge.assert_called_once_with(
            None,
            fallback_model_ids=["gpt-5.6-terra", "gpt-5.6-sol"],
            reload_page=False,
            wait_seconds=2.0,
        )

    def test_provider_bridge_rejects_install_without_exact_live_model_readback(self) -> None:
        source = {
            "ok": True,
            "native_desktop_runtime": {"native_authoritative": False},
            "catalog_path": "",
            "catalog_models": [],
            "provider_model_ids": ["provider-a", "provider-b"],
            "require_advanced_model_picker": False,
            "catalog_provenance": {"ok": True, "skipped": False},
        }
        ready = {"ok": True, "skipped": False}
        with (
            mock.patch.object(watcher, "source_state", return_value=source),
            ...
            mock.patch.object(runtime, "desktop_runtime_state", side_effect=[
                {"list_models_for_host_ok": False, "list_models_for_host_exact": False},
                {"list_models_for_host_ok": True, "list_models_for_host_exact": False, "list_models_for_host_models": ["provider-a"]},
                {"list_models_for_host_ok": True, "list_models_for_host_exact": False, "list_models_for_host_models": ["provider-a"]},
            ]),
            mock.patch.object(runtime, "apply_model_list_bridge_shim", return_value={"ok": True, "applied": True}),
        ):
            result = watcher.reconcile(reload_if_changed=False)
        self.assertFalse(result["ok"], result)
        self.assertEqual(result["reason"], "desktop_model_list_bridge_readback_failed")

    def test_runtime_only_reconcile_skips_full_startup_baseline_repair(self) -> None:
        source = {
            "ok": True,
            "catalog_path": "",
            "catalog_models": [],
        ...
        with (
            mock.patch.object(watcher, "source_state", return_value=source),
            mock.patch.object(watcher.codex_config_projection, "apply_projection", return_value=ready),
            mock.patch.object(watcher.codex_state_repair, "repair") as startup_repair,
        ):
            state = watcher.reconcile(
                reload_if_changed=True,
                repair_startup_baseline=False,
            )

        self.assertTrue(state["ok"], state)
        self.assertEqual(state["startup_baseline"]["reason"], "runtime_binding_only")
        startup_repair.assert_not_called()

    def test_provider_change_without_catalog_requests_one_page_reload(self) -> None:
        source = {
            "ok": True,
            "native_desktop_runtime": {"native_authoritative": False},
            "catalog_path": "",
        ...
        ready = {"ok": True, "skipped": False}
        reload_result = {"ok": True, "requested": True}
        with (
            mock.patch.object(watcher, "source_state", return_value=source),
            mock.patch.object(watcher.codex_config_projection, "apply_projection", return_value=ready),
            mock.patch.object(watcher.codex_state_repair, "repair", return_value=ready),
            mock.patch.object(runtime, "request_desktop_page_reload", return_value=reload_result) as reload_page,
        ):
            state = watcher.reconcile(reload_if_changed=True, source_changed=True)
        self.assertTrue(state["ok"], state)
        self.assertEqual(state["reason"], "native_provider_without_catalog_no_runtime_override_required")
    ...
    def test_host_persistence_failure_does_not_authorize_reload(self) -> None:
        source = {
            ...
            "catalog_provenance": {
                "ok": True,
                "skipped": False,
                "reason": "provider_catalog_projection_ready",
            },
        }
        ready = {"ok": True, "skipped": False}
        picker_failure = {
    ...
    def test_runtime_binding_requires_the_full_consumed_response_without_a_next_page(self) -> None:
        ...
        class FakeCdpClient:
            ...
            def evaluate(self, expression: str) -> dict[str, object]:
                ...
                if "statsig-live-probe/v1" in expression:
                    return {
                        "ok": True,
                        "version": "codex-statsig-allowlist-protection/v5",
        ...
        def binding_state(consumed_models: list[str], next_cursor: str | None) -> dict[str, object]:
            renderer_state: dict[str, object] = {
                ...
                "statsigVersion": "codex-statsig-allowlist-protection/v5",
            ...
            with (
                mock.patch.object(runtime, "native_desktop_model_runtime_state", return_value={"native_authoritative": False}),
