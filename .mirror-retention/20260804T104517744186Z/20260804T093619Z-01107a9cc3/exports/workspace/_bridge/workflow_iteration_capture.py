...
from workflow_review_presentation import project_required_checks, project_summary, title_for_signal
...
def parse_checkpoint(checkpoint: dict[str, Any] | str | Path | None) -> dict[str, Any]:
    ...
    logical_ref = _clean_text(source.get("logical_ref") or source.get("path"))
    ...
    return {
        ...
        "workspace_relative_path": _clean_text(source.get("workspace_relative_path")),
        "workspace_path": workspace_path,
        "source_ref": workspace_path or logical_ref,
...
def stable_candidate_id(
    *,
    ...
    """Return the semantic candidate identity independent of evidence location."""
...
def legacy_candidate_id(
    *,
    ...
    """Return the v1 checkpoint-coupled identity for existing queue rows."""
...
def _candidate(
    *,
    ...
    _projected_summary, presentation_complete = project_summary(text, kind="iteration_candidates")
    if not presentation_complete:
        return None, {
            ...
            "required_next_action": "producer supplies a Chinese-primary semantic summary before queue admission",
        }
    ...
    return {
        ...
        "title": title_for_signal(signal_kind),
        "summary": text,
        ...
        "required_checks": project_required_checks([
        ...
        "attributes": {
            ...
            "privacy_severity": severity,
            "identity_schema": "workflow_iteration_candidate.semantic.v2",
            "evidence_sources": [source_checkpoint],
            "identity": identity,
...
def capture_iteration_candidates(
    *,
    ...
    source_checkpoint = parsed["checkpoint_id"] or parsed["logical_ref"] or "closeout-structured-facts"
    ...
    for signal in signals[:100]:
        ...
        item, rejected = _candidate(
            ...
            checkpoint_path=parsed["source_ref"],
        )
        if rejected:
    ...
    return {
        ...
        "evidence_only_signal_count": len(_clean_list(regression_tests)),
...
def verify_candidate_identity(item: dict[str, Any]) -> bool:
    ...
    expected = stable_candidate_id(**identity_args)
    legacy = legacy_candidate_id(**identity_args)
    return bool(candidate_id) and candidate_id in {expected, legacy}
