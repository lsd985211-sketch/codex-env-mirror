...
def owner_result_cache_allowed(tool: str, request: dict[str, Any]) -> bool:
    ...
    if bool(request.get("need_materialization")):
        return False
    metadata = request.get("metadata") if isinstance(request.get("metadata"), dict) else {}
    action = str(metadata.get("package_action") or "").strip().lower()
    mode = str(metadata.get("package_execution_mode") or "").strip().lower()
    if action in {"install", "add"} or mode == "verified_auto_install":
        return False
    return True
