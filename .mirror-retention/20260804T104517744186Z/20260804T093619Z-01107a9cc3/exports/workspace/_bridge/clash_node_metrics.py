...
def read_cache() -> dict[str, Any]:
    expected_signature = current_profile_signature()
    ...
    try:
        ...
        if isinstance(data, dict):
            ...
            if str(data.get("profile_signature") or "") != expected_signature:
                return {
                    "schema": f"{SCHEMA_PREFIX}.cache.v1",
                    "created_at": now_iso(),
                    "updated_at": "",
                    "groups": {},
                    "profile_signature": expected_signature,
                    "invalidated_reason": "profile_config_or_group_changed",
                }
            return data
    except Exception:
        pass
...
def write_cache(cache: dict[str, Any]) -> None:
    ...
    cache["profile_signature"] = current_profile_signature()
...
def effective_group(group: str = "") -> str:
    """Use an explicit group or derive the current Verge gateway group."""

    if str(group or "").strip():
        return str(group).strip()
    return str(clash.verge_profile_snapshot().get("recommended_group") or "").strip()


def current_profile_signature(group: str = "") -> str:
    profile = clash.verge_profile_snapshot()
    payload = {
        "current_uid": str(profile.get("current_uid") or ""),
        "config_digest": str(profile.get("config_digest") or ""),
        "group": effective_group(group),
    }
...
def snapshot() -> dict[str, Any]:
    ...
    return {
        ...
        "current_profile": clash.verge_profile_snapshot(),
    }
...
def validate() -> dict[str, Any]:
    ...
    return {
        ...
        "ok": bool(snap.get("ok")) and mapping_ok and bool(effective_group()),
        "generated_at": now_iso(),
        "cache_path": str(METRICS_PATH),
        "cache_exists": METRICS_PATH.exists(),
