...
# Only facts that imply execution, state, permission, or closeout work can
# disqualify an otherwise self-contained simple request.  Soft classifier
# hints such as ``external_knowledge_candidate`` are deliberately excluded:
# they help later routing when routing is needed, but they are not authority to
# activate an owner or gate by themselves.
SUBSTANTIVE_TASK_FACTS = (
    "local_write",
    "config_change",
...
def _tool_guidance(
    ...
    semantic_capabilities = [item for item in _as_list(_as_dict(plan.get("environment_context")).get("semantic_capabilities")) if isinstance(item, dict)]
    if not semantic_capabilities:
        semantic_capabilities = [item for item in _as_list(environment_context.get("semantic_capabilities")) if isinstance(item, dict)]
    if semantic_capabilities and matched_terms(message, SEMANTIC_RETRIEVAL_TERMS):
        capability = semantic_capabilities[0]
        tools.append(
            {
                "kind": "tool",
                "name": "bge-m3-semantic-owner",
                "mode": "supporting_semantic_retrieval",
                "reason": "BGE-M3 is an owner-backed semantic candidate for recall and reranking",
                "action": str(capability.get("entry_point") or "python _bridge/semantic_capability_owner.py status --probe; embed|rerank|hybrid-rerank|select"),
                "use_for": "dense semantic retrieval, concept normalization, hybrid RRF, and ambiguity-aware candidate selection",
                "skip_when": "exact lexical/graph evidence already satisfies the acceptance predicate",
                "fallback": str(capability.get("fallback_route") or "consumer adapter's structured/FTS/graph route"),
                "authority": str(capability.get("source_authority") or "semantic_capability_owner"),
                "business_scope": str(capability.get("business_scope") or "domain owner remains primary"),
                "result_contract": "consume vector_used/vector_reason/retrievers/selection_margin; preserve source_ref/content_sha256 and domain citations",
                "semantic_modes": list(capability.get("business_results", {}).get("semantic_modes", []))[:4],
            }
        )
...
def _semantic_probe_result(message: str, environment_context: dict[str, Any]) -> dict[str, Any]:
    """Probe the existing semantic owner once for semantic routing turns.

    The result is a bounded transport-health fact. It does not expose the
    endpoint or model output and never grants execution authority.
    """

    capabilities = [
        item for item in _as_list(environment_context.get("semantic_capabilities"))
        if isinstance(item, dict)
    ]
    if not capabilities and not matched_terms(str(message or "").casefold(), SEMANTIC_RETRIEVAL_TERMS):
        return {}
    try:
        result = semantic_capability_owner.status(probe=True)
    except Exception as exc:  # a probe failure must preserve lexical fallback
        return {
            "callable": False,
            "healthy": False,
            "vector_reason": f"probe_failed:{type(exc).__name__}",
            "fallback": "structured_fts_graph",
        }
    return {
        "callable": result.get("callable"),
        "healthy": result.get("healthy"),
        "vector_reason": str(result.get("vector_reason") or "semantic_probe_unknown"),
        "fallback": str(result.get("fallback") or "structured_fts_graph"),
        "capability_id": str(result.get("capability_id") or "model:bge-m3"),
        "model": str(result.get("model") or "bge-m3"),
    }
...
def build_asset_guidance(
    ...
    *,
    ...
    ambiguity = _as_dict(route_decision.get("ambiguity"))
    message = str(plan.get("message") or "").casefold()
    semantic_requested = bool(_as_list(environment_context.get("semantic_capabilities"))) or bool(
        matched_terms(message, SEMANTIC_RETRIEVAL_TERMS)
    )
    self_contained_simple = bool(matched_terms(message, SELF_CONTAINED_SIMPLE_TERMS))
    simple = (
        domains == ["general"]
        and not any(bool(task_facts.get(key)) for key in SUBSTANTIVE_TASK_FACTS)
        and not semantic_requested
        and (
            self_contained_simple
            or (not tool_policies and not skills and not ambiguity.get("is_ambiguous"))
        )
    )
    if simple:
        return {
            ...
            "capability_bundle": {"schema": "business_environment.capability_bundle.v1", "simple_fast_path": True, "assets": []},
            "sequence": [],
            "rules": [],
            "skills": [],
            "owners": [],
    ...
    tools = _tool_guidance(plan, route_decision, tool_policies, resource_gate, environment_context)
    semantic_probe_result = _semantic_probe_result(str(plan.get("message") or ""), environment_context)
    # Import lazily: the resolver's standalone collection path imports the
    # workflow router, while this admission path only needs its pure composer.
    from business_environment_capability_resolver import build_capability_bundle

    initial_bundle = build_capability_bundle(
        str(plan.get("message") or ""),
        workflow_plan=plan,
        ...
        owners=owners,
        tools=tools,
        environment_decision_frame=_as_dict(environment_context.get("environment_decision_frame")),
        semantic_probe_result=semantic_probe_result,
    )
    from workflow_asset_domain_projection import build_domain_projection

    domain_projection = build_domain_projection(
        str(plan.get("message") or ""),
        primary_owner=str(_as_dict(initial_bundle.get("primary_owner")).get("name") or ""),
        skills=skills,
        owners=owners,
        tools=tools,
        task_facts=task_facts,
    )
    tools = [item for item in _as_list(domain_projection.get("filtered_tools")) if isinstance(item, dict)]
    from environment_capability_view import build_capability_view

    capability_view = build_capability_view(
        str(plan.get("message") or ""),
        skills=skills,
        owners=owners,
        tools=tools,
    ...
    primary_owner = dict(_as_dict(capability_bundle.get("primary_owner")))
    primary_owner["capability_view"] = {
        "schema": str(capability_view.get("schema") or ""),
        "source_signature": str(capability_view.get("source_signature") or ""),
        "decision_mode": str(capability_view.get("decision_mode") or "codex_select"),
        "candidate_refs": [
            "{}|class={}|owner={}|state={}|admitted={}|callable={}|healthy={}".format(
                str(item.get("capability_id") or ""),
                str(item.get("capability_class") or ""),
            ...
            for item in _as_list(capability_view.get("candidates"))[:6]
            if isinstance(item, dict)
        ],
        "issues": list(_as_list(capability_view.get("issues")))[:3],
        "next_action": str(capability_view.get("next_action") or ""),
    }
    sequence = [kind for kind, values in (("rules", rules), ("skills", skills), ("owners", owners), ("tools", tools)) if values]
    frame = _as_dict(environment_context.get("environment_decision_frame"))
    frame_active = str(frame.get("decision_mode") or "") in {
        "hard_gate", "fast_recommendation", "judgment_required", "insufficient_discovery"
    }
    return {
        "schema": "workflow_asset_guidance.v1",
        "active": bool(sequence) or frame_active,
        ...
        "capability_view": capability_view,
        "capability_view_decision_mode": str(capability_view.get("decision_mode") or "codex_select"),
        "environment_decision_frame": frame,
        "decision_mode": _as_dict(capability_bundle.get("environment_decision_frame")).get("decision_mode") or "fast_recommendation",
        "domain_projection": domain_projection,
        "semantic_probe": semantic_probe_result,
        "sequence": sequence,
        "rules": rules,
        "skills": skills,
        "owners": owners,
