...
class WslPlatformOwnerValidatorTests(unittest.TestCase):
    ...
    def test_windows_startup_audit_does_not_fight_runtime_marketplace_source(self) -> None:
        spec = {"source_type": "local", "source": r"\\?\C:\Users\45543\.codex\.tmp\bundled"}
        filtered = codex_state_audit.expected_marketplace_fields(
            "openai-bundled",
            spec,
...
""".strip()
    ...
    def test_wsl_hub_scratch_db_is_created_outside_work_git(self) -> None:
        expected = Path.home() / ".codex-app" / "sqlite" / "codex_scratch.sqlite"
        self.assertEqual(expected, local_mcp_hub.SCRATCH_DB_PATH)
        self.assertFalse(local_mcp_hub.BRIDGE_ROOT in local_mcp_hub.SCRATCH_DB_PATH.parents)
        local_mcp_hub.LocalMcpHub()
