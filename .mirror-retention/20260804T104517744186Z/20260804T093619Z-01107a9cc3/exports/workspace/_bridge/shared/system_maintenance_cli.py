...
def run_action(
    ...
    *,
    ...
    def execute(key: str) -> tuple[str, dict[str, Any]]:
        ...
        if action == "apply" and key == "resource_process":
            for field, option in (
                ("grant_ref", "--authorization-grant-ref"),
                ("thread_id", "--authorization-thread-id"),
                ("operation_id", "--authorization-operation-id"),
                ("state_root", "--authorization-state-root"),
            ):
                value = str(authorization.get(field) or "")
                if value:
                    resolved_command.extend([option, value])
        owner_started = time.perf_counter()
        result = run_json(resolved_command)
...
def main(argv: list[str] | None = None) -> int:
    ...
    p_apply.add_argument("--authorization-grant-ref", default="")
    p_apply.add_argument("--authorization-thread-id", default="")
    p_apply.add_argument("--authorization-operation-id", default="")
    p_apply.add_argument("--authorization-state-root", default="")
    ...
    if args.command == "list":
        ...
        payload = run_action(
            ...
            False,
            authorization={
                "grant_ref": args.authorization_grant_ref,
                "thread_id": args.authorization_thread_id,
                "operation_id": args.authorization_operation_id,
