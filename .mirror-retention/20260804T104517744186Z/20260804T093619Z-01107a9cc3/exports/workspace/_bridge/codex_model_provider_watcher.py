#!/usr/bin/env python3
"""Watch Codex provider/catalog changes and reconcile Desktop model runtime.

Ownership: provider-change detection and runtime reconciliation for the active
Codex Desktop model picker.
Non-goals: rewrite CC Switch catalogs, patch app.asar, choose a provider, or
restart/kill Codex. Config recovery is delegated to its owning projection and
startup-baseline modules.
Caller context: hidden scheduled task, elevated launcher fallback, diagnostics,
and regression tests.
"""

from __future__ import annotations

import argparse
import hashlib
import json
import os
import subprocess
import sys
import time
from pathlib import Path
from typing import Any

from bounded_output import governed_cli_payload

try:
    import codex_appserver_model_bridge
    import codex_config_projection
    import codex_config_guard
    import codex_desktop_model_runtime
    import codex_state_repair
except ModuleNotFoundError:  # Package-style imports from the workspace root.
    from _bridge import codex_appserver_model_bridge, codex_config_projection, codex_config_guard, codex_desktop_model_runtime, codex_state_repair


ROOT = Path(__file__).resolve().parents[1]
RUNTIME_DIR = ROOT / "_bridge" / "runtime" / "codex_model_provider_watcher"
STATE_PATH = RUNTIME_DIR / "state.json"
LOG_PATH = RUNTIME_DIR / "events.jsonl"
LOCK_PATH = RUNTIME_DIR / "watcher.lock"
TASK_NAME = "CodexModelProviderWatcher"
RESTART_FOR_IMPLEMENTATION_CHANGE = 75
DEFAULT_SUPERVISOR_MAX_RESTARTS = 5
DEFAULT_SUPERVISOR_RESTART_WINDOW_SECONDS = 30.0
SOURCE_RETRY_SECONDS = 5.0
RECONCILE_RETRY_SECONDS = 15.0
DEFAULT_DRIFT_CHECK_SECONDS = 10.0
REQUIRED_STABLE_SOURCE_POLLS = 2
RUNTIME_RECONCILE_COOLDOWN_SECONDS = 300.0
MAX_EVENT_LOG_BYTES = 2 * 1024 * 1024
EVENT_LOG_TAIL_BYTES = 1024 * 1024


def _implementation_paths() -> tuple[Path, ...]:
    paths = [
        Path(__file__).resolve(),
        Path(codex_appserver_model_bridge.__file__).resolve(),
        Path(codex_config_projection.__file__).resolve(),
        Path(codex_config_guard.__file__).resolve(),
        Path(codex_desktop_model_runtime.__file__).resolve(),
        Path(codex_state_repair.__file__).resolve(),
    ]
    return tuple(dict.fromkeys(paths))


def _implementation_fingerprint() -> str:
    digest = hashlib.sha256()
    for path in _implementation_paths():
        digest.update(str(path).encode("utf-8"))
        try:
            digest.update(path.read_bytes())
        except OSError as exc:
            digest.update(repr(exc).encode("utf-8"))
    return digest.hexdigest()


def _metadata_fingerprint(paths: tuple[Path, ...]) -> str:
    rows: list[tuple[str, int, int]] = []
    for path in paths:
        try:
            stat = path.stat()
            rows.append((str(path), int(stat.st_size), int(stat.st_mtime_ns)))
        except OSError:
            rows.append((str(path), -1, -1))
    return hashlib.sha256(json.dumps(rows, ensure_ascii=False, separators=(",", ":")).encode("utf-8")).hexdigest()


def _implementation_metadata_fingerprint() -> str:
    return _metadata_fingerprint(_implementation_paths())


def _source_probe_paths(source: dict[str, Any] | None = None) -> tuple[Path, ...]:
    source = source or {}
    paths = [
        codex_config_guard.CODEX_CONFIG,
        codex_config_projection.CC_SWITCH_DB,
        codex_config_projection.managed_projection_path(codex_config_guard.CODEX_CONFIG),
    ]
    catalog_text = str(source.get("catalog_path") or "").strip()
    paths.append(
        Path(catalog_text)
        if catalog_text
        else codex_config_guard.CODEX_CONFIG.parent / "cc-switch-model-catalog.json"
    )
    signature = source.get("signature") if isinstance(source.get("signature"), dict) else {}
    asar_text = str(signature.get("desktop_app_asar") or "").strip()
    if asar_text:
        paths.append(Path(asar_text))
    return tuple(dict.fromkeys(Path(path) for path in paths))


def _source_probe_fingerprint(source: dict[str, Any] | None = None) -> str:
    return _metadata_fingerprint(_source_probe_paths(source))


def _source_event_summary(source: dict[str, Any]) -> dict[str, Any]:
    return {
        "ok": bool(source.get("ok")),
        "config_path": str(source.get("config_path") or ""),
        "catalog_path": str(source.get("catalog_path") or ""),
        "model": str(source.get("model") or ""),
        "model_provider": str(source.get("model_provider") or ""),
        "cc_switch_provider_id": str((source.get("provider_authority") or {}).get("provider_id") or ""),
        "signature_hash": str(source.get("signature_hash") or ""),
        "projection_available": not str(source.get("config_projection_signature") or "").startswith("unavailable:"),
        "catalog_model_count": len(source.get("catalog_models") or []),
        "catalog_reasoning_effort_count": len(source.get("catalog_reasoning_efforts") or []),
        "catalog_provenance_ok": bool((source.get("catalog_provenance") or {}).get("ok")),
        "catalog_provenance_reason": str((source.get("catalog_provenance") or {}).get("reason") or ""),
    }


def _now() -> str:
    from datetime import datetime, timezone

    return datetime.now(timezone.utc).isoformat()


def _write_json(path: Path, payload: dict[str, Any]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    temporary = path.with_suffix(path.suffix + ".tmp")
    temporary.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    os.replace(temporary, path)


def _append_event(payload: dict[str, Any]) -> None:
    LOG_PATH.parent.mkdir(parents=True, exist_ok=True)
    if LOG_PATH.is_file() and LOG_PATH.stat().st_size > MAX_EVENT_LOG_BYTES:
        with LOG_PATH.open("rb") as handle:
            handle.seek(max(0, LOG_PATH.stat().st_size - EVENT_LOG_TAIL_BYTES))
            tail = handle.read()
        if len(tail) == EVENT_LOG_TAIL_BYTES:
            separator = tail.find(b"\n")
            tail = tail[separator + 1 :] if separator >= 0 else b""
        temporary = LOG_PATH.with_suffix(LOG_PATH.suffix + ".tmp")
        temporary.write_bytes(tail)
        os.replace(temporary, LOG_PATH)
    with LOG_PATH.open("a", encoding="utf-8") as handle:
        handle.write(json.dumps(payload, ensure_ascii=False, separators=(",", ":")) + "\n")


def _read_state() -> dict[str, Any]:
    if not STATE_PATH.is_file():
        return {}
    try:
        value = json.loads(STATE_PATH.read_text(encoding="utf-8"))
        return value if isinstance(value, dict) else {}
    except (OSError, json.JSONDecodeError):
        return {}


def _event_log_state() -> dict[str, Any]:
    size = LOG_PATH.stat().st_size if LOG_PATH.is_file() else 0
    return {
        "path": str(LOG_PATH),
        "size_bytes": size,
        "max_bytes": MAX_EVENT_LOG_BYTES,
        "bounded": size <= MAX_EVENT_LOG_BYTES,
        "retention": "single_file_tail_compaction",
    }


def source_state() -> dict[str, Any]:
    config_path = codex_config_guard.CODEX_CONFIG
    config = codex_config_guard.safe_toml(config_path)
    catalog_path = codex_config_guard.model_catalog_path(config, config_path)
    provider_authority = codex_config_projection.provider_authority_state()
    signature = codex_config_guard.model_signature(
        config,
        catalog_path,
        provider_authority=provider_authority,
    )
    catalog_provenance = codex_config_guard.model_catalog_provenance_state(
        config,
        config_path,
        provider_authority,
    )
    host_module = codex_appserver_model_bridge.discover_host_module()
    persisted_state_module = codex_desktop_model_runtime.discover_persisted_state_host_module()
    native_desktop_runtime = codex_desktop_model_runtime.native_desktop_model_runtime_state()
    signature["desktop_appserver_module"] = str(host_module.get("module_specifier") or "")
    signature["desktop_persisted_state_module"] = str(persisted_state_module.get("module_specifier") or "")
    signature["desktop_app_asar"] = str(host_module.get("asar_path") or "")
    catalog_models = codex_config_guard.read_catalog_slugs(catalog_path)
    provider_models = codex_config_guard.provider_model_list_state(config)
    return {
        "schema": "codex-model-provider-watcher/source-state/v1",
        "ok": bool(config),
        "config_path": str(config_path),
        "catalog_path": str(catalog_path) if catalog_path else "",
        "model": str(config.get("model") or ""),
        "model_provider": str(config.get("model_provider") or ""),
        "provider_authority": provider_authority,
        "catalog_provenance": catalog_provenance,
        "signature": signature,
        "signature_hash": codex_config_guard.model_signature_hash(signature),
        "config_projection_signature": codex_config_projection.projection_signature(),
        "catalog_models": catalog_models,
        "provider_model_ids": list(provider_models.get("model_ids") or []),
        "provider_model_discovery": provider_models,
        "catalog_reasoning_efforts": codex_desktop_model_runtime.catalog_requested_reasoning_efforts(catalog_path),
        "model_picker_policy": "full_catalog" if catalog_models else "native_default",
        "require_advanced_model_picker": bool(catalog_models),
        "desktop_host_module": host_module,
        "desktop_persisted_state_module": persisted_state_module,
        "native_desktop_runtime": native_desktop_runtime,
    }


def provider_scoped_startup_baseline_repair(*, dry_run: bool) -> dict[str, Any]:
    """Repair provider runtime prerequisites without changing Desktop host mode."""
    return codex_state_repair.repair(
        dry_run=dry_run,
        runtime_validation=False,
        reconcile_desktop_environment=False,
    )


def reconcile(
    *,
    reload_if_changed: bool = True,
    dry_run: bool = False,
    source_changed: bool = False,
    repair_startup_baseline: bool = True,
) -> dict[str, Any]:
    source = source_state()
    catalog_text = str(source.get("catalog_path") or "")
    catalog_path = Path(catalog_text) if catalog_text else None
    result: dict[str, Any] = {
        "schema": "codex-model-provider-watcher/reconcile/v1",
        "ok": False,
        "generated_at": _now(),
        "dry_run": dry_run,
        "source": source,
        "config_projection": {},
        "startup_baseline": {},
        "catalog_reasoning": {},
        "reasoning_hot_refresh": {},
        "bridge_shim": {},
        "statsig_protection": {},
        "model_picker_view_sync": {},
        "page_reload": {},
        "restart_required": False,
        "reason": "",
    }
    if not source.get("ok"):
        return {**result, "reason": "config_unreadable"}
    capture_lease = codex_config_projection.capture_lease_state()
    if not dry_run and capture_lease.get("active"):
        return {**result, "ok": True, "deferred": True, "capture_lease": capture_lease, "reason": "mirror_capture_lease_active"}
    config_projection = (
        codex_config_projection.snapshot()
        if dry_run
        else codex_config_projection.apply_projection(additions_only=True, sync_desktop=True)
    )
    result["config_projection"] = config_projection
    if not config_projection.get("ok"):
        return {**result, "reason": "config_projection_failed"}
    source = source_state()
    result["source"] = source
    catalog_text = str(source.get("catalog_path") or "")
    catalog_path = Path(catalog_text) if catalog_text else None
    provenance = source.get("catalog_provenance") if isinstance(source.get("catalog_provenance"), dict) else {}
    if not provenance.get("ok") and not provenance.get("skipped"):
        return {**result, "reason": "model_catalog_provenance_mismatch"}
    startup_baseline = (
        provider_scoped_startup_baseline_repair(dry_run=dry_run)
        if repair_startup_baseline
        else {"ok": True, "skipped": True, "reason": "runtime_binding_only"}
    )
    result["startup_baseline"] = startup_baseline
    # Session projection is a separate startup concern. Preserve its failure
    # receipt, but keep provider discovery and the renderer bridge actionable.
    # The model path below has its own readiness checks and remains authoritative
    # for this watcher result.
    native_runtime = (
        source.get("native_desktop_runtime")
        if isinstance(source.get("native_desktop_runtime"), dict)
        else codex_desktop_model_runtime.native_desktop_model_runtime_state()
    )
    provider_model_ids = [str(item) for item in source.get("provider_model_ids") or [] if str(item)]
    live_runtime = codex_desktop_model_runtime.desktop_runtime_state(provider_model_ids)
    legacy_runtime = codex_desktop_model_runtime.legacy_runtime_shim_state(wait_seconds=2.0)
    if (
        native_runtime.get("native_authoritative")
        and live_runtime.get("list_models_for_host_exact") is True
        and legacy_runtime.get("ok")
        and not legacy_runtime.get("skipped")
        and not legacy_runtime.get("legacy_markers_present")
    ):
        retirement = (
            {"ok": True, "skipped": True, "reason": "dry_run"}
            if dry_run
            else codex_desktop_model_runtime.retire_legacy_runtime_shims(wait_seconds=2.0)
        )
        ok = bool(retirement.get("ok"))
        return {
            **result,
            "ok": ok,
            "native_desktop_runtime": native_runtime,
            "legacy_runtime_shim_state": legacy_runtime,
            "legacy_runtime_retirement": retirement,
            "catalog_reasoning": {"ok": True, "skipped": True, "reason": "native_desktop_runtime_authoritative"},
            "appserver_shim": {"ok": True, "skipped": True, "reason": "native_desktop_runtime_authoritative"},
            "bridge_shim": {"ok": True, "skipped": True, "reason": "native_desktop_runtime_authoritative"},
            "statsig_protection": {"ok": True, "skipped": True, "reason": "native_desktop_runtime_authoritative"},
            "model_picker_view_sync": {"ok": True, "skipped": True, "reason": "native_desktop_runtime_authoritative"},
            "reasoning_hot_refresh": {"ok": True, "skipped": True, "reason": "native_desktop_runtime_authoritative"},
            "page_reload": {"ok": True, "requested": False, "reason": "native_desktop_runtime_authoritative"},
            "restart_required": False,
            "reason": "native_desktop_runtime_authoritative" if ok else "legacy_runtime_retirement_failed",
        }
    if not catalog_path or not source.get("catalog_models"):
        provider_model_ids = [str(item) for item in source.get("provider_model_ids") or [] if str(item)]
        provider_discovery = (
            source.get("provider_model_discovery")
            if isinstance(source.get("provider_model_discovery"), dict)
            else {}
        )
        discovery_required = bool(provider_discovery) and not bool(provider_discovery.get("skipped"))
        discovery_ready = bool(provider_model_ids) or not discovery_required
        # Without a provider catalog or live model enumeration there is no
        # renderer payload to install.  Do not mutate the active Desktop
        # bridge or cache merely to satisfy a health probe.
        if not provider_model_ids:
            return {
                **result,
                "ok": discovery_ready,
                "dynamic_model_count": 0,
                "dynamic_model_discovery_ready": discovery_ready,
                "bridge_shim": {"ok": True, "skipped": True, "reason": "no_provider_models"},
                "statsig_protection": {"ok": True, "skipped": True, "reason": "no_provider_models"},
                "bridge_runtime_readback": {},
                "page_reload": {"ok": True, "requested": False, "reason": "no_provider_models"},
                "restart_required": False,
                "reason": "native_provider_without_catalog_no_runtime_override_required",
            }
        allowlist = codex_desktop_model_runtime.statsig_allowlist_protection_state(
            None,
            fallback_model_ids=provider_model_ids,
            apply=True,
            reload_if_changed=False,
            wait_seconds=2.0,
        )
        page_reload = (
            codex_desktop_model_runtime.request_desktop_page_reload(wait_seconds=2.0)
            if reload_if_changed and source_changed
            else {"ok": True, "requested": False, "reason": "reload_not_required"}
        )
        # A renderer reload removes injected listeners. Bind only after the
        # requested reload has settled so the final target consumes the list.
        bridge = codex_desktop_model_runtime.apply_model_list_bridge_shim(
            None,
            fallback_model_ids=provider_model_ids,
            reload_page=False,
            wait_seconds=2.0,
        )
        bridge_runtime = codex_desktop_model_runtime.desktop_runtime_state(provider_model_ids)
        bridge_readback_ready = bridge_runtime.get("list_models_for_host_exact") is True
        allowlist_ready = bool(allowlist.get("ok")) and (
            not bool(allowlist.get("skipped"))
            or allowlist.get("reason") in {
                "native_desktop_runtime_authoritative",
                "native_desktop_runtime_no_provider_models",
            }
        )
        return {
            **result,
            "ok": discovery_ready and allowlist_ready and (bridge_readback_ready or not provider_model_ids) and bool(page_reload.get("ok")),
            "page_reload": page_reload,
            "bridge_shim": bridge,
            "bridge_runtime_readback": bridge_runtime,
            "statsig_protection": allowlist,
            "dynamic_model_count": len(provider_model_ids),
            "dynamic_model_discovery_ready": discovery_ready,
            "restart_required": not bool(page_reload.get("ok")),
            "reason": (
                "native_provider_without_catalog_refreshed"
                if page_reload.get("requested") and page_reload.get("ok")
                else "native_provider_without_catalog_runtime_binding_current"
                if page_reload.get("ok") and bridge_readback_ready
                else "desktop_model_list_bridge_readback_failed"
                if page_reload.get("ok")
                else "desktop_refresh_failed_restart_required"
            ),
        }
    catalog_reasoning = codex_desktop_model_runtime.catalog_reasoning_repair_plan(catalog_path)
    if dry_run:
        return {**result, "ok": True, "catalog_reasoning": catalog_reasoning, "reason": "dry_run"}

    catalog_reasoning = codex_desktop_model_runtime.apply_catalog_reasoning_repair(catalog_path)
    if not catalog_reasoning.get("ok"):
        return {
            **result,
            "catalog_reasoning": catalog_reasoning,
            "reason": "catalog_reasoning_repair_failed",
        }

    appserver = codex_desktop_model_runtime.apply_appserver_model_shim(
        catalog_path,
        wait_seconds=2.0,
    )
    appserver_ready = bool(appserver.get("ok")) and not bool(appserver.get("skipped"))
    bridge = (
        {
            "schema": "codex-desktop-model-runtime/model-list-bridge-shim/v1",
            "ok": True,
            "applied": False,
            "skipped": True,
            "reason": "appserver_primary_active",
        }
        if appserver_ready
        else codex_desktop_model_runtime.apply_model_list_bridge_shim(
            catalog_path,
            reload_page=False,
            wait_seconds=2.0,
        )
    )
    protection = codex_desktop_model_runtime.statsig_allowlist_protection_state(
        catalog_path,
        apply=True,
        reload_if_changed=False,
        wait_seconds=2.0,
    )
    picker_sync = codex_desktop_model_runtime.model_picker_view_sync_state(
        str(source.get("signature_hash") or ""),
        apply=True,
        require_advanced=bool(source.get("require_advanced_model_picker")),
        wait_seconds=2.0,
    )
    reasoning_hot_refresh = codex_desktop_model_runtime.reasoning_hot_refresh_state(
        catalog_path,
        apply=True,
        reload_if_changed=False,
        wait_seconds=2.0,
    )
    bridge_ready = bool(bridge.get("ok")) and not bool(bridge.get("skipped"))
    protection_ready = bool(protection.get("ok")) and not bool(protection.get("skipped"))
    picker_ready = bool(picker_sync.get("ok")) and not bool(picker_sync.get("skipped"))
    reasoning_ready = bool(reasoning_hot_refresh.get("ok")) and not bool(reasoning_hot_refresh.get("skipped"))
    picker_result = picker_sync.get("result") if isinstance(picker_sync.get("result"), dict) else {}
    reasoning_result = (
        reasoning_hot_refresh.get("result") if isinstance(reasoning_hot_refresh.get("result"), dict) else {}
    )
    reload_required = source_changed or (
        bool(picker_result.get("changed")) and bool(picker_result.get("reloadSafe"))
    ) or (
        bool(reasoning_result.get("settingChanged")) and bool(reasoning_result.get("settingApplied"))
    )
    page_reload = (
        codex_desktop_model_runtime.request_desktop_page_reload(wait_seconds=2.0)
        if reload_if_changed and reload_required
        else {"ok": True, "requested": False, "reason": "reload_not_required"}
    )
    reload_ready = bool(page_reload.get("ok"))
    ok = (appserver_ready or bridge_ready) and protection_ready and picker_ready and reasoning_ready and reload_ready
    return {
        **result,
        "ok": ok,
        "catalog_reasoning": catalog_reasoning,
        "appserver_shim": appserver,
        "bridge_shim": bridge,
        "statsig_protection": protection,
        "model_picker_view_sync": picker_sync,
        "reasoning_hot_refresh": reasoning_hot_refresh,
        "page_reload": page_reload,
        "restart_required": not reload_ready,
        "reason": "" if ok else "desktop_runtime_not_ready_or_reconcile_failed",
    }


def runtime_binding_state(
    expected_models: list[str],
    expected_reasoning_efforts: list[str] | None = None,
    expected_picker_signature: str = "",
    require_advanced_picker: bool = True,
) -> dict[str, Any]:
    expected_reasoning_efforts = list(expected_reasoning_efforts or [])
    native_runtime = codex_desktop_model_runtime.native_desktop_model_runtime_state()
    if native_runtime.get("native_authoritative"):
        live_runtime = codex_desktop_model_runtime.desktop_runtime_state(expected_models)
        legacy = codex_desktop_model_runtime.legacy_runtime_shim_state()
        legacy_present = bool(legacy.get("legacy_markers_present"))
        bound = (
            live_runtime.get("list_models_for_host_exact") is True
            and bool(legacy.get("ok"))
            and not bool(legacy.get("skipped"))
            and not legacy_present
        )
        return {
            "schema": "codex-model-provider-watcher/runtime-binding/v1",
            "ok": bool(legacy.get("ok")),
            "bound": bound,
            "binding_route": "native" if bound else "",
            "expected_models": expected_models,
            "expected_reasoning_efforts": expected_reasoning_efforts,
            "expected_picker_signature": expected_picker_signature,
            "native_desktop_runtime": native_runtime,
            "live_runtime": live_runtime,
            "legacy_runtime_shim_state": legacy,
            "legacy_runtime_shims_present": legacy_present,
            "reason": (
                "native_runtime_clean"
                if bound
                else "native_runtime_model_list_unavailable_or_mismatched"
                if live_runtime.get("list_models_for_host_exact") is not True
                else str(legacy.get("reason") or "legacy_runtime_shims_present")
            ),
        }
    port, ws_url, pages, reason = codex_desktop_model_runtime._find_codex_page()
    result: dict[str, Any] = {
        "schema": "codex-model-provider-watcher/runtime-binding/v1",
        "ok": True,
        "bound": False,
        "cdp_port": port,
        "page_count": len(pages),
        "expected_models": expected_models,
        "expected_reasoning_efforts": expected_reasoning_efforts,
        "expected_picker_signature": expected_picker_signature,
        "reason": reason,
    }
    if not ws_url:
        return {**result, "reason": reason or "codex_page_not_ready"}
    client = None
    picker_probe: dict[str, Any] = {}
    appserver_probe: dict[str, Any] = {}
    statsig_probe: dict[str, Any] = {}
    try:
        client = codex_desktop_model_runtime._CdpClient(ws_url)
        state = client.evaluate(
            """(() => ({
              appServerVersion: String(window.__codexAppServerModelShimVersion || ''),
              appServerModels: (window.__codexAppServerModelShimModels || []).map((item) => item.model || item.slug || ''),
              appServerConsumedModels: window.__codexAppServerModelShimConsumedModels || [],
              appServerGeneration: String(window.__codexAppServerModelShimGeneration || ''),
              appServerConsumedGeneration: String(window.__codexAppServerModelShimConsumedGeneration || ''),
              appServerConsumedNextCursor: window.__codexAppServerModelShimConsumedNextCursor ?? null,
              appServerConsumedAt: Number(window.__codexAppServerModelShimConsumedAt || 0),
              modelListVersion: String(window.__codexModelListShimVersion || ''),
              modelListModels: (window.__codexModelListShimModels || []).map((item) => item.slug || item.model || ''),
              modelListSignature: String(window.__codexModelListShimSignature || ''),
              modelListConsumedSignature: String(window.__codexModelListShimConsumedSignature || ''),
              modelListConsumedModels: window.__codexModelListShimConsumedModels || [],
              modelListConsumedNextCursor: window.__codexModelListShimConsumedNextCursor ?? null,
              modelListConsumedAt: Number(window.__codexModelListShimConsumedAt || 0),
              statsigVersion: String(window.__codexStatsigAllowlistProtectionVersion || ''),
              statsigRequiredModels: window.__codexStatsigAllowlistProtectionRequiredModels || [],
              reasoningVersion: String(window.__codexReasoningCapabilityVersion || ''),
              reasoningEfforts: window.__codexReasoningCapabilityEfforts || [],
              reasoningGates: window.__codexReasoningCapabilityGateOverrides || {}
            }))()"""
        )
        appserver_discovery = codex_appserver_model_bridge.discover_host_module()
        if appserver_discovery.get("ok"):
            appserver_value = client.evaluate(
                codex_appserver_model_bridge.build_probe_source(
                    str(appserver_discovery.get("module_specifier") or "")
                )
            )
            appserver_probe = appserver_value if isinstance(appserver_value, dict) else {}
        statsig_value = client.evaluate(
            codex_desktop_model_runtime._statsig_allowlist_live_probe_expression()
        )
        statsig_probe = statsig_value if isinstance(statsig_value, dict) else {}
        picker_value = client.evaluate(
            codex_desktop_model_runtime._model_picker_view_sync_expression(
                expected_picker_signature or "runtime-binding-probe",
                apply=False,
                require_advanced=require_advanced_picker,
            )
        )
        picker_probe = picker_value if isinstance(picker_value, dict) else {}
        state = state if isinstance(state, dict) else {}
        appserver_models = [str(item) for item in state.get("appServerModels", []) if item]
        appserver_consumed_models = [str(item) for item in state.get("appServerConsumedModels", []) if item]
        model_list_models = [str(item) for item in state.get("modelListModels", []) if item]
        model_list_consumed_models = [str(item) for item in state.get("modelListConsumedModels", []) if item]
        statsig_required_models = [str(item) for item in state.get("statsigRequiredModels", []) if item]
        statsig_available_models = [str(item) for item in statsig_probe.get("availableModels", []) if item]
        reasoning_efforts = [str(item) for item in state.get("reasoningEfforts", []) if item]
        appserver_live_models = [str(item) for item in appserver_probe.get("models", []) if item]
        appserver_live_consumed_models = [
            str(item) for item in appserver_probe.get("consumedModels", []) if item
        ]
        appserver_bound = (
            appserver_probe.get("version") == codex_appserver_model_bridge.SHIM_VERSION
            and bool(appserver_probe.get("wrapperActive"))
            and appserver_live_models == expected_models
            and appserver_live_consumed_models == expected_models
            and appserver_probe.get("consumedGeneration") == appserver_probe.get("generation")
            and appserver_probe.get("consumedNextCursor") in (None, "")
            and bool(appserver_probe.get("queryRefetchConfirmed"))
        )
        legacy_bound = (
            state.get("modelListVersion") == "codex-model-list-bridge-shim/v6"
            and model_list_models == expected_models
            and state.get("modelListConsumedSignature") == state.get("modelListSignature")
            and model_list_consumed_models == expected_models
            and state.get("modelListConsumedNextCursor") in (None, "")
        )
        appserver_supported = bool(appserver_discovery.get("ok"))
        binding_route = (
            "appserver"
            if appserver_bound
            else "legacy_model_list"
            if not appserver_supported and legacy_bound
            else ""
        )
        statsig_bound = (
            statsig_probe.get("version") == "codex-statsig-allowlist-protection/v5"
            and statsig_required_models == expected_models
            and bool(statsig_probe.get("ok"))
            and bool(statsig_probe.get("nativeSetItemRecovered"))
            and bool(statsig_probe.get("storageWrapperActive"))
            and int(statsig_probe.get("clientsProtected") or 0)
            == int(statsig_probe.get("clientsFound") or 0)
            and all(model in statsig_available_models for model in expected_models)
        )
        model_runtime_bound = bool(binding_route) and statsig_bound
        reasoning_bound = (
            state.get("reasoningVersion") == "codex-reasoning-capability-bridge/v1"
            and reasoning_efforts == expected_reasoning_efforts
        )
        picker_signature = str(
            picker_probe.get("signatureAfter") or picker_probe.get("previousSignature") or ""
        )
        picker_view = str(picker_probe.get("viewAfter") or picker_probe.get("viewBefore") or "")
        picker_persistence_confirmed = bool(picker_probe.get("persistenceConfirmed"))
        picker_signature_bound = not expected_picker_signature or picker_signature == expected_picker_signature
        picker_view_bound = (
            not expected_picker_signature
            or not require_advanced_picker
            or (picker_view == "advanced" and picker_persistence_confirmed)
        )
        picker_bound = picker_signature_bound and picker_view_bound
        bound = model_runtime_bound and reasoning_bound and picker_bound
        return {
            **result,
            "bound": bound,
            "appserver_supported": appserver_supported,
            "appserver_bound": appserver_bound,
            "appserver_models": appserver_models,
            "appserver_consumed_models": appserver_consumed_models,
            "appserver_live_probe": appserver_probe,
            "appserver_live_models": appserver_live_models,
            "appserver_live_consumed_models": appserver_live_consumed_models,
            "appserver_generation": appserver_probe.get("generation"),
            "appserver_consumed_generation": appserver_probe.get("consumedGeneration"),
            "appserver_query_refetch_confirmed": bool(appserver_probe.get("queryRefetchConfirmed")),
            "appserver_consumed_at": state.get("appServerConsumedAt"),
            "appserver_consumed_next_cursor": state.get("appServerConsumedNextCursor"),
            "legacy_bound": legacy_bound,
            "model_list_models": model_list_models,
            "model_list_consumed_models": model_list_consumed_models,
            "model_list_consumed_at": state.get("modelListConsumedAt"),
            "model_list_consumed_next_cursor": state.get("modelListConsumedNextCursor"),
            "binding_route": binding_route,
            "statsig_required_models": statsig_required_models,
            "statsig_available_models": statsig_available_models,
            "statsig_clients_found": int(statsig_probe.get("clientsFound") or 0),
            "statsig_clients_protected": int(statsig_probe.get("clientsProtected") or 0),
            "statsig_live_probe": statsig_probe,
            "statsig_bound": statsig_bound,
            "reasoning_bound": reasoning_bound,
            "reasoning_efforts": reasoning_efforts,
            "reasoning_gates": state.get("reasoningGates") if isinstance(state.get("reasoningGates"), dict) else {},
            "model_picker_view": picker_view,
            "model_picker_view_sync_signature": picker_signature,
            "model_picker_persistence_route": str(picker_probe.get("persistenceRoute") or ""),
            "model_picker_persistence_confirmed": picker_persistence_confirmed,
            "model_picker_signature_bound": picker_signature_bound,
            "model_picker_view_bound": picker_view_bound,
            "model_picker_view_sync_required": not picker_bound,
            "reason": "" if bound else "runtime_binding_missing_or_stale",
        }
    except Exception as exc:
        return {**result, "ok": False, "reason": "runtime_binding_probe_failed", "error": repr(exc)}
    finally:
        if client is not None:
            client.close()


def _scheduled_task_state() -> dict[str, Any]:
    if os.name != "nt":
        return {
            "exists": False,
            "skipped": True,
            "deferred": True,
            "reason": "windows_scheduled_task_owner_deferred",
        }
    script = f"""
$task = Get-ScheduledTask -TaskName '{TASK_NAME}' -ErrorAction SilentlyContinue
if ($task) {{
  $info = Get-ScheduledTaskInfo -TaskName '{TASK_NAME}' -ErrorAction SilentlyContinue
  [pscustomobject]@{{
    exists = $true
    state = [string]$task.State
    lastRunTime = $info.LastRunTime
    lastTaskResult = $info.LastTaskResult
    nextRunTime = $info.NextRunTime
    actions = @($task.Actions | ForEach-Object {{ [pscustomobject]@{{ execute = $_.Execute; arguments = $_.Arguments }} }})
  }} | ConvertTo-Json -Depth 6 -Compress
}} else {{
  [pscustomobject]@{{ exists = $false }} | ConvertTo-Json -Compress
}}
"""
    completed = subprocess.run(
        ["powershell", "-NoProfile", "-Command", script],
        capture_output=True,
        text=True,
        encoding="utf-8",
        errors="replace",
        timeout=10,
        creationflags=subprocess.CREATE_NO_WINDOW if hasattr(subprocess, "CREATE_NO_WINDOW") else 0,
        check=False,
    )
    try:
        value = json.loads(completed.stdout) if completed.stdout.strip() else {}
    except json.JSONDecodeError:
        value = {}
    return value if isinstance(value, dict) else {}


def snapshot() -> dict[str, Any]:
    state = _read_state()
    source = source_state()
    implementation_fingerprint = _implementation_fingerprint()
    if source.get("catalog_models"):
        binding = runtime_binding_state(
            list(source.get("catalog_models") or []),
            list(source.get("catalog_reasoning_efforts") or []),
            str(source.get("signature_hash") or ""),
        )
    else:
        binding = {
            "schema": "codex-model-provider-watcher/runtime-binding/v1",
            "ok": True,
            "bound": True,
            "skipped": True,
            "page_count": 0,
            "reason": "native_provider_without_catalog_no_runtime_override_required",
        }
    return {
        "schema": "codex-model-provider-watcher/snapshot/v1",
        "ok": True,
        "generated_at": _now(),
        "source": source,
        "state_path": str(STATE_PATH),
        "state": state,
        "event_log": _event_log_state(),
        "implementation_fingerprint": implementation_fingerprint,
        "implementation_current": state.get("implementation_fingerprint") == implementation_fingerprint,
        "runtime_binding": binding,
        "signature_current": bool(source.get("signature_hash"))
        and source.get("signature_hash") == state.get("last_successful_signature_hash"),
        "scheduled_task": _scheduled_task_state(),
        "config_projection": codex_config_projection.validate(),
    }


def validate() -> dict[str, Any]:
    snap = snapshot()
    task = snap.get("scheduled_task") if isinstance(snap.get("scheduled_task"), dict) else {}
    binding = snap.get("runtime_binding") if isinstance(snap.get("runtime_binding"), dict) else {}
    config_projection = snap.get("config_projection") if isinstance(snap.get("config_projection"), dict) else {}
    catalog_provenance = (
        snap.get("source", {}).get("catalog_provenance")
        if isinstance(snap.get("source", {}).get("catalog_provenance"), dict)
        else {}
    )
    desktop_ready = bool(binding.get("page_count"))
    task_deferred = bool(task.get("deferred"))
    action_text = " ".join(str(item.get("arguments") or "") for item in task.get("actions", []) if isinstance(item, dict))
    checks = [
        {"name": "source_signature_available", "ok": bool(snap.get("source", {}).get("signature_hash"))},
        {"name": "scheduled_task_exists", "ok": task_deferred or bool(task.get("exists"))},
        {"name": "scheduled_task_owns_watcher", "ok": task_deferred or ("codex_model_provider_watcher.py" in action_text and " supervise" in action_text)},
        {"name": "state_directory_bounded", "ok": str(STATE_PATH).startswith(str(ROOT / "_bridge" / "runtime"))},
        {"name": "source_signature_converged", "ok": task_deferred or bool(snap.get("signature_current"))},
        {"name": "watcher_implementation_current", "ok": task_deferred or bool(snap.get("implementation_current"))},
        {"name": "runtime_binding_current", "ok": not desktop_ready or bool(binding.get("bound"))},
        {"name": "event_log_bounded", "ok": task_deferred or bool(snap.get("event_log", {}).get("bounded"))},
        {"name": "config_projection_current", "ok": bool(config_projection.get("ok"))},
        {
            "name": "model_catalog_provenance_current",
            "ok": bool(catalog_provenance.get("ok")) or bool(catalog_provenance.get("skipped")),
        },
    ]
    return {
        "schema": "codex-model-provider-watcher/validate/v1",
        "ok": all(item["ok"] for item in checks),
        "generated_at": _now(),
        "checks": checks,
        "snapshot": snap,
    }


def _acquire_lock() -> Any:
    import msvcrt

    LOCK_PATH.parent.mkdir(parents=True, exist_ok=True)
    handle = LOCK_PATH.open("a+b")
    if handle.tell() == 0:
        handle.write(b"0")
        handle.flush()
    handle.seek(0)
    try:
        msvcrt.locking(handle.fileno(), msvcrt.LK_NBLCK, 1)
    except OSError:
        handle.close()
        return None
    return handle


def watch(
    *,
    poll_seconds: float,
    debounce_seconds: float,
    drift_check_seconds: float,
    max_iterations: int = 0,
) -> int:
    lock = _acquire_lock()
    if lock is None:
        return 0
    implementation_fingerprint = _implementation_fingerprint()
    implementation_metadata_fingerprint = _implementation_metadata_fingerprint()
    previous_state = _read_state()
    last_seen = str(previous_state.get("last_seen_signature_hash") or "")
    last_successful = str(previous_state.get("last_successful_signature_hash") or "")
    last_projection_signature = str(previous_state.get("last_config_projection_signature") or "")
    cached_source: dict[str, Any] | None = None
    last_source_probe = ""
    stable_source_probe = ""
    stable_source_poll_count = 0
    last_unavailable_probe = ""
    next_source_retry_at = 0.0
    next_projection_retry_at = 0.0
    next_reconcile_at = 0.0
    runtime_reconcile_key = ""
    next_runtime_reconcile_at = 0.0
    changed_at = 0.0
    last_drift_check = time.monotonic()
    iterations = 0
    startup_state = dict(previous_state)
    if startup_state.get("restart_reason") == "implementation_changed":
        startup_state["restart_required"] = False
        startup_state.pop("restart_reason", None)
        startup_state.pop("next_implementation_fingerprint", None)
    startup_state.pop("watcher_restart_required", None)
    startup_state.pop("watcher_restart_reason", None)
    try:
        _write_json(
            STATE_PATH,
            {
                **startup_state,
                "schema": "codex-model-provider-watcher/state/v1",
                "updated_at": _now(),
                "pid": os.getpid(),
                "implementation_fingerprint": implementation_fingerprint,
                "restart_required": bool(startup_state.get("restart_required")),
                "last_successful_signature_hash": last_successful,
                "last_config_projection_signature": last_projection_signature,
            },
        )
        while True:
            iterations += 1
            active_implementation_metadata = _implementation_metadata_fingerprint()
            active_implementation_fingerprint = (
                _implementation_fingerprint()
                if active_implementation_metadata != implementation_metadata_fingerprint
                else implementation_fingerprint
            )
            if active_implementation_fingerprint != implementation_fingerprint:
                state = {
                    **_read_state(),
                    "schema": "codex-model-provider-watcher/state/v1",
                    "updated_at": _now(),
                    "pid": os.getpid(),
                    "implementation_fingerprint": implementation_fingerprint,
                    "next_implementation_fingerprint": active_implementation_fingerprint,
                    "watcher_restart_required": True,
                    "watcher_restart_reason": "implementation_changed",
                }
                _write_json(STATE_PATH, state)
                _append_event(
                    {
                        "schema": "codex-model-provider-watcher/event/v1",
                        "time": _now(),
                        "event": "implementation_changed",
                        "implementation_fingerprint": implementation_fingerprint,
                        "next_implementation_fingerprint": active_implementation_fingerprint,
                    }
                )
                return RESTART_FOR_IMPLEMENTATION_CHANGE
            implementation_metadata_fingerprint = active_implementation_metadata
            now = time.monotonic()
            source_probe = _source_probe_fingerprint(cached_source)
            refresh_source = (
                cached_source is None
                or source_probe != last_source_probe
                or (not bool(cached_source.get("ok")) and now >= next_source_retry_at)
            )
            if refresh_source:
                source = source_state()
                cached_source = source
                last_source_probe = _source_probe_fingerprint(source)
                if not source.get("ok"):
                    unavailable_key = f"{last_source_probe}:{source.get('config_projection_signature', '')}"
                    if unavailable_key != last_unavailable_probe:
                        _append_event(
                            {
                                "schema": "codex-model-provider-watcher/event/v1",
                                "time": _now(),
                                "event": "source_unavailable",
                                "source": _source_event_summary(source),
                                "retry_seconds": SOURCE_RETRY_SECONDS,
                            }
                        )
                        last_unavailable_probe = unavailable_key
                    next_source_retry_at = now + SOURCE_RETRY_SECONDS
                else:
                    last_unavailable_probe = ""
                    next_source_retry_at = 0.0
            else:
                source = cached_source
            if not source.get("ok"):
                if max_iterations and iterations >= max_iterations:
                    return 0
                time.sleep(max(0.25, poll_seconds))
                continue
            observed_source_probe = _source_probe_fingerprint(source)
            if observed_source_probe == stable_source_probe:
                stable_source_poll_count += 1
            else:
                stable_source_probe = observed_source_probe
                stable_source_poll_count = 1
            signature_hash = str(source.get("signature_hash") or "")
            projection_signature = str(source.get("config_projection_signature") or "")
            projection_available = bool(projection_signature) and not projection_signature.startswith("unavailable:")
            if projection_available and projection_signature != last_projection_signature and now >= next_projection_retry_at:
                capture_lease = codex_config_projection.capture_lease_state()
                projection_result = (
                    {"ok": True, "deferred": True, "reason": "mirror_capture_lease_active", "capture_lease": capture_lease}
                    if capture_lease.get("active")
                    else codex_config_projection.apply_projection(additions_only=True, sync_desktop=True)
                )
                startup_baseline_result = (
                    provider_scoped_startup_baseline_repair(dry_run=False)
                    if projection_result.get("ok") and not projection_result.get("deferred")
                    else {"ok": False, "skipped": True, "reason": "config_projection_failed"}
                )
                if projection_result.get("deferred"):
                    startup_baseline_result = {"ok": True, "deferred": True, "reason": "mirror_capture_lease_active"}
                    next_projection_retry_at = now + min(RECONCILE_RETRY_SECONDS, max(1.0, poll_seconds))
                elif projection_result.get("ok") and startup_baseline_result.get("ok"):
                    last_projection_signature = codex_config_projection.projection_signature()
                    next_projection_retry_at = 0.0
                else:
                    next_projection_retry_at = now + RECONCILE_RETRY_SECONDS
                current_state = {
                    **_read_state(),
                    "schema": "codex-model-provider-watcher/state/v1",
                    "updated_at": _now(),
                    "pid": os.getpid(),
                    "implementation_fingerprint": implementation_fingerprint,
                    "last_config_projection_signature": last_projection_signature,
                    "last_config_projection_result": projection_result,
                    "last_startup_baseline_result": startup_baseline_result,
                }
                _write_json(STATE_PATH, current_state)
                _append_event({
                    "schema": "codex-model-provider-watcher/event/v1",
                    "time": _now(),
                    "event": "config_projection_reconcile",
                    "result": projection_result,
                    "startup_baseline": startup_baseline_result,
                })
            if signature_hash and signature_hash != last_seen:
                last_seen = signature_hash
                changed_at = now
                next_reconcile_at = 0.0
                runtime_reconcile_key = ""
                next_runtime_reconcile_at = 0.0
                _write_json(
                    STATE_PATH,
                    {
                        **_read_state(),
                        "schema": "codex-model-provider-watcher/state/v1",
                        "updated_at": _now(),
                        "pid": os.getpid(),
                        "implementation_fingerprint": implementation_fingerprint,
                        "restart_required": False,
                        "last_seen_signature_hash": last_seen,
                        "last_successful_signature_hash": last_successful,
                        "last_config_projection_signature": last_projection_signature,
                        "stable_source_poll_count": stable_source_poll_count,
                    },
                )
                _append_event(
                    {
                        "schema": "codex-model-provider-watcher/event/v1",
                        "time": _now(),
                        "event": "source_changed",
                        "source": _source_event_summary(source),
                    }
                )
            if (
                signature_hash
                and signature_hash != last_successful
                and now - changed_at >= debounce_seconds
                and stable_source_poll_count >= REQUIRED_STABLE_SOURCE_POLLS
                and now >= next_reconcile_at
            ):
                result = reconcile(reload_if_changed=True, source_changed=True)
                if result.get("ok") or result.get("restart_required"):
                    last_successful = signature_hash
                    last_drift_check = now
                    next_reconcile_at = 0.0
                else:
                    next_reconcile_at = now + RECONCILE_RETRY_SECONDS
                state = {
                    "schema": "codex-model-provider-watcher/state/v1",
                    "updated_at": _now(),
                    "pid": os.getpid(),
                    "implementation_fingerprint": implementation_fingerprint,
                    "restart_required": bool(result.get("restart_required")),
                    "last_seen_signature_hash": signature_hash,
                    "last_successful_signature_hash": last_successful,
                    "last_config_projection_signature": last_projection_signature,
                    "last_result": result,
                }
                _write_json(STATE_PATH, state)
                _append_event({"schema": "codex-model-provider-watcher/event/v1", "time": _now(), "event": "reconcile", "result": result})
            elif signature_hash and now - last_drift_check >= max(1.0, drift_check_seconds):
                last_drift_check = now
                binding = runtime_binding_state(
                    list(source.get("catalog_models") or []),
                    list(source.get("catalog_reasoning_efforts") or []),
                    signature_hash,
                )
                if binding.get("bound"):
                    runtime_reconcile_key = ""
                    next_runtime_reconcile_at = 0.0
                else:
                    binding_key = f"{signature_hash}:{binding.get('reason') or 'unbound'}"
                    if binding_key != runtime_reconcile_key or now >= next_runtime_reconcile_at:
                        result = reconcile(
                            # A renderer-local repair must stay in that renderer. Reloading
                            # here destroys the injected bridge and creates a periodic loop.
                            reload_if_changed=False,
                            source_changed=False,
                            repair_startup_baseline=False,
                        )
                        runtime_reconcile_key = binding_key
                        if result.get("ok") or result.get("restart_required"):
                            next_runtime_reconcile_at = now + RUNTIME_RECONCILE_COOLDOWN_SECONDS
                            last_successful = signature_hash
                            next_reconcile_at = 0.0
                        else:
                            next_runtime_reconcile_at = now + RECONCILE_RETRY_SECONDS
                            next_reconcile_at = now + RECONCILE_RETRY_SECONDS
                        state = {
                            "schema": "codex-model-provider-watcher/state/v1",
                            "updated_at": _now(),
                            "pid": os.getpid(),
                            "implementation_fingerprint": implementation_fingerprint,
                            "restart_required": bool(result.get("restart_required")),
                            "last_seen_signature_hash": signature_hash,
                            "last_successful_signature_hash": last_successful,
                            "last_config_projection_signature": last_projection_signature,
                            "runtime_binding": binding,
                            "last_result": result,
                        }
                        _write_json(STATE_PATH, state)
                        _append_event(
                            {
                                "schema": "codex-model-provider-watcher/event/v1",
                                "time": _now(),
                                "event": "runtime_binding_reconcile",
                                "runtime_binding": binding,
                                "result": result,
                            }
                        )
            if max_iterations and iterations >= max_iterations:
                return 0
            time.sleep(max(0.25, poll_seconds))
    finally:
        try:
            lock.close()
        except Exception:
            pass


def _watch_child_command(
    *,
    poll_seconds: float,
    debounce_seconds: float,
    drift_check_seconds: float,
    max_iterations: int,
) -> list[str]:
    command = [
        sys.executable,
        str(Path(__file__).resolve()),
        "watch",
        "--poll-seconds",
        str(poll_seconds),
        "--debounce-seconds",
        str(debounce_seconds),
        "--drift-check-seconds",
        str(drift_check_seconds),
    ]
    if max_iterations > 0:
        command.extend(["--max-iterations", str(max_iterations)])
    return command


def supervise(
    *,
    poll_seconds: float,
    debounce_seconds: float,
    drift_check_seconds: float,
    max_iterations: int = 0,
    restart_delay_seconds: float = 0.5,
    max_restarts: int = DEFAULT_SUPERVISOR_MAX_RESTARTS,
    restart_window_seconds: float = DEFAULT_SUPERVISOR_RESTART_WINDOW_SECONDS,
) -> int:
    """Run the watcher and immediately replace children that request a code reload."""

    restart_times: list[float] = []
    child_command = _watch_child_command(
        poll_seconds=poll_seconds,
        debounce_seconds=debounce_seconds,
        drift_check_seconds=drift_check_seconds,
        max_iterations=max_iterations,
    )
    creation_flags = getattr(subprocess, "CREATE_NO_WINDOW", 0)
    while True:
        completed = subprocess.run(
            child_command,
            stdin=subprocess.DEVNULL,
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL,
            creationflags=creation_flags,
            check=False,
        )
        exit_code = int(completed.returncode)
        if exit_code != RESTART_FOR_IMPLEMENTATION_CHANGE:
            return exit_code

        now = time.monotonic()
        restart_times = [stamp for stamp in restart_times if now - stamp < max(1.0, restart_window_seconds)]
        if max_restarts < 1 or len(restart_times) >= max_restarts:
            _append_event(
                {
                    "schema": "codex-model-provider-watcher/event/v1",
                    "time": _now(),
                    "event": "supervisor_restart_exhausted",
                    "exit_code": exit_code,
                    "restart_count": len(restart_times),
                    "restart_window_seconds": restart_window_seconds,
                }
            )
            return exit_code
        restart_times.append(now)
        _append_event(
            {
                "schema": "codex-model-provider-watcher/event/v1",
                "time": _now(),
                "event": "supervisor_child_restart",
                "exit_code": exit_code,
                "restart_count": len(restart_times),
            }
        )
        time.sleep(max(0.1, restart_delay_seconds))


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Codex model provider change watcher")
    parser.add_argument("action", choices=["snapshot", "validate", "once", "watch", "supervise"])
    parser.add_argument("--poll-seconds", type=float, default=2.0)
    parser.add_argument("--debounce-seconds", type=float, default=1.5)
    parser.add_argument(
        "--drift-check-seconds",
        type=float,
        default=DEFAULT_DRIFT_CHECK_SECONDS,
        help="Lightweight runtime-binding probe interval; full reconcile runs only when a bridge is missing or stale",
    )
    parser.add_argument("--max-iterations", type=int, default=0)
    parser.add_argument("--dry-run", action="store_true")
    parser.add_argument("--full", action="store_true", help="Emit the complete successful result.")
    args = parser.parse_args(argv)
    if args.action == "snapshot":
        result = snapshot()
    elif args.action == "validate":
        result = validate()
    elif args.action == "once":
        result = reconcile(dry_run=args.dry_run, source_changed=True)
    elif args.action == "watch":
        return watch(
            poll_seconds=args.poll_seconds,
            debounce_seconds=args.debounce_seconds,
            drift_check_seconds=args.drift_check_seconds,
            max_iterations=args.max_iterations,
        )
    else:
        return supervise(
            poll_seconds=args.poll_seconds,
            debounce_seconds=args.debounce_seconds,
            drift_check_seconds=args.drift_check_seconds,
            max_iterations=args.max_iterations,
        )
    output = governed_cli_payload(
        result,
        full=bool(args.full),
        full_result_ref=f"command:python _bridge/codex_model_provider_watcher.py {args.action} --full",
    )
    print(json.dumps(output, ensure_ascii=False, indent=2))
    return 0 if bool(result.get("ok", True)) else 1


if __name__ == "__main__":
    raise SystemExit(main())
