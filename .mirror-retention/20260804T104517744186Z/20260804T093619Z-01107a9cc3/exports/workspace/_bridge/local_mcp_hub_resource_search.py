...
def _resource_search_call_in_process(name: str, arguments: dict[str, Any]) -> dict[str, Any] | None:
    ...
    try:
        ...
        backend_outcomes: list[dict[str, str]] = []
        for backend in backend_attempts:
            try:
                results = _bounded_results(method(**{**kwargs, "backend": backend}), max_results)
            except Exception as exc:
                backend_outcomes.append({"backend": backend, "status": "backend_unavailable", "error_class": type(exc).__name__, "reason": str(exc)[:500]})
                continue
            if results:
                return {
                    **base,
                    ...
                    "attempted_backends": [*backend_outcomes, {"backend": backend, "status": "completed"}],
                    "route_mode": str(arguments.get("route_mode") or ""),
                    "proxy_used": bool(_proxy_for(arguments)),
                }
            backend_outcomes.append({"backend": backend, "status": "no_results", "error_class": "no_results", "reason": "no results"})
        unavailable_count = sum(1 for item in backend_outcomes if item.get("status") == "backend_unavailable")
        no_result_count = sum(1 for item in backend_outcomes if item.get("status") == "no_results")
        if unavailable_count and not no_result_count:
            status, error_class, reason = (
                "backend_unavailable",
                "search_backends_unavailable",
                "all configured search backends were unavailable",
            )
        elif unavailable_count:
            status, error_class, reason = (
                "failed",
                "search_backends_partially_unavailable",
                "some configured search backends were unavailable and the remaining backends returned no results",
            )
        else:
            status, error_class, reason = (
                "no_results",
                "search_backends_returned_no_results",
                "all configured search backends returned no usable results",
            )
        return {
            **base,
            "ok": False,
            ...
            "result_count": 0,
            "results": [],
            "attempted_backends": backend_outcomes,
            "route_mode": str(arguments.get("route_mode") or ""),
            "proxy_used": bool(_proxy_for(arguments)),
        }
    except Exception as exc:
