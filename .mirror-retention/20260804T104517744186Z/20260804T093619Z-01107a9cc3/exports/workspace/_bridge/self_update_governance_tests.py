...
class OwnerReviewItemsTests(unittest.TestCase):
    def test_closeout_owner_selection_is_surface_scoped(self) -> None:
        ...
        self.assertEqual(
            ...
            [
                "config_guard",
                "desktop_protocol_compatibility",
                "config_projection",
    ...
    def test_owner_catalog_is_derived_for_all_registered_systems(self) -> None:
        ...
        self.assertNotIn("environment_mirror", OWNER_SPECS)

    def test_registered_audio_and_hardware_changes_select_their_owners(self) -> None:
        audio_owners = select_self_update_owners(
            changed_files=["_bridge/runtime/music_library/corrections/reviewed.json"],
            changed_surfaces=[],
            task_kind="audio-maintenance",
            outcome="ok",
            major_change=False,
        )
        hardware_owners = select_self_update_owners(
            changed_files=["_bridge/usb_device_owner.py"],
            changed_surfaces=[],
            task_kind="hardware-maintenance",
            outcome="ok",
            major_change=False,
        )

        self.assertIn("music_library_owner", audio_owners)
        self.assertIn("audio_toolkit", audio_owners)
        self.assertIn("windows_hardware_owner", hardware_owners)
        self.assertIn("usb_device_owner", hardware_owners)
        self.assertIn("usb_device_control", hardware_owners)

    def test_closeout_uses_compatibility_probes_instead_of_ambient_health_doctors(self) -> None:
        self.assertEqual(
            OWNER_SPECS["mobile_bridge"]["command"],
            ["mobile_openclaw_bridge/mobile_openclaw_cli.py", "mobile-execution-contract-check"],
        )
        self.assertEqual(OWNER_SPECS["session_store"]["command"], ["codex_session_store_doctor.py", "validate"])
        self.assertEqual(OWNER_SPECS["audio_toolkit"]["command"], ["audio_toolkit/audio_toolkit.py", "validate"])
        self.assertEqual(OWNER_SPECS["record_store"]["command"], ["shared/record_store_maintenance.py", "validate"])
        self.assertEqual(OWNER_SPECS["workflow"]["timeout"], 150)
        self.assertEqual(OWNER_SPECS["mobile_bridge"]["timeout"], 30)
        self.assertEqual(OWNER_SPECS["session_store"]["timeout"], 30)
        self.assertEqual(OWNER_SPECS["record_store"]["timeout"], 30)
    ...
    def test_validated_membership_meta_files_do_not_expand_targeted_audio_change(self) -> None:
        payload = build_change_set(
            changed_files=[
                "workspace/_bridge/system_membership.py",
                "/home/codexlab/work/codex-workspace/workspace/_bridge/system_membership_tests.py",
                "_bridge/runtime/music_library/corrections/reviewed.json",
            ],
            task_kind="audio-maintenance",
            outcome="ok",
        ...
        self.assertIn("audio", payload["affected_systems"])
        self.assertNotIn("bridge", payload["affected_systems"])
        self.assertIn("music_library_owner", payload["selected_owners"])
    ...
    def test_windows_owner_is_deferred_without_running_inside_wsl(self) -> None:
        with patch("self_update_governance.execution_platform_scope", return_value="wsl"), patch(
            "self_update_governance.run_owner"
        ) as run_owner:
            payload = snapshot(selected_owners=["windows_hardware_owner"], force_fresh=True)

        run_owner.assert_not_called()
        owner = payload["owners"]["windows_hardware_owner"]
        self.assertTrue(owner["ok"])
        self.assertEqual(owner["execution_state"], "owner_deferred_platform_scope")
        self.assertEqual(owner["platform_scope"], "windows_host")
        self.assertEqual(payload["signals"], [])
    ...
    def test_validated_membership_meta_change_uses_specific_changed_file_closure(self) -> None:
        payload = build_change_set(
            changed_files=["workspace/_bridge/system_membership.py", "workspace/_bridge/codex_config_projection.py"],
    ...
    def test_only_live_owner_execution_is_authoritative_for_queue_reconciliation(self) -> None:
        ...
        with patch(
            ...
            live_payload = doctor(selected_owners=["workflow"], force_fresh=True)
        self.assertEqual(live_payload["authoritative_owners"], ["workflow"])

    def test_recent_success_receipt_is_reused_only_for_same_change_evidence(self) -> None:
        with tempfile.TemporaryDirectory() as temp, patch.object(self_update_governance, "RUNTIME_DIR", Path(temp)), patch(
            "self_update_governance.run_owner",
            return_value={
                "name": "workflow",
                "ok": True,
            ...
            first = snapshot(selected_owners=["workflow"], changed_files=["_bridge/workflow_orchestrator.py"])
            second = snapshot(selected_owners=["workflow"], changed_files=["_bridge/workflow_orchestrator.py"])
        self.assertEqual(run_owner.call_count, 1)
        self.assertEqual(first["selection"]["receipt_reuse_count"], 0)
        self.assertEqual(second["selection"]["receipt_reuse_count"], 1)
        self.assertEqual(second["owners"]["workflow"]["execution_state"], "validation_receipt_reused")
    ...
    def test_workflow_failed_checks_become_concrete_review_items(self) -> None:
        items = owner_review_items(
            "workflow",
            {
                "ok": False,
                "checks": [
                    {
                        ...
                        "ok": False,
                        "detail": {"member_gate_schemas": [], "state_change_expected": True},
                    }
                ],
            },
            "Run workflow_orchestrator.py validate --full and repair the failed check.",
        )
        self.assertEqual(len(items), 1)
        self.assertEqual(
            items[0]["source_item_id"],
            "self_update:workflow:system_member_change_gate_reaches_micro_route_pack",
        )
        self.assertIn("member_gate_schemas", items[0]["summary"])

    def test_workflow_bounded_actionable_failure_becomes_concrete_review_item(self) -> None:
        items = owner_review_items(
            "workflow",
            {
                "ok": False,
            ...
            "Run workflow_orchestrator.py validate --full.",
        )
        self.assertEqual(len(items), 1)
        self.assertEqual(
            items[0]["source_item_id"],
    ...
    def test_failed_owner_without_structured_evidence_gets_explicit_fallback_card(self) -> None:
        items = owner_review_items("workflow", {"ok": False}, "Run workflow owner doctor.")
        self.assertEqual(len(items), 1)
        self.assertEqual(items[0]["source_item_id"], "self_update:workflow:owner_evidence_unavailable")
        self.assertIn("no structured review item", items[0]["summary"])
