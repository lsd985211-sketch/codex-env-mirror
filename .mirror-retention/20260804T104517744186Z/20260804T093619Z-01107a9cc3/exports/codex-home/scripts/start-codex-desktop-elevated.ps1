...
# The Windows local host owns this profile. Remote WSL connections resolve
# their own home through the remote app-server service.
...
function Initialize-CodexDesktopLaunchEnvironment {
    $machinePath = [Environment]::GetEnvironmentVariable("Path", "Machine")
    $userPath = [Environment]::GetEnvironmentVariable("Path", "User")
    $entries = New-Object System.Collections.Generic.List[string]
    $seen = New-Object System.Collections.Generic.HashSet[string] ([System.StringComparer]::OrdinalIgnoreCase)
    $rejectedAmbiguousEntryCount = 0

    $preferredGitDirectory = Join-Path $env:ProgramFiles "Git\cmd"
    if (Test-Path -LiteralPath (Join-Path $preferredGitDirectory "git.exe") -PathType Leaf) {
        $entries.Add($preferredGitDirectory)
        [void]$seen.Add($preferredGitDirectory.TrimEnd("\"))
    } else {
        $preferredGitDirectory = ""
    }
    foreach ($scopePath in @($machinePath, $userPath)) {
        foreach ($rawEntry in ([string]$scopePath -split ";")) {
            $entry = [Environment]::ExpandEnvironmentVariables($rawEntry.Trim())
            $ambiguous = $entry -match '^[A-Za-z]$' -or $entry -match '^\\(?!\\)'
            if ($ambiguous) {
                $rejectedAmbiguousEntryCount += 1
            } elseif (-not [string]::IsNullOrWhiteSpace($entry) -and $seen.Add($entry.TrimEnd("\"))) {
                $entries.Add($entry)
            }
        }
    }
    ...
    $resolvedGit = if ($null -ne $gitCommand) { [string]$gitCommand.Source } else { "" }
    return [ordered]@{
        ...
        rejected_ambiguous_entry_count = [int]$rejectedAmbiguousEntryCount
        preferred_git_directory = $preferredGitDirectory
        git_path_present = -not [string]::IsNullOrWhiteSpace($resolvedGit)
        resolved_git = $resolvedGit
        path_input_signature = Get-CodexTextSha256 -Value ([string]$env:Path)
        codex_home_policy = "windows_local_host"
        codex_home_present_for_child = -not [string]::IsNullOrWhiteSpace($env:CODEX_HOME)
        ready = [bool]($entries.Count -gt 0 -and -not [string]::IsNullOrWhiteSpace($resolvedGit))
    }
}

function Write-CodexDesktopLaunchEnvironmentReceipt {
    param($Receipt)
    $path = Join-Path $stateDir "codex-desktop-launch-environment.json"
...
$desktopLaunchEnvironment = Initialize-CodexDesktopLaunchEnvironment
$desktopLaunchEnvironmentReceipt = Write-CodexDesktopLaunchEnvironmentReceipt -Receipt $desktopLaunchEnvironment
Write-StartLog "Canonical Desktop launch environment initialized. Ready=$([bool]$desktopLaunchEnvironment.ready); EntryCount=$($desktopLaunchEnvironment.entry_count); GitPathPresent=$([bool]$desktopLaunchEnvironment.git_path_present); ResolvedGit=$($desktopLaunchEnvironment.resolved_git); Receipt=$desktopLaunchEnvironmentReceipt"
...
function Start-CodexVersionFingerprintAsync {
    if ($dryRun -or $env:CODEX_UPDATE_INTELLIGENCE_DISABLED -eq "1") {
        return
    }
    $helperPath = Join-Path $env:USERPROFILE ".codex\scripts\codex-version-fingerprint.ps1"
    if (-not (Test-Path -LiteralPath $helperPath -PathType Leaf)) {
        Write-StartLog "Codex version fingerprint helper is unavailable; Desktop startup will continue. Helper=$helperPath"
        return
    }
    ...
    try {
        Start-Process `
            ...
            -ArgumentList @("-NoLogo", "-NoProfile", "-NonInteractive", "-ExecutionPolicy", "Bypass", "-File", $helperPath) `
            -WindowStyle Hidden | Out-Null
        Write-StartLog "Codex version fingerprint helper dispatched asynchronously."
    } catch {
        Write-StartLog "Codex version fingerprint helper dispatch failed; Desktop startup will continue. Error=$($_.Exception.Message)"
    }
}

Start-CodexVersionFingerprintAsync
...
$desktopRestartRequestPath = Join-Path $stateDir "codex-desktop-restart-request.json"
$desktopRestartConsumingPath = Join-Path $stateDir "codex-desktop-restart-request.consuming.json"
$desktopRestartInvalidPath = Join-Path $stateDir "codex-desktop-restart-request.invalid.json"
$desktopRestartReceiptPath = Join-Path $stateDir "codex-desktop-restart-receipt.json"
$script:CodexDesktopRestartRequest = $null
$script:CodexDesktopRestartBeforePids = @()
$script:CodexDesktopRestartExitMethod = ""

function Get-CodexDesktopRestartSignature {
    param($Request)
    $parts = @(
        [string](Get-ObjectPropertyValue -Object $Request -Name "schema" -Default ""),
        [string](Get-ObjectPropertyValue -Object $Request -Name "request_id" -Default ""),
        [string](Get-ObjectPropertyValue -Object $Request -Name "requested_at" -Default ""),
        [string](Get-ObjectPropertyValue -Object $Request -Name "expires_at" -Default ""),
        [string](Get-ObjectPropertyValue -Object $Request -Name "reason" -Default ""),
        [string](Get-ObjectPropertyValue -Object $Request -Name "token" -Default "")
    )
...
function Write-CodexDesktopRestartReceipt {
    param(
        [bool]$Ok,
        [string]$Status,
        [string]$Reason,
    ...
    $requestId = if ($null -ne $script:CodexDesktopRestartRequest) {
        [string](Get-ObjectPropertyValue -Object $script:CodexDesktopRestartRequest -Name "request_id" -Default "")
    } else { "" }
    [ordered]@{
        schema = "desktop_restart_request.v1.receipt"
        ok = $Ok
        status = $Status
        reason = $Reason
        request_id = $requestId
        completed_at = (Get-Date).ToUniversalTime().ToString("o")
        before_pids = @($script:CodexDesktopRestartBeforePids)
        after_pids = @($AfterPids)
        cdp_ready = $CdpReady
        force_kill_used = $false
        graceful_exit_method = $script:CodexDesktopRestartExitMethod
        execution_host = "windows_task_scheduler"
    } | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath $desktopRestartReceiptPath -Encoding UTF8
}

function Get-CodexDesktopMainProcessRecords {
    @(
...
function Receive-CodexDesktopRestartRequest {
    if (-not (Test-Path -LiteralPath $desktopRestartRequestPath)) {
        return $null
    }
    try {
        $request = Get-Content -LiteralPath $desktopRestartRequestPath -Raw -Encoding UTF8 | ConvertFrom-Json -ErrorAction Stop
        $schema = [string](Get-ObjectPropertyValue -Object $request -Name "schema" -Default "")
        $token = [string](Get-ObjectPropertyValue -Object $request -Name "token" -Default "")
        $signature = [string](Get-ObjectPropertyValue -Object $request -Name "input_signature" -Default "")
        $expectedSignature = Get-CodexDesktopRestartSignature -Request $request
        $expiresAt = [datetimeoffset]::Parse([string](Get-ObjectPropertyValue -Object $request -Name "expires_at" -Default ""))
        if (
            $schema -ne "desktop_restart_request.v1" -or
            $token -ne "FULL-DESKTOP-RESTART" -or
            $signature -ne $expectedSignature -or
            $expiresAt -le [datetimeoffset]::UtcNow
        ) {
            throw "request schema, token, signature, or expiry is invalid"
        }
        Move-Item -LiteralPath $desktopRestartRequestPath -Destination $desktopRestartConsumingPath -Force
        return $request
    } catch {
        Move-Item -LiteralPath $desktopRestartRequestPath -Destination $desktopRestartInvalidPath -Force -ErrorAction SilentlyContinue
        $script:CodexDesktopRestartRequest = $null
        Write-CodexDesktopRestartReceipt -Ok $false -Status "failed" -Reason "restart_request_invalid"
        Write-StartLog "Desktop full restart request rejected: $($_.Exception.Message)"
        return $null
    }
}

function Invoke-CodexDesktopRequestedRestart {
    $request = Receive-CodexDesktopRestartRequest
    if ($null -eq $request) {
        return
    }
    $script:CodexDesktopRestartRequest = $request
    $before = @(Get-CodexDesktopProcessRecords)
    $script:CodexDesktopRestartBeforePids = @($before | ForEach-Object { [int]$_.ProcessId })
    $mainProcesses = @(Get-CodexDesktopMainProcessRecords)
    $closeRequested = @()
    foreach ($record in $mainProcesses) {
        $process = Get-Process -Id ([int]$record.ProcessId) -ErrorAction Stop
        if ($process.CloseMainWindow()) {
            $closeRequested += [int]$record.ProcessId
        }
    }
    $deadline = (Get-Date).AddSeconds(5)
    do {
        Start-Sleep -Milliseconds 750
        $remaining = @(Get-CodexDesktopProcessRecords)
    } while ($remaining.Count -gt 0 -and (Get-Date) -lt $deadline)
    $gracefulExitMethod = "window_close"
    if ($remaining.Count -gt 0) {
        $workspace = "C:\Users\45543\Downloads\mcsmanager_windows_release\mcsmanager"
        $restartOwner = Join-Path $workspace "_bridge\desktop_restart_request.py"
        $pythonExe = Resolve-CodexStartupPython
        if (-not $pythonExe -or -not (Test-Path -LiteralPath $restartOwner)) {
            Write-CodexDesktopRestartReceipt -Ok $false -Status "blocked" -Reason "desktop_restart_owner_unavailable" -AfterPids @($remaining | ForEach-Object { [int]$_.ProcessId })
            Move-Item -LiteralPath $desktopRestartConsumingPath -Destination $desktopRestartInvalidPath -Force -ErrorAction SilentlyContinue
            Write-StartLog "Desktop full restart blocked because the typed CDP exit owner is unavailable. Owner=$restartOwner; Python=$pythonExe"
            exit 9
        }
        $requestId = [string](Get-ObjectPropertyValue -Object $request -Name "request_id" -Default "")
        $inputSignature = [string](Get-ObjectPropertyValue -Object $request -Name "input_signature" -Default "")
        $script:CodexDesktopRestartExitMethod = "electron_quit_ipc"
        $signal = $null
        $signalExitCode = -1
        try {
            Push-Location -LiteralPath $workspace
            $signalText = & $pythonExe $restartOwner "signal-exit" "--request-id" $requestId "--input-signature" $inputSignature "--wait-seconds" "20" 2>&1
            $signalExitCode = $LASTEXITCODE
            $signal = ($signalText -join [Environment]::NewLine) | ConvertFrom-Json -ErrorAction Stop
        } catch {
            $remaining = @(Get-CodexDesktopProcessRecords)
            Write-CodexDesktopRestartReceipt -Ok $false -Status "blocked" -Reason "desktop_cdp_exit_owner_failed" -AfterPids @($remaining | ForEach-Object { [int]$_.ProcessId })
            Move-Item -LiteralPath $desktopRestartConsumingPath -Destination $desktopRestartInvalidPath -Force -ErrorAction SilentlyContinue
            Write-StartLog "Desktop full restart blocked because the typed CDP exit owner failed. Error=$($_.Exception.Message); Remaining=$(@($remaining | ForEach-Object { [int]$_.ProcessId }) -join ',')"
            exit 9
        } finally {
            try { Pop-Location } catch { }
        }
        $gracefulExitMethod = [string](Get-ObjectPropertyValue -Object $signal -Name "method" -Default "electron_quit_ipc")
        $script:CodexDesktopRestartExitMethod = $gracefulExitMethod
        $remaining = @(Get-CodexDesktopProcessRecords)
        if ($signalExitCode -ne 0 -or -not [bool]$signal.ok -or $remaining.Count -gt 0) {
            Write-CodexDesktopRestartReceipt -Ok $false -Status "blocked" -Reason "desktop_cdp_exit_failed" -AfterPids @($remaining | ForEach-Object { [int]$_.ProcessId })
            Move-Item -LiteralPath $desktopRestartConsumingPath -Destination $desktopRestartInvalidPath -Force -ErrorAction SilentlyContinue
            Write-StartLog "Desktop full restart blocked because the native Electron quit IPC did not quiesce the process family. Method=$gracefulExitMethod; ExitCode=$signalExitCode; Reason=$([string]$signal.reason); Remaining=$(@($remaining | ForEach-Object { [int]$_.ProcessId }) -join ',')"
            exit 9
        }
    }
    $script:CodexDesktopRestartExitMethod = $gracefulExitMethod
    Remove-Item -LiteralPath $desktopRestartConsumingPath -Force -ErrorAction SilentlyContinue
    Write-CodexDesktopRestartReceipt -Ok $false -Status "relaunch_pending" -Reason "desktop_exited_gracefully"
    Write-StartLog "Desktop full restart request consumed by the scheduled launcher. BeforePids=$($script:CodexDesktopRestartBeforePids -join ','); CloseRequested=$($closeRequested -join ','); GracefulExitMethod=$gracefulExitMethod"
}

function Test-CodexConfigPreflightShouldBlock {
    param($Result)
    $strictMode = $env:CODEX_STARTUP_PREFLIGHT_FAIL_CLOSED -eq "1"
    $wslEnabled = [bool](Get-ObjectPropertyValue -Object $Result -Name "WslEnabled" -Default $false)
    $wslRuntimeReady = [bool](Get-ObjectPropertyValue -Object $Result -Name "WslRuntimeReady" -Default (-not $wslEnabled))
    $wslProjectionUnavailable = $wslEnabled -and -not $wslRuntimeReady
    return $strictMode -and -not $wslProjectionUnavailable
}
...
function Invoke-CodexDesktopProtocolCompatibilityPreflight {
    $workspace = "C:\Users\45543\Downloads\mcsmanager_windows_release\mcsmanager"
    $ownerScript = Join-Path $workspace "_bridge\codex_desktop_protocol_compatibility.py"
    if (-not (Test-Path -LiteralPath $ownerScript)) {
        Write-StartLog "Codex Desktop protocol compatibility owner is missing: $ownerScript"
        return @{ Ok = $false; Status = "owner_missing"; MigrationComplete = $false }
    }
    $pythonExe = Resolve-CodexStartupPython
    if (-not $pythonExe) {
        Write-StartLog "Codex Desktop protocol compatibility preflight failed: no Python executable was found."
        return @{ Ok = $false; Status = "missing_python"; MigrationComplete = $false }
    }
    try {
        $runId = [guid]::NewGuid().ToString("N")
        $protocolOut = Join-Path $logDir "codex-protocol-compatibility-$runId.out.log"
        $protocolErr = Join-Path $logDir "codex-protocol-compatibility-$runId.err.log"
        $process = Start-Process `
            -FilePath $pythonExe `
            -ArgumentList @($ownerScript, "inspect") `
            -WorkingDirectory $workspace `
            -WindowStyle Hidden `
            -PassThru `
            -RedirectStandardOutput $protocolOut `
            -RedirectStandardError $protocolErr
        $null = $process.Handle
        $timeoutSeconds = Get-EnvInt -Name "CODEX_PROTOCOL_PREFLIGHT_TIMEOUT_SECONDS" -Default 15
        if ($timeoutSeconds -lt 1) { $timeoutSeconds = 1 }
        if (-not $process.WaitForExit($timeoutSeconds * 1000)) {
            & taskkill.exe /PID $process.Id /T /F 2>&1 | Out-Null
            Write-StartLog "Codex Desktop protocol compatibility preflight timed out. Pid=$($process.Id); TimeoutSeconds=$timeoutSeconds; Output=$protocolOut; Error=$protocolErr"
            return @{ Ok = $false; Status = "inspection_timeout"; MigrationComplete = $false }
        }
        $exitCode = $process.ExitCode
        $output = if (Test-Path -LiteralPath $protocolOut) { Get-Content -LiteralPath $protocolOut -Raw -Encoding UTF8 } else { "" }
        $stderr = if (Test-Path -LiteralPath $protocolErr) { Get-Content -LiteralPath $protocolErr -Raw -Encoding UTF8 } else { "" }
        if ([string]::IsNullOrWhiteSpace($output)) {
            throw "protocol compatibility owner returned empty output; ExitCode=$exitCode; Error=$($stderr.Trim())"
        }
        $result = $output | ConvertFrom-Json -ErrorAction Stop
        $ok = [bool](Get-ObjectPropertyValue -Object $result -Name "ok" -Default $false)
        $status = [string](Get-ObjectPropertyValue -Object $result -Name "status" -Default "unknown")
        $rollbackCallCount = [int](Get-ObjectPropertyValue -Object $result -Name "rollback_call_count" -Default 0)
        $migrationComplete = [bool](Get-ObjectPropertyValue -Object $result -Name "migration_complete" -Default $false)
        $suppressionDeclared = [bool](Get-ObjectPropertyValue -Object $result -Name "native_notice_suppression_declared" -Default $false)
        Write-StartLog "Codex Desktop protocol compatibility preflight completed. Ok=$ok; Status=$status; RollbackCallCount=$rollbackCallCount; MigrationComplete=$migrationComplete; NativeNoticeSuppressionDeclared=$suppressionDeclared; ExitCode=$exitCode"
        if ($ok -and $exitCode -eq 0) {
            Remove-Item -LiteralPath $protocolOut, $protocolErr -Force -ErrorAction SilentlyContinue
        }
        return @{
            Ok = $ok
            Status = $status
            ...
            NativeNoticeSuppressionDeclared = $suppressionDeclared
        }
    } catch {
        Write-StartLog "Codex Desktop protocol compatibility preflight failed: $($_.Exception.Message)"
        return @{ Ok = $false; Status = "inspection_failed"; MigrationComplete = $false }
    }
}
...
function Invoke-CodexConfigPreflightSync {
    ...
    try {
        $timeoutSeconds = Get-EnvInt -Name "CODEX_STARTUP_PREFLIGHT_TIMEOUT_SECONDS" -Default 75
        ...
        $wslProjection = Get-ObjectPropertyValue -Object $result -Name "wsl_runtime_projection" -Default $null
        $wslEnabled = [bool](Get-ObjectPropertyValue -Object $wslProjection -Name "enabled" -Default $false)
        $wslRuntimeReady = [bool](Get-ObjectPropertyValue -Object $result -Name "wsl_runtime_ready" -Default (-not $wslEnabled))
        $environmentSelectionReady = [bool](Get-ObjectPropertyValue -Object $result -Name "environment_selection_ready" -Default $true)
        $wslProjectionStatus = [string](Get-ObjectPropertyValue -Object $wslProjection -Name "status" -Default "not_reported")
        $wslResumeContextProjection = Get-ObjectPropertyValue -Object $result -Name "wsl_resume_context_projection" -Default $null
        $wslResumeContextStatus = [string](Get-ObjectPropertyValue -Object $wslResumeContextProjection -Name "status" -Default "not_reported")
        $beforeOk = $false
        $afterOk = $false
        $beforeResult = Get-ObjectPropertyValue -Object $result -Name "before" -Default $null
        $afterResult = Get-ObjectPropertyValue -Object $result -Name "after" -Default $null
        $beforeOk = [bool](Get-ObjectPropertyValue -Object $beforeResult -Name "ok" -Default $false)
        $afterOk = [bool](Get-ObjectPropertyValue -Object $afterResult -Name "ok" -Default $false)
        Write-StartLog "Codex config preflight completed. Ok=$([bool]$result.ok); Applied=$applied; NeedsRestart=$needsRestart; BeforeOk=$beforeOk; AfterOk=$afterOk; EnvironmentSelectionReady=$environmentSelectionReady; WslEnabled=$wslEnabled; WslRuntimeReady=$wslRuntimeReady; WslProjectionStatus=$wslProjectionStatus; WslResumeContextStatus=$wslResumeContextStatus"
        $preflightResult = @{
            Ok = [bool]$result.ok
            Applied = $applied
            ...
            WslRuntimeReady = $wslRuntimeReady
            EnvironmentSelectionReady = $environmentSelectionReady
            WslProjectionStatus = $wslProjectionStatus
            WslResumeContextStatus = $wslResumeContextStatus
        }
        if ([bool]$result.ok -and $environmentSelectionReady -and $wslRuntimeReady) {
...
function Invoke-CodexDesktopNativeRuntimeRetirement {
    $workspace = "C:\Users\45543\Downloads\mcsmanager_windows_release\mcsmanager"
    $runtimeScript = Join-Path $workspace "_bridge\codex_desktop_model_runtime.py"
    if (-not (Test-Path -LiteralPath $runtimeScript)) {
        return @{ Ok = $false; NativeAuthoritative = $false; Retired = $false; Reason = "missing_runtime_script" }
    }
    $pythonExe = Resolve-CodexStartupPython
    if (-not $pythonExe) {
        return @{ Ok = $false; NativeAuthoritative = $false; Retired = $false; Reason = "missing_python" }
    }

    $waitSeconds = Get-EnvInt -Name "CODEX_NATIVE_RUNTIME_RETIRE_WAIT_SECONDS" -Default 30
    if ($waitSeconds -lt 0) { $waitSeconds = 0 }
    $env:PYTHONUTF8 = "1"
    $env:PYTHONIOENCODING = "utf-8"
    try {
        Push-Location -LiteralPath $workspace
        $text = & $pythonExe $runtimeScript "legacy-runtime-shims-retire" "--wait-seconds" "$waitSeconds" "--json" 2>&1
        if ($LASTEXITCODE -ne 0) {
            return @{ Ok = $false; NativeAuthoritative = $false; Retired = $false; Reason = "native_runtime_probe_failed" }
        }
        $result = ($text -join [Environment]::NewLine) | ConvertFrom-Json -ErrorAction Stop
        $nativeRuntime = Get-ObjectPropertyValue -Object $result -Name "native_runtime" -Default $null
        $nativeAuthoritative = [bool](Get-ObjectPropertyValue -Object $nativeRuntime -Name "native_authoritative" -Default $false)
        return @{
            Ok = [bool](Get-ObjectPropertyValue -Object $result -Name "ok" -Default $false)
            NativeAuthoritative = $nativeAuthoritative
            Retired = [bool](Get-ObjectPropertyValue -Object $result -Name "retired" -Default $false)
            Reason = [string](Get-ObjectPropertyValue -Object $result -Name "reason" -Default "")
        }
    } catch {
        return @{ Ok = $false; NativeAuthoritative = $false; Retired = $false; Reason = $_.Exception.Message }
    } finally {
        try { Pop-Location } catch { }
    }
}
...
function Invoke-CodexDesktopModelRuntimeReconcile {
    ...
    $nativeRuntime = Invoke-CodexDesktopNativeRuntimeRetirement
    Write-PhaseLog "Desktop native model runtime phase finished"
    if ([bool]$nativeRuntime.NativeAuthoritative) {
        if ([bool]$nativeRuntime.Ok) {
            Write-StartLog "$Context uses the native host-scoped model runtime. Legacy model-list, Statsig, reasoning, and stabilizer maintenance are retired. Retired=$([bool]$nativeRuntime.Retired); Reason=$($nativeRuntime.Reason)"
        } else {
            Write-StartLog "$Context native model runtime is authoritative, but legacy shim retirement could not be confirmed. Compatibility shims will not be reinstalled. Reason=$($nativeRuntime.Reason)"
        }
        return
    }
...
function Get-CodexCdpLaunchAcceptanceEvidence {
    param(
    ...
    $evidence = [ordered]@{
        schema = "codex.desktop.cdp_launch_acceptance.v1"
        evaluated_at = (Get-Date).ToUniversalTime().ToString("o")
        port = $Port
        started_process_id = $StartedProcessId
        expected_launch_argument_observed = $false
        started_process_alive = $false
        started_process_elevated = "Unknown"
        listener_present = $false
        listener_owner_pids = @()
        listener_owner_matches_started_pid = $false
        listener_owner_elevated = "Unknown"
        http_ready = [bool]$HttpReady
        ready = $false
        reason = ""
    }
    if ($StartedProcessId -le 0) {
        $evidence.reason = "dry_run_not_evaluated"
        return $evidence
    }
    $started = $null
    try {
        $started = Get-CimInstance Win32_Process -Filter "ProcessId = $StartedProcessId" -ErrorAction Stop
    } catch {
        Write-StartLog "Codex CDP launch acceptance process lookup failed. Pid=$StartedProcessId; Error=$($_.Exception.Message)"
    }
    if ($null -eq $started) {
        $evidence.reason = "started_process_not_alive"
        return $evidence
    }
    $evidence.started_process_alive = $true
    $commandLine = [string]$started.CommandLine
    $evidence.expected_launch_argument_observed = -not [string]::IsNullOrWhiteSpace($ExpectedArgument) -and $commandLine.IndexOf($ExpectedArgument, [System.StringComparison]::OrdinalIgnoreCase) -ge 0
    $evidence.started_process_elevated = Get-TokenElevation -ProcessId $StartedProcessId
    $owners = @(Get-NetTCPConnection -LocalPort $Port -State Listen -ErrorAction SilentlyContinue |
        Select-Object -ExpandProperty OwningProcess -Unique | ForEach-Object { [int]$_ })
    $evidence.listener_present = $owners.Count -gt 0
    $evidence.listener_owner_pids = @($owners | Sort-Object -Unique)
    $evidence.listener_owner_matches_started_pid = $owners -contains $StartedProcessId
    if ($evidence.listener_owner_matches_started_pid) {
        $evidence.listener_owner_elevated = Get-TokenElevation -ProcessId $StartedProcessId
    }
    if (-not $evidence.expected_launch_argument_observed) {
        $evidence.reason = "cdp_launch_argument_not_observed"
    } elseif (-not $evidence.listener_present) {
        $evidence.reason = "cdp_listener_missing"
    } elseif (-not $evidence.listener_owner_matches_started_pid) {
        $evidence.reason = "cdp_listener_owner_mismatch"
    } elseif ($evidence.started_process_elevated -ne "Yes" -or $evidence.listener_owner_elevated -ne "Yes") {
        $evidence.reason = "cdp_listener_not_elevated"
    } elseif (-not $evidence.http_ready) {
        $evidence.reason = "cdp_http_unready"
    } else {
        $evidence.ready = $true
        $evidence.reason = "cdp_ready_owned_by_started_elevated_process"
    }
    return $evidence
}
...
function Stop-CodexDesktopProcessFamilyForRelaunch {
    param(
        [object[]]$Processes,
        [string]$Reason,
        [string]$SuccessAction
    )
    Write-StartLog "Stopping Codex process family before relaunch. Reason=$Reason; Processes=$(Get-CodexProcessSummary -Processes $Processes)"
    $stop = Stop-StaleCodexDesktopProcesses -Processes $Processes
    Start-Sleep -Seconds 2
    $after = @(Get-CodexDesktopProcessRecords)
    if (-not (Test-CodexProcessScanReliable)) {
        Write-StartLog "Codex process family cleanup verification failed because process census is unavailable. Reason=$Reason"
        return @{ Ok = $false; Action = "cleanup_verification_scan_failed"; ProcessCount = 0; Stopped = $stop.Stopped; Failed = $stop.Failed }
    }
    if ($after.Count -gt 0) {
        Write-StartLog "Codex process family cleanup incomplete. Reason=$Reason; Stopped=$($stop.Stopped -join ','); Failed=$($stop.Failed -join '; '); Remaining=$(Get-CodexProcessSummary -Processes $after)"
        return @{ Ok = $false; Action = "cleanup_incomplete"; ProcessCount = $after.Count; Stopped = $stop.Stopped; Failed = $stop.Failed }
    }
    Write-StartLog "Codex process family cleaned before relaunch. Reason=$Reason; Stopped=$($stop.Stopped -join ',')"
    return @{ Ok = $true; Action = $SuccessAction; ProcessCount = 0; Stopped = $stop.Stopped; Failed = $stop.Failed }
}
...
function Ensure-CodexRestartBoundaryClean {
    ...
    return Stop-CodexDesktopProcessFamilyForRelaunch `
        -Processes $remaining `
        -Reason "stale_restart_boundary" `
        -SuccessAction "stale_processes_stopped"
}
...
Invoke-CodexDesktopRequestedRestart
...
if (-not [bool]$configPreflight.Ok) {
    if (Test-CodexConfigPreflightShouldBlock -Result $configPreflight) {
...
$protocolCompatibility = Invoke-CodexDesktopProtocolCompatibilityPreflight
Write-PhaseLog "Desktop protocol compatibility phase finished"
if (-not [bool]$protocolCompatibility.Ok) {
    if ($env:CODEX_STARTUP_PROTOCOL_FAIL_CLOSED -eq "1") {
        Write-StartLog "Codex Desktop startup blocked because protocol compatibility validation failed and CODEX_STARTUP_PROTOCOL_FAIL_CLOSED=1. Status=$($protocolCompatibility.Status)"
        exit 7
    }
    Write-StartLog "Codex Desktop startup continuing in degraded mode because protocol compatibility validation failed. Status=$($protocolCompatibility.Status). The signed package was not modified; update Codex Desktop before relying on deprecated thread mutation flows."
}
...
if ($existingCdpReady) {
    ...
    $allowAuthorizedTakeover = ($env:CODEX_ALLOW_STALE_CODEX_CLEANUP -eq "1") -and ($env:CODEX_DISABLE_STALE_CODEX_CLEANUP -ne "1") -and (-not $dryRun)
    if (-not $allowAuthorizedTakeover) {
    ...
    $takeoverProcesses = @(Get-CodexDesktopProcessRecords)
    if (-not (Test-CodexProcessScanReliable)) {
        Write-StartLog "Authorized non-elevated Codex takeover blocked because process census is unavailable. RemoteDebuggingPort=$remoteDebuggingPort"
        exit 8
    }
    Write-StartLog "Explicitly authorized manual elevated takeover of non-elevated Codex Desktop. RemoteDebuggingPort=$remoteDebuggingPort; Processes=$(Get-CodexProcessSummary -Processes $takeoverProcesses)"
    $takeoverCleanup = Stop-CodexDesktopProcessFamilyForRelaunch `
        -Processes $takeoverProcesses `
        -Reason "authorized_non_elevated_cdp_takeover" `
        -SuccessAction "non_elevated_processes_stopped"
    if (-not [bool]$takeoverCleanup.Ok) {
        Write-StartLog "Codex Desktop elevated startup blocked because authorized non-elevated takeover did not complete. Action=$($takeoverCleanup.Action); ProcessCount=$($takeoverCleanup.ProcessCount); RemoteDebuggingPort=$remoteDebuggingPort"
        exit 4
    }
    $beforePort = Get-PortOwnerSummary -Port $remoteDebuggingPort -Deep $deepDiag
    if ($beforePort -ne "free") {
        Write-StartLog "Codex Desktop elevated startup blocked because the CDP port is still occupied after authorized non-elevated takeover. RemoteDebuggingPort=$remoteDebuggingPort; PortAfterTakeover=$beforePort"
        exit 3
    }
    Write-StartLog "Authorized non-elevated Codex takeover completed; continuing with fresh elevated launch. RemoteDebuggingPort=$remoteDebuggingPort"
}
if ($beforePort -ne "free") {
...
if (-not [bool]$finalConfigPreflight.Ok) {
    if (Test-CodexConfigPreflightShouldBlock -Result $finalConfigPreflight) {
...
$finalWslEnabled = [bool](Get-ObjectPropertyValue -Object $finalConfigPreflight -Name "WslEnabled" -Default $false)
$finalWslRuntimeReady = [bool](Get-ObjectPropertyValue -Object $finalConfigPreflight -Name "WslRuntimeReady" -Default (-not $finalWslEnabled))
$finalWslProjectionStatus = [string](Get-ObjectPropertyValue -Object $finalConfigPreflight -Name "WslProjectionStatus" -Default "not_reported")
if ($finalWslEnabled -and -not $finalWslRuntimeReady) {
    Write-StartLog "WSL runtime projection is not ready after the clean restart boundary. Status=$finalWslProjectionStatus. Continuing with the native compatibility launch to preserve Codex availability; WSL mode can be retried on the next governed restart."
}
...
if ($dryRun) {
    ...
    $desktopLaunchEnvironment.desktop_pid = [int]$startedProcess.Id
    $desktopLaunchEnvironment.generated_at = (Get-Date).ToUniversalTime().ToString("o")
    $desktopLaunchEnvironmentReceipt = Write-CodexDesktopLaunchEnvironmentReceipt -Receipt $desktopLaunchEnvironment
}
...
$cdpAcceptance = Get-CodexCdpLaunchAcceptanceEvidence `
    -StartedProcessId $(if ($null -ne $startedProcess) { [int]$startedProcess.Id } else { 0 }) `
    -Port $remoteDebuggingPort `
    -ExpectedArgument "--remote-debugging-port=$remoteDebuggingPort" `
    -HttpReady $newVersionReady
$desktopLaunchEnvironment.cdp_acceptance = $cdpAcceptance
$desktopLaunchEnvironment.generated_at = (Get-Date).ToUniversalTime().ToString("o")
$desktopLaunchEnvironmentReceipt = Write-CodexDesktopLaunchEnvironmentReceipt -Receipt $desktopLaunchEnvironment
$newCdpAccepted = [bool]$cdpAcceptance.ready
Write-StartLog "Start requested. StartedPid=$startedPid; StartedProcessElevation=$startedElevation; RemoteDebuggingPort=$remoteDebuggingPort; PortAfter=$afterPort; VersionReady=$newVersionReady; CdpAcceptance=$($cdpAcceptance.reason); CdpAccepted=$newCdpAccepted; VersionWaitSeconds=$newProcessWaitSeconds"
if ($newCdpAccepted) {
    Invoke-CodexDesktopModelRuntimeReconcile -ReloadImmediately $true -Context "Codex Desktop startup"
} else {
    Write-StartLog "Codex Desktop Statsig allowlist sync skipped because CDP launch acceptance did not pass. Reason=$($cdpAcceptance.reason)"
    Write-PhaseLog "Desktop Statsig allowlist sync skip phase finished"
}
if ($null -ne $script:CodexDesktopRestartRequest) {
    $afterRestartProcesses = @(Get-CodexDesktopProcessRecords)
    $afterRestartPids = @($afterRestartProcesses | ForEach-Object { [int]$_.ProcessId })
    Write-CodexDesktopRestartReceipt `
        -Ok $newCdpAccepted `
        -Status $(if ($newCdpAccepted) { "completed" } else { "failed" }) `
        -Reason $(if ($newCdpAccepted) { "governed_desktop_relaunch_ready" } else { "governed_desktop_relaunch_cdp_acceptance_failed_$($cdpAcceptance.reason)" }) `
        -AfterPids $afterRestartPids `
        -CdpReady $newCdpAccepted
}
Start-CodexStartupBaselineRepairAsync
Write-PhaseLog "Baseline repair phase finished"
