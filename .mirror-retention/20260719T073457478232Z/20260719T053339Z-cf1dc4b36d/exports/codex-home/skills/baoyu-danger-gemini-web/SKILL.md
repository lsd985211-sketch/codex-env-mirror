...
- Confirm the reverse-engineered Gemini Web path is actually needed.
- Verify consent before first use.
- Check the profile/cookie path before blaming generation errors.

## Output Contract

- Return the model, session, and result path or text.
- Mention consent state and whether the path was text or image generation.
- If the route failed, say whether it was auth, profile, or request shape.
