...
"""
DAEMON_VULNERABLE = """        async def dispatch(self, request, call_next):
...
"""
DAEMON_FIXED = """        async def dispatch(self, request, call_next):
            if request.method == \"OPTIONS\" or request.url.path in (
                \"/internal/health\", \"/healthz\", \"/\",
            ):
                return await call_next(request)
...
def package_metadata(pmb_python: Path) -> dict[str, Any]:
    code = (
        "import inspect,json,pmb,pmb.core.search,pmb.core.workspace,pmb.mcp.daemon;"
        ...
        "'daemon_path':inspect.getsourcefile(pmb.mcp.daemon)}))"
    )
    result = _run_json([str(pmb_python), "-c", code], timeout=60)
    payload = result.get("payload") if isinstance(result.get("payload"), dict) else {}
    reported_paths = {key: str(payload.get(key) or "") for key in ("search_path", "workspace_path", "daemon_path")}
    local_paths = {
        key: str(local_pmb_memory_platform.local_accessible_path(value)) if value else ""
        for key, value in reported_paths.items()
    }
    return {
        "ok": (
            bool(result.get("ok"))
            and bool(payload.get("search_path"))
            and bool(payload.get("workspace_path"))
            and bool(payload.get("daemon_path"))
        ),
...
def package_patch_state(metadata: dict[str, Any]) -> dict[str, Any]:
    ...
    daemon_path = Path(str(metadata.get("daemon_path") or ""))
    if not search_path.is_file() or not workspace_path.is_file() or not daemon_path.is_file():
        return {
            "ok": False,
            "reason": "package_source_missing",
    ...
    daemon_fixed = DAEMON_FIXED in daemon_text
    return {
        "ok": search_fixed and workspace_fixed and daemon_fixed,
        ...
        "daemon": {"path": str(daemon_path), "sha256": sha256_path(daemon_path), "fixed": daemon_fixed, "vulnerable_signature": DAEMON_VULNERABLE in daemon_text},
    }
...
def apply_package_fixes(pmb_python: Path, *, apply: bool) -> dict[str, Any]:
    ...
    for key, vulnerable, fixed in (
        ("search", SEARCH_VULNERABLE, SEARCH_FIXED),
        ("workspace", WORKSPACE_VULNERABLE, WORKSPACE_FIXED),
        ("daemon", DAEMON_VULNERABLE, DAEMON_FIXED),
    ):
...
try:
    count = int(conn.execute('SELECT COUNT(*) FROM events WHERE archived_at IS NULL').fetchone()[0])
    unique_ulids = int(conn.execute('SELECT COUNT(DISTINCT ulid) FROM events WHERE archived_at IS NULL').fetchone()[0])
finally:
    conn.close()
print(json.dumps({'count': count, 'unique_ulids': unique_ulids, 'ok': count == unique_ulids}))
"""
    event_argument = local_pmb_memory_platform.process_argument_path(db_path, pmb_python)
    event_result = _run_json([str(pmb_python), "-c", event_code, str(event_argument)], timeout=60)
    event_payload = event_result.get("payload") if isinstance(event_result.get("payload"), dict) else {}
    state["events"] = {
        **event_payload,
        "ok": bool(event_result.get("ok")) and bool(event_payload.get("ok")),
        "error": event_result.get("stderr_preview") or event_result.get("error"),
    }
    code = """
...
"""
    bm25_argument = local_pmb_memory_platform.process_argument_path(bm25_path, pmb_python)
    bm25_result = _run_json([str(pmb_python), "-c", code, str(bm25_argument)], timeout=60)
...
"""
    vector_argument = local_pmb_memory_platform.process_argument_path(vector_path, pmb_python)
    result = _run_json([str(pmb_python), "-c", code, str(vector_argument)], timeout=240)
    payload = result.get("payload") if isinstance(result.get("payload"), dict) else {}
    return {
...
"""
    env = local_pmb_memory_platform.process_environment(
        ...
        pmb_home=pmb_home,
        workspace="",
        executable=pmb_python,
    )
    result = _run_json([str(pmb_python), "-c", code], env=env, timeout=60)
    payload = result.get("payload") if isinstance(result.get("payload"), dict) else {}
    rows = payload.get("rows") if isinstance(payload.get("rows"), list) else []
    resolved = {
        str(local_pmb_memory_platform.local_accessible_path(str(row.get("pmb_home") or "")).resolve()).casefold()
        for row in rows
