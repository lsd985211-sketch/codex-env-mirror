...
MUTATING_ACTIONS = {"repair-plan", "apply", "rollback", "transition", "dispose", "resolve", "bootstrap"}
...
def source_signature() -> str:
    ...
    for path in sorted(paths, key=lambda value: value.as_posix()):
        try:
            stat = path.stat()
            record = [path.relative_to(MAP_PATH.parent).as_posix(), stat.st_size, stat.st_mtime_ns]
...
def contract_stats() -> dict[str, Any]:
    ...
    declared = sum(
        int(item.get("contract_count") or 0)
        for item in load_surface_index().get("systems", [])
...
def build_index(*, apply: bool) -> dict[str, Any]:
    ...
    try:
        ...
        connection.executemany(
            ...
            (
                ("schema", "maintenance_capability_registry.v2"),
...
def _cached_source_rows(system: str, path_text: str, source_mtime_ns: int) -> tuple[dict[str, Any], ...]:
    del source_mtime_ns
    path = Path(path_text)
    return tuple(parse_surface_map(((system, path),)))
...
def _source_rows(system: str) -> tuple[dict[str, Any], ...]:
    ...
    if path is None:
        return ()
    return _cached_source_rows(system, str(path), path.stat().st_mtime_ns)
...
def index_fresh() -> bool:
    ...
    try:
        ...
        if "connection" in locals():
            connection.close()
    return bool(value and value[0] == source_signature())
...
def doctor() -> dict[str, Any]:
    ...
    if metric["contract_count"] != metric["declared_contract_count"]:
        issues.append("maintenance surface contract count differs from the navigation manifest")
