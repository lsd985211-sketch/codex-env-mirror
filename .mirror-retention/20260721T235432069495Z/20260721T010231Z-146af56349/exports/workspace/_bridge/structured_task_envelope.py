...
RESOURCE_OPERATIONS = {
    "discover", "search", "inspect", "fetch", "read", "download", "materialize",
    "repository_search", "repository_read", "repository_metadata", "code_search", "issue_search",
    "pull_request_search", "readme_read", "tree_read", "file_read", "release_read",
}
...
def _json_mapping(value: Any) -> dict[str, Any]:
    ...
    for key, item in _mapping(value).items():
        if isinstance(item, (str, bool, int, float)) or item is None:
            ...
            output[str(key)] = [child for child in item if isinstance(child, (str, bool, int, float)) or child is None]
    return output


def _execution_phases(value: Any) -> list[dict[str, Any]]:
    phases: list[dict[str, Any]] = []
    for index, raw in enumerate(value if isinstance(value, list | tuple) else []):
        phase = _mapping(raw)
        operation = str(phase.get("operation") or "").strip().lower()
        if not operation:
            continue
        phases.append(
            {
                "id": str(phase.get("id") or f"phase-{index + 1}").strip(),
                "operation": operation,
                "required": _bool(phase.get("required"), True),
                "depends_on": _list(phase.get("depends_on")),
                "limit": _int(phase.get("limit")),
                "selectors": _json_mapping(phase.get("selectors")),
                "deliverables": _list(phase.get("deliverables")),
                "acceptance": _json_mapping(phase.get("acceptance")),
            }
        )
    return phases
...
def normalize_resource_envelope(payload: dict[str, Any], *, strict: bool = True) -> dict[str, Any]:
    ...
    execution = _mapping(resource.get("execution"))
    execution_limits = _mapping(execution.get("limits"))
    execution_output = _mapping(execution.get("output"))
    execution_acceptance = _mapping(execution.get("acceptance"))
    operations = [item.lower() for item in _list(execution.get("operations"))]
    phases = _execution_phases(execution.get("phases"))
    normalized = {
        ...
        "resource": {
            ...
            "execution": {
                ...
                "selectors": _json_mapping(execution.get("selectors")),
                "deliverables": _list(execution.get("deliverables")),
                "phases": phases,
                "limits": {
                    "candidate_count": _int(execution_limits.get("candidate_count")),
                    "repository_count": _int(execution_limits.get("repository_count")),
                    "item_count": _int(execution_limits.get("item_count")),
                    "content_chars": _int(execution_limits.get("content_chars")),
                },
                "output": {
                    "mode": str(execution_output.get("mode") or "").strip(),
                    "evidence_depth": str(execution_output.get("evidence_depth") or "").strip(),
                },
                "acceptance": {
                    "required_deliverables": _list(execution_acceptance.get("required_deliverables")),
                    "required_phases": _list(execution_acceptance.get("required_phases")),
                    "minimum_completed_phases": _int(execution_acceptance.get("minimum_completed_phases")),
                    "allow_partial": _bool(execution_acceptance.get("allow_partial")),
                },
            },
            "transfer": {
    ...
    unsupported_operations = [item for item in operations if item not in RESOURCE_OPERATIONS]
    unsupported_operations.extend(
        str(phase.get("operation") or "") for phase in phases if phase.get("operation") not in RESOURCE_OPERATIONS
    )
    if unsupported_operations:
        errors.append("unsupported_resource_operation:" + ",".join(dict.fromkeys(unsupported_operations)))
