...
When a terminal transport has previously lost long-command output, submit the
same governed operation instead of launching a second publish: `python
_bridge/codex_workflow_entry.py mirror submit --operation publish --task-id
<bounded-id> --confirm PUBLISH-CODEX-MIRROR`. It returns a durable receipt
immediately. Consume the real terminal outcome through `python
_bridge/shared/long_command_receipt.py status --task-id <bounded-id>
--wait-seconds 30`; submission preserves confirmation, source gates, and the
one-at-a-time mirror lock.
