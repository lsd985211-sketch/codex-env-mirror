...
```text
projectPath = C:\Users\45543\Downloads\mcsmanager_windows_release\mcsmanager
```
...
If both Hub and native CodeGraph are unavailable or insufficient, use the project-local CLI:

```powershell
$cg = "C:\Users\45543\Downloads\mcsmanager_windows_release\mcsmanager\_bridge\tools\codegraph\node_modules\.bin\codegraph.cmd"
```
...
Current expected healthy baseline for mcsmanager:
...
- journal mode is `wal`
- unresolved refs for the mcsmanager index should remain near zero
...
`doctor` may still report Reasonix-side drift for the old 3c3u index. Do not treat that as a blocker for mcsmanager CodeGraph use.
