...
- For domain work, first recall the relevant memory and load the relevant skill,
  ...
  validated action.
- For tool-heavy work, query `_bridge/mcp_capability_routes.py lookup` first,
  then expand only the selected section of `_bridge/docs/mcp_capability_matrix.md`
  when the route projection lacks required detail: memory/skill routing first,
  slash template second, owning MCP third, bounded fallback fourth, closeout
  last. Keep `current_turn_callable` evidence separate from config or protocol
  evidence. The lookup is navigation, not a reduced replacement for the matrix.
...
- Resolve one owner command contract per task and command family. Consume
  machine-readable plan receipts, registries, and schemas before help text; a
  plan that already carries confirmation tokens and adjacent apply, commit, or
  integrate arguments is the reusable contract for those steps. Re-expand only
  when an argument is rejected, the owner/version/input signature changes, the
  contract is incomplete, or execution switches owners. Keep targeted help as
  a last-resort expansion path so reuse does not reduce capability.
