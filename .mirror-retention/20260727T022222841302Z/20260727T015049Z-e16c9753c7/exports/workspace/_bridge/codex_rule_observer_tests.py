...
class CodexRuleObserverTests(unittest.TestCase):
    ...
    def test_parallel_events_are_isolated_files(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(len(read_recent_events(runtime_root=root)), 24)

    def test_validate_uses_selected_codex_home_hooks_without_writing_fixture(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            ...
            hooks.write_text(
                json.dumps(
                    {
                        "hooks": {
                            event: [{"hooks": [{"type": "command", "command": f"python codex_rule_observer.py hook {event}"}]}]
            ...
            result = validate(hooks_path=hooks)
