...
class CodexEnvironmentMirrorTests(unittest.TestCase):
    def test_finalize_and_publish_reuses_matching_terminal_receipt(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir, \
                ...
                patch.object(mirror, "work_git_release_gate", return_value=source), \
    ...
    def test_finalize_and_publish_rejects_incomplete_remote_acceptance(self) -> None:
        source = {"ok": True, "work_git": {"worktree_head": "a", "bare_head": "a"}}
        result = {"ok": True, "readiness": {"mirror_valid": True, "capability_restore_ready": True}, "source_freshness": {"ok": True}, "push": {}}
        with tempfile.TemporaryDirectory() as temp_dir, \
                patch.dict(os.environ, {"CODEX_ENV_MIRROR_RUNTIME_ROOT": temp_dir}), \
                patch.object(mirror, "work_git_release_gate", return_value=source), \
    ...
    def test_publish_auto_changed_paths_uses_directed_refresh_when_plan_is_safe(self) -> None:
        ...
        reconcile.assert_called_once_with(mirror.DRIFT_REVIEW_CONFIRMATION, changed)
    ...
    def test_health_accepts_only_owner_proven_publication_pending_state(self) -> None:
        state = {
        ...
        refresh_scope = {
            ...
            "changed_paths": ["workspace/_bridge/owner.py"],
            "affected_source_plan": {
                "ok": True,
                "full_rebuild_required": False,
                "affected_source_ids": ["workspace-bridge-source"],
        ...
        ), patch.object(mirror, "publish_refresh_scope", return_value=refresh_scope):
    ...
    def test_health_rejects_mixed_or_out_of_closure_drift(self) -> None:
        state = {
            "ok": False,
            "latest_snapshot_id": "snapshot-1",
            "git": {"initialized": True, "clean": True},
    ...
    def test_health_accepts_owner_proven_full_rebuild(self) -> None:
        state = {
            "ok": False,
            "latest_snapshot_id": "snapshot-1",
            "git": {"initialized": True, "clean": True},
