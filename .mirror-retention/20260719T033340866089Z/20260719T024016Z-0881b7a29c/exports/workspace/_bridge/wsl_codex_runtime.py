...
def _plugin_config_tables() -> tuple[str, str]:
    ...
    for marketplace in marketplaces:
        relative = Path("bundled-marketplaces") / "openai-bundled" if marketplace == "openai-bundled" else Path("plugins")
...
def project_plugins(*, write: bool) -> dict[str, object]:
    ...
    if write:
        for marketplace in {row["marketplace"] for row in rows}:
            relative = Path("bundled-marketplaces") / "openai-bundled" if marketplace == "openai-bundled" else Path("plugins")
            source_root = WINDOWS_CODEX_HOME / ".tmp" / relative
            filename = "api_marketplace.json" if marketplace == "openai-api-curated" else "marketplace.json"
            source = source_root / ".agents" / "plugins" / filename
            target = CODEX_HOME / ".tmp" / source_root.relative_to(WINDOWS_CODEX_HOME / ".tmp") / ".agents" / "plugins" / filename
