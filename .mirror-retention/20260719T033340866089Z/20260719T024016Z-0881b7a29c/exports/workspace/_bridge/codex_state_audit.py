#!/usr/bin/env python3
"""Read-only Codex startup baseline audit for this Windows workspace."""
...
def run_codex_mcp_list(config: dict | None = None) -> tuple[bool, str]:
...
def build_checks(run_cli: bool = True) -> list[Check]:
    ...
    global_config_path = Path(baseline["global_config"])
    project_config_path = Path(baseline["project_config"])
    ...
    try:
        ...
        configured_cli_exists = bool(configured_cli and Path(configured_cli).exists())
        checks.append(
            Check(
        ...
        current_ok = bool(
            configured_cli
            and latest_cli
            and normalize_path_text(configured_cli) == normalize_path_text(latest_cli)
        )
        checks.append(
            Check(
                "codex_runtime_cli_path_current",
                current_ok,
                f"configured={configured_cli!r} latest={latest_cli!r}",
            )
        )
        checks.append(
            Check(
    ...
    if run_cli:
        ...
        ok, output = run_codex_mcp_list(cli_config)
