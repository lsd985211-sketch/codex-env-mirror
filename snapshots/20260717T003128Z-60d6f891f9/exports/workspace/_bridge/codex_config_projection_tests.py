...
def run() -> None:
    ...
    with tempfile.TemporaryDirectory() as raw:
        ...
        normalized, normalized_changed = projection.codex_state_repair.ensure_project_trusted(
            quoted,
            r"projects.c:\users\example.trust_level",
            "trusted",
        )
        assert not normalized_changed
        assert normalized.count("[projects.") == 1

        duplicate = quoted + "\n[projects.'c:\\users\\example']\ntrust_level = \"trusted\"\n"
        normalized, normalized_changed = projection.codex_state_repair.normalize_duplicate_project_tables(duplicate)
        assert normalized_changed and normalized.count("[projects.") == 1
        assert projection.tomllib.loads(normalized)["projects"][r"c:\users\example"]["trust_level"] == "trusted"
        ...
        runtime_local_common = (
            '[projects."c:\\\\users\\\\example"]\ntrust_level = "trusted"\n'
            '[mcp_servers.context7]\ncommand = "context7"\n'
            '[desktop]\nshow-context-window-usage = true\n'
        )
        config.write_text(runtime_local_live, encoding="utf-8")
        projection.managed_projection_path(config).unlink(missing_ok=True)
        connection = sqlite3.connect(database)
        try:
            connection.execute(
                "UPDATE settings SET value = ? WHERE key = ?",
                (runtime_local_common, projection.COMMON_KEY),
            )
            connection.commit()
        finally:
            connection.close()
        isolated = projection.apply_projection(config, database, sync_desktop=False, backup=False)
        assert isolated["ok"], isolated
        assert "[projects." not in read_common(database)
        assert "mcp_servers.context7" not in read_common(database)
        managed_payload = json.loads(projection.managed_projection_path(config).read_text(encoding="utf-8"))
        assert "projects" not in managed_payload["values"]
        assert "context7" not in managed_payload["values"].get("mcp_servers", {})

    print(json.dumps({"ok": True, "tests": 19}, ensure_ascii=False))
