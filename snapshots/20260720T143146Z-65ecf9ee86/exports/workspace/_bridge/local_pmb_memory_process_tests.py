...
class PmbProcessRuntimeTests(unittest.TestCase):
    ...
    def test_owner_uses_windows_runtime_with_explicit_wslenv_identity(self) -> None:
        env = owner.pmb_env()

        self.assertTrue(owner.PMB_EXE.is_file())
        self.assertTrue(str(owner.PMB_HOME).startswith("/mnt/c/"))
        self.assertTrue(str(env["PMB_HOME"]).startswith("C:\\"))
        self.assertEqual("mcsmanager", env["PMB_WORKSPACE"])
        self.assertIn("PMB_HOME", env["WSLENV"].split(":"))
        self.assertIn("PMB_WORKSPACE", env["WSLENV"].split(":"))
