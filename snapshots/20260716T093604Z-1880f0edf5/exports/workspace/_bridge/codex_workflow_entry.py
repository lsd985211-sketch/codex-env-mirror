...
from workflow_closeout_signals import optional_closeout_sections
...
def select_self_update_owners(
    *,
    ...
    from self_update_governance import OWNER_NAMES, select_owners_for_change
...
def closeout_cli_projection(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    summary = {
        ...
        "finalization": {
            ...
            "startup_baseline": finalization.get("startup_baseline"),
            "project_checkpoint": finalization.get("project_checkpoint"),
            "membership_reconciliation": reconciliation_summary(membership),
            "rule_reconciliation": reconciliation_summary(rule),
            "post_closeout_mirror": finalization.get("post_closeout_mirror"),
        },
        "pending_disposition": payload.get("pending_disposition"),
        "final_reply_must_show": payload.get("final_reply_must_show"),
        "detail_rule": "use --full-output only when the complete closeout package is required",
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    c.add_argument("--full-output", action="store_true", help="Print the complete closeout package instead of the bounded default summary.")
    ...
    output = closeout_cli_projection(payload) if args.command == "closeout" and not args.full_output else payload
    print_json(output)
