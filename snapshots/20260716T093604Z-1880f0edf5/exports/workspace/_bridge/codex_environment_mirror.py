...
def refresh(confirm: str, changed_paths: list[str] | None = None) -> dict[str, Any]:
    ...
    if previous_snapshot_id and previous_snapshot_id == committed_snapshot_id:
...
def execute(action: str, *, target_root: str = "", confirm: str = "", changed_paths: list[str] | None = None, left_snapshot: str = "", right_snapshot: str = "") -> dict[str, Any]:
    if action == "status":
        return status()
    if action == "doctor":
        return doctor()
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.action == "affected-source-plan":
        ...
        payload = execute(args.action, target_root=getattr(args, "target_root", ""), confirm=getattr(args, "confirm", ""), changed_paths=getattr(args, "changed", []), left_snapshot=getattr(args, "left", ""), right_snapshot=getattr(args, "right", ""))
    print_json(payload)
