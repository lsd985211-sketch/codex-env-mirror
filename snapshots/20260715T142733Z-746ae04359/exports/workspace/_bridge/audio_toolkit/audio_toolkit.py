...
DEFAULT_WORK_ROOT = PROJECT_ROOT / ".tools" / "audio-work"
...
def preferred_audio_python() -> Path | None:
    candidates = [
        PROJECT_ROOT / ".tools" / "whisper-venv" / "Scripts" / "python.exe",
        PROJECT_ROOT / ".tools" / "whisper-venv" / "bin" / "python",
    ]
    for candidate in candidates:
        if candidate.is_file():
...
def demucs_environment() -> dict[str, str]:
    env = os.environ.copy()
    env.setdefault("PYTHONIOENCODING", "utf-8")
    env["TORCH_HOME"] = str(PROJECT_ROOT / ".tools" / "models" / "torch")
...
def demucs_vocals(
    ...
    repo = PROJECT_ROOT / ".tools" / "models" / "demucs-local-repo"
