...
def mirror_refresh_required(changed_files: list[str]) -> bool:
    ...
    mirror = normalized_change_path(str(Path.home() / "codex-env-mirror"))
    for value in changed_files:
        path = normalized_change_path(value)
        if not path:
            continue
        if path == "agents.md" or path.startswith("_bridge/"):
            return True
        if path.startswith(workspace + "/_bridge/") or path == workspace + "/agents.md":
            return True
        if path == mirror or path.startswith(mirror + "/"):
            return True
        if path.startswith(home + "/.codex/") or path.startswith(home + "/.agents/skills/"):
            return True
        if path == home + "/.cc-switch/settings.json" or path.startswith(home + "/.cc-switch/skills/"):
            return True
    return False
