...
name: firecrawl-cli
description: |
  Search, scrape, and interact with the web via the Firecrawl CLI. Use this
  skill when the task is to execute generic web acquisition: scrape a known
  URL, fetch page content, search the web, map a site, crawl documentation,
  download a site, parse a local file, monitor a page, or interact with pages
  that need clicks or logins. Trigger on requests like "fetch this page",
  "pull the content from", "get the page at https://", or "crawl this docs
  site". This is the generic webpage execution layer, not the platform-routing
  layer. If the task first requires choosing which platform/source to use or
  combining results across platform-specific sources, route through agent-reach
  first. Do NOT trigger for local file operations unrelated to Firecrawl, git
  commands, deployments, or code editing tasks.
allowed-tools:
  - Bash(firecrawl *)
  - Bash(npx firecrawl *)
---
...
- Use this skill to execute generic webpage retrieval once the source is known or the task is clearly a normal web acquisition problem.
- Use `search`, `scrape`, `map`, `crawl`, `monitor`, and `interact` as the execution primitives.
- If the work first requires deciding which platform to query, which source mix to trust, or how to orchestrate multi-platform research, hand that decision to `agent-reach` first.
- Do not keep ownership of report writing, synthesis, or platform-specific routing after the content has been fetched.
...
0. **Route first if needed** - If the task is really about platform choice, source selection, or multi-platform collection strategy, route through `agent-reach` before using Firecrawl.
...
- **Task first needs platform routing, source choice, or multi-platform collection strategy** -> use `agent-reach` first
