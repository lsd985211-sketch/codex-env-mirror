...
- Confirm layout, style, aspect, and language before prompt generation.
- Preserve source data verbatim and do not silently summarize it away.
- If the content is too sparse for an infographic, say so early.

## Output Contract

- Return the selected layout/style combination and output path.
- State whether the source was saved and whether structured content was generated.
- Mention if the content needed splitting or had to be normalized.
