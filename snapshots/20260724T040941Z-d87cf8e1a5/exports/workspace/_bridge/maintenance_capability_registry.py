...
@lru_cache(maxsize=2)
def _cached_source_rows(source_mtime_ns: int) -> tuple[dict[str, Any], ...]:
    del source_mtime_ns
    return tuple(parse_surface_map())


def _source_rows() -> tuple[dict[str, Any], ...]:
    return _cached_source_rows(MAP_PATH.stat().st_mtime_ns)
    ...
    return bounded_payload(
        {
            ...
            "source": "maintenance_surface_map_fallback",
