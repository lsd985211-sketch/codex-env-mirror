...
class CodexDesktopEnvironmentSelectionTests(unittest.TestCase):
    def fixture(self, root: Path, *, host_value: bool | None, wsl_value: bool | None) -> tuple[Path, Path, Path]:
        host = root / "host.toml"
        wsl = root / "wsl.toml"
        state = root / "selection-state.json"
        host_text = "[desktop]\n"
        if host_value is not None:
            host_text += f"runCodexInWindowsSubsystemForLinux = {'true' if host_value else 'false'}\n"
        host.write_text(host_text + "codeFontSize = 13\n", encoding="utf-8")
        wsl_text = 'model = "test"\n'
    ...
    def test_restart_request_records_intent_without_touching_active_configs(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            host, wsl, state = self.fixture(root, host_value=False, wsl_value=False)
            ...
            self.assertTrue(result["selection_pending"])
            self.assertEqual(before_host, host.read_bytes())
            self.assertEqual(before_wsl, wsl.read_bytes())
            host_payload = json.loads(host_state.read_text(encoding="utf-8"))
            self.assertTrue(host_payload["selection_pending"])
            self.assertEqual("explicit_restart_request", host_payload["selection_source"])

    def test_pending_restart_request_is_consumed_at_reconcile(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            host, wsl, state = self.fixture(root, host_value=False, wsl_value=False)
            host_state = root / "host-state.json"
            pending = {
                ...
                "last_synced_value": False,
                "desired_value": True,
                "effective_value": False,
                "fallback_pending": False,
                "selection_pending": True,
                "selection_source": "explicit_restart_request",
            }
            state.write_text(json.dumps(pending) + "\n", encoding="utf-8")
            host_state.write_text(json.dumps(pending) + "\n", encoding="utf-8")
            ...
            self.assertTrue(result["ok"], result)
            self.assertTrue(result["selected_value"])
            self.assertEqual("pending_selection", result["selection_source"])
            self.assertFalse(result["selection_pending"])
            self.assertTrue(tomllib.loads(host.read_text(encoding="utf-8"))["desktop"][selection.ENVIRONMENT_KEY])
            self.assertTrue(tomllib.loads(wsl.read_text(encoding="utf-8"))["desktop"][selection.ENVIRONMENT_KEY])
            self.assertFalse(json.loads(host_state.read_text(encoding="utf-8"))["selection_pending"])

    def test_missing_host_key_preserves_synchronized_wsl_selection(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            host, wsl, state = self.fixture(root, host_value=None, wsl_value=True)
            host_state = root / "host-state.json"
            synchronized = {
                "schema": selection.STATE_SCHEMA,
                ...
                "selection_source": "host_changed",
            }
            state.write_text(json.dumps(synchronized) + "\n", encoding="utf-8")
            host_state.write_text(json.dumps(synchronized) + "\n", encoding="utf-8")
            ...
            self.assertTrue(result["ok"], result)
            self.assertIsNone(result["host_config_value"])
            self.assertTrue(result["selected_value"])
            self.assertEqual("host_missing_recovery", result["selection_source"])
            self.assertTrue(tomllib.loads(host.read_text(encoding="utf-8"))["desktop"][selection.ENVIRONMENT_KEY])
            self.assertTrue(tomllib.loads(wsl.read_text(encoding="utf-8"))["desktop"][selection.ENVIRONMENT_KEY])

    def test_explicit_host_windows_value_still_overrides_synchronized_wsl_selection(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            host, wsl, state = self.fixture(root, host_value=False, wsl_value=True)
            state.write_text(
                json.dumps({
                    "schema": selection.STATE_SCHEMA,
                    "last_synced_value": True,
