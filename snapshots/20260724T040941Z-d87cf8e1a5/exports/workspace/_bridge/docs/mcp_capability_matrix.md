...
- Large context payloads: use Hub `headroom.compress` for large JSON, logs,
  code/search results, and tool output when the token reduction is material;
  call `headroom.retrieve` only for omitted detail and `headroom.stats` for
  cache metrics. The 30-minute reversible cache is not long-term memory, does
  not replace PMB, and never replaces the authoritative owner receipt or
  stable artifact reference. Do not compress away decisions, gates,
  permission boundaries, failure causes, acceptance evidence, or next steps.
...
- Validate: `python _bridge\resource_fetcher_tests.py` plus one local-file
  ...
  resource layer uses
  `_bridge/runtime_dependencies/<package>/<abi>-<platform>`. These managed
  runtime trees are ignored Git state and enter recovery products only through
  the MCP bundle owner. Quick
