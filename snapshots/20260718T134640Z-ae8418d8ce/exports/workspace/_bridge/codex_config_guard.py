...
def run_once(apply: bool, startup_delay_seconds: int = 0, phase: str = "manual") -> dict[str, Any]:
    ...
    if phase == "pre-start-static":
        ...
        desktop_wsl_enabled = desktop_wsl_enabled_for_startup()
        ...
        projection_ok = bool(wsl_runtime_projection.get("ok", True)) and bool(
            wsl_resume_context_projection.get("ok", True)
        )
        projection_ready = bool(wsl_runtime_projection.get("ready", projection_ok)) and bool(
            wsl_resume_context_projection.get("ready", True)
        )
        ...
        result = {
            ...
            "phase": phase,
            "applied": bool(repair_apply and (repair.get("changed") or projection_changed)),
            "generated_at": utc_now(),
            "before": before_audit,
            "repair": repair,
            ...
            "runtime_applied": projection_changed,
            ...
            "after": after_audit,
            "needs_codex_restart": bool(repair_apply and (repair.get("needs_codex_restart") or projection_changed)),
            "policy": "static preflight performs merge-only baseline repair and always applies the WSL runtime and queued-follow-up context projections when Desktop WSL mode is enabled; both defer writes while Desktop runs and session compaction remains delegated to the stopped-process pre-launch owner",
        }
