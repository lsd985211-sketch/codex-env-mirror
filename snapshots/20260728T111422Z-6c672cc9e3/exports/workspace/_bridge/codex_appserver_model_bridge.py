...
def candidate_app_asars() -> list[Path]:
    ...
    windows_app_roots = [Path(r"C:\Program Files\WindowsApps")]
    # This fallback is meaningful only from WSL.  On Windows, pathlib turns
    # it into a protected drive-root probe (C:\\mnt\\c).
    if os.name != "nt" and Path("/mnt/c").is_dir():
        windows_app_roots.append(Path("/mnt/c/Program Files/WindowsApps"))
    for windows_apps in windows_app_roots:
        if not windows_apps.is_dir():
            continue
        fallback = sorted(
            windows_apps.glob("OpenAI.Codex_*/app/resources/app.asar"),
...
def asar_files(path: Path) -> dict[str, dict[str, Any]]:
    """Return ASAR file metadata for read-only compatibility inspectors."""
    return _asar_files(path)


def read_asar_asset(path: Path, relative_path: str) -> bytes:
    """Read one packed ASAR asset and verify its declared SHA-256 when present."""
    files = _asar_files(path)
    ...
    with path.open("rb") as handle:
        ...
        if len(header) != 16:
            raise ValueError("asar_header_short")
        _, header_size, _, _ = struct.unpack("<4I", header)
        handle.seek(8 + header_size + offset)
        payload = handle.read(size)
    if len(payload) != size:
        raise ValueError("asar_asset_short")
    integrity = metadata.get("integrity")
    expected_hash = str(integrity.get("hash") or "") if isinstance(integrity, dict) else ""
    if expected_hash and hashlib.sha256(payload).hexdigest() != expected_hash:
        raise ValueError("asar_asset_integrity_mismatch")
    return payload
