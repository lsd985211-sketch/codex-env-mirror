...
def _thread_fact(value: Any) -> tuple[dict[str, Any] | None, dict[str, str] | None]:
    """Accept only a current structured Codex App thread-state projection."""
...
def concurrent_closeout_handoff(
    *,
    ...
    """Build a handoff contract from live, eligible Codex App thread facts."""
    active, ineligible = _eligible_active_thread_ids(active_workspace_threads)
    ...
    return {
        ...
        "fact_source_rule": "eligible targets require current structured Codex App state with status=active, sameRepository=true, and currentTask=false; message receipts never replace that evidence",
    }
