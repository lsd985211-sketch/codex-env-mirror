...
from cli_anything.microsoft_office.core import documents, preview  # noqa: E402
from cli_anything.microsoft_office.utils.backend import OfficeBackendError  # noqa: E402
from cli_anything.microsoft_office.utils.repl_skin import ReplSkin  # noqa: E402
...
@cli.command("command-contract")
def command_contract() -> None:
    payload = {
        "schema": "microsoft_office.command_contract.v1",
        "ok": True,
        "actions": ["status"],
        "read_only_actions": ["status"],
        "action_commands": {"status": ["--json", "system", "status"]},
        "maintenance": {
            "system_id": "office",
            "signals": ["office.installation_changed", "office.com_registration_changed"],
            "freshness_ttl_seconds": 3600,
            "estimated_cost_ms": 3000,
            "independent_group": "office.read_only",
            "automation_level": "A0",
            "automatic_actions": ["status"],
    ...
    signature_input = json.dumps(payload, ensure_ascii=False, sort_keys=True, separators=(",", ":"))
    payload["parser_signature"] = hashlib.sha256(signature_input.encode("utf-8")).hexdigest()
    click.echo(json.dumps(payload, ensure_ascii=False, indent=2))
