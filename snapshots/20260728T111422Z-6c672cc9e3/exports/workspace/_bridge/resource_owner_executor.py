...
def execute_generic_search(request: dict[str, Any], gateway_plan: dict[str, Any], timeout: int) -> dict[str, Any]:
    ...
    for domain, search_query in search_lanes:
        phase_timeout = budget.timeout_seconds(cap=30)
        if phase_timeout <= 0:
            domain_outcomes.append({"domain": domain, "status": "budget_exhausted", "result_count": 0})
            break
        arguments = {
            "query": search_query,
    ...
    status = "handoff_required" if handoff else ("no_results" if no_results else "failed")
    return _normalized_json_result(
        ok=False,
        status=status,
        source="generic_search",
        error_class="owner_no_results" if no_results else "generic_search_failed",
        reason="generic search returned no usable results across the requested domain lanes",
        metadata={
            "query": query,
            "requested_domains": domains,
            "searched_domains": bounded_domains,
        ...
        next_action=(
            ...
            else ("refresh_network_route_and_retry" if status == "failed" else "continue_resource_layer_handoff_or_attach_result")
        ),
    )
