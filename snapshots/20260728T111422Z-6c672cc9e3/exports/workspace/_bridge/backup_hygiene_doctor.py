...
from platform_paths import host_accessible_path, resource_library_root

try:
    from .shared.backup_router import validate_manifest as validate_routed_backup_manifest
except ImportError:
    from shared.backup_router import validate_manifest as validate_routed_backup_manifest
...
CONTROL_MANIFEST_DEPTHS = (1, 2, 3, 4, 5)
...
def _control_manifest_payload_paths(path: Path) -> list[Path]:
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
    except Exception:
        return []
    if not str(payload.get("schema") or "").startswith("backup_router."):
        return []
    items = payload.get("items") if isinstance(payload.get("items"), list) else []
    set_dirs = {
        host_accessible_path(str(item.get("backup_set_dir") or ""), platform_name=os.name).resolve()
        for item in items
        if isinstance(item, dict) and item.get("backup_set_dir")
    }
    if path.parent.resolve() not in set_dirs:
        return []
    return [
        host_accessible_path(str(item.get("backup_path") or ""), platform_name=os.name).resolve()
        for item in items
        if isinstance(item, dict)
        and item.get("backup_mode") != "git_head_reference"
        and item.get("backup_path")
    ]


def _is_payload_manifest(path: Path, payload_roots: dict[Path, set[Path]]) -> bool:
    resolved = path.resolve()
    for ancestor in (resolved, *resolved.parents):
        control_manifests = payload_roots.get(ancestor, set())
        if any(control_manifest != resolved for control_manifest in control_manifests):
            return True
    return False


def bounded_manifest_candidates(root: Path) -> list[Path]:
    paths: list[Path] = []
    for depth in CONTROL_MANIFEST_DEPTHS:
        paths.extend(root.glob("/".join(["*"] * depth + ["manifest.json"])))
    if root == WORK_GIT_BACKUP_ROOT:
        paths.extend(root.glob("migrated/legacy-bridge-backups/manual/*/*/*/manifest.json"))
    elif root == GIT_REPOSITORY_BACKUP_ROOT:
        paths.extend(root.glob("*/migrated/backup/*/*/*/manifest.json"))
...
def manifest_paths() -> list[Path]:
    ...
    for root in PLANNED_BACKUP_ROOTS:
        if root.exists():
            paths.extend(bounded_manifest_candidates(root))
    candidates = sorted(set(paths), key=str)
    payload_roots: dict[Path, set[Path]] = defaultdict(set)
    for path in candidates:
        control_manifest = path.resolve()
        for payload_path in _control_manifest_payload_paths(path):
            payload_roots[payload_path].add(control_manifest)
    return [path for path in candidates if not _is_payload_manifest(path, payload_roots)]


def validate_manifest(path: Path) -> dict[str, Any]:
    return validate_routed_backup_manifest(path)
