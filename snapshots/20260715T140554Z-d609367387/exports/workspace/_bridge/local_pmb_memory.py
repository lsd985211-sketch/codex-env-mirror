...
def retired_member_archive_root() -> Path:
    roots = {
...
LEGACY_MEMORY_SOURCES = {
    ...
    "chroma_memory": RETIRED_MEMORY_ARCHIVE_ROOT / "data" / "_bridge" / "chroma_memory" / "chroma.sqlite3",
}
...
def build_candidates(include_sensitive: bool = False) -> list[Candidate]:
    ...
    for source in [
        ...
        candidates.extend(chroma_candidates(LEGACY_MEMORY_SOURCES[source], source))
...
def pmb_compat_apply(*, apply: bool) -> dict[str, Any]:
    ...
    source_paths = [
        ...
        for key in ("search_path", "workspace_path")
        if metadata.get(key)
    ]
    if not metadata.get("ok") or len(source_paths) != 2:
        return {
            "schema": "local-pmb-memory.compat_apply.v1",
            "ok": False,
            "applied": False,
    ...
    daemon = daemon_restart() if reindex_required else daemon_status()
