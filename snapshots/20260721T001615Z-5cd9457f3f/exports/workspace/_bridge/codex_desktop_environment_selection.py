...
def _reconcile_environment_selection_unlocked(
    *,
    ...
    selected, source = (
        ...
        else _select_value(effective_host, wsl_value, last, state_payload=state_payload)
    )
    ...
    state_changed = not _state_is_current(state_path, selected)
    host_state_changed = bool(
        host_state_path is not None
        and not _state_is_current(host_state_path, selected)
    )
