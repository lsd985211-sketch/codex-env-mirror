...
5. Run one approved refresh only after finalization. Use `--changed` only when the affected plan is safe; otherwise use full refresh. A valid committed snapshot with unchanged live sources is reused only when no explicit changed paths were supplied.
