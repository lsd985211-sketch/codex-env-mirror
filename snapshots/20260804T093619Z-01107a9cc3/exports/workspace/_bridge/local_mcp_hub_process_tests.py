...
class HubBytecodeCacheTests(unittest.TestCase):
    ...
    def test_reload_aborts_before_stop_when_cache_cleanup_fails(self) -> None:
        with patch.object(
            ...
            hub_process.hub_authorization, "authorize_reload", return_value={"ok": True}
        ), patch.object(
            hub_process.hub_authorization, "finish_reload", return_value={"ok": True}
    ...
    def test_confirmed_reload_without_permit_fails_before_first_side_effect(self) -> None:
        before = {"processes": [{"pid": 321, "authority": "wsl_user_systemd"}]}
        with patch.object(
            hub_process, "hub_service_status", return_value={"unit_exists": True}
        ), patch.object(
            hub_process, "hub_runtime_state", return_value=before
        ), patch.object(
            hub_process, "clear_hub_bytecode_cache", return_value={"ok": True, "dry_run": True}
        ) as clear_cache, patch.object(
            hub_process, "systemctl"
        ) as systemctl, patch.object(
            hub_process, "stop_process"
        ...
        self.assertFalse(result["ok"])
        self.assertEqual(result["reason"], "local_mcp_hub_reload_authorization_required")
        clear_cache.assert_called_once_with(dry_run=True)
        systemctl.assert_not_called()
        stop.assert_not_called()
        fallback.assert_not_called()

    def test_reload_scope_drift_blocks_before_permit_consumption(self) -> None:
        expected = hub_authorization.reload_scope(
            thread_id="thread-a", source_signature="head-a",
            authority="wsl_user_systemd", port=18881,
            matched_processes=[{"pid": 321, "authority": "wsl_user_systemd"}],
        )
        changed = dict(expected)
        changed["target_fingerprint"] = "stale-pid-or-port-binding"
        snapshot = {
            "ok": True, "intent_type": "task_intent",
            "executor": hub_authorization.OWNER, "audience": hub_authorization.OWNER,
            "operation_id": "reload-op", "workflow_semantic_hash": "workflow-a",
            "scope": changed, "environment_snapshot": {},
        }
        with patch.object(
            hub_authorization.authorization, "permit_snapshot", return_value=snapshot
        ), patch.object(hub_authorization.authorization, "consume_permit") as consume:
            result = hub_authorization.authorize_reload(
                "scoped-authorization:permit:test", operation_id="reload-op",
                workflow_semantic_hash="workflow-a", thread_id="thread-a",
                source_signature="head-a", authority="wsl_user_systemd",
                port=18881, matched_processes=[{"pid": 321, "authority": "wsl_user_systemd"}],
            )

        self.assertFalse(result["ok"])
        self.assertEqual(result["reason"], "local_mcp_hub_reload_scope_changed")
        self.assertIn("target_fingerprint", result["mismatched_fields"])
        consume.assert_not_called()

    def test_current_task_permit_is_consumed_before_hub_reload_effect(self) -> None:
        thread_id = "hub-reload-thread"
        source_signature = "head-a"
        matched = [{"pid": 321, "authority": "wsl_user_systemd"}]
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            ...
            rows = [
                {"type": "session_meta", "payload": {"id": thread_id}},
                {
                    ...
                    "payload": {
                        ...
                        "content": [{"text": "执行本地 Hub reload"}],
                    },
                },
            ]
            rollout.write_text("\n".join(json.dumps(row) for row in rows) + "\n", encoding="utf-8")
            scope = hub_authorization.reload_scope(
            ...
            assessment = {
                ...
                "action": {"id": hub_authorization.ACTION},
                "resource": {"target_fingerprint": scope["target_fingerprint"]},
                "environment": {
                    "owner": hub_authorization.OWNER,
                    "phase": hub_authorization.PHASE,
                    "source_signature": source_signature,
                    "recovery_ref": f"systemd:user:{hub_authorization.SERVICE}:restart",
                },
            ...
            environment = {
                ...
                "authorization_semantic_signature": "test",
            ...
            with patch.object(
                ...
                "snapshot", return_value=environment,
            ):
                prepared = hub_authorization.prepare_reload(
                    thread_id=thread_id, source_signature=source_signature,
                    authority="wsl_user_systemd", port=18881,
                    matched_processes=matched, assessment=assessment,
                    rollout_path=rollout, user_message_ref=f"{rollout.name}:line:2",
                    operation_id="reload-op", workflow_semantic_hash="workflow-a",
                    state_root=root / "authorization",
                )
                gate = hub_authorization.authorize_reload(
                    prepared.get("permit_ref", ""), operation_id="reload-op",
                    workflow_semantic_hash="workflow-a", thread_id=thread_id,
                    source_signature=source_signature, authority="wsl_user_systemd",
                    port=18881, matched_processes=matched,
                    state_root=root / "authorization",
                )
                finished = hub_authorization.finish_reload(
                    "reload-op",
                    {
                        "ok": True, "authority": "wsl_user_systemd", "port": 18881,
                        "matched_processes": matched,
                        "after_processes": [{"pid": 654, "authority": "wsl_user_systemd"}],
                    },
                    state_root=root / "authorization",
                )
        ...
        self.assertEqual(finished.get("status"), "completed")
