...
def evaluate_acceptance(contract: dict[str, Any], result: dict[str, Any]) -> dict[str, Any]:
    metadata = _dict(result.get("metadata"))
    if metadata.get("preview_truncated") is True:
        content_ref = str(metadata.get("content_ref") or result.get("content_ref") or "").strip()
        if not content_ref or not __import__("pathlib").Path(content_ref).is_file():
            return {
                "schema": "resource_acceptance_decision.v1",
                "accepted": False,
                "full": False,
                "partial": False,
                "reason": "full_content_reference_missing",
                "signature": str(contract.get("signature") or ""),
                "required_deliverables": _items(contract.get("required_deliverables")),
                "delivered": [],
                "missing_deliverables": ["consumable"],
                "minimum_quantity": max(1, _positive_int(contract.get("minimum_quantity"), 1)),
                "actual_quantity": 0,
                "next_action": "repair_content_reference_without_refetching",
            }
