...
VIDEO_TERMS = (
    "视频", "电影", "影片", "影视", "观看", "在线观看", "流媒体", "租赁", "购买",
    "video", "movie", "film", "watch", "streaming", "rent", "buy", "mp4", "clip",
)
...
def _documentation_domain_owner(domain: str) -> str:
    if _domain_matches(domain, OPENAI_DOC_DOMAINS):
        return "openai-docs"
    if _domain_matches(domain, ("learn.microsoft.com", "microsoft.com")):
        return "microsoftdocs"
...
def _documentation_route(request: dict[str, Any], route: dict[str, Any]) -> dict[str, Any]:
    ...
    domain_owners = {domain: _documentation_domain_owner(domain) for domain in domains}
    distinct_domain_owners = set(domain_owners.values())

    if domains and distinct_domain_owners == {"openai-docs"}:
        route_kind = "openai_official_docs"
        owner = "openai-docs"
    elif domains and distinct_domain_owners == {"microsoftdocs"}:
        route_kind = "microsoft_docs"
        owner = "microsoftdocs"
    elif domains:
        route_kind = "multi_domain_official_docs" if len(domains) > 1 else "official_vendor_docs"
        ...
        owner = "context7"
    elif matched_terms(text, ("openai", "chatgpt", "codex cli", "codex desktop", "codex sdk", "developers.openai.com", "platform.openai.com", "help.openai.com")):
        route_kind = "openai_official_docs"
        owner = "openai-docs"
    ...
    return {
        ...
        "domain_owner_partitions": domain_owners,
...
def classify_resource_kind(request: dict[str, Any], route: dict[str, Any]) -> str:
    ...
    intent = route_intent if route_intent and route_intent != ResourceIntent.UNKNOWN else str(request.get("intent") or "")
    primary_tool = str(route.get("primary_tool") or "")
    if intent == ResourceIntent.PACKAGE_DEPENDENCY or primary_tool == "package_manager":
        return "package"
    # A documentation-shaped request can still ask for research evidence.  Do
    # this before documentation owner selection so Context7 stays library/API-only.
...
def source_catalog() -> tuple[SourceCandidate, ...]:
    return (
        SourceCandidate(
            ...
            notes=("Read-only metadata route; preserves DOI and open-access landing URLs.",),
        ),
        SourceCandidate(
        ...
        SourceCandidate(
            id="video_platform_discovery",
            resource_kind="video",
            owner_tool="generic_search",
            source_type="managed_platform_search",
            priority=10,
            use_for=("legal viewing platforms", "rental or purchase availability", "video and film entity discovery"),
            avoid_when=("an explicit media URL is already available", "access-control bypass or unlicensed source is requested"),
            query_hint="Search normalized and original entity names; preserve platform URL, title, region, and access type.",
            materialization="return_platform_candidates_without_bypassing_access_controls",
            notes=("Entity aliases are candidates, never silent replacements.",),
        ),
        SourceCandidate(
...
def source_execution_capability(resource_kind: str, *, registered_owner_adapter: str = "") -> dict[str, Any]:
    if resource_kind == "academic_paper":
        return {
            "mode": "registered_academic_metadata_owner_adapter",
            "bounded_execution_available": True,
            "arbitrary_search_available": True,
            "registered_owner_adapter": "academic_search",
            "required_capability_for_arbitrary_search": "academic_metadata_search_owner_adapter",
            "runtime_health_required": True,
            "rule": "Crossref and OpenAlex are read-only metadata sources; a result URL is evidence, not permission to materialize content.",
        }
    if resource_kind in {"generic_web", "video"}:
        return {
...
def candidate_source_plan(request: dict[str, Any], route: dict[str, Any], *, limit: int = 6) -> dict[str, Any]:
    ...
    return {
        ...
        "classification_evidence": {
            ...
            "academic_hits": matched_terms(text, ACADEMIC_TERMS) or _has_word(text, ACADEMIC_WORD_TERMS),
            **documentation,
        },
...
def validate() -> dict[str, Any]:
    ...
    academic_docs = candidate_source_plan(
        {"task": "检索 retrieval practice 与 spacing 的医学教育论文证据", "target": "adaptive medical education journal paper", "intent": ResourceIntent.DOCUMENTATION_LOOKUP},
        {"primary_tool": "context7", "intent": ResourceIntent.DOCUMENTATION_LOOKUP},
    )
    ...
    mixed_vendor_docs = candidate_source_plan(
        {
            "task": "Research HP OMEN, Microsoft BitLocker, and NVIDIA Optimus official guidance",
            "target": "OMEN graphics switching",
            "intent": ResourceIntent.DOCUMENTATION_LOOKUP,
            "metadata": {
                "resource_kind_hint": "documentation",
                "source_domains": ["support.hp.com", "learn.microsoft.com", "nvidia.com"],
            },
        },
        {"primary_tool": "resource_router", "intent": ResourceIntent.DOCUMENTATION_LOOKUP},
    )
    ...
    checks = [
        {"name": "paper_has_academic_sources", "ok": paper["resource_kind"] == "academic_paper" and paper["candidates"][0]["id"] == "academic_openalex"},
        {"name": "academic_docs_bypass_context7", "ok": academic_docs["resource_kind"] == "academic_paper" and academic_docs["execution_capability"].get("registered_owner_adapter") == "academic_search"},
        ...
        {
            "name": "mixed_explicit_domains_do_not_collapse_to_one_specialized_owner",
            "ok": mixed_vendor_docs["candidates"][0]["owner_tool"] == "generic_search"
            and mixed_vendor_docs["classification_evidence"]["documentation_route_kind"] == "multi_domain_official_docs",
        },
