...
import wsl_workspace_owner_authorization as host_projection_authorization
import scoped_authorization_policy
import scoped_authorization_process
...
class DesktopProjectRegistrationTests(unittest.TestCase):
    ...
    def test_desktop_classifier_reports_cdp_launch_argument_degradation_without_page(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            receipt = Path(raw) / "launch-environment.json"
            receipt.write_text(json.dumps({
                "schema": "codex.desktop.launch_environment.v1",
                ...
                "cdp_acceptance": {
                    "ready": False,
                    "reason": "cdp_launch_argument_not_observed",
                    "port": 9229,
                    "started_process_id": 9748,
                    "expected_launch_argument_observed": False,
                    "started_process_alive": True,
                    "listener_present": False,
                    "listener_owner_matches_started_pid": False,
                    "started_process_elevated": "Yes",
            ...
            with patch.object(
                owner.codex_desktop_model_runtime,
                "_find_codex_page",
                return_value=(0, "", [], "9231:TimeoutError;9229:TimeoutError"),
            ):
                payload = owner.desktop_project_classifier_state(receipt)

        self.assertFalse(payload["recognized"])
        self.assertEqual("desktop_cdp_listener_missing_launch_argument_not_observed", payload["reason"])
        self.assertEqual(9229, payload["launch_environment"]["cdp"]["port"])
        self.assertFalse(payload["launch_environment"]["cdp"]["expected_launch_argument_observed"])
        self.assertFalse(payload["launch_environment"]["cdp"]["listener_present"])
    ...
    def test_mirror_export_projects_local_mcp_hub_status_and_unit(self) -> None:
        ...
        hub_owner = SimpleNamespace(hub_service_status=lambda: {"ok": True, "active": True})
        with patch.object(
            owner, "_local_mcp_hub_process", return_value=hub_owner
        ):
            status = owner.mirror_export(args)
        args.kind = "local-mcp-hub-user-unit"
        hub_owner = SimpleNamespace(hub_service_plan=lambda: {"ok": True, "unit_sha256": "hub-abc"})
        with patch.object(
            owner, "_local_mcp_hub_process", return_value=hub_owner
        ):
            unit = owner.mirror_export(args)
    ...
    def test_windows_delegate_entry_import_does_not_load_posix_hub_authorization(self) -> None:
        bridge = Path(owner.__file__).resolve().parent
        script = """
import builtins
import os
import sys
blocked = {"local_mcp_hub_process", "local_mcp_hub_authorization", "scoped_authorization", "fcntl"}
...
sys.path.insert(0, os.environ["WSL_OWNER_BRIDGE"])
import wsl_workspace_owner
print("owner_imported")
"""
        env = dict(os.environ, WSL_OWNER_BRIDGE=str(bridge))
        ...
        self.assertEqual("owner_imported", result.stdout.strip())
...
class HostCompatibilityProjectionTests(unittest.TestCase):
    def setUp(self) -> None:
        self.authorize_projection = patch.object(
            owner._host_projection_authorization(),
            "authorize_projection",
            return_value={"ok": True, "operation_id": "projection-test"},
        )
        self.finish_projection = patch.object(
            owner._host_projection_authorization(),
            "finish_projection",
            return_value={"ok": True, "status": "completed"},
        )
        self.authorize_projection.start()
        self.finish_projection.start()
        self.addCleanup(self.finish_projection.stop)
        self.addCleanup(self.authorize_projection.stop)
    ...
    def _roots(self) -> tuple[Path, Path]:
        ...
        (source / owner.HOST_STARTUP_BASELINE).write_text(
            json.dumps({
                ...
                "expected_plugins": [
                    {"id": "chrome@openai-bundled", "required": True},
                ],
                "decommissioned_plugins": ["browser@openai-bundled"],
                "decommissioned_marketplaces": ["retired"],
            }),
            encoding="utf-8",
        )
        (target / owner.HOST_STARTUP_BASELINE).write_text(
            json.dumps({
                ...
                "expected_plugins": [
                    {"id": "browser@openai-bundled", "required": True},
                ],
    ...
    def test_host_projection_includes_atomic_startup_maintenance_bundle(self) -> None:
        expected = {
            "_bridge/codex_state_audit.py",
            "_bridge/codex_session_store_doctor.py",
            "_bridge/codex_prelaunch_maintenance.py",
        }
        self.assertEqual(expected, set(owner.CODEX_STARTUP_MAINTENANCE_PROJECTION_FILES))
        self.assertTrue(expected.issubset(owner.HOST_PROJECTION_FILES))
    ...
    def test_host_projection_migrates_only_retired_project_baseline_fields(self) -> None:
        ...
        self.assertEqual(
            baseline["expected_plugins"],
            [{"id": "chrome@openai-bundled", "required": True}],
        )
        self.assertNotIn("browser@openai-bundled", baseline["expected_plugins"])
        self.assertEqual(baseline["decommissioned_plugins"], ["browser@openai-bundled"])
        self.assertEqual(baseline["decommissioned_marketplaces"], ["retired"])
...
class HostProjectionScopedAuthorizationTests(unittest.TestCase):
    def _roots(self) -> tuple[Path, Path]:
        return HostCompatibilityProjectionTests()._roots()

    def _rollout(self, root: Path, thread_id: str) -> tuple[Path, str]:
        ...
        rows = [
            {"type": "session_meta", "payload": {"id": thread_id}},
            {
                ...
                "payload": {
                    "type": "message",
                    "role": "user",
                    "content": [{"text": "批准刷新 Windows compatibility projection"}],
                },
            },
        ]
        rollout.write_text("\n".join(json.dumps(row) for row in rows) + "\n", encoding="utf-8")
        return rollout, f"{rollout.name}:line:2"

    def _assessment(self, plan: dict[str, object], thread_id: str) -> dict[str, object]:
        scope = host_projection_authorization.projection_scope(plan, thread_id=thread_id)
        costs = {
            name: {"actual": 0}
            for name in scoped_authorization_policy.load_policy()["cost_dimensions"]
        }
        return {
            "subject": {"thread_id": thread_id},
            "action": {"id": host_projection_authorization.ACTION},
            "resource": {"target_fingerprint": scope["target_fingerprint"]},
            "environment": {
                "owner": host_projection_authorization.OWNER,
                "phase": host_projection_authorization.PHASE,
                "source_signature": scope["source_signature"],
                "recovery_ref": f"backup-router:{scope['target_fingerprint']}",
            },
            "risk": {"level": "R2", "severity": "S1", "likelihood": "L2", "facts": []},
            "matched_forbids": [],
            "costs": costs,
        }

    def _prepare(
        self, plan: dict[str, object], root: Path, *, thread_id: str = "projection-thread",
        operation_id: str = "projection-op", workflow_hash: str = "workflow-a",
    ) -> dict[str, object]:
        rollout, user_message_ref = self._rollout(root, thread_id)
        environment = {
            "schema": "authorization_environment_snapshot.v1",
            "ok": True,
            "workflow_semantic_hash": workflow_hash,
            "authorization_semantic_signature": "projection-test",
            "required_sources": ["workflow"],
            "sources": {
                "workflow": {
                    "signature": workflow_hash,
                    "status": "ok",
                    "authorization_effect": "neutral",
                    "authority_ref": "test.workflow",
                }
            },
        }
        with patch.object(
            scoped_authorization_process.authorization_environment_provider,
            "snapshot",
            return_value=environment,
        ):
            return host_projection_authorization.prepare_projection(
                plan,
                thread_id=thread_id,
                assessment=self._assessment(plan, thread_id),
    ...
    def test_static_confirm_without_permit_blocks_before_backup(self) -> None:
        source, target = self._roots()
        with patch.object(owner, "_run", return_value={"ok": False, "stdout": ""}), patch.object(
            owner, "create_backup",
        ) as backup:
        ...
        self.assertFalse(payload["ok"])
        self.assertEqual("host_projection_scoped_authorization_required", payload["reason"])
        backup.assert_not_called()

    def test_permit_rejects_scope_thread_operation_workflow_and_reuse_changes(self) -> None:
        source, target = self._roots()
        with patch.object(owner, "_run", return_value={"ok": False, "stdout": ""}):
            plan = owner.host_compatibility_projection_plan(source_root=source, target_root=target)
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            prepared = self._prepare(plan, root)
            permit_ref = str(prepared.get("permit_ref") or "")
            state_root = root / "authorization"
            wrong_thread = host_projection_authorization.authorize_projection(
                permit_ref, plan, operation_id="projection-op", workflow_semantic_hash="workflow-a",
                thread_id="other-thread", state_root=state_root,
            )
            wrong_operation = host_projection_authorization.authorize_projection(
                permit_ref, plan, operation_id="other-op", workflow_semantic_hash="workflow-a",
                thread_id="projection-thread", state_root=state_root,
            )
            wrong_workflow = host_projection_authorization.authorize_projection(
                permit_ref, plan, operation_id="projection-op", workflow_semantic_hash="workflow-b",
                thread_id="projection-thread", state_root=state_root,
            )
            changed_plan = dict(plan)
            changed_plan["files"] = [dict(row) for row in plan["files"]]
            changed_plan["files"][0]["target_sha256"] = "changed-target-hash"
            changed_scope = host_projection_authorization.authorize_projection(
                permit_ref, changed_plan, operation_id="projection-op", workflow_semantic_hash="workflow-a",
                thread_id="projection-thread", state_root=state_root,
            )
            accepted = host_projection_authorization.authorize_projection(
                permit_ref, plan, operation_id="projection-op", workflow_semantic_hash="workflow-a",
                thread_id="projection-thread", state_root=state_root,
            )
            replay = host_projection_authorization.authorize_projection(
                permit_ref, plan, operation_id="other-op", workflow_semantic_hash="workflow-a",
                thread_id="projection-thread", state_root=state_root,
            )
            wrong_owner_snapshot = {
                ...
                "executor": "other_owner",
                "audience": "other_owner",
                "operation_id": "projection-op",
                "workflow_semantic_hash": "workflow-a",
                "scope": host_projection_authorization.projection_scope(plan, thread_id="projection-thread"),
                "environment_snapshot": {},
            }
            with patch.object(
                host_projection_authorization.authorization,
                "permit_snapshot",
                return_value=wrong_owner_snapshot,
            ):
                wrong_owner = host_projection_authorization.authorize_projection(
                    permit_ref, plan, operation_id="projection-op", workflow_semantic_hash="workflow-a",
                    thread_id="projection-thread", state_root=state_root,
                )

        self.assertTrue(prepared.get("ok"), prepared)
        self.assertEqual("host_projection_scope_changed", wrong_thread["reason"])
        self.assertEqual("host_projection_operation_changed", wrong_operation["reason"])
        self.assertEqual("host_projection_workflow_changed", wrong_workflow["reason"])
        self.assertEqual("host_projection_scope_changed", changed_scope["reason"])
        self.assertTrue(accepted.get("ok"), accepted)
        self.assertEqual("host_projection_operation_changed", replay["reason"])
        self.assertEqual("host_projection_owner_binding_changed", wrong_owner["reason"])

    def test_authorization_effects_are_unknown_on_failure_and_completed_after_readback(self) -> None:
        source, target = self._roots()
        with patch.object(owner, "_run", return_value={"ok": False, "stdout": ""}):
            plan = owner.host_compatibility_projection_plan(source_root=source, target_root=target)
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            state_root = root / "authorization"
            failed = self._prepare(plan, root, operation_id="projection-failed")
            failed_gate = host_projection_authorization.authorize_projection(
                str(failed.get("permit_ref") or ""), plan, operation_id="projection-failed",
                workflow_semantic_hash="workflow-a", thread_id="projection-thread", state_root=state_root,
            )
            unknown = host_projection_authorization.finish_projection(
                "projection-failed", {"ok": False, "reason": "backup_failed"}, state_root=state_root,
            )
            succeeded = self._prepare(plan, root, operation_id="projection-succeeded")
            success_gate = host_projection_authorization.authorize_projection(
                str(succeeded.get("permit_ref") or ""), plan, operation_id="projection-succeeded",
                workflow_semantic_hash="workflow-a", thread_id="projection-thread", state_root=state_root,
            )
            completed = host_projection_authorization.finish_projection(
                "projection-succeeded",
                {
                    "ok": True,
                    "selection_mode": "directed",
                    "projected_files": ["host_compatibility:_bridge/platform_paths.py"],
                    "full_projection_current": False,
                    "remaining_drift": ["host_compatibility:_bridge/windows_execution_agent.py"],
                },
                state_root=state_root,
            )

        self.assertTrue(failed.get("ok"), failed)
        self.assertTrue(failed_gate.get("ok"), failed_gate)
        self.assertEqual("effect_unknown", unknown["status"])
        self.assertTrue(succeeded.get("ok"), succeeded)
        self.assertTrue(success_gate.get("ok"), success_gate)
        self.assertEqual("completed", completed["status"])

    def test_backup_failure_records_effect_unknown(self) -> None:
        source, target = self._roots()
        with tempfile.TemporaryDirectory() as temp_dir, patch.object(
            owner, "_run", return_value={"ok": False, "stdout": ""},
        ), patch.object(
            owner, "create_backup", return_value={"ok": False, "reason": "backup_failed"},
        ):
            root = Path(temp_dir)
            plan = owner.host_compatibility_projection_plan(source_root=source, target_root=target)
            prepared = self._prepare(plan, root, operation_id="projection-backup-failure")
            result = owner.host_compatibility_projection_apply(
                confirm=owner.HOST_PROJECTION_CONFIRM,
                source_root=source,
                target_root=target,
                authorization_permit_ref=str(prepared.get("permit_ref") or ""),
                authorization_operation_id="projection-backup-failure",
                authorization_thread_id="projection-thread",
                workflow_semantic_hash="workflow-a",
        ...
        self.assertEqual("projection_backup_failed", result["reason"])
        self.assertEqual("effect_unknown", result["authorization_effect"]["status"])

    def test_already_current_is_read_only_and_does_not_need_a_second_permit(self) -> None:
        source, target = self._roots()
        with tempfile.TemporaryDirectory() as temp_dir, patch.object(
            owner, "_run", return_value={"ok": False, "stdout": ""},
        ), patch.object(
            owner, "create_backup", return_value={"ok": True, "manifest_paths": ["backup.json"]},
        ) as backup:
            root = Path(temp_dir)
            plan = owner.host_compatibility_projection_plan(source_root=source, target_root=target)
            prepared = self._prepare(plan, root)
            first = owner.host_compatibility_projection_apply(
                ...
                target_root=target,
                authorization_permit_ref=str(prepared.get("permit_ref") or ""),
                authorization_operation_id="projection-op",
                authorization_thread_id="projection-thread",
                workflow_semantic_hash="workflow-a",
                authorization_state_root=root / "authorization",
            )
        ...
        self.assertTrue(first["authorization_effect"].get("ok"), first)
        self.assertTrue(second["ok"], second)
        self.assertEqual("already_current", second["reason"])
        backup.assert_not_called()
