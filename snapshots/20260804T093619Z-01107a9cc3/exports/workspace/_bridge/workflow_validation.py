...
from workflow_asset_guidance import validate_with_build_plan as validate_asset_guidance_with_build_plan
from workflow_automation_delegation import POLICY_SCHEMA, single_authority_plan_check, validate_schema_authority
...
VALIDATION_SAMPLES = [
    ...
    "代码可维护性治理和重构目标选择",
    "代码模块系统深度整合到工作流",
    "检查 USB 设备、驱动状态和最近热插拔事件",
    "规划 USB 摄像头重启并要求显式确认和回滚边界",
    "整理 U 盘 Music 里的音频文件，按歌手专辑分类并配对歌词封面",
]
...
PROFILE_EXPECTATIONS = [
    ...
    ("检查 USB 设备和热插拔状态", "diagnose_only", False),
]
...
def independent_cli_contracts() -> dict[str, dict[str, Any]]:
    """Run the independent read-only CLI contract groups concurrently.

    Each group owns separate subprocesses and has no shared mutable plan or
    authorization state. Results are resolved in declaration order so callers
    retain the existing deterministic validation projection and failure shape.
    """
    groups = (
        ("machine_enum", machine_enum_cli_contract),
        ("online_access_gate", online_access_gate_contract),
        ("resource_broker", resource_broker_contract),
        ("maintenance_upgrade", maintenance_upgrade_governance_contract),
    )
    with ThreadPoolExecutor(
        max_workers=len(groups),
        thread_name_prefix="workflow-cli-contract",
    ) as executor:
        futures = {name: executor.submit(contract) for name, contract in groups}
        return {name: futures[name].result() for name, _ in groups}
...
def hardware_routing_semantics(build_plan: BuildPlan) -> dict[str, Any]:
    plan = build_plan("检查设备管理器中的 PCI 显卡、音频设备和硬件问题")
    compact_plan = build_plan("检查 USB 设备、驱动状态和最近热插拔事件", detail="micro")
    usb_plan = build_plan("检查 USB 设备、驱动状态和最近热插拔事件")
    control_plan = build_plan("规划 USB 摄像头重启并要求显式确认和回滚边界")
    cross_platform_plan = build_plan("检查 WSL GPU、PCI 和 Windows 宿主硬件状态")
    domains = [str(item.get("key") or "") for item in plan.get("domains", [])]
    maintenance = plan.get("maintenance", [])
    domain_detail = next((item for item in plan.get("domains", []) if item.get("key") == "hardware"), {})
    compact_domain_detail = next(
        (item for item in compact_plan.get("domains", []) if item.get("key") == "hardware"),
        {},
    )
    environment = plan.get("execution_route_pack", {}).get("environment_context", {})
    relevant_systems = [str(item.get("system") or "") for item in environment.get("relevant_systems", []) if isinstance(item, dict)]
    control_route = control_plan.get("execution_route_pack", {}).get("route_decision", {})
    route = plan.get("execution_route_pack", {}).get("route_decision", {})
    usb_route = usb_plan.get("execution_route_pack", {}).get("route_decision", {})
    active_policy_keys = {
        str(item.get("key") or "")
        for item in plan.get("execution_route_pack", {}).get("active_policies", [])
        if isinstance(item, dict)
    }
    control_environment = control_plan.get("execution_route_pack", {}).get("environment_context", {})
    control_systems = [str(item.get("system") or "") for item in control_environment.get("relevant_systems", []) if isinstance(item, dict)]
    control_gates = control_route.get("required_gates", []) if isinstance(control_route.get("required_gates"), list) else []
    cross_platform_domains = [str(item.get("key") or "") for item in cross_platform_plan.get("domains", [])]
    cross_platform_maintenance = cross_platform_plan.get("maintenance", [])
    routing_ok = (
        "hardware" in domains
        and any("windows_hardware_owner.py" in str(item) for item in maintenance)
        and any("usb_device_owner.py" in str(item) for item in maintenance)
        and any("usb_device_control.py" in str(item) for item in maintenance)
        and "hardware" in relevant_systems
        and any(
            "windows_hardware_owner.py" in str(item)
            for item in usb_plan.get("maintenance", [])
        )
        and "hardware" in cross_platform_domains
        and any("hardware_system_owner.py" in str(item) for item in cross_platform_maintenance)
        and any("wsl_hardware_owner.py" in str(item) for item in cross_platform_maintenance)
    )
    semantics_ok = (
        domain_detail.get("match_quality") == "strong"
        and bool(domain_detail.get("drives_execution"))
        and float(domain_detail.get("route_confidence", 0.0)) >= 0.5
        and "hardware" in compact_domain_detail.get("systems", [])
        and plan.get("profile", {}).get("profile") == "diagnose_only"
        and route.get("required_next_action") == "execute_primary_workflow_phase"
        and usb_route.get("required_next_action") == "execute_primary_workflow_phase"
        and "structured_state_policy" not in active_policy_keys
        and control_route.get("primary_domain") == "hardware"
        and not any(item.get("fact") == "reload_or_restart_required" for item in control_gates if isinstance(item, dict))
        and "startup" not in control_systems
    )
    return {
        "routing_ok": routing_ok,
        "semantics_ok": semantics_ok,
        ...
        "domain_detail": domain_detail,
        "compact_domain_detail": compact_domain_detail,
        "profile": plan.get("profile", {}),
        "usb_domains": [str(item.get("key") or "") for item in usb_plan.get("domains", [])],
        "cross_platform_domains": cross_platform_domains,
        "route": route,
        "usb_route": usb_route,
        "active_policy_keys": sorted(active_policy_keys),
        "control_route": control_route,
        "control_systems": control_systems,
    }


def audio_routing_semantics(build_plan: BuildPlan) -> dict[str, Any]:
    plan = build_plan("整理 U 盘 Music 里的音频文件，按歌手专辑分类并配对歌词封面")
    hardware_plan = build_plan("检查 USB 设备、驱动状态和最近热插拔事件")
    domains = [str(item.get("key") or "") for item in plan.get("domains", [])]
    route = plan.get("execution_route_pack", {}).get("route_decision", {})
    maintenance = [str(item) for item in plan.get("maintenance", [])]
    environment = plan.get("execution_route_pack", {}).get("environment_context", {})
    systems = [
        str(item.get("system") or "")
        for item in environment.get("relevant_systems", [])
        if isinstance(item, dict)
    ]
    audio_detail = next((item for item in plan.get("domains", []) if item.get("key") == "audio"), {})
    hardware_domains = [str(item.get("key") or "") for item in hardware_plan.get("domains", [])]
    routing_ok = (
        domains
        and domains[0] == "audio"
        and route.get("primary_domain") == "audio"
        and any("music_library_owner.py" in item for item in maintenance)
        and any("audio_toolkit" in item for item in maintenance)
        and any("usb_device_owner.py storage" in item for item in maintenance)
        and {"audio", "hardware"}.issubset(systems)
        and hardware_domains
        and hardware_domains[0] == "hardware"
    )
    semantics_ok = (
        audio_detail.get("match_quality") == "strong"
        and bool(audio_detail.get("drives_execution"))
        and "windows-audio-ops" in plan.get("skills", {}).get("selected", [])
    )
    return {
        ...
        "maintenance": maintenance,
        "systems": systems,
        "hardware_control_domains": hardware_domains,
        "domain_detail": audio_detail,
    }
...
def mechanism_conflict_routing_contract(build_plan: BuildPlan) -> dict[str, Any]:
    ...
    ok = (
        ...
        and mobile_route.get("task_mode") == "mobile_delegation"
        and mobile_route.get("required_next_action") == "execute_mobile_delegation_contract"
        and email_route.get("primary_owner") == "email_scheduler"
...
def closeout_surface_contract() -> dict[str, Any]:
    ...
    ok = (
        ...
        and int(review_summary.get("incomplete_count") or 0) >= 2
        and review_summary.get("detail_complete") is False
        and "### 审批卡片 1：" in markdown
        and "中文摘要待生产者补充；原始证据：" in markdown
        and "Validation external knowledge candidate" in markdown
        and "- 审批动作：" in markdown
    ...
    return {
        ...
        "kinds": sorted(kinds),
        "incomplete_count": review_summary.get("incomplete_count"),
        "detail_complete": review_summary.get("detail_complete"),
...
def classification_fallback(build_plan: BuildPlan) -> dict[str, Any]:
    ...
    ai_os_plan = build_plan(
        "继续扩大学习范围，将本机的codex工作环境变成可以真正投入实际生产使用的成熟高效ai操作系统"
    )
    ai_os_domains = [str(item.get("key") or "") for item in ai_os_plan.get("domains", [])]
    ai_os_guidance = ai_os_plan.get("execution_route_pack", {}).get("asset_guidance", {})
    construction_plan = build_plan(
        "注意对现有资产的优化或者利用，做出一个可以投入施工的计划"
    )
    construction_domains = [str(item.get("key") or "") for item in construction_plan.get("domains", [])]
    ...
    capability_inventory_plan = build_plan(
        "识别本机已经安装和可用的 OCR 能力，选择正确 owner 与工具处理扫描 PDF，并说明降级路径"
    )
    capability_inventory_domains = [str(item.get("key") or "") for item in capability_inventory_plan.get("domains", [])]
    capability_inventory_facts = (
        capability_inventory_plan.get("structured_route", {})
        .get("task_contract", {})
        .get("task_facts", {})
    )
    ok = (
        ...
        and ai_os_domains and ai_os_domains[0] == "workflow_governance"
        and construction_domains and construction_domains[0] == "workflow_governance"
        and "general" not in construction_domains
        and any(item.get("name") == "global-framework" for item in ai_os_guidance.get("skills", []) if isinstance(item, dict))
        and any(
            str(item.get("system") or item.get("name") or "") == "workflow"
            and str(item.get("authority") or "") == "_bridge/system_membership.py"
            for item in ai_os_guidance.get("owners", [])
            if isinstance(item, dict)
        )
        and {"codegraph", "gitnexus"}.issubset(
            {str(item.get("name") or "") for item in ai_os_guidance.get("tools", []) if isinstance(item, dict)}
        )
        and "resource_acquisition" not in negated_install_domains
        and "resource_acquisition" not in capability_inventory_domains
        and capability_inventory_facts.get("package_install") is False
        and capability_inventory_facts.get("system_member_change") is False
        and capability_inventory_facts.get("durable_closeout_required") is False
    )
    return {
        "ok": ok,
        ...
        "ai_os_domains": ai_os_domains,
        "construction_domains": construction_domains,
        "ai_os_asset_guidance": ai_os_guidance,
        "negated_install_domains": negated_install_domains,
        "capability_inventory_domains": capability_inventory_domains,
        "capability_inventory_facts": capability_inventory_facts,
    }
...
def detail_tier_semantics(build_plan: BuildPlan) -> dict[str, Any]:
    ...
    codegraph_guidance = codegraph_micro.get("execution_route_pack", {}).get("asset_guidance", {})
    codegraph_tools = {
        item.get("name"): item
        for item in codegraph_guidance.get("tools", [])
        if isinstance(item, dict)
    }
    ...
    ok = (
        ...
        and codegraph_tools.get("codegraph", {}).get("mode") == "primary_discovery"
        and "Hub codegraph.explore" in codegraph_tools.get("codegraph", {}).get("action", "")
        and codegraph_tools.get("gitnexus", {}).get("mode") == "supporting_discovery"
        and any(
    ...
    return {
        ...
        "codegraph_micro_asset_guidance": codegraph_guidance,
    }
...
def execution_route_pack_semantics(build_plan: BuildPlan) -> dict[str, Any]:
    ...
    ok = (
        ...
        and unresolved_pack.get("resource_gate", {}).get("completion_contract", {}).get("handoff_required_status") == "call_owner_tool_and_attach_result_to_same_request_id"
        and unresolved_pack.get("resource_gate", {}).get("task_lifecycle", {}).get("codex_during_running") == "consume_job_run_process_output_or_poll_progress; do_not_start_independent_replacement_fetch_for_same_need"
        and "handoff_required" in unresolved_pack.get("resource_gate", {}).get("task_lifecycle", {}).get("resource_layer_receipt_statuses", [])
        and "handoff_required" not in unresolved_pack.get("resource_gate", {}).get("task_lifecycle", {}).get("end_to_end_terminal_statuses", [])
        ...
        and unresolved_pack.get("resource_gate", {}).get("entrypoint", "").startswith("_bridge/codex_workflow_entry.py resource custom --request-json")
        and "custom --request-json" in str(unresolved_pack.get("resource_gate", {}).get("submit_entrypoint", ""))
        ...
        and "codex_workflow_entry.py resource custom" in unresolved_decision.get("resource_submit_command", "")
...
def automation_delegation_semantics(build_plan: BuildPlan) -> dict[str, Any]:
    ...
    decision = plan.get("execution_route_pack", {}).get("automation_decision", {})
    workflow_steps = [str(item) for item in plan.get("workflow", [])]
    ok = (
        policy.get("schema") == POLICY_SCHEMA
        and route_policy.get("schema") == POLICY_SCHEMA
        ...
        and bool(route_policy.get("deduplication_rules"))
        and route_policy.get("command_contract_reuse", {}).get("authority_order", [None])[0] == "owner_plan_receipt"
        and route_policy.get("command_contract_reuse", {}).get("authority_order", [None])[-1] == "targeted_help_last_resort"
        and "argument_rejected" in route_policy.get("command_contract_reuse", {}).get("expand_help_only_when", [])
        and "tool_schema_authority" in route_policy.get("live_contract_preconditions", {})
        and "thread_coordination" in route_policy.get("live_contract_preconditions", {})
        and "automate_only_a_declared_owner_operation_with_complete_inputs_and_a_stable_input_signature"
        in route_policy.get("machine_execution_invariants", [])
        and decision.get("decision_class") in {"auto_execute", "codex_deferred", "review_required", "blocked"}
        and bool(decision.get("input_signature"))
        and bool(decision.get("reuse_policy"))
        and decision.get("machine_execution_contract", {}).get("input_signature") == decision.get("input_signature")
        and "consumable_receipt" in decision.get("machine_execution_contract", {}).get("required_evidence", [])
        and "failure_state_erasure" in decision.get("machine_execution_contract", {}).get("forbidden", [])
...
def single_authority_semantics(build_plan: BuildPlan) -> dict[str, Any]:
    plan = build_plan("联网研究并更新共享工作流代码和工具路由", detail="full")
    return single_authority_plan_check(plan)
...
def routing_validation_checks(
    ...
    return [
        ...
        {
            "name": "hardware_routes_to_read_only_device_owner",
            "ok": hardware_result["routing_ok"] and hardware_result["semantics_ok"],
            "detail": hardware_result,
        },
        {
            "name": "audio_routes_to_music_owner_with_hardware_handoff",
            "ok": audio_result["routing_ok"] and audio_result["semantics_ok"],
            "detail": audio_result,
        },
        {
...
def contract_validation_checks(
    ...
    asset_guidance_result: dict[str, Any],
    single_authority_result: dict[str, Any],
    schema_authority_result: dict[str, Any],
    plans: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    return [
        {
        ...
        {
            "name": "proactive_asset_guidance_contract_ok",
            "ok": asset_guidance_result["ok"],
            "detail": asset_guidance_result,
        },
        {
            "name": "single_authority_projection_contract_ok",
            "ok": single_authority_result["ok"],
            "detail": single_authority_result,
        },
        {
            "name": "schema_version_authority_contract_ok",
            "ok": schema_authority_result["ok"],
            "detail": schema_authority_result,
        },
        {
...
def build_validation_checks(
    ...
    hardware_result = hardware_routing_semantics(build_plan)
    audio_result = audio_routing_semantics(build_plan)
    ...
    cli_contract_results = independent_cli_contracts()
    machine_enum_result = cli_contract_results["machine_enum"]
    online_gate_result = cli_contract_results["online_access_gate"]
    resource_broker_result = cli_contract_results["resource_broker"]
    maintenance_upgrade_result = cli_contract_results["maintenance_upgrade"]
    asset_guidance_result = validate_asset_guidance_with_build_plan(build_plan)
    single_authority_result = single_authority_semantics(build_plan)
    schema_authority_result = validate_schema_authority()
    ...
    return [
        ...
        *contract_validation_checks(
            ...
            asset_guidance_result,
            single_authority_result,
            schema_authority_result,
            plans,
        ),
