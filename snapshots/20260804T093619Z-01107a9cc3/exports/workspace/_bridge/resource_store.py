...
from resource_content_store import adopt_content_artifacts
...
def persist_manifest(
    *,
    ...
    for attempt in reversed(receipt.get("attempts", [])):
        result = attempt.get("result") if isinstance(attempt, dict) and isinstance(attempt.get("result"), dict) else {}
        metadata = result.get("metadata") if isinstance(result.get("metadata"), dict) else {}
        if metadata.get("content_index_ref"):
            content_metadata = metadata
            break
    adopted_content = adopt_content_artifacts(request_dir=request_dir, metadata=content_metadata) if content_metadata else {}
    ...
    updated_receipt = {
        **receipt,
        ...
        "content_ref": str(adopted_content.get("content_ref") or receipt.get("content_ref") or ""),
        "preview_ref": str(adopted_content.get("preview_ref") or ""),
        "content_index_ref": str(adopted_content.get("content_index_ref") or ""),
        "content_sha256": str(adopted_content.get("content_sha256") or receipt.get("sha256") or ""),
        "preview_truncated": bool(adopted_content.get("preview_truncated")),
    }
    saved_paths = {
    ...
    if adopted_content:
        saved_paths.update({
            "content_ref": str(adopted_content.get("content_ref") or ""),
            "preview_ref": str(adopted_content.get("preview_ref") or ""),
            "content_index_ref": str(adopted_content.get("content_index_ref") or ""),
            "raw_ref": str(adopted_content.get("raw_ref") or ""),
        })
    ...
    receipt.update(
        {
            ...
            "satisfaction": {
                ...
                "satisfied": True,
                "reason": "owner_result_attached",
                "next_action": "consume_resource",
                "result_kind": result_kind or "owner_result",
                "source_tool": source_tool,
            },
        }
    )
    request = manifest.get("request") if isinstance(manifest.get("request"), dict) else {}
    receipt["codex_guidance"] = build_codex_guidance(request, receipt)
    manifest["receipt"] = receipt
