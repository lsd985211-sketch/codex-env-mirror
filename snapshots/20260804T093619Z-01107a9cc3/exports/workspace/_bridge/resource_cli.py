...
from resource_request_admission import normalize_resource_input, url_batch_payload
...
from resource_content_store import expand_content
...
END_TO_END_TERMINAL_STATUSES = {"failed", "blocked"}
RESOURCE_LAYER_RECEIPT_STATUSES = END_TO_END_TERMINAL_STATUSES | {"completed"}
RESOURCE_JOB_ACQUISITION_OWNER = "resource_layer"
RESOURCE_JOB_REQUIRED_FIELDS = (
    "ownership",
    "resource_layer_terminal",
    "end_to_end_terminal",
...
def package_manager_metadata(args: argparse.Namespace) -> dict[str, Any]:
    ...
    authorization = {
        "grant_ref": str(getattr(args, "authorization_grant_ref", "") or "").strip(),
        "operation_id": str(getattr(args, "authorization_operation_id", "") or "").strip(),
        "thread_id": str(getattr(args, "authorization_thread_id", "") or "").strip(),
    }
...
def effective_owner_execution_mode(args: argparse.Namespace) -> str:
    return (
        "approved_write"
        if str(getattr(args, "package_execution_mode", "") or "").strip().lower() == "verified_auto_install"
        else str(getattr(args, "owner_execution_mode", "read_only") or "read_only")
    )
...
def command_request(args: argparse.Namespace) -> int:
    if args.json_payload:
        ...
        request = ResourceBrokerRequest(
            ...
            owner_execution_mode=effective_owner_execution_mode(args),
            metadata={
                "cli_command": "request",
                "purpose": args.purpose or "",
    ...
    if admission.get("kind") == "url_list" and admission.get("ok"):
        batch_payload = url_batch_payload(asdict(request), admission)
        batch = execute_batch(
            requests_from_payload(batch_payload),
            config=batch_config_from_payload(batch_payload),
            event_log=Path(args.event_log).expanduser().resolve(),
            receipt_log=Path(args.receipt_log).expanduser().resolve(),
            resource_log=None if args.no_resource_log else Path(args.resource_log).expanduser().resolve(),
            store_root=Path(args.store_root).expanduser().resolve(),
        )
        payload = {**batch, "promoted_from": "resource request", "admission": admission}
...
def build_job_delegation(args: argparse.Namespace) -> dict[str, Any]:
    ...
    return build_delegation(
        ...
        package_execution_mode=args.package_execution_mode or "",
        ...
        authorization_grant_ref=args.authorization_grant_ref or "",
        authorization_operation_id=args.authorization_operation_id or "",
        authorization_thread_id=args.authorization_thread_id or "",
...
def job_command_contract(request_id: str) -> dict[str, str]:
    entry = "python _bridge\\codex_workflow_entry.py resource job"
    return {
        "status": f"{entry} status --request-id {request_id} --json",
        "progress": f"{entry} progress --request-id {request_id} --json",
        "wait": f"{entry} wait --request-id {request_id} --json",
        "receipt": f"{entry} receipt --request-id {request_id} --json",
        "attach": f"{entry} attach --request-id {request_id} --source-tool <tool> --content-file <path> --json",
        "consume": f"{entry} consume --request-id {request_id} --consumed-path <path> --json",
    }
...
def compact_receipt_payload(receipt: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "acceptance_contract": data.get("acceptance_contract", {}),
...
def _resource_job_ownership_projection(payload: dict[str, Any]) -> dict[str, Any]:
    ownership = payload.get("ownership") if isinstance(payload.get("ownership"), dict) else {}
    duplicate = ownership.get("duplicate_fetch_policy") if isinstance(ownership.get("duplicate_fetch_policy"), dict) else {}
    return {
        key: ownership[key]
        for key in (
            "acquisition_owner",
            "owned_need_scope",
            "owned_until_terminal_receipt",
            "resource_layer_terminal",
            "end_to_end_terminal",
            "resource_need_satisfied",
            "same_need_fetch_allowed",
        ...
        "duplicate_fetch_policy": {
            key: duplicate[key]
            for key in ("same_need", "handoff_status")
            if key in duplicate
        }
    }


def _resource_job_artifact_ref(payload: dict[str, Any]) -> str:
    receipt = payload.get("receipt") if isinstance(payload.get("receipt"), dict) else {}
    candidates = (
        receipt.get("manifest_path"),
        receipt.get("content_ref"),
        receipt.get("artifact_path"),
        *[
            item.get("path")
            for item in payload.get("required_consume_paths") or []
            if isinstance(item, dict)
        ],
    )
    return next((str(item) for item in candidates if str(item or "").strip()), "")


def resource_job_cli_projection(payload: dict[str, Any], *, detail: str = "compact") -> dict[str, Any]:
    """Return the allowlisted Codex projection without changing owner state."""

    full = str(detail or "compact").lower() == "full"
    source = dict(payload)
    source["ownership"] = _resource_job_ownership_projection(source)
    if isinstance(source.get("receipt"), dict):
        source["receipt"] = receipt_payload_for_detail(source["receipt"], "full" if full else "compact")
    artifact_ref = _resource_job_artifact_ref(source)
    ...
    decision = context_projection_owner.decide_projection(
        ...
        consumer_purpose="codex_resource_job_full" if full else "codex_resource_job_default",
        inline_budget=32 * 1024 if full else 8 * 1024,
        artifact_ref=artifact_ref,
        required_field_names=RESOURCE_JOB_REQUIRED_FIELDS,
        preserve_field_names=RESOURCE_JOB_PRESERVE_FIELDS,
    )
...
def command_job_submit(args: argparse.Namespace) -> int:
    ...
    if args.job_command == "run" or args.foreground:
        ...
        result = {
            ...
            "receipt_detail": str(getattr(args, "receipt_detail", "compact") or "compact"),
            "receipt": receipt_payload_for_detail(receipt, str(getattr(args, "receipt_detail", "compact") or "compact")),
            "commands": job_command_contract(receipt.request_id),
        }
        output = resource_job_cli_projection(result, detail=args.receipt_detail)
        ...
        return 0 if result["ok"] or receipt.status == "handoff_required" else 1
...
def command_job_wait(args: argparse.Namespace) -> int:
    ...
    output = resource_job_cli_projection(payload, detail=getattr(args, "receipt_detail", "compact"))
    print(json.dumps(output, ensure_ascii=False, sort_keys=True) if args.json else json.dumps(output, ensure_ascii=False, indent=2, sort_keys=True))
...
def command_expand_content(args: argparse.Namespace) -> int:
    payload = expand_content(
        content_ref=Path(args.content_ref),
        query=args.query or "",
        line_start=args.line_start,
        line_count=args.line_count,
        byte_start=args.byte_start,
        byte_count=args.byte_count,
        max_chars=max(1, min(int(args.max_chars), 100_000)),
    )
    print(json.dumps(payload, ensure_ascii=False, sort_keys=True) if args.json else payload.get("text") or json.dumps(payload, ensure_ascii=False, indent=2, sort_keys=True))
    return 0 if payload.get("ok") else 1
...
def build_parser() -> argparse.ArgumentParser:
    return build_resource_parser(
        ResourceCliParserConfig(
            ...
            command_expand_content=command_expand_content,
