...
import resource_process_authorization  # noqa: E402
...
def codex_host_role(command_line: Any) -> str:
    ...
    if "openai.codex_" in text and any(
        marker in text
        for marker in (CODEX_HOST_MARKERS["desktop"], CODEX_HOST_MARKERS["desktop_shell"])
    ):
...
def cleanup_orphan_candidates(
    *,
    ...
    if not apply:
        return payload
    if not safe_apply or include_protected:
        return {
            **payload, "ok": False, "applied": False,
            "reason": "scoped_authorization_safe_non_protected_cleanup_required",
        }
    if not authorization_grant_ref or not authorization_thread_id:
        return {
            **payload, "ok": False, "applied": False,
            "reason": "scoped_authorization_grant_required",
            "static_apply_is_authorization": False,
            "authorization_plan_action": "cleanup-authorization-plan",
        }
    operation_id = authorization_operation_id or f"resource-process:cleanup:{resource_process_authorization.cleanup_scope(payload, thread_id=authorization_thread_id)['target_fingerprint']}"
    authorization = resource_process_authorization.consume(
        authorization_grant_ref, payload, thread_id=authorization_thread_id,
        operation_id=operation_id, state_root=authorization_state_root,
    )
    if not authorization.get("ok"):
        return {**payload, "ok": False, "applied": False, "reason": "scoped_authorization_rejected", "authorization": authorization}
    started = resource_process_authorization.effect_started(operation_id, state_root=authorization_state_root)
    if not started.get("ok"):
        return {**payload, "ok": False, "applied": False, "reason": "authorization_effect_journal_failed", "authorization_effect": started}
    execution = cleanup_execution_result(selected=selected, apply=True)
    result = {
        **payload,
        "ok": execution["cleanup_ok"] and bool(execution["post_validation_ok"]),
    ...
    effect = resource_process_authorization.effect_finished(operation_id, result, state_root=authorization_state_root)
    return {**result, "authorization_effect": effect}


def main() -> int:
    parser = argparse.ArgumentParser(description="Resource/MCP process lifecycle doctor")
    parser.add_argument("command", choices=["snapshot", "doctor", "repair-plan", "metrics", "validate", "startup-sources", "cleanup", "cleanup-authorization-plan"])
    ...
    parser.add_argument("--authorization-grant-ref", default="")
    parser.add_argument("--authorization-thread-id", default="")
    parser.add_argument("--authorization-operation-id", default="")
    ...
    if args.command == "snapshot":
        ...
        payload = cleanup_orphan_candidates(
            ...
            authorization_grant_ref=args.authorization_grant_ref,
    ...
    elif args.command == "cleanup-authorization-plan":
        preview = cleanup_orphan_candidates(
            safe_apply=True, include_protected=False, groups=list(args.group or []),
            min_age_minutes=float(args.min_age_minutes),
        )
        payload = resource_process_authorization.authorization_plan(
            preview, thread_id=args.authorization_thread_id,
            state_root=args.authorization_state_root or None,
        ) if args.authorization_thread_id else {
            "schema": "resource_process.cleanup_authorization_plan.v1", "ok": False,
            "reason": "authorization_thread_id_required",
        }
    else:
