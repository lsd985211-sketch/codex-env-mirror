...
QUICK_SCENARIO_REQUESTS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "task": "simulate verified package auto install scheduling",
        "intent": "package_dependency",
        "auto_owner": True,
        "allow_filesystem_write": True,
        "owner_execution_mode": "approved_write",
        "metadata": {
...
def evaluate_scenario(mode: str, batch: dict[str, Any], status: dict[str, Any]) -> dict[str, Any]:
    ...
    check_equals(checks, "inspect_then_install_queue", install_like.get("queue_class"), "package_metadata")
    verified_auto_install = planned_by_task(batch, "simulate verified package auto install scheduling")
    check_equals(checks, "verified_auto_install_queue", verified_auto_install.get("queue_class"), "install_or_clone")
