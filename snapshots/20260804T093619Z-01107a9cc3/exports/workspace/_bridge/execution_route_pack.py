...
from workflow_terminal_convergence import build_convergence_plan
from decision_authority_policy import classify_decision, project_decisions
from scoped_authorization_policy import consumer_enforcement, decide_gate, load_policy


ALWAYS_STOP_IF = (
    "side_effect_without_selected_pdp_decision_or_required_owner_controls",
...
def build_terminal_convergence_capsule(plan: dict[str, Any]) -> dict[str, Any]:
    """Build the read-only construction decision from explicit workflow facts."""

    declaration = _as_dict(plan.get("terminal_convergence"))
    actions = [item for item in _as_list(declaration.get("actions")) if isinstance(item, dict)]
    if not actions:
        return {"schema": "terminal_convergence.projection.v1", "relevant": False}
    result = build_convergence_plan(
        convergence_id=str(declaration.get("convergence_id") or "").strip(),
        intent_id=str(declaration.get("intent_id") or "").strip(),
        terminal_goal=str(declaration.get("terminal_goal") or "").strip(),
        source_state=_as_dict(declaration.get("source_state")),
        actions=actions,
        receipts=[item for item in _as_list(declaration.get("receipts")) if isinstance(item, dict)],
        entered_read_only_acceptance=bool(declaration.get("entered_read_only_acceptance")),
    )
    return {**result, "schema": "terminal_convergence.projection.v1", "relevant": True}
...
def build_authorization_capsule(
    plan: dict[str, Any], route_decision: dict[str, Any], environment_context: dict[str, Any]
) -> dict[str, Any]:
    """Project task facts to the sole risk/cost PDP and enforce only selected consumers."""

    task_facts = _as_dict(_as_dict(_as_dict(plan.get("structured_route")).get("task_contract")).get("task_facts"))
    required_facts = [name for name in AUTHORIZATION_FACTS if bool(task_facts.get(name))]
    task_contract = _as_dict(_as_dict(plan.get("structured_route")).get("task_contract"))
    assessment = _as_dict(task_contract.get("authorization_assessment"))
    owner_route = _as_dict(route_decision.get("owner_route"))
    policy_decisions = [item for item in _as_list(route_decision.get("policy_decisions")) if isinstance(item, dict)]
    relevant_systems = [item for item in _as_list(environment_context.get("relevant_systems")) if isinstance(item, dict)]
    workflow_semantics = {
        "primary_domain": route_decision.get("primary_domain"),
        "required_next_action": route_decision.get("required_next_action"),
        "owner": owner_route.get("owner") or owner_route.get("name"),
        "required_facts": required_facts,
        "policy_rules": sorted({str(item.get("rule_id") or "") for item in policy_decisions if item.get("rule_id")}),
    }
    workflow_hash = _stable_signature(workflow_semantics)
    sources = {
        "workflow": {
            "signature": workflow_hash,
            "status": "ok",
            "authorization_effect": "neutral",
            "authority_ref": "execution_route_pack.route_decision",
        },
        "owner_capability": {
            "signature": _stable_signature(owner_route),
            "status": "ok" if owner_route else "unavailable",
            "authorization_effect": "neutral" if owner_route else "unavailable",
            "authority_ref": str(owner_route.get("contract_ref") or owner_route.get("owner") or ""),
        },
        "system_membership": {
            "signature": _stable_signature(relevant_systems),
            "status": "ok" if relevant_systems else "unavailable",
            "authorization_effect": "neutral" if relevant_systems else "unavailable",
            "authority_ref": "workflow_environment_context.relevant_systems",
        },
        "rule_governance": {
            "signature": _stable_signature(policy_decisions),
            "status": "ok",
            "authorization_effect": "neutral",
            "authority_ref": "execution_route_pack.route_decision.policy_decisions",
        },
        "maintenance_capability": {
            "signature": _stable_signature(environment_context.get("tool_entrypoints") or []),
            "status": "ok" if environment_context.get("tool_entrypoints") else "unavailable",
            "authorization_effect": "neutral" if environment_context.get("tool_entrypoints") else "unavailable",
            "authority_ref": "workflow_environment_context.tool_entrypoints",
        },
        "state_write_authority": {
            "signature": _stable_signature([item for item in _as_list(route_decision.get("required_gates")) if isinstance(item, dict) and item.get("owner") == "workspace_editing_and_backup"]),
            "status": "required" if required_facts else "not_required",
            "authorization_effect": "neutral",
            "authority_ref": "execution_route_pack.required_gates",
        },
    }
    snapshot = {
        "schema": "authorization_environment_snapshot.v1",
        "ok": all(item["status"] != "unavailable" for item in sources.values()) if required_facts else True,
        "workflow_semantic_hash": workflow_hash,
        "authorization_semantic_signature": _stable_signature({"required_facts": required_facts, "owner": workflow_semantics["owner"]}),
        "required_sources": sorted(sources) if required_facts else [],
        "sources": sources,
    }
    if not required_facts:
        decision = {}
        enforcement = {"enforced": True, "selected_enforcement": "not_applicable", "decision_ref": "", "legacy_gate_retained": False}
    elif assessment:
        decision = decide_gate(assessment, policy_payload=load_policy())
        enforcement = consumer_enforcement(decision, consumer="execution_route_pack")
    else:
        dimensions = (load_policy().get("cost_dimensions") or {}).keys()
        bounded_assessment = {
            "subject": {"id": "current-task", "class": "codex_task"},
            "action": {"name": "unassessed_side_effect", "owner": str(workflow_semantics["owner"] or "unknown")},
            "resource": {"kind": "workflow_route", "id": workflow_hash},
            "environment": {"class": "unknown_pending_bounded_preflight"},
            "risk": {"level": "unknown", "facts": []},
            "costs": {name: None for name in dimensions},
            "matched_forbids": [],
        }
        decision = decide_gate(bounded_assessment, policy_payload=load_policy())
        enforcement = consumer_enforcement(decision, consumer="execution_route_pack")
    selected = str(enforcement.get("selected_enforcement") or "legacy_fail_closed")
    status_by_decision = {
        "not_applicable": "not_required",
        "allow_without_challenge": "allowed_without_challenge",
        "bounded_preflight": "assessment_required",
        "challenge_required": "challenge_required",
        "deny_non_overrideable": "denied_non_overrideable",
        "legacy_fail_closed": "legacy_gate_required",
    }
    return {
        "schema": "scoped_authorization.route_capsule.v3",
        "required": selected == "challenge_required",
        "challenge_required": selected == "challenge_required",
        "assessment_required": selected == "bounded_preflight",
        "status": status_by_decision.get(selected, "legacy_gate_required"),
        "effective_decision": selected,
        "selected_enforcement": selected,
        "required_facts": required_facts,
        "operation_control_facts": required_facts,
        "authority": "scoped_authorization_policy",
        "decision_ref": str(enforcement.get("decision_ref") or ""),
        "pdp_decision": decision,
        "consumer_enforcement": enforcement,
        "inspect_entrypoint": "python _bridge/scoped_authorization.py inspect",
        "permit_rule": "derive one permit per user turn or automation run; consume before first side effect",
        "environment_snapshot": snapshot,
        "environment_signature": _stable_signature(snapshot),
        "required_next_action": {
            "not_applicable": "continue_without_write_authorization",
            "allow_without_challenge": "continue_with_task_intent_and_owner_controls",
            "bounded_preflight": "run_one_bounded_read_only_preflight_then_reevaluate",
            "challenge_required": "create_or_reuse_one_exact_challenge",
            "deny_non_overrideable": "stop_non_overrideable",
            "legacy_fail_closed": "retain_existing_consumer_gate",
        }.get(selected, "retain_existing_consumer_gate"),
        "automatic_expansion_allowed": False,
    }


def build_decision_authority_capsule(
    plan: dict[str, Any], route_decision: dict[str, Any], authorization: dict[str, Any]
) -> dict[str, Any]:
    """Translate explicit workflow gates into the canonical authority contract."""

    task_facts = _as_dict(_as_dict(_as_dict(plan.get("structured_route")).get("task_contract")).get("task_facts"))
    authorization_required = (
        set(str(item) for item in _as_list(authorization.get("required_facts")))
        if authorization.get("challenge_required")
        else set()
    )
    decisions: list[dict[str, Any]] = []
    for gate in _as_list(route_decision.get("required_gates")):
        if not isinstance(gate, dict):
            continue
        fact = str(gate.get("fact") or gate.get("schema") or "workflow_gate").strip()
        is_authorizable = fact in authorization_required
        # Task-fact gates are owner controls (backup, scope, validation and
        # closeout). Only the selected PDP decision may make them authorization
        # blockers; a low-risk allow must leave these obligations visible while
        # keeping the decision authority projection non-blocking.
        owner_control_satisfied = bool(
            authorization.get("effective_decision") == "allow_without_challenge"
            and fact not in {"destructive_or_high_risk", "external_write", "package_install", "database_write"}
        )
        effective_decision = str(authorization.get("effective_decision") or "")
        decision_class = (
            "hard_gate"
            if effective_decision == "deny_non_overrideable"
            else "overrideable_policy"
            if is_authorizable
            else "recommendation"
        )
        decisions.append(
            classify_decision(
                ...
                authority_ref=str(gate.get("authority_ref") or gate.get("owner") or f"workflow.required_gates.{fact}"),
                condition_satisfied=not bool(gate.get("required", True)) or owner_control_satisfied,
                authorization_valid=is_authorizable and authorization.get("effective_decision") == "allow_without_challenge",
                user_goal_explicit=owner_control_satisfied,
                required_evidence=[str(gate.get("completion") or "")],
                requested_action=str(route_decision.get("required_next_action") or ""),
                input_context={"fact": fact, "task_fact": bool(task_facts.get(fact)), "gate_schema": gate.get("schema")},
            )
        )
    projection = project_decisions(decisions)
    return {
        **projection,
        "schema": "decision_authority.route_capsule.v1",
        "authority": "decision_authority",
        "authorization_ref": "execution_route_pack.authorization",
        "rule": "task_fact_controls_are_non_blocking until an owner reports a failed hard boundary; PDP challenge/deny remains authoritative",
    }
...
def _route_decision(
    ...
    return {
        ...
        "task_contract_ref": "structured_route.task_contract",
        ...
        "task_facts_ref": "structured_route.task_contract.task_facts",
        "matched_signals_ref": "structured_route.task_contract.matched_signals",
        ...
        "resource_completion_contract_ref": "resource_gate.completion_contract",
        "resource_task_lifecycle_ref": "resource_gate.task_lifecycle",
        "owner_route": {
...
def build_execution_route_pack(
    ...
    *,
    ...
    authorization = build_authorization_capsule(plan, route_decision, _as_dict(environment_context))
    route_pack = {
        ...
        "decision_authority": build_decision_authority_capsule(plan, route_decision, authorization),
        "memory": _memory_summary(plan),
        "automation_delegation": compact_automation_delegation_policy(),
        "automation_decision": automation_decision,
        "terminal_convergence": build_terminal_convergence_capsule(plan),
        ...
        "asset_guidance": build_asset_guidance(
            plan,
            route_decision=route_decision,
            tool_policies=tool_policies,
            environment_context=_as_dict(environment_context),
