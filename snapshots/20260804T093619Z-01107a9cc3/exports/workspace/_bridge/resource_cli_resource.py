...
def build_get_payload(args: argparse.Namespace) -> dict[str, Any]:
    package_execution_mode = str(getattr(args, "package_execution_mode", "") or "")
    authorization_grant_ref = str(getattr(args, "authorization_grant_ref", "") or "")
    authorization_operation_id = str(getattr(args, "authorization_operation_id", "") or "")
    authorization_thread_id = str(getattr(args, "authorization_thread_id", "") or "")
    ...
    return build_delegation(
        ...
        owner_execution_mode=(
            "approved_write"
            if package_execution_mode.strip().lower() == "verified_auto_install"
            else args.owner_execution_mode
        ),
        ...
        authorization_grant_ref=authorization_grant_ref,
...
def get_result_payload(args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        ...
        "commands": {
            "progress": f"python _bridge\\codex_workflow_entry.py resource job progress --request-id {receipt.request_id} --json",
            "receipt": f"python _bridge\\resource_cli.py job receipt --request-id {receipt.request_id} --json",
            "status": f"python _bridge\\codex_workflow_entry.py resource job status --request-id {receipt.request_id} --json",
