...
def project_thread_visibility(
    ...
    *,
    ...
    reassigned_count = 0
    valid_project_ids = set(state.get("local-projects") or {})
    if assignments_valid and isinstance(assignments, dict):
        for thread_id in thread_ids:
            if not isinstance(thread_id, str) or not thread_id:
                continue
            cwd = str((thread_cwds or {}).get(thread_id) or "")
            existing_assignment = assignments.get(thread_id)
            existing_project_id = (
                existing_assignment.get("projectId")
                if isinstance(existing_assignment, dict)
                else None
            )
            if thread_id in assigned_ids and existing_project_id in valid_project_ids:
                continue
