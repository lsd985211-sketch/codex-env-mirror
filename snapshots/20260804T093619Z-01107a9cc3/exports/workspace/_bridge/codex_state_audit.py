...
def expected_marketplace_fields(
    name: str,
    spec: dict,
    *,
    target: str,
) -> dict:
    """Exclude fields whose active writer is the Desktop runtime."""
...
def configured_runtime_cli_path(config: dict, *, wsl_active: bool) -> str:
    """Return the platform-owned runtime entry without persisting Desktop build paths in WSL."""

    if wsl_active:
        servers = config.get("mcp_servers")
        node_repl = servers.get("node_repl") if isinstance(servers, dict) else None
        return str(node_repl.get("command") or "").strip() if isinstance(node_repl, dict) else ""
    return node_repl_cli_path(config)
...
def build_checks(run_cli: bool = True) -> list[Check]:
    ...
    decommissioned_plugins = set(baseline.get("decommissioned_plugins") or [])
    ...
    try:
        ...
        configured_cli = configured_runtime_cli_path(global_config, wsl_active=wsl_active_config)
        ...
        configured_decommissioned_plugins = sorted(plugins & decommissioned_plugins)
        extra_plugins = sorted(plugins - expected_plugins - decommissioned_plugins)
        checks.append(Check("extra_plugins_not_in_baseline", True, f"extra={extra_plugins}"))
        checks.append(
            Check(
                "decommissioned_plugins_not_configured",
                not configured_decommissioned_plugins,
                f"configured_decommissioned={configured_decommissioned_plugins}",
            )
        )
        checks.append(
            Check(
        ...
        config_target = (
            "wsl_codex_config"
            if baseline.get("configuration_authority") == "wsl_active"
            else "windows_codex_config"
        )
        for name, declared_spec in expected_marketplaces.items():
