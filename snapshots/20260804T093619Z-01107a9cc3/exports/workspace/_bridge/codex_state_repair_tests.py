...
class CodexStateRepairWriteTests(unittest.TestCase):
    def test_windows_baseline_repair_does_not_fight_runtime_marketplace_source(self) -> None:
        spec = {"source_type": "local", "source": r"\\?\C:\Users\45543\.codex\.tmp\bundled"}
        filtered = repairer.expected_marketplace_fields(
            "openai-bundled",
            spec,
            target="windows_codex_config",
        )
        self.assertEqual({"source_type": "local"}, filtered)

    def test_production_baseline_does_not_reintroduce_retired_plugins(self) -> None:
        ...
        self.assertIn("browser@openai-bundled", baseline["decommissioned_plugins"])
        self.assertNotIn("open-code-review@open-code-review", baseline["expected_plugins"])
        self.assertIn("open-code-review@open-code-review", baseline["decommissioned_plugins"])
    ...
    def test_explicit_plugin_registration_reconciliation_removes_decommissioned_plugins(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            files = self.make_fixture(Path(raw))
            files[0].write_text(
            ...
            baseline.update({
                "configuration_authority": "wsl_active",
                "expected_plugins": ["chrome@openai-bundled"],
            ...
            with (
                mock.patch.object(repairer, "BASELINE_PATH", files[3]),
                mock.patch.object(repairer, "BACKUP_ROOT", files[4]),
                mock.patch.object(repairer, "refresh_runtime_artifacts") as refresh_runtime,
                mock.patch.object(repairer, "repair_global_state", return_value=({}, [])),
                mock.patch.object(repairer, "ensure_desktop_environment_selection") as select_environment,
                mock.patch.object(repairer, "ensure_wsl_runtime_projection") as project_runtime,
            ):
                result = repairer.repair(
                    dry_run=False,
                    runtime_validation=False,
            ...
            config = tomllib.loads(files[0].read_text(encoding="utf-8"))
            self.assertNotIn("browser@openai-bundled", config["plugins"])
            self.assertNotIn("retired", config.get("marketplaces", {}))
            self.assertTrue(config["plugins"]["chrome@openai-bundled"]["enabled"])
            self.assertEqual("keep-node", config["mcp_servers"]["node_repl"]["command"])
            self.assertIn(
                "global_plugin_remove_decommissioned_browser@openai-bundled",
                result["changed"],
            )
            self.assertIn("global_marketplace_remove_decommissioned_retired", result["changed"])
            self.assertEqual("skipped_explicit_registration", result["wsl_runtime_projection"]["status"])
            self.assertEqual("explicit_plugin_registration", result["global_config_owner"])
            self.assertNotIn("global_mcp_add_node_repl", result["changed"])
            refresh_runtime.assert_not_called()
            select_environment.assert_not_called()
            project_runtime.assert_not_called()
