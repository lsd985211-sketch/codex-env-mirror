...
class DependencyChangeIntelligenceTests(unittest.TestCase):
    ...
    def test_open_code_review_profile_tracks_delegation_and_telemetry_boundaries(self) -> None:
        profile, issues = intelligence.load_profile("open-code-review")
        ids = {item["capability_id"] for item in profile["capability_contracts"]}
        self.assertFalse(issues)
        self.assertEqual(
            {"code_review.delegation_mode", "code_review.telemetry_default_off"},
            ids,
        )
        self.assertEqual(
            "alibaba/open-code-review",
            profile["source_tasks"][0]["request"]["metadata"]["source_execution_contract"]["selectors"]["repository"],
        )

    def test_managed_python_profile_references_lock_without_copying_versions(self) -> None:
        profile, issues = intelligence.load_profile("managed-python-dependencies")
        self.assertFalse(issues)
        task = profile["source_tasks"][0]["request"]
        self.assertEqual(task["metadata"]["package_execution_mode"], "inspect_then_install")
        self.assertNotIn("install_approved", json.dumps(profile, ensure_ascii=False))
        self.assertNotIn("4.15.0", json.dumps(profile, ensure_ascii=False))
        contract = profile["capability_contracts"][0]
        self.assertIn("managed lock remains the sole executable", contract["contract"])
