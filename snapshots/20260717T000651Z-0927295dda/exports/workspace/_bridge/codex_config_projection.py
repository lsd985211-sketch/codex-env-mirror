...
def managed_projection_values(state: dict[str, Any]) -> dict[str, Any]:
    ...
    values = json.loads(json.dumps(state.get("managed") or {}, ensure_ascii=False))
...
def build_plan(state: dict[str, Any], *, additions_only: bool = False) -> dict[str, Any]:
    ...
    return {
        ...
        "projection_current": not additions and not updates and not managed_updates and not active_recovery_paths,
        "additions": additions,
        "updates": updates,
    ...
    *,
    ...
    try:
        ...
        return {
            ...
            and not any(not row.get("enabled") for row in after_state["common_config_flags"])
...
def main(argv: list[str] | None = None) -> int:
    ...
    if removals and args.confirm_remove != "EXPLICIT-REMOVE":
        ...
        result = {
            **{key: value for key, value in plan.items() if key not in {"additions", "updates", "common_only", "unsupported", "classifications", "replay_losses", "active_recovery_paths", "managed_updates", "desired_managed"}},
            "replay_loss_paths": [path_pointer(path) for path in plan["replay_losses"][:MAX_ACTION_ROWS]],
            "active_recovery_paths": [path_pointer(path) for path in plan["active_recovery_paths"][:MAX_ACTION_ROWS]],
