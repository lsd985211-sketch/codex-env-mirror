...
4. Run `mirror refresh --confirm REFRESH-CODEX-MIRROR` only with authorization.
5. Require `mirror_valid=true` and `capability_restore_ready=true` before reporting capture success.
6. Report `full_state_restore_ready=false` as an explicit archive/remote gap, not as mirror failure.
