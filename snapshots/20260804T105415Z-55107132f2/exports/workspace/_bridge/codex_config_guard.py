...
from platform_paths import codex_home  # noqa: E402
...
def snapshot(*, run_cli: bool = False) -> dict[str, Any]:
    ...
    global_config = codex_state_repair.baseline_host_path(baseline["global_config"])
    runtime_config = desktop_runtime_config_path(baseline, global_config)
    project_config = codex_state_repair.baseline_host_path(baseline["project_config"])
    state_path = codex_state_repair.baseline_host_path(
    ...
    runtime_config_data = safe_toml(runtime_config) if runtime_config.exists() else {}
    provider_model_list = provider_model_list_state(runtime_config_data)
    cc_switch_catalog = cc_switch_catalog_state(runtime_config_data, runtime_config, provider_model_list)
    ...
    catalog_path = model_catalog_path(runtime_config_data, runtime_config)
    desktop_runtime_model_state = codex_desktop_model_runtime.combined_state(
        expected_desktop_model_ids(runtime_config_data, provider_model_list, cc_switch_catalog),
        catalog_path,
    )
    return {
        ...
        "desktop_runtime_config": file_state(runtime_config),
        "project_config": file_state(project_config),
        "global_state": file_state(state_path),
        "desktop_model_refresh": desktop_model_refresh_state(runtime_config, app_model_list=desktop_app_model_list),
...
def desktop_runtime_config_path(
    baseline: dict[str, Any] | None = None,
    global_config: Path | None = None,
) -> Path:
    """Resolve mutable Desktop provider state without importing it into WSL config."""

    baseline = baseline or json.loads(codex_state_repair.BASELINE_PATH.read_text(encoding="utf-8"))
    global_config = global_config or codex_state_repair.baseline_host_path(baseline["global_config"])
    return codex_state_repair.desktop_host_config_path(baseline, global_config)
...
def provider_model_list_state(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if not model_ids:
        direct = _direct_provider_model_list_state(config, state)
        if direct is not None and direct.get("model_ids"):
            return direct
        return {
            **state,
            "ok": True,
...
def _direct_provider_model_list_state(config: dict[str, Any], state: dict[str, Any]) -> dict[str, Any] | None:
    """Read the active CC Switch provider's upstream catalog when its proxy is empty."""
    try:
        connection = sqlite3.connect(f"file:{codex_config_projection.CC_SWITCH_DB}?mode=ro", uri=True, timeout=5)
        try:
            row = connection.execute(
                "SELECT settings_config FROM providers WHERE lower(app_type) = 'codex' AND is_current = 1 "
        ...
        provider_config = tomllib.loads(str(settings.get("config") or ""))
        provider_name = str(provider_config.get("model_provider") or "")
        providers = provider_config.get("model_providers")
        upstream = providers.get(provider_name) if isinstance(providers, dict) else None
        if not isinstance(upstream, dict):
            return None
        upstream_base = str(upstream.get("base_url") or "").strip().rstrip("/")
        upstream_auth = settings.get("auth") if isinstance(settings.get("auth"), dict) else {}
        token = str(upstream_auth.get("OPENAI_API_KEY") or "")
        if (
            not upstream_base
            or not (upstream_base.startswith("http://") or upstream_base.startswith("https://"))
            or upstream_base == str(state.get("base_url") or "").rstrip("/")
            or not token
        ):
            return None
        request = urllib.request.Request(
            provider_models_endpoint(upstream_base),
        ...
        return {
            **state,
            "ok": current_model in model_ids if current_model else True,
            "authoritative": True,
            "usable": True,
            "degraded": False,
            "discovery_capability": "cc_switch_provider_direct",
            "discovery_route": "cc_switch_provider_direct",
            "http_status": status,
            "model_count": len(model_ids),
            "model_ids": model_ids,
            "configured_model_present": current_model in model_ids if current_model else False,
            "reason": "" if current_model in model_ids else "configured_model_missing_from_provider_models",
        }
    except Exception:
        return None
...
def doctor(*, run_cli: bool = False) -> dict[str, Any]:
    ...
    runtime_config = snap.get("desktop_runtime_config") if isinstance(snap.get("desktop_runtime_config"), dict) else {}
    cfg_path = runtime_config.get("path")
    runtime_config_data = safe_toml(Path(cfg_path)) if isinstance(cfg_path, str) else {}
    catalog_path = model_catalog_path(runtime_config_data, Path(cfg_path)) if isinstance(cfg_path, str) else None
...
def repair_plan() -> dict[str, Any]:
    ...
    runtime_config = desktop_runtime_config_path()
    config = safe_toml(runtime_config) if runtime_config.exists() else {}
    catalog_path = model_catalog_path(config, runtime_config)
...
def command_contract() -> dict[str, Any]:
    """Expose the existing startup owner contract to maintenance discovery."""

    actions = ["snapshot", "doctor", "repair-plan", "validate", "metrics", "run-once", "command-contract"]
    read_only_actions = ["snapshot", "doctor", "validate", "metrics", "command-contract"]
    action_commands = {action: [action] for action in actions}
    signature_payload = {
        "actions": actions,
        "read_only_actions": read_only_actions,
        "action_commands": action_commands,
        "member_ids": ["codex-startup-and-provider.core"],
    }
    return {
        "schema": "codex_config_guard.command_contract.v1",
        "ok": True,
        "owner": "codex_config_guard",
        "entrypoint": "python _bridge\\codex_config_guard.py",
        "actions": actions,
        "read_only_actions": read_only_actions,
        "mutating_actions": ["repair-plan", "run-once"],
        "action_commands": action_commands,
        ...
        "maintenance": {
            "system_id": "startup",
            "member_ids": ["codex-startup-and-provider.core"],
            "signals": ["startup.state_changed", "provider.state_changed"],
            "automatic_actions": ["snapshot", "doctor", "validate", "metrics"],
            ...
            "idempotent": True,
            "result_policy": "normalize_and_close_when_healthy",
        },
    }
...
def main() -> int:
    ...
    parser.add_argument(
        "action",
        choices=["snapshot", "doctor", "repair-plan", "validate", "metrics", "run-once", "command-contract"],
    )
