...
def normalized_query_candidates(value: str) -> list[str]:
    """Return explicit correction candidates without silently rewriting input."""

    text = str(value or "").strip()
    candidates: list[str] = []
    for alias, canonical in ENTITY_ALIAS_CANDIDATES.items():
        if alias in text:
            candidate = text.replace(alias, canonical)
...
def normalize_resource_input(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    query_text = str(payload.get("target") or payload.get("task") or "").strip()
    return {
        "schema": "resource_input.v1",
        "ok": True,
        ...
        "normalized_query_candidates": normalized_query_candidates(query_text),
        "normalization_mode": "candidate_only",
    }
