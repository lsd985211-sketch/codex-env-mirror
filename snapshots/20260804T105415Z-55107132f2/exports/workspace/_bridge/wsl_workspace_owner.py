...
import desktop_restart_request
...
from platform_paths import host_accessible_path, host_compatibility_root, windows_user_root
...
WINDOWS_GIT_CMD_DIR = r"C:\Program Files\Git\cmd"
...
CODEX_PROVIDER_RUNTIME_PROJECTION_FILES = (
    "_bridge/state_write_authority.py",
    "_bridge/contracts/state_write_authorities.json",
    "_bridge/codex_model_provider_watcher.py",
    "_bridge/codex_state_repair.py",
    "_bridge/codex_state_repair_mount_compatibility.py",
    ...
    "_bridge/codex_desktop_environment_selection.py",
    "_bridge/wsl_codex_runtime.py",
)
CODEX_STARTUP_MAINTENANCE_PROJECTION_FILES = (
    "_bridge/codex_state_audit.py",
    "_bridge/codex_session_store_doctor.py",
    "_bridge/codex_prelaunch_maintenance.py",
)
...
HOST_PROJECTION_FILES = (
    ...
    "_bridge/desktop_restart_request.py",
    *WINDOWS_SCHEDULER_RUNTIME_PROJECTION_FILES,
...
# Work Git owns portable capability expectations. The Windows target owns all
# other host-specific startup fields unless they are explicitly added here.
# Keeping this list beside the projection owner prevents derived projections
# from silently treating stale target values as authority.
HOST_STARTUP_SOURCE_OWNED_FIELDS = (
    "expected_plugins",
    "decommissioned_plugins",
    "decommissioned_marketplaces",
)
...
DEFAULT_DESKTOP_LAUNCH_ENVIRONMENT_STATE = Path(
    os.environ.get(
        "CODEX_DESKTOP_LAUNCH_ENVIRONMENT_STATE",
        r"C:\Users\45543\.codex\state\codex-desktop-launch-environment.json"
        if os.name == "nt"
        else "/mnt/c/Users/45543/.codex/state/codex-desktop-launch-environment.json",
    )
)
DEFAULT_WINDOWS_GIT_CONFIG = windows_user_root() / ".gitconfig"
...
def _write_json_object_atomic(path: Path, payload: dict[str, Any]) -> None:
    """Persist a small owner-owned JSON state file without exposing a partial document."""
...
def _derived_host_startup_baseline_projection(
    ...
    for field in HOST_STARTUP_SOURCE_OWNED_FIELDS:
        projected[field] = copy.deepcopy(source_baseline.get(field, []))
    ...
    return {
        ...
        "authority": {
            "source_owned_fields": list(HOST_STARTUP_SOURCE_OWNED_FIELDS),
            "target_owned_policy": "preserve_all_other_host_specific_fields",
...
def host_compatibility_projection_apply(
    *,
    ...
    gate = _host_projection_authorization().authorize_projection(
        authorization_permit_ref,
        plan,
    ...
    if not gate.get("ok"):
        return {
            ...
            "applied": False,
            "status": "blocked",
            "reason": gate.get("reason", "host_projection_authorization_blocked"),
            "authorization": gate,
            "plan": plan,
        }
    ...
    if not backup.get("ok"):
        ...
        result["authorization_effect"] = _host_projection_authorization().finish_projection(
            authorization_operation_id, result, state_root=authorization_state_root,
        )
        return result
    ...
    if not result["authorization_effect"].get("ok"):
        result["ok"] = False
        result["reason"] = "host_projection_effect_journal_failed"
    return result


def host_compatibility_projection_authorization_plan(
    *, thread_id: str, assessment: dict[str, Any], rollout_path: Path | str,
    user_message_ref: str, operation_id: str, workflow_semantic_hash: str,
    include: tuple[str, ...] = (), state_root: Path | str | None = None,
) -> dict[str, Any]:
    projection = host_compatibility_projection_plan(include=include)
    return _host_projection_authorization().prepare_projection(
        projection, thread_id=thread_id, assessment=assessment,
        rollout_path=rollout_path, user_message_ref=user_message_ref,
...
def _windows_git_executable() -> Path | None:
    candidates = [
        os.environ.get("CODEX_WINDOWS_GIT_EXE", ""),
        shutil.which("git.exe") or "",
        str(Path(os.environ.get("ProgramFiles", r"C:\Program Files")) / "Git" / "cmd" / "git.exe")
        if os.name == "nt"
        else "/mnt/c/Program Files/Git/cmd/git.exe",
    ]
...
def _canonical_windows_path_entry(value: str) -> str:
    return value.strip().strip('"').replace("\\", "/").rstrip("/").casefold()


def _ensure_windows_path_entry(current: str, required: str) -> tuple[str, bool]:
    entries = [entry.strip() for entry in current.split(";") if entry.strip()]
    required_key = _canonical_windows_path_entry(required)
    if any(_canonical_windows_path_entry(entry) == required_key for entry in entries):
        return current, False
    entries.append(required.rstrip("\\/"))
    return ";".join(entries), True


def _windows_path_snapshot() -> dict[str, Any]:
    script = r"""
...
[pscustomobject]@{
    ...
    machinePath = [string]$machinePath
    resolvedGit = if ($null -ne $git) { [string]$git.Source } else { '' }
    gitExists = Test-Path -LiteralPath 'C:\Program Files\Git\cmd\git.exe'
...
"""
    result = _run([
        ...
        *encoded_command_arguments(script),
    ])
    payload = _json_stdout(result)
    if not result.get("ok") or not payload:
        return {
            "ok": False,
            "reason": "windows_path_probe_failed",
...
def _malformed_windows_git_entries(value: str) -> list[str]:
    entries = [entry.strip() for entry in value.split(";") if entry.strip()]
    malformed: list[str] = []
    index = 0
    while index < len(entries):
        current = _canonical_windows_path_entry(entries[index])
        following = _canonical_windows_path_entry(entries[index + 1]) if index + 1 < len(entries) else ""
        if current == "c" and following == "/program files/git/cmd":
            malformed.append(f"{entries[index]};{entries[index + 1]}")
            index += 2
            continue
        if current == "/program files/git/cmd":
            malformed.append(entries[index])
        index += 1
    return malformed


def windows_git_path_state(raw: dict[str, Any] | None = None) -> dict[str, Any]:
    snapshot = raw or _windows_path_snapshot()
    if not snapshot.get("ok"):
        return {
            "schema": f"{SCHEMA}.windows_git_path_state",
            "ok": False,
            "reason": str(snapshot.get("reason") or "windows_path_probe_failed"),
            "path_discoverable": False,
            "user_entry_present": False,
            "malformed_machine_entries": [],
        }
    user_path = str(snapshot.get("userPath") or "")
    machine_path = str(snapshot.get("machinePath") or "")
    target_key = _canonical_windows_path_entry(WINDOWS_GIT_CMD_DIR)
    user_entries = [entry.strip() for entry in user_path.split(";") if entry.strip()]
    resolved_git = str(snapshot.get("resolvedGit") or "")
    user_entry_present = any(_canonical_windows_path_entry(entry) == target_key for entry in user_entries)
    path_discoverable = bool(resolved_git)
    git_exists = bool(snapshot.get("gitExists"))
    return {
        "schema": f"{SCHEMA}.windows_git_path_state",
        "ok": bool(path_discoverable and git_exists),
        "reason": "ready" if path_discoverable and git_exists else "git_not_discoverable_from_windows_path",
        "path_discoverable": path_discoverable,
        "resolved_git": resolved_git,
        "git_binary_exists": git_exists,
        "managed_user_entry": WINDOWS_GIT_CMD_DIR,
        "user_entry_present": user_entry_present,
        "user_entry_count": sum(_canonical_windows_path_entry(entry) == target_key for entry in user_entries),
        "malformed_machine_entries": _malformed_windows_git_entries(machine_path),
        "machine_path_rewritten": False,
    }


def _backup_windows_environment_path(snapshot: dict[str, Any]) -> dict[str, Any]:
    temporary_root = Path(tempfile.mkdtemp(prefix="codex-windows-environment-"))
    capture = temporary_root / "windows-user-environment.json"
    try:
        capture.write_text(json.dumps({
            "schema": f"{SCHEMA}.windows_environment_backup.v1",
            "captured_at": now_iso(),
            "user_path": str(snapshot.get("userPath") or ""),
            "machine_path": str(snapshot.get("machinePath") or ""),
            "restore": "set the captured user_path through Environment.SetEnvironmentVariable for User scope",
        }, ensure_ascii=False, indent=2) + "\n", encoding="utf-8", newline="\n")
        return create_backup(
            [str(capture)],
            category="codex-wsl-workspace",
            purpose="before-user-path-git-projection",
            trigger="wsl_workspace_owner.ensure_windows_git_path",
            remark="Before adding exact Windows Git directory to the user PATH",
        )
    finally:
        shutil.rmtree(temporary_root, ignore_errors=True)


def _set_windows_user_path(value: str) -> dict[str, Any]:
    encoded_value = base64.b64encode(value.encode("utf-8")).decode("ascii")
    script = rf"""
[Console]::OutputEncoding = New-Object System.Text.UTF8Encoding($false)
$value = [Text.Encoding]::UTF8.GetString([Convert]::FromBase64String('{encoded_value}'))
[Environment]::SetEnvironmentVariable('Path', $value, 'User')
Add-Type -Namespace Codex -Name NativeMethods -MemberDefinition @'
[DllImport("user32.dll", SetLastError=true, CharSet=CharSet.Auto)]
public static extern IntPtr SendMessageTimeout(IntPtr hWnd, uint Msg, UIntPtr wParam, string lParam, uint flags, uint timeout, out UIntPtr result);
'@
$result = [UIntPtr]::Zero
[void][Codex.NativeMethods]::SendMessageTimeout([IntPtr]0xffff, 0x001A, [UIntPtr]::Zero, 'Environment', 2, 3000, [ref]$result)
...
"""
    ...
    payload = _json_stdout(result)
    return {
        "ok": bool(result.get("ok") and payload.get("ok")),
        "returncode": result.get("returncode"),
        "stderr": str(result.get("stderr") or "")[-1200:],
    }


def ensure_windows_git_path() -> dict[str, Any]:
    before_raw = _windows_path_snapshot()
    before = windows_git_path_state(before_raw)
    if not before_raw.get("ok"):
        return {"ok": False, "changed": False, "reason": before.get("reason"), "before": before}
    updated, changed = _ensure_windows_path_entry(str(before_raw.get("userPath") or ""), WINDOWS_GIT_CMD_DIR)
    if not changed:
        return {
            "ok": bool(before.get("ok")),
            "changed": False,
            "reason": "already_current" if before.get("ok") else "path_entry_present_but_git_unavailable",
            "before": before,
            "after": before,
        }
    if not before.get("git_binary_exists"):
        return {"ok": False, "changed": False, "reason": "windows_git_binary_missing", "before": before}
    backup = _backup_windows_environment_path(before_raw)
    if not backup.get("ok"):
        return {"ok": False, "changed": False, "reason": "windows_environment_backup_failed", "before": before, "backup": backup}
    operation = _set_windows_user_path(updated)
    if not operation.get("ok"):
        return {"ok": False, "changed": False, "reason": "windows_user_path_write_failed", "before": before, "backup": backup, "operation": operation}
    after = windows_git_path_state(_windows_path_snapshot())
    accepted = bool(after.get("ok") and after.get("user_entry_present") and after.get("user_entry_count") == 1)
    return {
        "ok": accepted,
        "changed": True,
        "reason": "configured" if accepted else "windows_user_path_readback_failed",
        "before": before,
        "after": after,
        "backup": {"ok": backup.get("ok"), "manifest_paths": backup.get("manifest_paths", [])},
        "operation": operation,
        "restart_required": True,
    }


def _canonical_windows_git_path(value: str) -> str:
    return value.strip().replace("\\", "/").rstrip("/").casefold()


def _desktop_project_safe_directory() -> str:
    return WSL_DESKTOP_PROJECT_ROOT.replace("\\", "/")


def desktop_project_git_state() -> dict[str, Any]:
    """Probe the same Windows Git boundary used by Desktop project metadata."""

    expected = _desktop_project_safe_directory()
    git_executable = _windows_git_executable()
    path_state = windows_git_path_state()
    result: dict[str, Any] = {
        "schema": f"{SCHEMA}.desktop_project_git_state",
        "ok": False,
        "recognized": False,
        "git_executable": str(git_executable or ""),
        "safe_directory_expected": expected,
        "safe_directory_config": str(DEFAULT_WINDOWS_GIT_CONFIG),
        "safe_directory_configured": False,
        "path": path_state,
        "path_discoverable": bool(path_state.get("path_discoverable")),
    }
    if git_executable is None:
        return {**result, "reason": "windows_git_missing"}

    config = _run([str(git_executable), "config", "--global", "--get-all", "safe.directory"])
    safe_directories = [line.strip() for line in str(config.get("stdout") or "").splitlines() if line.strip()]
    expected_key = _canonical_windows_git_path(expected)
    safe_directory_configured = any(
        _canonical_windows_git_path(value) == expected_key
        for value in safe_directories
    )
    probe = _run([
        str(git_executable),
        "-C",
    ...
    lines = [line.strip() for line in str(probe.get("stdout") or "").splitlines() if line.strip()]
    recognized = bool(
        probe.get("ok")
        and len(lines) == 2
        and _canonical_windows_git_path(lines[0]) == expected_key
    )
    stderr = str(probe.get("stderr") or "")
    if recognized and path_state.get("ok"):
        reason = "recognized"
    elif recognized:
        reason = "windows_git_not_discoverable_from_path"
    elif "dubious ownership" in stderr.casefold():
        reason = "unsafe_repository_owner"
    else:
        reason = "metadata_probe_failed"
    return {
        **result,
        "ok": bool(recognized and path_state.get("ok")),
        "recognized": recognized,
        "reason": reason,
        "safe_directory_configured": safe_directory_configured,
        "safe_directories": safe_directories,
        "root": lines[0] if lines else "",
        "git_common_dir": lines[1] if len(lines) > 1 else "",
        "probe_returncode": probe.get("returncode"),
        "probe_stderr": stderr[:1200],
    }


def ensure_desktop_project_git_state() -> dict[str, Any]:
    """Idempotently admit only the exact WSL Work Git root to Windows Git."""

    path_acceptance = ensure_windows_git_path()
    before = desktop_project_git_state()
    if not path_acceptance.get("ok"):
        return {
            "schema": f"{SCHEMA}.ensure_desktop_project_git_state",
            "ok": False,
            "changed": bool(path_acceptance.get("changed")),
            "reason": "windows_git_path_acceptance_failed",
            "before": before,
            "path_acceptance": path_acceptance,
        }
    git_executable = _windows_git_executable()
    if git_executable is None:
        return {
            ...
            "ok": False,
            "changed": False,
            "reason": "windows_git_missing",
            "before": before,
        }
    changed = bool(path_acceptance.get("changed"))
    operation: dict[str, Any] = {"ok": True, "returncode": 0, "stderr": ""}
    if not before.get("safe_directory_configured"):
        operation = _run([
            str(git_executable),
            "config",
            "--global",
            "--add",
            "safe.directory",
            _desktop_project_safe_directory(),
        ])
        changed = bool(changed or operation.get("ok"))
        if not operation.get("ok"):
            return {
                "schema": f"{SCHEMA}.ensure_desktop_project_git_state",
                "ok": False,
                "changed": False,
                "reason": "safe_directory_write_failed",
                "before": before,
                "operation": operation,
            }
    after = desktop_project_git_state()
    accepted = bool(after.get("recognized") and after.get("safe_directory_configured") and after.get("path_discoverable"))
    return {
        "schema": f"{SCHEMA}.ensure_desktop_project_git_state",
        "ok": accepted,
        "changed": changed,
        "reason": "configured" if changed and accepted else "already_current" if accepted else "git_acceptance_failed",
        "before": before,
        "after": after,
        "path_acceptance": path_acceptance,
        "restart_required": bool(path_acceptance.get("restart_required")),
        "operation": {
            "ok": operation.get("ok"),
            "returncode": operation.get("returncode"),
            "stderr": str(operation.get("stderr") or "")[:1200],
        },
    }


def _desktop_project_classifier_expression() -> str:
    payload = json.dumps({"cwd": WSL_DESKTOP_PROJECT_ROOT}, ensure_ascii=False, separators=(",", ":"))
    return r"""
(async () => {
  const payload = __PAYLOAD__;
  const bridge = window.electronBridge;
  if (!bridge || typeof bridge.sendWorkerMessageFromView !== 'function' || typeof bridge.subscribeToWorkerMessages !== 'function') {
    return {ok:false, reason:'desktop_git_worker_bridge_unavailable'};
  }
  const call = (method, params) => new Promise((resolve) => {
    const id = `wsl-workspace-owner-${method}-${Date.now()}-${Math.random()}`;
    let completed = false;
    ...
    unsubscribe = bridge.subscribeToWorkerMessages('git', (message) => {
      if (message && message.response && message.response.id === id) finish(message.response.result);
    });
    bridge.sendWorkerMessageFromView('git', {
      type:'worker-request',
      workerId:'git',
      request:{enqueuedAtMs:Date.now(), id, method, params},
    }).catch((error) => finish({type:'error', error:String(error)}));
    setTimeout(() => finish({type:'error', error:'desktop_git_worker_timeout'}), 5000);
  });
  const operationSource = 'local_conversation_git_actions';
  const availabilityBefore = await call('availability', {operationSource});
  const metadata = await call('stable-metadata', {
    cwd:payload.cwd,
    operationSource,
  });
  await new Promise((resolve) => setTimeout(resolve, 250));
  const availabilityAfter = await call('availability', {operationSource});
  return {ok:true, operationSource, cwd:payload.cwd, availabilityBefore, metadata, availabilityAfter};
})()
""".replace("__PAYLOAD__", payload)


def _worker_response_summary(response: dict[str, Any], *, role: str) -> dict[str, Any]:
    response_type = str(response.get("type") or "missing")
    ...
    if role == "availability" and isinstance(value, dict):
        summary["available"] = bool(value.get("available"))
    if role == "metadata" and isinstance(value, dict):
        summary["root"] = str(value.get("root") or "")[:512]
        summary["common_dir"] = str(value.get("commonDir") or "")[:512]
    ...
    if error is not None:
        ...
        elif any(token in lowered for token in ("enoent", "not recognized", "cannot find", "failed to spawn")):
            failure_class = "process_launch_or_path"
        elif any(token in lowered for token in ("protocol", "method", "worker-request")):
            failure_class = "worker_protocol"
        elif any(token in lowered for token in ("cwd", "repository", "worktree")):
            failure_class = "cwd_or_repository"
        else:
            failure_class = "worker_error"
        summary["failure_class"] = failure_class
        summary["error"] = error_text[:1200]
        summary["error_truncated"] = len(error_text) > 1200
    return summary


def _desktop_launch_environment_summary(path: Path) -> dict[str, Any]:
    payload = _read_json_object(path)
    base = {
        "schema": f"{SCHEMA}.desktop_launch_environment_summary",
        "available": bool(payload),
        "evidence_ref": str(path),
    }
    if not payload:
        return {**base, "ready": False, "reason": "launch_environment_receipt_missing"}
    if payload.get("schema") != "codex.desktop.launch_environment.v1":
        return {**base, "ready": False, "reason": "launch_environment_receipt_schema_invalid"}
    cdp_acceptance = payload.get("cdp_acceptance") if isinstance(payload.get("cdp_acceptance"), dict) else {}
    cdp_reason = str(cdp_acceptance.get("reason") or "")[:120]
    return {
        **base,
        "ready": bool(payload.get("ready")),
        "reason": "ready" if payload.get("ready") else "launch_environment_not_ready",
        ...
        "desktop_pid": int(payload.get("desktop_pid") or 0),
        "entry_count": int(payload.get("entry_count") or 0),
        "rejected_ambiguous_entry_count": int(payload.get("rejected_ambiguous_entry_count") or 0),
        "preferred_git_directory": str(payload.get("preferred_git_directory") or "")[:512],
        "git_path_present": bool(payload.get("git_path_present")),
        "resolved_git": str(payload.get("resolved_git") or "")[:512],
        "path_input_signature": str(payload.get("path_input_signature") or "")[:128],
        "cdp": {
            "observed": bool(cdp_acceptance),
            "ready": bool(cdp_acceptance.get("ready")),
            "reason": cdp_reason,
            "port": int(cdp_acceptance.get("port") or 0),
            "started_process_id": int(cdp_acceptance.get("started_process_id") or 0),
            "expected_launch_argument_observed": bool(cdp_acceptance.get("expected_launch_argument_observed")),
            "started_process_alive": bool(cdp_acceptance.get("started_process_alive")),
            "listener_present": bool(cdp_acceptance.get("listener_present")),
            "listener_owner_matches_started_pid": bool(cdp_acceptance.get("listener_owner_matches_started_pid")),
            "started_process_elevated": str(cdp_acceptance.get("started_process_elevated") or "")[:32],
            "listener_owner_elevated": str(cdp_acceptance.get("listener_owner_elevated") or "")[:32],
            "http_ready": bool(cdp_acceptance.get("http_ready")),
        },
    }


def _desktop_cdp_degradation_reason(launch_environment: dict[str, Any]) -> str:
    cdp = launch_environment.get("cdp") if isinstance(launch_environment.get("cdp"), dict) else {}
    if not cdp.get("observed") or cdp.get("ready"):
        return ""
    reason = str(cdp.get("reason") or "")
    if reason == "cdp_launch_argument_not_observed" and not cdp.get("listener_present"):
        return "desktop_cdp_listener_missing_launch_argument_not_observed"
    return f"desktop_{reason}" if reason else "desktop_cdp_launch_acceptance_failed"


def desktop_project_classifier_state(
    launch_environment_path: Path = DEFAULT_DESKTOP_LAUNCH_ENVIRONMENT_STATE,
) -> dict[str, Any]:
    launch_environment = _desktop_launch_environment_summary(launch_environment_path)
    port, ws_url, pages, reason = codex_desktop_model_runtime._find_codex_page()
    base = {
        "schema": f"{SCHEMA}.desktop_project_classifier_state",
        "ok": False,
    ...
    if not ws_url:
        return {
            **base,
            "reason": _desktop_cdp_degradation_reason(launch_environment) or reason or "desktop_not_running",
            "probe": {"evidence_ref": str(launch_environment_path)},
    ...
    try:
        ...
        result = client.evaluate(_desktop_project_classifier_expression())
    except Exception as exc:
        return {**base, "reason": "desktop_git_worker_probe_failed", "error": type(exc).__name__}
    finally:
        if client is not None:
            client.close()
    result = result if isinstance(result, dict) else {}
    availability_before = result.get("availabilityBefore") if isinstance(result.get("availabilityBefore"), dict) else {}
    availability_after = result.get("availabilityAfter") if isinstance(result.get("availabilityAfter"), dict) else {}
    if not availability_before and isinstance(result.get("availability"), dict):
        availability_before = result.get("availability")
    if not availability_after:
        availability_after = availability_before
    metadata = result.get("metadata") if isinstance(result.get("metadata"), dict) else {}
    availability_before_value = availability_before.get("value") if availability_before.get("type") == "ok" and isinstance(availability_before.get("value"), dict) else {}
    availability_after_value = availability_after.get("value") if availability_after.get("type") == "ok" and isinstance(availability_after.get("value"), dict) else {}
    metadata_value = metadata.get("value") if metadata.get("type") == "ok" and isinstance(metadata.get("value"), dict) else None
    worker_available_before = bool(availability_before_value.get("available"))
    worker_available = bool(availability_after_value.get("available"))
    root = str(metadata_value.get("root") or "") if metadata_value else ""
    normalized_root = root.replace("\\", "/").rstrip("/").lower()
    expected_roots = {
        WSL_WORKSPACE_ROOT.replace("\\", "/").rstrip("/").lower(),
        WSL_DESKTOP_PROJECT_ROOT.replace("\\", "/").rstrip("/").lower(),
        _desktop_project_safe_directory().replace("\\", "/").rstrip("/").lower(),
    }
    metadata_root_matches = bool(root and normalized_root in expected_roots)
    # Availability is a worker lifecycle/cache signal. A successful
    # stable-metadata response for the exact Work Git root is authoritative;
    # retain availability separately as an advisory.
    metadata_ready = bool(metadata_value and metadata_root_matches)
    recognized = metadata_ready
    worker_responses = {
        "availability_before": _worker_response_summary(availability_before, role="availability"),
        "metadata": _worker_response_summary(metadata, role="metadata"),
        "availability_after": _worker_response_summary(availability_after, role="availability"),
    }
    if recognized and not worker_available:
        state_reason = "metadata_ready_worker_availability_advisory"
    elif recognized:
        state_reason = "recognized"
    elif metadata_value and not metadata_root_matches:
        state_reason = "stable_metadata_root_mismatch"
    elif metadata_ready and not worker_available:
        state_reason = "metadata_ready_worker_availability_advisory"
    elif not worker_available and not launch_environment.get("available"):
        state_reason = "desktop_git_worker_unavailable_launch_receipt_missing"
    elif not worker_available and not launch_environment.get("git_path_present"):
        state_reason = "desktop_launch_environment_git_missing"
    elif not worker_available and _desktop_cdp_degradation_reason(launch_environment):
        state_reason = _desktop_cdp_degradation_reason(launch_environment)
    elif not worker_available:
        state_reason = "desktop_git_worker_unavailable_after_ready_launch"
    else:
        state_reason = "stable_metadata_absent"
    return {
        **base,
        "ok": bool(result.get("ok") and recognized),
        "worker_available": worker_available,
        "worker_available_before": worker_available_before,
        "recognized": recognized,
        "metadata_ready": metadata_ready,
        "root": root,
        "metadata_root_matches": metadata_root_matches,
        "reason": state_reason,
        "probe": {
            "operation_source": str(result.get("operationSource") or "")[:120],
            "cwd": str(result.get("cwd") or "")[:512],
            "evidence_ref": str(launch_environment_path),
        },
...
def desktop_project_snapshot(global_state_path: Path = DEFAULT_DESKTOP_GLOBAL_STATE) -> dict[str, Any]:
    ...
    try:
        state = json.loads(global_state_path.read_text(encoding="utf-8-sig"))
    ...
    # Desktop may reorder project-order after restart (for example when a
    # remote conversation was opened).  That is a preference drift, not a
    # missing project registration.  Keep it visible through
    # projection_required while judging registration from the canonical entry.
    registered = bool(
        isinstance(project, dict)
        and project.get("name") == WSL_DESKTOP_PROJECT_NAME
        and roots == [WSL_DESKTOP_PROJECT_ROOT]
    )
    git_state = desktop_project_git_state()
    classifier = desktop_project_classifier_state()
    git_ready = bool(git_state.get("recognized") and git_state.get("path_discoverable", True))
    classifier_ready = bool(classifier.get("recognized"))
    ready = bool(registered and git_ready and classifier_ready)
    if not registered:
        reason = "desktop_project_projection_required"
    elif not git_ready:
        reason = "desktop_git_metadata_unavailable"
    elif not classifier_ready:
        reason = "desktop_classifier_rejected_git"
    else:
        reason = "registered_and_git_ready"
    return {
        **result,
        "ok": True,
        ...
        "git": git_state,
        "git_recognized": bool(git_state.get("recognized")),
        "desktop_classifier": classifier,
...
def _persist_canonical_desktop_project_state(global_state_path: Path) -> dict[str, Any]:
    """Persist the WSL project entry and active-project selection atomically."""
    persistence: dict[str, Any] = {
        "attempted": True,
        "changed": False,
        "reason": "canonical_project_state_already_current",
        "evidence_ref": str(global_state_path),
        "changed_fields": [],
    }
    try:
        state = json.loads(global_state_path.read_text(encoding="utf-8-sig"))
        if not isinstance(state, dict):
            raise ValueError("global_state_not_object")
        projection = ensure_wsl_desktop_project(state)
        expected_selected = {
            "type": "local",
            "projectId": str(projection.get("project_id") or ""),
        }
        if state.get("selected-project") != expected_selected:
            state["selected-project"] = expected_selected
            projection["changed"] = True
            projection["changed_fields"] = [
                *(projection.get("changed_fields") or []),
                "selected-project.wsl",
            ]
        expected_active_roots = [WSL_DESKTOP_PROJECT_ROOT]
        if state.get("active-workspace-roots") != expected_active_roots:
            state["active-workspace-roots"] = expected_active_roots
            projection["changed"] = True
            projection["changed_fields"] = [
                *(projection.get("changed_fields") or []),
                "active-workspace-roots.wsl",
            ]
        if projection.get("changed"):
            _write_json_object_atomic(global_state_path, state)
            persistence.update({
                "changed": True,
                "reason": "canonical_project_state_persisted",
                "changed_fields": list(projection.get("changed_fields") or [])[:24],
            })
    except (OSError, ValueError, json.JSONDecodeError) as exc:
        persistence.update({
            "reason": "canonical_project_state_persist_failed",
            "error": f"{type(exc).__name__}: {exc}",
        })
    return persistence
...
def desktop_project_apply(
    *,
    ...
    backup_paths = [str(global_state_path), str(DEFAULT_WINDOWS_GIT_CONFIG)]
    ...
    git_acceptance = ensure_desktop_project_git_state()
    if not git_acceptance.get("ok"):
        return {
            "schema": f"{SCHEMA}.desktop_project_apply",
            "ok": False,
            "status": "failed",
            "reason": "desktop_git_acceptance_failed",
            "applied": bool(git_acceptance.get("changed")),
            "before": before,
            "git_acceptance": git_acceptance,
            "backup": backup,
        }

    if before.get("registered"):
        persistence = _persist_canonical_desktop_project_state(global_state_path)
        ...
        page_refresh: dict[str, Any] = {
            ...
            "requested": False,
            "skipped": True,
            "reason": "desktop_project_already_ready" if after.get("ready") else "desktop_project_git_not_ready",
        }
        if not after.get("ready") and after.get("git_recognized"):
            page_refresh = codex_desktop_model_runtime.request_desktop_page_reload(
                wait_seconds=max(0.0, float(refresh_wait_seconds)),
            )
            if page_refresh.get("ok") and page_refresh.get("requested"):
                if refresh_wait_seconds > 0:
                    time.sleep(min(0.2, float(refresh_wait_seconds)))
                after = desktop_project_snapshot(global_state_path)
                deadline = time.monotonic() + max(0.0, float(refresh_wait_seconds))
                while not after.get("ready") and time.monotonic() < deadline:
                    time.sleep(0.2)
                    after = desktop_project_snapshot(global_state_path)
        accepted = bool(after.get("ready"))
        refresh_failed = page_refresh.get("ok") is False
        refresh_unavailable = bool(page_refresh.get("skipped") and not accepted)
        restart_required = bool(
            not accepted
            and after.get("git_recognized")
            and page_refresh.get("ok")
            and page_refresh.get("requested")
        )
        return {
            "schema": f"{SCHEMA}.desktop_project_apply",
            "ok": accepted,
            "status": (
                ...
                if restart_required or refresh_unavailable
                else "failed"
            ),
            "reason": (
                "desktop_project_git_repaired"
                if accepted and git_acceptance.get("changed")
                else "desktop_project_refreshed"
                if accepted and page_refresh.get("requested")
                else "desktop_project_already_ready"
                if accepted
                else "desktop_restart_required_for_git_worker"
                if restart_required
                else "desktop_page_refresh_unavailable"
                if refresh_unavailable
                else "desktop_page_refresh_failed"
                if refresh_failed
                else "desktop_project_git_readback_failed"
            ),
            "generated_at": now_iso(),
            "applied": bool(git_acceptance.get("changed")),
            ...
            "next_action": (
                "the process-preserving page refresh was exhausted; perform one governed full Desktop restart, then require the live git worker stable-metadata probe"
                if restart_required
                else "restore the governed Desktop CDP refresh route before retrying project acceptance"
                if refresh_unavailable or refresh_failed
                else ""
            ),
            "backup": {
                "ok": backup.get("ok"),
    ...
    persistence: dict[str, Any] = {
        "attempted": False,
        "changed": False,
        "reason": "ipc_not_dispatched",
        "evidence_ref": str(global_state_path),
        "changed_fields": [],
    }
    if live_result.get("dispatched"):
        persistence = _persist_canonical_desktop_project_state(global_state_path)
    after = desktop_project_snapshot(global_state_path)
    ...
    accepted = bool(after.get("ready") and live_result.get("visibleInDom") and persistence.get("reason") in {
        "canonical_project_state_persisted",
        "canonical_project_state_already_current",
    })
    return {
        "schema": f"{SCHEMA}.desktop_project_apply",
        "ok": accepted,
        ...
        "registration_applied": bool(live_result.get("dispatched")),
        "cdp_port": port,
        "page_count": len(pages),
...
def validate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    elif desktop_project.get("registered") and desktop_project.get("git_recognized") is False:
        issues.append({
            "severity": "risk",
            "code": "wsl_desktop_project_git_unavailable",
            "detail": desktop_project.get("git", {}),
            "next_action": f"run desktop-project-apply --confirm {DESKTOP_PROJECT_CONFIRM} to configure the exact UNC safe.directory and revalidate Git metadata",
        })
    elif desktop_project.get("registered") and desktop_project.get("desktop_classifier_recognized") is False:
        issues.append({
            "severity": "risk",
            "code": "wsl_desktop_project_classifier_rejected_git",
            "detail": desktop_project.get("desktop_classifier", {}),
            "next_action": "repair the Windows Git PATH through desktop-project-apply, perform one governed full Desktop restart, and consume the live stable-metadata result",
        })
    ...
    return {
        ...
        "acceptance": {
            ...
            "desktop_project_git_recognized": bool(desktop_project.get("git_recognized")),
            "desktop_project_classifier_recognized": bool(desktop_project.get("desktop_classifier_recognized")),
            "desktop_project_ready": bool(desktop_project.get("ready")),
...
def _local_mcp_hub_process() -> Any:
    """Load the Hub owner only for Hub-specific WSL exports.

    Windows invokes ordinary mirror exports from a UNC view of this source and
    immediately delegates them to WSL.  Importing the Hub process at module
    load time would unnecessarily import its POSIX-only authorization lock in
    that Windows interpreter before the delegation boundary is reached.
    Hub-specific exports still load the same owner in their WSL runtime.
    """
    return importlib.import_module("local_mcp_hub_process")


def _host_projection_authorization() -> Any:
    """Load projection authorization only when a projection action needs it.

    Windows runs ordinary mirror exports from a UNC source view and delegates
    immediately into WSL. Loading this owner at process startup would import
    the POSIX authorization lock before that boundary. The actual projection
    path still loads the unchanged authorization owner before its first side
    effect.
    """
    return importlib.import_module("wsl_workspace_owner_authorization")
...
def mirror_export(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if kind == "local-mcp-hub-service-status":
        service_status = _local_mcp_hub_process().hub_service_status()
        return {
    ...
    if kind == "local-mcp-hub-user-unit":
        unit = _local_mcp_hub_process().hub_service_plan()
        return {
...
def main(argv: list[str] | None = None) -> int:
    ...
    sub.add_parser("desktop-restart-plan")
    sub.add_parser("desktop-restart-status")
    desktop_restart = sub.add_parser("desktop-restart-apply")
    desktop_restart.add_argument("--confirm", default="")
    ...
    host_projection_authorization = sub.add_parser("host-projection-authorization-plan")
    host_projection_authorization.add_argument("--thread-id", required=True)
    host_projection_authorization.add_argument("--assessment-json", required=True)
    host_projection_authorization.add_argument("--rollout-path", required=True)
    host_projection_authorization.add_argument("--user-message-ref", required=True)
    host_projection_authorization.add_argument("--operation-id", required=True)
    host_projection_authorization.add_argument("--workflow-semantic-hash", required=True)
    host_projection_authorization.add_argument("--authorization-state-root", default="")
    host_projection_authorization.add_argument("--include", action="append", default=[])
    ...
    host_projection.add_argument("--authorization-permit-ref", default="")
    host_projection.add_argument("--authorization-operation-id", default="")
    host_projection.add_argument("--authorization-thread-id", default="")
    host_projection.add_argument("--workflow-semantic-hash", default="")
    host_projection.add_argument("--authorization-state-root", default="")
    ...
    elif args.command == "desktop-restart-plan":
        payload = desktop_restart_request.plan()
    elif args.command == "desktop-restart-status":
        payload = desktop_restart_request.status(desktop_snapshot=desktop_project_snapshot)
    elif args.command == "desktop-restart-apply":
        payload = desktop_restart_request.apply(str(args.confirm or ""))
    elif args.command == "host-projection-plan":
        payload = host_compatibility_projection_plan(include=tuple(args.include or ()))
    elif args.command == "host-projection-authorization-plan":
        try:
            assessment = json.loads(Path(args.assessment_json).read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            payload = {"ok": False, "reason": "host_projection_assessment_unreadable"}
        else:
            payload = host_compatibility_projection_authorization_plan(
                thread_id=args.thread_id,
                assessment=assessment,
                rollout_path=args.rollout_path,
                user_message_ref=args.user_message_ref,
                operation_id=args.operation_id,
                workflow_semantic_hash=args.workflow_semantic_hash,
                include=tuple(args.include or ()),
                state_root=args.authorization_state_root or None,
            )
    elif args.command == "host-projection-apply":
        payload = host_compatibility_projection_apply(
            confirm=str(args.confirm or ""),
            include=tuple(args.include or ()),
            authorization_permit_ref=args.authorization_permit_ref,
            authorization_operation_id=args.authorization_operation_id,
            authorization_thread_id=args.authorization_thread_id,
            workflow_semantic_hash=args.workflow_semantic_hash,
            authorization_state_root=args.authorization_state_root or None,
        )
