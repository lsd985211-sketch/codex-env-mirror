...
FOCUSED_OWNER_SPECS: dict[str, dict[str, Any]] = {
    ...
    "dependency_intelligence": {
        "system": "workflow",
        "command": ["dependency_change_intelligence.py", "status", "--pending-only"],
        "severity": "risk",
        "timeout": 30,
        "source": "focused_feedback",
    },
    ...
    try:
        ...
        for system in discovered_systems:
            ...
            for item in health_commands:
                ...
                primary_command = normalized_owner_command(item.get("args", []))
                ...
                focused_command = normalized_owner_command(spec.get("command", []))
                focused_uses_primary_contract = bool(
                    focused_command and focused_command == primary_command and focused_command != command
                )
                spec.update(
                    {
                        "system": str(system),
                        ...
                        "timeout": int(
                            item.get("timeout")
                            if focused_uses_primary_contract
                            else item.get("compatibility_timeout")
                            if isinstance(compatibility_args, list) and compatibility_args
                            else item.get("timeout") or spec.get("timeout") or 90
                        ),
                        "source": (
                            "system_membership.health_commands"
                            if focused_uses_primary_contract
                            else "system_membership.health_commands.compatibility_args"
                            if isinstance(compatibility_args, list) and compatibility_args
                            else "system_membership.health_commands"
                        ),
                        "platform_scope": str(item.get("platform_scope") or spec.get("platform_scope") or "all"),
                    }
                )
    ...
    for system in owner_system_order:
        ...
        for name, spec in derived_by_system.get(system, []):
            if name in focused_names:
                # The focused command is a narrow facade, while membership owns
                # its execution budget and platform contract.
                specs[name].update(
                    {
                        key: spec[key]
                        for key in ("system", "severity", "timeout", "source", "platform_scope")
                        if key in spec
                    }
                )
                continue
...
def is_membership_meta_path(value: Any) -> bool:
    normalized = str(value or "").strip().replace("\\", "/").casefold().lstrip("./")
    candidates = {normalized}
    if normalized.startswith("workspace/"):
        candidates.add(normalized[len("workspace/") :])
    marker = "/workspace/"
    if marker in normalized:
        candidates.add(normalized.split(marker, 1)[1])
    return bool(candidates & MEMBERSHIP_META_PATHS)
...
def resolve_affected_systems(
    *,
    ...
    non_meta_changed = [
        item
        for item in changed
        if not is_membership_meta_path(item)
    ]
        ...
        owner_steps.append(
            {
                ...
                "platform_scope": spec.get("platform_scope", "all"),
                "acceptance": {
                    "owner_ok": True,
...
def owner_review_items(
    ...
    *,
    ...
    if isinstance(checks, list):
        candidates.extend(
            {
                "code": item.get("name") or "workflow_validation_check",
                "message": item.get("detail") or item.get("message") or item.get("name"),
                "severity": "risk",
            }
            for item in checks
            if isinstance(item, dict) and item.get("ok") is False
        )
    if not candidates and payload.get("ok") is False:
        candidates.append(
            {
                "code": "owner_evidence_unavailable",
                "message": f"{surface} owner reported failure but returned no structured review item.",
                "severity": "risk",
    ...
    for index, item in enumerate(candidates[:30], start=1):
        code = str(item.get("code") or item.get("name") or item.get("id") or item.get("kind") or f"item-{index}")
...
def snapshot(
    ...
    *,
    ...
    for name in owner_names:
        required_platform = str(OWNER_SPECS[name].get("platform_scope") or "all")
        if not platform_scope_matches(required_platform, current_platform):
            unordered[name] = {
                "name": name,
                "ok": True,
                "execution_state": "owner_deferred_platform_scope",
                "platform_scope": required_platform,
                "current_platform": current_platform,
                "command": OWNER_COMMANDS[name],
                "elapsed_ms": 0.0,
            }
    ...
    return {
        ...
        "owners": {
            name: {
                ...
                "platform_scope": OWNER_SPECS[name].get("platform_scope", "all"),
