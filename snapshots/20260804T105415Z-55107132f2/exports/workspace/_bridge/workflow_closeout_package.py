...
def review_card(item: dict[str, Any]) -> dict[str, Any]:
    ...
    projected = project_card(card)
    projected["digest"] = projected.pop("summary", "")
    return projected
        ...
        sections.append(
            "\n".join(
                [
                    ...
                    f"- 摘要：{digest}",
                    f"- 来源：{source}",
                    f"- 属性：{_compact_attribute_text(attributes)}",
                    f"- 审批动作：{action}",
                    f"- 必需检查：{_compact_required_checks(checks)}",
                ]
            )
        )
    if hidden_count > 0:
        sections.append(f"### 更多审批卡片\n- 未显示数量：{hidden_count}")
    return "\n\n".join(sections)
...
def build_review_summary(package: dict[str, Any], *, limit: int = 8) -> dict[str, Any]:
        ...
        if review_values:
            for review_value in unique_review_items(review_values, kind=str(pending_item.get("kind") or ""), limit=limit * 4):
                if isinstance(review_value, dict):
                    card = review_card({**review_value, "kind": pending_item.get("kind", "")})
    ...
    return {
        ...
        "display_contract": {
            ...
            "maximum_cards_per_reply": min(10, visible_limit),
            "generic_count_only_forbidden": bool(cards),
        },
...
def build_closeout_package(ctx: dict[str, Any]) -> dict[str, Any]:
    ...
    terminal = ctx.get("terminal_convergence") if isinstance(ctx.get("terminal_convergence"), dict) else finalization.get("terminal_convergence", {})
    terminal = terminal if isinstance(terminal, dict) else {}
    terminal_projection = {
        "schema": "workflow_closeout.terminal_convergence.v1",
        "convergence_id": terminal.get("convergence_id", ""),
        "intent_id": terminal.get("intent_id", ""),
        "terminal_source_signature": terminal.get("terminal_source_signature", ""),
        "current_phase": terminal.get("current_phase", ""),
        "next_action": terminal.get("next_action", {}),
        "completed_action_ids": list(terminal.get("completed_action_ids", []) or []),
        "invalidated_action_ids": list(terminal.get("invalidated_action_ids", []) or []),
        "receipt_refs": list(terminal.get("receipt_refs", []) or []),
    ...
    package = {
        ...
        "proposals": ctx["proposals"],
        "finalization": ctx.get("finalization", {}),
        "terminal_convergence": terminal_projection,
        "persistence_gates": {
