...
class CodexEnvironmentMirrorTests(unittest.TestCase):
    def test_direct_cli_bounds_large_status_and_keeps_next_action(self) -> None:
        payload = {
        ...
        output = io.StringIO()
        with patch.object(mirror, "execute", return_value=payload), redirect_stdout(output):
            exit_code = mirror.main(["status"])
        ...
        self.assertEqual(projected["raw_result_ref"], "command:python _bridge/codex_environment_mirror.py status")
        self.assertLess(len(output.getvalue().encode("utf-8")), 16 * 1024)

    def test_direct_cli_keeps_failure_reason_and_next_action(self) -> None:
        payload = {
            "schema": "codex_environment_mirror.status.v1",
            "ok": False,
            "reason": "source_assets_changed",
            "next_action": "review the declared drift owner",
            "issues": [{"code": "source_assets_changed", "detail": "x" * 40000}],
        }
        output = io.StringIO()
        with patch.object(mirror, "execute", return_value=payload), redirect_stdout(output):
            exit_code = mirror.main(["status"])
        ...
        self.assertEqual(projected["output_mode"], "failure_bounded")
        self.assertEqual(projected["reason"], payload["reason"])
        self.assertEqual(projected["next_action"], payload["next_action"])
    ...
    def refresh_with_grant(self, *args: object, **kwargs: object) -> dict:
        with patch.object(mirror.refresh_authorization, "consume", return_value={"ok": True}), \
                patch.object(mirror.refresh_authorization, "effect_started", return_value={"ok": True}), \
                patch.object(mirror.refresh_authorization, "effect_finished", return_value={"ok": True, "status": "completed"}):
            return mirror.refresh(
                *args,
                authorization_grant_ref="scoped-authorization:grant:test",
                authorization_thread_id="thread-test",
                authorization_operation_id="mirror-refresh-test",
                **kwargs,
            )
    ...
    def test_refresh_requires_grant_after_mechanical_confirmation(self) -> None:
        with patch.object(mirror, "run_mirror") as owner:
            payload = mirror.refresh(mirror.REFRESH_CONFIRMATION)

        self.assertFalse(payload["ok"])
        self.assertEqual(payload["reason"], "scoped_authorization_grant_required")
        owner.assert_not_called()

    def test_refresh_consumes_grant_and_records_readback(self) -> None:
        result = {"schema": "codex_environment_mirror.refresh.v1", "ok": True, "snapshot_id": "snapshot-a"}
        with patch.object(mirror.refresh_authorization, "consume", return_value={"ok": True}) as consume, \
                patch.object(mirror.refresh_authorization, "effect_started", return_value={"ok": True}), \
                patch.object(mirror.refresh_authorization, "effect_finished", return_value={"ok": True, "status": "completed"}) as finished, \
                patch.object(mirror, "_refresh_unlocked", return_value=result):
            payload = mirror.refresh(
                mirror.REFRESH_CONFIRMATION, authorization_grant_ref="scoped-authorization:grant:test",
                authorization_thread_id="thread-test", authorization_operation_id="refresh-test",
            )

        self.assertTrue(payload["ok"])
        consume.assert_called_once()
        finished.assert_called_once()
        self.assertEqual(payload["authorization_effect"]["status"], "completed")
    ...
    def test_remote_tag_commit_accepts_matching_annotated_tag_object(self) -> None:
        ...
        with patch.object(mirror, "git_result", side_effect=git), patch.object(
            mirror, "git_network_attempts_for_remote", return_value=([{"name": "selected_route", "env": {}}], {})
        ), patch.object(
            mirror, "git_transport_result", return_value={"ok": True, "stdout": f"{tag_object}\trefs/tags/{tag}"}
        ):
            self.assertEqual(mirror.remote_tag_commit("origin", tag, commit), commit)

    def test_push_tag_advances_to_http11_and_reads_back_ambiguous_success(self) -> None:
        tag = "seed-v3.3.3"
        commit = "a" * 40
        tag_object = "b" * 40
        attempts = [
            {"name": "selected_route", "env": {"HTTPS_PROXY": "http://127.0.0.1:7897"}},
            {"name": "selected_route_http_1_1", "env": {"GIT_CONFIG_VALUE_1": "HTTP/1.1"}},
        ]
        state = {"remote": False, "pushes": 0}
        ...
        def transport(args: list[str], *, extra_env: dict[str, str] | None = None, **_kwargs: object) -> dict[str, object]:
            if args[0] == "ls-remote":
                return {"ok": True, "stdout": f"{tag_object}\trefs/tags/{tag}" if state["remote"] else ""}
            if args[0] == "push":
                state["pushes"] += 1
                if not extra_env.get("GIT_CONFIG_VALUE_1"):
                    return {"ok": False, "stderr_tail": "Recv failure: Connection reset"}
                state["remote"] = True
                return {"ok": True}
            self.fail(f"unexpected transport command: {args}")

        with patch.object(mirror, "git_result", side_effect=git), patch.object(
            mirror, "git_network_attempts_for_remote", return_value=(attempts, {"ok": True})
        ), patch.object(mirror, "git_transport_result", side_effect=transport):
            payload = mirror.push_tag_receipt("origin", tag, commit)

        self.assertTrue(payload["ok"], payload)
        self.assertEqual(state["pushes"], 2)
        self.assertEqual(payload["attempts"][-1]["name"], "selected_route_http_1_1")

    def test_remote_ref_read_advances_without_windows_git_transport(self) -> None:
        attempts = [
            {"name": "selected_route", "env": {}},
            {"name": "selected_route_http_1_1", "env": {"GIT_CONFIG_VALUE_0": "HTTP/1.1"}},
        ]
        transport_calls: list[dict[str, str]] = []

        def transport(args: list[str], *, extra_env: dict[str, str] | None = None, **kwargs: object) -> dict[str, object]:
            self.assertEqual(args, ["ls-remote", "--heads", "origin", "main"])
            transport_calls.append({**(extra_env or {}), "prefer": str(kwargs.get("prefer_wsl_native"))})
            if not extra_env.get("GIT_CONFIG_VALUE_0"):
                return {"ok": False, "stderr_tail": "connection reset"}
            return {"ok": True, "stdout": f"{'a' * 40}\trefs/heads/main"}

        with patch.object(mirror, "git_network_attempts_for_remote", return_value=(attempts, {"ok": True})), patch.object(
            mirror, "git_transport_result", side_effect=transport
        ), patch.object(mirror, "git_result") as windows_git:
            payload = mirror.remote_ref_commit(
                "origin",
                ["--heads", "main"],
        ...
        self.assertTrue(all(call["prefer"] == "True" for call in transport_calls))
        windows_git.assert_not_called()
    ...
    def test_refresh_lock_rejects_concurrent_writer(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.dict(os.environ, {"CODEX_ENV_MIRROR_ROOT": str(root), "CODEX_ENV_MIRROR_RUNTIME_ROOT": str(root / "runtime")}), patch.object(mirror, "_refresh_unlocked") as unlocked:
                payload = self.refresh_with_grant(mirror.REFRESH_CONFIRMATION)
            self.assertFalse(payload["ok"])
    ...
    def test_publish_auto_changed_paths_uses_directed_refresh_when_plan_is_safe(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir, \
                ...
                patch.object(mirror, "reusable_committed_snapshot_for_publish", return_value={"ok": False}), \
                patch.object(mirror, "drift_plan", return_value={"ok": True, "plan_digest": "plan-1", "source_signature": "source-1", "rows": []}) as drift, \
        ...
        reconcile.assert_called_once_with(
            ...
            scope={"ok": True, "full_rebuild_required": False},
            source_authority=source_authority,
            frozen_plan={"ok": True, "plan_digest": "plan-1", "source_signature": "source-1", "rows": []},
        )
    ...
    def test_publication_preflight_reuses_one_frozen_live_plan(self) -> None:
        authority = {"ok": True, "work_git": {"worktree_head": "a" * 40, "bare_head": "a" * 40}}
        scope = {"ok": True, "full_rebuild_required": False, "changed_paths": ["workspace/_bridge/a.py"], "affected_source_plan": {"ok": True, "full_rebuild_required": False}}
        plan = {"ok": True, "plan_digest": "plan-1", "source_signature": "source-1", "rows": []}
        reconciliation = {"ok": True, "refresh_preflight": {"schema": "codex_environment_mirror.refresh_preflight.v1", "ok": True}}
        with tempfile.TemporaryDirectory() as temp_dir, \
                patch.dict(os.environ, {"CODEX_ENV_MIRROR_RUNTIME_ROOT": temp_dir}), \
                patch.object(mirror, "work_git_release_gate", return_value=authority), \
                patch.object(mirror, "publish_refresh_scope", return_value=scope), \
                patch.object(mirror, "drift_plan", return_value=plan) as drift, \
                patch.object(mirror, "reconcile_declared_source_drift", return_value=reconciliation) as reconcile, \
                patch.object(mirror, "_current_work_git_head", return_value="a" * 40), \
                patch.object(mirror, "latest_snapshot_id", return_value="snapshot-1"), \
                patch.object(mirror, "validation_source_signature", return_value="source-1"):
            first = mirror.publication_preflight(["workspace/_bridge/a.py"], source_authority=authority, refresh_scope=scope)
            second = mirror.publication_preflight(["workspace/_bridge/a.py"], source_authority=authority, refresh_scope=scope)

        self.assertTrue(first["ok"], first)
        self.assertTrue(second["reused"])
        drift.assert_called_once_with(affected=scope["affected_source_plan"])
        reconcile.assert_called_once()

    def test_publication_preflight_consumes_fresh_global_review_without_second_live_drift_scan(self) -> None:
        authority = {"ok": True, "work_git": {"worktree_head": "a" * 40, "bare_head": "a" * 40}}
        scope = {
            "ok": True, "full_rebuild_required": False,
            "changed_paths": ["workspace/_bridge/a.py"],
            "affected_source_plan": {"ok": True, "full_rebuild_required": False},
        }
        reviewed = {
            "ok": True, "refresh_allowed": True, "review_consumed": True,
            "review_reason": "receipt_reused_within_preflight_ttl",
            "plan_digest": "global-plan", "source_signature": "source-1", "rows": [],
        }
        reconciliation = {"ok": True, "refresh_preflight": {"schema": "codex_environment_mirror.refresh_preflight.v1", "ok": True}}
        with tempfile.TemporaryDirectory() as temp_dir, \
                patch.dict(os.environ, {"CODEX_ENV_MIRROR_RUNTIME_ROOT": temp_dir}), \
                patch.object(mirror, "consume_fresh_drift_review_for_publish", return_value=reviewed) as consume, \
                patch.object(mirror, "drift_plan") as drift, \
                patch.object(mirror, "reconcile_declared_source_drift", return_value=reconciliation) as reconcile:
            payload = mirror.publication_preflight(
                ["workspace/_bridge/a.py"], source_authority=authority, refresh_scope=scope
            )

        self.assertTrue(payload["ok"], payload)
        self.assertTrue(payload["preflight_validation_reused"])
        self.assertEqual(payload["preflight_validation_reason"], "receipt_reused_within_preflight_ttl")
        consume.assert_called_once_with(authority)
        drift.assert_not_called()
        self.assertEqual(reconcile.call_args.kwargs["frozen_plan"], reviewed)

    def test_publication_preflight_fails_closed_when_frozen_source_changes(self) -> None:
        authority = {"ok": True, "work_git": {"worktree_head": "a" * 40, "bare_head": "a" * 40}}
        receipt = {
            "schema": "codex_environment_mirror.publication_preflight.v1", "ok": True,
            "generated_at": mirror.now_iso(), "input_signature": "signature",
            "source_authority": authority, "frozen_source_signature": "old-source",
        }
        with tempfile.TemporaryDirectory() as temp_dir, \
                patch.dict(os.environ, {"CODEX_ENV_MIRROR_RUNTIME_ROOT": temp_dir}), \
                patch.object(mirror, "_current_work_git_head", return_value="a" * 40), \
                patch.object(mirror, "latest_snapshot_id", return_value="snapshot-1"), \
                patch.object(mirror, "validation_source_signature", return_value="new-source"), \
                patch.object(mirror, "publication_preflight_input_signature", return_value="signature"):
            mirror.publication_preflight_receipt_path().write_text(json.dumps(receipt), encoding="utf-8")
            payload = mirror.publication_preflight([], source_authority=authority)

        self.assertFalse(payload["ok"])
        self.assertEqual(payload["reason"], "dynamic_source_changed_after_preflight")
    ...
    def test_gh_result_preserves_empty_per_process_proxy_overrides(self) -> None:
        completed = type("Completed", (), {"returncode": 0, "stdout": "{}", "stderr": ""})()
        with patch.object(mirror.subprocess, "run", return_value=completed) as run:
            payload = mirror.gh_result(["api", "rate_limit"], extra_env={"HTTPS_PROXY": "", "ALL_PROXY": ""})

        self.assertTrue(payload["ok"])
        self.assertEqual(run.call_args.kwargs["env"]["HTTPS_PROXY"], "")
        self.assertEqual(run.call_args.kwargs["env"]["ALL_PROXY"], "")
    ...
    def test_remote_history_recovery_plan_fetches_reviewable_single_file_fork_without_merging(self) -> None:
        local = "a" * 40
        remote = "b" * 40
        base = "c" * 40
        recovery_ref = f"refs/codex-mirror-recovery/origin-main-{remote[:12]}"
        ...
        def git(args: list[str], **_kwargs: object) -> dict[str, object]:
            ...
            if args == ["rev-parse", "-q", "--verify", f"{recovery_ref}^{{commit}}"]:
                return {"ok": True, "stdout": remote}
            if args == ["show", "-s", "--format=%P", recovery_ref]:
                return {"ok": True, "stdout": base}
            if args == ["merge-base", "HEAD", recovery_ref]:
                return {"ok": True, "stdout": base}
            if args in (["merge-base", "--is-ancestor", recovery_ref, "HEAD"], ["merge-base", "--is-ancestor", "HEAD", recovery_ref]):
                return {"ok": False, "returncode": 1, "stdout": ""}
            if args == ["diff-tree", "--no-commit-id", "--name-status", "-r", recovery_ref]:
                return {"ok": True, "stdout": "A\tOPTIMIZATION_SPEC.md\n"}
            if args == ["show", f"{recovery_ref}:OPTIMIZATION_SPEC.md"]:
                return {"ok": True, "stdout": "# reviewed draft\n"}
            if args == ["merge-tree", "--write-tree", "HEAD", recovery_ref]:
                return {"ok": True, "stdout": "d" * 40}
            self.fail(f"unexpected git command: {args}")

        with tempfile.TemporaryDirectory() as temp_dir, \
                patch.object(mirror, "git_result", side_effect=git), \
                patch.object(mirror, "remote_ref_commit", return_value={"ok": True, "commit": remote}), \
                patch.object(mirror, "git_network_attempts_for_remote", return_value=([{"name": "selected_route", "env": {}}], {"ok": True})), \
                patch.object(mirror, "git_transport_result", return_value={"ok": True}) as transport, \
                patch.dict(os.environ, {"CODEX_ENV_MIRROR_RUNTIME_ROOT": temp_dir}):
            payload = mirror.remote_history_recovery_plan(expected_remote_head=remote)

        self.assertTrue(payload["ok"], payload)
        self.assertEqual(payload["remote_review"][0]["path"], "OPTIMIZATION_SPEC.md")
        transport.assert_called_once_with(
            ["fetch", "--no-tags", "origin", f"refs/heads/main:{recovery_ref}"],
            timeout=300, extra_env={}, prefer_wsl_native=True,
        )
        self.assertFalse(any(args and args[0] in {"push", "merge", "commit"} for args in commands))

    def test_remote_history_recovery_plan_rejects_unreviewed_remote_paths(self) -> None:
        local = "a" * 40
        remote = "b" * 40
        base = "c" * 40
        recovery_ref = f"refs/codex-mirror-recovery/origin-main-{remote[:12]}"
        ...
        def git(args: list[str], **_kwargs: object) -> dict[str, object]:
            ...
            if args in (["merge-base", "--is-ancestor", recovery_ref, "HEAD"], ["merge-base", "--is-ancestor", "HEAD", recovery_ref]):
                return {"ok": False, "returncode": 1, "stdout": ""}
            if args == ["diff-tree", "--no-commit-id", "--name-status", "-r", recovery_ref]:
                return {"ok": True, "stdout": "A\tunknown.bin\n"}
            self.fail(f"unexpected git command: {args}")
        ...
        with tempfile.TemporaryDirectory() as temp_dir, \
                ...
                patch.object(mirror, "remote_ref_commit", return_value={"ok": True, "commit": remote}), \
                patch.object(mirror, "git_network_attempts_for_remote", return_value=([{"name": "selected_route", "env": {}}], {"ok": True})), \
                patch.object(mirror, "git_transport_result", return_value={"ok": True}), \
                patch.dict(os.environ, {"CODEX_ENV_MIRROR_RUNTIME_ROOT": temp_dir}):
            payload = mirror.remote_history_recovery_plan(expected_remote_head=remote)

        self.assertFalse(payload["ok"])
        self.assertTrue(payload["fail_closed"])
        self.assertEqual(payload["reason"], "remote_history_contains_unreviewed_changes")

    def test_remote_history_recovery_plan_accepts_reviewed_markdown_within_64kib_limit(self) -> None:
        local = "a" * 40
        remote = "b" * 40
        base = "c" * 40
        ...
        self.assertTrue(payload["ok"], payload)
        self.assertLess(payload["remote_review"][0]["byte_count"], mirror.REMOTE_HISTORY_RECOVERY_MAX_BYTES)

    def test_non_fast_forward_recovery_retries_push_without_refresh(self) -> None:
        failure = {"ok": False, "remote_head": "b" * 40, "attempts": [{"error": "rejected non-fast-forward (fetch first)"}]}
        plan = {"schema": mirror.REMOTE_HISTORY_RECOVERY_SCHEMA, "ok": True, "remote": "origin", "branch": "main", "local_head": "a" * 40, "remote_head": "b" * 40, "recovery_ref": "refs/recovery", "merge_tree": "c" * 40}
        merged = {"ok": True, "head": "d" * 40}
        resumed = {"ok": True, "push": {"returncode": 0}, "remote_verification": {"ok": True}}
        with patch.object(mirror, "remote_history_recovery_plan", return_value=plan) as planned, \
                patch.object(mirror, "apply_remote_history_recovery_plan", return_value=merged), \
                patch.object(mirror, "push_receipt", return_value=resumed) as pushed:
            payload = mirror._resume_push_after_non_fast_forward(failure, remote="origin", branch="main")

        self.assertTrue(payload["ok"])
        planned.assert_called_once_with(remote="origin", branch="main", expected_remote_head="b" * 40)
        pushed.assert_called_once_with(remote="origin", branch="main", allow_remote_history_recovery=False)
        self.assertEqual(payload["reason"], "reviewed_remote_history_merged_then_pushed")
        self.assertEqual(payload["remote_history_recovery"]["initial_push_failure"], failure)

    def test_push_stops_transport_retries_after_confirmed_non_fast_forward(self) -> None:
        head = "b" * 40
        remote = "a" * 40
        ...
        def git(args: list[str], **_kwargs: object) -> dict[str, object]:
            ...
            if args == ["rev-parse", "HEAD"]:
                return {"ok": True, "stdout": head}
            self.fail(f"unexpected local git command: {args}")

        def transport(args: list[str], **_kwargs: object) -> dict[str, object]:
            transport_calls.append(args)
            if args == ["ls-remote", "--heads", "origin", "main"]:
                return {"ok": True, "stdout": f"{remote}\trefs/heads/main"}
            if args == ["push", "origin", "HEAD:main"]:
                return {"ok": False, "returncode": 1, "stderr_tail": "rejected non-fast-forward (fetch first)"}
            self.fail(f"unexpected transport command: {args}")

        attempts = ([{"name": "selected_route", "env": {}}, {"name": "direct", "env": {}}], {"ok": True})
        with patch.object(mirror, "git_result", side_effect=git), patch.object(
            mirror, "git_transport_result", side_effect=transport
        ), patch.object(
            mirror, "git_network_attempts_for_remote", return_value=attempts
        ), patch.object(
            mirror, "_resume_push_after_non_fast_forward", return_value={"ok": False, "reason": "blocked"}
        ) as recover:
            payload = mirror.push_receipt()

        self.assertEqual(payload["reason"], "blocked")
        self.assertEqual(sum(1 for args in transport_calls if args[0] == "push"), 1)
    ...
    def test_incremental_refresh_expands_to_full_for_unanticipated_source_drift(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.dict(os.environ, {
                ...
                payload = self.refresh_with_grant(
                    mirror.REFRESH_CONFIRMATION,
    ...
    def test_release_plan_recommends_minor_for_control_plane_change(self) -> None:
        ...
        self.assertEqual(payload["decision_authority"]["decision_class"], "recommendation")
        self.assertFalse(payload["decision_authority"]["blocking"])

    def test_release_plan_explicit_goal_overrides_skip_recommendation_into_authorization_path(self) -> None:
        def git(args: list[str], *, timeout: int = 120, extra_env: dict | None = None) -> dict:
            if args[:2] == ["tag", "--list"]:
                return {"ok": True, "stdout": "seed-v2.1.2"}
            if "diff" in args or "ls-files" in args or args == ["status", "--short"]:
                return {"ok": True, "stdout": ""}
            return {"ok": False, "stdout": ""}
        with patch.object(mirror, "git_result", side_effect=git):
            payload = mirror.release_plan(user_goal_explicit=True, requested_action="create_milestone")
        self.assertFalse(payload["release_recommended"])
        self.assertEqual(payload["default_action"], "skip_release")
        self.assertEqual(payload["effective_candidate_tag"], "seed-v2.1.3")
        self.assertTrue(payload["release_authorization_required"])
        self.assertEqual(payload["release_authorization_plan_action"], "release-authorization-plan")
        self.assertFalse(payload["decision_authority"]["blocking"])
        self.assertTrue(payload["decision_authority"]["user_override_consumed"])
    ...
    def test_restore_plan_is_bounded_and_writes_complete_artifact(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(payload["recovery_acceptance"]["state"], "snapshot_plan_pending_stage")
    ...
    def test_stage_receipt_rejects_bundle_plan_without_implemented_recovery(self) -> None:
        owner = {
            "schema": "codex_mirror.stage_receipt.v1",
            "ok": True,
            "snapshot_id": "snapshot-1",
    ...
    def test_stage_receipt_requires_complete_owner_asset_reconciliation(self) -> None:
        owner = {
            "schema": "codex_mirror.stage_receipt.v1",
            "ok": True,
            "snapshot_id": "snapshot-1",
            "asset_count": 1,
            "assets": [{"asset_id": "a", "hash_verified": True}],
            "hashes_verified": True,
            "recovery_acceptance": {
                "expected_asset_count": 2,
                "stage_asset_inventory_complete": False,
                "stage_hashes_verified": True,
            },
            "membership_guard": {},
            "activation_performed": False,
        }
        bundles = {"ok": True, "capability_restore_ready": True, "blocked_missing_bundle": []}
        with tempfile.TemporaryDirectory() as temp_dir, patch.object(
            mirror, "runtime_root", return_value=Path(temp_dir)
        ), patch.object(mirror, "mcp_bundle_readiness", return_value=bundles):
            payload = mirror.stage_receipt(owner)
        self.assertTrue(payload["ok"])
        self.assertFalse(payload["recovery_acceptance"]["stage_asset_inventory_complete"])
        self.assertFalse(payload["capability_restore_ready"])
