...
def module_candidate_facts(module: dict[str, Any]) -> dict[str, Any]:
    """Project indexed source metadata as discovery-only candidate facts."""

    raw_path = str(module.get("module") or "").replace("\\", "/").strip()
    normalized_path = raw_path.removeprefix("workspace/")
    inside_root = normalized_path.startswith("_bridge/") and ".." not in Path(normalized_path).parts
    python_module = normalized_path.endswith(".py")
    owner_cli = str(module.get("owner_cli") or "").replace("\\", "/").strip()
    entrypoints = [item for item in module.get("public_entrypoints", []) if isinstance(item, dict)]
    validation = [str(item) for item in module.get("validation", []) if str(item).strip()]
    missing_requirements = ["owner_contract", "admission_decision", "runtime_probe"]
    risk_hints = ["derived_catalog_not_permission_authority"]
    if not inside_root:
        missing_requirements.append("declared_discovery_root")
        risk_hints.append("outside_declared_discovery_root")
    if not python_module:
        missing_requirements.append("supported_module_type")
        risk_hints.append("unsupported_asset_type")
    if not owner_cli:
        missing_requirements.append("owner_entry_ref")
    if not validation:
        missing_requirements.append("validation_ref")
    if not entrypoints:
        risk_hints.append("public_entrypoint_unverified")
    candidate_id = f"module:{normalized_path or 'unknown'}"
    candidate_signature = _stable_hash(
        {
            "candidate_id": candidate_id,
            "owner_ref": owner_cli,
            "entrypoints": [
                {key: item.get(key) for key in ("name", "kind", "line")}
                for item in entrypoints[:10]
            ],
            "validation": validation[:5],
            "boundary": str(module.get("boundary") or ""),
            "state_behavior": str(module.get("state_behavior") or ""),
        }
    )
    return {
        "capability_id": candidate_id,
        "candidate_id": candidate_id,
        "name": normalized_path or raw_path,
        "source_authority": "code_maintainability.module_capability_index",
        "authority_ref": "code_maintainability.module_capability_index",
        "source_ref": normalized_path or raw_path,
        "discovery_root": "_bridge",
        "owner_ref": owner_cli if inside_root and python_module else "",
        "contract_ref": "",
        "contract_signature": "",
        "candidate_signature": candidate_signature,
        "risk_class": "discovery_only",
        "admission_state": "observed_candidate",
        "observed": bool(raw_path),
        "admitted": False,
        "callable": False,
        "probe_ref": "",
...
def build_catalog(args: argparse.Namespace) -> dict[str, Any]:
    ...
    result = {
        ...
        "source_signature": _stable_hash(
            [str(item.get("candidate_signature") or "") for item in records]
        ),
        ...
        "rules": {
            ...
            "catalog_candidate_never_implies_execution_admission": True,
...
def compact_catalog_record(record: dict[str, Any], *, task_mode: str) -> dict[str, Any]:
    base = {
        ...
        "contract_signature": record.get("contract_signature"),
        ...
        "probe_ref": record.get("probe_ref"),
        "validation_ref": record.get("validation_ref"),
        "missing_requirements": list(record.get("missing_requirements") or [])[:6],
        "risk_hints": list(record.get("risk_hints") or [])[:6],
    }
