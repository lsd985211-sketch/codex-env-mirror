...
def build_delegation(
    *,
    ...
    intent_route = build_intent_route(
        " ".join(part for part in (task, target, url, path, name) if part),
    ...
    authorization = {
        "grant_ref": str(authorization_grant_ref or "").strip(),
        "operation_id": str(authorization_operation_id or "").strip(),
        "thread_id": str(authorization_thread_id or "").strip(),
    }
    ...
    effective_owner_execution_mode = (
        "approved_write"
        if str(package_execution_mode or "").strip().lower() == "verified_auto_install"
        else owner_execution_mode
    )
    request = ResourceBrokerRequest(
        ...
        owner_execution_mode=effective_owner_execution_mode,
        metadata=metadata,
    )
...
def build_delegation_from_envelope(envelope: dict[str, Any]) -> dict[str, Any]:
    ...
    return build_delegation(
        ...
        package_execution_mode=str(package.get("execution_mode") or ""),
...
def main() -> int:
    ...
    build.add_argument("--package-execution-mode", choices=("", "inspect_then_install", "verified_auto_install"), default="")
    ...
    build.add_argument("--authorization-grant-ref", default="")
    build.add_argument("--authorization-operation-id", default="")
    build.add_argument("--authorization-thread-id", default="")
