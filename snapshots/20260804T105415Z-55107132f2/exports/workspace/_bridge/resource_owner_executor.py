...
MAX_STRUCTURED_SEARCH_DOMAINS = 8
MAX_GITHUB_ENCODED_QUERY_BYTES = 512
MAX_GITHUB_RESPONSE_BYTES = 512_000
MAX_GITHUB_PAGE_COUNT = 3


class ResponseBudgetExceeded(ValueError):
    """Raised when an owner response exceeds its declared byte budget."""
...
def supports_owner_execution(tool: str, mode: str = "read_only") -> bool:
    if mode == "read_only":
        return tool in SUPPORTED_OWNER_TOOLS
    return mode == "approved_write" and tool == "package_manager"
...
def _open_json_url(
    ...
    *,
    ...
    request = urllib.request.Request(url, headers={"Accept": accept, "User-Agent": "codex-resource-layer"})
    with opener.open(request, timeout=max(1, min(timeout, 30))) as response:
        limit = max(1_024, min(int(max_response_bytes), MAX_GITHUB_RESPONSE_BYTES))
        body = response.read(limit + 1)
        if len(body) > limit:
            raise ResponseBudgetExceeded(f"github response exceeded {limit} bytes")
        return json.loads(body.decode("utf-8", errors="replace"))


def _encoded_github_query_bytes(query: str) -> int:
    return len(urllib.parse.urlencode({"q": str(query or "")}).encode("ascii"))


def _bounded_github_query(query: str, *, max_encoded_bytes: int = MAX_GITHUB_ENCODED_QUERY_BYTES) -> str:
    normalized = str(query or "").strip()
    limit = max(64, min(int(max_encoded_bytes), MAX_GITHUB_ENCODED_QUERY_BYTES))
    encoded_bytes = _encoded_github_query_bytes(normalized)
    if encoded_bytes > limit:
        raise ValueError(f"github encoded query is {encoded_bytes} bytes; limit is {limit}")
    return normalized


def _bounded_hub_payload(payload: Any, *, max_response_bytes: int) -> Any:
    encoded = json.dumps(payload, ensure_ascii=False, separators=(",", ":")).encode("utf-8")
    if len(encoded) > max_response_bytes:
        raise ResponseBudgetExceeded(f"github hub response exceeded {max_response_bytes} bytes")
    return payload
...
def _truncate_utf8(text: str, max_bytes: int) -> str:
    ...
    if len(encoded) <= max_bytes:
        return str(text or "")
    suffix = "\n\n[resource content truncated by structured response byte limit]"
    suffix_bytes = suffix.encode("utf-8")
    body = encoded[: max(0, max_bytes - len(suffix_bytes))]
...
def _github_api_read(
    ...
    *,
    max_response_bytes: int = MAX_GITHUB_RESPONSE_BYTES,
    max_encoded_query_bytes: int = MAX_GITHUB_ENCODED_QUERY_BYTES,
) -> dict[str, Any]:
    if path.startswith("/search/") and query.get("q") is not None:
        try:
            query = {**query, "q": _bounded_github_query(str(query.get("q") or ""), max_encoded_bytes=max_encoded_query_bytes)}
        except ValueError as exc:
            return {
                "ok": False,
                "reason": "github_encoded_query_too_long",
                "error_class": "github_encoded_query_too_long",
                "detail": str(exc),
                "attempted_routes": [],
            }
    ...
    if hub_payload.get("ok"):
        try:
            bounded_payload = _bounded_hub_payload(
                hub_payload.get("result"),
                max_response_bytes=max(1_024, min(int(max_response_bytes), MAX_GITHUB_RESPONSE_BYTES)),
            )
        except ResponseBudgetExceeded as exc:
            return {
                "ok": False,
                "reason": "github_response_budget_exceeded",
                "error_class": "github_response_budget_exceeded",
                "detail": str(exc),
                "attempted_routes": [],
            }
        return {
    ...
    for candidate in _github_packages_to_try(gateway_plan, package):
        try:
            ...
            errors.append({"route_mode": str(candidate.get("route_mode") or ""), "error_class": "github_response_budget_exceeded", "reason": str(exc)})
            break
...
def _github_search_query_with_selectors(request: dict[str, Any], selectors: dict[str, Any]) -> str:
    ...
    return " ".join(part for part in (query, *qualifiers) if part).strip()
...
def execute_github_request(request: dict[str, Any], gateway_plan: dict[str, Any], timeout: int) -> dict[str, Any]:
    ...
    response_byte_limit = max(4_096, min(int(limits.get("response_bytes") or MAX_GITHUB_RESPONSE_BYTES), MAX_GITHUB_RESPONSE_BYTES))
    encoded_query_limit = max(64, min(int(limits.get("encoded_query_bytes") or MAX_GITHUB_ENCODED_QUERY_BYTES), MAX_GITHUB_ENCODED_QUERY_BYTES))
    requested_page_count = max(1, int(limits.get("page_count") or 1))
    page_count = min(requested_page_count, MAX_GITHUB_PAGE_COUNT)
    ...
    def api(operation: str, path: str, query: dict[str, Any] | None = None) -> Any:
        ...
        result = _github_api_read(
            ...
            max_encoded_query_bytes=encoded_query_limit,
        )
    ...
    content = _truncate_utf8(content, response_byte_limit)
    metadata = {
        **route_evidence,
        ...
        "limits_applied": {
            "encoded_query_bytes": encoded_query_limit,
            "page_count": page_count,
            "requested_page_count": requested_page_count,
            "item_count": item_limit,
            "content_chars": content_limit,
            "response_bytes": response_byte_limit,
...
def execute_github_repository_search(request: dict[str, Any], gateway_plan: dict[str, Any], timeout: int) -> dict[str, Any]:
    ...
    metadata = request.get("metadata") if isinstance(request.get("metadata"), dict) else {}
    encoded_query_limit = max(
        64,
        min(int(metadata.get("encoded_query_bytes") or MAX_GITHUB_ENCODED_QUERY_BYTES), MAX_GITHUB_ENCODED_QUERY_BYTES),
    )
    response_byte_limit = max(
        4_096,
        min(int(metadata.get("response_bytes") or MAX_GITHUB_RESPONSE_BYTES), MAX_GITHUB_RESPONSE_BYTES),
    )
    try:
        query = _bounded_github_query(query, max_encoded_bytes=encoded_query_limit)
    except ValueError as exc:
        return _json_result(
            ok=False,
            status="degraded",
            error_class="github_encoded_query_too_long",
            reason="github_encoded_query_too_long",
            metadata={"encoded_query_bytes": _encoded_github_query_bytes(query), "limit": encoded_query_limit, "detail": str(exc)},
            next_action="split_query_into_capability_scoped_tasks",
        )
    package = execution_package_from_gateway_plan(gateway_plan)
    if not package.get("ok"):
        return _json_result(ok=False, status="handoff_required", reason="network_package_unavailable")
    per_page = max(1, min(int(metadata.get("per_page") or 10), 25))
    hub_payload = call_hub_tool(
        "github.api",
        {
            "method": "GET",
    ...
    if hub_payload.get("ok") and isinstance(hub_payload.get("result"), dict):
        try:
            payload = _bounded_hub_payload(hub_payload["result"], max_response_bytes=response_byte_limit)
        except ResponseBudgetExceeded as exc:
            return _json_result(
                ok=False,
                status="degraded",
                error_class="github_response_budget_exceeded",
                reason="github_response_budget_exceeded",
                metadata={"limit": response_byte_limit, "detail": str(exc)},
                next_action="narrow_fields_or_item_limit",
            )
    ...
    for candidate in _github_packages_to_try(gateway_plan, package):
        ...
        try:
            payload = _open_json_url(api_url, candidate, timeout, max_response_bytes=response_byte_limit)
            break
        except ResponseBudgetExceeded as exc:
            errors.append({"route_mode": str(candidate.get("route_mode") or ""), "error_class": "github_response_budget_exceeded", "reason": str(exc)})
            break
...
def _normalized_search_domains(values: list[Any]) -> list[str]:
    domains: list[str] = []
    for value in values:
        ...
        domain = (parsed.netloc or parsed.path.split("/", 1)[0]).split(":", 1)[0].strip(". ")
        if domain and "." in domain and domain not in domains:
            domains.append(domain)
    return domains


def _search_candidates(results: list[Any], *, resource_kind: str, requested_domain: str) -> list[dict[str, Any]]:
...
def _academic_query(request: dict[str, Any]) -> str:
    return _request_text(request)[:600].strip()


def _crossref_candidates(payload: Any) -> list[dict[str, Any]]:
    message = payload.get("message") if isinstance(payload, dict) else {}
    works = message.get("items") if isinstance(message, dict) and isinstance(message.get("items"), list) else [message] if isinstance(message, dict) else []
    candidates: list[dict[str, Any]] = []
    for work in works:
        if not isinstance(work, dict):
            continue
        doi = str(work.get("DOI") or "").strip()
        landing_url = str(work.get("URL") or (f"https://doi.org/{doi}" if doi else "")).strip()
        title_values = work.get("title") if isinstance(work.get("title"), list) else []
        title = str(title_values[0] if title_values else work.get("title") or "").strip()
        issued = work.get("published") if isinstance(work.get("published"), dict) else work.get("issued") if isinstance(work.get("issued"), dict) else {}
        date_parts = issued.get("date-parts") if isinstance(issued, dict) else []
        year = date_parts[0][0] if date_parts and isinstance(date_parts[0], list) and date_parts[0] else ""
        if title or doi or landing_url:
            candidates.append({
                "source": "crossref",
...
def _openalex_candidates(payload: Any) -> list[dict[str, Any]]:
    works = payload.get("results") if isinstance(payload, dict) and isinstance(payload.get("results"), list) else [payload] if isinstance(payload, dict) else []
    candidates: list[dict[str, Any]] = []
    for work in works:
        if not isinstance(work, dict):
            continue
        ...
        landing_url = str(location.get("landing_page_url") or doi_url or work.get("id") or "").strip()
        open_access_url = str(open_access.get("oa_url") or location.get("pdf_url") or "").strip()
        title = str(work.get("title") or "").strip()
        if title or doi or landing_url:
            candidates.append({
                "source": "openalex",
                "source_id": str(work.get("id") or doi or landing_url),
...
def execute_academic_search(request: dict[str, Any], gateway_plan: dict[str, Any], timeout: int) -> dict[str, Any]:
    query = _academic_query(request)
    if not query:
        return _normalized_json_result(ok=False, status="deferred", source="academic_search", error_class="academic_query_missing", reason="academic search requires a task, target, or name", next_action="refine_resource_delegation_and_retry")
    package = execution_package_from_gateway_plan(gateway_plan)
    if not package.get("ok"):
        return _normalized_json_result(ok=False, status="handoff_required", source="academic_search", error_class="network_package_unavailable", reason="network gateway did not provide a read-only execution package", next_action="refresh_network_route_and_retry")
    metadata = request.get("metadata") if isinstance(request.get("metadata"), dict) else {}
    limit = max(1, min(int(metadata.get("max_results") or 5), 10))
    budget = ResourceExecutionBudget.start(timeout)
    doi_match = re.search(r"\b(10\.\d{4,9}/[-._;()/:a-z0-9]+)", query, flags=re.IGNORECASE)
    doi = doi_match.group(1).rstrip(".,;)") if doi_match else ""
    if doi:
        encoded_doi = urllib.parse.quote(doi, safe="")
        endpoints = (
            ("openalex", f"https://api.openalex.org/works/https://doi.org/{encoded_doi}", _openalex_candidates),
            ("crossref", f"https://api.crossref.org/works/{encoded_doi}", _crossref_candidates),
        )
    else:
        endpoints = (
            (
                "openalex",
                "https://api.openalex.org/works?" + urllib.parse.urlencode({"search": query, "per-page": limit}),
                _openalex_candidates,
            ),
            (
                "crossref",
                "https://api.crossref.org/works?" + urllib.parse.urlencode({"query.bibliographic": query, "rows": limit}),
                _crossref_candidates,
            ),
        )
    outcomes: list[dict[str, Any]] = []
    candidates: list[dict[str, Any]] = []
    for index, (source, url, normalizer) in enumerate(endpoints):
        remaining = budget.lane_timeout_seconds(owner_cap=15, remaining_lane_count=len(endpoints) - index - 1, minimum_per_remaining=2)
        if remaining <= 0:
            outcomes.append({"source": source, "status": "budget_reserved_for_remaining_source"})
            continue
        try:
            payload = _open_json_url(url, package, remaining, accept="application/json")
            source_candidates = normalizer(payload)
            candidates.extend(source_candidates)
            outcomes.append({"source": source, "status": "completed" if source_candidates else "no_results", "result_count": len(source_candidates), "url": url})
        except (urllib.error.URLError, urllib.error.HTTPError, TimeoutError, ValueError, ResponseBudgetExceeded, json.JSONDecodeError) as exc:
            outcomes.append({"source": source, "status": "backend_unavailable", "error_class": type(exc).__name__, "reason": str(exc)[:500], "url": url})
    deduplicated_by_identity: dict[str, dict[str, Any]] = {}
    for item in candidates:
        identity = str(item.get("doi") or item.get("source_id") or item.get("url"))
        existing = deduplicated_by_identity.get(identity)
        if not existing:
            deduplicated_by_identity[identity] = {
                **item,
                "sources": [str(item.get("source") or "")],
                "source_urls": [str(item.get("url") or "")],
            }
            continue
        merged = {**item, **existing}
        merged["sources"] = list(dict.fromkeys([*(existing.get("sources") or []), str(item.get("source") or "")]))
        merged["source_urls"] = list(dict.fromkeys([*(existing.get("source_urls") or []), str(item.get("url") or "")]))
        deduplicated_by_identity[identity] = merged
    deduplicated = list(deduplicated_by_identity.values())[: limit * 2]
    metadata_payload = {
        "query": query,
        "items": deduplicated,
        ...
        "owner_execution_route": "network_gateway_read_only",
    ...
    if deduplicated:
        return _normalized_json_result(
            ...
            result_kind="academic_paper_metadata_search",
            content=json.dumps({"query": query, "candidates": deduplicated}, ensure_ascii=False),
            candidates=deduplicated,
            metadata=metadata_payload,
            next_action="consume_resource",
        )
    unavailable = [item for item in outcomes if item.get("status") == "backend_unavailable"]
    no_results = [item for item in outcomes if item.get("status") == "no_results"]
    if unavailable and not no_results:
        status, error_class, next_action = "backend_unavailable", "academic_backends_unavailable", "refresh_network_route_and_retry"
    elif unavailable:
        status, error_class, next_action = "failed", "academic_backends_partially_unavailable", "surface_backend_failure_with_source_outcomes"
    else:
        status, error_class, next_action = "no_results", "academic_backends_returned_no_results", "surface_terminal_no_results_with_route_chain"
    return _normalized_json_result(
        ok=False,
        status=status,
        source="academic_search",
        ...
        reason="academic metadata search returned no usable paper records",
        metadata=metadata_payload,
...
def execute_generic_search(request: dict[str, Any], gateway_plan: dict[str, Any], timeout: int) -> dict[str, Any]:
    ...
    if explicit_url:
        ...
        payload = call_hub_tool(
            "resource_search.extract",
            {"url": explicit_url, "format": "text_markdown"},
            timeout=max(1, min(int(timeout), 45)),
        )
        content = mcp_text_content(payload)
        if not content and isinstance(payload.get("result"), dict):
            content = mcp_text_content(payload["result"])
        if content:
            candidate = {"source_id": explicit_url, "url": explicit_url, "title": str(request.get("name") or explicit_url), "snippet": content[:500]}
            return _normalized_json_result(
                ok=True,
                status="completed",
                ...
                metadata={"items": [candidate], "preview_text": content, "source_id": explicit_url, "owner_execution_route": payload.get("hub_transport", "local_http_mcp_hub")},
                next_action="consume_resource",
            )
        return _normalized_json_result(
            ok=False,
            status="handoff_required" if payload.get("gateway_status") or payload.get("isError") else "failed",
            source="generic_search",
            error_class=str(payload.get("gateway_status") or "owner_empty_content"),
            reason="generic URL extraction returned no usable content",
            next_action="use_codex_current_turn_owner_tool" if payload.get("gateway_status") else "try_next_route",
        )
    ...
    domains = _normalized_search_domains(domain_candidates)
    bounded_domains = domains[:MAX_STRUCTURED_SEARCH_DOMAINS]
    normalization = metadata.get("query_normalization") if isinstance(metadata.get("query_normalization"), dict) else {}
    normalized_candidates = [str(item).strip() for item in normalization.get("candidates") or [] if str(item).strip()]
    search_queries = list(dict.fromkeys([*normalized_candidates, query]))
    search_lanes = [
        (domain, search_query)
        for domain in (bounded_domains or [""])
    ...
    per_lane_results = max(1, min(10, (max_results + len(search_lanes) - 1) // len(search_lanes)))
    budget = ResourceExecutionBudget.start(timeout)
    candidates: list[dict[str, Any]] = []
    domain_outcomes: list[dict[str, Any]] = []
    payloads: list[dict[str, Any]] = []
    for lane_index, (domain, search_query) in enumerate(search_lanes):
        phase_timeout = budget.lane_timeout_seconds(
            owner_cap=30,
            remaining_lane_count=max(0, len(search_lanes) - lane_index - 1),
            minimum_per_remaining=2,
        )
        if phase_timeout <= 0:
            domain_outcomes.append({
                "domain": domain,
                "query": search_query,
                "status": "budget_reserved_for_remaining_lanes",
                "result_count": 0,
                "lane_plan": budget.lane_plan(owner_cap=30, remaining_lane_count=max(0, len(search_lanes) - lane_index - 1), minimum_per_remaining=2),
            })
            continue
        arguments = {
            "query": search_query,
            ...
            "site_or_domain": domain,
            "proxy_url": str(package.get("proxy_url") or ""),
            "route_mode": str(package.get("route_mode") or ""),
            "timeout_seconds": phase_timeout,
        }
        payload = call_hub_tool(
            f"resource_search.{tool_kind}",
            arguments,
            timeout=max(1, min(phase_timeout + 2, 35)),
        )
        payloads.append(payload)
        results = payload.get("results") if isinstance(payload.get("results"), list) else []
        lane_candidates = _search_candidates(results, resource_kind=resource_kind, requested_domain=domain)
        candidates.extend(lane_candidates)
        domain_outcomes.append(
            {
                "domain": domain,
                "query": search_query,
                "status": "completed" if lane_candidates else ("no_results" if payload.get("ok") else "failed"),
                "result_count": len(lane_candidates),
                "reason": str(payload.get("reason") or payload.get("error_class") or ""),
                "error_class": str(payload.get("error_class") or ""),
                "attempted_backends": list(payload.get("attempted_backends") or []),
            }
        )
    candidates = list({str(item["source_id"]): item for item in candidates}.values())[:max_results]
    if candidates:
        successful_payload = next((item for item in payloads if item.get("ok")), {})
        return _normalized_json_result(
            ok=True,
            status="completed",
            source="generic_search",
            ...
            metadata={
                ...
                "searched_domains": bounded_domains,
                "unsearched_domains": domains[MAX_STRUCTURED_SEARCH_DOMAINS:],
                "covered_domains": list(
                    dict.fromkeys(
                        str(item.get("requested_domain") or "")
                        for item in candidates
                        if item.get("requested_domain")
                    )
                ),
                "domain_outcomes": domain_outcomes,
                "attempted_queries": search_queries,
                "owner_execution_route": successful_payload.get("hub_transport", "local_http_mcp_hub"),
                "backend": successful_payload.get("backend", "ddgs"),
                "route_mode": package.get("route_mode", ""),
            },
            next_action="consume_resource",
        )
    handoff_reasons = {"hub_unreachable", "hub_initialize_error", "hub_tool_error"}
    handoff = any(str(item.get("reason") or "") in handoff_reasons for item in payloads)
    no_results = bool(payloads) and all(
        str(item.get("status") or "") == "no_results"
        or str(item.get("error_class") or "") == "search_backends_exhausted"
        or (item.get("ok") is True and not (item.get("results") or []))
        for item in payloads
    )
    backend_unavailable = bool(payloads) and all(
        str(item.get("status") or "") == "backend_unavailable"
        or str(item.get("error_class") or "") in {"search_backends_unavailable", "no_configured_search_backend_available"}
        for item in payloads
    )
    partial_backend_unavailable = not no_results and any(
        str(item.get("status") or "") == "backend_unavailable"
        or str(item.get("error_class") or "") in {"search_backends_unavailable", "no_configured_search_backend_available"}
        for item in payloads
    )
    status = "handoff_required" if handoff else ("no_results" if no_results else ("backend_unavailable" if backend_unavailable else "failed"))
    error_class = (
        "owner_no_results"
        if no_results
        else ("search_backends_unavailable" if backend_unavailable else ("search_backends_partially_unavailable" if partial_backend_unavailable else "generic_search_failed"))
    )
    return _normalized_json_result(
        ok=False,
        status=status,
        ...
        error_class=error_class,
        reason=(
            "generic search backends were unavailable across the requested domain lanes"
            if backend_unavailable
            else "generic search returned no usable results across the requested domain lanes"
        ),
        metadata={
            "query": query,
            "requested_domains": domains,
            ...
            "attempted_backends": [
                attempt
                for item in payloads
                for attempt in (item.get("attempted_backends") or [])
                if isinstance(attempt, dict)
            ],
            "route_mode": package.get("route_mode", ""),
        },
        next_action=(
            "surface_terminal_no_results_with_route_chain"
            if status == "no_results"
            else ("refresh_network_route_and_retry" if status in {"failed", "backend_unavailable"} else "continue_resource_layer_handoff_or_attach_result")
        ),
    )
...
def execute_owner_tool(
    *,
    ...
    if not supports_owner_execution(tool, mode):
        return normalize_owner_result(_json_result(ok=False, status="handoff_required", reason="owner_tool_not_supported_for_requested_execution_mode"))
    ...
    if tool == "github":
        ...
        result = normalize_owner_result(execute_academic_search(request, gateway_plan, timeout))
...
def _request_text(request: dict[str, Any]) -> str:
    parts = [str(request.get(key) or "").strip() for key in ("task", "target", "url", "name")]
...
def validate() -> dict[str, Any]:
    ...
    return {
        ...
        "ok": bool(
            ...
            and supports_owner_execution("academic_search")
