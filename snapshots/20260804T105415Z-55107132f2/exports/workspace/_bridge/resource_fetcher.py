...
from resource_content_store import persist_content


DEFAULT_CHUNK_BYTES = 1024 * 1024
DEFAULT_POLICY_MAX_BYTES = 100 * 1024 * 1024
DEFAULT_PREVIEW_BYTES = 8192
READ_ONLY_STAGES = frozenset({"discover", "probe", "preview", "audit"})
...
def confirmation_policy_for_stage(policy: "ResourcePolicy", stage: str) -> dict[str, Any]:
    """Describe confirmation at the operation boundary, not the intent boundary.

    External and package dependencies need confirmation only when the selected
    stage can create a durable artifact or otherwise cause a side effect.  A
    discover/probe/preview/audit request is evidence gathering and remains
    read-only even when its intent is classified as a dependency.
    """

    read_only = str(stage or "").strip().lower() in READ_ONLY_STAGES
    required = bool(policy.requires_confirmation and not read_only)
    return {
        "schema": "resource_confirmation_policy.v1",
        "intent": policy.intent,
        "stage": str(stage or ""),
        "mode": "explicit_confirmation" if required else "read_only",
        "required": required,
        "side_effect": not read_only,
        "reason": (
            "materialization_or_side_effect_requires_confirmation"
            if required
            else "read_only_resource_evidence_does_not_require_confirmation"
        ),
    }
...
def validate_http_url_request(request: ResourceRequest, *, stage: str) -> ResourceResult | None:
    ...
    if any(character.isspace() or ord(character) < 32 or ord(character) == 127 for character in request.url):
        return network_error_result(
            request,
            stage=stage,
            error="url contains unencoded whitespace or control characters",
            error_type="malformed_url",
            risk_flags=("malformed_url",),
        )
...
def decision_result(
    ...
    *,
    ...
    stage = str((request.metadata or {}).get("broker_stage") or "").strip()
    confirmation = confirmation_policy_for_stage(policy, stage) if stage else {}
    return ResourceResult(
        ok=False,
        source=request.source,
        name=request.name,
        ...
        metadata={
            **(request.metadata or {}),
            **({"confirmation_policy": confirmation} if confirmation else {}),
...
def annotate_result(result: ResourceResult, request: ResourceRequest, policy: ResourcePolicy) -> ResourceResult:
    ...
    metadata = {
        ...
        "resource_kind": resource_kind_for_request(request),
        "next_action": policy.next_action,
        **({"confirmation_policy": confirmation_policy_for_stage(policy, stage)} if stage else {}),
        **(result.metadata or {}),
    }
    return ResourceResult(
        **{
...
def acquire_resource_with_policy(
    ...
    *,
    ...
    if stage == ResourceStage.DISCOVER:
        confirmation = confirmation_policy_for_stage(active_policy, stage)
        return ResourceResult(
            ok=True,
            source=request.source,
            name=request.name,
    ...
    confirmation = confirmation_policy_for_stage(active_policy, stage)
    if confirmation["required"] or (not active_policy.auto_acquire and stage not in READ_ONLY_STAGES):
        reason = "policy_requires_confirmation" if confirmation["required"] else "policy_deferred_no_auto_acquire"
        error = (
            "resource acquisition requires explicit confirmation"
            if confirmation["required"]
            else "resource acquisition is deferred by policy"
        )
        risk_flags = ("requires_confirmation",) if confirmation["required"] else ("auto_acquire_disabled",)
        return decision_result(
            request,
            active_policy,
            decision="deferred",
    ...
    for attempt in attempts:
        try:
            ...
            with attempt.opener.open(req, timeout=request.timeout_seconds) as response:
                ...
                content_limit = request.max_bytes if request.max_bytes is not None else DEFAULT_POLICY_MAX_BYTES
                data = response.read(content_limit + 1)
            if len(data) > content_limit:
                return ResourceResult(
                    ok=False,
                    source=request.source,
                    name=request.name,
                    error=f"resource exceeds max_bytes={content_limit}",
                    metadata={**metadata, "content_limit": content_limit, "bytes_read": len(data)},
                    decision="blocked",
                    resource_kind="url",
                    next_action="use_durable_transfer_owner_for_large_or_binary_resource",
                    risk_flags=("size_limit_exceeded",),
                )
            content_type = str(metadata.get("content_type") or "")
            content_key = hashlib.sha256(request.url.encode("utf-8")).hexdigest()[:20]
            content = persist_content(
                content_dir=request.target_dir / "_content" / content_key,
                raw=data,
                source_url=request.url,
                content_type=content_type,
                preview_bytes=preview_limit,
            )
            preview_text = Path(content["preview_ref"]).read_text(encoding="utf-8", errors="replace")
            metadata.update(
                {
                    "preview_bytes": len(preview_text.encode("utf-8")),
                    "preview_limit": preview_limit,
                    "preview_truncated": bool(content.get("preview_truncated")),
                    "preview_text": preview_text,
                    "raw_ref": content.get("raw_ref", ""),
                    "content_ref": content.get("content_ref", ""),
                    "preview_ref": content.get("preview_ref", ""),
                    "content_index_ref": content.get("content_index_ref", ""),
                    "content_sha256": content.get("content_sha256", ""),
                    "raw_sha256": content.get("raw_sha256", ""),
                    "content_bytes": content.get("content_bytes", 0),
                    "content_chars": content.get("content_chars", 0),
                    "content_parser": content.get("parser", ""),
                }
            )
            return ResourceResult(
                ok=True,
                ...
                decision="previewed",
                resource_kind="url",
                next_action="consume_or_expand_content_ref",
            )
        except urllib.error.HTTPError as exc:
