...
try:
    from codex_environment_mirror_reconciliation import partition_scoped_drift_rows
except ModuleNotFoundError:
    from _bridge.codex_environment_mirror_reconciliation import partition_scoped_drift_rows
...
try:
    ...
    from _bridge import scoped_authorization

try:
    import codex_environment_mirror_authorization as refresh_authorization
except ModuleNotFoundError:
    from _bridge import codex_environment_mirror_authorization as refresh_authorization

try:
    from decision_authority_policy import classify_decision
except ModuleNotFoundError:
    from _bridge.decision_authority_policy import classify_decision

try:
    from bounded_output import governed_cli_payload
except ModuleNotFoundError:
    from _bridge.bounded_output import governed_cli_payload
...
DRIFT_REVIEW_PREPUBLISH_REUSE_TTL_SECONDS = 180
...
REMOTE_HISTORY_RECOVERY_SCHEMA = "codex_environment_mirror.remote_history_recovery.v1"
REMOTE_HISTORY_RECOVERY_ALLOWED_PATHS = frozenset({"OPTIMIZATION_SPEC.md"})
# A reviewed prose-only recovery artifact stays bounded for operator review.
# 64 KiB covers the currently reviewed 40 KiB Markdown specification while
# rejecting larger or opaque remote payloads.
REMOTE_HISTORY_RECOVERY_MAX_BYTES = 64 * 1024
...
def publication_preflight_receipt_path() -> Path:
    """Persist the one live-validation result shared by publish and refresh."""
    return runtime_root() / "publication-preflight.json"
...
def publication_preflight_input_signature(
    source_authority: dict[str, Any],
    *,
    changed_paths: list[str] | None = None,
    remote: str = "",
...
def cli_projection(payload: dict[str, Any], *, action: str, full: bool = False) -> dict[str, Any]:
    """Keep mirror CLI results consumable without changing owner facts."""

    return governed_cli_payload(
        payload,
        full=full,
        full_result_ref=f"command:python _bridge/codex_environment_mirror.py {action}",
    )
...
def restore_acceptance_projection(
    owner: dict[str, Any],
    bundles: dict[str, Any],
    *,
    stage: bool,
) -> dict[str, Any]:
    """Normalize restore evidence without inferring target activation."""
    owner_acceptance = owner.get("recovery_acceptance")
    owner_acceptance = owner_acceptance if isinstance(owner_acceptance, dict) else {}
    owner_handoffs = {str(item) for item in owner_acceptance.get("owner_handoffs_pending", []) if str(item)}
    owner_handoffs.update(str(item) for item in owner.get("external_archive_gaps", []) if str(item))
    bundle_handoffs = {
        *[str(item) for item in bundles.get("blocked_missing_bundle", []) if str(item)],
        *[str(item) for item in bundles.get("owner_reacquire_required", []) if str(item)],
        *[str(item) for item in bundles.get("remote_reconnect_required", []) if str(item)],
    }
    expected_count = int(owner_acceptance.get("expected_asset_count") or owner.get("action_count") or owner.get("asset_count") or 0)
    actual_count = int(owner.get("asset_count") or 0)
    stage_complete = bool(
        stage
        and owner_acceptance.get("stage_asset_inventory_complete") is True
        and owner_acceptance.get("stage_hashes_verified") is True
        and expected_count == actual_count
        and owner.get("hashes_verified") is True
    )
    return {
        "schema": "codex_environment_mirror.restore_acceptance.v1",
        "expected_asset_count": expected_count,
        "staged_asset_count": actual_count if stage else 0,
        "snapshot_restore_plan_ready": bool(owner_acceptance.get("snapshot_restore_plan_ready")),
        "stage_asset_inventory_complete": stage_complete,
        "stage_hashes_verified": bool(stage_complete and owner.get("hashes_verified") is True),
        "owner_handoffs_pending": sorted(owner_handoffs | bundle_handoffs),
        "capability_restore_ready": False,
        "state": "staged_pending_owner_handoffs" if stage_complete else "snapshot_plan_pending_stage",
        "next_action": (
            "consume each owner dependency/reconnect receipt and target capability probe before activation"
            if stage_complete else "stage the complete plan and verify every asset hash"
        ),
    }
...
def restore_plan_receipt(owner: dict[str, Any]) -> dict[str, Any]:
    ...
    acceptance = restore_acceptance_projection(owner, bundles, stage=False)
    return {
...
def stage_receipt(owner: dict[str, Any]) -> dict[str, Any]:
    ...
    bundles = mcp_bundle_readiness()
    if not bundles.get("ok") or not bundles.get("capability_restore_ready") or bundles.get("blocked_missing_bundle"):
        return failure_receipt(
            "codex_environment_mirror.stage.v1",
            {
                **receipt,
    ...
    acceptance = restore_acceptance_projection(receipt, bundles, stage=True)
    return {
        ...
        "recovery_acceptance": acceptance,
        "capability_restore_ready": False,
        "owner_handoffs_pending": acceptance["owner_handoffs_pending"],
        "membership_guard": {
...
def _wsl_windows_executable_command(executable: Path, arguments: list[str]) -> list[str]:
    """Invoke a Windows executable through WSL's stable binfmt interpreter."""

    init = Path("/init")
    if os.name == "nt":
        return [_wsl_path_to_windows(executable), *arguments]
    if not init.is_file():
        return []
    windows_argv0 = _wsl_path_to_windows(executable)
    return [str(init), str(executable), windows_argv0, *arguments]
...
def _windows_owner_command(cli: Path, args: list[str]) -> tuple[list[str], dict[str, str]] | None:
    ...
    if python_path is None:
        return None
    command = _wsl_windows_executable_command(
        python_path,
        [_wsl_path_to_windows(cli), *[str(item) for item in args]],
    )
    if not command:
        return None
...
def _windows_owner_probe_command(owner_command: list[str], script: str) -> list[str]:
    """Replace the mirror CLI position with a Python probe while retaining its argv."""

    cli_index = 3 if owner_command[:1] == ["/init"] else 1
    if len(owner_command) <= cli_index:
        return []
    return [*owner_command[:cli_index], "-c", script, owner_command[cli_index]]
...
def remote_ref_commit(remote: str, ref_args: list[str], remote_url: str = "") -> dict[str, Any]:
    """Read one GitHub remote ref through bounded WSL-native transport attempts."""
    resolved_url = remote_url
    if not resolved_url:
        result = git_result(["remote", "get-url", remote])
        if not result.get("ok"):
            return {"ok": False, "reason": "git_remote_missing", "remote": remote, "attempts": []}
        resolved_url = str(result.get("stdout") or "")
    attempts, network_route = git_network_attempts_for_remote(resolved_url)
    options = [value for value in ref_args if str(value).startswith("-")]
    patterns = [value for value in ref_args if not str(value).startswith("-")]
    rows: list[dict[str, Any]] = []
    for attempt in attempts:
        name = str(attempt.get("name") or f"attempt_{len(rows) + 1}")
        env = {str(key): str(value) for key, value in (attempt.get("env") or {}).items()}
        result = git_transport_result(
            ["ls-remote", *options, remote, *patterns],
            timeout=120,
            extra_env=env,
            prefer_wsl_native="github.com" in resolved_url.lower(),
        )
        row = {"name": name, "ok": bool(result.get("ok"))}
        if result.get("ok"):
            sha = str(result.get("stdout") or "").split()[0] if result.get("stdout") else ""
            rows.append(row)
            return {"ok": True, "commit": sha, "result": result, "attempts": rows, "network_route": network_route}
        row["error"] = str(result.get("stderr_tail") or result.get("reason") or "")[:300]
        rows.append(row)
    return {"ok": False, "reason": "git_remote_ref_unavailable", "attempts": rows, "network_route": network_route}
...
def remote_history_recovery_receipt_path(remote: str, branch: str, remote_head: str = "") -> Path:
    safe_remote = re.sub(r"[^A-Za-z0-9_.-]+", "-", remote or "origin").strip("-") or "origin"
    safe_branch = re.sub(r"[^A-Za-z0-9_.-]+", "-", branch or "main").strip("-") or "main"
    suffix = remote_head[:12] if GIT_SHA.fullmatch(remote_head) else "pending"
    return runtime_root() / "remote-history-recovery" / f"{safe_remote}-{safe_branch}-{suffix}.json"


def _remote_history_recovery_ref(remote: str, branch: str, remote_head: str) -> str:
    safe_remote = re.sub(r"[^A-Za-z0-9_.-]+", "-", remote).strip("-") or "origin"
    safe_branch = re.sub(r"[^A-Za-z0-9_.-]+", "-", branch).strip("-") or "main"
    return f"refs/codex-mirror-recovery/{safe_remote}-{safe_branch}-{remote_head[:12]}"


def _git_stdout_sha(result: dict[str, Any]) -> str:
    values = str(result.get("stdout") or "").strip().split()
    return values[0] if values and GIT_SHA.fullmatch(values[0]) else ""


def _remote_history_recovery_failure(reason: str, *, remote: str, branch: str, local_head: str = "", remote_head: str = "", **details: Any) -> dict[str, Any]:
    receipt = {
        "schema": REMOTE_HISTORY_RECOVERY_SCHEMA,
        "ok": False,
        "generated_at": now_iso(),
        "remote": remote,
        "branch": branch,
        ...
        "reason": reason,
        "fail_closed": True,
        "next_action": "inspect_the_remote_history_recovery_receipt; do_not_force_push_or_discard_remote_history",
        **details,
    }
    _write_runtime_json(remote_history_recovery_receipt_path(remote, branch, remote_head), receipt)
    return receipt


def remote_history_recovery_plan(*, remote: str = "", branch: str = "", expected_remote_head: str = "") -> dict[str, Any]:
    """Fetch and review one narrow remote fork without changing mirror main."""
    remote_name = (remote or "origin").strip()
    branch_name = branch.strip()
    status = git_result(["status", "--short"])
    if not status.get("ok") or status.get("stdout"):
        return _remote_history_recovery_failure("git_worktree_not_clean_for_remote_history_recovery", remote=remote_name, branch=branch_name or "main", status=str(status.get("stdout") or ""))
    remote_url = git_result(["remote", "get-url", remote_name])
    if not remote_url.get("ok"):
        return _remote_history_recovery_failure("git_remote_missing", remote=remote_name, branch=branch_name or "main")
    if not branch_name:
        branch_name = str(git_result(["branch", "--show-current"]).get("stdout") or "").strip()
    local_head = _git_stdout_sha(git_result(["rev-parse", "HEAD"]))
    if not branch_name or not local_head:
        return _remote_history_recovery_failure("git_local_identity_unresolved", remote=remote_name, branch=branch_name or "main")
    remote_before = remote_ref_commit(remote_name, ["--heads", branch_name], str(remote_url.get("stdout") or ""))
    remote_head = str(remote_before.get("commit") or "")
    if not remote_before.get("ok") or not GIT_SHA.fullmatch(remote_head):
        return _remote_history_recovery_failure("remote_head_unavailable", remote=remote_name, branch=branch_name, local_head=local_head, remote_head=remote_head, remote_read=remote_before)
    if expected_remote_head and remote_head != expected_remote_head:
        return _remote_history_recovery_failure("remote_head_changed_before_fetch", remote=remote_name, branch=branch_name, local_head=local_head, remote_head=remote_head, expected_remote_head=expected_remote_head)
    recovery_ref = _remote_history_recovery_ref(remote_name, branch_name, remote_head)
    attempts, network_route = git_network_attempts_for_remote(str(remote_url.get("stdout") or ""))
    fetch_rows: list[dict[str, Any]] = []
    fetched: dict[str, Any] = {}
    for attempt in attempts:
        name = str(attempt.get("name") or f"attempt_{len(fetch_rows) + 1}")
        env = {str(key): str(value) for key, value in (attempt.get("env") or {}).items()}
        fetched = git_transport_result(["fetch", "--no-tags", remote_name, f"refs/heads/{branch_name}:{recovery_ref}"], timeout=300, extra_env=env, prefer_wsl_native="github.com" in str(remote_url.get("stdout") or "").lower())
        fetch_rows.append({"name": name, "ok": bool(fetched.get("ok"))})
        if fetched.get("ok"):
            break
    if not fetched.get("ok"):
        return _remote_history_recovery_failure("remote_fetch_failed", remote=remote_name, branch=branch_name, local_head=local_head, remote_head=remote_head, fetch_attempts=fetch_rows, network_route=network_route)
    fetched_head = _git_stdout_sha(git_result(["rev-parse", "-q", "--verify", f"{recovery_ref}^{{commit}}"]))
    remote_after = remote_ref_commit(remote_name, ["--heads", branch_name], str(remote_url.get("stdout") or ""))
    remote_after_head = str(remote_after.get("commit") or "")
    if fetched_head != remote_head or not remote_after.get("ok") or remote_after_head != remote_head:
        return _remote_history_recovery_failure("remote_head_changed_during_fetch", remote=remote_name, branch=branch_name, local_head=local_head, remote_head=remote_after_head, expected_remote_head=remote_head, fetched_head=fetched_head, recovery_ref=recovery_ref)
    parents = [item for item in str(git_result(["show", "-s", "--format=%P", recovery_ref]).get("stdout") or "").split() if GIT_SHA.fullmatch(item)]
    merge_base = _git_stdout_sha(git_result(["merge-base", "HEAD", recovery_ref]))
    if git_result(["merge-base", "--is-ancestor", recovery_ref, "HEAD"]).get("ok"):
        return _remote_history_recovery_failure("remote_history_already_integrated", remote=remote_name, branch=branch_name, local_head=local_head, remote_head=remote_head, recovery_ref=recovery_ref)
    if git_result(["merge-base", "--is-ancestor", "HEAD", recovery_ref]).get("ok"):
        return _remote_history_recovery_failure("remote_history_contains_local_head", remote=remote_name, branch=branch_name, local_head=local_head, remote_head=remote_head, recovery_ref=recovery_ref)
    if len(parents) != 1 or not merge_base or parents[0] != merge_base:
        return _remote_history_recovery_failure("remote_history_not_a_single_reviewable_fork", remote=remote_name, branch=branch_name, local_head=local_head, remote_head=remote_head, recovery_ref=recovery_ref, common_ancestor=merge_base, parents=parents)
    changed = git_result(["diff-tree", "--no-commit-id", "--name-status", "-r", recovery_ref])
    rows = [line.split("\t", 1) for line in str(changed.get("stdout") or "").splitlines() if "\t" in line]
    paths = [row[1].replace("\\", "/") for row in rows if len(row) == 2]
    if not changed.get("ok") or len(paths) != 1 or paths[0] not in REMOTE_HISTORY_RECOVERY_ALLOWED_PATHS or any(not row[0].startswith(("A", "M")) for row in rows):
        return _remote_history_recovery_failure("remote_history_contains_unreviewed_changes", remote=remote_name, branch=branch_name, local_head=local_head, remote_head=remote_head, recovery_ref=recovery_ref, changed_paths=paths, changed_statuses=[row[0] for row in rows])
    content_result = git_result(["show", f"{recovery_ref}:{paths[0]}"])
    content = str(content_result.get("stdout") or "").encode("utf-8", errors="replace")
    if not content_result.get("ok") or not content or len(content) > REMOTE_HISTORY_RECOVERY_MAX_BYTES or b"\x00" in content:
        return _remote_history_recovery_failure("remote_history_content_not_reviewable", remote=remote_name, branch=branch_name, local_head=local_head, remote_head=remote_head, recovery_ref=recovery_ref, path=paths[0], byte_count=len(content))
    merge_tree = _git_stdout_sha(git_result(["merge-tree", "--write-tree", "HEAD", recovery_ref]))
    if not merge_tree:
        return _remote_history_recovery_failure("remote_history_merge_conflict", remote=remote_name, branch=branch_name, local_head=local_head, remote_head=remote_head, recovery_ref=recovery_ref, common_ancestor=merge_base)
    receipt = {"schema": REMOTE_HISTORY_RECOVERY_SCHEMA, "ok": True, "generated_at": now_iso(), "remote": remote_name, "branch": branch_name, "local_head": local_head, "remote_head": remote_head, "common_ancestor": merge_base, "remote_parent": parents[0], "recovery_ref": recovery_ref, "merge_tree": merge_tree, "remote_review": [{"path": paths[0], "status": rows[0][0], "sha256": hashlib.sha256(content).hexdigest(), "byte_count": len(content)}], "fetch_attempts": fetch_rows, "network_route": network_route, "fail_closed": True, "next_action": "consume_this_remote_history_recovery_plan_before_retrying_push"}
    path = remote_history_recovery_receipt_path(remote_name, branch_name, remote_head)
    _write_runtime_json(path, receipt)
    return {**receipt, "receipt_path": str(path)}


def apply_remote_history_recovery_plan(plan: dict[str, Any]) -> dict[str, Any]:
    """Merge one previously reviewed remote fork without force-pushing it."""
    if plan.get("schema") != REMOTE_HISTORY_RECOVERY_SCHEMA or not plan.get("ok"):
        return {"ok": False, "reason": "remote_history_recovery_plan_invalid", "fail_closed": True}
    remote_name, branch_name = str(plan.get("remote") or "origin"), str(plan.get("branch") or "")
    local_head, remote_head, recovery_ref = str(plan.get("local_head") or ""), str(plan.get("remote_head") or ""), str(plan.get("recovery_ref") or "")
    if not branch_name or not GIT_SHA.fullmatch(local_head) or not GIT_SHA.fullmatch(remote_head) or not recovery_ref:
        return {"ok": False, "reason": "remote_history_recovery_plan_identity_invalid", "fail_closed": True}
    if str(git_result(["status", "--short"]).get("stdout") or ""):
        return {"ok": False, "reason": "git_worktree_not_clean_for_remote_history_merge", "fail_closed": True}
    current_head = _git_stdout_sha(git_result(["rev-parse", "HEAD"]))
    fetched_head = _git_stdout_sha(git_result(["rev-parse", "-q", "--verify", f"{recovery_ref}^{{commit}}"]))
    remote_url = git_result(["remote", "get-url", remote_name])
    observed = remote_ref_commit(remote_name, ["--heads", branch_name], str(remote_url.get("stdout") or "")) if remote_url.get("ok") else {}
    if current_head != local_head or fetched_head != remote_head or str(observed.get("commit") or "") != remote_head:
        return {"ok": False, "reason": "remote_history_recovery_plan_stale", "fail_closed": True, "current_head": current_head, "fetched_head": fetched_head, "observed_remote_head": observed.get("commit", "")}
    if _git_stdout_sha(git_result(["merge-tree", "--write-tree", "HEAD", recovery_ref])) != str(plan.get("merge_tree") or ""):
        return {"ok": False, "reason": "remote_history_merge_tree_changed", "fail_closed": True}
    merged = git_result(["merge", "--no-ff", "--no-commit", recovery_ref], timeout=300)
    if not merged.get("ok"):
        git_result(["merge", "--abort"], timeout=120)
        return {"ok": False, "reason": "remote_history_merge_apply_failed", "fail_closed": True, "detail": merged}
    committed = git_result(["commit", "-m", f"Merge reviewed remote mirror history {remote_head[:12]}"], timeout=300)
    if not committed.get("ok"):
        git_result(["merge", "--abort"], timeout=120)
        return {"ok": False, "reason": "remote_history_merge_commit_failed", "fail_closed": True, "detail": committed}
    merged_head = _git_stdout_sha(git_result(["rev-parse", "HEAD"]))
    if not merged_head or not git_result(["merge-base", "--is-ancestor", remote_head, "HEAD"]).get("ok"):
        return {"ok": False, "reason": "remote_history_merge_readback_failed", "fail_closed": True, "head": merged_head}
    return {"ok": True, "remote": remote_name, "branch": branch_name, "remote_head": remote_head, "prior_local_head": local_head, "head": merged_head, "recovery_ref": recovery_ref, "reason": "reviewed_remote_history_merged_without_force_push"}
...
def drift_plan_from_validation_issues(
    issues: list[dict[str, Any]],
    *,
    affected: dict[str, Any] | None = None,
    validation: dict[str, Any] | None = None,
) -> dict[str, Any]:
    """Build a receipt-compatible drift plan from one validator result."""
    ...
    return {
        **plan,
        ...
        "validation": {
            "schema": str((validation or {}).get("schema") or ""),
            "ok": bool((validation or {}).get("ok")),
            "issue_count": len(issues),
            "source_freshness_checked": bool((validation or {}).get("source_freshness_checked")),
            "source_freshness_ok": (validation or {}).get("source_freshness_ok"),
        },
...
def drift_plan(*, affected: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    return drift_plan_from_validation_issues(issues, affected=affected, validation=owner)
...
def _runtime_owner_validation() -> dict[str, Any]:
    ...
    if windows_owner is not None:
        ...
        return run_json(
            _windows_owner_probe_command(owner_command, script),
            timeout=90,
...
def _cached_publication_preflight(
    input_signature: str, source_authority: dict[str, Any]
) -> dict[str, Any]:
    """Reuse a short-lived frozen preflight only while source identities match."""

    receipt = read_lock_owner(publication_preflight_receipt_path())
    if receipt.get("schema") != "codex_environment_mirror.publication_preflight.v1" or not receipt.get("ok"):
        return {}
    if receipt.get("input_signature") != input_signature:
        return {}
    recorded_source = receipt.get("source_authority") if isinstance(receipt.get("source_authority"), dict) else {}
    recorded_work_git = recorded_source.get("work_git") if isinstance(recorded_source.get("work_git"), dict) else {}
    current_work_git = source_authority.get("work_git") if isinstance(source_authority.get("work_git"), dict) else {}
    if (
        not source_authority.get("ok")
        or recorded_work_git.get("worktree_head") != current_work_git.get("worktree_head")
        or recorded_work_git.get("bare_head") != current_work_git.get("bare_head")
        or _current_work_git_head() != str(current_work_git.get("worktree_head") or "")
    ):
        return {}
    try:
        generated_at = datetime.fromisoformat(str(receipt.get("generated_at") or "").replace("Z", "+00:00"))
        age_seconds = max(0.0, (datetime.now(timezone.utc) - generated_at).total_seconds())
    except ValueError:
        return {}
    if age_seconds > DRIFT_REVIEW_PREPUBLISH_REUSE_TTL_SECONDS:
        return {}
    frozen_signature = str(receipt.get("frozen_source_signature") or "")
    if frozen_signature and frozen_signature != validation_source_signature(latest_snapshot_id()):
        return {
            "schema": "codex_environment_mirror.publication_preflight.v1",
            "ok": False,
            "reason": "dynamic_source_changed_after_preflight",
            "receipt_path": str(publication_preflight_receipt_path()),
            "next_action": "inspect_the_changed_dynamic_source_and_start_a_new_explicit_preflight",
        }
    return {**receipt, "reused": True, "receipt_age_seconds": round(age_seconds, 3)}
...
def publication_preflight(
    ...
    *,
    ...
    refresh_scope: dict[str, Any] | None = None,
) -> dict[str, Any]:
    """Freeze one live validation for declared drift review and the first refresh.

    A later capture remains authoritative for dynamic sources.  If it observes a
    change, publish reports that exact invalidation instead of silently rerunning
    owner review against a different dynamic inventory.
    """

    authority = source_authority if isinstance(source_authority, dict) else work_git_release_gate()
    if not authority.get("ok"):
        return {
            "schema": "codex_environment_mirror.publication_preflight.v1",
            "ok": False, "reason": "work_git_release_not_ready",
            "source_authority": authority,
        }
    signature = publication_preflight_input_signature(
        authority, changed_paths=changed_paths, remote=remote, branch=branch
    )
    cached = _cached_publication_preflight(signature, authority)
    if cached:
        return cached
    scope = refresh_scope if isinstance(refresh_scope, dict) else publish_refresh_scope(changed_paths, authority)
    review_paths = list(
        (scope.get("mirror_impact") or {}).get("impacted_paths")
        or (scope.get("delta") or {}).get("changed_paths")
    ...
    reconciliation: dict[str, Any] = {
        ...
        "ok": True, "applied": False, "reason": "no_declared_source_delta",
        "reverse_absorption_allowed": False,
    }
    # A current global review already contains the live dynamic-source result,
    # owner evidence, and source/version fence needed for the preflight.  Do
    # not repeat that expensive validation just to partition capture roots.
    # The snapshot transaction remains the single final live-source check.
    reviewed_global = consume_fresh_drift_review_for_publish(authority)
    frozen_plan: dict[str, Any] = (
        reviewed_global if reviewed_global.get("review_consumed") else {}
    )
    preflight_validation_reused = bool(frozen_plan)
    if review_paths:
        affected = scope.get("affected_source_plan") if isinstance(scope.get("affected_source_plan"), dict) else None
        if not frozen_plan:
            frozen_plan = drift_plan(affected=affected)
        reconciliation = reconcile_declared_source_drift(
            DRIFT_REVIEW_CONFIRMATION, review_paths, scope=affected,
            source_authority=authority, frozen_plan=frozen_plan,
        )
    receipt = {
        "schema": "codex_environment_mirror.publication_preflight.v1",
        ...
        "frozen_plan_digest": str(frozen_plan.get("plan_digest") or ""),
        "frozen_source_signature": str(frozen_plan.get("source_signature") or ""),
        "preflight_validation_reused": preflight_validation_reused,
        "preflight_validation_reason": str(
            frozen_plan.get("review_reason") or "fresh_live_validation_required"
        ),
        "dynamic_source_policy": "fail_closed_after_preflight_without_automatic_review_retry",
        "next_action": (
            "consume_the_frozen_preflight_for_refresh_or_publish"
            if reconciliation.get("ok") else "resolve_the_reported_drift_owner_blockers"
        ),
    }
    _write_runtime_json(publication_preflight_receipt_path(), receipt)
    return receipt


def publication_convergence_plan(changed_paths: list[str] | None = None, *, remote: str = "", branch: str = "") -> dict[str, Any]:
    """Give callers one resumable plan without issuing a terminal mutation."""

    authority = work_git_release_gate()
    signature = publication_preflight_input_signature(authority, changed_paths=changed_paths, remote=remote, branch=branch) if authority.get("ok") else ""
    cached = _cached_publication_preflight(signature, authority) if signature else {}
    return {
        "schema": "codex_environment_mirror.publication_convergence_plan.v1",
        "ok": bool(authority.get("ok")),
        "source_authority": authority,
        "input_signature": signature,
        "steps": [
            {"id": "publication-preflight", "state": "reusable" if cached else "required", "terminal_mutation": False},
            {"id": "finalize-and-publish", "state": "required", "terminal_mutation": True},
            {"id": "contract-review-plan", "state": "required_after_publish", "terminal_mutation": False},
            {"id": "release", "state": "explicit_user_approval_required", "terminal_mutation": True},
        ],
        "first_invalid_action": "finalize-and-publish" if cached else "publication-preflight",
        "automatic_loop_allowed": False,
        "next_action": "run_only_the_first_invalid_action" if authority.get("ok") else "resolve_work_git_release_gate",
    }
...
def reconcile_declared_source_drift(
    ...
    *,
    ...
    # A publication preflight owns exactly one live validation.  Its frozen
    # plan must be reused for receipt consumption instead of being recomputed
    # by publish (dynamic inventory rows can legitimately change between two
    # scans).  The legacy path remains for callers without that preflight.
    if isinstance(frozen_plan, dict):
        plan = frozen_plan
        reviewed_plan = consume_drift_review(plan)
    else:
        reviewed_plan = consume_fresh_drift_review_for_publish(source_authority or {})
        plan = reviewed_plan
        if not reviewed_plan.get("review_consumed"):
            plan = drift_plan(affected=scope)
            reviewed_plan = consume_drift_review(plan)
    global_review_consumed = bool(
        reviewed_plan.get("review_consumed") and reviewed_plan.get("refresh_allowed")
    )
    effective_plan = reviewed_plan if global_review_consumed else plan
    rows = [row for row in effective_plan.get("rows", []) if isinstance(row, dict)]
    allowed_source_ids = {
        str(source_id)
    ...
    partitioned = partition_scoped_drift_rows(
        rows,
        allowed_source_ids,
        global_review_consumed=global_review_consumed,
    )
    scoped_rows = partitioned["scoped_rows"]
    isolated_reviewed_rows = partitioned["isolated_reviewed_rows"]
    blockers: list[dict[str, Any]] = list(partitioned["blockers"])
    ...
    if not effective_plan.get("ok") or blockers:
        return {
            "schema": "codex_environment_mirror.scoped_drift_reconciliation.v1",
            "ok": False,
            "reason": "scoped_drift_reconciliation_blocked",
            "scope": scope_summary,
            "plan_digest": str(effective_plan.get("plan_digest") or ""),
            "row_count": len(rows),
            "blockers": blockers,
    ...
    if global_review_consumed:
        return {
            ...
            "ok": True,
            "applied": False,
            "reason": "current_global_drift_review_reused",
            "scope": scope_summary,
            "plan_digest": str(effective_plan.get("plan_digest") or ""),
            "row_count": len(rows),
            "decision_count": len(scoped_rows),
            "isolated_reviewed_count": len(isolated_reviewed_rows),
            "isolated_reviewed_rows": isolated_reviewed_rows,
            "receipt_path": str(effective_plan.get("receipt_path") or ""),
            "review": {
                "reason": str(effective_plan.get("review_reason") or ""),
                "receipt_age_seconds": effective_plan.get("receipt_age_seconds"),
            },
            "refresh_preflight": _refresh_preflight_receipt(
                normalized_paths,
                scope=scope,
                drift_plan_result=effective_plan,
                source_authority=source_authority,
            ),
            "reverse_absorption_allowed": False,
        }
...
def consume_drift_review(plan: dict[str, Any]) -> dict[str, Any]:
    ...
    if receipt.get("reverse_absorption_allowed") is not False:
        return {
            **plan,
            "review_consumed": False,
            "review_reason": "receipt_reverse_absorption_boundary_invalid",
...
def consume_fresh_drift_review_for_publish(source_authority: dict[str, Any]) -> dict[str, Any]:
    """Reuse a just-reviewed receipt without a duplicate live-source scan.

    The following capture still performs live-source validation. This fast path
    only removes the gap between a signature-bound owner review and the first
    capture attempt, where mutable private sources could otherwise drift.
    """
    ...
    snapshot_id = latest_snapshot_id()
    if not receipt:
        return {"review_consumed": False, "review_reason": "receipt_missing"}
    if receipt.get("schema") != "codex_environment_mirror.drift_review_receipt.v1" or not receipt.get("ok"):
        return {"review_consumed": False, "review_reason": "receipt_invalid", "receipt_path": str(path)}
    if receipt.get("reverse_absorption_allowed") is not False:
        return {"review_consumed": False, "review_reason": "receipt_reverse_absorption_boundary_invalid", "receipt_path": str(path)}
    try:
        generated_at = datetime.fromisoformat(str(receipt.get("generated_at") or "").replace("Z", "+00:00"))
        age_seconds = max(0.0, (datetime.now(timezone.utc) - generated_at).total_seconds())
    except ValueError:
        return {"review_consumed": False, "review_reason": "receipt_timestamp_invalid", "receipt_path": str(path)}
    if age_seconds > DRIFT_REVIEW_PREPUBLISH_REUSE_TTL_SECONDS:
        return {"review_consumed": False, "review_reason": "receipt_preflight_ttl_expired", "receipt_path": str(path)}
    if not work_git_head or receipt.get("work_git_head") != work_git_head or _current_work_git_head() != work_git_head:
        return {"review_consumed": False, "review_reason": "receipt_work_git_head_mismatch", "receipt_path": str(path)}
    if receipt.get("snapshot_id") != snapshot_id or receipt.get("source_signature") != validation_source_signature(snapshot_id):
        return {"review_consumed": False, "review_reason": "receipt_snapshot_signature_mismatch", "receipt_path": str(path)}
    plan = {
        "ok": True,
        "plan_digest": str(receipt.get("plan_digest") or ""),
        "snapshot_id": snapshot_id,
        "source_signature": str(receipt.get("source_signature") or ""),
        "rows": [row for row in receipt.get("rows", []) if isinstance(row, dict)],
    }
    consumed = consume_drift_review(plan)
    if not consumed.get("review_consumed"):
        return consumed
    return {
        **consumed,
        "review_reason": "receipt_reused_within_preflight_ttl",
        "receipt_age_seconds": round(age_seconds, 3),
    }
...
def health() -> dict[str, Any]:
    ...
    candidate = bool(
        issues
        and issue_codes.issubset(RETRYABLE_VALIDATION_ISSUES)
    ...
    reviewed_drift = consume_drift_review(drift_plan_from_validation_issues(issues)) if candidate else {}
    release_gate = work_git_release_gate() if reviewed_drift.get("review_consumed") else {}
    publication_pending = bool(
        candidate
        and reviewed_drift.get("review_consumed")
        and reviewed_drift.get("refresh_allowed")
    ...
    if publication_pending:
        return {
            ...
            "convergence": {
                ...
                "drift_review_receipt": reviewed_drift.get("receipt_path", ""),
                "drift_plan_digest": reviewed_drift.get("plan_digest", ""),
                "drift_row_count": reviewed_drift.get("row_count", 0),
                "refresh_mode": "full" if reviewed_drift.get("affected_source_plan", {}).get("full_rebuild_required") else "directed_or_full_owner_selected",
    ...
    return {
        ...
        "source_ids": sorted(issue_source_ids),
        "drift_review": {
            "consumed": bool(reviewed_drift.get("review_consumed")),
            "reason": reviewed_drift.get("review_reason", ""),
            "receipt_path": reviewed_drift.get("receipt_path", ""),
        },
...
def _is_non_fast_forward_rejection(value: Any) -> bool:
    text = str(value or "").casefold()
    return "non-fast-forward" in text or "fetch first" in text or "tip of your current branch is behind" in text


def _resume_push_after_non_fast_forward(failure: dict[str, Any], *, remote: str, branch: str) -> dict[str, Any]:
    plan = remote_history_recovery_plan(
        remote=remote, branch=branch, expected_remote_head=str(failure.get("remote_head") or ""),
    )
    if not plan.get("ok"):
        return {**failure, "remote_history_recovery": {"plan": plan}, "reason": "remote_history_recovery_plan_failed"}
    merged = apply_remote_history_recovery_plan(plan)
    if not merged.get("ok"):
        return {**failure, "remote_history_recovery": {"plan": plan, "result": merged}, "reason": "remote_history_recovery_apply_failed"}
    resumed = push_receipt(remote=remote, branch=branch, allow_remote_history_recovery=False)
    return {
        **resumed,
        "remote_history_recovery": {
            "initial_push_failure": failure,
            "plan": plan,
            "result": merged,
            "resumed_push": resumed.get("push", {}),
        },
        "resumed": bool(resumed.get("ok")),
        "reason": "reviewed_remote_history_merged_then_pushed" if resumed.get("ok") else "reviewed_remote_history_merged_but_push_failed",
    }


def push_receipt(*, remote: str = "", branch: str = "", allow_remote_history_recovery: bool = True) -> dict[str, Any]:
    remote_name = (remote or "origin").strip()
    branch_name = branch.strip()
    ...
    for attempt in network_attempts:
        ...
        if not push.get("ok"):
            ...
            if verified_sha == head_sha:
                ...
                break
            attempt_receipts.append(row)
            # A non-fast-forward rejection is a deterministic history state,
            # not a transport fault. Further route retries cannot resolve it.
            if _is_non_fast_forward_rejection(row["error"]):
                break
            continue
        ...
        if remote_sha == head_sha:
            ...
            break
    if remote_sha != head_sha:
        rejection = "\n".join(str(row.get("error") or "") for row in attempt_receipts)
        failure = {
            "ok": False,
        ...
        if allow_remote_history_recovery and _is_non_fast_forward_rejection(rejection):
            return _resume_push_after_non_fast_forward(
                failure,
    ...
    *,
    ...
    return {
        ...
        "phase_timings_ms": {
            ...
            "capture_attempts": [item.get("elapsed_ms") for item in attempts],
            "capture_retry_count": max(0, len(attempts) - 1),
        },
        "elapsed_ms": round((time.perf_counter() - started) * 1000, 1),
    }
...
def refresh(
    ...
    *,
    ...
    authorization_required = inherited_barrier is None
    authorization_source = source_authority if isinstance(source_authority, dict) else work_git_release_gate()
    if authorization_required and not authorization_source.get("ok"):
        return {
            "schema": "codex_environment_mirror.refresh.v1", "ok": False,
            "phase": "work_git_release_gate", "reason": "work_git_release_not_ready",
            "source_authority": authorization_source,
        }
    if authorization_required and (not authorization_grant_ref or not authorization_thread_id):
        return {
            "schema": "codex_environment_mirror.refresh.v1", "ok": False,
            "reason": "scoped_authorization_grant_required",
            "static_confirmation_is_authorization": False,
            "authorization_plan_action": "refresh-authorization-plan",
        }
    operation_id = ""
    if authorization_required:
        expected_scope = refresh_authorization.refresh_scope(
            normalized_changed_paths, authorization_source, thread_id=authorization_thread_id,
        )
        operation_id = authorization_operation_id or f"mirror:refresh:{expected_scope['target_fingerprint']}"
        authorization = refresh_authorization.consume(
            authorization_grant_ref, normalized_changed_paths, authorization_source,
            thread_id=authorization_thread_id, operation_id=operation_id,
            state_root=authorization_state_root,
        )
        if not authorization.get("ok"):
            return {
                "schema": "codex_environment_mirror.refresh.v1", "ok": False,
                "reason": "scoped_authorization_rejected", "authorization": authorization,
            }
        started = refresh_authorization.effect_started(operation_id, state_root=authorization_state_root)
        if not started.get("ok"):
            return {
                "schema": "codex_environment_mirror.refresh.v1", "ok": False,
                "reason": "authorization_effect_journal_failed", "authorization_effect": started,
            }
    ...
    try:
        with exclusive_operation_lock(lock_path, "refresh"):
            ...
            if barrier is None:
                result = {"schema": "codex_environment_mirror.refresh.v1", "ok": False, "phase": "state_write_authority_gate", "reason": "state_writer_busy"}
                return {**result, "authorization_effect": refresh_authorization.effect_finished(operation_id, result, state_root=authorization_state_root)} if authorization_required else result
            try:
                result = _refresh_unlocked(
                    confirm,
...
def refresh_authorization_plan(
    changed_paths: list[str] | None = None, *, thread_id: str,
    authorization_state_root: Path | str | None = None,
) -> dict[str, Any]:
    return refresh_authorization.authorization_plan(
        normalize_changed_paths_for_mirror_owner(changed_paths), work_git_release_gate(),
        thread_id=thread_id, state_root=authorization_state_root,
    )
...
def _publish_under_state_barrier(started: float, barrier: state_write_authority.StateWriteLease, *, changed_paths: list[str] | None, remote: str, branch: str) -> dict[str, Any]:
            ...
            preflight = publication_preflight(
                effective_changed_paths, remote=remote, branch=branch,
                source_authority=source_authority, refresh_scope=refresh_scope,
            )
            drift_reconciliation = preflight.get("drift_reconciliation") if isinstance(preflight.get("drift_reconciliation"), dict) else {}
            if not preflight.get("ok"):
                return {
                    ...
                    "next_action": "resolve the reported preflight owner blockers; reuse this receipt after the first invalid action is repaired",
                }
            refreshed = refresh(
...
def release_plan(*, user_goal_explicit: bool = False, requested_action: str = "") -> dict[str, Any]:
    ...
    explicit_candidate = candidate or (next_semantic_tag(current_tag, "patch") if user_goal_explicit else "")
    recommendation = classify_decision(
        decision_class="recommendation",
        authority_ref="codex_environment_mirror.release_plan.release_recommended",
        condition_satisfied=bump != "none",
        user_goal_explicit=user_goal_explicit,
    ...
    return {
        ...
        "recommended_bump": bump,
        "recommended_tag": candidate,
        "effective_candidate_tag": explicit_candidate,
        "default_action": "skip_release" if bump == "none" else "review_recommended_release",
        "decision_authority": recommendation,
        "release_authorization_required": bool(user_goal_explicit),
        "release_authorization_plan_action": "release-authorization-plan" if user_goal_explicit else "",
        "reason": reason,
        "changed_path_count": len(paths),
...
def remote_tag_commit(
    ...
    *,
    extra_env: dict[str, str] | None = None,
    network_attempts: list[dict[str, Any]] | None = None,
) -> str:
    """Resolve an immutable GitHub tag through the same transport as branch push."""
    if network_attempts is None:
        remote_url = git_result(["remote", "get-url", remote])
        derived, _ = git_network_attempts_for_remote(str(remote_url.get("stdout") or ""))
        network_attempts = derived or [{"name": "selected_route", "env": extra_env or {}}]
    local_ref = git_result(["rev-parse", "-q", "--verify", f"refs/tags/{tag}"])
    local_ref_sha = str(local_ref.get("stdout") or "").strip()
    for attempt in network_attempts:
        env = {str(key): str(value) for key, value in (attempt.get("env") or {}).items()}
        remote_result = git_transport_result(
        ...
        if not remote_result.get("ok"):
            continue
        entries = [line.split(None, 1) for line in str(remote_result.get("stdout") or "").splitlines() if line.strip()]
        peeled = next((entry[0] for entry in entries if len(entry) == 2 and entry[1] == f"refs/tags/{tag}^{{}}"), "")
        if peeled == local_tag_commit:
            return local_tag_commit
        remote_ref = next((entry[0] for entry in entries if len(entry) == 2 and entry[1] == f"refs/tags/{tag}"), "")
        # An equal annotated-tag object hash authenticates its immutable target;
        # a lightweight tag has the commit SHA in both fields.
        if remote_ref and remote_ref == local_ref_sha:
            return local_tag_commit
    return ""


def push_tag_receipt(remote: str, tag: str, local_tag_commit: str) -> dict[str, Any]:
    """Push one immutable tag with bounded failover and ambiguous-result readback."""
    remote_url = git_result(["remote", "get-url", remote])
    if not remote_url.get("ok"):
        return {"ok": False, "reason": "git_remote_missing", "remote": remote}
    attempts, network_route = git_network_attempts_for_remote(str(remote_url.get("stdout") or ""))
    rows: list[dict[str, Any]] = []
    for attempt in attempts:
        name = str(attempt.get("name") or f"attempt_{len(rows) + 1}")
        ...
        if delay_seconds:
            time.sleep(delay_seconds)
        before = remote_tag_commit(remote, tag, local_tag_commit, network_attempts=[attempt])
        row: dict[str, Any] = {"name": name, "already_current": before == local_tag_commit}
        if before == local_tag_commit:
            rows.append(row)
            return {"ok": True, "tag": tag, "commit": local_tag_commit, "attempts": rows, "network_route": network_route}
        pushed = git_transport_result(["push", remote, f"refs/tags/{tag}"], timeout=300, extra_env=env, prefer_wsl_native=True)
        row["push_ok"] = bool(pushed.get("ok"))
        after = remote_tag_commit(remote, tag, local_tag_commit, network_attempts=[attempt])
        row["verified_after_push"] = after == local_tag_commit
        if not pushed.get("ok"):
            row["error"] = str(pushed.get("stderr_tail") or pushed.get("reason") or "")[:300]
        rows.append(row)
        if after == local_tag_commit:
            if not pushed.get("ok"):
                row["recovered_from_ambiguous_push_result"] = True
            return {"ok": True, "tag": tag, "commit": local_tag_commit, "attempts": rows, "network_route": network_route}
    return {"ok": False, "reason": "git_tag_push_failed", "tag": tag, "commit": local_tag_commit, "attempts": rows, "network_route": network_route}
...
def existing_release_for_current_state(
    ...
    *,
    ...
    remote_branch = remote_ref_commit(
        remote_name,
        ["--heads", branch_name],
        str(remote_url.get("stdout") or ""),
    )
    remote_head = str(remote_branch.get("commit") or "")
...
def release_authorization_scope(tag: str, *, thread_id: str) -> dict[str, str]:
    """Bind milestone authorization to exact source and recovery state."""

    snapshot_id = latest_snapshot_id()
    mirror_head = str(git_result(["rev-parse", "HEAD"]).get("stdout") or "").strip()
    work_git_head = str(git_result_at(str(WORK_GIT_ROOT), ["rev-parse", "HEAD"]).get("stdout") or "").strip()
    target = {
        "tag": str(tag or ""),
        "snapshot_id": snapshot_id,
        "mirror_head": mirror_head,
        "work_git_head": work_git_head,
    }
    return scoped_authorization.build_scope(
        thread_id=thread_id,
        action="codex_environment_mirror.release",
        target=target,
        risk="critical",
        phase="release",
        ...
        requested_by_owner="codex_environment_mirror",
    )


def release_authorization_plan(
    tag: str,
    *,
    thread_id: str,
    authorization_state_root: Path | str | None = None,
    ...
    if not parse_semantic_tag(tag):
        return {"schema": "codex_environment_mirror.release_authorization_plan.v1", "ok": False, "reason": "semantic_tag_invalid", "tag": tag}
    scope = release_authorization_scope(tag, thread_id=thread_id)
    if any(not value for value in scope.values()):
        return {"schema": "codex_environment_mirror.release_authorization_plan.v1", "ok": False, "reason": "release_source_identity_incomplete", "scope": scope}
    challenge = scoped_authorization.create_challenge(
        scope,
        target_summary=f"release {tag} from snapshot/source identities bound in scope",
        state_root=authorization_state_root,
    )
    return {"schema": "codex_environment_mirror.release_authorization_plan.v1", "ok": bool(challenge.get("ok")), "tag": tag, "scope": scope, "authorization_challenge": challenge}


def release_effect_started(operation_id: str, *, state_root: Path | str | None) -> dict[str, Any]:
    return scoped_authorization.record_effect(
        operation_id,
        executor="codex_environment_mirror",
        status="effect_started",
        state_root=state_root,
    )


def release_effect_finished(
    operation_id: str,
    result: dict[str, Any],
    *,
    tag: str,
    ...
    if not result.get("ok"):
        scoped_authorization.record_effect(
            operation_id,
            executor="codex_environment_mirror",
            status="effect_unknown",
            details={"tag": tag, "result_phase": result.get("phase", ""), "reason": result.get("reason", "")},
            state_root=state_root,
        )
        return result
    receipt_ref = f"github-release:{tag}:{result.get('head', '')}:{result.get('snapshot_id', '')}"
    observed = scoped_authorization.record_effect(
        operation_id,
        executor="codex_environment_mirror",
        status="effect_observed",
        effect_receipt_ref=receipt_ref,
        details={"tag": tag, "release_url": (result.get("release") or {}).get("url", "")},
        state_root=state_root,
    )
    completed = (
        scoped_authorization.record_effect(
    ...
    return {**result, "authorization_effect_ref": receipt_ref, "authorization_effect_status": completed.get("status", completed.get("reason", ""))}
...
def release(
    ...
    *,
    ...
    operation_id = authorization_operation_id or f"mirror:release:{tag}"
    try:
        with exclusive_operation_lock(lock_path, "release"):
            ...
            if existing.get("ok"):
                return existing
            if not authorization_grant_ref or not authorization_thread_id:
                return {
                    "schema": "codex_environment_mirror.release.v1",
                    "ok": False,
                    "reason": "scoped_authorization_grant_required",
                    "static_confirmation_is_authorization": False,
                    "authorization_plan_action": "release-authorization-plan",
                }
            if existing.get("terminal"):
                if existing.get("reason") != "existing_tag_points_elsewhere":
                    return existing
                authorization = scoped_authorization.consume_grant(
                    authorization_grant_ref,
                    release_authorization_scope(tag, thread_id=authorization_thread_id),
                    consumer_owner="codex_environment_mirror",
                    operation_id=operation_id,
                    state_root=authorization_state_root,
                )
                if not authorization.get("ok"):
                    return {"schema": "codex_environment_mirror.release.v1", "ok": False, "reason": "scoped_authorization_rejected", "authorization": authorization}
                started = release_effect_started(operation_id, state_root=authorization_state_root)
                if not started.get("ok"):
                    return {"schema": "codex_environment_mirror.release.v1", "ok": False, "reason": "authorization_effect_journal_failed", "authorization_effect": started}
                resumed = resume_ancestor_release_draft(tag, remote=remote)
                return release_effect_finished(operation_id, resumed, tag=tag, state_root=authorization_state_root)
            plan = release_plan(user_goal_explicit=True, requested_action="release")
            ...
            return release_effect_finished(operation_id, result, tag=tag, state_root=authorization_state_root)
    except MirrorOperationBusy as exc:
        return {
            "schema": "codex_environment_mirror.release.v1",
            "ok": False,
...
def execute(action: str, *, target_root: str = "", confirm: str = "", changed_paths: list[str] | None = None, left_snapshot: str = "", right_snapshot: str = "", remote: str = "", branch: str = "", tag: str = "", title: str = "", release_impact: str = "", force_fresh: bool = False, authorization_grant_ref: str = "", authorization_thread_id: str = "", authorization_operation_id: str = "", authorization_state_root: Path | str | None = None) -> dict[str, Any]:
    if action == "status":
    ...
    if action == "publication-preflight":
        return publication_preflight(changed_paths, remote=remote, branch=branch)
    if action == "convergence-plan":
        return publication_convergence_plan(changed_paths, remote=remote, branch=branch)
    if action == "remote-history-recovery-plan":
        return remote_history_recovery_plan(remote=remote, branch=branch)
    ...
    if action == "refresh":
        return compact_operation_receipt(
            action,
            refresh(
                confirm, changed_paths, authorization_grant_ref=authorization_grant_ref,
                authorization_thread_id=authorization_thread_id,
                authorization_operation_id=authorization_operation_id,
                authorization_state_root=authorization_state_root,
            ),
        )
    if action == "refresh-authorization-plan":
        return refresh_authorization_plan(
            changed_paths, thread_id=authorization_thread_id,
            authorization_state_root=authorization_state_root,
        )
    if action == "publish":
    ...
    if action == "release-plan":
        return release_plan()
    if action == "release-authorization-plan":
        return release_authorization_plan(tag, thread_id=authorization_thread_id, authorization_state_root=authorization_state_root)
    ...
    if action == "release":
        return compact_operation_receipt(action, release(confirm, tag=tag, title=title, remote=remote, branch=branch, authorization_grant_ref=authorization_grant_ref, authorization_thread_id=authorization_thread_id, authorization_operation_id=authorization_operation_id, authorization_state_root=authorization_state_root))
...
def main(argv: list[str] | None = None) -> int:
    ...
    for name in ("plan", "doctor", "health", "validate", "drift-plan", "release-plan", "contract-review-plan", "publication-preflight", "convergence-plan", "remote-history-recovery-plan"):
        sub.add_parser(name)
    for name in ("publication-preflight", "convergence-plan", "remote-history-recovery-plan"):
        parser_for_action = sub.choices[name]
        parser_for_action.add_argument("--changed", action="append", default=[])
        parser_for_action.add_argument("--remote", default="")
        parser_for_action.add_argument("--branch", default="")
    ...
    refresh_parser.add_argument("--authorization-grant-ref", default="")
    refresh_parser.add_argument("--authorization-thread-id", default="")
    refresh_parser.add_argument("--authorization-operation-id", default="")
    refresh_authorization_parser = sub.add_parser("refresh-authorization-plan")
    refresh_authorization_parser.add_argument("--changed", action="append", default=[])
    refresh_authorization_parser.add_argument("--authorization-thread-id", required=True)
    ...
    release_parser.add_argument("--authorization-grant-ref", default="")
    release_parser.add_argument("--authorization-thread-id", default="")
    release_parser.add_argument("--authorization-operation-id", default="")
    release_authorization_parser = sub.add_parser("release-authorization-plan")
    release_authorization_parser.add_argument("--tag", required=True)
    release_authorization_parser.add_argument("--authorization-thread-id", required=True)
    ...
    if args.action == "affected-source-plan":
        ...
        payload = execute(
            ...
            authorization_thread_id=getattr(args, "authorization_thread_id", ""),
            authorization_operation_id=getattr(args, "authorization_operation_id", ""),
        )
    print_json(cli_projection(payload, action=args.action, full=bool(getattr(args, "full", False))))
