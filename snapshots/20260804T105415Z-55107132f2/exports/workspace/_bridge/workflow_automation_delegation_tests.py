...
from shared.long_command_receipt import SCHEMA as LONG_COMMAND_RECEIPT_SCHEMA
from shared.long_command_receipt import SCHEMA_FAMILY as LONG_COMMAND_RECEIPT_SCHEMA_FAMILY
...
class WorkflowAutomationDelegationTests(unittest.TestCase):
    ...
    def test_long_command_schema_version_is_not_copied_into_consumers(self) -> None:
        bridge_root = Path(__file__).resolve().parent
        producer = bridge_root / "shared" / "long_command_receipt.py"
        result = validate_consumer_schema_authority(
            family=LONG_COMMAND_RECEIPT_SCHEMA_FAMILY,
            schema=LONG_COMMAND_RECEIPT_SCHEMA,
            producer_path=producer,
            consumer_root=bridge_root,
        )
        self.assertTrue(result["ok"], result["copied_literals"])
    ...
    def test_unknown_tool_argument_is_caller_error_not_contract_drift(self) -> None:
        result = tool_contract_preflight(
            tool_name="codex_app.send_message_to_thread",
            current_schema={
                "type": "object",
            ...
            attempted_arguments={"threadId": "thread-1", "message": "hello"},
            prior_schema_fingerprint="",
        )
        self.assertEqual(result["decision"], "caller_arguments_invalid")
        self.assertEqual(result["unknown_arguments"], ["message"])
        self.assertEqual(result["missing_arguments"], ["prompt"])
        self.assertFalse(result["contract_drift_observed"])
        self.assertTrue(result["single_schema_rebuild_allowed"])

    def test_tool_contract_drift_requires_two_distinct_authoritative_fingerprints(self) -> None:
        current_schema = {
            "type": "object",
            "required": ["threadId", "prompt"],
            "properties": {"threadId": {"type": "string"}, "prompt": {"type": "string"}},
        ...
        initial = tool_contract_preflight(
            tool_name="codex_app.send_message_to_thread",
            current_schema=current_schema,
            attempted_arguments={"threadId": "thread-1", "prompt": "hello"},
        )
        drifted = tool_contract_preflight(
            tool_name="codex_app.send_message_to_thread",
            current_schema={**current_schema, "required": ["threadId", "message"], "properties": {"threadId": {"type": "string"}, "message": {"type": "string"}}},
            attempted_arguments={"threadId": "thread-1", "message": "hello"},
            prior_schema_fingerprint=initial["current_schema_fingerprint"],
        )
        self.assertEqual(drifted["decision"], "contract_drift_observed")
        self.assertTrue(drifted["contract_drift_observed"])

    def test_second_argument_repair_is_blocked(self) -> None:
        result = tool_contract_preflight(
            tool_name="owner.cli",
            current_schema={"type": "object", "required": ["path"], "properties": {"path": {"type": "string"}}, "additionalProperties": False},
            attempted_arguments={"target": "x"},
            repair_attempted=True,
        )
        self.assertEqual(result["decision"], "caller_arguments_invalid")
        self.assertFalse(result["single_schema_rebuild_allowed"])
        self.assertEqual(result["next_action"], "stop_and_report_unconsumable_contract")

    def test_thread_coordination_policy_requires_self_exclusion_and_send_time_revalidation(self) -> None:
        rule = compact_automation_delegation_policy()["live_contract_preconditions"]["thread_coordination"]
        self.assertIn("current thread by ID", rule)
        self.assertIn("immediately before every dispatch", rule)
        self.assertIn("send_message_to_thread", rule)
        self.assertIn("current collaboration tree", rule)
        self.assertIn("CODEX_THREAD_ID", rule)
        self.assertIn("does not alter otherwise valid coordination", rule)

    def test_native_app_tool_absence_requires_current_schema_evidence(self) -> None:
        rule = compact_automation_delegation_policy()["live_contract_preconditions"]["tool_schema_authority"]
        self.assertIn("Missing MCP configuration", rule)
        self.assertIn("persisted dynamic-tool metadata", rule)
        self.assertIn("current exposed App schema", rule)

    def test_long_command_contract_prefers_one_native_handle_then_single_call_convergence(self) -> None:
        contract = compact_automation_delegation_policy()["long_command_contract"]
        self.assertEqual(
            contract["preference_order"],
            ["consume_native_process_or_session_handle", "single_call_durable_terminal_convergence"],
        )
        self.assertIn("failed terminal result", contract["reuse"])
        self.assertIn("fails closed", contract["conflict"])
        self.assertIn("stays alive until the durable terminal receipt", contract["convergence"])
        self.assertIn("never emit another status or follow command", contract["convergence"])
