...
def native_app_tool_discovery() -> dict[str, Any]:
    """Describe the boundary for tools exposed by the current Codex App turn."""

    return {
        "state": "current_turn_schema_required",
        "authority": "current exposed Codex App tool schema",
        "persistence_rule": "Missing MCP configuration, CLI visibility, or persisted thread_dynamic_tools metadata does not prove a native Codex App tool is unavailable.",
        "required_before_unavailable_claim": [
            "inspect the current exposed Codex App tool schema",
            "use the operation-specific native discovery entrypoint",
            "record the exact current-turn absence or call failure",
        ],
        "coordination_example": "Use list_threads to discover a Codex App task, then send_message_to_thread with its threadId and hostId; collaboration-agent messaging accepts only current-tree agent identifiers.",
        "callability_rule": "A successful current-turn native call proves callability; static or persisted metadata is only a discovery hint.",
    }
...
def thread_surface_snapshot(thread_id: str | None, current_cli_version: str) -> dict[str, Any]:
    ...
    return {
        ...
        "interpretation": (
            "The persisted dynamic-tool metadata is stale or empty. It does not prove that current-turn native Codex App tools are absent; inspect the current exposed App schema before selecting a fallback or reporting an unavailable tool."
...
def snapshot(thread_id: str | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "native_app_tool_surface": native_app_tool_discovery(),
...
def doctor(snap: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if thread_surface.get("state") in {"thread_runtime_surface_drift", "thread_cli_version_stale"}:
        ...
        issues.append(
            {
                "severity": "advisory",
                "code": "codex_thread_dynamic_tools_empty_native_surface_check_required",
                "tool": "*",
                "transport": "codex-session",
                "detail": {
                    "thread_surface": thread_surface,
                    "native_app_tool_surface": snap.get("native_app_tool_surface", native_app_tool_discovery()),
                },
            }
        )
    return {
...
def validate(snap: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "note": "This validates MCP config/CLI/process exposure plus native App discovery boundaries and recorded current-turn tool-surface failures; a tool is usable only after the applicable fresh active-turn success observation.",
    }
