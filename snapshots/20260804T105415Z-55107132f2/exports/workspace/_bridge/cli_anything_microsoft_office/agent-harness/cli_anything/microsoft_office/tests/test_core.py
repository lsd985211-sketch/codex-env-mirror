...
def test_command_contract_binds_nested_status_argv() -> None:
    result = CliRunner().invoke(cli, ["command-contract"])
    assert result.exit_code == 0
    payload = json.loads(result.output)
    assert payload["actions"] == ["status"]
    assert payload["action_commands"]["status"] == ["--json", "system", "status"]
    assert payload["maintenance"]["automation_level"] == "A0"
