...
class CodexWorkflowEntryLongCommandTests(unittest.TestCase):
    def test_publication_preflight_is_exposed_by_the_unified_facade(self) -> None:
        output = io.StringIO()
        result = {"schema": "codex_environment_mirror.publication_preflight.v1", "ok": True, "reused": True}
        with patch.object(entry, "execute_mirror_command", return_value=result) as owner, redirect_stdout(output):
            exit_code = entry.main(["mirror", "publication-preflight", "--changed", "workspace/_bridge/a.py"])

        self.assertEqual(0, exit_code)
        self.assertEqual(owner.call_args.args, ("publication-preflight",))
        self.assertEqual(owner.call_args.kwargs["changed_paths"], ["workspace/_bridge/a.py"])

    def test_convergence_plan_is_read_only_and_keeps_first_invalid_action(self) -> None:
        output = io.StringIO()
        result = {
            "schema": "codex_environment_mirror.publication_convergence_plan.v1",
            "ok": True,
            "first_invalid_action": "publication-preflight",
            "automatic_loop_allowed": False,
        }
        with patch.object(entry, "execute_mirror_command", return_value=result) as owner, redirect_stdout(output):
            exit_code = entry.main(["mirror", "convergence-plan"])

        self.assertEqual(0, exit_code)
        self.assertEqual(owner.call_args.args, ("convergence-plan",))
        self.assertFalse(json.loads(output.getvalue())["automatic_loop_allowed"])

    def test_refresh_authorization_reference_is_forwarded_to_durable_child(self) -> None:
        terminal = {"ok": True, "terminal": True, "status": "completed", "exit_code": 0}
        output = io.StringIO()
        with patch.object(entry, "converge_or_reuse_long_command", return_value=terminal) as converge, redirect_stdout(output):
            exit_code = entry.main([
                "mirror", "refresh", "--confirm", "REFRESH-CODEX-MIRROR",
                "--authorization-grant-ref", "scoped-authorization:grant:test",
                "--authorization-thread-id", "thread-test",
                "--authorization-operation-id", "refresh-test",
            ])

        self.assertEqual(0, exit_code)
        command = converge.call_args.args[1]
        self.assertIn("--authorization-grant-ref", command)
        self.assertIn("scoped-authorization:grant:test", command)
    ...
    def test_custom_submit_intent_is_accepted_when_owner_task_identity_matches(self) -> None:
        intent_id = "resolve-approved-global-agents-node-repl-20260728-f9c993de"
        task_id = f"intent-{hashlib.sha256(intent_id.encode('utf-8')).hexdigest()[:32]}"
        with patch.dict(
            "os.environ",
            {
                "CODEX_LONG_COMMAND_TASK_ID": task_id,
                "CODEX_LONG_COMMAND_INTENT_ID": intent_id,
                "CODEX_LONG_COMMAND_EXECUTION_SIGNATURE": "b" * 64,
            },
            clear=True,
        ):
            self.assertTrue(entry.durable_mirror_child_context_valid("publish"))

    def test_durable_child_context_rejects_mismatched_task_or_malformed_signature(self) -> None:
        intent_id = "custom-stable-publish-intent"
        cases = (
            ("intent-wrong", "c" * 64),
            (entry.task_id_for_intent(intent_id), "not-a-signature"),
        )
        for task_id, signature in cases:
            with self.subTest(task_id=task_id, signature=signature):
                with patch.dict(
                    "os.environ",
                    {
                        "CODEX_LONG_COMMAND_TASK_ID": task_id,
                        "CODEX_LONG_COMMAND_INTENT_ID": intent_id,
                        "CODEX_LONG_COMMAND_EXECUTION_SIGNATURE": signature,
                    },
                    clear=True,
                ):
                    self.assertFalse(entry.durable_mirror_child_context_valid("publish"))
