...
from resource_progress_view import progress_for_batch, progress_for_manifest, progress_for_request, request_progress_from_receipt
...
def issue_test_package_install_grant(
    *, request: dict[str, object], package_name: str, target_dir: Path, locked_spec: str,
    state_root: Path, thread_id: str, operation_id: str,
) -> dict[str, str]:
    """Issue a real one-turn scoped grant in an isolated authorization store."""
    authorization = {
    ...
    scope = resource_python_package_installer.package_install_authorization_scope(
        request, package_name, target_dir, locked_spec
    )
    issued = datetime(2026, 7, 28, tzinfo=timezone.utc)
    rollout = state_root.parent / f"{thread_id}.jsonl"
    rollout.write_text(
        json.dumps({"type": "session_meta", "payload": {"id": thread_id}}) + "\n",
        encoding="utf-8",
    )
    challenge = scoped_authorization.create_challenge(
        scope, target_summary=f"install {locked_spec}", state_root=state_root, issued_at=issued
    )
    user_event = {
    ...
    with rollout.open("a", encoding="utf-8") as handle:
        handle.write(json.dumps(user_event, ensure_ascii=False) + "\n")
    grant = scoped_authorization.grant_from_thread(
        challenge["challenge_ref"], rollout_path=rollout, state_root=state_root,
        checked_at=issued + timedelta(seconds=2),
    )
    assert_ok(bool(grant.get("ok")), f"test scoped grant should be issued: {grant}")
    authorization["grant_ref"] = str(grant["grant_ref"])
...
def run_resource_fetcher_tests() -> dict[str, str]:
    ...
    with tempfile.TemporaryDirectory(prefix="resource-fetcher-tests-") as tmp:
        ...
        with staged_http_server() as staged_url:
            malformed_preview = preview_url_resource(
                ResourceRequest(
                    source="test-malformed-url",
                    target_dir=cache,
                    name="malformed.txt",
                    url="https://example.invalid/guide extra context",
                )
            )
            ...
            assert_ok(not malformed_preview.ok, "malformed URL must not reach urllib")
            assert_ok(malformed_preview.policy_reason == "malformed_url", "malformed URL reason missing")
            assert_ok("unencoded whitespace" in malformed_preview.error, "malformed URL error is not actionable")
        ...
        external_dependency_audit = acquire_resource_with_policy(
            ResourceRequest(source="test-external-dependency-audit", target_dir=cache, name="external-dependency-audit"),
            intent=ResourceIntent.EXTERNAL_DEPENDENCY,
            stage=ResourceStage.AUDIT,
        )
        assert_ok(external_dependency_audit.ok, "external dependency audit should remain read-only")
        assert_ok(external_dependency_audit.decision == "audited", "external dependency audit decision missing")
        assert_ok(
            (external_dependency_audit.metadata or {}).get("confirmation_policy", {}).get("required") is False,
            "external dependency audit must not require side-effect confirmation",
        )
        results["policy_external_dependency_audit_read_only"] = "ok"

        package_dependency_audit = acquire_resource_with_policy(
            ResourceRequest(source="test-package-dependency-audit", target_dir=cache, name="package-dependency-audit"),
            intent=ResourceIntent.PACKAGE_DEPENDENCY,
            stage=ResourceStage.AUDIT,
        )
        assert_ok(package_dependency_audit.ok, "package dependency audit should remain read-only")
        assert_ok(package_dependency_audit.decision == "audited", "package dependency audit decision missing")
        assert_ok(
            (package_dependency_audit.metadata or {}).get("confirmation_policy", {}).get("required") is False,
            "package dependency audit must not require side-effect confirmation",
        )
        results["policy_package_dependency_audit_read_only"] = "ok"
        ...
        versioned_package_route = route_resource(
            target="graphify",
            intent=ResourceIntent.PACKAGE_DEPENDENCY,
            task="install official latest package version through npm",
            resource_kind_hint="package",
            source_kind_hint="package_registry",
            site_or_domain="registry.npmjs.org",
        )
        assert_ok(
            versioned_package_route.primary_tool == "package_manager",
            "explicit package installs must not be diverted to version-documentation discovery",
        )
        assert_ok(
            versioned_package_route.recommended_stage == ResourceStage.AUDIT,
            "explicit package installs must retain the package-manager audit stage",
        )
        ...
        assert_ok(
            paper_source_plan["candidates"][0]["id"] == "academic_openalex"
            and paper_source_plan["candidates"][0]["owner_tool"] == "academic_search",
            "paper source strategy should prefer the executable OpenAlex metadata owner first",
        )
        assert_ok(
        ...
        assert_ok(
            source_strategy_step["source_strategy"]["candidates"][0]["id"] == "academic_openalex",
        ...
        assert_ok(broker_docs_progress["progress"]["percent"] < 100, "docs handoff must remain an intermediate state")
        assert_ok(broker_docs_progress["resource_layer_terminal"] is False, "docs handoff must not be resource-layer terminal")
        ...
        if search_health.get("ok"):
            assert_ok(search_health.get("ok"), "Hub generic search dependency health failed")
        else:
            assert_ok(
                search_health.get("execution_owner") == "resource_package_owner"
                and search_health.get("runtime_key"),
                f"missing generic search dependency should route to the current runtime owner: {search_health}",
            )
        ...
        if search_health.get("ok"):
            ...
            assert_ok(
                unicode_worker_result.get("execution_owner") == "resource_package_owner"
                and unicode_worker_result.get("runtime_key") == search_health.get("runtime_key"),
                f"resource search worker should preserve the current runtime owner boundary: {unicode_worker_result}",
            )
        ...
        abstract_docs_request = ResourceBrokerRequest(
            target="IANA reserved example domains official documentation",
            task="find official IANA documentation about reserved example domains",
            intent=ResourceIntent.EXTERNAL_DEPENDENCY,
            auto_owner=True,
            metadata={"resource_kind_hint": "documentation", "validation_profile": "quick"},
        )
        generic_gateway_request = resource_broker.network_gateway_request_for_request(
            abstract_docs_request,
            route_for_request(abstract_docs_request),
        )
        assert_ok(
            generic_gateway_request.get("target_kind") == "docs"
            and generic_gateway_request.get("owner_tool") == "generic_search"
            and generic_gateway_request.get("target") == "https://duckduckgo.com/",
            f"abstract documentation discovery must consume the docs network owner: {generic_gateway_request}",
        )
        ...
        try:
            def fake_low_relevance_owner(**kwargs):
                if kwargs.get("tool") == "generic_search":
                    return {
                        "ok": True,
                        "status": "completed",
                        "source": "generic_search",
                        ...
                        "candidates": [{"title": "Python json", "url": "https://docs.python.org/3/library/json.html", "source_id": "https://docs.python.org/3/library/json.html"}],
                        "metadata": {"top_url": "https://docs.python.org/3/library/json.html"},
                        "next_action": "consume_resource",
                    }
                return {
                    "ok": True,
        ...
        assert_ok(relevance_fallback.ok, "low-relevance documentation owner should fall back to managed generic search")
        ...
        assert_ok(
            [item["tool"] for item in relevance_fallback.attempts] == ["context7", "generic_search"],
            "URL-free documentation fallback must not invoke resource_cli probe",
        )
        ...
        echoed_query_relevance = owner_result_relevance(
            request={
                "task": "compare codex environment mirror projects and recovery practices",
                "target": "codex environment mirror",
                "metadata": {"required_source_count": 2},
            },
            tool="generic_search",
            result={
                "ok": True,
                ...
                "content": json.dumps(
                    {
                        "query": "compare codex environment mirror projects and recovery practices",
                        "candidates": [
                            {"title": "Hermes Agent", "url": "https://example.invalid/hermes", "summary": "unrelated agent"}
                        ],
                    },
                    ensure_ascii=False,
                ),
        ...
        assert_ok(not echoed_query_relevance.ok, "echoed query must not satisfy generic-search relevance")
        assert_ok(echoed_query_relevance.reason == "low_relevance", "echoed query should expose low relevance")
        assert_ok(echoed_query_relevance.threshold == 0.5, "multi-source generic search should use the stricter threshold")
        ...
        assert_ok(structured_batch["failed_item_ids"] == ["cloakbrowser"], "batch receipt must not classify deferred items as failures")
        assert_ok(structured_batch["deferred_item_ids"] == ["superversion"], "batch receipt should expose deferred item boundaries separately")
        ...
        python_authorization_root = root / "python-package-authorization"
        ...
        boolean_only_install = resource_package_owner.execute_package_metadata(
            python_install_request,
        ...
        assert_ok(
            boolean_only_install.get("status") == "handoff_required",
            "install_approved must remain a safety interlock rather than an authorization grant",
        )
        issue_test_package_install_grant(
            request=python_install_request,
            package_name="beautifulsoup4",
            target_dir=python_install_target,
            locked_spec="beautifulsoup4==4.15.0",
            state_root=python_authorization_root,
            thread_id="resource-fetcher-python-install-turn",
            operation_id="resource-fetcher-python-install-operation",
        )
        ...
        try:
            def fake_python_install_run(command, **_kwargs):
                ...
                if "--target" not in command:
                    return original_python_install_run(command, **_kwargs)
                target = Path(command[command.index("--target") + 1])
                (target / "bs4").mkdir(parents=True, exist_ok=True)
                (target / "bs4" / "__init__.py").write_text("__version__ = '4.15.0'\n", encoding="utf-8")
                dist_info = target / "beautifulsoup4-4.15.0.dist-info"
                dist_info.mkdir(parents=True, exist_ok=True)
                (dist_info / "METADATA").write_text("Name: beautifulsoup4\nVersion: 4.15.0\n", encoding="utf-8")
                return SimpleNamespace(returncode=0, stdout="installed beautifulsoup4", stderr="")
            ...
            official_metadata = {
                ...
                "metadata": {"latest": "4.15.0", "command_kind": "pypi_json_metadata"},
            }
            with mock.patch.object(scoped_authorization, "DEFAULT_STATE_ROOT", python_authorization_root), mock.patch.object(
                resource_package_owner,
                "_lookup_python_metadata",
                return_value=official_metadata,
            ) as metadata_lookup, mock.patch.object(
                resource_python_package_installer.managed_python_runtime,
                "probe_imports",
                return_value={"ok": True, "imports": [{"name": "bs4", "ok": True}]},
            ):
        ...
        assert_ok(metadata_lookup.call_count == 1, "approved install should consume one official metadata result")
        assert_ok(python_install.get("result_kind") == "python_package_install", "Python install should not complete as metadata")
        assert_ok(python_install.get("metadata", {}).get("installed_version") == "4.15.0", "Python install should verify distribution metadata")
        ...
        handoff_batch_path = resource_scheduler.write_batch_manifest(
            broker_cache,
            "handoff-attach-recompute",
            {
                "schema": "resource_scheduler.batch.v1",
                "batch_id": "handoff-attach-recompute",
                "status": "deferred",
                "request_count": 1,
                "planned": [],
                "results": [
                    {
                        ...
                        "manifest_path": broker_docs.manifest_path,
                        "acceptance": {
                            "accepted": False,
        ...
        handoff_batch_before_attach = batch_status_from_manifest(handoff_batch_path)
        assert_ok(handoff_batch_before_attach["status"] == "deferred", "owner handoff should remain a non-terminal batch state")
        assert_ok(handoff_batch_before_attach["failed_item_ids"] == [], "owner handoff must not be classified as a failed item")
        ...
        assert_ok(attached_docs["satisfaction"]["satisfied"] is True, "attached owner result should satisfy the resource need")
        assert_ok(
            attached_docs["codex_guidance"]["resource_need_satisfied"] is True,
            "attached owner result should refresh Codex guidance from the completed receipt",
        )
        handoff_batch_after_attach = batch_status_from_manifest(handoff_batch_path)
        assert_ok(handoff_batch_after_attach["status"] == "completed", "batch status should consume the attached child receipt")
        assert_ok(handoff_batch_after_attach["accepted_item_ids"] == ["documentation"], "attached child result should satisfy batch acceptance")
        ...
        assert_ok(compact_payload["output_mode"] == "default_bounded", "resource job should default to bounded output")
        assert_ok(
            compact_payload["consume_required"]
            == request_progress_from_receipt(compact_payload["receipt"])["consume_required"],
            "bounded resource job output should preserve the owner-derived consume state",
        )
        assert_ok(compact_payload["context_projection"]["functional_recall"] == 1.0, "bounded resource job output should preserve functional fields")
        compact_status_stdout = io.StringIO()
        with contextlib.redirect_stdout(compact_status_stdout):
            compact_status_code = resource_cli_main(
                [
                    "job",
                    "status",
                    "--request-id",
                    compact_payload["request_id"],
                    "--receipt-log",
                    str(root / "job-compact-receipts.jsonl"),
                    "--json",
                ]
            )
        compact_status_payload = json.loads(compact_status_stdout.getvalue())
        assert_ok(
            compact_status_code == 0 and compact_status_payload["output_mode"] == "default_bounded",
            "resource status should default to the compact bounded projection",
        )
        assert_ok(
            len(compact_status_stdout.getvalue().encode("utf-8")) <= 12 * 1024,
            "default resource status should remain bounded",
        )
        full_stdout = io.StringIO()
        with contextlib.redirect_stdout(full_stdout):
            full_code = resource_cli_main(
                [
                    "job",
                    "status",
                    "--request-id",
        ...
        assert_ok(full_code == 0 and full_payload["output_mode"] == "full_bounded", "full resource status should remain available")
        assert_ok(len(full_stdout.getvalue()) > len(compact_status_stdout.getvalue()), "full resource status should be richer than default output")
        assert_ok(len(full_stdout.getvalue().encode("utf-8")) <= 36 * 1024, "full resource status should remain bounded")
        ...
        code_python_route = build_intent_resource_route("修复 Python 模块中的函数调用问题并验证影响范围")
        code_python_contract = code_python_route["resource_layer_contract"]
        assert_ok(code_python_contract["required"] is False, "bare language name in a code action must not require external resources")
        assert_ok(code_python_contract["task_class"] != "research_only", "bare language name in a code action must not become research-only")
        code_file_route = build_intent_resource_route("修复 src/parser.py 中的函数调用并运行测试")
        assert_ok(code_file_route["resource_layer_contract"]["required"] is False, "code-file maintenance must stay local without an external intent")
        ocr_inventory_route = build_intent_resource_route(
            "识别本机已经安装和可用的 OCR 能力，选择正确 owner 与工具处理扫描 PDF，并说明降级路径"
        )
        ocr_inventory_contract = ocr_inventory_route["resource_layer_contract"]
        assert_ok(ocr_inventory_contract["required"] is False, "OCR capability inventory must not require package installation")
        assert_ok(ocr_inventory_contract["task_class"] != "package_install", "OCR capability inventory must not become package-install")
        assert_ok(
            not any(item.get("key") == "package_manager" for item in ocr_inventory_route.get("resources", [])),
            "OCR capability inventory must not expose package-manager ownership",
        )
        install_after_probe_route = build_intent_resource_route("未安装 PaddleOCR，需要安装依赖")
        assert_ok(
            install_after_probe_route["resource_layer_contract"]["task_class"] == "package_install",
            "an explicit install after a capability probe must retain package-install routing",
        )
        docs_python_route = build_intent_resource_route("查询 Python 官方文档")
        assert_ok(docs_python_route["resource_layer_contract"]["required"] is True, "explicit Python documentation lookup must require the resource layer")
        research_python_route = build_intent_resource_route("联网研究 Python 3.14")
        assert_ok(research_python_route["resource_layer_contract"]["required"] is True, "explicit online Python research must require the resource layer")
