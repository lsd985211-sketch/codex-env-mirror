...
import state_write_authority  # noqa: E402
...
SCOPE_BOUND_ROOTS = WSL_RUNTIME_LOCAL_ROOTS | {"desktop"}
RUNTIME_LOCAL_PATHS = {
    ("desktop", "runCodexInWindowsSubsystemForLinux"),
}
...
DESKTOP_RUNTIME_SETTING_KEYS: dict[PathTuple, str] = {
    ("desktop", "runCodexInWindowsSubsystemForLinux"): "runCodexInWindowsSubsystemForLinux",
}
ENVIRONMENT_SELECTION_STATE_SCHEMA = "codex-desktop-environment-selection.v1"
...
def config_runtime_scope(config_path: Path) -> str:
    parts = {part.casefold() for part in config_path.parts}
    normalized = config_path.as_posix().casefold()
    linux_local_codex_home = (
        os.name != "nt"
        and config_path.is_absolute()
        and not re.match(r"^/mnt/[a-z](?:/|$)", normalized)
        and bool(parts & {".codex", ".codex-app"})
    )
    return "wsl" if ".codex-app" in parts or linux_local_codex_home else "windows_host"


def external_runtime_paths(*, target: str = "windows_codex_config") -> set[PathTuple]:
    """Resolve product-owned mutable fields from the state authority contract."""

    domain = state_write_authority.domain_contract("codex_config")
    return {
...
def classify_path(
    ...
    *,
    ...
    runtime_scope: str = "windows_host",
) -> str:
    if not path:
        return "unowned"
    ...
    if is_secret(path, value):
        return "secret"
    if runtime_scope == "windows_host" and path in external_runtime_paths():
        return "external_runtime"
    if path[-1].casefold() in TRANSIENT_NAMES:
        return "transient_generated"
    if path in RUNTIME_LOCAL_PATHS or path[0] == "projects":
        return "runtime_local"
    if runtime_scope == "wsl" and path[0] in WSL_RUNTIME_LOCAL_ROOTS:
        return "runtime_local"
    if path[0] == "desktop":
...
def eligible(classification: str, value: Any) -> bool:
    return classification not in {"provider_owned", "secret", "transient_generated", "runtime_local", "external_runtime"} and supported_value(value)
...
def projection_runtime_dir(config_path: Path = CODEX_CONFIG) -> Path:
    return config_path.parent / "state" / "runtime" / "codex_config_projection"


def projection_lock_path(config_path: Path = CODEX_CONFIG) -> Path:
    return projection_runtime_dir(config_path) / "projection.lock"


def read_managed_projection(path: Path) -> tuple[str, dict[str, Any], str]:
    text = path.read_text(encoding="utf-8").strip() if path.exists() else ""
    if not text.strip():
        return "", {}, ""
    payload = json.loads(text)
    values = payload.get("values") if isinstance(payload, dict) else {}
    runtime_scope = str(payload.get("runtime_scope") or "") if isinstance(payload, dict) else ""
    return text, values if isinstance(values, dict) else {}, runtime_scope
...
def load_state(config_path: Path = CODEX_CONFIG, db_path: Path = CC_SWITCH_DB) -> dict[str, Any]:
    ...
    managed_text, managed, managed_runtime_scope = read_managed_projection(managed_path)
    return {
        "live_text": live_text,
        "live": live,
        "provider": provider,
        ...
        "runtime_scope": config_runtime_scope(config_path),
        "managed_runtime_scope": managed_runtime_scope,
    }
...
def environment_selection_state_path(config_path: Path) -> Path:
    return config_path.parent / "state" / "desktop-environment-selection.json"


def runtime_local_projection_values(config_path: Path) -> dict[str, Any]:
    """Read runtime-owner intent without admitting it to the portable ledger."""

    path = environment_selection_state_path(config_path)
    ...
    try:
        ...
        return {}
    if payload.get("schema") != ENVIRONMENT_SELECTION_STATE_SCHEMA:
        return {}
    desired = payload.get("desired_value")
    if not isinstance(desired, bool):
        return {}
    return {"desktop": {"runCodexInWindowsSubsystemForLinux": desired}}
...
def managed_projection_values(
    ...
    *,
    ...
    """Preserve managed Desktop intent while adopting other verified live values.

    Desktop can rewrite missing settings to package defaults during startup. Once
    these settings have entered the independent ledger, that transient rewrite
    must not become the new intent. A first-run value can still be seeded from
    the active config or native readback when the ledger has no prior value.
    """

    values: dict[str, Any] = {}
    live_flat = flatten(state["live"])
    provider_paths = set(flatten(state["provider"]["config"]))
    runtime_scope = str(state.get("runtime_scope") or "windows_host")
    managed_runtime_scope = str(state.get("managed_runtime_scope") or "")
    managed_desktop_intent_paths: set[PathTuple] = set()
    for path, value in flatten(state.get("managed") or {}).items():
        if managed_runtime_scope != runtime_scope and path[0] in SCOPE_BOUND_ROOTS:
            continue
        classification = classify_path(path, value, provider_paths=provider_paths, runtime_scope=runtime_scope)
        if eligible(classification, value):
            _set_nested(values, path, value)
            if path in DESKTOP_SETTING_KEYS:
                managed_desktop_intent_paths.add(path)
    for path, value in live_flat.items():
        classification = classify_path(path, value, provider_paths=provider_paths, runtime_scope=runtime_scope)
        if eligible(classification, value):
            if path in managed_desktop_intent_paths:
                continue
            _set_nested(values, path, value)
...
def decommissioned_plugins() -> tuple[str, ...]:
    """Read negative plugin authority from the startup baseline.

    The projection ledger is recoverable state, so a retired plugin must be
    removed from both the active config and the ledger. Otherwise an
    additions-only replay can resurrect it after the registration owner has
    removed it.
    """

    try:
        baseline = json.loads(codex_state_repair.BASELINE_PATH.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError):
        return ()
    values = baseline.get("decommissioned_plugins")
    if not isinstance(values, list):
        return ()
    return tuple(sorted({str(item) for item in values if str(item).strip()}))


def decommissioned_marketplaces() -> tuple[str, ...]:
    """Read negative marketplace authority from the startup baseline."""

    try:
        baseline = json.loads(codex_state_repair.BASELINE_PATH.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError):
        return ()
    values = baseline.get("decommissioned_marketplaces")
    if not isinstance(values, list):
        return ()
    return tuple(sorted({str(item) for item in values if str(item).strip()}))


def _remove_nested(root: dict[str, Any], path: PathTuple) -> bool:
...
def build_plan(
    ...
    *,
    ...
    runtime_scope = str(state.get("runtime_scope") or "windows_host")
    managed_runtime_scope = str(state.get("managed_runtime_scope") or "")
    scope_migration_required = managed_runtime_scope != runtime_scope
    ...
    retired_plugin_paths = [("plugins", plugin) for plugin in decommissioned_plugins()]
    retired_marketplace_paths = [("marketplaces", marketplace) for marketplace in decommissioned_marketplaces()]
    retired_paths = [*retired_plugin_paths, *retired_marketplace_paths]
    for path in retired_paths:
        _remove_nested(desired_managed, path)
    ...
    active_recovery_paths = [
        ...
        if (
            path not in live_flat
            or (path in DESKTOP_SETTING_KEYS and live_flat[path] != value)
        )
        and eligible(
            classify_path(path, value, provider_paths=provider_paths, runtime_scope=runtime_scope),
            value,
        )
    ]
    ...
    action_rows = [
        ...
        {"path": path_pointer(path), "action": "remove-decommissioned", "owner_class": "startup_negative_authority"}
        for path in retired_paths
        if path in live_flat or path in managed_flat
    ]
    if scope_migration_required:
        action_rows.append({
            "path": "/managed-projection/runtime_scope",
            "action": "migrate-ledger-scope",
            "owner_class": "owner_projection",
            "from": managed_runtime_scope or "legacy_unscoped",
            "to": runtime_scope,
        })
    ...
    return {
        ...
        "projection_current": not scope_migration_required and not managed_updates and not managed_removals
        ...
        "retired_plugin_paths": retired_plugin_paths,
        "retired_marketplace_paths": retired_marketplace_paths,
        "scope_migration_required": scope_migration_required,
        ...
        "authority_policy": "CC Switch provider state owns provider fields and model catalog projection; declared external runtime fields stay live and never enter the recovery ledger; runtime owners own MCP, plugin, marketplace, shell, notify, project, Windows, and environment-selection roots; the independent ledger owns only portable non-provider recovery; retired common surfaces must remain absent",
    }
...
def render_active_config(
    ...
    *,
    ...
    runtime_local_values: dict[str, Any] | None = None,
) -> tuple[str, list[str]]:
    """Restore owner-managed fields and enforce durable Desktop display intent."""
    ...
    changed: list[str] = []
    for plugin in decommissioned_plugins():
        text, did_change = codex_state_repair.remove_table_tree(text, (f'plugins."{plugin}"',))
        if did_change:
            changed.append(f"remove-decommissioned:/plugins/{plugin}")
    for marketplace in decommissioned_marketplaces():
        text, did_change = codex_state_repair.remove_table_tree(
            text,
            (f"marketplaces.{marketplace}", f'marketplaces."{marketplace}"'),
        )
        if did_change:
            changed.append(f"remove-decommissioned:/marketplaces/{marketplace}")
    ...
    for path, value in managed_flat.items():
        classification = classify_path(path, value, provider_paths=provider_paths, runtime_scope=runtime_scope)
        live_matches = path in live_flat and live_flat[path] == value
        protected_desktop_drift = path in DESKTOP_SETTING_KEYS and path in live_flat and not live_matches
        if (path in live_flat and not protected_desktop_drift) or not eligible(classification, value):
            continue
        text, did_change = _set_path(text, path, value)
        if did_change:
            changed.append(f"active-config:{path_pointer(path)}")
    for path, value in flatten(runtime_local_values or {}).items():
        if path in live_flat or path not in RUNTIME_LOCAL_PATHS or not supported_value(value):
            continue
        text, did_change = _set_path(text, path, value)
        if did_change:
            changed.append(f"runtime-local-active-config:{path_pointer(path)}")
    tomllib.loads(text)
    return text, changed
...
def desktop_readback_state() -> dict[str, Any]:
    ...
    try:
        ...
        result = client.evaluate(
            _desktop_readback_expression(
                list({**DESKTOP_SETTING_KEYS, **DESKTOP_RUNTIME_SETTING_KEYS}.values())
            )
        )
    except Exception as exc:
...
def desktop_projection_state(effective_config: dict[str, Any], *, apply: bool) -> dict[str, Any]:
    ...
    setting_keys = {**DESKTOP_SETTING_KEYS, **DESKTOP_RUNTIME_SETTING_KEYS}
    entries = [
        {"key": setting_key, "value": live_flat[path]}
        for path, setting_key in setting_keys.items()
...
def _acquire_lock(config_path: Path = CODEX_CONFIG) -> Any:
    runtime_dir = projection_runtime_dir(config_path)
    runtime_dir.mkdir(parents=True, exist_ok=True)
    handle = projection_lock_path(config_path).open("a+b")
    if handle.tell() == 0:
        handle.write(b"0")
        handle.flush()
    handle.seek(0)
...
def projection_signature(config_path: Path = CODEX_CONFIG, db_path: Path = CC_SWITCH_DB) -> str:
    try:
        ...
        payload = {
            ...
            "decommissioned_plugins": list(decommissioned_plugins()),
            "decommissioned_marketplaces": list(decommissioned_marketplaces()),
        }
...
def apply_projection(
    ...
    *,
    ...
    state_lease = state_write_authority.try_acquire_state_write_lease(
        ...
        state_root=(
            config_path.parent
            if config_path != CODEX_CONFIG
            else state_write_authority.codex_config_coordination_root()
        ),
        timeout_seconds=10.0,
        advance_generation_on_acquire=False,
    )
    if state_lease is None:
        return {"schema": f"{SCHEMA}/apply", "ok": False, "reason": "state_write_authority_busy"}
    ...
    try:
        state = load_state(config_path, db_path)
        runtime_local_values = runtime_local_projection_values(config_path)
        ...
        if managed_change_needed or active_config_change_needed or legacy_cleanup_needed:
            state_lease.advance_generation()
            ...
            if managed_change_needed:
                if plan.get("scope_migration_required"):
                    changed.append(
                        f"managed-scope:{state.get('managed_runtime_scope') or 'legacy_unscoped'}->{state['runtime_scope']}"
                    )
