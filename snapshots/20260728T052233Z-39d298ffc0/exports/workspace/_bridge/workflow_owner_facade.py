...
OWNER_CAPABILITIES: dict[str, dict[str, Any]] = {
    ...
    "wsl_workspace": {
        "operations": [
            ...
            "workspace_work_git_release",
        ],
        "lifecycle": ["run", "status"],
        "state_source": "wsl_workspace_owner_receipt",
        "owner_entrypoint": "_bridge/wsl_workspace_owner.py",
        "long_lived_member": True,
        "activation_boundary": "bootstrap validates/prepares only; no host runtime import or default WSL switch",
    },
}
...
def build_action(
    ...
    *,
    ...
    wsl_bootstrap = selected_owner == "wsl_workspace" and selected_operation == "workspace_bootstrap"
    approval_required = selected_owner == "email" or office_mutation or wsl_bootstrap or _bool(supplied.get("install_approved")) or _bool(supplied.get("requires_approval")) or (selected_owner == "network" and selected_operation == "lease_stop")
    ...
    read_only = (
        ...
        or (selected_owner == "wsl_workspace" and selected_operation in {"workspace_status", "workspace_plan", "workspace_validate", "workspace_handoff", "workspace_cleanup_plan", "workspace_work_git_release"})
    )
...
def _maintenance_command(arguments: dict[str, Any]) -> tuple[list[str], list[str]]:
    ...
    return [str(PYTHON), str(script), *[str(item) for item in command_argv], *[str(item) for item in cli_args]], issues
...
def _wsl_workspace_command(operation: str, arguments: dict[str, Any]) -> list[str]:
    command_name = {
        ...
        "workspace_cleanup_plan": "cleanup-plan",
        "workspace_work_git_release": "mirror-export",
    }.get(operation)
    if not command_name:
        return []
    command = [str(PYTHON), str(BRIDGE / "wsl_workspace_owner.py")]
    for key, option in (
    ...
    if command_name == "bootstrap" and arguments.get("confirm"):
        command.extend(["--confirm", str(arguments["confirm"])])
    if command_name == "mirror-export":
        command.extend(["--kind", "work-git-release"])
    return command
...
def execute_action(action: dict[str, Any], *, timeout_seconds: int | None = None) -> dict[str, Any]:
    ...
    elif owner == "wsl_workspace":
        operation = str(action.get("operation") or "")
        command = _wsl_workspace_command(operation, arguments)
        receipt = _normalize_owner_command(action, _run_command(command, timeout), status_source="wsl_workspace_owner_receipt") if command else _receipt(
            action, status="unsupported", ok=False, error_class="wsl_workspace_operation_not_supported", error_reason=operation
        )
    else:
        receipt = _receipt(
            action,
...
def lifecycle_status(workflow_run_id: str, *, wait: bool = False, timeout: int = 300, interval: float = 1.0) -> dict[str, Any]:
    ...
    elif owner in {"mcp", "mobile", "office", "wsl_workspace"}:
        updated = dict(receipt)
        updated["updated_at"] = now_iso()
    elif owner == "network":
...
def validate() -> dict[str, Any]:
    ...
    wsl_action = build_action({}, message="WSL工作仓库状态", owner="wsl_workspace")
    ...
    if wsl_action.get("owner") != "wsl_workspace" or wsl_action.get("operation") != "workspace_status" or not wsl_action.get("read_only"):
        issues.append("wsl_workspace_action_synthesis_invalid")
