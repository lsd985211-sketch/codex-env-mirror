...
def build_plan(
    ...
    *,
    ...
    return {
        ...
        and not active_recovery_paths and not legacy_surface_count,
        ...
        "authority_policy": "provider config owns provider fields; independent ledger owns safe non-provider recovery; retired common surfaces must remain absent",
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.reseed_managed and args.confirm_reseed != "RESEED-MANAGED":
        ...
        result = {
            **{key: value for key, value in plan.items() if key not in {"unsupported", "classifications", "replay_losses", "active_recovery_paths", "managed_updates", "managed_removals", "legacy_flag_rows", "desired_managed"}},
            "replay_loss_paths": [path_pointer(path) for path in plan["replay_losses"][:MAX_ACTION_ROWS]],
            "active_recovery_paths": [path_pointer(path) for path in plan["active_recovery_paths"][:MAX_ACTION_ROWS]],
