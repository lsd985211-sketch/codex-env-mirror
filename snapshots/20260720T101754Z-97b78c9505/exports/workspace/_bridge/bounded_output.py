...
OUTPUT_EVIDENCE_POLICY = {
    ...
    "compression_rule": "remove_redundant_representation_only; required_functional_fields_must_remain_inline_or_referenceable",
}
...
# These fields carry execution meaning rather than presentation detail. A
# bounded projection may shorten descriptive payloads, but it must never make
# a gate, owner, failure cause, or next action appear complete when it was
# actually omitted. Keep this list small and authoritative; callers may add
# task-specific functional fields through required_keys.
FUNCTIONAL_KEYS = (
    "required_gates",
    "policy_decisions",
    "owner",
...
def bounded_payload(
    ...
    *,
    ...
    """Return a deterministic projection without silently losing function.

    Descriptive fields may be bounded inline. Functional fields are checked
    after projection; if they cannot fit, the result explicitly requires a
    reference/expansion instead of masquerading as a complete success.
    """
    original_bytes = json_size_bytes(payload)
    effective_preserve_keys = tuple(dict.fromkeys((*DEFAULT_DECISION_KEYS, *preserve_keys)))
    # preserve_keys controls presentation priority; required_keys extends the
    # shared execution semantics for a specific caller.
    functional_keys = tuple(dict.fromkeys((*FUNCTIONAL_KEYS, *required_keys)))
    ordered_payload = {
    ...
    for item_limit, string_limit, depth_limit in attempts:
        ...
        missing = _missing_functional_fields(payload, projected, functional_keys)
        if json_size_bytes(projected) <= max_bytes and not missing:
            functional_loss = ()
            break
        functional_loss = missing
    else:
        essential = {
        ...
        functional_inline = {
            key: bounded_value(payload[key], max_depth=max_depth, max_items=max_items, max_string=max_string)
            for key in functional_keys
            if key in payload
        }
        if functional_inline and not _contains_truncation_marker(functional_inline) and json_size_bytes(functional_inline) <= max_bytes:
            projected = {
                **functional_inline,
                "summary": "redundant context omitted; required functional fields preserved inline",
            }
            functional_loss = ()
        else:
            projected = {
            ...
            functional_loss = _missing_functional_fields(payload, projected, functional_keys)

    compression = {
        "functional_integrity": "preserved" if not functional_loss else "reference_required" if artifact_ref else "blocked_no_reference",
        "required_fields": [key for key in functional_keys if key in payload],
        "preserved_inline": [key for key in functional_keys if key in payload and key not in functional_loss],
        "reference_required": list(functional_loss),
        "compression_rule": "redundant_representation_only",
    }
    if functional_loss:
        compression["safe_next_step"] = (
            "fetch the complete result from artifact_ref before acting"
            if artifact_ref
            else "rerun with an explicit full-output/artifact destination before acting"
        )
        projected["compression_blocked"] = True
    ...
    budget = {
        ...
        "functional_compression": compression,
    }
...
def _missing_functional_fields(
    original: Mapping[str, Any],
    projected: Mapping[str, Any],
    functional_keys: Sequence[str],
) -> tuple[str, ...]:
    """Identify required fields that were omitted or recursively shortened."""
    missing: list[str] = []
    for key in functional_keys:
        if key not in original:
            continue
        if key not in projected or _contains_truncation_marker(projected[key]):
            missing.append(key)
...
def _contains_truncation_marker(value: Any) -> bool:
    ...
    if isinstance(value, Mapping):
        return any(
            str(key).startswith("_truncated") or _contains_truncation_marker(child)
