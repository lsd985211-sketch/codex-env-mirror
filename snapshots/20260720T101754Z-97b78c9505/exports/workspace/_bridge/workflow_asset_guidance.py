...
def _selected_skills(plan: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if orchestration.get("ok"):
        return [item for item in _as_list(orchestration.get("selected_skills")) if isinstance(item, dict)]
    return [
...
def _owner_guidance(environment_context: dict[str, Any], message: str) -> list[dict[str, Any]]:
    items: list[dict[str, Any]] = []
...
def build_asset_guidance(
    ...
    *,
    ...
    owners = _owner_guidance(environment_context, str(plan.get("message") or ""))
...
def validate_with_build_plan(build_plan: Any) -> dict[str, Any]:
    ...
    runtime_owners = {str(item.get("name") or "") for item in _as_list(guidance["runtime"].get("owners")) if isinstance(item, dict)}
    ...
    checks = {
        ...
        "runtime_prefers_pmb_owner": any("pmb" in item.lower() for item in runtime_owners),
