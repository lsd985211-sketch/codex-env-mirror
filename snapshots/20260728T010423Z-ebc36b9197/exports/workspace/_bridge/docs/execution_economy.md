...
- A changed input invalidates only the affected step and its dependents. A
  ...
  that receipt.
- A mirror `publish` always checks whether the latest committed snapshot and
  validation receipt already cover the current Work Git HEAD before honoring a
  changed-path refresh request. Explicit `refresh` followed by `publish` must
  reuse that evidence rather than capture the same source twice.
