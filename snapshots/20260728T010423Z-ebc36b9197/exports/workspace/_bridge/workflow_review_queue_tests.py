...
class WorkflowReviewQueueTests(unittest.TestCase):
    ...
    def test_batch_decision_is_revision_guarded_atomic_and_readable(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            envelope = prepare_delivery_envelope(package["package_id"], db_path=db_path)
            planned = decision_plan(package["package_id"], "approve", envelope_id=envelope["envelope_id"], db_path=db_path)
            self.assertTrue(planned["ok"], planned)
            applied = decision_apply(planned["confirm_token"], user_evidence_ref="user-message:approve-all", db_path=db_path)
    ...
    def test_stale_package_cannot_apply_to_new_revision(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            planned = decision_plan(package["package_id"], "approve", db_path=db_path)
            ...
            applied = decision_apply(planned["confirm_token"], user_evidence_ref="user-message:stale", db_path=db_path)
