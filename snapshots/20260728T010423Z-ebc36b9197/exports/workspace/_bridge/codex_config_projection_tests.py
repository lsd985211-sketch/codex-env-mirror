...
def run() -> None:
    ...
    external_source = ("marketplaces", "openai-bundled", "source")
    with mock.patch.object(
        projection.state_write_authority,
        "domain_contract",
        return_value={
            "external_runtime_fields": [
                {"target": "windows_codex_config", "path": list(external_source)}
            ]
        },
    ):
        assert projection.classify_path(
            external_source,
            r"\\?\C:\Users\example\.codex\.tmp\bundled-marketplaces\openai-bundled",
            provider_paths=set(),
            runtime_scope="windows_host",
        ) == "external_runtime"
        assert projection.classify_path(
            external_source,
            "/home/example/.codex-app/.tmp/bundled-marketplaces/openai-bundled",
            provider_paths=set(),
            runtime_scope="wsl",
        ) == "runtime_local"
    for config in (
    ...
    with tempfile.TemporaryDirectory() as raw:
        root = Path(raw)
        live_with_desktop_marketplace = (
            PROVIDER_CONFIG
            + '[marketplaces.openai-bundled]\n'
            + "source = '\\\\?\\C:\\Users\\example\\.codex\\.tmp\\bundled-marketplaces\\openai-bundled'\n"
            + 'source_type = "local"\n'
            + 'last_updated = "2026-07-26T10:05:37Z"\n'
        )
        config, database = make_fixture(root, live=live_with_desktop_marketplace)
        ...
        managed_path.write_text(
            json.dumps({
                ...
                "values": {
                    "marketplaces": {
                        "openai-bundled": {
                            "source": "/mnt/c/Users/example/.codex/.tmp/bundled-marketplaces/openai-bundled",
                            "source_type": "local",
                        }
                    }
                },
        ...
        external_fields = {
            "allowed_writers": ["codex_config_projection"],
            "external_runtime_fields": [
                {"target": "windows_codex_config", "path": ["marketplaces", "openai-bundled", "source"]},
                {"target": "windows_codex_config", "path": ["marketplaces", "openai-bundled", "last_updated"]},
            ]
        }
        with mock.patch.object(projection.state_write_authority, "domain_contract", return_value=external_fields):
            applied = projection.apply_projection(config, database, sync_desktop=False, backup=False)
        assert applied["ok"], applied
        active = tomllib.loads(config.read_text(encoding="utf-8"))
        managed = json.loads(managed_path.read_text(encoding="utf-8"))["values"]
        assert active["marketplaces"]["openai-bundled"]["source"].startswith("\\\\?\\C:")
        assert "last_updated" in active["marketplaces"]["openai-bundled"]
        assert "source" not in managed["marketplaces"]["openai-bundled"]
        assert "last_updated" not in managed["marketplaces"]["openai-bundled"]
        assert managed["marketplaces"]["openai-bundled"]["source_type"] == "local"

    with tempfile.TemporaryDirectory() as raw:
        root = Path(raw)
        live_with_retired_plugin = (
            PROVIDER_CONFIG
            + '[plugins."browser@openai-bundled"]\nenabled = true\n'
            + '[plugins."documents@openai-primary-runtime"]\nenabled = true\n'
            + '[marketplaces.retired]\nsource = "https://example.invalid/retired.git"\n'
        )
        config, database = make_fixture(root, live=live_with_retired_plugin)
        managed_path = projection.managed_projection_path(config)
        managed_path.parent.mkdir(parents=True, exist_ok=True)
        managed_path.write_text(
            json.dumps(
                {
                    ...
                    "values": {
                        "marketplaces": {
                            "retired": {"source": "https://example.invalid/retired.git"},
                        },
                        "plugins": {
        ...
        baseline_path = root / "codex_startup_baseline.json"
        ...
        with mock.patch.object(projection.codex_state_repair, "BASELINE_PATH", baseline_path):
        ...
        active = tomllib.loads(config.read_text(encoding="utf-8"))
        managed = json.loads(managed_path.read_text(encoding="utf-8"))["values"]
        assert "browser@openai-bundled" not in active.get("plugins", {})
        assert "browser@openai-bundled" not in managed.get("plugins", {})
        assert "retired" not in active.get("marketplaces", {})
        assert "retired" not in managed.get("marketplaces", {})
        assert active["plugins"]["documents@openai-primary-runtime"]["enabled"] is True
    ...
    with tempfile.TemporaryDirectory() as raw:
        ...
        protected = projection.apply_projection(config, database, sync_desktop=False, backup=False)
        assert protected["ok"], protected
        managed = json.loads(managed_path.read_text(encoding="utf-8"))["values"]
        assert managed["desktop"]["show-context-window-usage"] is True
        protected_config = tomllib.loads(config.read_text(encoding="utf-8"))
        assert protected_config["desktop"]["show-context-window-usage"] is True

        managed["desktop"]["show-ultra-in-model-picker-slider"] = True
        managed_path.write_text(
            json.dumps(
                {
                    "schema": "codex-managed-config-projection/v1",
        ...
        assert values["desktop"]["show-ultra-in-model-picker-slider"] is True
    ...
    print(json.dumps({"ok": True, "tests": 41}, ensure_ascii=False))
