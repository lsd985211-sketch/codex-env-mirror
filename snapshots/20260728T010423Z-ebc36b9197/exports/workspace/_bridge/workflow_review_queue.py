...
def decision_plan(package_id: str, action: str, *, include: list[str] | None = None, exclude: list[str] | None = None, envelope_id: str = "", db_path: Path = QUEUE_PATH) -> dict[str, Any]:
    from workflow_review_delivery import decision_plan as owner

    return owner(package_id, action, include=include, exclude=exclude, envelope_id=envelope_id, db_path=db_path)


def decision_apply(confirm_token: str, *, user_evidence_ref: str, note: str = "", db_path: Path = QUEUE_PATH) -> dict[str, Any]:
    from workflow_review_delivery import decision_apply as owner

    return owner(confirm_token, user_evidence_ref=user_evidence_ref, note=note, db_path=db_path)
...
def main() -> int:
    ...
    apply_decision.add_argument("--user-evidence-ref", required=True)
    ...
    if args.command == "snapshot":
        ...
        payload = decision_plan(args.package_id, args.action, include=args.include, exclude=args.exclude, envelope_id=args.envelope_id)
    elif args.command == "decision-apply":
        payload = decision_apply(args.confirm_token, user_evidence_ref=args.user_evidence_ref, note=args.note)
