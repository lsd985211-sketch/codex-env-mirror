...
For this Windows workspace, GPU OCR has been verified with RTX 3060 Laptop GPU, PaddlePaddle GPU 3.3.1, CUDA runtime 12.9 wheels, and PaddleOCR 3.7.0 in `_bridge/venvs/ocr-gpu-py312`. Prefer `GUI_OCR_PYTHON=_bridge/venvs/ocr-gpu-py312/Scripts/python.exe` plus `GUI_OCR_DEVICE=gpu`, but keep `GUI_OCR_FALLBACK_PYTHON=_bridge/venvs/ocr-py312/Scripts/python.exe` so OCR can fall back to CPU if GPU startup fails.
