...
PACKAGE_EXECUTION_MODES = {"inspect_then_install", "verified_auto_install"}


def package_execution_mode(request: dict[str, Any]) -> str:
    metadata = request.get("metadata") if isinstance(request.get("metadata"), dict) else {}
    explicit = str(metadata.get("package_execution_mode") or "").strip().lower()
    if explicit in PACKAGE_EXECUTION_MODES:
        return explicit
    action = str(metadata.get("package_action") or metadata.get("action") or "").strip().lower()
    return "verified_auto_install" if action in {"install", "add"} else ""
...
def _package_install_plan(request: dict[str, Any], package_name: str, *, metadata_check: dict[str, Any] | None = None) -> dict[str, Any]:
    plan = managed_python_runtime.managed_dependency_plan(package_name)
    metadata = request.get("metadata") if isinstance(request.get("metadata"), dict) else {}
    missing_permissions: list[str] = []
    if not _truthy(metadata.get("install_approved")):
        missing_permissions.append("install_approved")
    if not _truthy(request.get("allow_filesystem_write")):
        missing_permissions.append("allow_filesystem_write")
    authorization = metadata.get("authorization") if isinstance(metadata.get("authorization"), dict) else {}
    for field in ("grant_ref", "operation_id", "thread_id"):
        if not str(authorization.get(field) or "").strip():
            missing_permissions.append(f"authorization.{field}")
    return {
        **plan,
        "execution_mode": package_execution_mode(request) or "inspect_then_install",
        "metadata_check": metadata_check or {},
        "missing_permissions": missing_permissions,
        "install_ready": bool(plan.get("ok") and not missing_permissions),
        "install_request": {
            "target": package_name,
            ...
            "metadata": {
                ...
                "package_execution_mode": "verified_auto_install",
                "package_spec": str(plan.get("locked_spec") or package_name),
                "install_approved": True,
                "dependency_lock_signature": str(plan.get("lock_signature") or ""),
                "authorization": {
                    "grant_ref": "<scoped-authorization-grant-ref>",
                    "operation_id": "<stable-operation-id>",
                    "thread_id": "<current-turn-or-automation-id>",
                },
            },
        },
    }
...
def _metadata_matches_lock(package_name: str, result: dict[str, Any]) -> dict[str, Any]:
    plan = managed_python_runtime.managed_dependency_plan(package_name)
    result_metadata = result.get("metadata") if isinstance(result.get("metadata"), dict) else {}
    observed = str(result_metadata.get("latest") or "")
    expected = str(plan.get("version") or "")
    ok = bool(result.get("ok") and plan.get("ok") and observed == expected)
    return {
        "ok": ok,
        "package": package_name,
        "observed_version": observed,
        "locked_version": expected,
        "lock_signature": str(plan.get("lock_signature") or ""),
        "metadata_source": str(result_metadata.get("command_kind") or result_metadata.get("cache_source") or ""),
        "reason": "metadata_and_lock_match" if ok else "package_metadata_lock_mismatch",
    }
...
def execute_package_metadata(request: dict[str, Any], gateway_plan: dict[str, Any], timeout: int, json_result: JsonResultFactory) -> dict[str, Any]:
    ...
    mode = package_execution_mode(request)
    if mode == "verified_auto_install":
        preliminary = _package_install_plan(request, package_name)
        if preliminary.get("missing_permissions"):
            return json_result(
                ok=False,
                status="handoff_required",
                source="package_manager",
                result_kind="python_package_install_plan",
                error_class="verified_auto_install_requires_approved_write",
                reason="verified auto install requires install approval and filesystem write permission",
                metadata={"package_install_plan": preliminary, "package_execution_mode": mode},
                next_action="consume_package_install_plan_and_resubmit_with_matching_permissions",
            )
    metadata_result = _lookup_python_metadata(request, gateway_plan, timeout, json_result, package_name)
    metadata_check = _metadata_matches_lock(package_name, metadata_result)
    if mode == "inspect_then_install":
        plan = _package_install_plan(request, package_name, metadata_check=metadata_check)
        return json_result(
            ok=bool(metadata_result.get("ok")),
            status="completed" if metadata_result.get("ok") else str(metadata_result.get("status") or "failed"),
            source="package_manager",
            result_kind="python_package_install_plan",
            content=str(metadata_result.get("content") or "")[:4000],
            metadata={"package_install_plan": plan, "package_execution_mode": mode},
            error_class=str(metadata_result.get("error_class") or ""),
            reason=str(metadata_result.get("reason") or ""),
            next_action="review_package_install_plan_then_submit_verified_auto_install",
        )
    if mode == "verified_auto_install":
        if not metadata_check.get("ok"):
            return json_result(
                ok=False,
                status="failed",
                source="package_manager",
                result_kind="python_package_install_plan",
                error_class="package_metadata_lock_mismatch",
                reason="official package metadata does not match the managed dependency lock",
                metadata={"package_install_plan": _package_install_plan(request, package_name, metadata_check=metadata_check), "package_execution_mode": mode},
                next_action="refresh_and_review_the_managed_dependency_lock",
            )
        package = execution_package_from_gateway_plan(gateway_plan)
        if not package.get("ok"):
            return json_result(ok=False, status="handoff_required", reason="network_package_unavailable")
        installed = execute_python_package_install(request, package_name, package, timeout, json_result)
        installed_metadata = installed.get("metadata") if isinstance(installed.get("metadata"), dict) else {}
        installed["metadata"] = {
            **installed_metadata,
            "package_execution_mode": mode,
            "metadata_check": metadata_check,
            "package_install_plan": _package_install_plan(request, package_name, metadata_check=metadata_check),
        }
        return installed
