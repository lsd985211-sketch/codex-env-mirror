...
from resource_acceptance import build_acceptance_contract
...
from resource_request_admission import normalize_resource_input
...
class ResourceReceipt:
    ...
    strategy_shadow: dict[str, Any] = field(default_factory=dict)
...
def network_target_kind_for_request(request: ResourceBrokerRequest, route: ResourceRoute) -> str:
    ...
    if route.primary_tool == "github":
        return "github"
    # Resource-kind classification is more specific than a legacy
    # documentation_lookup intent.  This prevents academic requests from
    # probing Context7/DuckDuckGo before the academic owner gets its gateway plan.
...
def structured_source_domains(request: ResourceBrokerRequest) -> list[str]:
    ...
    custom_source_policy = custom.get("source_policy") if isinstance(custom.get("source_policy"), dict) else {}
    envelope = resource_contract_from_metadata(metadata)
    resource = envelope.get("resource", {}) if envelope else {}
    source_policy = resource.get("source_policy") if isinstance(resource.get("source_policy"), dict) else {}
    values: list[str] = []
    for candidate in (
        ...
        custom_source_policy.get("domains"),
        custom_source_policy.get("source_domains"),
    ):
...
def route_for_request(request: ResourceBrokerRequest) -> ResourceRoute:
    ...
    if request.owner_execution_mode == "approved_write" and route.primary_tool == "package_manager":
        return replace(
            route,
            read_only=False,
            recommended_stage=ResourceStage.MATERIALIZE,
            safety_boundaries=tuple(dict.fromkeys((*route.safety_boundaries, "approved_write_revalidated_by_package_owner"))),
        )
    return route
...
def candidate_tools(route: ResourceRoute, request: ResourceBrokerRequest | None = None) -> list[str]:
    ...
    _url, path = source_fields(request) if request else ("", "")
    if request and (_url or path or request.need_materialization) and "resource_cli" not in tools:
        tools.append("resource_cli")
    if request and request.auto_owner and request.url and (
        request.intent == ResourceIntent.DOCUMENTATION_LOOKUP
        or "/docs/" in request.url
        or "docs." in request.url
    ) and "generic_search" not in tools:
        tools.append("generic_search")
    if (
        request
        and not _url
        and not path
        and route.intent == ResourceIntent.DOCUMENTATION_LOOKUP
        and route.primary_tool in {"context7", "microsoftdocs"}
        and "generic_search" not in tools
    ):
        tools.append("generic_search")
...
def strategy_plan_for_request(request: ResourceBrokerRequest, route: ResourceRoute) -> list[dict[str, Any]]:
    ...
    if (
        ...
        and not request.need_materialization
...
def stage_for_tool(route: ResourceRoute, request: ResourceBrokerRequest, tool: str) -> str:
    ...
    url, path = source_fields(request)
    if not url and not path and route.recommended_stage == ResourceStage.PROBE:
        return ResourceStage.DISCOVER
...
def independent_owner_lane_remaining(
    planned_tools: list[str],
    current_index: int,
    current_tool: str,
    attempt: ResourceAttempt,
) -> bool:
    """Keep independent read-only owner lanes reachable after transport failure."""

    if attempt.status == "blocked" or attempt.error_class in {
        ...
        return False
    return any(
        tool != current_tool and tool in MCP_OR_EXTERNAL_TOOLS
        for tool in planned_tools[current_index + 1 :]
    )
...
def receipt_from_attempts(
    ...
    if owner_failure and not successful:
    ...
    if successful:
        ...
        # A later owner result that satisfies the request supersedes an earlier
        # handoff marker. Requiring an artifact or preview here made useful
        # structured owner results loop back to handoff_required even after the
        # canonical satisfaction predicate had accepted them.
        ...
        return ResourceReceipt(
            ...
            content_ref=str(metadata.get("content_ref") or artifact or request_id),
    ...
    terminal_no_results = bool(last and (last.status == "no_results" or last.error_class == "owner_no_results"))
    status = "handoff_required" if handoff else ("no_results" if terminal_no_results else "failed")
    next_action = handoff.next_action if handoff else (last.next_action if last else "classify_resource")
    error_class = handoff.error_class if handoff else (
        "route_chain_exhausted_no_results" if terminal_no_results else (last.error_class if last else "no_attempts")
    )
    return ResourceReceipt(
        ok=False,
        request_id=request_id,
...
def handle_request(
    ...
    *,
    ...
    admission = normalize_resource_input(asdict(request))
    if not admission.get("ok") or admission.get("kind") == "url_list":
        error_class = str(
            admission.get("reason")
            or ("multiple_urls_require_batch" if admission.get("kind") == "url_list" else "ambiguous_resource_input")
        )
        next_action = (
            "promote_to_resource_batch"
            if admission.get("kind") == "url_list"
            else str(admission.get("next_action") or "correct_resource_input")
        )
        emit("admission", "blocked", "resource request rejected before routing or network work", admission=admission)
        receipt = ResourceReceipt(
            ok=False,
            request_id=request_id,
            status="blocked",
            result_kind="none",
            route={"ok": False, "source_kind": "ambiguous", "risk_flags": [error_class]},
            attempts=[],
            progress_events=events,
            error_class=error_class,
            next_action=next_action,
            ...
            execution_budget=execution_budget.snapshot(phase="admission_blocked"),
        )
        emit("reported", receipt.status, "resource admission receipt produced", ok=False, next_action=next_action)
        ...
        return receipt
    if admission.get("kind") == "discovery_query" and admission.get("normalized_query_candidates"):
        request = replace(
            request,
            metadata={
                **dict(request.metadata or {}),
                "query_normalization": {
                    "schema": "resource_query_normalization.v1",
                    "mode": "candidate_only",
                    "original_query": str(admission.get("original_query") or ""),
                    "candidates": list(admission.get("normalized_query_candidates") or []),
                },
            },
        )
    ...
    acceptance_contract = build_acceptance_contract(asdict(request))
    source_strategy = next(
        (
            item.get("source_strategy")
            for item in strategy_plan
            if isinstance(item, dict) and item.get("tool") == "resource_source_strategy"
        ),
        {},
    )
    inferred_resource_kind = str((source_strategy or {}).get("resource_kind") or "").strip()
    if inferred_resource_kind and not str((request.metadata or {}).get("resource_kind_hint") or "").strip():
        request = replace(request, metadata={**dict(request.metadata or {}), "resource_kind_hint": inferred_resource_kind})
        acceptance_contract = build_acceptance_contract(asdict(request))
    strategy_shadow = build_shadow_plan(
        request=asdict(request),
        route=route.to_dict(),
        live_plan=strategy_plan,
        source_plan=source_strategy,
        acceptance=acceptance_contract,
    )
    live_tools = [str(item.get("tool") or "") for item in strategy_plan if isinstance(item, dict)]
    strategy_execution = controlled_execution_plan(request=asdict(request), shadow_plan=strategy_shadow, live_tools=live_tools)
    ...
    if route.ok:
        ...
        planned_tools = list(strategy_execution.get("tools") or [])
        tools_to_attempt = planned_tools or candidate_tools(route, effective_request)
        for tool_index, tool in enumerate(tools_to_attempt):
            remaining_count = max(0, len(tools_to_attempt) - tool_index - 1)
            lane_plan = execution_budget.lane_plan(
                owner_cap=effective_request.timeout_seconds,
                remaining_lane_count=remaining_count,
            )
            remaining_timeout = int(lane_plan.get("allocated_timeout_seconds") or 0)
            if remaining_timeout <= 0:
                exhausted = ResourceAttempt(
                    ...
                    started_at=now_iso(),
                    finished_at=now_iso(),
                    result={"ok": False, "execution_budget": execution_budget.snapshot(phase="before_attempt"), "lane_plan": lane_plan},
                    error_class="budget_exhausted_before_owner",
                    reason="remaining budget is reserved for later declared owners",
                    next_action="continue_to_reserved_owner_or_report_route_chain_exhaustion",
                )
                attempts.append(attempt_with_recovery_decision(exhausted))
                emit(
                    "attempting",
                    "skipped" if remaining_count else "failed",
                    "lane skipped to preserve the shared deadline for later owners" if remaining_count else "total request budget exhausted",
                    tool=tool,
                    error_class="budget_exhausted_before_owner",
                )
                if remaining_count:
            ...
            if should_continue_after_attempt(asdict(attempt), need_materialization=effective_request.need_materialization):
                continue
            if independent_owner_lane_remaining(tools_to_attempt, tool_index, tool, attempt):
                emit(
                    "attempting",
                    "degraded",
                    "failed source lane did not suppress a later independent owner lane",
                    tool=tool,
                    next_tools=tools_to_attempt[tool_index + 1 :],
                    error_class=attempt.error_class,
                )
                continue
            break
    ...
    receipt = replace(
        ...
        execution_budget=execution_budget.snapshot(phase="reported"),
    )
