...
def build_task_runtime(task: dict[str, str], identities: dict[str, Identity]) -> dict[str, Any]:
    ...
    metadata = task_metadata(task)
    record_key_override = slugify(metadata.get("delivery_id", ""))
    return {
        "task_name": task_name,
        "trigger": trigger,
        "sender": sender,
        ...
        "record_key": record_key_override or (build_record_key(task_name, scheduled_at) if scheduled_at else slugify(task_name)),
    }
...
def _deeptutor_recipient_identity(
    source_channel: str, source_sender_id: str, identities: dict[str, Identity]
) -> tuple[Identity | None, str]:
    """Resolve a chat sender only through an exact identity-table account.

    This deliberately does not use display names or aliases.  An automatic
    attachment email is allowed only when the identity table explicitly binds
    the originating chat account to one mailbox, so a reused nickname can
    never redirect a learning artifact to another person.
    """
    channel = str(source_channel or "").strip().casefold()
    sender_id = str(source_sender_id or "").strip()
    if channel != "napcat":
        return None, "unsupported_source_channel"
    if not sender_id or not sender_id.isascii() or not sender_id.isdigit():
        return None, "invalid_napcat_sender_id"
    account = f"{sender_id}@qq.com".casefold()
    matches = [
        identity
        for identity in identities.values()
        if any(str(candidate).strip().casefold() == account for candidate in identity.accounts)
    ]
    if len(matches) != 1:
        return None, "recipient_identity_missing" if not matches else "recipient_identity_ambiguous"
    if not matches[0].default_account:
        return None, "recipient_account_missing"
    return matches[0], ""


def _deeptutor_artifact_records(
    artifacts: list[dict[str, Any]], artifact_root: Path
) -> tuple[list[dict[str, Any]], str]:
    """Validate public output files before they enter the mail task pipeline."""
    ...
    for artifact in artifacts:
        ...
        if not raw_path or not url.startswith("/api/outputs/"):
            return [], "artifact_not_public"
        try:
            path = Path(raw_path).expanduser().resolve()
            path.relative_to(root)
        except (OSError, ValueError):
            return [], "artifact_outside_public_root"
        if not path.is_file():
        ...
        records.append(
            {
                "path": path,
                "url": url,
                "filename": Path(str(artifact.get("filename") or path.name)).name or path.name,
                "mime_type": str(artifact.get("mime_type") or mimetypes.guess_type(path.name)[0] or "application/octet-stream"),
                "size_bytes": size_bytes,
                "sha256": sha256_file(path),
            }
        )
    issues = validate_attachment_paths([item["path"] for item in records])
    if issues:
        return [], issues[0]
    return records, "" if records else "artifact_missing"


def _deeptutor_artifact_body(records: list[dict[str, Any]], topic: str) -> str:
    """Keep the body link-only; files remain regular scheduler attachments."""
    base_url = os.environ.get("DEEPTUTOR_PUBLIC_URL", "http://localhost:3782").strip().rstrip("/")
    links = []
    for item in records:
        url = str(item.get("url") or "").strip()
        if not url:
            continue
        links.append(url if re.match(r"^https?://", url, re.IGNORECASE) else f"{base_url}{url}")
    return "\n".join(links)


def create_deeptutor_artifact_delivery(
    *,
    partner_id: str,
    source_channel: str,
    ...
    artifact_root: Path,
    sender_identity: str = "主发送者",
    scheduled_at: datetime | None = None,
) -> dict[str, Any]:
    """Create one scheduler-owned attachment delivery for verified outputs.

    The function only materializes an existing scheduler task and its normal
    three-stage records.  It never starts a worker and never sends SMTP.
    """
    if str(partner_id or "").strip() != "medical-textbook-tutor":
        return {"ok": True, "created": False, "queued": False, "reason": "unsupported_partner"}
    _, identities = load_world()
    recipient, recipient_reason = _deeptutor_recipient_identity(
        source_channel, source_sender_id, identities
    )
    if recipient is None:
        return {"ok": True, "created": False, "queued": False, "reason": recipient_reason}
    sender = identity_by_name(identities, sender_identity)
    sender_ready, sender_reason = sender_can_deliver(sender_identity, identities)
    if sender is None or not sender_ready:
        return {
            "ok": True,
            "created": False,
            "queued": False,
            "reason": sender_reason or "sender_identity_missing",
        }
    records, artifact_reason = _deeptutor_artifact_records(artifacts, artifact_root)
    if artifact_reason:
        return {"ok": True, "created": False, "queued": False, "reason": artifact_reason}

    source = {
        "source_channel": str(source_channel).strip(),
        "source_sender_id": str(source_sender_id).strip(),
        "source_chat_id": str(source_chat_id).strip(),
        "source_session_key": str(session_key).strip(),
        "recipient_identity": recipient.name,
        "recipient_account": recipient.default_account,
        "artifacts": [
            {key: str(item[key]) for key in ("url", "filename", "mime_type", "sha256")}
            | {"size_bytes": int(item["size_bytes"])}
            for item in records
        ],
    }
    delivery_id = "deeptutor-artifact-" + hashlib.sha256(
        json.dumps(source, ensure_ascii=False, sort_keys=True, separators=(",", ":")).encode("utf-8")
    ).hexdigest()[:24]
    when = (scheduled_at or now_beijing()).replace(second=0, microsecond=0)
    topic_text = re.sub(r"[\r\n\t,]+", " ", str(topic or "学习产物")).strip()[:80] or "学习产物"
    subject = f"DeepTutor 学习产物：{topic_text}"
    body = _deeptutor_artifact_body(records, topic_text)
    task_name = f"DeepTutor产物投递-{delivery_id[-12:]}"
    metadata = {
        "delivery_id": delivery_id,
        "delivery_kind": "deeptutor_artifact",
        ...
        "source_session_key": source["source_session_key"],
        "subject": subject,
        "artifact_count": str(len(records)),
        "artifact_urls": "|".join(item["url"] for item in records),
        "attachments": "|".join(str(item["path"]) for item in records),
        "artifact_filenames": "|".join(item["filename"] for item in records),
        "artifact_mime_types": "|".join(item["mime_type"] for item in records),
        "artifact_size_bytes": "|".join(str(item["size_bytes"]) for item in records),
        "artifact_sha256": "|".join(item["sha256"] for item in records),
        "artifact_body_summary": body,
    }
    task = {
        "任务名": task_name,
        "任务类型": "单次附件投递",
        "触发方式": "单次",
        "目标": recipient.name,
        "执行动作": "按现有邮件调度器发送已验证 DeepTutor 学习产物附件",
        "状态": "启用",
        "责任身份": sender.name,
        "说明": "，".join(
            [
                f"北京时间 {when.strftime('%Y-%m-%d %H:%M')} 触发",
                *[f"{key}={value}" for key, value in metadata.items() if key != "artifact_body_summary"],
                f"静态正文：{body}",
            ]
        ),
    }
    append_task_row(MAIL_TASK_TXT, task)
    job = create_three_stage_email_job(task, identities)
    return {
        "ok": bool(job.get("ok")),
        "created": bool(job.get("created")),
        "queued": bool(job.get("ok")),
        "sends_mail": False,
        "delivery_id": delivery_id,
        "schedule_run_id": job.get("schedule_run_id", delivery_id),
        "task_name": task_name,
        "recipient_identity": recipient.name,
        "recipient_account": recipient.default_account,
