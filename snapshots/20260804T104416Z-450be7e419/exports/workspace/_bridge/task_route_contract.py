...
from intent_routing import has_install_action, matched_terms
...
FALLBACK_DESCRIPTION_TERMS = (
    "降级路径", "降级方案", "降级策略", "fallback path", "fallback plan", "degradation path",
)
...
AUTHORIZATION_GOVERNANCE_TERMS = (
    "权限系统", "权限治理", "权限边界", "授权意图", "授权机制", "授权流程",
    "permission system", "permission governance", "permission boundary",
    "authorization intent", "authorization mechanism", "authorization flow",
)

AUTHORIZATION_REQUEST_TERMS = (
    "需要授权后继续", "请求授权执行", "需要权限执行", "request authorization",
    "requires authorization", "needs permission to execute",
)

NEGATED_SECRET_OR_PERMISSION_MARKERS = (
    "不读取", "不要读取", "不使用", "不要使用", "无需使用",
    "do not read", "do not use", "without credential", "without token",
)

BARE_SECRET_OR_PERMISSION_TERMS = {
    "密钥", "令牌", "密码", "凭据", "secret", "token", "password", "credential",
}
...
TASK_FACT_TERMS: dict[str, tuple[str, ...]] = {
    ...
    "secret_or_permission_use": (
        "密钥", "读取密钥", "使用密钥", "令牌", "读取令牌", "使用令牌", "密码", "凭据",
        "读取凭据", "使用凭据", "提权", "管理员权限", "使用管理员权限",
        "secret", "read secret", "use secret", "api token", "read token", "use token",
        "password", "credential", "read credential", "use credential", "elevated", "administrator",
    ),
...
def derive_task_facts(
    ...
    for fact, terms in TASK_FACT_TERMS.items():
        ...
        if fact == "secret_or_permission_use":
            hits.extend(matched_terms(text, AUTHORIZATION_REQUEST_TERMS))
            governance_context = matched_terms(text, AUTHORIZATION_GOVERNANCE_TERMS)
            has_negated_secret_phrase = any(marker in text for marker in NEGATED_SECRET_OR_PERMISSION_MARKERS)
            if governance_context and has_negated_secret_phrase and set(hits).issubset(BARE_SECRET_OR_PERMISSION_TERMS):
                hits = []
            if not hits and governance_context:
                provenance[fact] = {
                    "source": "governance_context_non_authorizing",
                    "value": False,
                    "matched": governance_context[:6],
                    "non_blocking": True,
                }
                facts[fact] = False
                continue
        if fact == "package_install" and not has_install_action(text):
            hits = []
        value = bool(hits)
...
def build_system_change_gate(message: str) -> dict[str, Any] | None:
    ...
    if matched_terms(text, FALLBACK_DESCRIPTION_TERMS):
        action_hits = [hit for hit in action_hits if hit not in {"降级", "degrade", "degradation"}]
        soft_action_hits = [hit for hit in soft_action_hits if hit not in {"降级", "degrade", "degradation"}]
    if not has_install_action(text):
        soft_action_hits = [hit for hit in soft_action_hits if hit not in {"安装", "install"}]
...
def resolve_task_route_contract(
    ...
    governance_action = bool(matched_terms(text, GOVERNANCE_ACTION_TERMS))
    governance_change = governance_action and (
        "workflow_governance" in domains
        or bool(
            matched_terms(
                text,
                (
                    "系统治理", "规则治理", "规范重构", "全局规范", "机制冲突", "路由冲突",
                    "工作环境", "资源层", "workflow governance", "rule governance",
                    "resource layer", "mcp routing",
                ),
            )
        )
    )
    ...
    if "code_review_refactor" in domains:
        return TaskRouteContract(
            "code_review", "code_review_refactor", "ocr_scope_rules_plus_git_diff",
            "code_review_refactor", "read_only_review", "execute_code_review_owner_route",
            "explicit_code_review_contract", system_change_gate, facts, signals, gates,
            [*validation, "consume OCR preview/rule evidence and inspect the accepted Git diff before reporting findings"], closeout,
        )
    if "email" in domains:
        return TaskRouteContract(
    ...
    if domains == {"workflow_governance"}:
        return TaskRouteContract(
            "workflow_governance", "workflow_governance", "workflow_contract_readback",
            "workflow_governance", "maintenance_governance", "execute_primary_workflow_phase",
            "single_explicit_governance_domain", system_change_gate, facts, signals, gates, validation, closeout,
        )
    return TaskRouteContract(
...
def validate() -> dict[str, Any]:
    ...
    capability_inventory = resolve_task_route_contract(
        "识别本机已经安装和可用的 OCR 能力，选择正确 owner 与工具处理扫描 PDF，并说明降级路径",
        ["resource_acquisition", "mcp_tools"],
    )
    ok = (
        ...
        and capability_inventory.task_facts.get("package_install") is False
        and capability_inventory.task_facts.get("system_member_change") is False
        and capability_inventory.task_facts.get("durable_closeout_required") is False
        and capability_inventory.system_change_gate is None
    )
    return {
        ...
        "capability_inventory": capability_inventory.to_dict(),
    }
