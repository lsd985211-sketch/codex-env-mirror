...
class StateWriteAuthorityTests(unittest.TestCase):
    ...
    def test_primary_runtime_marketplace_source_is_desktop_owned(self) -> None:
        paths = authority.external_runtime_field_paths("windows_codex_config")
        self.assertIn(("marketplaces", "openai-primary-runtime", "source"), paths)
        contract = authority.domain_contract("codex_config")
        binding = next(
            item
            for item in contract["authority_bindings"]
            if item["binding_id"] == "primary_runtime_marketplace_source"
        )
        self.assertEqual("codex_desktop_runtime", binding["active_writer"])
