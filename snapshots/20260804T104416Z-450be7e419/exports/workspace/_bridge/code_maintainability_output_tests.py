...
class ModuleOutputBudgetTests(unittest.TestCase):
    ...
    def test_missing_indexes_fall_back_to_read_only_source_analysis(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            with patch.object(code_maintainability, "MODULE_CAPABILITY_INDEX", root / "full.json"), patch.object(
                code_maintainability, "MODULE_CAPABILITY_CORE_INDEX", root / "core.json"
            ):
                result = code_maintainability.load_module_index()

        self.assertTrue(result["ok"], result)
        self.assertEqual(result["source"]["kind"], "derived_in_memory_fallback")
        self.assertFalse(result["rules"]["fallback_writes_runtime"])
        self.assertTrue(any(item["module"] == "_bridge/workflow_plan_detail.py" for item in result["modules"]))
