...
from workflow_closeout_signals_process import coordination_target_admission
from workflow_terminal_convergence import terminal_action_adapter, terminal_receipt_for_action, terminal_source_signature
...
def _terminal_source_state(changed_files: list[str], source_state: dict[str, Any] | None) -> dict[str, Any]:
    supplied = source_state if isinstance(source_state, dict) else {}
    return {"changed_files": sorted(dict.fromkeys(changed_files)), **supplied}


def terminal_publish_receipt(
    *,
    convergence_id: str,
    intent_id: str,
    ...
    """Return the bounded reusable projection of one mirror publish result."""

    signature = terminal_source_signature(
    ...
    return {**receipt, "schema": "workflow_closeout.terminal_publish_receipt.v1"}


def converge_closeout_actions(
    ...
    *,
    ...
    entered_read_only_acceptance: bool = False,
    published_signature: str = "",
) -> dict[str, Any]:
    """Converge closeout through at most one publish and read-only acceptance."""

    convergence_id = convergence_id or "legacy-closeout"
    intent_id = intent_id or convergence_id
    state = _terminal_source_state(changed_files, source_state)
    signature = terminal_source_signature(
        convergence_id=convergence_id,
        intent_id=intent_id,
        source_state=state,
    ...
    convergence = {
        "schema": "workflow_closeout.terminal_convergence.v1",
        "convergence_id": convergence_id,
        "intent_id": intent_id,
        "terminal_source_signature": signature,
        "current_phase": "external_publish",
        "completed_action_ids": [],
        "invalidated_action_ids": [],
        "receipt_refs": [],
        "automatic_loop_allowed": False,
    }
    finalization["terminal_convergence"] = convergence
    if published_signature and published_signature != signature:
        convergence.update({
            "first_invalid_action": "mirror.publish",
            "next_action": {"action_id": "mirror.publish", "decision": "block", "reason": "terminal_signature_changed_after_mutation"},
        })
        finalization["ok"] = False
        finalization["blocked_reason"] = "terminal_signature_changed_after_mutation"
        return finalization

    if entered_read_only_acceptance:
        try:
            from codex_environment_mirror import status

            readback = status(force_fresh=True)
        except Exception as exc:
            readback = {"ok": False, "reason": f"{type(exc).__name__}:{exc}"}
        accepted = bool(
            readback.get("ok")
            and (readback.get("readiness") or {}).get("mirror_valid")
            and (readback.get("readiness") or {}).get("capability_restore_ready")
            and (readback.get("source_freshness") or {}).get("ok")
        )
        decision = {"schema": "workflow_closeout.post_mirror_publish.v1", "required": True, "applied": False, "decision": "verify", "ok": accepted, "result": readback}
        finalization["post_closeout_mirror"] = decision
        convergence.update({
            "current_phase": "read_only_acceptance",
            "next_action": {"action_id": "mirror.verify", "decision": "reuse" if accepted else "block", "reason": "read_only_acceptance"},
        })
        if not accepted:
            finalization["ok"] = False
            finalization["blocked_reason"] = "terminal_read_only_acceptance_failed"
        return finalization

    mirror_action = terminal_action_adapter("mirror.publish", input_signature=signature)
    expected_receipt = terminal_receipt_for_action(
        action_id="mirror.publish",
        input_signature=signature,
        intent_id=intent_id,
        result={"ok": True},
    )
    expected_key = expected_receipt.get("reuse_key", "") if mirror_action.get("ok") else ""
    receipt = next(
        (
            item
            for item in terminal_receipts or []
            if isinstance(item, dict)
            and item.get("action_id") == "mirror.publish"
            and item.get("reuse_key") == expected_key
            and item.get("accepted") is True
        ),
        None,
    )
    if receipt:
        result = receipt.get("result") if isinstance(receipt.get("result"), dict) else {"ok": True}
        finalization["post_closeout_mirror"] = {
            ...
            "decision": "reuse",
            "ok": True,
            "reason": "matching_terminal_receipt",
            "result": result,
        }
        convergence.update({
            "completed_action_ids": ["mirror.publish"],
            "receipt_refs": [str(receipt.get("ref") or "")],
            "next_action": {"action_id": "mirror.publish", "decision": "reuse", "reason": "matching_terminal_receipt"},
        })
        return finalization

    return _apply_post_closeout_mirror_legacy(
        finalization,
        changed_files=changed_files,
        apply=apply,
        outcome=outcome,
        owner_checks_ok=owner_checks_ok,
        concurrent_handoff=concurrent_handoff,
...
def _unique_thread_ids(values: list[str] | None) -> list[str]:
    """Deduplicate already-validated thread IDs without granting eligibility."""
    result: list[str] = []
    seen: set[str] = set()
...
def concurrent_closeout_handoff(
    *,
    ...
    revalidated_workspace_threads: list[str] | None = None,
) -> dict[str, Any]:
    """Build a handoff contract from live facts, with a send-time revalidation gate."""
    admission = coordination_target_admission(
        current_thread_id_value=current_thread_id,
        discovery_threads=active_workspace_threads,
        revalidated_threads=revalidated_workspace_threads,
        requested_target_thread=handoff_target_thread,
    )
    own_thread = str(admission["current_thread_id"])
    active = list(admission["active_workspace_threads"])
    ineligible = list(admission["ineligible_thread_facts"])
    send_time_active = list(admission["send_time_active_workspace_threads"])
    revalidation_supplied = bool(admission["send_time_revalidation_supplied"])
    required = bool(current_task_complete and active)
    target = str(admission["target_thread"])
    blockers: list[dict[str, Any]] = list(admission["blockers"])
    if required and own_thread and not target:
    ...
    return {
        ...
        "send_time_revalidation_required": bool(required),
        ...
        "fact_source_rule": "eligible targets require current structured Codex App state with status=active, sameRepository=true, and currentTask=false; CODEX_THREAD_ID is required whenever a candidate exists, current thread is always excluded, and send-time revalidation is required before dispatch; message receipts never replace that evidence",
    }


def _apply_post_closeout_mirror_legacy(
    finalization: dict[str, Any],
    *,
    changed_files: list[str],
    apply: bool,
    ...
    try:
        from codex_environment_mirror import PUBLISH_CONFIRMATION, finalize_and_publish

        terminal = finalize_and_publish(PUBLISH_CONFIRMATION, changed_paths=publish_paths)
        result = terminal.get("result") if terminal.get("ok") and isinstance(terminal.get("result"), dict) else terminal
    except Exception as exc:
    ...
    decision["terminal_receipt"] = terminal if 'terminal' in locals() else {}
    finalization["post_closeout_mirror"] = decision
    if isinstance(convergence, dict):
        receipt = terminal_publish_receipt(
            convergence_id=convergence_id,
            intent_id=intent_id,
        ...
        convergence.update({
            "completed_action_ids": ["mirror.publish"] if result.get("ok") else [],
            "receipt_refs": [receipt["ref"]] if receipt.get("ref") else [],
            "terminal_publish_receipt": receipt,
            "next_action": {
                "action_id": "mirror.verify" if result.get("ok") else "mirror.publish",
                "decision": "verify" if result.get("ok") else "block",
                "reason": "terminal_mutation_complete" if result.get("ok") else "terminal_mutation_failed",
            },
        })
...
def apply_post_closeout_mirror(
    ...
    *,
    ...
    """Compatibility entry routed through the bounded convergence owner."""

    return converge_closeout_actions(
        finalization,
        changed_files=changed_files,
        apply=apply,
        outcome=outcome,
