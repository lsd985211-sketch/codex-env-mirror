...
from platform_paths import host_compatibility_root, memory_root, resource_library_root  # noqa: E402


HOST_BRIDGE_ROOT = host_compatibility_root() / "_bridge"
RECORD_STORE_INDEX_PATH = resource_library_root() / "文档" / "系统维护" / "索引" / "record_store.sqlite"
EMAIL_STATE_INDEX_PATH = first_existing_path(
    BRIDGE_ROOT / "shared" / "email_scheduler_state" / "email_state.sqlite",
    HOST_BRIDGE_ROOT / "shared" / "email_scheduler_state" / "email_state.sqlite",
)
...
SCRATCH_DB_PATH = Path(
    ...
    or Path.home() / ".codex-app" / "sqlite" / "codex_scratch.sqlite"
).expanduser()
...
def ensure_scratch_database() -> None:
    """Materialize the Hub-owned scratch DB outside the declarative worktree."""

    SCRATCH_DB_PATH.parent.mkdir(parents=True, exist_ok=True)
    with sqlite3.connect(SCRATCH_DB_PATH):
        pass
...
from local_mcp_hub_process import (  # noqa: E402
    INSTALL_CONFIRM as HUB_SERVICE_INSTALL_CONFIRM,
    hub_runtime_state,
    hub_service_plan,
    hub_service_status,
    reload_local_hub,
    windows_interop_command,
)
...
from local_mcp_hub_graph_tools import (  # noqa: E402
    gitnexus_call,
    gitnexus_list_tools,
    graphify_call,
    graphify_list_tools,
    validate as graph_tools_validate,
)
from local_mcp_hub_headroom import call as headroom_call  # noqa: E402
from local_mcp_hub_headroom import validate as headroom_validate  # noqa: E402
...
from resource_request_admission import normalize_resource_input, url_batch_payload  # noqa: E402
...
from github_hub_client import resolve_github_cli  # noqa: E402
...
def local_command(cmd: list[str]) -> list[str]:
    """Resolve Hub-owned commands for the platform running this process."""

    resolved = list(cmd)
    if not resolved:
        return resolved
    if resolved[0].casefold() in {"python", "python.exe", "pythonw.exe"}:
        resolved[0] = console_python_executable()
    elif resolved[0].casefold() in {"gh", "gh.exe"}:
        resolved[0] = resolve_github_cli()
    if os.name != "nt" and len(resolved) > 1 and resolved[1].startswith("_bridge\\"):
        resolved[1] = resolved[1].replace("\\", "/")
    return resolved
...
def pmb_tool_call(tool: str, arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    if payload.get("transport_error"):
        payload["daemon_service"] = {
            "managed_by": "codex-pmb-memory.service",
            "hub_may_start_daemon": False,
            "retryable": True,
            "next_action": "python3 workspace/_bridge/local_pmb_memory_service.py validate",
            "policy": "Hub connects to the single systemd-owned daemon and never starts a second PMB process",
        }
...
class LocalMcpHub:
    ...
    def all_tool_specs(self) -> list[dict[str, Any]]:
        ...
        tools.extend(self.graph_tool_specs())
        tools.extend(self.headroom_tool_specs())
    ...
    def graph_tool_specs(self) -> list[dict[str, Any]]:
        return graph_tool_specs()

    def headroom_tool_specs(self) -> list[dict[str, Any]]:
        return headroom_tool_specs()
    ...
    def tools_call(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        if name == "gitnexus.list_tools":
            return text_result(gitnexus_list_tools(arguments))
        if name == "gitnexus.call":
            return text_result(gitnexus_call(arguments))
        if name == "graphify.list_tools":
            return text_result(graphify_list_tools(arguments))
        if name == "graphify.call":
            return text_result(graphify_call(arguments))
        if name == "headroom.compress":
            return text_result(headroom_call("headroom_compress", arguments))
        if name == "headroom.retrieve":
            return text_result(headroom_call("headroom_retrieve", arguments))
        if name == "headroom.stats":
            return text_result(headroom_call("headroom_stats", arguments))
    ...
    def resource_request(self, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            admission = normalize_resource_input(arguments)
            if admission.get("kind") == "url_list" and admission.get("ok"):
                batch_arguments = url_batch_payload(arguments, admission)
                batch_arguments["store_root"] = str(store_root) if store_root else ""
                batch_arguments["detail"] = str(arguments.get("detail") or "compact")
                result = self.resource_request_batch(batch_arguments)
                result["promoted_from"] = "resource.request"
    ...
    def github_gh(self, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        # `gh auth status` only reports the active authentication state. Keep
        # it available for capability probes while preserving the token-output
        # block above and acknowledgement requirements for auth mutations.
        read_patterns.add(("auth", "status"))
    ...
    def capabilities(self) -> dict[str, Any]:
        ...
        return {
            ...
            "hub_native": {
                ...
                "gitnexus": {
                    "mode": "hub_first_fresh_stdio_read_only",
                    "tools": ["gitnexus.list_tools", "gitnexus.call"],
                    "policy": "only upstream read-only MCP tools run; indexing and editor setup stay outside the Hub call",
                },
                "graphify": {
                    "mode": "hub_first_fresh_stdio_read_only",
                    "tools": ["graphify.list_tools", "graphify.call"],
                    "policy": "only managed external graph.json paths and upstream read-only MCP tools run",
                },
                "chrome_devtools": {
    ...
    def doctor(self) -> dict[str, Any]:
        ...
        if not service.get("unit_exists"):
            issues.append(
                {
                    "severity": "risk",
                    "code": "hub_user_unit_missing",
                    "next_action": f"python _bridge\\local_mcp_hub_process.py install --confirm {HUB_SERVICE_INSTALL_CONFIRM}",
                }
            )
        elif service_plan.get("installed_unit_sha256") != service_plan.get("unit_sha256"):
            issues.append({"severity": "risk", "code": "hub_user_unit_stale", "detail": service_plan})
        if service.get("unit_exists") and not service.get("enabled"):
            issues.append({"severity": "risk", "code": "hub_user_unit_not_enabled", "detail": service})
        if service.get("unit_exists") and not service.get("active"):
            issues.append({"severity": "risk", "code": "hub_user_unit_not_active", "detail": service})
        ...
        if not task.get("ok"):
            ...
            if task.get("startWhenAvailable") is not True:
                issues.append({"severity": "advisory", "code": "scheduled_task_start_when_available_disabled", "task": "CodexLocalMcpHub", "detail": task})
            if int(task.get("restartCount") or 0) < 3:
                issues.append({"severity": "advisory", "code": "scheduled_task_restart_count_too_low", "task": "CodexLocalMcpHub", "detail": task})
            if str(task.get("restartInterval") or "").upper() in {"", "PT0S"}:
                issues.append({"severity": "advisory", "code": "scheduled_task_restart_interval_missing", "task": "CodexLocalMcpHub", "detail": task})
        return {
    ...
    def validate(self) -> dict[str, Any]:
        ...
        graph_tools = graph_tools_validate()
        if not graph_tools.get("ok"):
            issues.append({"severity": "risk", "code": "graph_tools_adapter_invalid", "details": graph_tools})
        headroom = headroom_validate()
        if not headroom.get("ok"):
            issues.append({"severity": "risk", "code": "headroom_adapter_invalid", "details": headroom})
        for name in missing:
...
def scheduled_task_state(task_name: str = "CodexLocalMcpHub") -> dict[str, Any]:
    ...
    try:
        powershell_command = powershell_encoded_command(command)
        proc = subprocess.run(
            windows_interop_command(powershell_command[0], *powershell_command[1:]),
            cwd=str(ROOT),
            capture_output=True,
            text=True,
        ...
        "actions": [
            {
                ...
                "mode": "approved_user_systemd_install_or_ensure",
                "command": f"python _bridge\\local_mcp_hub_process.py install --confirm {HUB_SERVICE_INSTALL_CONFIRM}",
                "writes_files": True,
                "network_scope": "127.0.0.1 only",
                "recovery_policy": "atomically installs one user unit, enables it, and requires listener plus health acceptance",
            },
            {
                ...
                "writes_files": False,
                "network_scope": "127.0.0.1 only",
                "safety_boundary": "restarts only codex-local-mcp-hub.service; Windows task is used only when the WSL user unit is absent",
                "use_when": "running HTTP Hub exposes an older tool schema than local source or smoke/validate disagree after a code update",
            },
            {
                "id": "verify_windows_recovery_fallback",
                "mode": "read_only_fallback_check",
                "command": "Get-ScheduledTask -TaskName CodexLocalMcpHub | Select-Object TaskName,State,Settings",
                "writes_files": False,
                "network_scope": "none",
                "safety_boundary": "Task Scheduler is not the primary Hub lifecycle authority and must not replace the WSL unit",
                "use_when": "WSL user-systemd is unavailable and a bounded Windows wake/recovery route is required",
            }
        ],
    }
...
def parse_args() -> argparse.Namespace:
    ...
    reload_p.add_argument("--authorization-permit-ref", default="")
    reload_p.add_argument("--authorization-operation-id", default="")
    reload_p.add_argument("--authorization-thread-id", default="")
    reload_p.add_argument("--workflow-semantic-hash", default="")
    reload_p.add_argument("--authorization-state-root", default="")
...
def main() -> int:
    ...
    if args.command == "snapshot":
        ...
        payload = reload_local_hub(
            confirm_reload=bool(args.confirm_reload), port=int(args.port),
