...
def lightweight_auto_maintain_gate(
    *,
    ...
    rollover_detail: dict[str, Any] | None = None
    if current_active_path != saved_active_path:
        if not current_active or not saved_active_path:
            return {
                "ok": True,
                "should_scan": True,
        ...
        previous_active = Path(saved_active_path)
        try:
            previous_bytes = int(previous_active.stat().st_size)
            current_bytes = int(current_active.stat().st_size)
        except OSError:
            return {"ok": True, "should_scan": True, "reason": "active_candidate_rollover_stat_failed"}
        saved_previous_bytes = int(state.get("active_candidate_bytes") or 0)
        previous_growth = max(0, previous_bytes - saved_previous_bytes)
        if previous_bytes < saved_previous_bytes:
            return {"ok": True, "should_scan": True, "reason": "previous_active_candidate_shrank"}
        if previous_growth >= DEFAULT_AUTO_MAINTAIN_GROWTH_MB * 1024 * 1024:
            return {
                "ok": True,
                "should_scan": True,
                "reason": "previous_active_candidate_grew",
                "detail": {"growth_mb": round(previous_growth / (1024 * 1024), 2)},
            }
        if current_bytes >= DEFAULT_RISK_MB * 1024 * 1024:
            return {"ok": True, "should_scan": True, "reason": "new_active_candidate_is_risk_sized"}
        rollover_detail = {
            "previous": saved_active_path,
            "current": current_active_path,
            "previous_growth_mb": round(previous_growth / (1024 * 1024), 2),
            "current_mb": round(current_bytes / (1024 * 1024), 2),
        }
    if current_active and rollover_detail is None:
        try:
    ...
    return {
        "ok": True,
        "should_scan": False,
        "reason": "active_candidate_rolled_over_within_cooldown" if rollover_detail else "cooldown_metadata_unchanged",
        "writes_files": False,
        "detail": {
            "age_hours": round(age_hours, 2),
            "cooldown_hours": DEFAULT_AUTO_MAINTAIN_COOLDOWN_HOURS,
            "active_candidate_path": current_active_path,
            **({"rollover": rollover_detail} if rollover_detail else {}),
        },
    }
