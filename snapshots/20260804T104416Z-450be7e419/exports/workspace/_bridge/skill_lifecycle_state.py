...
def state_db_path(
    ...
    *,
    ...
    """Resolve one derived-state authority for every linked Work Git worktree."""

    configured = str(override or "").strip()
    if configured:
        return Path(configured).expanduser().resolve()
    common_dir = str(git_common_dir or "").strip()
    if not common_dir:
        try:
    ...
    if common_dir:
        ...
        authority_root = common_path.parent
        authority_bridge = authority_root / "workspace" / "_bridge"
        if authority_bridge.is_dir():
            return authority_bridge / "runtime" / "skill_lifecycle" / "skill_lifecycle.sqlite"
    return root / "_bridge" / "runtime" / "skill_lifecycle" / "skill_lifecycle.sqlite"


STATE_DB = state_db_path(override=os.environ.get("CODEX_SKILL_LIFECYCLE_STATE_DB", ""))
SCHEMA_VERSION = 2
...
def connect(path: Path = STATE_DB, *, readonly: bool = False) -> sqlite3.Connection:
    if readonly:
