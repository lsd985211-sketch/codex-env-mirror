...
def recover_owned_result_from_history_sources(
    ...
    if result_code and should_poll_session_store:
        ...
        if not bool(session_poll.get("ok")):
...
def audit_owned_result_recovery(queue: MobileQueue, task_id: str) -> dict[str, Any]:
    """Audit one exact task against durable session-store result evidence."""
    ...
    already_corrected = task_event_exists(queue, tid, "owned_result_corrective_reply_accepted")
    accepted_reply_present = task_event_exists(queue, tid, "final_reply_weixin_accepted")
    status = str(task.get("status") or "")
    mode = "corrective_reply" if status == "pushed_to_wecom" or str(task.get("push_status") or "") == "pushed_to_wecom" else "normal_completion"
    reason = ""
    if not candidate_text:
        ...
        reason = "existing_task_result_is_not_a_known_placeholder"
    elif mode == "normal_completion" and status not in {"sent_to_codex", "processing", "done", "push_failed"}:
        reason = f"unsupported_task_status:{status}"
    elif mode == "normal_completion" and accepted_reply_present:
        reason = "accepted_reply_already_present"
    ...
    return {
        ...
        "task_id": tid,
        "eligible": eligible,
        "reason": reason or "exact_owned_result_recovery_ready",
        "mode": mode,
        "task_status": status,
        "push_status": str(task.get("push_status") or ""),
        ...
        "accepted_reply_already_present": accepted_reply_present,
    }
...
def recover_owned_result(
    ...
    *,
    ...
    """Recover one exact task; corrective resend is allowed only for a pushed placeholder."""
    ...
    if not bool(candidate.get("ok")) or not is_usable_owned_result_text(candidate_text):
        return {
            ...
            "applied": False,
            "task_id": tid,
            "reason": str(candidate.get("reason") or "exact_owned_result_changed_before_apply"),
            "session_store": candidate,
        }
    ...
    event_payload = {
        ...
        "policy": "single explicit task only; sender acceptance receipt is authoritative",
    }
    queue.add_event("local", "owned_result_recovery_started", event_payload, tid)

    mode = str(audit.get("mode") or "")
    if mode == "normal_completion":
        queue.complete(tid, candidate_text, status="done")
        task = queue.get_task(tid) or task
    reply = push_final_reply(queue, task, candidate_text, config)
    accepted = bool(reply.get("ok")) and final_reply_delivery_accepted(reply)
    if not accepted:
        queue.add_event(
            "wecom",
            "owned_result_corrective_reply_failed" if mode == "corrective_reply" else "owned_result_recovery_reply_failed",
        ...
        return {
            ...
            "applied": True,
            "task_id": tid,
            "reason": "weixin_sender_did_not_accept_owned_result",
            "reply": reply,
            "source": source,
        }
    ...
    event_type = "owned_result_corrective_reply_accepted" if mode == "corrective_reply" else "owned_result_recovery_reply_accepted"
    queue.add_event("wecom", event_type, {**event_payload, "delivery_accepted": True}, tid)
    return {
        "ok": True,
        "applied": True,
        "task_id": tid,
