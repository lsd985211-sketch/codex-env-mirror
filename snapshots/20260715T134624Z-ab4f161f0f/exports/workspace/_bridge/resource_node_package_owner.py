...
DEFAULT_DEPENDENCY_ROOT = BRIDGE_ROOT / "runtime_dependencies" / "node"
...
def _target_dir(request: dict[str, Any], package_name: str) -> tuple[Path, bool]:
    ...
    return (DEFAULT_DEPENDENCY_ROOT / safe_name).resolve(), False
...
def validate() -> dict[str, Any]:
    ...
    return {
        ...
        "ok": blocked.get("error_class") == "install_requires_explicit_approval" and NODE.exists() and NPM_CLI.exists(),
        "approval_gate_ok": blocked.get("error_class") == "install_requires_explicit_approval",
        "node_available": NODE.exists(),
        "npm_cli_available": NPM_CLI.exists(),
