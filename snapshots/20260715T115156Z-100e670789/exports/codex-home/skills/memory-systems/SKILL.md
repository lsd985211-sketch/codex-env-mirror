---
name: memory-systems
description: "Agent 记忆系统设计：跨会话持久化、实体一致性。触发：agent memory, 持久化, knowledge graph"
---
...
- Decide whether the task is about persistence, retrieval, entity consistency, or temporal validity.
- Choose the smallest memory layer that can answer the question.
- If the request is about operational state, verify it live instead of assuming memory is current.
...
For local long-lived systems, memory is a work-control layer rather than a passive archive. The operating loop is:

1. Recall: retrieve the smallest relevant memory layer before and during work.
2. Verify: check drift-prone facts against current files, logs, doctor output, metrics, or tools.
3. Act: let memory guide where to inspect, which prior root causes to test, and which commands to reuse.
4. Record: when a reusable lesson is verified, write a concise candidate or use the appropriate memory system; do not leave it only in chat.
5. Iterate: route durable changes through the iteration layer so skills, baselines, docs, command registries, and memory stay aligned.
6. Post-fix closure: after resolving a non-trivial issue, explicitly decide whether memory, checkpoint, skill, baseline, or maintenance-contract updates are needed; record the decision or state why no durable update is needed.

Use this loop continuously for bridge, MCP/tooling, mail, scheduler, maintenance, performance, PMB, skills, baselines, seed modules, and automation. Do not treat memory as only a task-start checklist.

Candidate notes are not final memory. They need review, deduplication, conflict checks, source scope, and a destination decision. Use `_bridge/memory_governance.py snapshot|doctor|repair-plan|validate|metrics` for the local governance surface when working in the mcsmanager workspace.

When a candidate note is created, do not only ask for generic approval. Present
a detailed absorption proposal first: source note, proposed destination
namespace, stable points to keep, incident details to exclude, sensitive-content
screening result, existing-memory conflict or duplicate check, expected future
behavior impact, backup boundary, and validation plan. Use
`_bridge/memory_governance.py absorb-plan --limit 20` for the read-only draft.

PMB itself also needs periodic organization. Use
`_bridge/memory_governance.py pmb-organize-plan --limit 1000` to produce a
dry-run plan for duplicate facts, drift-prone current-state memories, and
sensitive-content candidates. The default action is no write: merging,
archiving, deletion, or rewriting PMB requires explicit approval with details,
backup, and post-apply validation. Safe automatic actions are limited to
index/metric refreshes after validation; never auto-delete or auto-overwrite
long-term memories.

When reading PMB organize plans, treat severity as a routing signal rather than
a verdict. Medium/high sensitive candidates should contain credential-like
values or bearer/key/value forms; generic mentions of token, secret, password,
or authorization are policy mentions unless a real value shape is present.
Medium drift candidates should be live operational claims such as current-turn
tool state, active session binding, reboot state, or transport failures.
Dated validation records, migrated implementation history, stable baselines,
and temporal user-profile facts are reviewable context but should usually stay
low severity unless they claim to describe the live state.
...
- State which memory layer is appropriate and why.
- Distinguish verified facts from memory-derived inference.
