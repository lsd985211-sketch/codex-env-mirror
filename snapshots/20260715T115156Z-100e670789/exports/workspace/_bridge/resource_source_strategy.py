...
def _documentation_route(request: dict[str, Any], route: dict[str, Any]) -> dict[str, Any]:
    ...
    if any(_domain_matches(domain, ("learn.microsoft.com", "microsoft.com")) for domain in domains):
...
def classify_resource_kind(request: dict[str, Any], route: dict[str, Any]) -> str:
    ...
    if intent == ResourceIntent.PACKAGE_DEPENDENCY or primary_tool == "package_manager":
        return "package"
    if intent == ResourceIntent.DOCUMENTATION_LOOKUP or primary_tool in {"context7", "microsoftdocs"}:
...
def validate() -> dict[str, Any]:
    ...
    checks = [
        ...
        {"name": "openai_docs_use_official_search", "ok": openai_docs["candidates"][0]["owner_tool"] == "generic_search" and openai_docs["classification_evidence"]["documentation_route_kind"] == "official_vendor_docs"},
