    ...
    *,
    ...
    if running and apply:
        ...
        return {
            ...
            "policy": "automatic compaction runs only at restart boundary when Codex process family is not running",
        }
    if apply:
        state = read_json_file(AUTO_MAINTAIN_STATE_PATH)
        root = sessions_root or default_sessions_root()
    ...
    if apply:
        gate = auto_maintain_gate(
            ...
            thresholds=thresholds,
            state=state,
            include_active_candidate=False,
        )
    ...
    return {
        ...
        "plan": plan,
        "result": result,
        "policy": "automatic compaction runs only at restart boundary, never rewrites the active candidate, and skips while Codex is running",
    }
...
def validate(*, sessions_root: Path | None = None, thresholds: Thresholds = Thresholds()) -> dict[str, Any]:
    ...
    checks = [
        ...
        {
            "name": "auto_maintain_active_candidate_excluded",
            "ok": bool(gate.get("detail", {}).get("active_candidate_excluded")),
            "detail": "automatic maintenance may compact inactive sessions only; the active candidate requires checkpoint/continuation handling",
        },
        {
