...
def model_signature(config: dict[str, Any], catalog: Path | None) -> dict[str, Any]:
    provider_name = ""
    provider = config.get("model_provider")
    providers = config.get("model_providers") if isinstance(config.get("model_providers"), dict) else {}
...
def desktop_model_refresh_state(
    ...
    *,
    ...
    signature = model_signature(config, catalog)
...
def pid_is_alive(pid: Any) -> bool:
    ...
    if os.name == "nt":
        ...
        handle = kernel32.OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION, False, numeric_pid)
        if not handle:
            return False
        try:
...
def run_once(apply: bool, startup_delay_seconds: int = 0, phase: str = "manual") -> dict[str, Any]:
    ...
    if phase == "pre-start-static":
        session_store_maintenance = safe_session_store_auto_maintain(apply=repair_apply)
        ...
        result = {
            ...
            "ok": bool(repair.get("ok")) and bool(after_audit.get("critical_ok")),
            "phase": phase,
            "applied": bool(repair_apply and repair.get("changed")),
            "generated_at": utc_now(),
            "before": before_audit,
            "repair": repair,
            "session_store_maintenance": session_store_maintenance,
            ...
            "policy": "static preflight performs merge-only baseline repair plus safe session compaction; runtime model, process, and network checks remain in their owning phases",
        }
        append_log("run_once_static", result)
        return result
    maintenance_apply = repair_apply
    config = safe_toml(CODEX_CONFIG) if CODEX_CONFIG.exists() else {}
    catalog_path = model_catalog_path(config, CODEX_CONFIG)
    catalog_normalization = catalog_normalization_disabled(catalog_path)
    session_store_maintenance = safe_session_store_auto_maintain(apply=maintenance_apply)
    process_manager_hygiene = compact_process_manager_state(apply=maintenance_apply)
