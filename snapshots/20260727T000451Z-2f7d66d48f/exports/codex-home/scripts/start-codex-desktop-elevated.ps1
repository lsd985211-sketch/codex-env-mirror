...
if ($null -ne $script:CodexDesktopRestartRequest) {
    ...
    Write-CodexDesktopRestartReceipt `
        -Ok $newVersionReady `
        -Status $(if ($newVersionReady) { "completed" } else { "failed" }) `
        -Reason $(if ($newVersionReady) { "governed_desktop_relaunch_ready" } else { "governed_desktop_relaunch_not_ready" }) `
        -AfterPids $afterRestartPids `
        -CdpReady $newVersionReady
}
Start-CodexStartupBaselineRepairAsync
Write-PhaseLog "Baseline repair phase finished"
