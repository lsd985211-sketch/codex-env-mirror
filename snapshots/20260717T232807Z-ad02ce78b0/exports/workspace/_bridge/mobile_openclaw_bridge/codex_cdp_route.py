...
from codex_cdp_route_process import launch_start_script_detached
...
def ensure_codex_cdp(config: dict[str, Any]) -> dict[str, Any]:
    ...
    for candidate in settings["start_scripts"]:
        if not candidate.exists():
            continue
        launch = launch_start_script_detached(
            candidate,
            port=int(port),
            cwd=PROJECT_ROOT,
        )
    ...
    return {
        ...
        "version_ready": bool(current_version.get("ok")),
        "reason": (
            "codex_cdp_start_in_progress"
            if any(bool(item.get("launched")) for item in launches)
            else "codex_cdp_transport_not_ready"
            if not current.get("ok")
            else "codex_cdp_version_not_ready"
        ),
