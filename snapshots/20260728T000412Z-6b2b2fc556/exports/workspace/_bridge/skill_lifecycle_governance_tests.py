...
import skill_active_catalog
import skill_admission_discover as admission
import skill_lifecycle_governance as lifecycle
import skill_lifecycle_state
import skill_orchestrator
from shared import backup_router


class SkillLifecycleGovernanceTests(unittest.TestCase):
    def test_linked_worktrees_share_common_skill_lifecycle_state(self) -> None:
        with TemporaryDirectory() as directory:
            root = Path(directory)
            authority = root / "main"
            linked = root / "linked" / "workspace"
            (authority / "workspace" / "_bridge").mkdir(parents=True)
            linked.mkdir(parents=True)

            resolved = skill_lifecycle_state.state_db_path(
                linked,
                git_common_dir=str(authority / ".git"),
            )

            self.assertEqual(
                resolved,
                authority / "workspace" / "_bridge" / "runtime" / "skill_lifecycle" / "skill_lifecycle.sqlite",
            )

    def test_skill_lifecycle_state_override_remains_available(self) -> None:
        with TemporaryDirectory() as directory:
            override = Path(directory) / "isolated.sqlite"

            resolved = skill_lifecycle_state.state_db_path(override=str(override))

            self.assertEqual(resolved, override.resolve())

    def test_work_git_projection_coverage_requires_both_governed_roots(self) -> None:
        with TemporaryDirectory() as directory:
            root = Path(directory)
            source = root / "branch" / "codex-home" / "skills"
            projected_source = root / "main" / "codex-home" / "skills"
            canonical = root / "canonical"
            runtime_root = root / "runtime"
            (source / "alpha").mkdir(parents=True)
            (projected_source / "alpha").mkdir(parents=True)
            (source / "alpha" / "SKILL.md").write_text("# alpha\n", encoding="utf-8")
            (projected_source / "alpha" / "SKILL.md").write_text("# alpha\n", encoding="utf-8")
            canonical.mkdir()
            runtime_root.mkdir()
            (canonical / "alpha").symlink_to(projected_source / "alpha", target_is_directory=True)
            with (
                mock.patch.object(lifecycle, "WORK_GIT_SKILLS", source),
                mock.patch.object(lifecycle, "GLOBAL_SKILLS", canonical),
                mock.patch.object(lifecycle, "RUNTIME_SKILLS", runtime_root),
            ):
                missing = lifecycle.work_git_projection_coverage()
                (runtime_root / "alpha").symlink_to(projected_source / "alpha", target_is_directory=True)
                current = lifecycle.work_git_projection_coverage()

            self.assertFalse(missing["ok"])
            self.assertEqual(1, missing["issue_count"])
            self.assertEqual("runtime", missing["issues"][0]["target"])
    ...
    def test_configured_plugin_without_local_skill_payload_is_deferred_to_plugin_health_owner(self) -> None:
        with TemporaryDirectory() as directory:
            root = Path(directory)
            config = root / "config.toml"
            cache = root / "cache"
            ...
            self.assertTrue(snapshot["ok"])
            self.assertEqual(
                snapshot["deferred_plugins_without_local_skill_payload"],
                ["browser@openai-bundled"],
            )
            self.assertEqual(snapshot["plugin_health_authority"], "codex_plugin_config_health.py")

    def test_malformed_plugin_config_still_fails_catalog_readability(self) -> None:
        with TemporaryDirectory() as directory:
            root = Path(directory)
            config = root / "config.toml"
            config.write_text('[plugins."broken"\n', encoding="utf-8")

            snapshot = skill_active_catalog.catalog_snapshot(
                plugin_cache=root / "cache",
                config_path=config,
            )

            self.assertFalse(snapshot["ok"])
            self.assertIn("TOMLDecodeError", snapshot["config_parse_error"])
