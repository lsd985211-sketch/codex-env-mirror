...
DEFAULT_ROOTS = [
    ...
    ROOT / "_bridge" / "business_environment_control_plane.py",
...
def validate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    explicit_target_ok = (
        normalize_module_path(explicit_target_sample.get("owner_module")) == "_bridge/code_maintainability.py"
...
def load_module_index(*, prefer_full: bool = True) -> dict[str, Any]:
    candidates = (
        (MODULE_CAPABILITY_INDEX, MODULE_CAPABILITY_CORE_INDEX)
        if prefer_full
        else (MODULE_CAPABILITY_CORE_INDEX, MODULE_CAPABILITY_INDEX)
    )
    for index_path in candidates:
        if index_path.exists():
            return json.loads(index_path.read_text(encoding="utf-8"))
    files = iter_python_files(DEFAULT_ROOTS)
    modules = [
        module_capability_for_analysis(item)
        for item in (analyze_file(path) for path in files)
        if isinstance(item, dict) and item.get("ok") and item.get("path")
    ]
    modules.sort(key=lambda item: (str(item.get("boundary") or ""), str(item.get("module") or "")))
    return {
        ...
        "generated_at": now_iso(),
        "source": {
            "kind": "derived_in_memory_fallback",
            "source_of_truth": ["python_source", "_bridge/docs/code_maintainability_guidelines.md"],
            "scan_scope": "governance_default",
            "file_count": len(modules),
        },
        "rules": {
            "index_is_not_source_of_truth": True,
            "fallback_writes_runtime": False,
        },
        "modules": modules,
    }
...
def module_context(args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        ...
        "ok": True,
        "command_contract": cli_command_contract("module-context"),
        "generated_at": now_iso(),
...
def direct_target_route(target: str) -> dict[str, Any] | None:
    """Analyze an explicit in-workspace Python target when the derived index is absent."""

    raw = Path(str(target or "").replace("\\", "/"))
    ...
    for candidate in candidates:
        ...
        if resolved.is_file() and resolved.suffix == ".py":
            return module_route_for_analysis(analyze_file(resolved))
    return None
...
def placement_plan(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if target and selected_route is None:
        selected_route = direct_target_route(target)
    change = classify_change_kind(message, terms)
    owner_module = str((selected_route or {}).get("module") or target or "")
    normalized_intent = f"{message} {' '.join(terms)}".lower()
    global_control_plane_intent = (
        ("business environment" in normalized_intent or "业务环境" in normalized_intent)
        and ("control" in normalized_intent or "控制面" in normalized_intent)
        and ("global" in normalized_intent or "全局" in normalized_intent or "整个 codex" in normalized_intent.lower())
    )
    if global_control_plane_intent and not target:
        owner_module = "_bridge/business_environment_control_plane.py"
        selected_route = {
            "module": owner_module,
            "purpose": "read_only_cross_domain_business_environment_control_plane",
            "boundary": "workflow_governance",
            "state_behavior": "read_only_shadow",
            "validation": ["python _bridge\\business_environment_control_plane_tests.py"],
    ...
    explicit_target_owner = bool(
        target
        and selected_route
        and not large_file_risk
        and change["kind"] not in {"permission_boundary", "process_lifecycle"}
    )
    if explicit_target_owner:
        existing_purpose = {"ok": False, "reason": "explicit_target_owner_boundary"}
    if global_control_plane_intent:
        existing_purpose = {"ok": False, "reason": "declared_global_control_plane_boundary"}
    ...
    return {
        ...
        "ok": True,
        "command_contract": cli_command_contract("placement-plan"),
        "generated_at": now_iso(),
        "message": message,
        "terms": terms,
        "target": target,
...
def cli_command_contract(command: str) -> dict[str, Any]:
    """Project the current argparse contract without maintaining a second option list."""
    ...
    signature_payload = {"command": command, "options": options}
    return {
        "schema": "code_maintainability.command_contract.v1",
        "owner": "code_maintainability",
        "entrypoint": "python _bridge\\code_maintainability.py",
        "command": command,
        "accepted_options": options,
        "parser_signature": hashlib.sha256(
            json.dumps(signature_payload, sort_keys=True, separators=(",", ":")).encode("utf-8")
        ).hexdigest(),
        "reuse_rule": "reuse_until_parser_signature_or_owner_changes_or_an_argument_is_rejected",
        "targeted_help": f"python _bridge\\code_maintainability.py {command} --help",
    }
