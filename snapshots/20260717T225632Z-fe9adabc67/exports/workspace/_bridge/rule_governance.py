...
def pattern_matches(path: str, pattern: str) -> bool:
    ...
    if len(normalized) > 2 and normalized[1:3] == ":/":
        return fnmatch.fnmatchcase(str(Path(candidate).resolve()).replace("\\", "/").casefold(), normalized.casefold())
...
def doctor(*, full: bool = False) -> dict[str, Any]:
    ...
    for index, surface in enumerate(surfaces):
        ...
        if strength in {"instruction_only", "observational"}:
            issues.append(
                {
                    "severity": "risk",
                    "code": "rule_not_runtime_enforced",
                    "rule_id": rule_id,
                    "effect": "declared semantics remain authoritative, but compliance depends on instruction following or closeout observation",
                    "enforcement_strength": strength,
                }
            )
        if lifecycle in {"active", "deprecated", "foreign_scoped"} and not expand_source(str(surface.get("source") or "")):
    ...
    for index, contract in enumerate(activation_contracts):
        ...
        for consumer in contract.get("consumers") or []:
            if not expand_source(str(consumer or "")):
...
def impact(changed: Iterable[str]) -> dict[str, Any]:
    ...
    rows: list[dict[str, Any]] = []
    unmatched: list[str] = []
    for value in dict.fromkeys(str(item) for item in changed if str(item).strip()):
        normalized = value.replace("\\", "/")
        matches: list[tuple[dict[str, Any], str]] = []
        for item in surfaces:
            rule_id = str(item.get("rule_id") or "")
            if pattern_matches(normalized, str(item.get("source") or "")):
                matches.append((item, "authority_source"))
                continue
            if any(pattern_matches(normalized, pattern) for pattern in consumers_by_rule.get(rule_id, [])):
    ...
    return {
        ...
        "ok": bool(rows) or not list(changed),
        "generated_at": now_iso(),
...
def validate() -> dict[str, Any]:
    ...
    checks = [
        ...
        {"name": "foreign_agents_classified", "ok": int(snap.get("foreign_scoped_count") or 0) >= 1, "detail": snap.get("foreign_scoped_count")},
        {
