...
- Confirm deck type, audience, language, and target slide count before outline generation.
- Treat outline review and prompt review as real gates when enabled.
- Back up any existing outline, prompts, or images before regenerating.

## Output Contract

- Return the outline state, prompt state, and final deck paths.
- Report which slides were kept, merged, split, or regenerated.
- Mention any unresolved readability or overflow risk.
