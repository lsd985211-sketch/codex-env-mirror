...
def ensure_wsl_runtime_projection(*, enabled: bool, dry_run: bool) -> dict:
    ...
    if not enabled:
        return {"ok": True, "status": "not_required", "enabled": False, "changed": False}
    wsl = shutil.which("wsl.exe")
    if not wsl:
        return {"ok": False, "status": "wsl_executable_missing", "enabled": True, "changed": False}
    command = [
        wsl,
        "-d",
        WSL_DISTRIBUTION,
    ...
    return {
        "ok": ok,
        "status": "planned" if dry_run and ok else "applied" if ok else "owner_failed",
        "enabled": True,
