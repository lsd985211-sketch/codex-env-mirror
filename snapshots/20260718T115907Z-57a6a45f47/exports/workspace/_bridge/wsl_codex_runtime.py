...
def _session_projection_file(source: Path, target: Path) -> tuple[bool, str]:
    """Copy a JSONL session and translate only its session metadata cwd."""
    translated_from = ""
    translated_to = ""
            ...
            for raw_line in source_handle:
                ...
                if record.get("type") == "session_meta":
                    ...
                    if isinstance(payload, dict) and isinstance(payload.get("cwd"), str):
                        translated_from = payload["cwd"]
                        translated_to, _ = windows_cwd_to_wsl(translated_from)
                        if translated_to != translated_from:
                            payload["cwd"] = translated_to
