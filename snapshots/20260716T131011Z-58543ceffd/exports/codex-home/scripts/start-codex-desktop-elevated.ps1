...
function Invoke-ShortcutSelfRepair {
    ...
    try {
        ...
        Write-StartLog "Shortcut self-repair completed."
    } catch {
        Write-StartLog "Shortcut self-repair failed: $($_.Exception.Message)"
    }
}
...
function Invoke-CodexConfigPreflightSync {
    ...
    try {
        ...
        $needsRestart = [bool]$result.needs_codex_restart
        $beforeOk = $false
        $afterOk = $false
...
function Invoke-CodexSessionStorePreLaunchMaintenance {
    ...
    $workspace = "C:\Users\45543\Downloads\mcsmanager_windows_release\mcsmanager"
    $maintenanceScript = Join-Path $workspace "_bridge\codex_session_store_doctor.py"
    if (-not (Test-Path -LiteralPath $maintenanceScript)) {
        Write-StartLog "Codex session-store pre-launch maintenance skipped: script missing at $maintenanceScript"
        return @{ Ok = $false; Applied = $false; Error = "missing_session_store_doctor" }
    }
    $pythonExe = Resolve-CodexStartupPython
    if (-not $pythonExe) {
    ...
    try {
        ...
        $output = & $pythonExe $maintenanceScript auto-maintain --apply --boundary pre-launch 2>&1
        if ($LASTEXITCODE -ne 0) {
        ...
        if ($null -ne $result.PSObject.Properties["reason"]) {
            $reason = [string]$result.PSObject.Properties["reason"].Value
        } elseif ($null -ne $result.PSObject.Properties["gate"] -and $null -ne $result.gate.PSObject.Properties["reason"]) {
            $reason = [string]$result.gate.PSObject.Properties["reason"].Value
        } elseif ($null -ne $result.PSObject.Properties["result"] -and $null -ne $result.result.PSObject.Properties["reason"]) {
            $reason = [string]$result.result.PSObject.Properties["reason"].Value
        }
        Write-StartLog "Codex session-store pre-launch maintenance completed. Applied=$([bool]$result.should_apply); ResultApplied=$([bool]$result.result.applied); Reason=$reason"
        return @{ Ok = [bool]$result.ok; Applied = [bool]$result.result.applied; Reason = $reason }
    } catch {
...
function Start-CodexModelProviderWatcherAsync {
    ...
    try {
        $process = Start-Process `
            -FilePath $pythonExe `
            -ArgumentList @($watcherScript, "watch", "--poll-seconds", "2", "--debounce-seconds", "1.5") `
            -WorkingDirectory $workspace `
            -WindowStyle Hidden `
            -PassThru
        Write-StartLog "Codex model provider watcher launch requested. Pid=$($process.Id); lock prevents duplicate active watchers."
    } catch {
...
function Get-CodexDesktopProcessRecords {
    ...
    try {
        ...
        Write-StartLog "Codex process scan failed: $($_.Exception.Message)"
    }
    return $records
}
...
function Wait-CodexProcessQuiescence {
    ...
    do {
        ...
        if (Test-CodexCdpVersionReady -Port $RemoteDebuggingPort -TimeoutSeconds 1) {
            return @{ State = "cdp_ready"; Attempts = $attempt; Processes = @(Get-CodexDesktopProcessRecords) }
        }
    ...
    return @{ State = "still_running"; Attempts = $attempt; Processes = @(Get-CodexDesktopProcessRecords) }
}
...
function Ensure-CodexRestartBoundaryClean {
    ...
    $allowCleanup = ($env:CODEX_DISABLE_STALE_CODEX_CLEANUP -ne "1") -and (-not $dryRun)
    if (-not $allowCleanup) {
        Write-StartLog "Codex restart boundary still dirty, but cleanup is disabled. Processes=$(Get-CodexProcessSummary -Processes $remaining)"
        return @{ Ok = $false; Action = "cleanup_disabled"; ProcessCount = $remaining.Count }
    }
...
Invoke-ShortcutSelfRepair
Write-PhaseLog "Shortcut self-repair phase finished"
...
if (Test-CodexCdpVersionReady -Port $remoteDebuggingPort) {
