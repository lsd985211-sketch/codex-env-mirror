...
def doctor(snap: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if not snap.get("cli_hub", {}).get("command"):
        issues.append({"severity": "blocker", "code": "cli_hub_missing", "message": "cli-hub executable is not on PATH"})
    if not snap.get("cli_hub", {}).get("list_ok"):
        issues.append({"severity": "risk", "code": "catalog_unavailable", "message": "cli-hub list --json failed"})
    ...
    for name in sorted(REQUIRED_LOCAL_HARNESSES - installed_names):
        issues.append({"severity": "risk", "code": "required_local_harness_missing", "entrypoint": name})
    return {
...
def validate(snap: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if not office_surface.get("ok") or missing_paths:
        failures.append({"severity": "risk", "code": "office_harness_command_surface_invalid", "missing_paths": missing_paths})
    return {
        "schema": "cli_anything.validate.v1",
        "ok": bool(snap.get("ok")) and not failures,
        "generated_at": now_iso(),
        "failures": failures,
        "advisory_count": sum(1 for item in doc.get("issues", []) if item.get("severity") == "advisory"),
        "office_harness": {"ok": office_surface.get("ok"), "missing_paths": missing_paths},
    }
