...
- Confirm whether the input is plain text or already markdown.
- Decide whether to optimize structure or only fix typography.
- Back up any existing formatted output before overwriting.

## Output Contract

- Return the formatted file path and the kinds of changes made.
- Mention whether frontmatter was added or preserved.
- If the file was already markdown, state which route was chosen.
