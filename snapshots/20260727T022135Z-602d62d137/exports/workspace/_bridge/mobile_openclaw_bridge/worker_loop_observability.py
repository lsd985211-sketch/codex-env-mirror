...
PASSIVE_LOOP_ACTIONS = frozenset(
    {
        "idle",
        "recovery_cycle",
        "idle_waiting_owned_result_gate",
        "idle_no_dispatchable_thread",
        "idle_thread_mismatch",
        "paused",
        "stop_requested",
    }
)
...
def worker_loop_should_log(result: dict[str, Any], previous_signature: str) -> tuple[bool, str]:
    ...
    should_log = (not ok) or bool(queue_activity) or action not in PASSIVE_LOOP_ACTIONS or bool(skipped)
...
def worker_loop_has_activity(result: dict[str, Any]) -> bool:
    ...
    if any(count > 0 for count in activity_counts):
        return True
    if result.get("action") not in PASSIVE_LOOP_ACTIONS:
        return True
