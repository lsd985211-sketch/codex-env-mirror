...
def _git_config_env(env: dict[str, str], *entries: tuple[str, str]) -> dict[str, str]:
    """Append per-process Git config without changing user or system config."""
    result = dict(env)
    count = int(result.get("GIT_CONFIG_COUNT") or 0)
...
def _git_http_1_1_env(env: dict[str, str]) -> dict[str, str]:
    """Return a per-process Git HTTP/1.1 overlay without changing user config."""
    return _git_config_env(env, ("http.version", "HTTP/1.1"))


def git_transport_plan(remote_url: str) -> dict[str, Any]:
    """Build bounded forward-only Git transport attempts for a GitHub remote."""
    if "github.com" not in str(remote_url or "").lower():
        return {
            "schema": f"{SCHEMA_PREFIX}.git_transport_plan.v1",
            "ok": True,
            "used": False,
            "reason": "non_github_remote",
            "attempts": [{"name": "default", "env": {}}],
            "writes_system_proxy": False,
        }
    selected = route_plan(
        target_kind="github",
    ...
    selected_env = {str(key): str(value) for key, value in (plan.get("env") or {}).items()}
    for key in plan.get("unset_env") or []:
        selected_env[str(key)] = ""
    # Windows Git launched from WSL does not consistently consume the proxy
    # environment inherited across the interop boundary.  Project the same
    # route into Git's native per-process configuration as the transport
    # authority; keep the environment variables only for compatible runtimes.
    selected_proxy = str(plan.get("proxy_url") or selected_env.get("HTTPS_PROXY") or "").strip()
    if selected_proxy:
        selected_env = _git_config_env(selected_env, ("http.proxy", selected_proxy))
    attempts: list[dict[str, Any]] = [
        {"name": "selected_route", "env": selected_env},
        {"name": "selected_route_http_1_1", "env": _git_http_1_1_env(selected_env)},
    ]
    direct = _git_http_1_1_env({
        **direct_env("codex_gateway:github:git"),
        "HTTP_PROXY": "",
        "HTTPS_PROXY": "",
        "ALL_PROXY": "",
        "http_proxy": "",
    ...
    if direct != attempts[-1]["env"]:
        attempts.append({"name": "direct_http_1_1", "env": direct})
    attempts.append({
    ...
    return {
        ...
        "selected_route": {
            ...
            "git_native_proxy_projected": bool(selected_proxy),
        },
        "attempts": attempts[:4],
        "writes_system_proxy": False,
        "writes_git_config": False,
        "fallback_policy": (
            "forward_only_per_process; after an ambiguous push result read the remote branch back; "
            "retry the selected route at most once inside the same publication; "
            "stop after the first verified remote result"
        ),
    }
...
def parse_args() -> argparse.Namespace:
    ...
    git_transport = sub.add_parser("git-transport-plan")
    git_transport.add_argument("--remote-url", required=True)
...
def main() -> int:
    ...
    try:
        ...
        elif args.cmd == "git-transport-plan":
            emit(git_transport_plan(args.remote_url))
        elif args.cmd == "smoke":
