...
def decision_apply(
    ...
    *,
    ...
    try:
        ...
        for ref in refs:
            current = conn.execute("SELECT status,revision FROM review_items WHERE review_id=?", (ref["review_id"],)).fetchone()
            ...
            results.append({"review_id": ref["review_id"], "revision": ref["revision"], "from_status": "pending", "status": target_status})
        result = {"schema": "workflow_review_queue.decision_apply.v1", "ok": True, "decision_id": str(row["decision_id"]), "package_id": str(row["package_id"]), "action": action, "results": results}
...
def decision_readback(decision_id: str, *, db_path: Path | None = None) -> dict[str, Any]:
    ...
    return {"schema": "workflow_review_queue.decision_readback.v1", "ok": True, "decision_id": decision_id, "package_id": str(row["package_id"]), "status": str(row["status"]), "action": str(row["action"]), "user_evidence_ref": str(row["user_evidence_ref"]), "result": result}
