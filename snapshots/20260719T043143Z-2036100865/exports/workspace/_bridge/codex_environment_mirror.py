...
def compact_operation_receipt(action: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    for key in ("source_authority", "validation", "commit", "retention_commit", "metadata_commit", "push"):
        value = payload.get(key)
        if value in (None, "", [], {}):
            continue
        if key == "validation":
...
def publish(confirm: str, *, changed_paths: list[str] | None = None, remote: str = "", branch: str = "") -> dict[str, Any]:
    ...
    try:
        with exclusive_operation_lock(lock_path, "publish"):
            ...
            committed_candidate = reusable_committed_snapshot_for_publish(source_authority) if not changed_paths else {
                "ok": False,
