...
def reconcile_declared_source_drift(confirm: str, changed_paths: list[str]) -> dict[str, Any]:
    ...
    scope = affected_source_plan(normalized_paths)
    plan = drift_plan()
...
def push_receipt(*, remote: str = "", branch: str = "") -> dict[str, Any]:
    ...
    network_attempts, network_route = git_network_attempts_for_remote(str(remote_url.get("stdout", "")))
    if not branch_name:
    ...
    for attempt in network_attempts:
        ...
        before = git_result(["ls-remote", "--heads", remote_name, branch_name], timeout=120, extra_env=env)
        ...
        if not before.get("ok"):
            ...
            attempt_receipts.append(row)
            continue
        push = git_result(["push", remote_name, f"HEAD:{branch_name}"], timeout=600, extra_env=env)
        ...
        if not push.get("ok"):
            ...
            attempt_receipts.append(row)
            continue
        verify = git_result(["ls-remote", "--heads", remote_name, branch_name], timeout=120, extra_env=env)
...
def _refresh_unlocked(confirm: str, changed_paths: list[str] | None = None, *, state_write_barrier: state_write_authority.StateWriteLease | None = None) -> dict[str, Any]:
    started = time.perf_counter()
...
def refresh(confirm: str, changed_paths: list[str] | None = None) -> dict[str, Any]:
    ...
    try:
        with exclusive_operation_lock(lock_path, "refresh"):
            ...
            try:
                return _refresh_unlocked(confirm, normalized_changed_paths, state_write_barrier=barrier)
            finally:
