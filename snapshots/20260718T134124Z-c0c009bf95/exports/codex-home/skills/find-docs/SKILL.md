---
name: find-docs
description: "Routing skill for current library, framework, API, SDK, package, or CLI documentation lookups via Context7. Use when the user needs official/current documentation or code examples for a named software dependency or developer tool. This skill decides the Context7 docs path; if the task is generic webpage retrieval, site crawling, or non-library web research, route to the web acquisition layer instead."
---
...
- Use this skill when the task is specifically about current library/framework/API/SDK/CLI documentation.
- Use it to route the request into the correct Context7 docs lookup flow.
- Do not use it for generic webpage capture, blog/article scraping, or multi-platform web research.
- Do not keep ownership once the request leaves the library-docs problem space.

## Handoff Rules

- **Named library/framework/tool docs**: stay in this skill and use Context7.
- **Generic webpage/article/docs-site content capture**: hand off to `firecrawl-cli`.
- **Platform/source selection or multi-source web research**: hand off to `agent-reach`.
- **Context7 installation, skill management, or MCP setup**: hand off to `context7-cli`.
...
- Confirm the user is asking for named library or CLI docs, not generic web research.
- Resolve the library ID before answering from docs.
- Stop after the best documented match if the search stays ambiguous.

## Output Contract

- Return the selected library ID, then the answer from docs.
- State when the docs search was ambiguous or quota-limited.
- Prefer current documentation over training memory.
