...
Owns: argparse registration and dispatch for auditing or recovering exact tasks
that already have durable owned-result evidence.
...
def register_historical_recovery_parsers(subparsers: Any) -> None:
    ...
    audit_owned = subparsers.add_parser(
        "audit-owned-result-recovery",
        help="Read-only exact-task audit against durable Codex session owned-result evidence",
    )
    audit_owned.add_argument("--task-id", required=True, help="Audit exactly this task id")

    recover_owned = subparsers.add_parser(
        "recover-owned-result",
        help="Recover exactly one task from durable Codex session owned-result evidence",
    )
    recover_owned.add_argument("--task-id", required=True, help="Recover exactly this task id")
    recover_owned.add_argument("--apply", action="store_true", help="Apply the one-task recovery; default is dry-run")
    recover_owned.add_argument("--confirm", default="", help="Required for apply: CORRECT-OWNED-RESULT")
    recover_owned.add_argument("--expected-sha256", default="", help="Required for apply: candidate SHA-256 returned by audit")
...
def run_historical_recovery_command(
    ...
    *,
    ...
    audit_owned_result_recovery: Callable[..., dict[str, Any]],
    recover_owned_result: Callable[..., dict[str, Any]],
    ...
    if args.cmd == "audit-owned-result-recovery":
        result = audit_owned_result_recovery(queue, str(args.task_id or ""))
        return result, 0 if result.get("ok") else 1

    if args.cmd == "recover-owned-result":
        result = recover_owned_result(
            queue,
            config,
            str(args.task_id or ""),
            apply=bool(args.apply),
