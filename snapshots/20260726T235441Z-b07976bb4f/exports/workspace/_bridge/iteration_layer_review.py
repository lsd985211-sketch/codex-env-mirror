...
ACTIONABLE_FINDING_KINDS = {"skill-entrypoint-gap"}
...
def validation_for_route(route: str) -> str:
    ...
    if "tool registry" in normalized:
        return "Run python _bridge/developer_toolchain_owner.py validate."
    if "cli" in normalized:
...
def actionable_findings(findings: list[Finding]) -> list[Finding]:
    """Return explicit gaps only; recency and existing evidence are observations."""
    return [finding for finding in findings if finding.kind in ACTIONABLE_FINDING_KINDS]
...
def build_decision_summary(
    ...
    if not batches and not promotion_map:
        summary_text = "No actionable promotion candidates; retain observations as evidence without requesting approval."
    else:
        summary_text = (
    ...
    if "tool-registry-review" in group_names:
        actions.append(
            RecommendedAction(
                ...
                validation="Run python _bridge/developer_toolchain_owner.py validate.",
            )
        )
...
def build_approval_block(
    ...
    *,
    ...
    return {
        "status": "awaiting_user_approval" if has_proposals else "no_actionable_proposals",
        "apply_mode": "proposal_only",
        "user_decision_required": has_proposals,
        ...
        "approval_prompt": (
            ...
            if has_proposals
            else "No approval is requested because no explicit actionable gap was found."
        ),
...
def validation_plan(profile: str = "quick") -> list[ValidationStep]:
    quick_steps = [
        ...
        ValidationStep(
            name="developer-toolchain-validate",
            command=[sys.executable, str(ROOT / "_bridge" / "developer_toolchain_owner.py"), "validate"],
            purpose="Validate the authoritative WSL developer toolchain without opening the Windows-owned mobile queue.",
        ),
        ValidationStep(
...
def build_report(limit: int, *, run_validation: bool, validation_profile: str, validation_timeout: int) -> dict[str, object]:
    ...
    proposal_findings = actionable_findings(findings)
    proposals = build_proposals(proposal_findings)
    steps = validation_plan(validation_profile)
    knowledge = extract_iteration_knowledge([asdict(item) for item in proposal_findings])
    ...
    groups = build_aligned_proposal_groups(
        build_proposal_groups(proposal_findings, knowledge_suggestions=knowledge["knowledge_promotion_suggestions"]),
        knowledge_promotion_batches=knowledge_promotion_batches,
        promotion_readiness_summary=promotion_readiness_summary,
    )
    ...
    return {
        ...
        "actionable_finding_count": len(proposal_findings),
        "observation_count": len(findings) - len(proposal_findings),
        "actionable_findings": [asdict(item) for item in proposal_findings],
