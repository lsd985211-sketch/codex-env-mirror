...
- Do not use Git commands as a default proof step for local file edits. Many
  workspace targets live outside the repo, are untracked, or are generated
  artifacts; `git diff/status` can waste time without proving the change.
  Prefer targeted reads, `Select-String`/`rg`, hashes, validators, and backup
  comparison. Use Git only when the question actually requires version-control
  state, commits, tracked-file diffs, branches, or repository history.
