...
def run_codex_mcp_list(config: dict | None = None, baseline: dict | None = None) -> tuple[bool, str]:
    ...
    command: list[str]
    cwd = ROOT
    if os.name != "nt" and isinstance(baseline, dict) and baseline.get("workspace_role") == "windows_host_compatibility_projection":
        python_candidates = [
...
def build_checks(run_cli: bool = True) -> list[Check]:
    ...
    project_config_path = host_path(baseline["project_config"])
