...
def reconcile(
    *,
    ...
    startup_baseline = (
        codex_state_repair.repair(dry_run=dry_run, runtime_validation=False)
...
def validate() -> dict[str, Any]:
    ...
    checks = [
        ...
        {"name": "scheduled_task_owns_watcher", "ok": "codex_model_provider_watcher.py" in action_text and " supervise" in action_text},
    ...
    try:
        ...
        while True:
            ...
            if projection_available and projection_signature != last_projection_signature and now >= next_projection_retry_at:
                projection_result = codex_config_projection.apply_projection(additions_only=True, sync_desktop=True)
                startup_baseline_result = (
                    codex_state_repair.repair(dry_run=False, runtime_validation=False)
                    if projection_result.get("ok")
                    else {"ok": False, "skipped": True, "reason": "config_projection_failed"}
                )
                if projection_result.get("ok") and startup_baseline_result.get("ok"):
