...
from mcp_session_profile_drift import current_turn_callability_disposition, profile_registration_issues  # noqa: E402
...
CODEX_STATE_DB = Path.home() / ".codex" / "sqlite" / "state_5.sqlite"
...
def codex_state_db_candidates() -> list[Path]:
    configured_home = Path(os.environ.get("CODEX_HOME", "")).expanduser() if os.environ.get("CODEX_HOME") else None
    values = [
        Path(os.environ["CODEX_STATE_DB"]).expanduser() if os.environ.get("CODEX_STATE_DB") else None,
        CODEX_STATE_DB,
        configured_home / "sqlite" / "state_5.sqlite" if configured_home else None,
        configured_home / "state_5.sqlite" if configured_home else None,
        Path.home() / ".codex" / "state_5.sqlite",
        Path.home() / ".codex-app" / "sqlite" / "state_5.sqlite",
        Path.home() / ".codex-app" / "state_5.sqlite",
    ]
    return list(dict.fromkeys(path for path in values if path is not None))


def _state_db_thread_row(path: Path, requested_thread_id: str) -> tuple[dict[str, Any] | None, str]:
    ...
    try:
        ...
        return ({key: row[key] for key in row.keys()} if row is not None else None), lookup_mode
    finally:
        con.close()


def _thread_row_anchor_ms(row: dict[str, Any]) -> int:
    values: list[int] = []
...
def thread_freshness_anchor(
    thread_id: str | None = None,
    *,
    state_db_candidates: list[Path] | None = None,
) -> dict[str, Any]:
    requested_thread_id = str(thread_id or "").strip()
    candidate_paths = codex_state_db_candidates() if state_db_candidates is None else state_db_candidates
    attempts: list[dict[str, Any]] = []
    available: list[tuple[int, Path, dict[str, Any], str]] = []
    existing_paths: list[Path] = []
    for path in candidate_paths:
        path = Path(path).expanduser()
        if not path.exists():
            attempts.append({"path": str(path), "state": "missing"})
            continue
        existing_paths.append(path)
        try:
            row, lookup_mode = _state_db_thread_row(path, requested_thread_id)
        except Exception as exc:
            attempts.append({"path": str(path), "state": "error", "error": repr(exc)})
            continue
        if row is None:
            attempts.append({"path": str(path), "state": "thread_not_found"})
            continue
        anchor_ms = _thread_row_anchor_ms(row)
        attempts.append({"path": str(path), "state": "candidate", "thread_id": str(row.get("id") or ""), "anchor_ms": anchor_ms})
        available.append((anchor_ms, path, row, lookup_mode))

    if not available and not existing_paths:
        return {
            "state": "state_db_missing",
            "path": str(candidate_paths[0] if candidate_paths else CODEX_STATE_DB),
            "requested_thread_id": requested_thread_id,
            "selected_thread_id": "",
            "anchor_field": "",
            "anchor_at": "",
            "note": "No thread freshness anchor is available because the Codex state database is missing.",
            "state_db_candidates": attempts,
        }
    if not available:
        error_attempts = [item for item in attempts if item.get("state") == "error"]
        return {
            "state": "state_db_error" if error_attempts else "thread_not_found",
            "path": str(existing_paths[0]),
            "requested_thread_id": requested_thread_id,
            "selected_thread_id": "",
            "anchor_field": "",
            "anchor_at": "",
            "lookup_mode": "requested_thread" if requested_thread_id else "latest_thread",
            "state_db_candidates": attempts,
        }

    _, selected_path, row, lookup_mode = max(available, key=lambda item: item[0])
    candidates: list[tuple[str, datetime]] = []
    for field in ("updated_at_ms", "recency_at_ms", "created_at_ms"):
        anchor = _ms_to_datetime(row.get(field))
    ...
    if not candidates:
        return {
            ...
            "requested_thread_id": requested_thread_id,
            "selected_thread_id": str(row.get("id") or ""),
            "anchor_field": "",
            "anchor_at": "",
...
def recovery_strategy_for_topology(profile: McpProfile) -> dict[str, Any]:
    ...
    if profile.protected:
        ...
        return base
    if topology == "local_session_bound_stdio_kernel":
        base.update(
            {
                "strategy": "new_session_real_call_acceptance",
                "max_auto_recover_attempts": 0,
                "recover_steps": [
                    "record current-turn negative evidence",
                    "do not retry or rebind the closed task-local handle",
                    "open a new Desktop task after the runtime is available",
                    "complete a real tool call before recording positive evidence",
                ],
            }
        )
        return base
...
MCP_PROFILES: tuple[McpProfile, ...] = (
    ...
    McpProfile(
        ...
        "local_session_bound_stdio_kernel",
    ),
    McpProfile(
        "context7",
        "context7",
...
def _validate_snapshot_profiles(
    ...
    session_bound_acceptance_pending: list[dict[str, str]],
) -> None:
    ...
    for profile in payload.get("profiles", []):
        ...
        if profile.get("session_surface_missing_or_stale") or current_turn.get("callable") is False:
            ...
            disposition = current_turn_callability_disposition(
                ...
                transport_topology=str(profile.get("transport_topology") or "external_stdio"),
                current_turn_state=str(current_turn.get("state") or "session_surface_missing_or_stale"),
                fallback_available=bool(profile.get("fallback_tool_call_ok")),
                gateway_available=isinstance(gateway_profile, dict) and gateway_profile.get("last_gateway_ok") is True,
            )
            if disposition["severity"] == "risk":
                issues.append(disposition["message"])
                continue
            advisories.append(disposition["message"])
            if disposition["code"] == "session_bound_acceptance_pending":
                session_bound_acceptance_pending.append(
                    {
                        "profile": str(profile.get("name") or "unknown"),
                        "state": str(current_turn.get("state") or "session_surface_missing_or_stale"),
                        "transport_topology": str(profile.get("transport_topology") or "external_stdio"),
...
def validate(snap: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    _validate_snapshot_profiles(
        payload,
        gateway_profiles,
        issues,
        advisories,
        unproven_native_callability,
        session_bound_acceptance_pending,
    )
    ...
    advisory_summary = {
        ...
        "session_bound_acceptance_pending_count": len(session_bound_acceptance_pending),
        "session_bound_acceptance_pending_profiles": session_bound_acceptance_pending,
        "actionable_advisory_count": len(advisories),
