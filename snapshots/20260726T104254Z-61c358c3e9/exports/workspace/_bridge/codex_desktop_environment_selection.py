...
def reconcile_environment_selection(
    *,
    ...
    backup_creator: Callable[..., dict[str, object]] = create_backup,
    writer: Callable[[Path, str], None] = atomic_write_text,
    state_lease: state_write_authority.StateWriteLease | None = None,
) -> dict[str, object]:
    if not write:
    ...
    owned_lease = state_lease is None
    if state_lease is None:
        state_lease = state_write_authority.try_acquire_state_write_lease(
            "codex_config",
            "codex_desktop_environment_selection",
            state_root=host_config.parent,
            timeout_seconds=LOCK_TIMEOUT_SECONDS,
        )
    if state_lease is None:
        return {"ok": False, "status": "state_write_authority_busy", "changed": False}
