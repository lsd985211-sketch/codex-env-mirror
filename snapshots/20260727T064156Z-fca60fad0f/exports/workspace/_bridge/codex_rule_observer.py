...
def active_codex_home() -> Path:
    return Path(
        ...
        or os.environ.get("WSL_CODEX_HOME")
        or (Path.home() / ".codex-app")
    ).expanduser().resolve()
...
def read_recent_events(
    *,
    ...
    limit: int = MAX_EVENT_FILES,
) -> list[dict[str, Any]]:
    """Read a bounded cross-session event window for governed derived analysis."""

    root = _event_root(runtime_root)
    cutoff = datetime.now(timezone.utc) - timedelta(hours=max(1, int(max_age_hours)))
    rows: list[dict[str, Any]] = []
    try:
        files = sorted(root.glob("**/*.json"), key=lambda item: item.stat().st_mtime, reverse=True)[: max(1, int(limit))]
    except OSError:
        return []
    for path in files:
        try:
            ...
            if recorded.tzinfo is None:
                recorded = recorded.replace(tzinfo=timezone.utc)
            if recorded >= cutoff and item.get("schema") == SCHEMA:
                rows.append(item)
        except (OSError, ValueError, TypeError, json.JSONDecodeError):
            continue
    rows.sort(key=lambda item: str(item.get("recorded_at") or ""))
...
def admission_output(payload: dict[str, Any], *, runtime_root: Path | None = None, persist: bool = True) -> dict[str, Any]:
    ...
    if persist:
        try:
            write_event(event, runtime_root)
        except Exception:
            pass
...
def validate(*, hooks_path: Path | None = None) -> dict[str, Any]:
    checks: list[dict[str, Any]] = []
    implicit = admission_output(
        {"session_id": "validate", "turn_id": "1", "prompt": "推荐当前可用的 USB 诊断工具"},
        persist=False,
    )
    ...
    selected_hooks_path = hooks_path or (active_codex_home() / "hooks.json")
    configured: dict[str, Any] = {}
    try:
        configured = json.loads(selected_hooks_path.read_text(encoding="utf-8"))
    ...
    checks.append(
        {
            ...
            "path": str(selected_hooks_path),
        }
    )
    serialized_hooks = json.dumps(configured, ensure_ascii=False) if isinstance(configured, dict) else ""
    checks.append(
        {
            "name": "configured_hooks_target_current_observer",
            "ok": all(event in serialized_hooks for event in ALLOWED_EVENTS)
            and "codex_rule_observer.py" in serialized_hooks,
            "detail": str(selected_hooks_path),
        }
    )
