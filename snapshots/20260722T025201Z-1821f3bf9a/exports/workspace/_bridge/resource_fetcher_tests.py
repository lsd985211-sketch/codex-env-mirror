...
def run_resource_fetcher_tests() -> dict[str, str]:
    ...
    with tempfile.TemporaryDirectory(prefix="resource-fetcher-tests-") as tmp:
        ...
        if os.name == "nt":
            ...
            assert_ok(
                search_health.get("platform_deferred")
                and search_health.get("execution_owner") == "windows_host_compatibility_projection",
                f"WSL generic search should defer to the Windows Hub owner: {search_health}",
            )
        ...
        if os.name == "nt":
            ...
            assert_ok(
                unicode_worker_result.get("platform_deferred")
                and unicode_worker_result.get("execution_owner") == "windows_host_compatibility_projection",
                f"WSL resource search worker should preserve the host boundary: {unicode_worker_result}",
            )
