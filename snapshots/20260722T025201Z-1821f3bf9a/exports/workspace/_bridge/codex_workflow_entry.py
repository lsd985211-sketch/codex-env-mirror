...
from workflow_closeout_signals import apply_post_closeout_mirror, concurrent_closeout_handoff, optional_closeout_sections
...
def main(argv: list[str] | None = None) -> int:
    ...
    mirror_submit.add_argument("--task-id", required=True)
    ...
    if args.command == "plan":
        ...
        if args.mirror_action == "submit":
            ...
            payload = start_long_command(args.task_id, command, timeout_seconds=args.timeout_seconds, cwd=str(ROOT))
            payload.update({
                "schema": "codex_workflow_entry.mirror_submit.v1",
                "mirror_operation": args.operation,
                "status_command": f"python _bridge/shared/long_command_receipt.py status --task-id {args.task_id} --wait-seconds 30",
            })
        else:
