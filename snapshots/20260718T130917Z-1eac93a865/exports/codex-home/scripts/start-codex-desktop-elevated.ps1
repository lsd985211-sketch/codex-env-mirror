...
function Invoke-CodexConfigPreflightSync {
    ...
    try {
        ...
        Write-StartLog "Codex config preflight completed. Ok=$([bool]$result.ok); Applied=$applied; NeedsRestart=$needsRestart; BeforeOk=$beforeOk; AfterOk=$afterOk; WslEnabled=$wslEnabled; WslRuntimeReady=$wslRuntimeReady; WslProjectionStatus=$wslProjectionStatus"
        $preflightResult = @{
            Ok = [bool]$result.ok
            Applied = $applied
