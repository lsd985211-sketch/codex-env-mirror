...
def run_once(apply: bool, startup_delay_seconds: int = 0, phase: str = "manual") -> dict[str, Any]:
    ...
    if phase == "pre-start-static":
        ...
        projection_ok = bool(wsl_runtime_projection.get("ok", True))
        projection_ready = bool(wsl_runtime_projection.get("ready", projection_ok))
        projection_changed = bool(wsl_runtime_projection.get("changed"))
        result = {
            ...
            "policy": "static preflight performs merge-only baseline repair and always applies the WSL projection when Desktop WSL mode is enabled; session compaction remains delegated to the stopped-process pre-launch owner",
        }
