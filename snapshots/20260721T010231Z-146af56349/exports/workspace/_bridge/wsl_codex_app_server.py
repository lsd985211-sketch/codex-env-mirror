...
def install(confirm: str, *, workspace: Path = DEFAULT_WORKSPACE, codex_home: Path = DEFAULT_CODEX_HOME) -> dict[str, Any]:
    ...
    backup = create_backup([str(path)], category="wsl-workspace", purpose="before-codex-app-server-user-unit", remark="codex-app-server-user-unit", trigger="wsl_codex_app_server.install") if path.exists() else {"ok": True, "skipped": "unit_absent"}
    if not backup.get("ok"):
        return {"schema": f"{SCHEMA}.install", "ok": False, "status": "blocked", "reason": "backup_failed", "backup": backup}
    ...
    enable_result = systemctl("enable", "--now", SERVICE_NAME, timeout=60)
    result = validate(workspace=workspace, codex_home=codex_home)
    return {"schema": f"{SCHEMA}.install", "ok": bool(reload_result.get("ok") and enable_result.get("ok") and result.get("ok")), "status": "completed" if result.get("ok") else "failed", "generated_at": now_iso(), "backup": backup, "reload": reload_result, "enable_start": enable_result, "validation": result}
