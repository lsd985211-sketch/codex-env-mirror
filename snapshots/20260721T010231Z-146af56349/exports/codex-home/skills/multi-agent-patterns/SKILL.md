...
description: Multi-agent system design patterns for context isolation, supervisors, handoffs, agent coordination, repository work-start coordination, and swarm-style workflows. Use for multi-agent architecture, Codex tasks working concurrently in one Git repository, or collaboration and handoff design.
---
...
This skill owns coordination patterns and repository work-start handoff
requirements. It does not create Codex tasks, grant repository permissions,
edit files, choose Git integration policy, or own commit, publication, and
closeout state. Those actions remain with Codex App thread tools, workspace
rules, and the declared repository or workflow owner.

## Repository Work-Start Coordination

Before editing or mutating repository-owned state:

1. Resolve the canonical Git repository identity, including linked worktrees.
2. Use Codex App thread state to list other tasks and keep only tasks that are currently active in that repository. Exclude the current task and idle, not-loaded, completed, archived, or unrelated tasks.
3. Send every matching task one bounded message containing the goal, declared paths or owner surfaces, expected state writes, branch/worktree, and a request to report overlap or ownership constraints.
4. Treat the send receipt as delivery evidence only. Resolve explicit or plausible overlap before editing; proceed without an indefinite wait when scopes are clearly disjoint.
5. Prefer an isolated worktree and declared-path commit gate when work is parallel. Re-check active tasks before integration and use the existing closeout handoff when another task still owns final publication.

If task discovery is unavailable, do not mutate a shared worktree. Proceed only in an isolated declared-path worktree when Git evidence shows no known overlap, and record the missing thread evidence.

Return the repository identity, active task IDs, notice receipts, declared scope, overlap decision, and any blocked paths. Do not create a second task registry or infer task activity from stale branches, worktrees, or process lists.
