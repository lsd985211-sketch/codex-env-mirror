...
from workflow_review_queue import prepare_delivery_envelope, prepare_delivery_packages, sync_review_groups
...
from shared.long_command_receipt import follow_command as follow_long_command
from shared.long_command_receipt import start_command as start_long_command
from shared.long_command_receipt import submit_or_reuse as submit_or_reuse_long_command
...
def concurrent_handoff_summary(handoff: dict[str, Any]) -> dict[str, Any]:
    ...
    keys = (
        ...
        "target_thread", "receipt_targets", "message_required", "mirror_delegated",
    ...
    return {key: handoff.get(key) for key in keys if handoff.get(key) not in (None, "", [], {})}
...
def closeout(
    *,
    ...
    delivery_packages = prepare_delivery_packages()
    ...
    if delivery_packages.get("ok") and delivery_packages.get("packages"):
        first_package = delivery_packages["packages"][0]
        envelope = prepare_delivery_envelope(str(first_package.get("package_id") or ""))
        ...
        package["final_reply_must_show"]["delivery_required"] = True
        package["final_reply_must_show"]["delivery_complete"] = False
        package["final_reply_must_show"]["delivery_next_action"] = (
            "render the current package cards in the final reply; response persistence owner then marks the envelope delivered"
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    c.add_argument("--active-workspace-thread", action="append", default=[], help="Active Codex task ID in this Work Git repository from Codex App thread state; repeat as needed.")
    ...
    mirror_submit.add_argument("--operation", choices=("refresh", "publish", "release"), required=True)
    ...
    mirror_submit.add_argument("--follow-seconds", type=float, default=900.0)
    ...
    if args.command == "plan":
        ...
        if args.mirror_action == "submit":
            ...
            task_id = str(submission.get("task_id") or args.task_id)
            if submission.get("submit_ok") and task_id and args.follow_seconds > 0:
                payload = follow_long_command(task_id, wait_seconds=args.follow_seconds)
                payload["submission"] = {
                    key: submission.get(key)
                    for key in ("submit_ok", "reused", "reuse_state")
                    if key in submission
                }
            else:
                payload = submission
            payload.update({
                "schema": "codex_workflow_entry.mirror_submit.v2",
                "mirror_operation": args.operation,
                "follow_command": f"python _bridge/shared/long_command_receipt.py follow --task-id {task_id} --wait-seconds {max(30, int(args.follow_seconds))}",
            })
        else:
