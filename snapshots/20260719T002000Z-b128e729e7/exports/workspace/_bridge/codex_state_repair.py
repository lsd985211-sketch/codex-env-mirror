...
def load_top_level_thread_ids(path: Path) -> dict:
    """Read current top-level task identities without changing the source DB."""
    if not path.is_file():
    ...
    try:
        ...
        rows = connection.execute(
            f"SELECT id, source, rollout_path FROM threads "
    ...
    for thread_id, source, rollout_path in rows:
...
def ensure_wsl_resume_context_projection(
    *,
    ...
    projection = project_wsl_resume_state(state, list(thread_index.get("thread_ids") or []))
    ...
    backup_dir = backup_files(
        [global_state_path],
        labels=["global_state"],
        changed=["global_state_wsl_resume_context_and_task_visibility_projection"],
    )
...
def repair(dry_run: bool, *, runtime_validation: bool = True) -> dict:
    ...
    desktop_wsl_enabled = "runCodexInWindowsSubsystemForLinux = true" in global_text
    ...
    return {
        ...
        "needs_codex_restart": bool(changed or written or wsl_runtime_projection.get("changed")),
