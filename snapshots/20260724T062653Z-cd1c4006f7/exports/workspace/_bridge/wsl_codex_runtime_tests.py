...
def smoke(name: str, command: list[str], *, tool_call: dict[str, object] | None = None) -> dict[str, object]:
    ...
    try:
        ...
        if tool_call is not None:
            proc.stdin.write(json.dumps({"jsonrpc": "2.0", "id": 3, "method": "tools/call", "params": tool_call}) + "\n")
            proc.stdin.flush()
            called = read_json_line(proc.stdout)
        return {
            "name": name,
            "ok": (
                initialized.get("id") == 1
                and "result" in initialized
                and tools.get("id") == 2
                and "result" in tools
                and (called is None or (called.get("id") == 3 and "result" in called))
            ),
            ...
            "tool_called": called is not None and called.get("id") == 3 and "result" in called,
            "tool_call_error": (called or {}).get("error"),
        }
    finally:
        proc.kill()
        proc.wait(timeout=5)
...
def main() -> int:
    ...
    results.append(smoke(
        "node_repl",
        [str(ROOT / "workspace" / "_bridge" / "codex_node_repl_wsl.sh")],
        tool_call={
            "name": "js",
            "arguments": {"code": "nodeRepl.write({ok:true})", "title": "Validate WSL sandbox projection"},
            "_meta": {
