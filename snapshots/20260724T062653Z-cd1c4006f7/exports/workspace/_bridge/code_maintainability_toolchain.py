...
from developer_toolchain_owner import desired_executable_contracts
...
DEV_TOOLCHAIN = {
    name: {
        ...
        "use": str(contract.get("use") or ""),
        "expected_version_prefix": str(contract.get("version_prefix") or ""),
    }
    for name, contract in desired_executable_contracts().items()
}
...
def search_hygiene_snapshot() -> dict[str, Any]:
    ...
    explicit_target = "_bridge/runtime"
...
def developer_toolchain_snapshot() -> dict[str, Any]:
    ...
    for name, spec in DEV_TOOLCHAIN.items():
        ...
        item["use"] = spec["use"]
        item["expected_version_prefix"] = spec["expected_version_prefix"]
        item["version_match"] = bool(
            item.get("ok")
            and str(item.get("version") or "").startswith(str(spec["expected_version_prefix"]))
        )
        item["ok"] = bool(item.get("ok") and item["version_match"])
        tools.append(item)
    ...
    return {
        ...
        "usage_policy": {
            ...
            "rg_runtime_opt_in": "when runtime/cache/browser-profile evidence is needed, use an explicit target plus -u/--no-ignore, for example rg -u --files _bridge/runtime",
            ...
            "gh": "use the native WSL GitHub CLI; its wrapper borrows Windows gh authentication in memory without persisting a second token",
            "jq": "use for bounded structured JSON receipt filtering; do not replace schema-aware parsing with fragile text slicing",
        },
    }
