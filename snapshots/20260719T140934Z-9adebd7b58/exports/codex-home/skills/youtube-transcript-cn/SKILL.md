...
### 基本用法
```bash
python scripts/get_transcript.py "https://www.youtube.com/watch?v=dQw4w9WgXcQ"
```

### 带时间戳输出
```bash
python scripts/get_transcript.py -t "https://youtu.be/VIDEO_ID"
```

### 保存到文件
```bash
python scripts/get_transcript.py -o output.md "https://www.youtube.com/watch?v=VIDEO_ID"
```

### JSON 格式（便于后处理）
```bash
python scripts/get_transcript.py -f json "VIDEO_ID"
```
