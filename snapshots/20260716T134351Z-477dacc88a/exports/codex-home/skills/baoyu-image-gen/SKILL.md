...
- Confirm provider, aspect ratio, and whether reference images are involved.
- Prefer sequential generation unless the user explicitly wants parallel runs.
- Check API key and base URL before blaming the prompt.

## Output Contract

- Return the image path and the exact provider/model used.
- Report retries and partial failures honestly.
- Keep provider selection explicit when multiple options exist.
