...
from resource_source_strategy import candidate_source_plan, validate as validate_source_strategy
...
def run_resource_fetcher_tests() -> dict[str, str]:
    ...
    with tempfile.TemporaryDirectory(prefix="resource-fetcher-tests-") as tmp:
        ...
        assert_ok(docs_route.read_only and docs_route.primary_tool == "context7", "docs route should prefer context7")
        ...
        assert_ok(route_payload["primary_tool"] == "context7", "resource_cli route docs primary tool mismatch")
        ...
        assert_ok(openai_docs_plan["candidates"][0]["owner_tool"] == "generic_search", "OpenAI official docs should use official-domain search")
        assert_ok(
        ...
        assert_ok(
            openai_docs_plan["execution_capability"]["registered_owner_adapter"] == "generic_search",
            "OpenAI docs should expose the generic search owner adapter",
        )
        assert_ok(
            openai_docs_plan["candidates"][0]["owner_tool"] not in {"microsoftdocs", "context7"},
            "incidental defect text must not route OpenAI docs to Microsoft Docs or Context7",
        )
        ...
        assert_ok(docs_source_result["available_owner_adapter"] == "generic_search", "documentation source selection should name the next owner")
        ...
        docs_probe = next(item for item in broker_docs.attempts if item["tool"] == "resource_cli")
        docs_network = docs_probe["result"].get("metadata", {}).get("network", {})
        assert_ok(docs_network.get("gateway_target_kind") == "docs", "docs broker probe should preserve gateway target kind")
        ...
        assert_ok(
            openai_broker_tools[:2] == ["resource_source_strategy", "generic_search"],
            f"official documentation owner should immediately follow source strategy: {openai_broker_tools}",
        )
        assert_ok(
            next(item for item in openai_broker_plan if item["tool"] == "generic_search")["executable_by_broker"],
            "official documentation search should be executable by the broker",
        )
        ...
        try:
            def fake_generic_search_hub_call(tool: str, arguments: dict[str, object], *, timeout: int) -> dict[str, object]:
                captured_search.update({"tool": tool, "arguments": arguments, "timeout": timeout})
                return {
                    "ok": True,
                    "results": [{"title": "OpenAI Codex", "href": "https://developers.openai.com/codex/", "body": "Official Codex documentation"}],
                    "backend": "test",
                }
        ...
        assert_ok(
            (captured_search.get("arguments") or {}).get("site_or_domain") == "openai.com",
            f"official documentation domain constraint was not forwarded: {captured_search}",
        )
        ...
        try:
            resource_broker.execute_source_selection = lambda *_args, **_kwargs: {
                ...
                "available_owner_adapter": "generic_search",
            ...
            resource_broker.execute_owner_tool = original_execute_owner_tool_for_search
        assert_ok(
            [item["tool"] for item in openai_docs_receipt.attempts[:2]] == ["resource_source_strategy", "generic_search"],
