...
def module_gate_needed(message: str, selected_domain_keys: list[str]) -> bool:
    ...
    if not any(
        ...
        for key in ("workflow_governance", "mcp_tools", "network_routing", "bridge", "memory", "email", "cli_harness", "resource_acquisition")
    ):
        return False
...
def infer_profile(message: str, selected_domain_keys: list[str], *, enable_module_gate: bool, risk: str) -> dict[str, Any]:
    ...
    if task_contract.profile_override:
        ...
        profile = "maintenance_governance"
        reasons.append("workflow_governance_domain")
    elif any(key in selected_domain_keys for key in ("mcp_tools", "network_routing", "records_resources", "resource_acquisition", "memory", "email", "structured_state")):
    ...
    return {
        ...
        "maintenance_upgrade_policy": {
            "enabled": "code_maintainability" in domain_keys or any(
                key in domain_keys for key in ("workflow_governance", "mcp_tools", "resource_acquisition", "network_routing")
            ),
