...
def run_once(apply: bool, startup_delay_seconds: int = 0, phase: str = "manual") -> dict[str, Any]:
    ...
    if phase == "pre-start-static":
        ...
        result = {
            ...
            "policy": "static preflight first reconciles the Desktop environment selection, then performs mode-specific merge-only baseline repair and WSL projections; selection-owner failure falls back to the current host value so Codex startup remains available",
        }
