...
5. Call known Hub tools directly. Use `owner_mcp.call_readonly` for explicit
   allowlisted Context7, Microsoft Docs, filesystem-read, and MarkItDown calls.
...
- Code structure: `codegraph` first for indexed source flow. For broad text,
  Markdown, config, generated assets, and rule lookups, use `rg` with standard
  exclusions first.
...
- External docs / online research: submit a resource-layer request first. The
  ...
  and should choose `microsoftdocs` for Microsoft/Windows/Azure, `context7` for
...
- Tool-utilization audit is the route-evidence layer. For workflow, resource,
  state, memory, browser, MCP, or maintainability work, use
  `_bridge/tool_utilization_audit.py audit|validate` to detect when a
  high-value owner tool is missing from the route plan. It must not become a
  fixed checklist; it only flags tools whose task semantics require them.
...
Heavy session-bound profiles use `_bridge/mcp_lazy_stdio_proxy.py`: Desktop
...
2. Hub-direct stage: known same-boundary Hub aliases, including
   ...
   `owner_mcp.call_readonly` for Context7, Microsoft Docs, filesystem reads,
   and MarkItDown.
...
- Direct Hub map: GitHub -> `github.api` / `github.gh`; resource layer ->
  ...
  Context7 and Microsoft Docs -> known `mcp_gateway.call` with profile
  `context7` or `microsoftdocs` and exact target tool/arguments. Do not use
...
- Topology: project CLI broker plus Hub-native wrapper. It classifies through
  ...
  (`github`, `package_manager`, `microsoftdocs`, `context7`, `markitdown`,
...
- Optimize calls: GitHub is Hub-first. Call Hub `github.api` or `github.gh` for
  ...
  closed. Hub `github.gh` uses the existing `gh` keyring and still blocks
  token-printing commands.
...
- Trigger them proactively when the user explicitly asks to search online, look
  up external knowledge, or consult docs. `microsoftdocs` owns
