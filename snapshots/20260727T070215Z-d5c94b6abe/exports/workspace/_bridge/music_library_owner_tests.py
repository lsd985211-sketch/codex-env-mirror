...
class MusicLibraryOwnerTests(unittest.TestCase):
    def test_ffprobe_resolution_uses_platform_windows_user_root_from_wsl(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            host_user = Path(temp)
            ffprobe = host_user / "AppData" / "Local" / "Microsoft" / "WinGet" / "Links" / "ffprobe.exe"
            write_file(ffprobe, b"fixture")
            with patch.dict(os.environ, {"CODEX_FFPROBE": ""}, clear=False), patch.object(
                planner.shutil, "which", return_value=None
            ), patch.object(planner, "windows_user_root", return_value=host_user):
                resolved = planner.find_ffprobe()

        self.assertEqual(resolved, str(ffprobe))
