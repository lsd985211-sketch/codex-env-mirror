...
DOMAINS: tuple[Domain, ...] = (
    ...
    Domain(
        ...
        (),
        (),
        ("_bridge/usb_device_owner.py", "_bridge/usb_device_control.py", "_bridge/docs/maintenance_surface_map.md"),
        ("usb_device_owner.py doctor", "usb_device_owner.py validate", "usb_device_control.py validate"),
    ),
    Domain(
        "mcp_tools",
        "MCP / tool stability",
...
def module_gate_needed(message: str, selected_domain_keys: list[str]) -> bool:
    ...
    if not any(
        ...
        for key in ("workflow_governance", "mcp_tools", "network_routing", "bridge", "hardware", "memory", "email", "cli_harness", "resource_acquisition")
    ):
        return False
...
def infer_profile(message: str, selected_domain_keys: list[str], *, enable_module_gate: bool, risk: str) -> dict[str, Any]:
    ...
    if task_contract.profile_override:
        ...
        profile = "maintenance_governance"
        reasons.append("workflow_governance_domain")
    elif any(key in selected_domain_keys for key in ("mcp_tools", "network_routing", "records_resources", "resource_acquisition", "memory", "email", "structured_state", "hardware")):
...
def workflow_tools_contract(
    *,
    ...
    domain_keys = _selected_domain_keys(selected_domains)
    ...
    return {
        ...
        "maintenance_upgrade_policy": {
            "enabled": "code_maintainability" in domain_keys or any(
                key in domain_keys for key in ("workflow_governance", "mcp_tools", "resource_acquisition", "network_routing", "hardware")
            ),
        ...
        "tool_utilization_policy": {
            "enabled": any(key in domain_keys for key in tool_utilization_domains),
            "query_rule": "For system governance, resource, state, memory, browser, MCP, or maintainability work, run a read-only utilization audit so high-value owner tools become visible route evidence instead of optional habits.",
            "route_terms": ["tool_utilization_audit", "codegraph", "sqlite_state", "resource_acquisition", "network_routing", "memory_router"],
            "validation": "python _bridge\\tool_utilization_audit.py validate",
            "non_goal": "do not force a fixed tool checklist; audit only checks tools whose task semantics require them",
        },
    }
...
def build_plan(
    ...
    *,
    ...
    plan["structured_route"]["route_decision"] = plan["execution_route_pack"].get("route_decision", {})
    plan["structured_route"]["decision_owner"] = "execution_route_pack"
    plan["structured_route"]["downstream_rule"] = (
        "consume_structured_route.route_decision; do_not_reclassify_raw_message_without_new_evidence"
    )
...
def validate() -> dict[str, Any]:
    ...
    retirement_probe_member = retirement_members[0] if retirement_members else "retired-member"
    retirement_plan = validation_plan(f"清除 {retirement_probe_member} 退役成员残留", detail="full")
    retirement_guard = retirement_plan.get("retirement_guard", {})
    checks.append(
        {
            "name": "retirement_guard_reaches_workflow_plan",
            "ok": bool(retirement_guard.get("triggered")) and retirement_probe_member in retirement_guard.get("do_not_route", []),
            "detail": {
    ...
    ordinary_environment = ordinary_plan.get("environment_context", {})
    ordinary_systems = {
        item.get("system")
    ...
    standard_environment = standard_environment_plan.get("environment_context", {})
    standard_members = [
        {
