...
def release_plan() -> dict[str, Any]:
    ...
    non_snapshot = sorted(path for path in paths if not path.startswith("snapshots/"))
    if not non_snapshot:
        bump = "none"
    ...
    return {
        ...
        "snapshot_change_count": len(paths) - len(non_snapshot),
...
def main(argv: list[str] | None = None) -> int:
    ...
    for name in ("status", "plan", "doctor", "validate", "release-plan"):
        sub.add_parser(name)
