...
Status: first bounded durable-recovery wake integration implemented; readiness
and service-objective coverage remains incremental.
...
- The unified scheduler may invoke the persistent task kernel's guarded
  expired-lease recovery only. It cannot claim or execute tasks, and interrupted
  execution remains `recovery_required` until acceptance evidence is reviewed.
