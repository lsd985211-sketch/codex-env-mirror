...
class UsbDeviceControlTests(unittest.TestCase):
    ...
    def test_core_usb_and_input_devices_are_always_blocked(self) -> None:
        rows = [
            ...
            camera(child_topology_known=False),
    ...
    def test_identity_change_does_not_trigger_enable_recovery(self) -> None:
        before = camera()
        replaced_disabled = camera(driver_version="9.9.9", status="Error", problem_code=22)
        plan = control.build_plan("restart", before["instance_id"], query=lambda _: before, admin=True)
        queries = iter([before, before, replaced_disabled])
        calls: list[list[str]] = []
        with tempfile.TemporaryDirectory() as temp_dir:
            result = control.apply_control(
                action="restart",
    ...
    def test_restart_tolerates_transient_reenumeration_query_failure(self) -> None:
        before = camera()
        plan = control.build_plan("restart", before["instance_id"], query=lambda _: before, admin=True)
        observations: list[object] = [before, before, control.ControlError("temporarily absent"), before]
    ...
    def test_windows_only_control_checks_defer_from_wsl(self) -> None:
        with patch.object(control.os, "name", "posix"):
            doctor = control.doctor()
            validation = control.validate()

        for result in (doctor, validation):
            self.assertTrue(result["ok"])
