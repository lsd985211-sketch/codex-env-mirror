...
State behavior: isolated SQLite orchestration state; no worker is auto-started.
Caller context: the unified scheduler may invoke only expired-lease recovery;
future dispatch integration remains explicit and separately admitted.
"""
...
def claim_task(
    ...
    *,
    ...
    """Lease one exact task so a coordinator never consumes another owner's work."""

    if not task_id or not lease_owner:
        return {"ok": False, "reason": "task_id_and_lease_owner_required"}
    current = datetime.now(timezone.utc)
    now = current.isoformat()
    expires = (current + timedelta(seconds=max(1, int(lease_seconds)))).isoformat()
    conn = connect(db_path)
    try:
        ...
        if task.get("state") not in READY_STATES or str(task.get("next_attempt_at") or "") > now:
            conn.execute("COMMIT")
            return {"ok": False, "reason": "task_not_ready", "task": task}
        conn.execute(
            """UPDATE tasks SET state='leased',lease_owner=?,lease_expires_at=?,
               attempt_count=attempt_count+1,updated_at=? WHERE task_id=?""",
            (lease_owner, expires, now, task_id),
        )
        event(conn, task_id, "leased", {"lease_owner": lease_owner, "lease_expires_at": expires, "exact": True})
        task = fetch(conn, task_id)
        conn.execute("COMMIT")
        return {"ok": True, "empty": False, "task": task}
    except Exception:
...
def settle_durable_terminal(
    ...
    *,
    ...
    """Atomically consume a durable executor terminal receipt after transport loss.

    The durable executor has already fenced the command by operation and
    execution signature. This transition never launches work; it only settles
    an acknowledged/executing/recovery task from that terminal evidence.
    """
    ...
    try:
        ...
        if task.get("state") == "succeeded":
            conn.execute("COMMIT")
            return {"ok": True, "reused": True, "task": task}
        if task.get("state") not in {"queued", "acked", "executing", "recovery_required"}:
            conn.execute("COMMIT")
            return {"ok": False, "reason": "terminal_settlement_state_invalid", "task": task}
        payload = task.get("payload") if isinstance(task.get("payload"), dict) else {}
        if (
            str(result.get("operation_id") or "") != str(payload.get("operation_id") or "")
            or str(result.get("input_signature") or "") != str(payload.get("input_signature") or "")
            or str(result.get("execution_signature") or "") != str(payload.get("execution_signature") or "")
        ):
            conn.execute("COMMIT")
            return {"ok": False, "reason": "terminal_evidence_identity_mismatch", "task": task}
        now = datetime.now(timezone.utc)
        if success:
            state = "succeeded"
            next_attempt_at = ""
            ...
            exhausted = int(task.get("attempt_count") or 0) >= int(task.get("max_attempts") or 1)
            state = "dead_letter" if exhausted else "retry_wait"
            next_attempt_at = (
                ""
                if exhausted
                else (now + timedelta(seconds=int(task.get("retry_delay_seconds") or 60))).isoformat()
            )
            last_error = str(reason or "durable_command_failed")
            result_json = dump(result)
        conn.execute(
            """UPDATE tasks SET state=?,result_json=?,next_attempt_at=?,lease_owner='',
               lease_expires_at='',last_error=?,updated_at=? WHERE task_id=?""",
            (state, result_json, next_attempt_at, last_error, now.isoformat(), task_id),
        )
        event(
            conn,
            task_id,
            "durable_terminal_settled",
            {"state": state, "success": bool(success), "reason": last_error},
        )
        task = fetch(conn, task_id)
        conn.execute("COMMIT")
        return {"ok": True, "reused": False, "task": task}
    except Exception:
        if conn.in_transaction:
            conn.execute("ROLLBACK")
        raise
...
def validate(*, db_path: Path = DEFAULT_DB) -> dict[str, Any]:
    ...
    return {
        ...
        "db_path": str(db_path),
        "issues": issues,
        "contract": "sidecar only; scheduler may invoke expired-lease recovery, but no owner command, task execution, mail delivery, Codex thread, or worker autostart",
    }
...
def behavior_evaluation() -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="persistent-task-kernel-") as temp:
        ...
        replay_execution = begin(task["task_id"], lease_owner="worker-a", db_path=db_path)
        checks.append({"name": "terminal_task_not_reexecuted", "ok": not replay_execution.get("ok")})

        protected = enqueue(task_id="approval-1", idempotency_key="approval", task_type="fixture", target_module="fixture", action_type="protected", payload={}, priority=10, db_path=db_path)
        ...
        leased = enqueue(task_id="lease-1", idempotency_key="lease", task_type="fixture", target_module="fixture", action_type="noop", payload={}, priority=20, db_path=db_path)
        ...
        uncertain = enqueue(task_id="uncertain-1", idempotency_key="uncertain", task_type="fixture", target_module="fixture", action_type="external", payload={}, priority=30, db_path=db_path)
        ...
        dead = enqueue(task_id="dead-1", idempotency_key="dead", task_type="fixture", target_module="fixture", action_type="noop", payload={}, priority=40, max_attempts=1, db_path=db_path)
