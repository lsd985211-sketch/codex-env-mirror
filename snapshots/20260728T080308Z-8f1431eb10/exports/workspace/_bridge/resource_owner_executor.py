...
def execute_generic_search(request: dict[str, Any], gateway_plan: dict[str, Any], timeout: int) -> dict[str, Any]:
    ...
    search_lanes = bounded_domains or [""]
    tool_kind = {
        "image": "images",
        "news": "news",
        "video": "videos",
    ...
    return _normalized_json_result(
        ...
        status=status,
        source="generic_search",
        error_class="owner_no_results" if payloads and all(item.get("ok") for item in payloads) else "generic_search_failed",
