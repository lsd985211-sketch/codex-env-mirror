...
- For broad MCP usage, query `_bridge/mcp_capability_routes.py lookup` first,
  then read only the selected section of `_bridge/docs/mcp_capability_matrix.md`
  when its purpose, trigger, fallback, validation, or circuit breaker needs
  complete detail. The route index is navigation and must not replace or weaken
  the matrix contract. Do not optimize one MCP in isolation when the symptom is
  tool-layer wide.
