...
class CodexStartupChainTests(unittest.TestCase):
    def run_static_preflight(self, *, wsl_enabled: bool, projection_changed: bool) -> tuple[dict, mock.Mock]:
    ...
    def test_static_preflight_applies_wsl_projection_when_baseline_is_satisfied(self) -> None:
        result, ensure = self.run_static_preflight(wsl_enabled=True, projection_changed=True)
    ...
    def test_static_preflight_preserves_native_mode_when_wsl_is_disabled(self) -> None:
        result, ensure = self.run_static_preflight(wsl_enabled=False, projection_changed=False)
