...
python scripts/md_to_image.py --file /path/to/content.md

# 指定输出目录
python scripts/md_to_image.py --file /path/to/content.md --output-dir /path/to/output

# 调整每页条目数
python scripts/md_to_image.py --file /path/to/content.md --items-per-page 4
```
...
首次使用需安装：
```bash
python -m pip install playwright markdown requests
