...
- For long commands, submit the business command once and consume the native process or session handle until exit. If direct handle consumption is unavailable or unreliable, derive one stable intent ID, submit once through the durable long-command owner, and consume one quiet follow process through its native handle. Reuse matching running or terminal receipts, including terminal failure; reject a changed command, cwd, or timeout for the same intent. Do not resubmit the business command or loop over short status calls as polling. Compact output must retain terminal state, exit code, and a stable full-result reference.
