...
from local_mcp_hub_process import reload_local_hub  # noqa: E402
...
from codegraph_query_runtime import query_codegraph  # noqa: E402
...
class LocalMcpHub:
    ...
    def codegraph_explore(self, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        payload = query_codegraph(
            query,
            project_path=str(arguments.get("projectPath") or ROOT).strip(),
    ...
    def capabilities(self) -> dict[str, Any]:
        ...
        return {
            ...
            "hub_native": {
                ...
                "desktop_weixin": {
                    "mode": "same_safety_hub_native_and_fresh_stdio_fallback",
    ...
    def snapshot(self) -> dict[str, Any]:
        ...
        return {
            ...
            "tools": {
                ...
                "desktop_weixin": {"enabled": True, "mode": "same_safety_hub_native_and_fresh_stdio_fallback"},
                "github": {
                    "enabled": True,
                    "mode": "full_proxy_existing_permissions",
                    "tools": ["github.api", "github.gh"],
    ...
    def doctor(self) -> dict[str, Any]:
        ...
        sqlite_db = BRIDGE_ROOT / "data" / "sqlite" / "codex_scratch.sqlite"
        if not sqlite_db.exists():
            issues.append({"severity": "risk", "code": "sqlite_scratch_missing", "path": str(sqlite_db)})
        bridge_db = BRIDGE_ROOT / "mobile_openclaw_bridge" / "mobile_openclaw_bridge.db"
        ...
        "actions": [
            {
                ...
                "command": "powershell -NoProfile -ExecutionPolicy Bypass -File _bridge\\shared\\run-local-mcp-hub.ps1",
                "writes_files": False,
