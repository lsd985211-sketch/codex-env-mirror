...
def backup_baseline() -> Path:
    ...
    backup_dir = BACKUP_ROOT / f"{stamp}-codex-baseline-update"
...
def plugin_enabled_names(config: dict, existing_baseline: dict) -> list[str]:
    names = set(existing_baseline.get("expected_plugins", []))
...
def required_values_from_config(global_config: dict, project_config: dict, workspace: str) -> tuple[dict, dict]:
    ...
    global_values: dict[str, object] = dict(baseline.get("global_required_values", {}))
    ...
    if isinstance(projects, dict):
        project = projects.get(workspace.lower())
        if isinstance(project, dict) and "trust_level" in project:
            global_values[f"projects.{workspace.lower()}.trust_level"] = project["trust_level"]
...
def build_updated_baseline(reason: str) -> tuple[dict, dict]:
    ...
    updated["project_required_values"] = project_values
