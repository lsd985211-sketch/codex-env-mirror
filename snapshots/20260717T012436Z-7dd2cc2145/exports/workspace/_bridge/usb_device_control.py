...
POLICY_VERSION = "2026-07-17.phase2"
...
def apply_control(
    *,
    ...
    with mutex_factory():
        ...
        after = {} if action == "rescan" else _safe_query(query, str(before.get("instance_id") or instance_id))
        if action == "rescan":
        ...
        if action in {"restart", "disable"} and _healthy(before) and not accepted and _owner_disabled(after):
            try:
            ...
            recovery_after = _safe_query(query, str(before.get("instance_id") or instance_id))
            recovery = {
                "attempted": True,
    ...
    with mutex_factory():
        ...
        if _healthy(current):
            ...
            try:
                ...
                execution = _execution_error(exc)
            after = _safe_query(query, instance_id)
