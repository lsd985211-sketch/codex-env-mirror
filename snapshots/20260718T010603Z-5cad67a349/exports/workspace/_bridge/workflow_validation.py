...
def core_validation_checks(
    ...
    # A degraded doctor with only advisory/risk rows remains usable for routing.
    doctor_status = str(doc.get("status") or "")
    doctor_issues = doc.get("issues") if isinstance(doc.get("issues"), list) else []
    doctor_blockers = [
        item for item in doctor_issues
        if isinstance(item, dict) and str(item.get("severity") or "").lower() == "blocker"
    ]
    return [
        {
            "name": "doctor_ok",
            "ok": bool(doc.get("ok")) or (doctor_status == "degraded" and not doctor_blockers),
            "detail": {
                "status": doctor_status,
                "advisory_or_risk_count": len(doctor_issues) - len(doctor_blockers),
                "blocker_count": len(doctor_blockers),
            },
        },
        {
