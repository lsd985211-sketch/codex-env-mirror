...
def control_plane_status() -> dict[str, Any]:
    ...
    milestone = payload.get("milestone") if isinstance(payload.get("milestone"), dict) else {}
    return {
        "ok": str(snapshot.get("snapshot_id") or "") == latest_snapshot_id(),
...
def release_plan() -> dict[str, Any]:
    ...
    elif any(path == "SECURITY.md" or path.startswith("manifests/schemas/") for path in non_snapshot):
        bump = "major"
        reason = "restore_schema_or_security_boundary_changed"
...
def contract_review(confirm: str, *, decisions: list[str], summary: str = "", remote: str = "", branch: str = "") -> dict[str, Any]:
    ...
    *,
    ...
    try:
        with exclusive_operation_lock(lock_path, "release"):
            ...
            full_owner = run_mirror(["validate", "--live-sources", "--snapshot", snapshot_id], timeout=300)
            validation = validation_receipt(full_owner)
            ...
            if local_tag.get("ok"):
                ...
                tag_created = True
            remote_tag = git_result(["ls-remote", "--tags", remote_name, f"refs/tags/{tag}^{{}}"], extra_env=network_env)
            remote_tag_sha = str(remote_tag.get("stdout") or "").split()[0] if remote_tag.get("stdout") else ""
            ...
            view = gh_result(["release", "view", tag, "--json", "tagName,isDraft,url,targetCommitish,name"], extra_env=network_env)
            ...
            if not view.get("ok"):
                created = gh_result([
                    "release", "create", tag, f"{manifest_path}#snapshot-manifest.json",
                    "--title", title or f"Codex Environment Mirror {tag}",
                    "--notes-file", str(notes_path), "--draft", "--verify-tag",
                ], timeout=300, extra_env=network_env)
                if not created.get("ok"):
            ...
            verified = gh_result(["release", "view", tag, "--json", "tagName,isDraft,url,targetCommitish,name"], extra_env=network_env)
            try:
                release_view = json.loads(str(verified.get("stdout") or "{}"))
            except json.JSONDecodeError:
                release_view = {}
            if not verified.get("ok") or release_view.get("tagName") != tag or release_view.get("isDraft") is not False:
                return {
                    "schema": "codex_environment_mirror.release.v1",
                    "ok": False,
...
def execute(action: str, *, target_root: str = "", confirm: str = "", changed_paths: list[str] | None = None, left_snapshot: str = "", right_snapshot: str = "", remote: str = "", branch: str = "", tag: str = "", title: str = "") -> dict[str, Any]:
    if action == "status":
        return status()
    if action == "doctor":
        return doctor()
    ...
    if action == "contract-review":
        return contract_review(confirm, decisions=changed_paths or [], summary=title, remote=remote, branch=branch)
    if action == "release":
        return release(confirm, tag=tag, title=title, remote=remote, branch=branch)
