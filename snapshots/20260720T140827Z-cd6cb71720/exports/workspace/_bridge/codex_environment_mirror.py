...
def mcp_bundle_readiness() -> dict[str, Any]:
    ...
    try:
        ...
        from mcp_recovery_bundle_owner import DEFAULT_MANIFEST, DEFAULT_ARCHIVE_ROOT
    ...
    *,
    ...
    try:
        with exclusive_operation_lock(lock_path, "release"):
            ...
            release_files.extend(f"{item['path']}#{item['name']}" for item in bundle_assets.get("assets", []))
            if not view.get("ok"):
                created = gh_result([
                ...
                if missing_files:
                    uploaded = gh_result(["release", "upload", tag, *missing_files, "--clobber"], timeout=1200, extra_env=network_env)
            ...
            if (
                ...
                or not release_has_required_attachments(
                    release_view,
                    {"mcp-bundle-index.json", *(item["name"] for item in bundle_assets.get("assets", []))},
                )
            ):
                return {
                    "schema": "codex_environment_mirror.release.v1",
