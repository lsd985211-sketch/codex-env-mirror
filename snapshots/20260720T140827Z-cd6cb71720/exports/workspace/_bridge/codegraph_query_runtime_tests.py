...
class CodeGraphQueryRuntimeTests(unittest.TestCase):
    ...
    def test_windows_hub_resolves_nested_work_git_workspace_bridge(self) -> None:
        project = Path(r"\\wsl.localhost\Codex-Wsl-Lab\home\codexlab\work\codex-workspace")
        workspace = Path(str(project) + r"\workspace")
        with mock.patch.object(runtime, "ROOT", workspace):
            bridge = runtime.codegraph_wsl_bridge_root(project)
            prefix = runtime.codegraph_command_prefix(project, platform_name="nt")
        self.assertEqual(bridge, "/home/codexlab/work/codex-workspace/workspace/_bridge")
        self.assertTrue(prefix[-1].endswith("/workspace/_bridge/tools/codegraph/node_modules/.bin/codegraph"))

    def test_repo_root_selects_nested_workspace_only_when_it_owns_the_index(self) -> None:
        repository = self.base / "repository"
        nested = repository / "workspace"
        (nested / "_bridge").mkdir(parents=True)
        create_index(nested)
        self.assertEqual(runtime.resolve_codegraph_project_root(repository), nested.resolve())

        create_index(repository)
        self.assertEqual(runtime.resolve_codegraph_project_root(repository), repository.resolve())
    ...
    def test_windows_repo_root_index_inspection_uses_nested_runtime(self) -> None:
        project = Path(r"\\wsl.localhost\Codex-Wsl-Lab\home\codexlab\work\codex-workspace")
        workspace = Path(str(project) + r"\workspace")
        completed = SimpleNamespace(
            returncode=0,
            stdout=json.dumps({"schema": "codegraph_query_runtime.index.v1", "ok": True}),
            stderr="",
        )
        with mock.patch.object(runtime, "ROOT", workspace), mock.patch.object(
            runtime.subprocess, "run", return_value=completed
        ) as runner:
            result = runtime.inspect_index_usability(project, platform_name="nt")
        self.assertTrue(result["ok"])
        command = runner.call_args.args[0]
        self.assertIn("/home/codexlab/work/codex-workspace/workspace/_bridge/codegraph_query_runtime.py", command)
        self.assertEqual(command[-1], "/home/codexlab/work/codex-workspace")
