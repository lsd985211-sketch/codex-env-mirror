...
Use only the minimum questions needed to route the task:
- What type of content are you trying to create? (article, post, reflection, tutorial, proposal, script)
- What stage is the input at? (none, scattered ideas, outline, draft, source material)
- What is the output target? (platform, audience, length, tone, purpose)
- What is the main bottleneck? (start, structure, completion, revision, repurposing, synthesis)

If the user already provided enough context, skip questions and choose the method directly.

Decision rule:
- Use conversational writing when the user has lived experience but no structure.
- Use outline-based writing when the user knows what to say but needs expansion.
- Use completion-style writing when the structure exists but the draft is thin.
- Use inspiration generation when the user needs angles or topics.
- Use revision optimization when the draft exists but quality is weak.
- Use material integration when the user has multiple sources to synthesize.
...
- Confirm the target platform or default to the user's stated context.
- Confirm audience and purpose if they are missing.
- If the user wants repurposing, decide the target format before writing.
- If the user has a source article, extract the core claims first and then write.
- Prefer one clear method over blending multiple methods unless the user asks for a hybrid.
