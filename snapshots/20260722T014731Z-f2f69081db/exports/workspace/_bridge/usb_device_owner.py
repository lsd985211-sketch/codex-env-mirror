...
USBIPD_ROW_RE = re.compile(
    r"^\s*(?P<busid>\d+-\d+(?:\.\d+)?)\s+(?P<vid>[0-9a-f]{4}):(?P<pid>[0-9a-f]{4})\s+"
    r"(?P<device>.*?)\s{2,}(?P<state>Not shared|Shared|Attached)\s*$",
    re.IGNORECASE,
)
USBIPD_BUSID_RE = re.compile(r"^\d+-\d+(?:\.\d+)?$")
...
def _storage_script(drive_letter: str) -> str:
    return rf"""
$ErrorActionPreference='Stop'
[Console]::OutputEncoding=[System.Text.UTF8Encoding]::new($false)
$drive='{drive_letter}'
...
if([string]$disk.BusType -ne 'USB'){{throw "drive_not_backed_by_usb_disk:$drive"}}
$volume=Get-Volume -DriveLetter $drive -ErrorAction Stop
$cimDisk=Get-CimInstance Win32_DiskDrive -Filter "Index = $($disk.Number)" -ErrorAction SilentlyContinue
[pscustomobject]@{{
  captured_at=(Get-Date).ToUniversalTime().ToString('o')
  disk=[pscustomobject]@{{
    number=[int]$disk.Number
    friendly_name=[string]$disk.FriendlyName
    model=[string]$cimDisk.Model
    serial_number=([string]$disk.SerialNumber).Trim()
    pnp_device_id=[string]$cimDisk.PNPDeviceID
...
def deferred_platform_payload(action: str) -> dict[str, Any]:
    """Keep a Windows-only owner from probing WSL devices or dependencies."""
    ...
    return {
        ...
        "diagnostics": {
            ...
            "next_action": "run this owner validator or operation on the Windows host",
        },
    }
...
def usbipd_path() -> Path:
    """Return the fixed usbipd-win location without accepting caller input."""

    program_files = os.environ.get("ProgramW6432") or os.environ.get("ProgramFiles") or r"C:\\Program Files"
    return Path(program_files) / "usbipd-win" / "usbipd.exe"


def _normalize_usbipd_state(value: str) -> str:
    state = str(value or "").casefold()
    if state == "attached":
        return "wsl_attached"
    if state == "shared":
        return "shared_waiting_for_wsl"
    return "windows_available"


def _usbipd_handoff(item: dict[str, str]) -> dict[str, str]:
    transport_state = _normalize_usbipd_state(item.get("state", ""))
    if transport_state == "wsl_attached":
        return {
            "transport_state": transport_state,
            "windows_mtp_access": "unavailable_while_attached",
            "wsl_usb_access": "available",
            "next_action": "coordinate_before_detaching_or_rebinding_the_exact_busid",
        }
    if transport_state == "shared_waiting_for_wsl":
        return {
            "transport_state": transport_state,
            "windows_mtp_access": "may_be_in_use_until_wsl_attach",
            "wsl_usb_access": "not_attached",
            "next_action": "check_windows_processes_and_coordinate_before_any_usbipd_attach",
        }
    return {
        "transport_state": transport_state,
        "windows_mtp_access": "available_to_windows",
        "wsl_usb_access": "not_attached",
        "next_action": "preserve_windows_binding_unless_an_explicit_wsl_transfer_is_requested",
    }


def usbip_transport(*, busid: str = "") -> dict[str, Any]:
    """Report usbipd-win transport ownership without changing device state."""

    requested_busid = str(busid or "").strip()
    if requested_busid and not USBIPD_BUSID_RE.fullmatch(requested_busid):
        raise ValueError("busid must use the bounded USB/IP form <bus>-<port>")
    executable = usbipd_path()
    if not executable.is_file():
        return {
            "schema": f"{SCHEMA}.transport",
            "ok": False,
            "read_only": True,
            "available": False,
            "reason": "usbipd_win_not_installed",
            "next_action": "install_or_repair_usbipd_win_before_requesting_wsl_usb_transfer",
        }

    rows: list[dict[str, Any]] = []
    for line in _run_fixed([str(executable), "list"], timeout=15).splitlines():
        match = USBIPD_ROW_RE.match(line)
        if not match:
            continue
        row = {key: value.strip() for key, value in match.groupdict().items()}
        if requested_busid and row["busid"] != requested_busid:
            continue
        rows.append({**row, **_usbipd_handoff(row)})
    return {
        "schema": f"{SCHEMA}.transport",
        "ok": True,
        "read_only": True,
        "available": True,
        "captured_at": now_iso(),
        "requested_busid": requested_busid,
        "device_count": len(rows),
        "devices": rows,
        "state_model": {
            "windows_available": "Windows owns the device; WSL USB/IP is not attached.",
            "shared_waiting_for_wsl": "The device can be transferred, but an in-use Windows MTP session can still block attachment.",
            "wsl_attached": "WSL owns the USB transport; Windows MTP access is unavailable until explicit detach.",
        },
        "mutation_boundary": "usbipd bind, attach, and detach are not exposed by this read-only owner",
    }
...
def normalize_drive_letter(value: str) -> str:
    ...
    if not re.fullmatch(r"[A-Z]", drive):
        raise ValueError("drive_letter_must_be_single_ascii_letter")
    return drive


def storage_identity_fingerprint(disk: dict[str, Any], partition: dict[str, Any]) -> str:
    identity = {
        "friendly_name": str(disk.get("friendly_name") or "").strip(),
        "model": str(disk.get("model") or "").strip(),
        "serial_number": str(disk.get("serial_number") or "").strip(),
        "pnp_device_id": str(disk.get("pnp_device_id") or "").strip(),
        "size_bytes": int(disk.get("size_bytes") or 0),
        "partition_number": int(partition.get("partition_number") or 0),
        "partition_size_bytes": int(partition.get("size_bytes") or 0),
    }
    encoded = json.dumps(identity, ensure_ascii=False, sort_keys=True, separators=(",", ":")).encode("utf-8")
    return hashlib.sha256(encoded).hexdigest()


def storage_binding(drive_letter: str) -> dict[str, Any]:
    drive = normalize_drive_letter(drive_letter)
    raw = _run_powershell(_storage_script(drive), timeout=20)
    if not isinstance(raw, dict):
        raise ValueError("storage result must be a JSON object")
    disk = raw.get("disk") if isinstance(raw.get("disk"), dict) else {}
    partition = raw.get("partition") if isinstance(raw.get("partition"), dict) else {}
    volume = raw.get("volume") if isinstance(raw.get("volume"), dict) else {}
    issues: list[dict[str, Any]] = []
    operational = {str(item).casefold() for item in _listify(disk.get("operational_status"))}
    volume_operational = {str(item).casefold() for item in _listify(volume.get("operational_status"))}
    checks = (
        (str(disk.get("bus_type") or "").casefold() == "usb", "storage_not_usb"),
        (not bool(disk.get("is_offline")), "storage_offline"),
        (not bool(disk.get("is_read_only")), "storage_read_only"),
        (str(disk.get("health_status") or "").casefold() == "healthy", "storage_unhealthy"),
        ("online" in operational, "storage_not_online"),
        (str(volume.get("health_status") or "").casefold() == "healthy", "volume_unhealthy"),
        (not volume_operational or bool(volume_operational & {"ok", "healthy"}), "volume_not_operational"),
        (str(partition.get("drive_letter") or "").upper() == drive, "drive_letter_mismatch"),
    )
    for ok, code in checks:
        if not ok:
            issues.append({"code": code})
    fingerprint = storage_identity_fingerprint(disk, partition)
    return {
        "schema": f"{SCHEMA}.storage",
        "ok": True,
        "read_only": True,
        "captured_at": str(raw.get("captured_at") or now_iso()),
        "drive_letter": drive,
        "stable_fingerprint": fingerprint,
        "safe_for_content_mutation": not issues,
        "issues": issues,
        "disk": disk,
        "partition": partition,
        "volume": volume,
        "handoff": {
            "consumer_rule": "downstream content owners may bind plans to stable_fingerprint but do not inherit device-control permission",
...
def validate() -> dict[str, Any]:
    ...
    checks.append(
        {
            "name": "storage_handoff_contract",
            "ok": "powershell_storage" in ALLOWED_EXTERNAL_ACTIONS,
            "read_only": True,
            "identity_fields": ["serial_number", "pnp_device_id", "size_bytes", "partition_number"],
        }
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    for action in ("snapshot", "doctor", "android", "transport", "validate"):
        command = sub.add_parser(action)
        command.add_argument("--full", action="store_true")
        if action == "transport":
            command.add_argument("--busid", default="")
    storage = sub.add_parser("storage")
    storage.add_argument("--drive-letter", required=True)
    storage.add_argument("--full", action="store_true")
...
def main() -> int:
    ...
    try:
        if os.name != "nt":
            payload = deferred_platform_payload(args.action)
        elif args.action == "snapshot":
            payload = collect_snapshot()
        elif args.action == "storage":
            payload = storage_binding(args.drive_letter)
            ...
            payload = usbip_transport(busid=args.busid)
        else:
            payload = validate()
    except Exception as exc:
        payload = {
