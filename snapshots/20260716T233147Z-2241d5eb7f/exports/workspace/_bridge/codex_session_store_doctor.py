...
try:
    ...
    from shared.process_liveness import process_is_alive
    ...
    from _bridge.shared.process_liveness import process_is_alive
...
def auto_maintain_lock() -> Any:
    ...
    for attempt in range(2):
        try:
            ...
            if attempt == 0 and age_seconds >= AUTO_MAINTAIN_LOCK_STALE_SECONDS and not observed_owner_alive:
                try:
...
def compact_file_plan(
    ...
    *,
    ...
    return {
        ...
        "recovery_summary_valid": recovery_summary_invalid_count == 0,
...
def compact_file_apply(
    ...
    *,
    ...
    if not bool(plan.get("recovery_summary_valid", True)):
        return {
            "ok": False,
            "applied": False,
        ...
        with path.open("r", encoding="utf-8", errors="replace") as src, temp_path.open(
            ...
            for line in src:
                ...
                try:
                    ...
                    dst.write(line)
                    continue
                compacted, _saved, reason = compact_json_object(obj, keep_chars=keep_chars)
                if compacted is None:
                    continue
                if reason == "recovery_summary_validation_failed":
                    raise ValueError("recovery_summary_validation_failed")
                dst.write(json.dumps(compacted, ensure_ascii=False, separators=(",", ":")) + "\n")
        with temp_path.open("r", encoding="utf-8") as check:
            for line in check:
                obj = json.loads(line)
                if obj.get("type") != "compacted":
                    continue
                validation = validate_compacted_record(obj)
                if not validation.get("ok"):
                    raise ValueError("recovery_summary_validation_failed")
        after_integrity = recovery_summary_integrity(temp_path)
        if (
            not after_integrity.get("ok")
            or after_integrity.get("summary_fingerprint") != plan.get("recovery_summary_fingerprint")
            or after_integrity.get("core_fingerprint") != plan.get("compacted_core_fingerprint")
        ):
